...
NODE_ONLY_MODULES = {"tests/runtime/test_army_train_logistics.py", "tests/runtime/test_causal_connections.py"}
NODE_PARALLELISM = 4
...
def _pytest(target: str, *, timeout_seconds: int, label: str) -> int:
    ...
    # Basetemps are isolated by module/node label and retained for the current
    # certification batch. Deleting an entire Git-backed fixture tree after each
    # tiny module costs more than the tests themselves. The suite removes the
    # shared temp root once certification completes; interrupted batches can
    # safely discard the temp root before resuming from the checkpoint.

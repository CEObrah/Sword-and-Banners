...
TMP_BASE = Path("/tmp/sword-release-cert")
LOCK_PATH = Path("/tmp/sword-release-cert.lock")
RUN_TOKEN = str(__import__("os").getpid())
# Every certifier invocation owns a distinct disposable root. If an externally
# killed parent leaves a pytest child alive, a later resumable invocation must
# never delete that child's basetemp out from under it.
TMP_ROOT = TMP_BASE / RUN_TOKEN
MODULE_TIMEOUT_SECONDS = 70
NODE_TIMEOUT_SECONDS = 90
NODE_ONLY_MODULES = {"tests/runtime/test_army_train_logistics.py", "tests/runtime/test_causal_connections.py", "tests/runtime/test_hosted_horizon_performance.py", "tests/runtime/test_long_horizon.py", "tests/runtime/test_personal_combat_multi_actor.py", "tests/runtime/test_real_campaign_acceptance.py", "tests/runtime/test_rules_parity_adversarial.py", "tests/runtime/test_warfare.py"}
NATIVE_TEMP_MODULES = {"tests/runtime/test_command_staff_continuity.py"}
NODE_PARALLELISM = 2
MODULE_PARALLELISM = 2
SERIAL_NODE_MODULES = {"tests/runtime/test_hosted_horizon_performance.py", "tests/runtime/test_long_horizon.py"}
SERIAL_MODULES = {"tests/runtime/test_living_world_intelligence.py"}
...
def _pytest_native_temp(target: str, *, timeout_seconds: int) -> int:
    cmd = [
        "timeout", "-k", "5s", f"{timeout_seconds}s",
        sys.executable, "tools/run_pytest_module.py", "-q", target,
    ]
    print("release_gate: native-temp", target, flush=True)
    result = subprocess.run(cmd, cwd=ROOT)
    return int(result.returncode)
...
def _certify_nodes(module: str, checkpoint: dict[str, object]) -> None:
    ...
    module_serial = module in SERIAL_NODE_MODULES
    serial = [(idx, node) for idx, node in pending if module_serial or node in SERIAL_NODE_TIMEOUTS]
    parallel = [(idx, node) for idx, node in pending if not module_serial and node not in SERIAL_NODE_TIMEOUTS]
    ...
    for idx, node in serial:
        ...
        code = _pytest(node, timeout_seconds=timeout_seconds, label=f"serial-node-{module_token}-{idx}")
        if code != 0:
        ...
        with ThreadPoolExecutor(max_workers=len(batch)) as pool:
            ...
            for future in as_completed(futures):
                ...
                if code != 0:
                    ...
                    continue
                # Persist each completed node immediately. An external executor
                # cutoff after its sibling finishes must not erase green work.
                already.add(node)
                node_map[module] = sorted(already)
                _save_checkpoint(checkpoint)
        if failed:
...
def _run_structural_validator() -> None:
    print("release_gate: tools/validate_release.py", flush=True)
    subprocess.run([sys.executable, "tools/validate_release.py"], cwd=ROOT, check=True)


def _run_release_state_guard() -> None:
    print("release_gate: tools/verify_release_state.py", flush=True)
    subprocess.run([sys.executable, "tools/verify_release_state.py"], cwd=ROOT, check=True)


def _run_skill_contract_guard() -> None:
    print("release_gate: tools/sync_gm_skill_contract.py --check", flush=True)
    subprocess.run([sys.executable, "tools/sync_gm_skill_contract.py", "--check"], cwd=ROOT, check=True)


def _run_mutation_audit() -> None:
    print("release_gate: tools/mutation_audit.py", flush=True)
    subprocess.run([sys.executable, "tools/mutation_audit.py"], cwd=ROOT, check=True)


def _run_playability_lab() -> None:
    print("release_gate: tools/playability_lab.py --no-mutations", flush=True)
    subprocess.run(
        [sys.executable, "tools/playability_lab.py", "--no-mutations"],
        cwd=ROOT,
        check=True,
    )
...
def main() -> int:
    ...
    # resumed run discards only this invocation's disposable root. Older orphaned
    # children may still be finishing after an external timeout and must retain
    # their own run-scoped basetemps until they exit.
    _clean_path(TMP_ROOT)
    _ensure_tmp_root()
    modules = sorted(
        [
            *(ROOT / "tests/runtime").glob("test_*.py"),
    ...
    if checkpoint.get("quick_check_passed") is not True:
        ...
        _run_release_state_guard()
        _run_skill_contract_guard()
        _run_playability_lab()
        _run_mutation_audit()
    ...
    def flush_normal_batch() -> None:
        ...
        with ThreadPoolExecutor(max_workers=len(batch)) as pool:
            futures = {
                ...
                for index, module in batch
            }
            for future in as_completed(futures):
                module = futures[future]
                code = int(future.result())
                if code == 0:
                    # Checkpoint each successful module as soon as it completes.
                    # A host/tool timeout between parallel siblings no longer
                    # forces already-green modules to rerun on resume.
                    mark_passed(module)
                elif code in TIMEOUT_CODES:
                    timed_out.append(module)
                else:
                    failed.append((module, code))
        if failed:
            module, code = failed[0]
            raise subprocess.CalledProcessError(code, module)
        for module in timed_out:
    ...
    for index, path in enumerate(modules, start=1):
        ...
        if module in passed:
            ...
            continue
        if module in NATIVE_TEMP_MODULES:
            flush_normal_batch()
            print(f"release_gate: certify [{index}/{len(modules)}] {module}", flush=True)
            code = _pytest_native_temp(module, timeout_seconds=max(MODULE_TIMEOUT_SECONDS, 90))
            if code != 0:
                raise subprocess.CalledProcessError(code, module)
            mark_passed(module)
            continue

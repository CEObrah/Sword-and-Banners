...
def validate(root: Path | None = None) -> list[str]:
    ...
    top_roots = {f"state_{s}" for s in STATES} | {"polity_quanrong", "polity_yotanwa_confederation", "polity_northern_steppe", "polity_jo"}
    for ref, row in loc_by_ref.items():
    ...
    for fort_ref, route_or_row in fort_index.get("fortifications", {}).items():
        if isinstance(route_or_row, str):
            try:
                row = load(route_or_row)
            except (OSError, ValueError, json.JSONDecodeError):
                errors.append(f"materialized fortification route does not resolve: {fort_ref} -> {route_or_row}")
                continue
        elif isinstance(route_or_row, dict):
            # Compatibility with older inline-index snapshots. The index is still
            # routing only; validation is against the exact row shape.
            row = route_or_row
        else:
            errors.append(f"materialized fortification route is invalid: {fort_ref}")
            continue
        if str(row.get("fortification_ref", row.get("owner_id", ""))) not in {"", str(fort_ref)}:
            errors.append(f"materialized fortification identity mismatch: {fort_ref}")
        location_ref = row.get("location_ref") or row.get("site_ref")
        if not isinstance(location_ref, str) or location_ref not in loc_by_ref:
            errors.append(f"materialized fortification has invalid site: {fort_ref} -> {location_ref}")
    ...
    # real enclosing fortified ancestor, but that ancestor may be Inner Walls or
    ...
    for state in STATES:
        ...
        current_local_total = 0
        for ref, row in sites.items():
            civilian_strata = row.get("civilian_strata", {}) if isinstance(row, dict) else {}
            civilian_current = sum(max(0, int(v)) for v in civilian_strata.values()) if isinstance(civilian_strata, dict) else 0
            allocations = row.get("service_allocations", {}) if isinstance(row, dict) else {}
            service_current = sum(
                max(0, int(v.get("personnel", 0)))
                for v in allocations.values()
                if isinstance(v, dict)
            ) if isinstance(allocations, dict) else 0
            reservations = row.get("candidate_reservations", {}) if isinstance(row, dict) else {}
            reserved_current = sum(
                sum(max(0, int(n)) for n in (v.get("source_strata", {}) or {}).values())
                for v in reservations.values()
                if isinstance(v, dict) and isinstance(v.get("source_strata", {}), dict)
            ) if isinstance(reservations, dict) else 0
            if civilian_current != int(row.get("civilian_population", civilian_current)):
                errors.append(f"{state} local civilian mirror mismatch: {ref}")
            if service_current != int(row.get("service_population", service_current)):
                errors.append(f"{state} local service mirror mismatch: {ref}")
            if reserved_current != int(row.get("candidates_reserved", reserved_current)):
                errors.append(f"{state} local candidate-reservation mirror mismatch: {ref}")
            if max(0, int(civilian_strata.get("agricultural", 0))) != int(row.get("agricultural_available", 0)):
                errors.append(f"{state} local agricultural mirror mismatch: {ref}")
            current_local_total += civilian_current + service_current + reserved_current + max(0, int(row.get("displaced", 0)))
        if current_local_total != int(pop.get("population_total", current_local_total)):
            errors.append(f"{state} current local population partition mismatch: {current_local_total}!={pop.get('population_total', current_local_total)}")
    ...
    # House Tang has one exact military manpower owner. Outer Wall, Inner Walls,
    # and Inner Citadel are assignments/command sectors, never parallel force owners.
    owner_index = load("state/index/owner-index.json").get("owners", {})
    house_route = owner_index.get("force_house_tang")
    if not house_route or not (ROOT / str(house_route).split("#", 1)[0]).is_file():
        errors.append("unified House Tang force owner does not resolve")
    else:
        house_force = load(str(house_route).split("#", 1)[0])
        expected_roles = {"house_infantry": 164060, "house_cavalry": 12000}
        if int(house_force.get("headcount", 0)) != 176060:
            errors.append(f"House Tang headcount {house_force.get('headcount')} != 176060")
        if int(house_force.get("authorized_strength", 0)) != 176060:
            errors.append("House Tang authorized strength is not 176060")
        if house_force.get("authorized_by_role") != expected_roles:
            errors.append(f"House Tang authorized roles are not the two-species establishment: {house_force.get('authorized_by_role')}")
    retired_force_ids = {
    ...
    for force_id in sorted(retired_force_ids):
        if force_id in owner_index:
            errors.append(f"retired House Tang parallel force owner is still routed: {force_id}")
    for path in sorted((ROOT / "state/forces").glob("*.json")):
        force = json.loads(path.read_text(encoding="utf-8"))
        if force.get("owner_id") in retired_force_ids:
            errors.append(f"retired House Tang parallel force file still exists: {path.name}")
    allowed_house_roles = {"house_infantry", "house_cavalry"}
    for path in sorted((ROOT / "state/formations").glob("*.json")):
        formation = json.loads(path.read_text(encoding="utf-8"))
        if formation.get("owner_force_ref") != "force_house_tang":
            continue
        composition = formation.get("composition", {}) if isinstance(formation.get("composition"), dict) else {}
        illegal = {str(role) for role, count in composition.items() if int(count) > 0} - allowed_house_roles
        if illegal:
            errors.append(f"{formation.get('formation_ref')} has retired House Tang troop species: {sorted(illegal)}")
    ...
    # Aggregate capability belongs to the conserved troop/cohort owner itself.
    # A parallel manpower-capability state tree is forbidden because it can drift
    # from the exact pool count and current development state.
    cap_dir = ROOT / "state/manpower-capability"
    if cap_dir.exists() and any(cap_dir.glob("*.json")):
        errors.append("parallel state/manpower-capability owners must not exist")

    # Military owner validity is derived directly from current authoritative owners.
    # Do not persist a generated classification index in hot state merely so a
    # validator can restate what the schemas/paths already prove.
    for path in sorted((ROOT / "state/forces").glob("*.json")):
        force = json.loads(path.read_text(encoding="utf-8"))
        if not force.get("owner_id") or int(force.get("headcount", 0)) < 0:
            errors.append(f"invalid exact force owner: {path.name}")
    for path in sorted((ROOT / "state/formations").glob("*.json")):
        formation = json.loads(path.read_text(encoding="utf-8"))
        if not formation.get("formation_ref") or not formation.get("owner_force_ref"):
            errors.append(f"invalid persistent formation owner: {path.name}")
    for path in sorted((ROOT / "state/merc").rglob("*.json")):
        data = json.loads(path.read_text(encoding="utf-8"))
        owner_id = data.get("owner_id")
        if owner_id and owner_id not in owner_index:
            errors.append(f"mercenary owner is not routed: {path.relative_to(ROOT).as_posix()}:{owner_id}")
    ...
    for path in sorted((ROOT / "state/institutions").glob("*.json")):
        ...
        if doc.get("schema") == "regional-institution-index" and isinstance(doc.get("records"), dict):
            institutions = list(doc["records"].values())
        else:
            institutions = [doc]
    ...
    # Living minor polities use the same conserved population/force/mount/economy authority as states.
    minor = load("game/data/world/minor-polities.json")
    living_minor = {
        "minor_polity_quanrong": ("polity_quanrong", "population_quanrong", "force_quanrong", "quanrong"),
        "minor_polity_yotanwa_confederation": ("polity_yotanwa_confederation", "population_yotanwa_confederation", "force_yotanwa_confederation", "yotanwa_confederation"),
        "minor_polity_northern_steppe": ("polity_northern_steppe", "population_northern_steppe", "force_northern_steppe", "northern_steppe"),
        "minor_polity_jo": ("polity_jo", "population_jo", "force_jo", "jo"),
    }
    for cold_ref, (polity_ref, pop_ref, force_ref, state_key) in living_minor.items():
        row = minor.get("polities", {}).get(cold_ref, {})
        for key, expected in (("polity_ref", polity_ref), ("exact_population_ref", pop_ref), ("exact_force_ref", force_ref)):
            if row.get(key) != expected:
                errors.append(f"{cold_ref} living minor-polity authority missing {key}: {expected}")
        try:
            pop = load(f"state/population/{state_key}.json")
            force_path = owner_index.get(force_ref)
            force = load(force_path) if isinstance(force_path, str) else {}
            active = int(pop.get("strata", {}).get("active_military", 0))
            if int(force.get("headcount", -1)) != active:
                errors.append(f"{cold_ref} exact force does not reconcile to active_military population")
            if sum(int(v) for v in pop.get("strata", {}).values()) != int(pop.get("population_total", -1)):
                errors.append(f"{cold_ref} exact population strata do not conserve population_total")
        except Exception as exc:
            errors.append(f"{cold_ref} exact authority failed to load: {exc}")

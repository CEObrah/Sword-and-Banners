...
COHORT_TESTS = {"tests/runtime/test_exact_aggregate_conservation.py", "tests/runtime/test_production_exceptional_progression.py", "tests/runtime/test_runtime_invariants.py", "tests/runtime/test_combat_cohort_integration.py", "tests/runtime/test_military_logistics.py"}
PERSON_TESTS = {"tests/runtime/test_exact_aggregate_conservation.py", "tests/runtime/test_character_progression_schema.py", "tests/runtime/test_runtime_invariants.py"}
...
SCHEDULER_TESTS = {"tests/runtime/test_scheduler_frontier.py", "tests/runtime/test_production_living_world.py", "tests/runtime/test_long_horizon.py"}
...
def select(paths: list[str]) -> list[str]:
    ...
    for raw in paths:
        ...
        if (
            ...
            or path == "state/index/geography-index.json"
        ...
        if path in {"runtime/sword_runtime/scheduler_frontier.py", "state/runtime.json", "tools/migrate_scheduler_frontier.py"}:
        ...
        if (
            ...
            or path.startswith("state/economy/merchant-houses.json")
            or path.startswith("state/contract/tang-supply-contracts.json")
            or path.startswith("game/schemas/sword-merchant-house-registry.schema.json")
        ...
        if (
            ...
            or path.startswith("runtime/sword_runtime/house_tang_production.py")
            or path.startswith("game/data/mechanics/house-tang-production.json")
        ...
        if (
            ...
            or path.startswith("runtime/sword_runtime/smart_training.py")
        ...
        if (
            ...
            or path.startswith("tools/repair_progression_integrity.py")
            or path.startswith("tools/validate_release.py")
        ):
...
def main(argv: list[str]) -> int:
    ...
    for test_path in tests:
        ...
        subprocess.run(
            [sys.executable, "tools/run_pytest_module.py", "-q", test_path],
            cwd=ROOT,
            check=True,
        )

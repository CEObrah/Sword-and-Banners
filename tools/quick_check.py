...
def validate_repository_routes() -> None:
    docs = [
        ...
        ROOT / "plugins" / "sword-and-banners" / "skill" / "sword-and-banners-game-master" / "references" / "repository-map.md",
    ]
...
def _registered_combat_role_target(role: str, profiles: dict[str, object], fallbacks: dict[str, object]) -> str | None:
    """Resolve the same explicit registry aliases used by mass combat.

    Some conserved forces (notably mercenary troop pools) keep provenance in
    their physical role key, e.g. ``merc_pool_03_crossbow_infantry``.  Those
    keys are executable only when the combat-role registry explicitly maps a
    token in that key to a registered profile. Unknown/typo roles still fail the
    release gate; this does not use the runtime's defensive line-infantry default.
    """
    if role in profiles and isinstance(profiles.get(role), dict):
        return role
    text = role.lower()
    for needle, target in fallbacks.items():
        target_ref = str(target)
        if str(needle) in text and isinstance(profiles.get(target_ref), dict):
            return target_ref
    return None
...
def validate_troop_role_registry(parsed: dict[Path, object]) -> None:
    ...
    fallbacks = profiles.get("fallbacks", {}) if isinstance(profiles, dict) else {}
    if not isinstance(role_profiles, dict):
        raise SystemExit("combat-role profile registry missing profiles map")
    if not isinstance(fallbacks, dict):
        raise SystemExit("combat-role profile registry missing fallbacks map")
    unresolved: list[str] = []
    for path in (ROOT / "state/formations").glob("*.json"):
        ...
        for role, value in comp.items():
            if max(0, int(value or 0)) <= 0:
                continue
            if _registered_combat_role_target(str(role), role_profiles, fallbacks) is None:
...
def validate_fortified_live_depot_routes(parsed: dict[Path, object]) -> None:
    fort_index = parsed.get(ROOT / "state" / "fortifications" / "index.json")
    owner_index = parsed.get(ROOT / "state" / "index" / "owner-index.json")
    if not isinstance(fort_index, dict) or not isinstance(owner_index, dict):
        raise SystemExit("fortification/owner routing authority missing")
    owners = owner_index.get("owners")
    static = fort_index.get("static_profiles")
    if not isinstance(owners, dict) or not isinstance(static, dict):
        raise SystemExit("fortification live-depot routing indexes are invalid")
    broken: list[str] = []
    for site_ref, row in sorted(static.items()):
        if not isinstance(row, dict):
            continue
        depot_ref = row.get("live_logistics_depot_ref")
        if depot_ref is None:
            continue
        if not isinstance(depot_ref, str) or not depot_ref:
            broken.append(f"{site_ref}:invalid_ref")
            continue
        route = owners.get(depot_ref)
        if not isinstance(route, str) or "#" in route:
            broken.append(f"{site_ref}:{depot_ref}:unrouted")
            continue
        depot = parsed.get(ROOT / route)
        if not isinstance(depot, dict):
            broken.append(f"{site_ref}:{depot_ref}:missing_owner")
            continue
        if depot.get("schema") != "sword-depot" or depot.get("owner_id") != depot_ref:
            broken.append(f"{site_ref}:{depot_ref}:wrong_owner")
            continue
        if site_ref not in {depot.get("site_ref"), depot.get("location_ref")} :
            broken.append(f"{site_ref}:{depot_ref}:wrong_site")
    if broken:
        raise SystemExit(f"fortified live logistics depot routes are stale/broken: {broken[:10]}")
...
def main() -> int:
    ...
    validate_fortified_live_depot_routes(parsed)

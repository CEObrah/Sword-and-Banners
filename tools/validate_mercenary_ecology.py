...
def validate(root: Path) -> list[str]:
    ...
    contract = _j(root / "state/contract/tang-contracted-defense.json")
    ...
    if int(contract.get("anonymous_total", -1)) != 75000:
        errors.append("House Tang standing mercenary troop contract must remain 75,000")
    standing = set(str(x) for x in contract.get("member_force_ids", []))
    ...
    for path in company_paths:
        ...
        if schema not in {"mercenary", "mercenary-company", "regional-mercenary-company.v1"}:
            continue
    ...
    actual = {
        ...
        "specialist": standing_total + independent_specialist_total,
    ...
    if represented != int(market.get("represented_total", -1)) or represented != 450000:
        errors.append(f"represented mercenary total must be exactly 450,000; got {represented}")
    if standing_total != 75000:
        errors.append(f"House Tang standing companies sum {standing_total}, expected 75,000")

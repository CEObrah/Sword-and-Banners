...
def validate(root: Path) -> list[str]:
    ...
    for path in company_paths:
        ...
        for pool in pools:
            ...
            cap = pool.get("capability")
            if not isinstance(cap, dict):
                errors.append(f"{owner}:{pool.get('pool_id') or pool.get('id')} missing embedded capability")
                continue
            ...
            if dist_total != pool_count:
                errors.append(f"{owner}:{pool.get('pool_id') or pool.get('id')} experience_distribution {dist_total} != exact pool {pool_count}")

    regional_index = _j(root / "state/merc/regional.json")
    regional_entries = regional_index.get("entries", []) if isinstance(regional_index, dict) else []
    regional_total = sum(max(0, int(row.get("count", 0))) for row in regional_entries if isinstance(row, dict))
    materialized_refs = {
        str(row.get("id"))
        for row in regional_entries
        if isinstance(row, dict) and row.get("materialized") is True and isinstance(row.get("id"), str)
    }
    exact_regional_refs = {
        str(_j(p).get("owner_id", _j(p).get("id", "")))
        for p in (root / "state/merc/regional").glob("*.json")
    }
    if exact_regional_refs != materialized_refs:
        errors.append("regional mercenary materialization index does not match exact owners")

...
def all_routes(m=None):
    ...
    return out
# control plane
for f in ['RUNTIME.md','VOICE.md','REPOSITORY_MAP.md','data/runtime/repository-map.json','data/runtime/temporal-settlement.json','data/runtime/autonomous-world-simulation.json']:
...
for key,route in all_routes(m).items():
    ...
    for field in ['g','wg']:
        for pat in route.get(field,[]):
            ...
            # Empty materialized-state directories are valid before the first record exists.
...
mapped=set(rj(m.get('directory_map','data/runtime/directory-map.json')).get('dirs',{}))
actual=set()
...
te=rj('data/runtime/temporal-settlement.json')
...
# frontier recurrence and opening closure
front=rj('state/time/frontier.json'); world=front.get('world_time')
covreq=rj('data/runtime/coverage-requirements.json')
covered=set()
...
for base in ['state/person/ht','state/person/world','state/person']:
    ...
    if q.exists():
        for p in q.rglob('*.json'):
            x=getid(p)
            if x: candidates.add(x)
# top-level real forces/units/mercenary companies must also be covered; registry containers without an id are excluded
...
# Homogeneous units/teams may be covered directly or through an explicitly declared descendant-aggregate parent group.
aggregate_parents=set()
for _g in covreq.get('required_coverage_groups',[]):
    if isinstance(_g,dict) and _g.get('mode')=='descendant_aggregate':
        aggregate_parents.update(_g.get('parent_owner_ids',[]))
for _pat in ['state/unit/*.json','state/team/tactical/*.json']:
    for _p in ROOT.glob(_pat):
        ...
        _oid=_d.get('id') or _d.get('unit_id')
        _parent=_d.get('parent_force') or _d.get('owner')
        if _oid and _oid not in covered and _parent not in aggregate_parents:
            err(f'unit_or_team_without_direct_or_parent_coverage:{_oid}:{_parent}')
# runtime catch-up vectors are meaningful
...
if wk:
    ...
    while due<=target:
        c+=1; last=due; due+=step
    if c!=wk['expected_full_boundaries'] or last!=wk['expected_last_boundary_seconds'] or target-last!=wk['expected_residual_seconds']:
        err('weekly_catchup_vector_failed')
...
# autonomous offscreen actors must be able to create and resolve material operations without player presence.
_aw=rj('data/runtime/autonomous-world-simulation.json')
...
if errs:
    ...
    sys.exit(1)
# Autonomous operation storage routes must resolve to real authorities/directories.
...
for _name,_rel in _st.items():
    if not (ROOT/_rel).exists(): err('autonomous_storage_target_missing:'+str(_name)+':'+str(_rel))

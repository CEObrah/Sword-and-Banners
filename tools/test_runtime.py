...
def all_routes(m=None):
    ...
    return out
# control plane
for f in ['RUNTIME.md','VOICE.md','REPOSITORY_MAP.md','data/runtime/repository-map.json','state/meta.json','state/player.json','state/scene.json']:
...
for key,route in all_routes(m).items():
    ...
    for field in ['g','wg']:
        for pat in route.get(field,[]):
            ...
            if prefix and not base.exists() and prefix not in set(rj(m.get('directory_map','data/runtime/directory-map.json')).get('dirs',{})):

...
check(
    "github_actions_verification_present",
    workflow_path.is_file()
    and "tools/quick_check.py" in workflow_text
    and "tools/test_changed.py" in workflow_text
    and "tests/runtime/test_transactions.py" in workflow_text,
)
...
for ref, route in sorted(owners.items()):
    ...
    if owner.get("schema")=="sab_character" and str(ref).startswith("char_"):
        ...
        custody=owner.get("custody_state",{}) if isinstance(owner.get("custody_state"),dict) else {}
        custody_status=str(custody.get("status","")).lower()
        if life in {"dead","deceased"} or custody_status in {"captured","prisoner"}:
            assignment=owner.get("command_assignment",{}) if isinstance(owner.get("command_assignment"),dict) else {}
            military=owner.get("military_command",{}) if isinstance(owner.get("military_command"),dict) else {}
            for scope in (owner.get("current_formation_id"), assignment.get("formation_ref"), military.get("formation_scope")):
    ...
    command_structure=owner.get("command_structure",{}) if isinstance(owner.get("command_structure"),dict) else {}
    unit_command=command_structure.get("unit_command",{}) if isinstance(command_structure.get("unit_command"),dict) else {}
    if not isinstance(formation_ref,str) or not formation_ref.startswith("formation_") or unit_command.get("representation")!="aggregate":
        continue
    allocation=owner.get("external_personnel_allocations",{}).get(formation_ref,{}) if isinstance(owner.get("external_personnel_allocations"),dict) else {}
    allocated_command=sum(int(value) for role,value in allocation.items() if isinstance(role,str) and "command" in role) if isinstance(allocation,dict) else 0
    if int(unit_command.get("commander_billets",0) or 0)>=1 and allocated_command>=1:
...
for path in sorted((ROOT/"state/formations").glob("*.json")):
    ...
    strength=int(formation.get("authorized_strength",formation.get("personnel",0)) or 0)
    if strength<500 or formation.get("status") in {"disbanded","destroyed"}: continue
    formation_ref=formation.get("formation_ref")
    commander=formation.get("commander_ref")
    if commander is None:
        if formation_ref in aggregate_command_scopes:
            attached=formation.get("attached_unit_command_by_role",{})
            if not isinstance(attached,dict) or sum(int(v) for v in attached.values())<1:
                exact_500_errors.append((formation_ref,"aggregate_command_missing_attached_billet"))
            continue
        if formation.get("status")=="commander_vacant" or formation_ref in unavailable_command_scopes:
            continue
        exact_500_errors.append((formation_ref,"missing_commander_without_vacancy_or_unavailable_scope"))
        continue
    ...
    if not isinstance(commander,str) or not isinstance(route,str) or not route.startswith(("state/char/","state/player.json")):
        exact_500_errors.append((formation_ref,commander,route))
        continue
    person=_read_owner_route(route)
    life=str(person.get("life_status",person.get("health_status","active"))).lower() if isinstance(person,dict) else "invalid"
    custody=person.get("custody_state",{}) if isinstance(person,dict) and isinstance(person.get("custody_state"),dict) else {}
    custody_status=str(custody.get("status","")).lower()
    if life in {"dead","deceased"} or custody_status in {"captured","prisoner"}:
        exact_500_errors.append((formation_ref,commander,"unavailable",life,custody_status))
...
check("universal_instantiated_500_plus_commanders_are_represented_and_available", not exact_500_errors, str(exact_500_errors[:12]))
...
check("formation_cadre_state_contains_no_derived_allocations", all(
    ...
    or (
        row.get("formation_ref") in aggregate_command_scopes
        and row.get("officer_cadre") is None
    )
...
for host_id, host in sorted(rt_scheduler.get("hosts", {}).items()):
    ...
    if isinstance(resolved_text, str):
        ...
        if resolved > now_scheduler:
            next_text_for_resolution = host.get("next_due")
            safe_text_for_resolution = host.get("safe_through")
            lawful_one_shot_horizon = False
            if int(host.get("recurrence_seconds", 0) or 0) == 0 and isinstance(next_text_for_resolution, str) and isinstance(safe_text_for_resolution, str):
                next_for_resolution = CampaignTime.parse(next_text_for_resolution)
                safe_for_resolution = CampaignTime.parse(safe_text_for_resolution)
                lawful_one_shot_horizon = resolved == safe_for_resolution == next_for_resolution.add_seconds(-1)
            if not lawful_one_shot_horizon:
...
check("causal_host_resolution_frontier_semantics_valid", not future_resolved_hosts, str(future_resolved_hosts[:12]))
...
_handoff_vitality = summarize_playability_vitality(RepositoryStore(ROOT))
_handoff_defects = {
    key: int(_handoff_vitality.get(key, 0) or 0)
    for key in (
        "blocked_pending_qin_operational_briefings",
        "pending_qin_operational_briefing_responses_without_actionability",
        "redundant_qin_support_campaign_follow_on_hosts",
        "campaign_command_decisions_exposed_before_delivery",
    )
}
check(
    "campaign_command_handoff_vitality_clean",
    not any(_handoff_defects.values()),
    json.dumps(_handoff_defects, sort_keys=True, separators=(",", ":")),
)

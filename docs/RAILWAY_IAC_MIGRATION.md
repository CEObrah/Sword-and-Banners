...
3. Compare the imported service configuration against the still-active `railway.toml`, preserving the Sword runtime requirements:
   ...
   - `PYTHONPATH=/app/runtime python -m sword_runtime.branch_bootstrap` start command, with `SWORD_GIT_BRANCH=main` supplied as the source-branch environment configuration; the wrapper preserves that value as `SWORD_SOURCE_BRANCH` and rewrites `SWORD_GIT_BRANCH` internally to `campaign-baselines/<campaign_id>/<release_baseline_id>` before normal bootstrap; this namespace does not conflict with the legacy `campaign/<campaign_id>` ref, certified baseline changes select a distinct durability lineage, and the retired lineage's completed WAL/receipt store is archived before writes resume;
   - `/health` health check with sufficient startup timeout for main-branch Git synchronization, integrity validation, and recovery;

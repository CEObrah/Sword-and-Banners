...
Deploy `CEObrah/Sword-and-Banners` from `main` only after local `python tools/quick_check.py` and `python tools/run_release_suite.py` are green. GitHub Actions is not a release authority for this project. Attach a persistent volume at `/data` for the campaign Git checkout plus private WAL/receipt/recovery data.

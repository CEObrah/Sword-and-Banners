...
Deploy `CEObrah/Sword-and-Banners` from `main` after `python tools/run_release_suite.py` is green. Attach a persistent volume at `/data` for the campaign Git checkout plus private WAL/receipt/recovery data.

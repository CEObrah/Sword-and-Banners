...
registry=rj(ROOT/'game/schemas/registry.json') or {}
for p in ROOT.rglob('*.json'):
 ...
 for o in walk(d):
  ...
  if jsonschema:
   spec=rj(ROOT/'game'/'schemas'/target)
   if spec:
...
items=_sharded_registry('game/data/items.json','items'); loads=_sharded_registry('game/data/loadouts.json','loadouts')
...
_covreq=rj(ROOT/'game/data/runtime/coverage-requirements.json') or {}
...
if not (ROOT/'game/data/mechanics/reputation.json').exists():err('reputation_mechanics_missing')
...
_role_profiles=(rj(ROOT/'game/data/people/role-profiles.json') or {}).get('profiles',{})
for pth in champ:
 d=rj(pth) or {}
...
_dev=rj(ROOT/'game/data/development/model.json') or {}
...
_bal=rj(ROOT/'game/data/balance/state-military-identities.json') or {}
if len(_bal.get('states',{}))!=7:err('state_military_identity_registry')

# Cleanup and mechanical closure
if (ROOT/'game/schemas/compat').exists():err('compat_schema_directory_present')
...
_sreg=rj(ROOT/'game/schemas/registry.json') or {}
for _fn in _required_mechanics:
 _p=ROOT/'game/data/mechanics'/_fn
...
# No release-history aliases/debris inside canonical gameplay game/data/game/rules/docs.
...
_ma=rj(ROOT/'game/data/content/mission-archetypes.json') or {}; _mt=_ma.get('archetypes',[])
_ea=rj(ROOT/'game/data/content/world-event-archetypes.json') or {}; _wa=_ea.get('archetypes',[])
...
if (ROOT/'game/data/runtime/opening-metrics.json').exists():err('release_opening_metrics_in_runtime')
...
if (ROOT/'game/data/provenance.json').exists():err('release_provenance_in_live_gameplay_tree')
if (ROOT/'game/schemas/release-provenance-v38.schema.json').exists():err('release_provenance_schema_in_live_gameplay_tree')
...
_chars_text=(ROOT/'game/rules/characters.md').read_text(encoding='utf-8')
...
_kprofiles=(rj(ROOT/'game/data/people/knowledge-profiles.json') or {}).get('profiles',{})
for _pth in (ROOT/'state/char').glob('*.json'):
 _d=rj(_pth) or {}
...
_docs=sharded_records('game/data/mil/doctrines.json','doctrines')
_trains=sharded_records('game/data/mil/training.json','profiles')
_ttypes=(rj(ROOT/'game/data/organization/troop-types.json') or {}).get('types',{})
...
_cat=rj(ROOT/'game/data/people/latent-identities.json') or {}
...
for _r in _tx.get('records',[]):
 ...
 if _ev.get('partition_authority')!='game/data/mechanics/unit-partition.json':err(f'unit_transaction_partition_evidence_missing:{_tid}')
...
_cmd=rj(ROOT/'game/data/mechanics/command.json') or {}
...
_ur=rj(ROOT/'game/data/mechanics/unit-resolution.json') or {}
...
_um=rj(ROOT/'game/data/organization/unit-model.json') or {}
_part=rj(ROOT/'game/data/mechanics/unit-partition.json') or {}

...
um=rj('game/data/organization/unit-model.json'); ur=rj('game/data/mechanics/unit-resolution.json')
...
# Support semantics.
sup=rj('game/data/mechanics/support.json'); classes=sup.get('combat_classes',{})
...
if is_shinobi:
    ...
    man=rj('game/data/tech/manifest.json').get('techniques',{})
    ...
    if not (R/'game/data/people/latent-identities.json').exists():err('latent_identity_catalog_missing')
if errs:

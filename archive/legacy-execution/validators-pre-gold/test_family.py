...
def all_routes():
    m=rj('game/data/runtime/repository-map.json');out=dict(m.get('routes',{}))
    for rel in m.get('route_shards',{}).values():out.update(rj(rel).get('routes',{}))
    return out
idx=rj('state/family/index.json'); mech=rj('game/data/mechanics/family.json')
...
router=rj('game/data/runtime/rule-router.json').get('domains',{})

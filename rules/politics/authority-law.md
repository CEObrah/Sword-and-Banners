...
Coalition support and effective political weight use `data/mechanics/politics.json`. Missing or duplicated power-basis inputs block resolution rather than being estimated.
...
Effective occupation reach uses the minimum-gate formula in `data/mechanics/politics.json`.
...
Military rank and command require the grantor, jurisdiction, legal basis, limits, succession, and revocation method defined in `rules/careers.md`. Command never implies ownership of state, household, allied, or contracted troops.
...
Use the information taxonomy in `rules/agency.md`. A correct judgment is not guaranteed. An incorrect judgment remains a real legal and political event. A successful investigation cannot create evidence that does not exist.

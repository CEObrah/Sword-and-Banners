...
`data/mechanics/injury.json` is the structured numerical authority for wounds, armor interaction, impairment, bleeding, shock, infection, recovery, equipment failure, and lethal-state resolution.





`rules/characters.md` and `rules/careers.md` owns the universal character kernel, development, materialization, and featured-character rules. `rules/doctrine.md`, `rules/doctrine.md`, `rules/loadouts.md`, and `rules/careers.md` own reusable force profiles. This file owns detailed personal combat, injury, command, bodyguard, scouting, and operational resolution.
...
Infection pressure is calculated by `data/mechanics/injury.json`.

Fixed clinical factor levels and infection-pressure bands are defined only in `data/mechanics/injury.json`. Review only when time, symptoms, care, environment, or wound state changes. The same unchanged state cannot randomly become infected twice.
...
Recovery progress is calculated by `data/mechanics/injury.json`.

...
`data/mechanics/formation.json` is the numerical authority for frontage, formation margins, contact opportunities, integrity penalties, exchange timing, reserve commitment, pursuit, casualty-rate arithmetic, and formation capability means. `data/mechanics/combat.json` and `data/mechanics/injury.json` resolve representative physical contacts. `data/mechanics/morale.json` owns morale pressure and rally.
...
Formation Margin maps to the registered contact fraction in `data/mechanics/formation.json`. That fraction creates potential contacts only.
...
Each exchange advances real time. Standard stable exchange intervals are registered in `data/mechanics/formation.json`; ranged cycles use actual weapon cycles and maneuvers use actual route/work time.
...
Formation morale uses `data/mechanics/morale.json` only. Casualties, command loss, flank/rear state, isolation, supply, registered fear effects, cohesion and explicit rally actions create the pressure/result.
...
Pursuit score is calculated by `data/mechanics/formation.json`. A successful pursuit still resolves physical contacts and cannot capture bodies the pursuer cannot reach.
...
No formation template creates extra soldiers, horses, ammunition or equipment.

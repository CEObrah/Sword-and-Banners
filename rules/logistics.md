...
`data/mechanics/logistics.json` is the structured numerical authority for ration endurance, forage, travel, throughput, sale price, and delivered quantity.
...
Person-days of food are calculated by `data/mechanics/logistics.json`.

Horse-days of fodder are calculated by `data/mechanics/logistics.json`.
...
Water demand is stored as an exact planning rate for the current climate and workload. Baseline water-planning constants are defined in `data/mechanics/logistics.json`; heat, exertion, wounds, cooking, washing, and medical use must be saved as explicit adjustments. A river, well, or spring is not automatically safe, accessible, uncontested, or fast enough for an army.
...
Forage yield is calculated by `data/mechanics/logistics.json`.
...
Travel time is calculated by `data/mechanics/logistics.json` using saved factors and splits when those factors materially change.

Party size, movement mode, load, terrain, weather, caution/security, injury, fatigue, and baseline water-planning constants are defined only in `data/mechanics/logistics.json`. Physically impassable routes are illegal rather than converted into arbitrary delay multipliers. The slowest essential element controls unless the force explicitly splits.

Scouting and guides reduce search, navigation, ambush, and route-choice delays. They do not erase distance.

## Route events and camps
...
A camp records location, perimeter, water, latrines, fire policy, watch, hospital, animal lines, baggage, prisoners, command post, and departure readiness. Poor sanitation and crowding accumulate disease risk over time.
...
Route throughput is the minimum-gate calculation in `data/mechanics/logistics.json`.
...
Unit sale price is calculated by `data/mechanics/logistics.json`.
...
Delivered quantity is calculated by `data/mechanics/logistics.json` and cannot fall below zero.

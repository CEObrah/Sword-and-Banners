...
def test_project_cancellation_releases_regional_workers_and_refunds_unused_inputs(campaign):
    ...
    site_after_start, _local_after_start = _local_economy(planner, eco_path, location_ref)
    assert site_after_start == source_site
    allocation = planner.read(eco_path)["labor_allocation"]["projects"]["project_cancel_regional_test"]
    assert str(allocation["location_ref"]) == source_site
    assert int(allocation["workers"]) > 0
    ...
    assert "project_cancel_regional_test" not in planner.read(eco_path)["labor_allocation"]["projects"]
...
def test_project_workers_remain_reserved_in_exact_regional_economy_during_production(campaign):
    ...
    allocation = planner.read(eco_path)["labor_allocation"]["projects"]["project_labor_regional"]
    ...
    after_allocation = planner.read(eco_path)["labor_allocation"]["projects"]["project_labor_regional"]
    assert int(after_allocation["workers"]) == reserved
    assert str(after_allocation["location_ref"]) == source_site

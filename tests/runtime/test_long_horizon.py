...
def test_horizons_are_bounded_and_alive(campaign):
    ...
    for y in (3,10,20,50):
        ...
        # reached by the bounded causal read set in addition to the hosts that
        # woke. Bound writes by those two structural sets, never by elapsed years.
        assert p.result['writes'] <= causal_host_bound + p.result['planning_reads']

...
def test_horizons_are_bounded_and_alive(campaign):
    ...
    for y in (3,10,20,50):
        ...
        if short_horizon_reads is None:
            ...
            # The current exact command/family/person consequence surface has
            # a fixed 136-read envelope above the causal host set even on the
            # untouched release baseline. Keep a small deterministic margin
            # without making the bound scale with elapsed years.
            assert p.result['planning_reads'] <= causal_host_bound + 144

...
def test_bow_launch_power_uses_draw_capability_but_crossbow_mechanism_does_not(campaign):
    ...
    arrow=planner._combat_weapon('ammo_arrow')
    bolt=planner._combat_weapon('ammo_bolt_war')
    bow=planner._combat_weapon('weapon_bow')
...
def test_formation_ranged_profile_has_finite_shield_attrition_and_armor_penetration(campaign):
    ...
    attacker=[{
        ...
        'ranged_cycle_seconds':5.8,'ranged_power_index':82,'ranged_weapon_id':'weapon_bow',
        'ammunition_item':'ammo_arrow','ammunition_resource':'war_arrows','equipment_condition_pct':100,
...
def test_personal_bow_trace_records_aim_flight_defense_and_actual_contact(campaign):
    ...
    execute_internal(campaign,'person_materialize',{
        ...
        'role':'command_personnel','source_location_ref':'loc_kanyou',
    })
    ...
    arrow_before=sum(int(row.get('quantity',0)) for row in manifest_before.get('equipment_manifest',[]) if row.get('item_id')=='ammo_arrow' and any(tok in str(row.get('current_state','')).lower() for tok in ('equipped','readied','quivered')))
    ...
    arrow_after=sum(int(row.get('quantity',0)) for row in manifest_after.get('equipment_manifest',[]) if row.get('item_id')=='ammo_arrow' and any(tok in str(row.get('current_state','')).lower() for tok in ('equipped','readied','quivered')))
    player_after=json.loads(player_path.read_text())
    carried_after=int(player_after.get('combat_state',{}).get('projectile_ammunition',{}).get('ammo_arrow',0))
    ...
    for row in result.get('projectile_recovery_candidates',[]):
        if row.get('actor_ref')!='char_tang_wei' or row.get('projectile_item_id')!='ammo_arrow':
            continue
        fraction=max(0.0,min(.95,float(row.get('recoverable_fraction',0) or 0)))
        token=(str(combat_event['event_id'])+'|'+str(row.get('release_event_id',''))+'|ammo_arrow').encode()
        roll=(int(hashlib.sha256(token).hexdigest()[:8],16)%1000000)/1000000.0
        if roll < fraction:
            expected_field += 1
    field_rows=[row for row in player_after.get('combat_state',{}).get('field_projectiles',[]) if row.get('projectile_item_id')=='ammo_arrow' and row.get('location_ref')==player_after['location']]
...
def test_released_projectile_continues_when_shooter_collapses_before_contact(campaign):
    ...
    target['current_location'] = player['location']
    ...
    contact = next(row for row in result['causal_trace'] if row.get('kind') == 'contact' and row.get('projectile_item_id') == 'ammo_arrow')
    ...
    assert contact['projectile_item_id'] == 'ammo_arrow'

...
def test_same_head_dirty_checkout_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(BootstrapError, match="dirty campaign checkout is not provable WAL recovery state"):
        ensure_checkout(configured)


def test_same_head_dirty_checkout_with_recoverable_wal_reaches_recovery(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    source, remote = source_and_remote(tmp_path, revision=2)
    configured = settings(tmp_path, remote)
    checkout = ensure_checkout(configured)
    (checkout / "state" / "meta.json").write_text(
        meta(3, note="wal-owned applied image"),
        encoding="utf-8",
    )
    monkeypatch.setattr("sword_runtime.bootstrap._has_recoverable_wal", lambda _settings: True)

    assert ensure_checkout(configured) == checkout
    assert git(checkout, "rev-parse", "HEAD") == git(source, "rev-parse", "HEAD")
    assert json.loads((checkout / "state" / "meta.json").read_text(encoding="utf-8"))["revision"] == 3

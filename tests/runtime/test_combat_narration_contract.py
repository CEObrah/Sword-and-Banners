...
PERSONAL_COMBAT = ROOT / "runtime/sword_runtime/personal_combat.py"
...
def test_combat_has_separate_gm_private_truth_and_player_disclosure_boundary():
    text = PERSONAL_COMBAT.read_text(encoding="utf-8")
    assert '"gm_private_director_context"' in text
    assert "gm_private_scene_bounded_omniscient_truth_not_player_knowledge" in text
    assert '"participant_capability_and_condition"' in text
    assert '"attributes"' in text
    assert '"skills"' in text
    assert '"health"' in text
    assert '"fatigue"' in text
    assert '"equipment"' in text
    assert "disclose only what Tang Wei could perceive" in text

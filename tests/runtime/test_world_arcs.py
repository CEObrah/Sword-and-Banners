...
def test_arc_review_creates_runtime_owned_initiative_from_saved_goals(campaign):
    ...
    if runtime["initiative_count"]:
        ...
        assert event["opportunity_template"]["canonical"] is False
        assert event["opportunity_template"]["archetype_id"].startswith("mission.")
        assert "issuer_ref" in event["opportunity_template"]["required_before_canonical"]
...
def test_northern_steppe_arc_has_exact_non_zhao_driver(campaign):
    ...
    assert 'faction_northern_steppe_confederation' in drivers
...
def test_every_current_active_arc_first_review_reports_truthful_domain_status(campaign):
    ...
    assert any(result == "material_action_settled" for _actor, result in results.values())
...
def test_state_and_house_arc_preparation_never_counts_as_strategic_progress(campaign):
    ...
    cases = (
        ("arc_qin_succession_crisis_245", "house_shou_bun_kun_household", "world_arc_priorities"),
        ("arc_ryo_fui_northern_wei_campaign", "state_qin", "world_arc_priorities"),
    )
    for arc_ref, owner_ref, queue_key in cases:
        planner = ProductionCampaignPlanner(campaign)
        planner._reset()
        now = str(planner.read("state/runtime.json")["world_time"])
        ...
        event_ref = arc_after["runtime"]["last_initiative_ref"]
        event = planner.read("state/event/events-messages-and-movement.json")["causal_events"][event_ref]
        assert event["result"] == "work_queued"
        assert int(arc_after["runtime"]["pressure_momentum"]) == momentum_before
        assert "material_evidence" not in event["provenance"]

        owner_path = planner.owner_path(owner_ref) if owner_ref != "state_qin" else "state/states/qin.json"
        owner = planner.read(owner_path)
        queue = [row for row in owner.get(queue_key, []) if isinstance(row, dict) and row.get("arc_ref") == arc_ref]
        assert len(queue) == 1
        first_spend = int(queue[0].get("coordination_spent_silver", 0))
        assert first_spend > 0

        # Reaffirming the same unresolved priority must neither spend again nor
        # create a fresh queue row.  Repeated reviews cannot buy fake momentum.
        result = planner._world_arc_domain_action(event["actor_ref"], event.get("target_ref"), event.get("basis_goal", ""), now, arc_ref)
        assert result["status"] == "work_queued"
        owner2 = planner.read(owner_path)
        queue2 = [row for row in owner2.get(queue_key, []) if isinstance(row, dict) and row.get("arc_ref") == arc_ref]
        assert len(queue2) == 1
...
def test_delivered_one_shot_world_arc_report_scheduler_route_is_garbage_collected(campaign):
    ...
    owner.setdefault("causal_events", {})[source_ref] = {
        ...
        "provenance": {"kind": "world_arc_orchestration", "arc_owner_ref": "kingdom_arcs", "review_count": 1, "domain_status": "material_action_settled", "domain_action_ref": "house_gc_action"},
    }
...
def test_queued_person_arc_work_does_not_compound_momentum_or_duplicate_attempts(campaign):
    ...
    now = str(planner.read("state/runtime.json")["world_time"])
    first = planner._world_arc_person_action("char_hyou", None, "depart into service under hidden political pressure", now, arc_ref)
    second = planner._world_arc_person_action("char_hyou", None, "depart into service under hidden political pressure", now, arc_ref)
    assert first["status"] == second["status"] == "work_queued"
    person = planner.read(planner.owner_path("char_hyou"))
    attempts = [row for row in person.get("runtime", {}).get("autonomous_action_attempts", []) if row.get("arc_ref") == arc_ref and row.get("status") == "queued"]
...
def test_queued_person_priority_is_consumed_by_actor_host_before_arc_observes_it(campaign):
    ...
    now = str(planner.read("state/runtime.json")["world_time"])
    arc_ref = "arc_shin_hyou_departure"
    queued = planner._world_arc_person_action("char_hyou", "char_shin", "seize the first lawful opportunity to enter service", now, arc_ref)
    ...
    person = planner.read(planner.owner_path("char_hyou"))
    row = next(x for x in person["runtime"]["autonomous_action_attempts"] if x.get("action_ref") == queued["action_ref"])
    ...
    observed = planner._world_arc_domain_action("char_hyou", "char_shin", row["goal"], host["next_due"], arc_ref)
...
def test_world_arc_priority_scheduler_route_is_one_shot_and_arc_observes_completed_work(campaign):
    ...
    planner.put("state/runtime.json", runtime)
    now = str(runtime["world_time"])
    queued = planner._world_arc_state_action("state_qin", "state_wei", "protect core territory and maintain military readiness", now, "arc_ryo_fui_northern_wei_campaign")
    scheduled = planner.read("state/runtime.json")
    ...
    observed = planner._world_arc_completed_priority("state_qin", "arc_ryo_fui_northern_wei_campaign")

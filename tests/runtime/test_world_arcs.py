...
def test_world_arc_full_person_domain_uses_saved_schema_not_char_prefix(campaign):
    from sword_runtime.production_planner import ProductionCampaignPlanner
    planner = ProductionCampaignPlanner(campaign); planner._reset()
    actor_ref = "officer.test.promoted.world_arc"
    actor_path = "state/person/promoted-world-arc-test.json"
    source = copy.deepcopy(planner.read(planner.owner_path("char_shin")))
    source["owner_id"] = actor_ref
    source["id"] = actor_ref
    source["name"] = "Promoted World Arc Test"
    source.pop("house_ref", None)
    source.setdefault("runtime", {}).pop("autonomous_priorities", None)
    planner.put(actor_path, source)
    planner._register_owner(actor_ref, actor_path)

    now = str(planner.read("state/runtime.json")["world_time"])
    arc_ref = "arc_shin_hyou_departure"
    result = planner._world_arc_domain_action(
        actor_ref,
        None,
        "seek lawful service under current political pressure",
        now,
        arc_ref,
    )
    assert result["status"] == "work_queued"
    assert result["actor_ref"] == actor_ref
    assert result["action"] == "personal_initiative_attempt"
    person = planner.read(actor_path)
    assert result["action_ref"] in person["runtime"]["autonomous_priorities"]
...
def test_protected_canon_pressure_anchors_activate_by_year_without_forcing_outcomes(campaign):
    ...
    # Isolate activation semantics from the live revision's current diplomacy.
    # This test asks whether the registered 241 BCE pressure can activate for all
    # six potential rival actors when they remain eligible; a separate test below
    # verifies that exact alliances/client relations lawfully filter that set.
    potential = coalition["facts"]["protected_pressure"]["potential_actor_refs"]
    for state_ref in potential:
        path = planner.owner_path(state_ref)
        state = copy.deepcopy(planner.read(path))
        diplomacy = state.setdefault("diplomacy", {})
        row = diplomacy.setdefault("state_qin", {})
        row["status"] = "neutral"
...
def test_qin_world_arc_retasks_player_commanded_state_detachment_instead_of_ignoring_it(campaign):
    ...
    planner = ProductionCampaignPlanner(campaign); planner._reset()
    now = str(planner.read("state/runtime.json")["world_time"])
    arc_ref = "arc_test_qin_player_detachment"
...
def test_state_operational_order_never_commandeers_house_tang_formations(campaign):
    ...
    planner = ProductionCampaignPlanner(campaign); planner._reset()
    now = str(planner.read("state/runtime.json")["world_time"])
    arc_ref = "arc_test_qin_house_exclusion"

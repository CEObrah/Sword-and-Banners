...
def test_every_semantic_command_has_a_fail_closed_hostile_case(campaign):
    ...
    tests.  It prevents new commands from entering the release surface with no hostile
    ...
    cases = {
        ...
        'battlefield_control': {'action': 'open', 'operation_ref': 'operation_missing', 'battlefield_ref': 'battlefield_hostile', 'name': 'Hostile', 'side_refs': ['side_a', 'side_b'], 'layout_ref': 'battlefield.layout.line_three'},
        ...
        'polity_action': {'polity_ref': 'polity_missing', 'action': 'set_strategic_goal', 'goal': 'impossible'},
        'project_resolve': {'institution_ref': 'inst_qin_fortification_bureau', 'project_ref': 'project_does_not_exist'},
        'project_cancel': {'institution_ref': 'inst_qin_fortification_bureau', 'project_ref': 'project_does_not_exist'},
        'recruitment': {'state': 'qin', 'personnel': 0, 'source_stratum': 'agricultural', 'role': 'line_infantry'},
        'recruitment_campaign_start': {'state': 'qin', 'campaign_ref': 'recruitment.hostile.start', 'applicant_count': 0, 'destination_force_ref': 'force_tang_wei_personal', 'role': 'household_retainer', 'location_ref': 'loc_tang_manor_garrison_yard'},
        'recruitment_campaign_stage': {'campaign_ref': 'recruitment.missing', 'selection_profile': 'wei_basic_eligibility', 'retain_count': 1},
        'recruitment_campaign_train': {'campaign_ref': 'recruitment.missing', 'hours': 0},
        'recruitment_campaign_finalize': {'campaign_ref': 'recruitment.missing'},
        'recruitment_campaign_cancel': {'campaign_ref': 'recruitment.missing'},
...
def test_every_semantic_command_rejects_unknown_payload_fields(campaign):
    ...
    for command_type in sorted(catalog):
        actor = RepositoryCommandPlanner.INTERNAL_ACTOR if command_type == 'repair' else baseline['player_id']
        mode = 'maintenance' if command_type == 'repair' else 'gameplay'
        command = CommandEnvelope(
            baseline['campaign_id'], f'hostile-unknown-field-{command_type}', actor,
            command_type, baseline['revision'], baseline['time'],
            {'__unexpected_shadow_field__': True}, mode=mode,
        )

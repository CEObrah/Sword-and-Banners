...
def _seed_active_qin_scope(planner: ProductionCampaignPlanner) -> None:
    # This is a pre-arrival lifecycle fixture, not a snapshot assertion about the
    # mutable live campaign.  Revision 45 already has Tang Wei's field command at
    # Sanyou, so explicitly restore the disposable fixture to the eastern depot.
    # Otherwise a newly delivered concentration order is correctly auto-completed
    # as a zero-distance arrival and this test stops exercising mid-advance
    # briefing activation at all.
    origin = "loc_qin_eastern_depot"
    player = copy.deepcopy(planner.read("state/player.json"))
    player["location"] = origin
    player["current_location"] = origin

...
def test_sword_manor_fort_supply_is_house_tang_owned_not_qin_state(campaign):
    ...
    depot=p.read('state/depots/fort-sword-manor.json')
    ...
    assert 'Qin state depots are not an implicit source' in depot['resupply_sources']['rule']
    assert depot['artillery_ref']=='artillery_fort_sword_manor'
    for ref in p._formations_at('loc_sword_manor'):
        _fp,f=p._load_formation(ref)
        assert f['supply_depot_ref']=='depot_fort_sword_manor'
        assert f['logistics']['source_depot_ref']=='depot_fort_sword_manor'
...
def test_kankoku_uses_exact_specialized_hot_owners(campaign):
    ...
    assert result['garrison_personnel']==50400
...
def test_tang_manor_uses_one_canonical_storage_capacity_and_finite_siege_stock(campaign):
    ...
    assert depot['storage_capacity']['grain_kg']==500_000_000
    assert depot['storage_capacity']['fodder_kg']==220_000_000
    ...
    fort={'site_ref':'loc_tang_manor','location_ref':'loc_tang_manor','garrison_formation_refs':p._formations_at('loc_tang_manor')}
...
def test_tang_manor_static_blueprint_and_live_depot_share_one_exact_capacity_authority(campaign):
    p=planner_for(campaign)
    profile=p._fortification_profile_for_site('loc_tang_manor')['logistics_blueprint']
    depot=p.read('state/depots/house-tang.json')
    master=p.read('game/data/world/tang-manor-master-plan.json')['strategic_storage']
    assert depot['storage_capacity']==profile['storage_capacity']
    assert depot['storage_capacity']['grain_kg']==master['grain_capacity_kg']==500_000_000
    assert depot['storage_capacity']['fodder_kg']==master['fodder_capacity_kg']==220_000_000
    assert depot['storage_capacity']['carts']==master['military_cart_team_capacity']==12_000
    assert depot['medical_reserve']['bed_capacity']==master['medical_bed_capacity']==12_000
    assert depot['wagon_staging']['covered_cart_capacity']==master['military_cart_team_capacity']==12_000
    assert depot['water_reserve']['capacity_person_days']==profile['water_system']['reserve_capacity_person_days']==master['water_reserve_capacity_person_days']==9_000_000
    assert 0 <= depot['water_reserve']['current_person_days'] <= depot['water_reserve']['capacity_person_days']
    # A normal hot-site review must preserve, not regenerate, the exact Tang-specific capacities.
    p._ensure_hot_fortified_site_resources('loc_tang_manor',at=str(p._world_time()),authority_ref='house_tang')
    after=p.read('state/depots/house-tang.json')
    assert after['storage_capacity']==depot['storage_capacity']
    assert after['water_reserve']['capacity_person_days']==9_000_000
    assert after['water_reserve']['current_person_days']<=9_000_000

...
def test_named_house_tang_activity_host_settles_canonical_monthly_hours(campaign):
    ...
    fractional_before = float(activity.get("fractional_training_hour_credit", 0.0) or 0.0)
    ...
    expected_fraction = fractional_before + 205.714286 - last["hours"]
    assert after["autonomous_activity_state"]["fractional_training_hour_credit"] == pytest.approx(expected_fraction, abs=1e-6)
...
def test_player_and_child_current_training_policies_match_age_and_role(campaign):
    ...
    # This is a policy invariant, not a frozen campaign-age snapshot. Tang Kai
    # legitimately ages as the release baseline moves forward; the child
    # household program remains the correct contract only while he is below
    # the protected-field-service threshold at age 10.
    assert age_years(kai, CampaignTime.parse(runtime["world_time"])) < 10

...
def test_current_exact_present_family_gets_private_director_context_before_conversation_session(campaign, tmp_path):
    ...
    # Stage Wei at the relatives' exact current location.  The live campaign
    # moves over time, so this regression must test co-location semantics rather
    # than assuming the family is still wherever an older baseline placed it.
    import json
    from pathlib import Path

    owner_index = json.loads((Path(campaign) / "state/index/owner-index.json").read_text())["owners"]
    kai_record = json.loads((Path(campaign) / owner_index["char_tang_kai"]).read_text())
    family_location = kai_record.get("current_location", kai_record.get("location"))

...
def test_recover_projectiles_consumes_field_candidates_and_restores_exact_ammunition(campaign):
    ...
    arrow = next(row for row in manifest["equipment_manifest"] if row.get("item_id") == "ammo_arrow_war")
    before = int(arrow["quantity"])
    player.setdefault("combat_state", {})["field_projectiles"] = [{
        "projectile_item_id": "ammo_arrow_war",
        "quantity": 3,
    ...
    player["combat_state"].setdefault("projectile_ammunition", {})["ammo_arrow_war"] = before
    ...
    assert result["recovered_by_item"] == {"ammo_arrow_war": 3}
    ...
    arrow_after = next(row for row in manifest_after["equipment_manifest"] if row.get("item_id") == "ammo_arrow_war")
    assert int(arrow_after["quantity"]) == before + 3
    assert int(player_after["combat_state"]["projectile_ammunition"]["ammo_arrow_war"]) == before + 3

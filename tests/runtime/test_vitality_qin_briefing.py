...
from sword_runtime.vitality import (
    _pending_qin_briefings_without_support_path,
    _redundant_qin_support_follow_on_hosts,
)
...
def test_vitality_flags_qin_support_shadow_for_campaign_follow_on_request():
    attempt_ref = "interaction_attempt_follow_on_fixture"
    store = _Store(_operation())
    store.data["state/index/interaction-attempts.json"] = {
        "attempts": [{
            "event_id": attempt_ref,
            "actor_id": "char_tang_wei",
            "action": "request",
            "expects_response": True,
            "topic": "campaign_command:next_objective",
            "player_statement": "State the next objective you require of me.",
        }]
    }
    hosts = {
        "host_shadow": {
            "kind": "qin_command_support_review",
            "support_kind": "operational_briefing",
            "work_ref": attempt_ref,
            "next_due": "244-BCE-12-23T19:00:01+08:00",
        }
    }

    assert _redundant_qin_support_follow_on_hosts(store, hosts) == 1

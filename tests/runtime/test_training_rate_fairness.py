...
def test_young_commanders_and_elite_martial_operatives_are_not_capped_at_56(campaign):
    ...
    assert _resolve(planner, "char_kyoukai")[0] == "elite_professional"
    for ref in ("char_mou_ten", "char_ou_hon", "char_kyoukai"):
        assert _resolve(planner, ref)[2] == pytest.approx(205.714286)


def test_selective_academy_and_martial_aspirant_standards_resolve_from_saved_path(campaign):
    planner = CommandRoutedProductionPlanner(campaign)
    planner._reset()
    assert _resolve(planner, "char_mou_ki")[0] == "household_professional"
    assert _resolve(planner, "char_shin")[0] == "martial_aspirant"
    assert _resolve(planner, "char_hyou")[0] == "martial_aspirant"
    for ref in ("char_mou_ki", "char_shin", "char_hyou"):
        assert _resolve(planner, ref)[2] == pytest.approx(205.714286)


def test_house_tang_uses_same_clock_with_better_institutional_quality(campaign):
    planner = CommandRoutedProductionPlanner(campaign)
    planner._reset()
    regimen_ref, _regimen, hours = _resolve(planner, "char_duan_jin")
    assert regimen_ref == "house_tang_max_sustainable"
    assert hours == pytest.approx(205.714286)


def test_generic_nonmilitary_person_stays_on_civilian_fallback(campaign):
    planner = CommandRoutedProductionPlanner(campaign)
    planner._reset()
    regimen_ref, _regimen, hours = _resolve(planner, "char_karyoten")
    assert regimen_ref == "strategic_apprentice"
    assert hours == pytest.approx(205.714286)
...
def test_resolved_regimen_quality_matches_resolved_hours(campaign):
    ...
    assert hours == pytest.approx(205.714286)
    assert regimen["facility_grade"] == "good"

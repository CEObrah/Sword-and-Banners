...
def test_personal_combat_uses_independent_action_ready_timestamps(campaign):
    ...
    director=result['gm_private_director_context']
    assert director['privacy']=='gm_private_scene_bounded_omniscient_truth_not_player_knowledge'
    assert director['participant_capability_and_condition']['char_tang_wei']['alignment']=='player'
    assert director['participant_capability_and_condition'][opponent]['alignment']=='opponent'
    assert director['participant_capability_and_condition']['char_tang_wei']['start_state']['position']
    assert director['participant_capability_and_condition'][opponent]['end_state']['position']
...
def test_plain_natural_punch_intent_uses_combat_law_not_a_punch_command(campaign):
    from conftest import execute
    from sword_runtime.command_contracts import COMMAND_PAYLOAD_KEYS

    # Natural-language attack vocabulary is not the runtime operation catalog.
    # The player may declare a punch; the existing personal-combat law resolves
    # contact, defense, fatigue and injury without a bespoke ``punch`` command.
    assert 'punch' not in COMMAND_PAYLOAD_KEYS

    player=json.loads((campaign/'state/player.json').read_text())
    owners=json.loads((campaign/'state/index/owner-index.json').read_text())['owners']
    opponent='char_tang_ling'
    opponent_path=campaign/owners[opponent]
    opponent_sheet=json.loads(opponent_path.read_text())
    if opponent_sheet.get('current_location') != player['location']:
        opponent_sheet['current_location']=player['location']
        opponent_path.write_text(json.dumps(opponent_sheet,ensure_ascii=False,indent=2)+'\n')
        _commit_state_edit(campaign,owners[opponent])
    ...
    assert result['intent_sequence'][0]['intent']=='punch him'
    player_attacks=[
        row for row in result['causal_trace']
        if row.get('kind')=='attack' and row.get('actor_ref')=='char_tang_wei'
    ]
    assert player_attacks
    assert any(row.get('declared_intent')=='punch him' for row in player_attacks)

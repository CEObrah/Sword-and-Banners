...
def test_qin_detachment_is_four_real_2000_units_with_full_external_unit_command():
    ...
    for index, formation_ref in enumerate(unit_refs, start=1):
        ...
        assert len([row for row in formation["command_structure"]["internal_hierarchy"] if row["scale"] == 1000]) == 1
        assert formation["command_structure"]["internal_hierarchy"][0]["count"] == 2
        assert formation["command_structure"]["internal_hierarchy"][1]["count"] == 4
        assert formation["command_structure"]["internal_hierarchy"][2]["count"] == 20
...
def test_lin_zhen_field_deputy_capability_is_peer_to_wei_jian_and_mounted_command_ready():
    ...
    for key in (
        ...
        "Mass Combat", "Training", "Spear", "Sword", "Shield", "Bow", "Riding",
        "Defense", "Formation Fighting",
    ):
...
def test_all_formal_command_billets_use_mounted_relevant_loadout_and_no_random_weapon_specialties():
    ...
    relevant = ("Spear", "Sword", "Shield", "Bow", "Riding", "Defense", "Formation Fighting")
    unrelated = ("Axe", "Crossbow", "Dagger", "Glaive", "Mace", "Staff")
    for ref in sorted(npc_refs):
        ...
        assert items["mount"] == "horse_tang_heavy_war", ref
        assert items["primary_melee_weapon"] == "weapon_lance_cavalry", ref
        assert items["shield"] == "shield_tang", ref
        assert items["sidearm"] == "weapon_sword_one_hand_long", ref
        assert all(int(person["skills"].get(key, 0)) >= 125 for key in relevant), ref
        primary = [int(person["skills"][key]) for key in ("Spear", "Sword", "Shield", "Bow", "Riding")]
        unrelated_ceiling = round((sum(primary) / len(primary)) * 0.45)
        assert all(int(person["skills"].get(key, 0)) <= unrelated_ceiling for key in unrelated), ref
...
def test_internal_1000_500_officers_use_troop_loadout_and_role_relative_stats():
    ...
    for ref, loadout in probes:
        ...
        assert officer["equipment_manifest"]["loadout_id"] == loadout
        skills = officer["stats"]["skills"]
        assert skills["Formation Command"] > 0
        assert skills["Leadership"] > 0
        assert skills["Tactics"] > 0
        assert skills["Spear"] > 0 and skills["Shield"] > 0 and skills["Sword"] > 0
        if loadout == "loadout_house_champion":
            assert officer["equipment_manifest"]["items"]["mount"] == "horse_tang_heavy_war"
            assert skills["Riding"] > 0
            primary_keys = ("Spear", "Sword", "Shield", "Bow", "Riding")
        else:
            assert "mount" not in officer["equipment_manifest"]["items"]
            primary_keys = ("Spear", "Sword", "Shield", "Bow")
        primary = [int(skills[key]) for key in primary_keys]
        unrelated_ceiling = round((sum(primary) / len(primary)) * 0.45)
        for key in ("Axe", "Crossbow", "Dagger", "Glaive", "Mace", "Staff"):
            assert int(skills.get(key, 0)) <= unrelated_ceiling, (ref, key)
        if loadout != "loadout_house_champion":
            assert int(skills.get("Riding", 0)) <= unrelated_ceiling, ref
...
def test_person_lite_commanders_use_compact_shards_and_wei_scope_for_qin_assignment():
    ...
    for shard in shards:
        ...
        assert doc.get("representation") == "person_lite_roster_shard"
        assert doc.get("record_count") == len(doc.get("records", {}))
        assert int(doc.get("record_count", 0)) >= 1
        if doc.get("scope_ref"):
            assert doc.get("shard_policy") == "soft_target_only_no_world_cap"
...
def test_formation_promotion_becomes_one_nested_army_unit_with_conserved_succession(campaign):
    ...
    assert available + allocated + external_materialized == force["headcount"]
...
def test_field_army_hq_commands_units_without_repeating_each_unit_top_echelon():
    ...
    for formation_ref in direct_formations:
        ...
        internal = unit.get("command_structure", {}).get("internal_hierarchy", [])

...
def test_deployed_tang_field_officers_no_longer_hold_inner_walls_duty_or_regimen():
    ...
    for ref in refs:
        ...
        formation_ref = person.get("command_assignment", {}).get("formation_ref")
        assert isinstance(formation_ref, str) and formation_ref
        formation = load_route(owners[formation_ref])
        assert (person.get("location") or person.get("current_location")) == formation.get("location_ref")

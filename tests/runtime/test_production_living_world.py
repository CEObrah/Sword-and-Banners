...
def test_campaign_causal_work_catches_up_without_rewind_and_delivers_notice_once(campaign: Path) -> None:
        ...
        json.dumps(
            {
                ...
                "targets": [
                    ...
                    },
                    {
                        ...
                        "priority": 50,
                        "status": "pending",
                        "effect": {"summary": "The test staff channel returns a procedural response.", "target_ref": "char_tang_wei"},
                        "wake": True,
                    },
                ],
            },
    ...
    runtime = planner.read("state/runtime.json")
    assert first.get("interrupted") is not True
    assert first.get("wake_required") is not True
    assert first["events_processed"] >= 2
    assert runtime["world_time"] == str(now.add_seconds(3600))
    assert runtime.get("pending_wake") is None
    notices = first["campaign_event_notices"]
    matching_notices = [
        row for row in notices
        if row.get("campaign_event_ref") == "event_test_staff_response"
    ]
    ...
    # A later advance does not re-arm the already-triggered campaign events.
    # The authority:false routing remains unchanged; exact triggered records suppress
    # duplicate scheduling and the one-shot scheduler routes have been retired.
    second = planner._advance_runtime(str(now.add_seconds(7200)))
    ...
    assert runtime2["world_time"] == str(now.add_seconds(7200))
...
def test_campaign_causal_player_delivery_rejects_mismatched_delivery_target(campaign: Path) -> None:
    planner = ProductionCampaignPlanner(campaign)
    planner.PLAYER_ACTOR = planner.read("state/meta.json")["player_id"]
    planner._reset()
    now = CampaignTime.parse(str(planner.read("state/runtime.json")["world_time"]))
        ...
        json.dumps(
            {
                ...
                "targets": [
                    {
                        "work_ref": "event_test_mismatched_delivery_target",
    ...
    planner._active_command_type = "advance_time"
    with pytest.raises(ValueError, match="delivery target must match Tang Wei"):
                ...
                "targets": [
                    {
                        ...
                        "priority": 50,
                        "status": "pending",
                        "effect": {"summary": "This must never become campaign truth.", "target_ref": "char_tang_wei"},
                        "wake": True,
                    }
                ],
            }

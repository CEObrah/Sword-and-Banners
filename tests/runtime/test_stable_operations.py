...
def test_campaign_event_wake_allows_normal_player_response(campaign: Path) -> None:
    ...
    ordinary = CommandEnvelope(
        campaign_id=meta["campaign_id"],
        request_id="wake.preview.campaign-event-interaction",
        actor_id=meta["player_id"],
        command_type="interaction_action",
        expected_revision=meta["revision"],
        submitted_at=meta["time"],
        payload={
            "target_ref": runtime.store.read_json("state/player.json")["location"],
            "action": "seek_contact",
            "player_statement": "Tang Wei asks to receive the newly arrived staff response.",
        },
        mode="gameplay",
    )

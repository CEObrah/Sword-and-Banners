...
def test_campaign_event_settlement_commits_through_production_transaction(campaign: Path) -> None:
        ...
        "targets": [
            {
                ...
                "status": "pending",
                    "effect": {
                        "summary": "The transactional test staff response arrives.",
    ...
    # The production campaign may already have other lawful work due at the
    # current boundary (for example an automatic command briefing). This test
    # owns only the injected fixture, so verify that it settles exactly once
    # without assuming the rest of the living world is idle.
    ...
    fixture_notices = [
        ...
        if row["campaign_event_ref"] == "event_test_transactional_staff_response"
    ]
...
def _state_digest(root: Path) -> tuple[str, int, int]:
    ...
    for path in sorted((root / "state").rglob("*.json")):
        ...
        digest.update(rel + b"\0" + payload + b"\0")
        count += 1
        size += len(payload)
    return digest.hexdigest(), count, size


def test_compact_play_context_is_bounded_and_read_only(campaign: Path) -> None:
    runtime = ProductionSwordRuntime(campaign, runtime_root=campaign.parent / "runtime-context-stability")
    operations = StableCampaignOperations(runtime)
    before = _state_digest(campaign)
    payloads = []
    for _ in range(3):
        compact = compact_play_context(operations.play_context())
        payloads.append(json.dumps(compact, ensure_ascii=False, sort_keys=True, separators=(",", ":")))
    after = _state_digest(campaign)

    assert before == after
    assert payloads[0] == payloads[1] == payloads[2]
    assert len(payloads[0].encode("utf-8")) < 48_000
    compact_context = json.loads(payloads[0])
    commands = compact_context["commands"]
    assert "mechanic_families" in commands
    assert "intent_families" not in commands
    assert "supported_command_types" not in commands
    assert "command_types" not in commands
    guidance = compact_context.get("narration_guidance", {})
    assert "hidden_truth_rule" not in guidance
    assert "decision_scaffolding" not in guidance
    assert guidance.get("stale_scene_policy") == (
        "revision_matched_projection_is_current; continuity_anchor_is_presentation_only"
    )
    semantic = compact_context["semantic_action_contract"]

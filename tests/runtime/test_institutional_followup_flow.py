...
from sword_runtime.campaign_communications import command_message_route
...
def _append_attempt(planner, *, attempt_label: str, process_ref: str, at: str, statement: str) -> str:
    ...
    attempt = {
        ...
        "origin_location_ref": str(planner.read("state/player.json")["location"]),
    }
...
def test_old_qin_appointment_request_catches_up_without_granting_office(campaign) -> None:
    ...
    origin = str(planner.read("state/player.json")["location"])
    route = command_message_route(planner.read, origin, "loc_kanyou", round_trip=True)
    expected = CampaignTime.parse(requested_at).add_seconds(route["travel_seconds"] + 21600)
    assert CampaignTime.parse(host["next_due"]) == expected
    assert host["communication_travel_seconds"] == route["travel_seconds"]
    assert host["late_catch_up"] is False

    event_ref = _settle_institutional_followup(planner, host, str(expected))
...
def test_old_ouki_appointment_request_catches_up_on_exact_review_event(campaign) -> None:
    ...
    planner._sync_contact_request_routes(runtime)
    host = _followup_host(runtime, contact_ref)
    ouki = planner.read(planner.owner_path("char_ouki"))
    ouki_location = str(ouki["current_location"])
    origin = str(planner.read("state/player.json")["location"])
    route = command_message_route(planner.read, origin, ouki_location, round_trip=True)
    expected_raw = CampaignTime.parse(requested_at).add_seconds(route["travel_seconds"] + 21600)
    expected = current if expected_raw < current else expected_raw
    assert CampaignTime.parse(host["next_due"]) == expected
    assert host["source_location_ref"] == ouki_location

...
def test_duplicate_receipt_recursively_thaws_nested_json() -> None:
    ...
    operations.store = SimpleNamespace(read_json=lambda path: {"player_id": "char_tang_wei"})
    command = CommandEnvelope(
        campaign_id="campaign.test", request_id="request.test", actor_id="char_tang_wei",
        command_type="interaction_action", expected_revision=1,
        submitted_at="244-BCE-09-17T18:22:48+08:00", payload={}, mode="gameplay",
    )
    record = operations.lookup_command_receipt(command)

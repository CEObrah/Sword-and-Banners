...
def test_elite_state_formations_are_mixed_conserved_assignments_under_zero_body_commands():
    ...
    for ref,group in EXPECTED.items():
        ...
        commander_ref = f.get('commander_ref')
        assert isinstance(commander_ref, str) and commander_ref.startswith('char_')
        assert commander_ref in j('state/index/owner-index.json')['owners']
        assert commander_ref != j(f'state/cmd/command-groups/{group}.json')['commander_ref']
        assert 'command_structure' not in f
        rules=j('game/data/mechanics/warfare-organization.json')
        command=build_formation_command_structure(f, rules)
        assert command['unit_command']['outside_fighting_establishment'] is True
        assert f['formation_identity']['state_force_ownership_preserved'] is True
        mounted = sum(int(f['composition'].get(role, 0)) for role in ('cavalry', 'light_cavalry', 'heavy_cavalry'))
        assert sum(f.get('mounts',{}).values()) == mounted
...
def test_seika_retsubi_nanyou_juuko_are_real_geographic_commands_not_spawned_force_owners():
    cases={
      'formation_zhao_seika_field_army':('cmdgrp.shibashou.field_army','loc_seika'),
      'formation_zhao_retsubi_gate_command':('cmdgrp.retsubi.garrison','loc_retsubi'),
      'formation_han_nanyou_western_command':('cmdgrp.nanyou.western','loc_nanyou'),
      'formation_chu_juuko_defense_army':('cmdgrp.juuko.defense','loc_juuko'),
    }
    for ref,(group_ref,home_loc) in cases.items():
        f=formation_by_ref(ref)
        group=j(f'state/cmd/command-groups/{group_ref}.json')
        assert f['higher_command_ref']==group_ref
        assert group['location']==home_loc
        assert isinstance(f['location_ref'],str) and f['location_ref'].startswith('loc_')

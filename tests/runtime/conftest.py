...
def campaign(tmp_path):
    dst=tmp_path/'repo'
    # Build every disposable campaign from the release files themselves. The
    # release must not depend on historical Git objects or an existing branch
    # lineage, so tests initialize a fresh one-commit repository exactly as a
    # new installation would.
    shutil.copytree(
        SOURCE,
        dst,
        ignore=shutil.ignore_patterns('.git', '.pytest_cache', '__pycache__', '*.pyc'),
    )
    subprocess.run(['git','init','--quiet',str(dst)],check=True)
    ...
    subprocess.run(['git','-C',str(dst),'commit','--quiet','-m','Sword and Banners release baseline'],check=True)
    try:
        yield dst
    finally:

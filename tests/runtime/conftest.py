...
def _campaign_repository_base(tmp_path_factory):
    ...
    Every test still receives a separate detached Git worktree below.  The old
    fixture recopied, rehashed, and recommitted the entire release for every
    individual test, so large regression modules spent most of their wall time
    constructing identical Git history rather than exercising runtime behavior.
    """
...
def campaign(tmp_path, _campaign_repository_base):
    ...
    # A detached worktree has the exact same one-commit starting image but owns
    # its working files, index, HEAD, and later test commits independently.
    subprocess.run(['git','-C',str(base),'worktree','add','--quiet','--detach',str(dst),'HEAD'],check=True)
    try:
        yield dst
    finally:
        subprocess.run(['git','-C',str(base),'worktree','remove','--force',str(dst)],check=False,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
...
def execute_production(root, command_type, payload, *, actor='char_tang_wei', mode='gameplay', request_id=None):
    """Execute through the hosted production planner composition on a disposable repo."""

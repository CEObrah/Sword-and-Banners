...
def _co_locate_qin_mobilization_anchors(planner):
    """Place disposable army anchors beside a real conserved Qin reserve.

    Live campaigns may move Ouki, Tou, and Mou Gou away from replacement depots.
    Mobilization tests should establish physical reserve access rather than rely on
    their revision-1 deployment.
    """
    location_ref = "loc_kanyou"
    force = planner.read("state/forces/state-qin.json")
    assert sum(int(v) for v in force["available_by_location"][location_ref].values()) > 0
    assert sum(int(v) for v in force["available_equipment_by_location"][location_ref].values()) > 0
    for formation_ref in (
        "formation_qin_ouki_vanguard",
        "formation_qin_tou_mobile_army",
...
def _raise_mobilization_targets(campaign, *, general=60000, great_general=120000):
...
def test_nested_state_armies_reinforce_children_before_parent_without_touching_private_children(campaign):
    ...
    _co_locate_qin_mobilization_anchors(planner)

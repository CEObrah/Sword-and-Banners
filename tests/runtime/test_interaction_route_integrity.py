...
from sword_runtime.api.interaction_surface import INTERACTION_ATTEMPT_PREFIX, _format_triggered_interaction, record_interaction_attempt
...
from sword_runtime.contact_request_flow import _followup_response_ref, _settle_contact_request, _settle_institutional_followup
...
def test_generic_response_without_explicit_player_target_is_not_player_visible():
    raw = {
        "event_ref": "event_other_actor_response",
        "kind": "institutional_response",
        "status": "triggered",
        "triggered_at": "244-BCE-09-18T09:37:48+08:00",
        "summary": "An institution answers someone else's request.",
    }
    assert _format_triggered_interaction(raw["event_ref"], raw) is None
...
def test_campaign_contact_then_substantive_request_produces_one_later_superior_reply(campaign) -> None:
    ...
    assert len(contact_hosts) == 2
    assert len(request_hosts) == 2
    by_ref = {row["contact_ref"]: row for row in request_hosts}
    assert set(by_ref) == {first_ref, second_ref}
    assert by_ref[first_ref]["request_topics"] == ["march_orders"]
    request_host = by_ref[second_ref]
    ...
    contact_by_ref = {row["contact_ref"]: row for row in contact_hosts}
    assert CampaignTime.parse(request_host["next_due"]) >= CampaignTime.parse(contact_by_ref[second_ref]["next_due"]).add_seconds(15 * 60)
    assert get_causal_event_from_reader(planner, request_host["contact_ref"]) is None

    for contact in contact_hosts:
        _settle_contact_request(planner, contact, str(contact["next_due"]))
    for pending in request_hosts:
        response_ref = _settle_institutional_followup(planner, pending, str(pending["next_due"]))
    ...
    response = get_causal_event_from_reader(planner, _followup_response_ref(second_ref))
    assert "vanguard" in response["summary"].lower()
    assert "remains unresolved" in response["summary"].lower()
    assert "neither grants nor denies" in response["summary"].lower()
    assert "binding assignment" in response["summary"].lower()
...
def test_reply_to_exact_message_has_receipt_route_without_inventing_requested_travel(campaign) -> None:
    ...
    assert host["communication_travel_seconds"] > 30 * 60
    assert host["recipient_target_location_ref"] == "loc_tang_manor_inner_citadel_family_hall"
    response_ref = _settle_institutional_followup(planner, host, str(host["next_due"]))
    response = get_causal_event_from_reader(planner, response_ref)
    assert response is not None
    assert response["kind"] == "message_receipt"
    assert response["actor_ref"] == PLAYER
    assert response["target_ref"] == "char_tang_kai"
    assert response["process_stage"] == "reply_received"
    assert response["visibility_class"] == "hidden"
    assert "requested travel" in response["summary"]
    assert _format_triggered_interaction(response_ref, response) is None
...
def test_remote_reply_channel_must_be_addressed_to_player(campaign) -> None:
    import subprocess
    from pathlib import Path

    root=Path(campaign)
    event_path=root/'state/event/events-messages-and-movement.json'
    owner=json.loads(event_path.read_text())
    meta=json.loads((root/'state/meta.json').read_text())
    event_ref='event_test_message_not_for_player'
    owner['causal_events'][event_ref]={
        'event_ref':event_ref,
        'kind':'message',
        'status':'triggered',
        'due_at':str(meta['time']),
        'triggered_at':str(meta['time']),
        'actor_ref':'char_mou_gou',
        'target_ref':'char_tang_ling',
        'process_kind':'test_private_correspondence',
        'process_stage':'delivered',
        'summary':'A message from Mou Gou to Tang Ling, not to Tang Wei.',
        'delivery':{'target_ref':'char_tang_ling','location_ref':'loc_tang_manor_inner_citadel_family_hall','route':'private messenger'},
        'provenance':{'kind':'causal_runtime_settlement','source_owner_ref':'char_mou_gou','work_ref':event_ref,'late_catch_up':False},
    }
    event_path.write_text(json.dumps(owner,ensure_ascii=False,sort_keys=True,separators=(',',':'))+'\n')
    subprocess.run(['git','-C',str(root),'add',str(event_path.relative_to(root))],check=True)
    subprocess.run(['git','-C',str(root),'commit','--quiet','-m','test: seed message not addressed to player'],check=True)

    token='m'*48; headers={'Authorization':f'Bearer {token}'}
    with TestClient(create_app(root,token)) as client:
        context=client.get('/v1/play/context',headers=headers).json(); base=context['campaign']
        assert 'char_mou_gou' in context['permitted_person_ids']
        direct={
            'campaign_id':base['campaign_id'],'request_id':'wrong-addressee-reply','actor_id':base['player_id'],
            'command_type':'interaction_action','expected_revision':base['revision'],'submitted_at':base['world_time'],
            'payload':{'target_ref':'char_mou_gou','process_ref':event_ref,'action':'request','player_statement':'I reply to Mou Gou.'},
            'mode':'gameplay',
        }
        rejected=client.post('/v1/commands/preview',headers=headers,json=direct)
        assert rejected.status_code==404
        assert rejected.json()['detail']['code']=='interaction_process_not_player_visible'


def test_remote_reply_channel_addressed_to_player_remains_legal(campaign) -> None:
    import subprocess
    from pathlib import Path

    root=Path(campaign); event_path=root/'state/event/events-messages-and-movement.json'
    owner=json.loads(event_path.read_text()); meta=json.loads((root/'state/meta.json').read_text()); event_ref='event_test_message_for_player'
    owner['causal_events'][event_ref]={
        'event_ref':event_ref,'kind':'message','status':'triggered','due_at':str(meta['time']),'triggered_at':str(meta['time']),
        'actor_ref':'char_mou_gou','target_ref':PLAYER,'process_kind':'test_direct_correspondence','process_stage':'delivered',
        'summary':'A direct message from Mou Gou to Tang Wei.',
        'delivery':{'target_ref':PLAYER,'location_ref':'loc_tang_manor_inner_citadel_family_hall','route':'direct messenger'},
        'provenance':{'kind':'causal_runtime_settlement','source_owner_ref':'char_mou_gou','work_ref':event_ref,'late_catch_up':False},
    }
    event_path.write_text(json.dumps(owner,ensure_ascii=False,sort_keys=True,separators=(',',':'))+'\n')
    subprocess.run(['git','-C',str(root),'add',str(event_path.relative_to(root))],check=True)
    subprocess.run(['git','-C',str(root),'commit','--quiet','-m','test: seed direct message to player'],check=True)
    token='n'*48; headers={'Authorization':f'Bearer {token}'}
    with TestClient(create_app(root,token)) as client:
        context=client.get('/v1/play/context',headers=headers).json(); base=context['campaign']
        direct={'campaign_id':base['campaign_id'],'request_id':'right-addressee-reply','actor_id':base['player_id'],'command_type':'interaction_action','expected_revision':base['revision'],'submitted_at':base['world_time'],'payload':{'target_ref':'char_mou_gou','process_ref':event_ref,'action':'request','player_statement':'I reply to Mou Gou.'},'mode':'gameplay'}
        allowed=client.post('/v1/commands/preview',headers=headers,json=direct)
        assert allowed.status_code==200
        assert allowed.json()['world_response_status']=='not_established_by_attempt'

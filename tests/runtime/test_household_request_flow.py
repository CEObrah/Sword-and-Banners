...
def test_house_request_classifier_distinguishes_live_recruitment_chain() -> None:
    ...
    assert _classify_request(None, {
        **common,
        ...
        "player_statement": "Mother, Father: please start recruiting and training Initiates for Sword Manor within the treasury-safe ceiling we discussed.",
    }) == "recruitment_start"
    assert _classify_request(None, {
        **common,
        "action": "ask",
        "target_ref": "char_tang_ling",
        "player_statement": "Mother, what is the treasury-safe ceiling, and how soon can Sword Manor Initiate intake actually open?",
    }) == "recruitment_numbers"
    assert _classify_request(None, {
        **common,
        "action": "ask",
        "target_ref": "char_tang_zhu",
        "player_statement": "Father, what practical constraint limits Sword Manor Initiate intake?",
    }) == "recruitment_parallel_constraints"
    assert _classify_request(None, {
        **common,
        "action": "request",
        "target_ref": "char_tang_ling",
        "player_statement": "Mother, when Sword Manor recruitment intake actually opens, send me word immediately. I want the concrete figures and what has begun.",
...
def test_household_scene_exposes_exact_sword_manor_handle(campaign) -> None:
    ...
    assert "force_sword_manor" in context["permitted_object_refs"]
    rows = context["scene"]["household_military_forces"]
    assert rows == [{"object_ref": "force_sword_manor", "name": "Sword Manor", "relation": "House Tang military force"}]

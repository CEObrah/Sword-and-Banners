...
def test_generic_force_training_advances_embedded_person_lites(campaign):
    ...
    assert round(float(after["verified_training_hours"]) - float(before.get("verified_training_hours", 0)), 3) == pytest.approx(205.714, abs=1e-3)
    assert round(float(after["verified_role_exposure_hours"]) - float(before.get("verified_role_exposure_hours", 0)), 3) == 96.0
...
def test_existing_exact_command_route_reconciles_stale_cached_rate(campaign):
    ...
    assert "verified" in after["autonomous_activity_state"]["verification_rule"]


def test_partial_house_tang_command_contract_is_completed_without_losing_program(campaign):
    planner = CommandRoutedProductionPlanner(campaign)
    planner._reset()
    ref = "char_sword_manor_senior_commander"
    path = planner.owner_path(ref)
    before = deepcopy(planner.read(path))
    assert before["activity_contract"]["training_program_ref"] == "program.sword_officer"
    # Recreate the partial-contract migration shape that caused the stale fallback.
    before["activity_contract"] = {
        "training_program_ref": "program.sword_officer",
    ...
    assert contract["training_program_ref"] == "program.sword_officer"
...
def test_bastion_current_setting_baseline_and_monthly_training_owner(campaign):
    planner = CommandRoutedProductionPlanner(campaign)
    planner._reset()
    runtime = planner.read("state/runtime.json")
    host = deepcopy(runtime["hosts"]["host_sword_manor"])
    force_path = "state/forces/bastion-iron-rampart.json"
    before_force = deepcopy(planner.read(force_path))
    before = before_force["cohort_ledger"]["cohorts"]["cohort_force_bastion_iron_rampart_bastion_heavy_infantry"]
    baseline = before.get("training_tracking_baseline", {})
    assert baseline.get("baseline_kind") == "current_setting_qualified_capability"
    assert float(before.get("verified_training_hours_per_person", 0.0)) == 0.0

    planner._autonomy_manor(host, 1, host["next_due"])
    after_force = planner.read(force_path)
    after = after_force["cohort_ledger"]["cohorts"]["cohort_force_bastion_iron_rampart_bastion_heavy_infantry"]
    assert float(after["verified_training_hours_per_person"]) == pytest.approx(205.714, abs=1e-3)
    assert float(after["verified_role_exposure_hours_per_person"]) == pytest.approx(96.0, abs=1e-3)
    assert after.get("training_history")

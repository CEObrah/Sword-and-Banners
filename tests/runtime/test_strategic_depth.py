...
def test_demographic_maturation_and_site_local_production_share_one_authority(campaign):
    ...
    assert any(eco["local_regions"]["regions"][ref]["production_runtime"].get("last_output") for ref in regions_before)
...
def test_npc_war_intent_requires_material_interest_and_can_be_generated(campaign):
    ...
    # Isolate this generator from the live checkpoint's already-authorized Zhao intent.
    qin["war_intents"] = []
...
def test_asymmetric_treaties_require_direction_and_diplomatic_people_require_authority(campaign):
    ...
    with pytest.raises(PermissionError, match="hostage giver"):
        planner._activate_diplomatic_treaty({"proposal_ref": "bad_hostage", "proposer_ref": "state_zhao", "target_ref": "state_qin", "kind": "hostage_exchange", "direction": "proposer_to_target", "terms": {"hostage_person_ref": "char_mou_ki"}}, at)

    hostage_ref = planner._activate_diplomatic_treaty({"proposal_ref": "good_hostage", "proposer_ref": "state_qin", "target_ref": "state_zhao", "kind": "hostage_exchange", "direction": "proposer_to_target", "terms": {"hostage_person_ref": "char_mou_ki"}}, at)
    hostage = planner.read(planner.owner_path("char_mou_ki"))
    assert hostage["career_state"]["hostage_status"]["treaty_ref"] == hostage_ref

    marriage_ref = planner._activate_diplomatic_treaty({"proposal_ref": "marriage_test", "proposer_ref": "state_qin", "target_ref": "state_zhao", "kind": "marriage_alliance", "direction": "mutual", "terms": {"marriage_person_ref": "char_ri_shi", "marriage_partner_ref": "char_bananji"}}, at)
...
def test_npc_sovereign_can_originate_multilateral_coalition_from_shared_threat(campaign):
    ...
    for ref, other in (("state_qin", "state_zhao"), ("state_zhao", "state_qin")):
        ...
        if ref == "state_qin":
            # The live checkpoint already has one lawful outbound Han offer. This
            # fixture is testing coalition generation, not the one-offer capacity gate.
...
def test_state_ministries_act_with_exact_money_and_local_resources(campaign):
    planner = planner_for(campaign); planner._reset(); at = str(planner.read("state/runtime.json")["world_time"])
    fp, formation = planner._load_formation("formation_qin_mobile_reserve"); formation = copy.deepcopy(formation); formation["readiness"] = 30; formation["command_authority"] = "state_qin"; formation["commander_ref"] = None; planner.put(fp, formation)
    ...
    assert "formation_qin_mobile_reserve" in planner.read(planner.owner_path("inst_qin_military_bureau"))["military_review"]["trained_formations"]
...
def _polity_test_command(planner):
    ...
    return SimpleNamespace(actor_id=planner.PLAYER_ACTOR, expected_revision=int(meta["revision"]), command_type="polity_action", digest="v6-test", semantic_digest="v6-test", submitted_at=str(meta["time"]), mode="gameplay")
...
def test_every_explicit_civilian_population_owner_settles_births_deaths_and_grows(campaign):
    ...
    # Tang Manor is a detailed occupational projection over Qin's local Tang
    # civilian partition, not a second demographic clock.
    tang_before = copy.deepcopy(planner.read(planner.owner_path("population_tang_manor")))
    planner._autonomy_population({"owner_ref": "population_tang_manor"}, 1, "244-BCE-12-04T07:22:48+08:00")
    tang_after = planner.read(planner.owner_path("population_tang_manor"))
    qin = planner.read(planner.owner_path("population_qin"))
    assert tang_after["demography"]["authority"] == "parent_population_only"
    assert "last_births" not in tang_after["demography"] and "last_deaths" not in tang_after["demography"]
    assert int(tang_after["population_total"]) == int(qin["local_population"]["sites"]["loc_tang_manor"]["civilian_population"])
    assert int(tang_after["population_total"]) == sum(int(v) for v in tang_after["strata"].values())
    assert "last_parent_sync" not in tang_after
    assert "last_population_mobility" not in tang_after


def test_demographic_balance_produces_long_run_population_growth_without_magic_multiplier(campaign):
    planner = planner_for(campaign); planner._reset()
    owner_ref = "population_qin"
    path = planner.owner_path(owner_ref)
    ...
    tang = planner.read(planner.owner_path("population_tang_manor"))
    qin_tang = after["local_population"]["sites"]["loc_tang_manor"]
    assert tang["demography"]["authority"] == "parent_population_only"
    assert int(tang["population_total"]) == int(qin_tang["civilian_population"])



def test_interstate_strategy_preserves_field_armies_and_can_split_real_axes(campaign):
    from sword_runtime.strategic_war_planning import build_interstate_strategic_plan

    planner = planner_for(campaign); planner._reset()
    config = planner._interstate_theater_config(planner.read("game/data/world/autonomous-theaters.json"))
    cq = next(row for row in config["theaters"] if row["theater_ref"] == "chu_qin_border")
    plan = build_interstate_strategic_plan(
        planner,
        theater_ref="test_qin_chu_multi_axis",
        attacker="qin",
        defender="chu",
        primary_target="loc_chu_north_fort",
        attacker_formation_refs=cq["formation_ref_lists"]["qin"],
        defender_formation_refs=cq["formation_ref_lists"]["chu"],
        at=str(planner.read("state/runtime.json")["world_time"]),
    )
    assert plan["concentration_mode"] == "multi_axis"
    objectives = {front["objective_ref"] for front in plan["fronts"]}
    assert {"loc_chu_north_fort", "loc_juuko"}.issubset(objectives)
    assigned_commands = [
        row["command_group_ref"]
        for row in plan["command_assignments"]["qin"]
        if row.get("command_group_ref")
    ]
    assert "cmdgrp.mou_gou.field_army" in assigned_commands
    assert "cmdgrp.mou_bu.field_army" in assigned_commands
    assert "cmdgrp.ousen.field_army" not in assigned_commands
    assert "cmdgrp.kanki.field_army" not in assigned_commands
    mou_gou = next(row for row in plan["command_assignments"]["qin"] if row.get("command_group_ref") == "cmdgrp.mou_gou.field_army")
    assert "formation_qin_ousen_central" in mou_gou["formation_refs"]
    assert "formation_qin_kanki_raider_host" in mou_gou["formation_refs"]
    assert len(set(assigned_commands)) == len(assigned_commands)
    assert plan["strategic_reserve_commands"]["qin"]
    # A real ungrouped state formation remains a usable independent command;
    # campaign admission does not silently make it part of a named general's army.
    planned_refs = set(plan["formation_objectives"]["qin"]) | set(plan["strategic_reserve_formation_refs"]["qin"])
    assert "formation_qin_crossbow_corps" in planned_refs
    command_index = planner.read("state/cmd/command-groups/index.json")
    assert "formation_qin_crossbow_corps" not in command_index.get("primary_formation_group", {})

def test_strategic_reserve_is_real_intact_command_and_front_resolution_waits(campaign):
    from sword_runtime.strategic_war_operations import commit_one_reserve_command, war_fronts_resolved
    from sword_runtime.strategic_war_planning import build_interstate_strategic_plan
    ...
    plan = build_interstate_strategic_plan(
        planner,
        theater_ref="test_reserve_commitment",
        attacker="qin",
        defender="chu",
        primary_target="loc_chu_north_fort",
        attacker_formation_refs=cq["formation_ref_lists"]["qin"],
    ...
    reserve = list(plan["strategic_reserve_commands"]["qin"])
    assert reserve
    command = reserve[0]
    expected_refs = set(command["formation_refs"])
    target = sorted(plan["fronts"], key=lambda row: (-int(row["priority"]), row["front_ref"]))[0]
    before = set(target["attacker_formation_refs"])
    committed = commit_one_reserve_command(
        plan, side="qin", front=target, at=str(planner._world_time()), reason="test threatened axis"
    )
    assert committed is not None
    assert set(committed["formation_refs"]) == expected_refs
    assert expected_refs <= set(target["attacker_formation_refs"])
    assert before <= set(target["attacker_formation_refs"])
    assert expected_refs.isdisjoint(set(plan["strategic_reserve_formation_refs"]["qin"]))
    assert all(plan["formation_objectives"]["qin"][ref] == target["objective_ref"] for ref in expected_refs)

    # A siege/advancing secondary front prevents a single occupied objective from
    # ending the war. Only terminal front states may hand off to settlement.
    target["status"] = "occupied"
    another = next((row for row in plan["fronts"] if row is not target), None)
    if another is not None:
        another["status"] = "besieging"
        assert war_fronts_resolved(plan["fronts"]) is False
        another["status"] = "defender_holds"
    assert war_fronts_resolved(plan["fronts"]) is True

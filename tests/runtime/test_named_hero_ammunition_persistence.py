...
def test_named_ranged_hero_spends_and_persists_personal_ammunition(campaign):
    # Use the stable Qin eastern-depot ranged formation so the disposable
    # enemy commander is drawn from a real conserved command-personnel reserve.
    # The test is about exact-person ammunition custody, not cross-state
    # materialization at whatever location a live House formation currently holds.
    champion_ref = "formation_qin_kyoukai_command"
    champion_path, champion = _owner_doc(campaign, champion_ref)
    battle_location = str(champion["location_ref"])
    assert battle_location == "loc_qin_eastern_depot"
    ...
    _fixture_update(campaign, "state/player.json", {"skills": skills, "location": battle_location, "current_location": battle_location})
    ...
    # This enemy is line infantry and does not need missile stock for the test.
    # Do not conjure arrows into its newly created Kanyou field cache merely to
    # exercise Tang Wei's independent exact-person ammunition accounting.

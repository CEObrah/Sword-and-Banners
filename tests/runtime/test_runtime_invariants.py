...
def test_release_has_one_current_authority_tree_and_local_release_gate(campaign):
    ...
    assert (campaign/'.github/workflows/runtime-tests.yml').is_file()
...
def test_release_certifier_temp_root_is_process_scoped():
    import importlib.util
    from conftest import SOURCE

    spec = importlib.util.spec_from_file_location("sword_release_suite_temp_root_test", SOURCE / "tools/run_release_suite.py")
    assert spec is not None and spec.loader is not None
    release_suite = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(release_suite)

    assert release_suite.TMP_ROOT.parent == release_suite.TMP_BASE
    assert release_suite.TMP_ROOT.name == release_suite.RUN_TOKEN
    assert release_suite.LOCK_PATH != release_suite.TMP_ROOT

...
def test_release_has_one_current_authority_tree_and_local_release_gate(campaign):
    ...
    # Accounting-only mercenary market rows do not own scheduler clocks.
    # Tactical companies gain causal work only when current mechanics actually
    # materialize an active obligation; the clean baseline therefore carries
    # no generic mercenary review hosts.
    assert sum(1 for h in rt['hosts'].values() if h.get('kind')=='mercenary')==0

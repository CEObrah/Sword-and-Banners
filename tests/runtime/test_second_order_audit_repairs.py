...
def test_live_accounting_mercenary_contracts_are_causally_routed(campaign):
    ...
    for _path, row in _mercenary_rows(campaign):
        ...
        if mercenary_has_live_contract(row):
            ...
            cold.add(ref)
    # Canonical campaign population can legitimately evolve.  The invariant is
    # not a historical cardinality; every currently live mercenary obligation
    # must have exactly one scheduler host, and cold accounting-only companies
    # must have none.

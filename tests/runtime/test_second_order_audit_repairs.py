...
from sword_runtime.scheduler_frontier import repair_core_autonomous_routes
...
def test_live_accounting_mercenary_contracts_are_causally_routed(campaign):
    ...
    # A packaged host may be overdue at the exact checkpoint. Scheduler
    # preparation must repair it from the current contract owner without any
    # campaign-state edit.
    repair_core_autonomous_routes(planner, runtime)
    hosts = {
    ...
    for _path, row in _mercenary_rows(campaign):
        ...
        if mercenary_has_live_contract(row):
            ...
            cold.add(ref)
    assert live, "canonical campaign should retain at least one live mercenary obligation"
...
def test_mercenary_callback_requeues_from_new_offer_deadline_not_generic_cadence(campaign):
    class OfferPlanner(ProductionCampaignPlanner):
        def _run_due_host(self, host, due_text):
            assert host.get("kind") == "mercenary"
            ref = str(host["owner_ref"])
            path = self.owner_path(ref)
            company = copy.deepcopy(self.read(path))
            assert not mercenary_has_live_contract(company)
            company["status"] = "considering_offer"
            company.setdefault("contracts", []).append({
                "contract_ref": "contract.test.callback.offer",
    ...
    planner = OfferPlanner(campaign)
    planner._reset()
    ref = "merc.major.11"
    company = planner.read(planner.owner_path(ref))
    assert bool(company.get("accounting_only")) and not mercenary_has_live_contract(company)

    runtime = copy.deepcopy(planner.read("state/runtime.json"))
    current = CampaignTime.parse(str(runtime["world_time"]))
    due = current.add_seconds(10)
    host_id = "host_test_mercenary_callback"
    event_id = "event_test_mercenary_callback"
    runtime["hosts"] = {host_id: {
        "kind": "mercenary",
        "owner_ref": ref,
        "recurrence_seconds": 90 * 86400,
    ...
    runtime["scheduler"]["next_safety_reconcile_at"] = str(due.add_days(2))
    ...
    planner._advance_causal_runtime(str(due))
    after = planner.read("state/runtime.json")
    expected = due.add_days(1)
    assert after["hosts"][host_id]["next_due"] == str(expected)
    assert next(row for row in after["events"] if row["event_id"] == event_id)["due_at"] == str(expected)

...
def test_reconnaissance_transport_is_strict_and_hosted_time_owner_is_unchanged():
    ...
    assert ProductionTimeIntegrationMixin in HostedProductionPlanner.mro()
...
def test_unrouted_operation_response_fails_closed_and_recon_report_arrives(campaign):
    ...
    with TestClient(create_app(campaign, token)) as client:
        ...
        active_recon = [
            row for row in after_observation.get('active_player_processes', [])
            if row.get('process_ref') == recon_ref
        ]
        ...
        if courier_hours <= 0:
            # If Wei is physically with the scouts, the observation can be
            # reported immediately. Requiring an artificial courier phase here
            # would make information causality less physical, not more.
            assert active_recon == []
            assert exact['phase'] == 'completed'
            assert reports_now
            assert player_ref in commander_report['knowers']
            assert exact['report_delivered_at'] == observation_due_at
            delivery = commander_report['deliveries'][-1]
            assert delivery['source_location_ref'] == scout_location
            assert delivery['target_location_ref'] == after_observation['player']['location']
            assert delivery['travel_hours'] == 0
            return

        assert active_recon and active_recon[0]['phase'] == 'report_in_transit', {
            'active': active_recon,
            'advance_result': observed.json()['result'],
            'runtime': operations.store.read_json('state/runtime.json'),
        }
        assert not reports_now

...
from sword_runtime.store.json_fragments import split_json_fragment, select_json_fragment
...
def _read_json(root: Path, path: str):
    base, tokens = split_json_fragment(path)
    value = json.loads((root / base).read_text(encoding="utf-8"))
    return select_json_fragment(value, tokens) if tokens else value
...
def test_materialized_person_reclassifies_one_existing_force_slot(campaign: Path) -> None:
    ...
    assert _materialized_personnel(after_first, person_ref) == 1
    ...
    assert _materialized_personnel(after_repeat, person_ref) == 1
...
def test_wei_personal_recruitment_is_cohort_first_and_selective_materialization_conserves_population(campaign: Path) -> None:
    ...
    assert _materialized_personnel(after_person_force, person_ref) == 1
    owner_index = _read_json(campaign, "state/index/owner-index.json")["owners"]
    person = _read_json(campaign, owner_index[person_ref])
    assert person["schema"] == "person-lite"
    assert person.get("source_cohort_ref")
    assert "development_state" not in person or "inherited_training_baseline" not in person["development_state"]
    assert len(person["stats"]["attributes"]) == 9
    assert len(person["stats"]["skills"]) == 21
    assert "professional_skills" not in person or isinstance(person["professional_skills"], dict)
...
def test_large_wei_recruitment_campaign_selects_thousands_to_three_hundred_without_individualizing(campaign: Path) -> None:
    ...
    assert ref not in registry.get("campaigns", {}), "finalized recruitment is removed from hot state"

...
def test_wei_personal_recruitment_is_cohort_first_and_selective_materialization_conserves_population(campaign: Path) -> None:
    ...
    inherited = person["development_state"]["inherited_training_baseline"]
    assert inherited["source_cohort_ref"] == person["source_cohort_ref"]
    assert inherited["verified_training_hours_per_person"] >= 0
    assert "must never be re-settled" in inherited["rule"]

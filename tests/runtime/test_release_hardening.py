...
def test_personal_combat_training_and_recovery_use_exact_people_and_elapsed_time(campaign):
    ...
    before=CampaignTime.parse(meta(campaign)['time']); result=execute(campaign,'personal_combat',{'opponent_ref':'char_tang_zhu','objective':'controlled spar','duration_minutes':30}).receipt.result; after=CampaignTime.parse(meta(campaign)['time'])
    assert result['opponent_ref']=='char_tang_zhu' and before.seconds_until(after)==1800
...
def test_release_has_one_current_authority_tree_and_current_ci_gate(campaign):
    ...
    assert sum(1 for h in rt['hosts'].values() if h.get('kind')=='mercenary')==124  # 24 named/specialist + 100 regional exact companies
    workflow=(campaign/'.github/workflows/release.yml').read_text()
    assert 'tools/quick_check.py' in workflow
    assert 'tools/run_release_suite.py' in workflow

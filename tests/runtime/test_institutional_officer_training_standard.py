...
def test_command_ref_representation_uses_owner_schema_not_identity_prefix() -> None:
    from sword_runtime.cohort_tx_support import CohortTxSupportMixin

    class Classifier(CohortTxSupportMixin):
        def __init__(self):
            self.paths = {
                "officer.promoted.full": "state/person/promoted.json",
                "char_legacy_lite": "state/person/lite.json",
            }
            self.docs = {
                "state/person/promoted.json": {"schema": "sab_character", "owner_id": "officer.promoted.full"},
                "state/person/lite.json": {"schema": "person-lite", "id": "char_legacy_lite"},
            }
    ...
    lite, exact = classifier._ct_command_refs({
        "commander_ref": "officer.promoted.full",
        "embedded_person_refs": ["char_legacy_lite"],
    })
    assert exact == {"officer.promoted.full": "persistent_unit_commander"}
    assert lite == {"char_legacy_lite": "internal_commander"}


def test_missing_non_char_command_ref_is_not_inferred_as_person_lite() -> None:
    from sword_runtime.cohort_tx_support import CohortTxSupportMixin
    ...
    with pytest.raises(ValueError, match='command person has no authoritative owner'):
        Classifier()._ct_command_refs({
            "commander_ref": "char_legacy_missing_fixture",
            "embedded_person_refs": ["officer.phantom.missing"],
        })


def test_full_command_activity_routing_uses_owner_schema_not_storage_layout() -> None:
    from sword_runtime.service_runtime import CommandRoutedProductionPlanner

    classify = CommandRoutedProductionPlanner._is_full_exact_command_person
    assert classify(
        "state/person/promoted-officer.json",
        {"schema": "sab_character", "owner_id": "officer.promoted.full"},
    ) is True
    assert classify(
        "state/char/materialized-officer.json",
        {"schema": "sword-materialized-person", "owner_id": "officer.materialized"},
    ) is True
    assert classify(
        "state/person/materialized-officer.json",
        {"schema": "sword-materialized-person", "owner_id": "officer.materialized"},
    ) is True
    assert classify(
        "state/person/person-lite/shard.json#/records/officer.lite",
        {"schema": "person-lite", "id": "officer.lite"},
    ) is False

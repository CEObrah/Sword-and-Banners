...
def test_standing_formation_training_delegates_adaptive_development_to_person_lite_officers(campaign: Path):
    ...
    delegated = after["delegated_training_last_officer_results"]
    person_lite = delegated["person_lite"]
    trained_internal = [row for row in person_lite if row.get("role") in {"internal_1000_commander", "internal_500_commander"} and row.get("trained")]
    assert trained_internal
    assert all(row.get("skill_focuses") for row in trained_internal)
    assert all(row.get("attribute_focuses") for row in trained_internal)

    index = planner.read("state/cmd/command-personnel.json")
    officer_ref = trained_internal[0]["person_ref"]
    ...
    if officer_ref == probe_ref:
        assert officer["development_state"]["verified_training_hours"] >= probe_hours_before + 8.0
    else:
        assert officer["development_state"]["verified_training_hours"] >= 8.0
    assert officer["development_state"]["training_history"]
...
def test_sword_manor_monthly_owner_persists_and_trains_all_internal_officers(campaign: Path):
    ...
    assert record["development_state"]["verified_training_hours"] >= inherited_hours + 240.0
    assert record["development_state"]["verified_role_exposure_hours"] > 0.0
    assert record["development_state"]["training_history"]

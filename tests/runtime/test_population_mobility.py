...
def test_autonomous_jo_households_follow_real_city_headroom(campaign) -> None:
    planner = _planner(campaign)
    mobility = copy.deepcopy(planner.read("state/mobility/population-transit.json"))
    mobility["population_owner_paths"] = ["state/population/jo.json"]
    planner.put("state/mobility/population-transit.json", mobility)
    at = str(planner._world_time())
    before = copy.deepcopy(planner.read("state/population/jo.json"))

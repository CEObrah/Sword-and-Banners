...
from sword_runtime.operation_routing import current_operational_order
...
def _current_order(operation):
    order = current_operational_order(operation)
...
def _seed_active_scope(planner, *, formation_refs=None):
    ...
    if len(refs) > 1:
        ...
        # The deployment baseline is now already briefed. These tests explicitly
        # exercise the pre-briefing path, so establish that precondition here
        # instead of depending on a mutable campaign snapshot.
        ...
        for row in reversed(operation.get("operational_orders", [])):
            ...
            if order_ref and str(row.get("order_ref", "")) != order_ref:
                continue
            row["status"] = "strategic_directive_pending_operational_briefing"
            row["actionability_status"] = "pending_operational_briefing"
            row.pop("mission_packet", None)
            row.pop("staff_briefed_at", None)
            break
...
def test_operation_request_gets_playable_qin_campaign_briefing_and_mission_packet(campaign):
    ...
    assert "Other Qin forces on the campaign roster" in summary
    assert "176,800" in summary
    assert "actionable" not in summary.lower()
    assert "operation owners" not in summary.lower()
    assert "Sanyou" in summary
    assert "official estimate" in summary
    assert "opposing commanders:" in summary
    ...
    order = _current_order(operation)
...
def test_arrival_handoff_completes_only_after_all_assigned_units_reach_operational_area(campaign):
    ...
    op_path = planner.read("state/operations/index.json")["operations"][OPERATION_REF]
    operation = planner.read(op_path)
    packet = _current_order(operation)["mission_packet"]
...
def test_pending_qin_strategic_order_routes_one_automatic_staff_briefing(campaign):
    ...
    op_path = planner.read("state/operations/index.json")["operations"][OPERATION_REF]
    operation = copy.deepcopy(planner.read(op_path))
    order = copy.deepcopy(_current_order(operation))
...
def test_stale_qin_support_delivery_index_cannot_suppress_first_exact_briefing(campaign):
    planner = _planner(campaign)
    refs = list(QIN_FORMATION_REFS)
    _seed_active_scope(planner, formation_refs=refs)
    op_path = planner.read("state/operations/index.json")["operations"][OPERATION_REF]
    operation = copy.deepcopy(planner.read(op_path))
    order = copy.deepcopy(_current_order(operation))
    order["order_ref"] = "operational_order_test_stale_delivery_index"
    order["objective"] = "establish a fresh Qin screening posture while testing delivery authority"
    order["status"] = "strategic_directive_pending_operational_briefing"
    order["actionability_status"] = "pending_operational_briefing"
    order.pop("mission_packet", None)
    order.pop("staff_briefed_at", None)
    ...
    # Compute the deterministic briefing identity without persisting the
    # information owner.  The routing index is then poisoned with that exact
    # identity to reproduce the stale-index suppression bug.
    ...
    dossier = build_campaign_dossier(planner, OPERATION_REF)
    _summary = render_campaign_briefing(planner, dossier, packet)
    ...
    digest = hashlib.sha256(f"{arc_ref}|{fingerprint}".encode("utf-8")).hexdigest()[:20]
    expected_info_ref = f"information.qin_campaign_briefing.{digest}"
    assert planner.read_optional(f"state/information/{expected_info_ref}.json") is None

    planner.put(QIN_COMMAND_SUPPORT_DELIVERY_INDEX, {
        "schema": "generic-object",
        "authority": False,
        "by_operation": {OPERATION_REF: {"information_ref": expected_info_ref}},
    })

    result = settle_qin_command_support(planner, host, at)

    assert result is not None
    assert result["information_ref"] == expected_info_ref
    assert planner.read_optional(f"state/information/{expected_info_ref}.json") is not None
...
def test_campaign_follow_on_request_is_not_double_routed_as_qin_bureau_briefing(campaign):
    planner = _planner(campaign)
    _seed_active_scope(planner)
    request_id = "test-qin-follow-on-route-dedup"
    _record_attempt(
        planner,
        request_id=request_id,
        action="request",
        target_ref=OPERATION_REF,
        statement="General Mou Gou, state the next objective you require of me.",
    ...
    assert _support_hosts(runtime, request_id) == []


def test_sync_retires_preexisting_qin_support_shadow_for_campaign_follow_on_request(campaign):
    planner = _planner(campaign)
    _seed_active_scope(planner)
    request_id = "test-qin-follow-on-shadow-retire"
    _record_attempt(
        planner,
        request_id=request_id,
        action="request",
        target_ref=OPERATION_REF,
        statement="State the next objective you require of me.",
        topic="campaign_command:next_objective",
    )
    work_ref = f"interaction_attempt_{hashlib.sha256(request_id.encode('utf-8')).hexdigest()[:24]}"
    host_id = "host_qin_command_support_shadow_fixture"
    event_id = "event_qin_command_support_shadow_fixture"
    runtime = copy.deepcopy(planner.read("state/runtime.json"))
    ...
    runtime["events"].append({
        ...
        "priority": 43,
        "target_host": host_id,
        "due_at": runtime["world_time"],
    })

    sync_qin_command_support(planner, runtime)

    assert host_id not in runtime["hosts"]
    assert not any(row.get("target_host") == host_id for row in runtime["events"] if isinstance(row, dict))

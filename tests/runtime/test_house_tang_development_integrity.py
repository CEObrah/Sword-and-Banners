...
def test_integrity_sync_preserves_baseline_and_charges_sword_manor_once(campaign) -> None:
    ...
    trainees = conserved_establishment_role_count(sword, "trainee")
    materialized_trainees = sum(
        int(count)
        for person_ref, count in sword["materialized_people"].items()
        if person_ref.startswith("officer.sword_manor.trainee.")
    )
    assert trainees == 20060
    assert materialized_trainees == 60
    assert sword["authorized_by_role"]["trainee"] == trainees
    assert sword["headcount"] == 30060
    assert sum(conserved_establishment_role_count(sword, str(role)) for role in sword["authorized_by_role"]) == 30060

...
def test_pending_directive_promotes_and_physically_reaches_war_without_command_loop(campaign, tmp_path):
    planner = _planner(campaign)
    op_path = _prepare_actionable_scope(planner)
    ...
    evidence = _issue_broad_directive(planner)
    pending_ref = str(evidence["order_ref"])
    runtime_state = copy.deepcopy(planner.read("state/runtime.json"))
    sync_qin_command_support(planner, runtime_state)
    hosts = _auto_hosts(runtime_state)
    assert len(hosts) == 1

    wake = settle_qin_command_support(planner, hosts[0], str(hosts[0]["next_due"]))
    assert wake is not None
    operation = planner.read(op_path)
    assert operation["last_operational_order_ref"] == pending_ref
    promoted = next(row for row in operation["operational_orders"] if row.get("order_ref") == pending_ref)
    assert promoted["status"] == "staff_briefed_awaiting_commander_execution"
    assert promoted["actionability_status"] == "actionable"
    destination = str(promoted["mission_packet"]["destination_ref"])
    assert destination == "loc_sanyou"

    # ProductionCampaignPlanner is a transaction planner, so its test setup lives
    # in the pending write set until a runtime commit. Materialize that disposable
    # fixture before constructing the real hosted runtime used for the travel leg.
    for rel, value in planner._writes.items():
        if rel != "state" and not rel.startswith("state/"):
            continue
        path = campaign / rel
    ...
    subprocess.run(
        ["git", "-C", str(campaign), "commit", "--quiet", "-m", "Seed directive-to-war regression"],
        check=True,
    )

    runtime = ProductionSwordRuntime(campaign, runtime_root=tmp_path / "runtime-directive-to-war")
    meta = runtime.store.read_json("state/meta.json")
    command = CommandEnvelope(
        campaign_id=meta["campaign_id"],
        request_id="test-directive-to-war-e2e",
    ...
    assert {
        runtime.store.read_json(str(owners[ref]))["location_ref"]
        for ref in QIN_REFS
    } == {destination}

    arrived = runtime.store.read_json(op_path)
    assert arrived["last_operational_order_ref"] == pending_ref
    assert arrived["order_status"] not in {
        "awaiting_operational_briefing",
        "staff_briefed_awaiting_commander_execution",
    }
    assert arrived["campaign_phase"] in {"operational_area_arrival", "enemy_contact"}

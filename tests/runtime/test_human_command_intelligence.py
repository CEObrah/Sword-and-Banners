...
def test_every_assigned_strategist_contributes_not_only_first_four():
    from sword_runtime.commander_cognition import _strategist_support
    ...
    planner = Planner()
    refs = [f"staff_{i}" for i in range(4)] + ["staff_z_best"]
    for ref in refs[:4]:
        planner.people[ref] = {
            "schema": "sab_character",
            "id": ref,
            "skills": {"Strategy": 10, "Tactics": 10, "Logistics": 10, "Formation Command": 10},
        }
    planner.people["staff_z_best"] = {
        "schema": "sab_character",
        "id": "staff_z_best",
        "skills": {"Strategy": 200, "Tactics": 200, "Logistics": 200, "Formation Command": 200},
    }
    group = {"role_assignments": {ref: "strategist" for ref in refs}}
    support = _strategist_support(planner, "commander", group)
    assert support["strategist_count"] == 5
    assert support["strategist_refs"] == sorted(refs)
    assert support["strategist_refs_truncated"] is False
    assert support["support_milli"] == 900

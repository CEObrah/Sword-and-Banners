...
PFORCE_PATH = "state/pforce/wei.json"
...
class _MemoryPlanner:
    def __init__(self) -> None:
        self.docs = {
            ...
            PFORCE_PATH: {
                "assigned_formations": ["formation_red_lance_a", "formation_high_guard_infantry_01a"],
            },
            EVENT_PATH: {
    ...
    def _load_formation(self, formation_ref):
        formations = {
            ...
            "formation_high_guard_infantry_01a": {
                "formation_ref": "formation_high_guard_infantry_01a",
                "name": "High Guard Infantry 1A",
                "administrative_owner": "house_tang",
            },
        }
        return f"state/formations/{formation_ref}.json", copy.deepcopy(formations[formation_ref])
...
def _material_stub(_planner, *, formation_ref, arrow_loads_total, at):
    assert arrow_loads_total == 30
    assert at == AT
    return {
        "formation_ref": formation_ref,
        "targets": {"war_arrows": 3},
        "issued": {"war_arrows": 3},
        "shortfalls": {},
    }
...
def test_field_preparation_keeps_exact_issue_report_out_of_causal_event(monkeypatch) -> None:
    ...
    assert prep["material_issue_ref"] == EVENT_REF
    assert prep["material_issued_at"] == AT
    assert prep["shortfalls"] == report["shortfalls"]
    assert "material_issue_report" not in prep
    assert "material_issue_request_id" not in prep
    assert "manufacturing_truth" not in prep
    ...
    assert "Red Lance A receives" in event["summary"]
    assert "High Guard Infantry 1A receives" in event["summary"]

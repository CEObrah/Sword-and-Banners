...
def test_distinct_world_arc_action_reissues_same_semantics_with_new_order_identity_after_completion(campaign):
    planner = _planner(campaign)
    op_path = _prepare_player_commanded_qin_operation(planner)
    now = str(planner.read("state/runtime.json")["world_time"])
    ...
    first = planner._priority_operation_evidence(
        actor_ref="state_qin",
        action_ref="action_same_semantics_first",
        arc_ref=ARC_REF,
        goal="open offensive operations against northern Wei",
        target_ref="state_wei",
        at=now,
    ...
    first_ref = str(first["order_ref"])

    operation = copy.deepcopy(planner.read(op_path))
    first_order = next(row for row in operation["operational_orders"] if row.get("order_ref") == first_ref)
    first_order["status"] = "completed"
    first_order["actionability_status"] = "completed"
    operation["last_operational_order_ref"] = first_ref
    operation["order_status"] = "completed"
    operation["campaign_phase"] = "phase_complete"
    planner.put(op_path, operation)

    second = planner._priority_operation_evidence(
        actor_ref="state_qin",
        action_ref="action_same_semantics_second",
        arc_ref=ARC_REF,
        goal="open offensive operations against northern Wei",
        target_ref="state_wei",
        at=now,
    ...
    assert second["kind"] == "player_command_operational_order_issued"
    assert second["order_ref"] != first_ref

    after = planner.read(op_path)
    refs = [row.get("order_ref") for row in after["operational_orders"]]
    assert len(refs) == len(set(refs))
    second_order = next(row for row in after["operational_orders"] if row.get("order_ref") == second["order_ref"])
    assert second_order["source_action_ref"] == "action_same_semantics_second"

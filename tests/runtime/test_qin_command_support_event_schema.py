...
def test_player_interaction_event_kinds_must_target_tang_wei() -> None:
    event = _qin_support_event()
    event["target_ref"] = "char_tang_ling"
    errors = list(_event_validator().iter_errors(event))
    assert errors


def test_player_interaction_event_kinds_require_target_ref() -> None:
    event = _qin_support_event()
    event.pop("target_ref")
    errors = list(_event_validator().iter_errors(event))
    assert errors


def test_player_interaction_delivery_target_must_match_tang_wei() -> None:
    event = _qin_support_event()
    event["delivery"] = dict(event["delivery"])
    event["delivery"]["target_ref"] = "char_tang_ling"
    errors = list(_event_validator().iter_errors(event))
    assert errors

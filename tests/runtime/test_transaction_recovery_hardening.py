...
def test_production_house_duty_accepts_materialized_exact_personal_retainer(campaign, tmp_path):
    from sword_runtime.commands import CommandEnvelope
    from sword_runtime.service_runtime import ProductionSwordRuntime

    owners = json.loads((campaign / "state/index/owner-index.json").read_text())['owners']
    person_ref = "char_ouki"
    person_path = campaign / owners[person_ref]
    person = json.loads(person_path.read_text())
    person["schema"] = "sword-materialized-person"
    person["affiliation"] = ""
    person["current_formation_id"] = ""
    person_path.write_text(json.dumps(person, ensure_ascii=False, sort_keys=True, separators=(",", ":")) + "\n")

    pforce_path = campaign / "state/pforce/wei.json"
    pforce = json.loads(pforce_path.read_text())
    pforce.setdefault("members", []).append(person_ref)
    pforce["members"] = list(dict.fromkeys(pforce["members"]))
    pforce_path.write_text(json.dumps(pforce, ensure_ascii=False, sort_keys=True, separators=(",", ":")) + "\n")

    current = meta(campaign)
    runtime = ProductionSwordRuntime(campaign, tmp_path / "runtime-materialized-duty")
    command = CommandEnvelope(
        current["campaign_id"],
        "materialized-retainer-duty-regression",
        current["player_id"],
        "house_action",
        current["revision"],
        current["time"],
        {
            ...
            "duty": "personal escort rotation",
        },
        mode="gameplay",
    )
    runtime._validate_player_authored_agency(command)

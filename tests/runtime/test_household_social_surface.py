...
def test_co_located_parents_are_direct_scene_contacts(campaign):
    ...
    assert present["char_tang_ling"]["relation"] == "mother"
    ...
    mother_sheet = operations.person_sheet("char_tang_ling")["person"]
    assert mother_sheet["sex"] == "female"
    assert mother_sheet["pronouns"]["subject"] == "she"
    assert mother_sheet["family_role"] == "mother of Tang Wei and Tang Kai"
    ...
    assert "Family and household participants" in contract["household_scene_direction_rule"]
...
def test_direct_parent_scene_uses_owner_identity_not_char_prefix(campaign):
    _co_locate_parents(campaign)
    alias = "person.test.retained_father_identity"

    father_path = Path(campaign) / "state/char/tang-zhu.json"
    father = json.loads(father_path.read_text())
    alias_path = Path(campaign) / "state/char/retained-father-identity-test.json"
    father["id"] = alias
    alias_path.write_text(json.dumps(father, ensure_ascii=False, sort_keys=True, separators=(",", ":")) + "\n")

    owners_path = Path(campaign) / "state/index/owner-index.json"
    owners = json.loads(owners_path.read_text())
    owners["owners"][alias] = str(alias_path.relative_to(campaign))
    owners_path.write_text(json.dumps(owners, ensure_ascii=False, sort_keys=True, separators=(",", ":")) + "\n")

    family_path = Path(campaign) / "state/family/index.json"
    family = json.loads(family_path.read_text())
    links = family.get("person_index", {}).get("char_tang_wei", {}).get("parentage", [])
    replaced = False
    for ref in links:
        parentage_path = family.get("parentage", {}).get(ref)
        if not isinstance(parentage_path, str):
            continue
        path = Path(campaign) / parentage_path
        parentage = json.loads(path.read_text())
        if parentage.get("child_id") != "char_tang_wei":
            continue
        for link in parentage.get("parent_links", []):
            if isinstance(link, dict) and link.get("parent_id") == "char_tang_zhu":
                link["parent_id"] = alias
                replaced = True
    ...
    context = _ops(campaign).play_context()
    present = _present_people(context)
    assert alias in present
    assert present[alias]["relation"] == "parent"
    assert alias in context["permitted_person_ids"]
...
def test_direct_family_scene_recovers_missing_person_index_routes(campaign):
    _co_locate_parents(campaign)
    family_path = Path(campaign) / "state/family/index.json"
    family = json.loads(family_path.read_text())

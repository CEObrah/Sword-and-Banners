...
def test_co_located_parents_are_direct_scene_contacts(campaign):
    ...
    local_rule = context["scene"]["scene_local_narration_contract"]["co_located_social_rule"]
    assert "direct scene-local approach" in local_rule
    assert "not seek_contact" in local_rule
...
def test_present_parent_can_receive_a_real_consequential_interaction(campaign):
    ...
    command = CommandEnvelope(
        ...
        payload={
            ...
            "action": "present",
            "player_statement": "I want to discuss the Great Bow Guard proposal with you.",
        },
        mode="gameplay",
    )

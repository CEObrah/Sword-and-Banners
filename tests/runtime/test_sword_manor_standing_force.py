...
def test_sword_manor_is_permanent_force_not_mobilization_pool(campaign):
    ...
    assert sum(v["personnel"] for v in force["allocated_to_formations"].values()) == 30060
    ...
    assert sum(f["personnel"] for f in formations) == 30060
    assert sorted(f["personnel"] for f in formations) == sorted([1500, 3500, 5000, 5015, 5015, 5015, 5015])
    ...
    assert all(f["command_accounting"]["top_commander_inside_troop_strength"] is False for f in formations)
    assert all(f["command_accounting"]["top_deputy_inside_troop_strength"] is False for f in formations)
    assert all(f["command_accounting"]["embedded_1000_500_100_commanders_inside_troop_strength"] is True for f in formations)
...
def test_house_tang_field_army_nests_sword_manor_bastions_and_house_branches(campaign):
    ...
    assert [row["ref"] for row in bastions["units"] if row["kind"] == "nested_army"] == [
        "cmdgrp.bastion.iron_rampart",
        "cmdgrp.bastion.red_crane",
        "cmdgrp.bastion.white_lantern",
        "cmdgrp.bastion.deep_earth",
    ]

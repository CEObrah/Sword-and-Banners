...
def test_unanswered_qin_offer_lapses_and_no_longer_blocks_future_career_flow(campaign) -> None:
    ...
    assert CampaignTime.parse(expires_at) == CampaignTime.parse(at).add_seconds(7 * 86400)

    wake = settle_player_story_review(planner, {"kind": "player_story_review"}, expires_at)
    assert wake is not None
    lapse_ref = f"{offer_ref}.lapsed"
    lapse = get_causal_event(planner, lapse_ref)
    assert lapse is not None
    assert lapse["process_stage"] == "offer_lapsed_unanswered"
    assert wake["campaign_event_ref"] == lapse_ref

...
def _formation(role: str, *, count: int = 1000, arrows: int = 0, bolts: int = 0):
    return {
        ...
        "cohesion":90, "logistics":{"war_arrows":arrows,"war_bolts":bolts},
        "mounts":{},
    }
...
def test_exact_leaf_commander_and_ouki_higher_command_remain_separate_from_cohort_mean(campaign):
    from sword_runtime.engine import RepositoryCommandPlanner
    planner=RepositoryCommandPlanner(campaign)
    _, formation, force=planner._combat_prepare_formation("formation_house_ouki_household_01")
    assert formation["commander_ref"] == "char_cmd_house_ouki_household_01"
    ...
    by_ref={x["person_ref"]:x for x in snap["named_participants"]}
    leaf=by_ref["char_cmd_house_ouki_household_01"]
    ouki=by_ref["char_ouki"]
    household=by_ref["char_ouki_house_field_commander"]
    assert leaf["role"] == "commander"
    assert leaf["included_in_personnel"] is False
    ...
    assert ouki["melee_skill"] == "Polearms"
    assert ouki["melee_weapon_id"] == "weapon_glaive"
    assert ouki["melee_reach_m"] == 1.9
    assert household["role"] == "higher_commander"
    assert household["included_in_personnel"] is False
    assert snap["command_effects"]["higher_command_mode"] == "higher_commander"
    assert abs(snap["command_effects"]["higher_command_score"] - ouki["command_score"]) < 1e-9
...
def test_red_lance_exact_500_commander_and_higher_commanders_remain_separate_named_combatants(campaign):
    from sword_runtime.engine import RepositoryCommandPlanner
    planner = RepositoryCommandPlanner(campaign)
    ...
    assert by_ref["char_tang_command_red_lance_1000"]["role"] == "higher_commander"
    assert by_ref["char_tang_command_red_lance_1000"]["included_in_personnel"] is False
    assert by_ref["char_tang_wei"]["role"] == "higher_commander"
    assert by_ref["char_tang_wei"]["included_in_personnel"] is False

    # The universal exact-character threshold begins at persistent 500-man command.
    # Internal 100-man echelons remain aggregate by default and therefore do not
    # subtract or clone named bodies inside this 500-man fighting establishment.
    included_internal = [row for row in snap["named_participants"] if row.get("included_in_personnel") is True]
    assert included_internal == []
    assert snap["cohort_personnel"] == formation["personnel"] == 500
...
def test_person_lite_inside_personal_formation_is_separate_without_cloning_body(campaign):
    ...
    # Named/person-lite combat may select the loadout sidearm when its current
    # skill/handling score beats the declared primary weapon. Both remain exact
    # equipment from the canonical loadout; no weapon is materialized by combat.
    assert standout["melee_weapon_id"] in {"weapon_spear", "weapon_sword"}
    selected_weapon = planner._combat_weapon(standout["melee_weapon_id"])
    assert standout["melee_reach_m"] == selected_weapon["reach_m"]
    assert standout["ammunition_resource"] == "war_arrows"
    assert _materialized_personnel(force, "char_test_person_lite_battle") == 1
...
def test_field_battle_consumes_finite_crossbow_bolts_with_named_commander_ammo_separate(campaign):
    ...
    execute_internal(campaign,"resupply",{"formation_ref":qref,"war_arrows":5000,"war_bolts":2400})
...
def test_person_lite_can_resolve_personal_combat_with_own_stats_and_loadout(campaign):
    ...
    assert result.receipt.result['opponent_equipment']['best_weapon']=='weapon_spear'
...
def test_unified_outer_wall_exact_unit_command_and_aggregate_internal_echelons_are_mechanically_live(campaign):
    from sword_runtime.engine import RepositoryCommandPlanner
    from sword_runtime.warfare_depth import build_formation_command_structure

    planner=RepositoryCommandPlanner(campaign)
    _, formation, force=planner._combat_prepare_formation("formation_house_tang_outer_wall_iron_wall_01")
    assert formation["commander_ref"] == "char_cmd_house_tang_outer_wall_iron_wall_01"
    assert formation["composition"] == {"house_infantry": formation["personnel"]}
    assert "command_structure" not in formation
    rules = planner.read("game/data/mechanics/warfare-organization.json")
    projected = build_formation_command_structure(formation, rules)
    assert projected["unit_command"]["outside_fighting_establishment"] is True
    assert projected["unit_command"]["named_commander_ref"] == formation["commander_ref"]
    assert [row["scale"] for row in projected["internal_hierarchy"]] == [1000, 500, 100]
    assert all(row["inside_fighting_establishment"] is True for row in projected["internal_hierarchy"])

    admission=planner._combat_command_admission(formation)
    assert admission["mode"] == "exact_commander"
    assert admission["commander_ref"] == formation["commander_ref"]
    assert admission["aggregate_unit_post"] is False
    ...
    assert "char_lin_jiao" in higher
    assert "char_tang_zhu" in higher
    assert all(row["included_in_personnel"] is False for row in higher.values())
    assert snap["cohort_personnel"] == formation["personnel"]

def test_combat_derives_command_hierarchy_from_authorized_establishment(campaign):
    from sword_runtime.engine import RepositoryCommandPlanner
    planner=RepositoryCommandPlanner(campaign)
    formation={
        ...
        "cohesion":80,
        # A stale persisted projection must never override current authority.
        "command_structure":{
            "internal_hierarchy":{"summary":[{"scale":100,"count":50}]},
        },
    }
...
def test_mixed_role_battle_casualties_keep_report_formation_and_force_roles_identical(campaign):
    ...
    for side in ('a','b'):
        ...
        execute_internal(campaign,'person_materialize',{'state':'qin','person_ref':cmd,'name':f'Mixed Battle {side} Commander','birth_date':'270-BCE-01-01','role':'command_personnel','source_location_ref':location})
        execute_internal(campaign,'formation_create',{'state':'qin','formation_ref':ref,'personnel':500,'composition':composition,'location_ref':location,'commander_ref':cmd})
        execute_internal(campaign,'formation_mobilize',{'formation_ref':ref})
    op=activate_operation(campaign,'operation_mixed_battle_roles',refs,location)
...
def test_personal_equipment_burden_is_wearer_stat_derived_not_fixed(campaign):
    ...
    planner = RepositoryCommandPlanner(campaign)
    loadout = {
        'body_armor':'armor_heavy', 'helmet':'helmet_standard',
        'primary_melee_weapon':'weapon_spear', 'shield':'shield_standard',
    }

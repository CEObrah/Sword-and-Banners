...
def test_commander_and_deputy_remain_separate_named_combatants(campaign):
    ...
    assert snap["cohort_personnel"] == 495
    assert len(included_internal) == 5
    assert all(row.get("role") == "internal_100_commander" for row in included_internal)
    assert snap["cohort_personnel"] + len(included_internal) == formation["personnel"] == 500
...
def test_bastion_aggregate_unit_command_and_embedded_echelons_are_mechanically_live(campaign):
    from sword_runtime.engine import RepositoryCommandPlanner
    planner=RepositoryCommandPlanner(campaign)
    _, formation, force=planner._combat_prepare_formation("formation_bastion_iron_wall_01")
    assert formation["commander_ref"] is None
    ...
    # No phantom named commander is inferred from the derived topology. This
    # formation now has a real conserved aggregate unit-command attachment, so
    # that staffed post is mechanically live while higher command remains above it.
    assert admission["mode"] == "aggregate_unit_command"
    assert admission["commander_ref"] is None
    assert admission["aggregate_unit_post"] is True

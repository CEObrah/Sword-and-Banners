...
def _attempt(label: str, audience_ref: str, action: str, statement: str) -> dict:
    return {
...
def test_existing_disposition_host_is_enriched_before_due_settlement(campaign) -> None:
    ...
    attempt_ref = _append_attempt(planner, at, "scene_precommit_request", ask)

    host_id, event_id = _disposition_ids(attempt_ref)

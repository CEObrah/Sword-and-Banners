...
def test_production_hosted_horizon_is_bounded_atomic_windows(campaign, days: int):
    ...
    # Revision 45 carries a mature, front-loaded causal frontier. The untouched
    # merged package settles 280 events at 30 days, 825 at 90 days, and 3,418 at
    # 365 days. Those checkpoints are near-linear at roughly 9.2-9.4 events/day;
    # the older 8/day ceiling and 2,739-event comment described an earlier save.
    # Keep a 10/day slope plus a bounded startup allowance so genuine event
    # explosion remains release-blocking without pinning the test to stale history.
    assert events_processed <= (days * 10) + 125

    # Early horizons also pay a one-time owner/index routing cost. Keep the
    # per-event slope strict and model only that bounded fixed cost separately.
    # This remains an engine-complexity guard rather than a host-speed benchmark.

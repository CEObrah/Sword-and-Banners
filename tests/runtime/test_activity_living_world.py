...
def test_routed_activity_uses_canonical_hours_and_advances_named_person_attributes(campaign):
    ...
    prior_credit = float(
        duan_before["autonomous_activity_state"].get("fractional_training_hour_credit", 0.0) or 0.0
    )
    expected_hours = int(cycle_hours + prior_credit + 1e-9)
    ...
    last = duan["autonomous_activity_state"]["last_training"]

    assert last["hours"] == expected_hours
    assert last["program_ref"]
    assert "autonomous_development_history" not in duan
...
def test_annual_person_review_does_not_double_award_routed_training(campaign):
    ...
    before = deepcopy(planner.read("state/char/duan-jin.json").get("development_state", {}))

    planner._autonomy_person(person_host, 1, str(person_host["next_due"]))
    after = planner.read("state/char/duan-jin.json").get("development_state", {})
...
def test_short_chronology_steps_do_not_force_full_activity_route_rescan(campaign):
    ...
    runtime.setdefault("person_activity_routing", {})["last_route_scan_at"] = str(now)

...
def test_semantic_wait_stops_when_causal_owner_report_is_delivered(campaign):
    ...
    with TestClient(create_app(campaign, token)) as client:
        ...
        # The mature campaign may have Wei and every controlled formation at the
        # same operational location. This test is specifically about semantic
        # waiting on a causal courier source, so create a remote recipient only
        # inside this disposable fixture rather than depending on campaign geography.
        player_doc = operations.store.read_json('state/player.json')
        player_doc['location'] = 'loc_sanyou'
        player_doc['current_location'] = 'loc_sanyou'

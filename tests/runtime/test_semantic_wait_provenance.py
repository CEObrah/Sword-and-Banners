...
from sword_runtime.geography import location_chain, shortest_path
...
def test_semantic_wait_stops_when_causal_owner_report_is_delivered(campaign):
    ...
    with TestClient(create_app(campaign, token)) as client:
        ...
        # This test is specifically about semantic waiting on a causal courier
        # source. Select an actually reachable location different from the scout
        # instead of hard-coding a place that may become co-located as the
        # campaign advances. A co-located report is correctly delivered
        # immediately and cannot prove the in-transit wait contract.
        routes_doc = operations.store.read_json('game/data/world/routes.json')
        direct_candidates = []
        for route in list(routes_doc.get('routes', [])) + list(routes_doc.get('local_routes', [])):
            if not isinstance(route, dict) or 'courier' not in route.get('modes', []):
                continue
            if route.get('a') == scout_location and isinstance(route.get('b'), str):
                direct_candidates.append(route['b'])
            elif route.get('b') == scout_location and isinstance(route.get('a'), str):
                direct_candidates.append(route['a'])
        remote_location = None
        for candidate in direct_candidates:
            try:
                path = shortest_path(operations.store.read_json, scout_location, candidate, modes=('courier',))
            except ValueError:
                continue
            if int(path.get('duration_hours', 0) or 0) > 0:
                remote_location = candidate
                break
        assert isinstance(remote_location, str) and remote_location != scout_location

        player_doc = operations.store.read_json('state/player.json')
        player_doc['location'] = remote_location
        player_doc['current_location'] = remote_location
        operations.store.resolve('state/player.json').write_text(
            json.dumps(player_doc, ensure_ascii=False, indent=2) + '\n',
            encoding='utf-8',
        )
        subprocess.run(['git', '-C', str(campaign), 'add', 'state/player.json'], check=True)
        subprocess.run(
            ['git', '-C', str(campaign), 'commit', '--quiet', '-m', 'test: remote reconnaissance recipient'],
            check=True,
        )

...
def test_casualties_do_not_demote_surviving_officer_rank_inventory():
    ...
    cadre = formation["command_structure"]["officer_cadre"]
    assert cadre["rank_inventory"] == before
    assert cadre["active_billets"]["1000_commander"] == 1
    assert cadre["cadre_reserve"]["1000_commander"] == 4
    assert cadre["active_billets"]["500_commander"] == 2
    assert cadre["cadre_reserve"]["500_commander"] == 8
...
def test_verified_training_can_fill_rank_vacancies_without_changing_headcount():
    ...
    assert formation["command_structure"]["officer_cadre"]["vacant_billets"]["1000_commander"] == 0

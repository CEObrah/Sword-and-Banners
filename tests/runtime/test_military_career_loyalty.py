...
def test_formation_loyalty_is_cohort_scale_and_preserves_ownership_and_manpower(campaign):
    planner = _planner(campaign)
    path, original = planner._load_formation("formation_qin_wei_unit_01")
    ...
    at = str(planner.read("state/runtime.json")["world_time"])
    planner._update_formation_loyalty("formation_qin_wei_unit_01", at)
...
def test_causal_battle_memory_changes_loyalty_without_changing_manpower(campaign):
    ...
    history["events"].append({
        ...
        "kind": "battle_result",
        "summary": "formation_qin_wei_unit_01 suffered catastrophic mass casualty losses after being sacrificed in a failed withdrawal.",
    })
...
def test_service_entry_classifier_covers_combatant_and_staff_aspirants_without_recruiting_officials(campaign):
    ...
    assert tracks["char_shin"] == "combatant"
    assert tracks["char_hyou"] == "combatant"
    assert tracks["char_kyoukai"] == "combatant"
    assert tracks["char_karyoten"] == "staff"
...
def test_independent_service_entry_uses_current_local_state_control_and_requires_exact_location(campaign):
    planner = _planner(campaign)
    _ten_path, ten = planner._exact_person("char_karyoten", active=False)
    _kyoukai_path, kyoukai = planner._exact_person("char_kyoukai", active=False)
    assert planner._service_entry_target_state(ten) == "state_qin"
    assert planner._service_entry_target_state(kyoukai) is None

    at = str(planner.read("state/runtime.json")["world_time"])
    planner._settle_person_career("char_kyoukai", at)
    _path, after = planner._exact_person("char_kyoukai", active=False)
...
def test_staff_service_entry_reclassifies_one_conserved_body_without_granting_command(campaign):
    planner = _planner(campaign)
    at = str(planner.read("state/runtime.json")["world_time"])
    planner._settle_person_career("char_karyoten", at)
    _path, ten = planner._exact_person("char_karyoten", active=False)
    petition_ref = ten["military_career_state"]["active_petition_refs"][0]
    petition_path = planner._petition_path(petition_ref)
    petition = copy.deepcopy(planner.read(petition_path))
    ...
    _path, ten_after = planner._exact_person("char_karyoten", active=False)
    ...
    assert int(force_after["materialized_people"]["char_karyoten"]) == 1
    assert ten_after["career_state"]["current_professional_path"] == "state_military_staff_candidate"
    assert "no command appointment" in ten_after["career_state"]["office_or_command"].lower()
    assert ten_after["military_rank"]["grade"] == "recruit"
...
def test_combatant_service_entry_posts_same_conserved_recruit_to_real_formation(campaign):
    planner = _planner(campaign)
    at = str(planner.read("state/runtime.json")["world_time"])
    planner._settle_person_career("char_shin", at)
    _path, shin = planner._exact_person("char_shin", active=False)
    petition_ref = shin["military_career_state"]["active_petition_refs"][0]
    petition_path = planner._petition_path(petition_ref)
    petition = copy.deepcopy(planner.read(petition_path))
    ...
    _path, entered = planner._exact_person("char_shin", active=False)
    ...
    if petition["execution_status"] == "entered_state_military_awaiting_initial_posting":
        ...
        assert order["officer_ref"] == "char_shin"

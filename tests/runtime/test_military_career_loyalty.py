...
from sword_runtime.engine import _compact_hot_state_value
...
def _petition_test_person(planner, person_ref):
    """Return a disposable exact-person fixture without current-save petition latches.

    The maintained campaign is allowed to accumulate live career petitions. Tests
    that exercise petition creation must not assume a historical exact person is
    petition-free merely because an older save happened to be.
    """
    path, original = planner._exact_person(person_ref, active=False)
    person = copy.deepcopy(dict(original))
    career = copy.deepcopy(person.get("military_career_state", {})) if isinstance(person.get("military_career_state"), dict) else {}
    career["active_petition_refs"] = []
    person["military_career_state"] = career
    return path, original, person

def test_compacted_personnel_handoff_matches_registered_petition_schema(campaign):
    at = str(_planner(campaign).read("state/runtime.json")["world_time"])
    petition = {
        "schema": "sword-military-career-petition",
        "owner_id": "military_career_petition_compaction_contract",
        "petition_ref": "military_career_petition_compaction_contract",
        "officer_ref": "char_ordo",
        "state_ref": "state_yan",
        "request_kind": "permanent_transfer",
        "status": "authorized_handoff",
        "submitted_at": at,
        "review_due_at": at,
        "attraction_milli": 700,
        "evidence_refs": [],
        "personnel_action_handoff": {
            "required_authority_ref": "state_yan",
            "requested_action": "permanent_transfer",
            "officer_ref": "char_ordo",
            "desired_commander_ref": "char_geki_shin",
            "rule": "explanatory prose is deliberately removed from hot state",
        },
    }
    compacted = _compact_hot_state_value(petition, training_through=at)
    assert "rule" not in compacted["personnel_action_handoff"]

    schema = json.loads((campaign / "game/schemas/sword-military-career-petition.schema.json").read_text(encoding="utf-8"))
    validator_class = jsonschema.validators.validator_for(schema)
    validator_class.check_schema(schema)
    validator_class(schema).validate(compacted)
...
def test_transfer_interest_creates_petition_and_never_teleports_officer(campaign):
    planner = _planner(campaign)
    at = str(planner.read("state/runtime.json")["world_time"])
    path, original, person = _petition_test_person(planner, "char_ouki")
    ...
    petition = planner.read(planner._petition_path(petition_ref))
    assert petition["status"] == "approved_pending_commander_delivery"
    assert petition["institutional_decision"] == "approved_subject_to_prospective_commander_response"
    assert get_causal_event(planner, f"event_{petition_ref}_delivered") is None
    runtime = planner.read("state/runtime.json")
    delivery_host = next(
        host for host in runtime["hosts"].values()
        if isinstance(host, dict)
        and host.get("kind") == "military_career_petition_delivery"
        and host.get("petition_ref") == petition_ref
    )
    planner._settle_player_petition_delivery(delivery_host, delivery_host["next_due"])
    petition = planner.read(planner._petition_path(petition_ref))
...
def test_player_petition_delivery_chases_wei_if_he_moves_before_courier_arrival(campaign):
    planner = _planner(campaign)
    at = str(planner.read("state/runtime.json")["world_time"])
    path, original, person = _petition_test_person(planner, "char_ouki")
    petition_ref = planner._create_petition(
        person, state_ref="state_qin", desired_commander_ref="char_tang_wei",
        request_kind="campaign_attachment", attraction_milli=900,
        evidence_refs=[], at=at,
    )
    planner.put(path, person)
    review_at = str(CampaignTime.parse(at).add_days(15))
    ...
    runtime = planner.read("state/runtime.json")
    host_id, host = next(
        (host_id, host) for host_id, host in runtime["hosts"].items()
        if isinstance(host, dict)
        and host.get("kind") == "military_career_petition_delivery"
        and host.get("petition_ref") == petition_ref
    )
    ...
    planner._settle_player_petition_delivery(host, host["next_due"])
    assert planner.read(petition_path)["status"] == "approved_pending_commander_delivery"
    assert get_causal_event(planner, f"event_{petition_ref}_delivered") is None

    rerouted_runtime = planner.read("state/runtime.json")
    rerouted = rerouted_runtime["hosts"][host_id]
    assert rerouted["response_target_location_ref"] == "loc_kanyou"
    assert rerouted.get("response_reroute", {}).get("origin_ref") == "loc_sanyou"
    recurrence = int(rerouted.get("recurrence_seconds", 0))
    assert recurrence > 0
    arrived_at = str(CampaignTime.parse(host["next_due"]).add_seconds(recurrence))
    planner._settle_player_petition_delivery(rerouted, arrived_at)
    delivered = planner.read(petition_path)
    assert delivered["status"] == "awaiting_commander_response"
    assert get_causal_event(planner, delivered["delivered_event_ref"])["delivery"]["location_ref"] == "loc_kanyou"
...
def test_cross_state_service_is_not_treated_as_ordinary_transfer(campaign):
    ...
    person_path, original, person = _petition_test_person(planner, "char_ouki")

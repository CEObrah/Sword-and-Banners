...
def test_granary_procurement_moves_existing_grain_and_cash(campaign):
    ...
    planner._autonomy_institution({'owner_ref':'inst_qin_granary_depot_office'}, 1, '245-BCE-12-31T00:00:00+08:00')
...
def test_faction_review_uses_distinct_saved_resources_and_actions(campaign):
    ...
    planner._autonomy_faction({'owner_ref':'faction_qin_ryofui_network'}, 6, '245-BCE-12-31T00:00:00+08:00')
...
def test_occupation_tracks_control_claim_administration_loyalty_and_resistance(campaign):
    planner = planner_for(campaign); planner._reset()
    planner._occupation_initialize('loc_gyou', 'state_qin', 'state_zhao', '245-BCE-12-31T00:00:00+08:00', 'battle_example')
...
def test_nonstate_faction_actions_touch_real_linked_owners(campaign):
    ...
    planner._autonomy_faction({'owner_ref':'faction_mercenary_brokers'}, 6, '245-BCE-12-31T00:00:00+08:00')
    ...
    planner._autonomy_faction({'owner_ref':'faction_river_transport_leagues'}, 6, '245-BCE-12-31T00:00:00+08:00')
    ...
    planner._autonomy_faction({'owner_ref':'faction_martial_school_networks'}, 6, '245-BCE-12-31T00:00:00+08:00')
    ...
    planner._autonomy_faction({'owner_ref':'faction_yotanwa_confederation'}, 6, '245-BCE-12-31T00:00:00+08:00')
...
def test_interstate_peace_creates_first_class_treaty_terms(campaign):
    ...
    record.update({
        ...
        'started_at': '245-BCE-09-01T00:00:00+08:00',
    })
    ...
    assert qin['diplomacy']['zhao']['treaty_ref'] == treaty['treaty_ref']
...
def test_occupation_tracks_displacement_disease_and_food_security_pressure(campaign):
    ...
    planner._settle_occupation_administration('qin', 1, '244-BCE-03-31T00:00:00+08:00')
...
def test_autonomous_battle_losses_create_saved_state_war_burden(campaign):
    ...
    assert after['war_burden']['last_loss']['formation_ref'] == 'formation_qin_border_line'
...
def test_generic_house_lineage_close_keeps_aggregate_branch_provenance(campaign):
    ...
    planner._autonomy_house({'owner_ref': ref, 'recurrence_seconds': 7776000}, 1, '245-BCE-12-31T00:00:00+08:00')
...
def test_merchant_houses_extend_and_recover_conserved_private_credit(campaign):
    ...
    planner._autonomy_faction({'owner_ref':'faction_central_merchant_houses'}, 6, '245-BCE-12-31T00:00:00+08:00')
...
def test_house_tang_cash_close_is_transfer_based_and_defense_registry_is_live(campaign):
    planner = planner_for(campaign); planner._reset()
    defense_ref = 'contract_tang_contracted_defense'
    defense = copy.deepcopy(planner.read(planner.owner_path(defense_ref)))
    member_refs = list(defense['member_force_ids'])
    ...
    merc_before = sum(int(planner.read(planner.owner_path(ref)).get('treasury_silver', 0)) for ref in member_refs)
    total_before = treasury_before + private_before + merc_before
    planner._autonomy_house({'owner_ref':'house_tang', 'recurrence_seconds':7776000}, 1, '245-BCE-12-31T00:00:00+08:00')
    treasury = planner.read('state/treasury/treasury-house-tang.json')
    private = planner.read('state/economy/private/qin.json')
    defense_after = planner.read(planner.owner_path(defense_ref))
    merc_after = sum(int(planner.read(planner.owner_path(ref)).get('treasury_silver', 0)) for ref in member_refs)
    total_after = int(treasury['silver']) + int(private['cash_silver']) + merc_after
    assert total_after == total_before
    assert treasury['civil_finance']['cash_close_rule'].startswith('all realized revenue')
    exact_heads = sum(int(planner.read(planner.owner_path(ref)).get('headcount', 0)) for ref in member_refs)
    assert defense_after['anonymous_total'] == exact_heads
    assert defense_after['combined_total'] == exact_heads + 2  # named marshal + named deputy are external to the 75k troop count
    assert defense_after['runtime']['last_payment']['paid_silver'] > 0
    assert all(planner.read(planner.owner_path(ref))['contract_ref'] == defense_ref for ref in member_refs)

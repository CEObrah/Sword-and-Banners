...
def test_play_context_exposes_exact_player_owned_equipment_keys(campaign):
    ...
    assert context["player"]["owned_equipment_truncated"] is False
...
def test_owned_equipment_reader_keeps_true_total_before_hot_context_bound():
    class Store:
        def read_json(self, path):
            assert path == "state/player-detail/equipment-manifest.json"
            return {
                "equipment_manifest": [
                    {"item_id": f"item_{index:02d}", "quantity": 1, "custody": "player", "current_state": "stored"}
                    for index in range(70)
                ]
            }

    ops = EquipmentAwareCampaignOperations.__new__(EquipmentAwareCampaignOperations)
    ops.runtime = type("Runtime", (), {"store": Store()})()
    rows = ops._owned_equipment_view()

    assert len(rows) == 70
    assert rows[64]["item_key"] == "item_64"

...
def test_repaired_snapshot_revision_matches_provenance_and_has_no_superseded_qin_briefing_refs(campaign):
    ...
    repair = json.loads((root / "state/history/repairs/tang-inner-walls-artillery-authority-20260827.json").read_text())
    assert meta["revision"] == repair["campaign_revision"]

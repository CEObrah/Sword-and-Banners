...
def test_command_group_training_theme_cannot_invent_gain_focus(campaign):
    ...
    # The live campaign may legitimately have Lin elsewhere.  This test is
    # about deterministic gain authority, not travel/presence rejection, so
    # explicitly stage the disposable training fixture at Wei's exact location
    # instead of inheriting whatever geography the current save happens to use.
    lin_fixture = copy.deepcopy(planner.read(lin_path))
    lin_fixture["current_location"] = before_wei["current_location"]
    planner.put(lin_path, lin_fixture)
    before_lin = copy.deepcopy(lin_fixture)

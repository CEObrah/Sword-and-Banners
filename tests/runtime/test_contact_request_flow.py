...
def _contact_attempt(request_id: str, process_ref: str) -> dict:
    return {
...
def test_seek_contact_schedules_access_before_any_petition(campaign) -> None:
    ...
    request_id = "contact-test-schedule"
    _install_process_event(planner, process_ref, at)

    history = copy.deepcopy(planner.read("state/history/events/index.json"))
    attempt = _contact_attempt(request_id, process_ref)
    history["events"].append({
        "at": at,
        "event_id": "scene_test_contact_schedule",
    ...
    contact_hosts = [
        ...
        if row.get("kind") == "contact_request" and row.get("request_id") == request_id
    ]
    ...
    assert get_causal_event_from_reader(planner, _response_ref(request_id)) is None
...
def test_contact_settlement_establishes_hearing_only(campaign) -> None:
    ...
    assert route is not None
    host = {
        "request_id": "contact-test-hearing",

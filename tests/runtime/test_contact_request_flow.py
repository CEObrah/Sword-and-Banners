...
def _contact_attempt(label: str, process_ref: str, *, origin_location_ref: str = "loc_kanyou") -> dict:
    return {
        ...
        "origin_location_ref": origin_location_ref,
    }
...
def test_location_seek_contact_requires_physical_presence_at_receiving_institution(campaign) -> None:
    planner = _planner(campaign)
    at = str(planner.read("state/runtime.json")["world_time"])
    process_ref = "event_test_contact_process"
    _install_process_event(planner, process_ref, at)
    remote = _route_for_attempt(
        planner, _contact_attempt("contact-test-remote", process_ref, origin_location_ref="loc_sanyou")
    )
...
def test_contact_settlement_establishes_hearing_only(campaign) -> None:
    ...
    host = {
        ...
        "response_target_location_ref": "loc_kanyou",
    }

...
def _attempt(label: str, process_ref: str, action: str, statement: str) -> dict:
    return {
        ...
        "origin_location_ref": "loc_kanyou",
    }


def _append_attempt(planner, at: str, _event_id: str, attempt: dict) -> str:
...
def test_qualifying_request_schedules_audience_disposition(campaign) -> None:
    ...
    attempt_ref = _append_attempt(planner, at, "scene_test_audience_disposition", ask)
    ...
    hosts = [
        ...
        if row.get("kind") == "audience_disposition" and row.get("contact_ref") == attempt_ref
    ]
    assert len(hosts) == 1
    assert CampaignTime.parse(hosts[0]["next_due"]) == CampaignTime.parse(at).add_seconds(300)
    assert get_causal_event_from_reader(planner, _disposition_response_ref(attempt_ref)) is None
...
def test_completed_examination_can_recommend_without_appointing(campaign) -> None:
    ...
    attempt_ref = _append_attempt(planner, at, "scene_test_recommendation_request", ask)
    match = _match_disposition(planner, ask)
    assert match is not None

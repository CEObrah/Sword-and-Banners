...
def test_formation_movement_route_commander_and_reconstitution_are_causal(campaign):
    ...
    execute_internal(campaign,'formation_create',{'state':'qin','formation_ref':ref,'role':'line_infantry','personnel':1000,'location_ref':'loc_qin_eastern_depot'})
...
def test_split_identity_resupply_exactness_and_single_commander(campaign):
    ...
    with pytest.raises(ValueError):
        execute(campaign,'formation_split',{'formation_ref':ref,'personnel':10,'new_formation_ref':'formation_qin_wei_unit_01'})

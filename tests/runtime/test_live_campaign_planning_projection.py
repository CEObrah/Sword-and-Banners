...
def test_live_planning_overlay_counts_recursive_house_subordinates_without_reowning_them(campaign):
    ...
    # Shared bottlenecks are state-dependent: once commands have physically reached
    # their objectives, the live projection may correctly have none. Validate any
    # surfaced bottleneck rather than requiring a pre-arrival historical shape.
    for load in planning["shared_bottlenecks"]:
        throughput = int(load.get("daily_troop_throughput", 0) or 0)
        assert len(load.get("command_refs", [])) >= 2 or throughput <= 0 or int(load["combined_strength"]) > throughput
...
def test_compact_live_context_keeps_campaign_decisions_and_drops_redundant_bulk(campaign):
    ...
    assert planning["shared_bottleneck_count"] == len(full_operation["campaign_command"]["march_planning"]["shared_bottlenecks"])

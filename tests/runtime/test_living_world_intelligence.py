...
def test_autonomous_state_uses_bounded_memory_and_objective_fit(campaign: Path) -> None:
    ...
    assert len(memory["formation_memory"]) <= _MAX_FORMATION_MEMORY
    assert "formation_qin_border_line" not in memory["formation_memory"]
    ...
    # first two blueprint entries as the earlier fixed-window approach did.
...
def test_current_campaign_120_day_replay_is_deterministic(campaign: Path, tmp_path: Path) -> None:
    # Run the two disposable replays sequentially and remove the first clone before
    # materializing the second.  The assertion still exercises same-process
    # determinism, while avoiding two complete Git-backed campaign copies competing
    # for filesystem cache and fsync bandwidth in constrained CI/container disks.
    snapshots = []
    source_digest = _state_digest(campaign)
    for label in ("a", "b"):
        replay = tmp_path / f"replay-{label}"
        runtime_root = tmp_path / f"runtime-{label}"
        # Clone the exact one-commit disposable campaign locally.  ``--shared``
        # reuses immutable Git objects but gives the replay an independent index,
        # HEAD, refs, and working tree, so replay commits cannot alias the source
        # fixture's Git metadata.
        import subprocess
        subprocess.run(["git", "clone", "--quiet", "--shared", str(campaign), str(replay)], check=True)
        digest = _advance_horizon(replay, runtime_root, 120)
        meta = ProductionSwordRuntime(replay, runtime_root=tmp_path / f"runtime-{label}-read").store.read_json("state/meta.json")
        snapshots.append((digest, int(meta["revision"]), str(meta["time"])))
        shutil.rmtree(replay, ignore_errors=True)
        shutil.rmtree(runtime_root, ignore_errors=True)
        shutil.rmtree(tmp_path / f"runtime-{label}-read", ignore_errors=True)

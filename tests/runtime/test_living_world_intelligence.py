...
def test_current_campaign_120_day_replay_is_deterministic(campaign: Path, tmp_path: Path) -> None:
    # Run the two disposable replays sequentially inside pytest's isolated
    # basetemp.  Do not recursively delete Git-backed clones inside the assertion
    # loop: write-heavy long-horizon replays can spend longer tearing down object
    # stores than simulating.  Basetemp owns cleanup after certification.
    snapshots = []

...
def test_interstate_contact_commits_resumable_wake_before_battle(campaign: Path) -> None:
    ...
    for formation_ref in (qi_ref, han_ref):
        ...
        # The formation/commander continuity rules are exact authority. A test
        # fixture that teleports a formation but leaves its commander behind
        # correctly produces ``commander_detached`` instead of contact. Move the
        # existing exact commander with the disposable formation so this test
        # exercises the intended interstate-contact wake boundary rather than an
        # impossible command-state fixture.
        commander_ref = formation.get("commander_ref")
        if isinstance(commander_ref, str) and commander_ref and commander_ref != meta["player_id"]:
            commander_path = owner_index.get(commander_ref)
            if isinstance(commander_path, str):
                commander = _read_json(campaign, commander_path)
                commander["location"] = target_location
                commander["current_location"] = target_location
                _write_json(campaign, commander_path, commander)
    player = _read_json(campaign, "state/player.json")
    player["location"] = target_location
    _write_json(campaign, "state/player.json", player)

    # Contact wakes are a hosted-chronology contract.  The causal living-world
    # layer owns contact detection, while ProductionTimeIntegrationMixin owns
    # the hard interruption and resumable scheduler state.
    ...
    planner2 = ProductionCampaignPlanner(campaign)

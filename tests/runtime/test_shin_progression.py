...
def test_shin_routes_to_current_combined_arms_commander_program(campaign):
    ...
    assert program_ref == "program.commander_combined_arms"
    # Shin now commands the Hi Shin command group rather than double-hatting a
    # leaf formation. His explicit closed commander program therefore owns the
    # activity context without inventing a leaf formation training_ref/role.
    assert training_ref == ""
    assert role == ""


def test_shin_monthly_activity_uses_only_combined_arms_commander_domains(campaign):
    planner = ProductionCampaignPlanner(campaign)
    planner._reset()
    ...
    assert last["program_ref"] == "program.commander_combined_arms"
    registry = planner.read("game/data/mil/deterministic-training-programs.json")
    allowed = _allowed_skills(registry, "program.commander_combined_arms")
    developed = {
        str(skill)
...
def test_shin_current_progression_state_is_compact_and_preserved(campaign):
    ...
    assert shin["activity_contract"]["training_program_ref"] == "program.commander_combined_arms"

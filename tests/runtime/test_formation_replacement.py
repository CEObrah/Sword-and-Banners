...
def test_stale_house_force_index_cannot_route_foreign_force_into_house_review(campaign):
    from sword_runtime.production_planner import ProductionCampaignPlanner

    planner = ProductionCampaignPlanner(campaign); planner._reset()
    foreign_path = "state/forces/house_karin_house.json"
    assert planner.read(foreign_path)["administrative_owner"] == "house_karin_house"
    index = copy.deepcopy(planner.read("state/index/house-force-index.json"))
    index.setdefault("house_force_paths", {}).setdefault("house_tang", []).append(foreign_path)
    planner.put("state/index/house-force-index.json", index)

    paths = planner._house_force_paths("house_tang")
    assert "state/forces/house-tang.json" in paths
    assert foreign_path not in paths
    assert "state/forces/sword-manor.json" not in paths

...
class Harness(CombatCapabilityMixin):
    def __init__(self):
        ...
        self.doctrine_ref = "doc.test.hero.personal_combat"
        self.doctrine_record = {
            "schema": "doctrine-record",
            "id": self.doctrine_ref,
            "doctrine": {
                "domain": "personal_combat",
                "targeting": {
                    "lethal_priority": [
    ...
    def read(self, path):
        if path == "game/data/mil/doctrines.json":
            return {"record_index": {self.doctrine_ref: "game/data/mil/doctrine-records/test-hero.json"}}
        if path == "game/data/mil/doctrine-records/test-hero.json":
...
def _hero(role="notable"):
    return {
        ...
        "strength": 180.0,
        "awareness": 180.0,
        "combat_doctrine_ref": "doc.test.hero.personal_combat",
    }
...
def test_hero_micro_window_is_bounded_and_uses_saved_targeting_doctrine():
    ...
    assert row["aim_selection_basis"] == "registered_combat_doctrine"

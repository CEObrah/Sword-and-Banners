...
def test_reference_bands_are_not_progression_boundaries() -> None:
    assert MODEL["capability_scale"]["legendary_reference"] == 200
    assert MODEL["aptitude_scale"]["legendary_reference"] == 200
...
def test_saved_skill_above_reference_is_valid_and_not_clamped() -> None:
    person = _person(217, bank=0.0)
    result = settle_skill_training(person, "Sword", 12, AT, TRAINING)
    assert person["skills"]["Sword"] == 217
    assert result["skill_score"] == 217
    assert "skill_hard_cap" not in result


def test_evidence_backed_breakthrough_crosses_reference_band_and_consumes_evidence() -> None:

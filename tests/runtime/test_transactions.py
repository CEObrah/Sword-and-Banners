...
def test_unpartitioned_wal_files_are_not_adopted_by_release_runtime(campaign):
    from sword_runtime.engine import SwordRuntime

    runtime = SwordRuntime(campaign)
    stray = runtime.coordinator.wal.directory / 'unrelated-layout.json'
    stray.write_text('{"state":"prepared","request_id":"unrelated-layout"}\n')

    restarted = SwordRuntime(campaign)
    assert restarted.recover() == ()
    assert stray.exists()
    assert not tuple(restarted.coordinator.wal.pending_directory.glob('*.json'))


def test_pending_whole_owner_delete_masks_baseline_and_fragment_caches(campaign):
    from sword_runtime.production_planner import ProductionCampaignPlanner

    planner = ProductionCampaignPlanner(campaign)
    planner._reset()
    path = "state/runtime.json"
    fragment = path + "#/scheduler"

    # Prime both cache shapes before staging the delete.
    assert isinstance(planner.read(path), dict)
    assert isinstance(planner.read(fragment), dict)
    planner.delete(path)

    with pytest.raises(FileNotFoundError):
        planner.read(path)
    with pytest.raises(FileNotFoundError):
        planner.read(fragment)
    assert planner.read_optional(path) is None
    assert planner.read_optional(fragment) is None


def test_planner_pending_whole_owner_delete_masks_baseline_and_cached_fragments(campaign):
    from sword_runtime.production_planner import ProductionCampaignPlanner

    planner = ProductionCampaignPlanner(campaign)
    owner_path = "state/runtime.json"
    fragment_path = "state/runtime.json#/world_time"

    assert isinstance(planner.read(owner_path), dict)
    assert isinstance(planner.read(fragment_path), str)

    planner.delete(owner_path)

    with pytest.raises(FileNotFoundError):
        planner.read(owner_path)
    with pytest.raises(FileNotFoundError):
        planner.read(fragment_path)
    assert planner.read_optional(owner_path) is None
    assert planner.read_optional(fragment_path) is None

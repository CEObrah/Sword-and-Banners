...
def test_tang_manor_uses_shared_land_labor_food_close(campaign):
    ...
    assert row["resident_population"] == 725000
    ...
    assert row["production_runtime"]["last_land_basis"]["agriculture_km2"] == 4300.0
    assert row["production_runtime"]["last_labor_basis"]["productive_land_access"]["factor"] <= 1.0
...
def test_tang_manor_commute_access_is_physical_not_owner_bonus(campaign):
    ...
    assert out.get("resident_site_ref") == "loc_sword_manor"

...
def _create_tang_siege_pair(campaign):
    ...
    execute_internal(campaign,'formation_create',{
        'state':'qin','formation_ref':'formation_siege_physics_qin','composition':{'line_infantry':900,'siege_engineering':100},
        'personnel':1000,'commander_ref':qcmd,'location_ref':qloc,
    })
    execute_internal(campaign,'formation_create',{
        'state':'zhao','formation_ref':'formation_siege_physics_zhao','composition':{'line_infantry':800,'siege_engineering':200},
    ...
    for depot_name,units in [('qin',5000),('zhao',500)]:
    ...
    execute_internal(campaign,'resupply',{'formation_ref':'formation_siege_physics_zhao','timber_tonnes':200,'iron_tonnes':12,'construction_material_units':300})
    execute_internal(campaign,'resupply',{'formation_ref':'formation_siege_physics_qin','construction_material_units':5000})
    prepare_field_formation(campaign,'formation_siege_physics_qin','loc_tang_manor',food_per_person=40)
    prepare_field_formation(campaign,'formation_siege_physics_zhao','loc_tang_manor',food_per_person=40)
    execute_internal(campaign,'fortification_materialize',{
        'fortification_ref':'fort_tang_physics','location_ref':'loc_tang_manor',
        'garrison_formation_refs':['formation_siege_physics_qin'],'food_kg':10000,'state':'qin','commander_ref':qcmd,
    })
...
def test_tang_wall_rejects_short_ladders_and_grapnel_never_grants_ram_access(campaign):
    _q,z=_create_tang_siege_pair(campaign)
    # A full wall-sector assault needs a 70 m crossing for wheeled/ladder approach.
    for _ in range(4):
        execute_internal(campaign,'siege_action',{
            'siege_ref':'siege_tang_physics','action':'build_work','blueprint_ref':'siege_trestle_bridge_20m',
    ...
    # The registered 30 m grapnel can reach the 24 m wall directly after a swim,
...
def test_nested_tang_fortifications_are_exact_and_route_to_their_own_perimeters(campaign):
    ...
    sword=master['sword_manor']; inner=master['inner_citadel']
    assert sword['plan_length_m']*sword['plan_width_m']/1_000_000 == 60.0
    assert 2*(sword['plan_length_m']+sword['plan_width_m'])/1000 == 32.0
    assert sword['tower_station_interval_m']*sword['tower_count']/1000 == 32.0
    assert inner['plan_length_m']*inner['plan_width_m']/1_000_000 == 10.0
    assert 2*(inner['plan_length_m']+inner['plan_width_m'])/1000 == 13.0
    assert inner['tower_station_interval_m']*inner['tower_count']/1000 == 13.0

    assert enclosing_fortification_site(read,'loc_sword_manor_outer_gate') == 'loc_sword_manor'
    ...
    sword_fort={'profile':by_site['loc_sword_manor'],'physical_state':initial_physical_state(by_site['loc_sword_manor'])}
    inner_fort={'profile':by_site['loc_tang_inner_citadel'],'physical_state':initial_physical_state(by_site['loc_tang_inner_citadel'])}
    assert sword_fort['physical_state']['model']=='exact_perimeter'
    assert sword_fort['physical_state']['perimeter']['wall_height_m']==26.0
    assert required_crossing_length(sword_fort,'gate')==18.0
    assert required_crossing_length(sword_fort,'wall')==45.0

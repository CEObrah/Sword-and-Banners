...
def test_legacy_qin_appointment_routes_current_operation_from_command_group_context(campaign) -> None:
    ...
    due = CampaignTime.parse(str(hosts[0]["next_due"]))
    assert due > current.add_hours(1)
    assert int(hosts[0]["communication_travel_seconds"]) > 0
    planner.put("state/runtime.json", runtime)

    planner._active_command_type = "advance_time"
    early = planner._advance_causal_runtime(str(current.add_hours(1)))
    assert early["campaign_event_notices"] == []
    assert planner.read(op_path)["order_status"] == "awaiting_operational_briefing"
    result = planner._advance_causal_runtime(str(due))
    ...
    target_order = next(
        row for row in operation["operational_orders"]
        if row.get("order_ref") == "operational_order_test_legacy_scope_auto_briefing"
    )
    assert target_order["actionability_status"] == "completed"
...
def test_normal_advance_time_auto_briefs_legacy_qin_appointment(campaign) -> None:
    ...
    assert len(auto_hosts) == 1
    due = CampaignTime.parse(str(auto_hosts[0]["next_due"]))
    assert due > current.add_hours(1)
    planner.put("state/runtime.json", runtime)
    ...
    execute_hosted_production(
        campaign, "advance_time", {"target_time": str(due)},
    ...
    target_order = next(
        row for row in operation["operational_orders"]
        if row.get("order_ref") == "operational_order_test_legacy_scope_public_advance"
    )
    assert target_order["actionability_status"] == "actionable"

...
def command(planner):
    ...
    return SimpleNamespace(actor_id=planner.PLAYER_ACTOR, expected_revision=int(meta["revision"]), command_type="polity_action", digest="political-depth-test", semantic_digest="political-depth-test", submitted_at=str(meta["time"]), mode="gameplay")
...
def test_court_admits_exact_evidence_and_keeps_finding_separate_from_judgment(campaign):
    ...
    relationship=decided["judgment_evidence_relationship"]
    assert relationship["decision"]=="dismiss"
    assert relationship["legal_support_band"]==decided["evidentiary_assessment"]["legal_support_band"]
    assert isinstance(relationship["diverges_from_evidentiary_support"], bool)
    assert "rule" not in relationship

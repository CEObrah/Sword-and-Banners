...
def test_formation_credit_settlement_consumes_whole_hours_without_time(campaign):
    ...
    for person_ref in champion_100_refs:
        ...
        history = dev.get("training_history", [])
        matching = [
            row for row in history
            if str(row.get("evidence_ref", "")).startswith("standing_training_settle:test-formation-standing-settle")
        ]
        assert matching
        assert float(matching[-1].get("requested_deliberate_hours", 0.0)) == 2.0
        assert 0.0 < float(matching[-1].get("deliberate_hours", 0.0)) <= 2.0
    assert str(planner.read("state/runtime.json")["world_time"]) == current_text

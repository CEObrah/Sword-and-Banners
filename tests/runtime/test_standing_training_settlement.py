...
def test_formation_credit_settlement_consumes_whole_hours_without_time(campaign):
    ...
    assert capability["aggregate_represented_personnel"] == 495
    assert capability["person_lite_represented_personnel"] == 5
    assert capability["total_represented_personnel"] == 500
    command_index = planner.read("state/cmd/command-personnel.json")["record_index"]
    champion_100_refs = [f"officer.house_tang.tang_champion.100.{idx:03d}" for idx in range(1, 6)]
    for person_ref in champion_100_refs:
        person = planner.read(command_index[person_ref])
        dev = person.get("development_state", {})
        assert float(dev.get("verified_training_hours", 0.0)) > 0.0
        last = dev.get("last_training", {})
        assert str(last.get("evidence_ref", "")).startswith("standing_training_settle:test-formation-standing-settle")
        assert 0.0 < float(last.get("deliberate_hours", 0.0)) <= 2.0
        assert last.get("formation_ref") == "formation_tang_champions_first"
        assert "training_history" not in dev
    assert str(planner.read("state/runtime.json")["world_time"]) == current_text

...
def test_state_ministries_act_with_exact_money_and_local_resources(campaign):
    planner = planner_for(campaign); planner._reset(); at = str(planner.read("state/runtime.json")["world_time"])
    fp, formation = planner._load_formation("formation_qin_border_line"); formation = copy.deepcopy(formation); formation["readiness"] = 30; formation["command_authority"] = "state_qin"; formation["commander_ref"] = None; planner.put(fp, formation)
    ...
    assert "formation_qin_border_line" in planner.read(planner.owner_path("inst_qin_military_bureau"))["military_review"]["trained_formations"]
...
def test_nonadjacent_defensive_obligation_can_create_access_aware_expeditionary_theater(campaign):
    ...
    assert expeditionary["target_location_ref"].startswith("loc_chu_")

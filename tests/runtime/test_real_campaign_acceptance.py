...
def test_personal_gameplay_and_exact_retail(campaign):
    ...
    # Direct relationship changes are face-to-face mechanics.  Lin Zhen is in
    # Kanyou in the current canonical fixture, while Wei is still in the Tang
    # family hall at this point in the flow.  Use a genuinely co-located family
    # member so this acceptance probe tests the command rather than bypassing
    # the physical-presence invariant it is meant to protect.
    execute(campaign,'relationship_change',{'target_ref':'char_tang_kai','kind':'trust','delta':2})

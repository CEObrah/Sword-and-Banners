...
def test_sword_manor_and_champions(campaign):
    ...
    assert first['mounts']=={'horse_tang_heavy_war':500}
...
def test_army_lifecycle_and_population_conservation(campaign):
    ...
    assigned={str(ref) for ref,row in force.get('materialized_assignments',{}).items() if isinstance(row,dict) and row.get('formation_ref')}
    ...
    assert force['headcount']==sum(force['available_by_role'].values())+sum((v['personnel'] if isinstance(v,dict) else v) for v in force['allocated_to_formations'].values())+unassigned_materialized
...
def test_private_household_battle_losses_reconcile_population(campaign):
    ...
    champion=json.load(open(champion_path)); logistics=dict(champion['logistics']); logistics['food_kg']=500; logistics['fodder_kg']=400

import json
import subprocess
from conftest import execute, execute_internal, execute_production, execute_hosted_production, meta, prepare_field_formation, activate_operation, move_formation_internal

def _co_located_relationship_target(campaign):
    player = json.load(open(campaign / 'state/player.json'))
    player_ref = player.get('owner_id', 'char_tang_wei')
    player_loc = player.get('location') or player.get('current_location')
    relationships = json.load(open(campaign / 'state/relationships.json'))
    ...
    for edge in relationships.get('edges', []):
        ...
        other = target if source == player_ref else source if target == player_ref else None
        if isinstance(other, str) and other != player_ref and other not in candidates:
            candidates.append(other)
    for ref in candidates:
        path = owners.get(ref)
        if not isinstance(path, str):
            continue
        person = json.load(open(campaign / path))
        location = person.get('location') or person.get('current_location') or person.get('location_ref')
        if location == player_loc and person.get('life_status', 'active') != 'dead':
            return ref
    raise AssertionError('fixture needs one exact relationship target co-located with Tang Wei')
...
def test_personal_gameplay_and_exact_retail(campaign):
    ...
    # Direct relationship changes are face-to-face mechanics. The mature save
    # moves companions and relatives over time, so select a relationship target
    # who is physically co-located in this disposable fixture instead of pinning
    # a historical home-base location.
    relationship_target = _co_located_relationship_target(campaign)
    execute(campaign,'relationship_change',{'target_ref':relationship_target,'kind':'trust','delta':2})
...
def test_state_house_institution_autonomy(campaign):
    ...
    house_due=CampaignTime.parse(runtime['hosts']['host_house_tang']['next_due'])
    target=str(house_due.add_days(1))
...
def test_state_reacts_to_known_enemy_action(campaign):
    ...
    op=json.load(open(campaign/'state/operations/operation_auto_zhao_border_response.json')); assert op['autonomous'] is True and op['status'] in {'mobilizing','active'}
...
def test_private_household_battle_losses_reconcile_population(campaign):
    ...
    # Materialize and form the disposable Qin opponent where conserved Qin bodies
    # actually exist, then move the real formation to the battle site. The mature
    # campaign no longer keeps a spare Qin command cohort at Wei's field location,
    # and the test must not mint one merely to arrange a casualty probe.
    qin_source_location='loc_qin_eastern_depot'
    execute_internal(campaign,'person_materialize',{'state':'qin','person_ref':opponent_cmd,'name':'House Battle Qin Commander','birth_date':'270-BCE-01-01','role':'command_personnel','source_location_ref':qin_source_location})
    execute_internal(campaign,'formation_create',{'state':'qin','formation_ref':'formation_house_opponent','role':'line_infantry','personnel':5000,'location_ref':qin_source_location,'commander_ref':opponent_cmd})
    # The disposable opponent is pure line infantry; no missile stock is required
    # for casualty/population reconciliation. Do not conjure arrows into its cache.
    execute_internal(campaign,'formation_mobilize',{'formation_ref':'formation_house_opponent'})
    move_formation_internal(campaign,'formation_house_opponent',battle_location)

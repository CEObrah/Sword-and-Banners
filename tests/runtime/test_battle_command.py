...
def test_battle_plan_groups_tang_wei_state_and_house_troops_without_transferring_ownership(campaign):
    ...
    wei_mission = next(row for row in qin["missions"] if row.get("recipient_ref") == "char_tang_wei")
    assert set(wei_mission["formation_refs"]) == set(TANG_WEI_FORMATIONS)
    assert wei_mission["personnel"] == 9500
    assert len({bf["assignments"][ref]["sector_ref"] for ref in TANG_WEI_FORMATIONS}) == 1

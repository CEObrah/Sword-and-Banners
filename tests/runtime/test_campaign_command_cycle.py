...
from sword_runtime.scene_sessions import active_scene_session, attach_open_question
...
def test_supreme_commander_identity_uses_full_person_schema_not_char_prefix(campaign):
    planner = ProductionCampaignPlanner(campaign); planner._reset()
    person_ref = "officer.test.promoted.supreme"
    person_path = "state/person/promoted-supreme-test.json"
    person = copy.deepcopy(planner.read(planner.owner_path("char_shou_hei_kun")))
    person["owner_id"] = person_ref
    person["id"] = person_ref
    person["name"] = "Promoted Supreme Test"
    planner.put(person_path, person)
    planner._register_owner(person_ref, person_path)
    assert _supreme_commander(planner, {"campaign_commander_ref": person_ref}) == person_ref
    assert _supreme_commander(
        planner,
        {"campaign_commander_ref": "officer.qin.kankoku.army.chief_of_staff"},
    ) is None
...
def _planner(campaign):
    ...
    # These regressions exercise creation of a campaign-command cycle. The live
    # campaign save may already have held this council, so normalize only the
    # disposable test copy back to a pre-council command state instead of tying
    # invariant tests to the current playthrough snapshot.
    ...
    for key in (
        "campaign_command_cycle_ref", "campaign_commander_ref",
        "campaign_command_assignment", "campaign_command_directives",
        "last_campaign_command_directive_ref", "last_campaign_command_directive_at",
    ):
    ...
    # Re-stage the operation authority itself at the formal pre-entry council
    # venue. Moving only Wei would leave the operation's HQ in the mature live
    # field location, correctly causing the runtime to choose a field military
    # headquarters instead of the royal-court forum this fixture is testing.
    operation["location_ref"] = "loc_kanyou"
    ...
    if isinstance(cycle_ref, str) and cycle_ref:
        ...
        if isinstance(owners, dict):
            owners.pop(cycle_ref, None)
            planner.put("state/index/owner-index.json", owner_index)

    runtime = copy.deepcopy(planner.read("state/runtime.json"))
    hosts = runtime.get("hosts") if isinstance(runtime, dict) else None
    if isinstance(hosts, dict):
        runtime["hosts"] = {
            key: value for key, value in hosts.items()
            if not (isinstance(value, dict) and str(value.get("kind", "")).startswith("campaign_command_"))
        }
    events = runtime.get("events") if isinstance(runtime, dict) else None
    if isinstance(events, list):
        runtime["events"] = [
            row for row in events
            if not (isinstance(row, dict) and str(row.get("kind", "")).startswith("campaign_command_"))
        ]
...
def test_untouched_war_council_session_closes_at_soft_end_and_returns_commanders(campaign):
    planner = _planner(campaign)
    runtime = _register_cycle(planner)
    _result, _council_at = _hold_council(planner, runtime)

    session = active_scene_session(planner)
    assert session is not None
    assert session["last_updated_at"] == session["started_at"]

    cycle_ref = campaign_command_projection(planner, OPERATION_REF)["cycle_ref"]
    before_cycle = planner.read(planner.owner_path(cycle_ref))
    venue_ref = before_cycle["venue_ref"]
    expected_returns = {
        row["person_ref"]: row["origin_ref"]
        for row in before_cycle["war_council"].get("travel_obligations", [])
        if isinstance(row, dict)
        and isinstance(row.get("person_ref"), str)
        and isinstance(row.get("origin_ref"), str)
        and row.get("origin_ref") != venue_ref
        and planner.read(planner.owner_path(row["person_ref"])).get("current_location") == venue_ref
    }

    runtime = planner.read("state/runtime.json")
    return_host = next(row for row in runtime["hosts"].values() if row.get("kind") == "campaign_command_council_return")
    settle_campaign_command_host(planner, return_host, return_host["next_due"])

    closed = planner.read("state/index/active-scene-session.json")
    assert closed["status"] == "closed"
    assert closed["close_reason"] == "completed"
    after_cycle = planner.read(planner.owner_path(cycle_ref))
    assert after_cycle["war_council"]["return_completed_at"] == return_host["next_due"]
    assert set(after_cycle["war_council"]["returned_person_refs"]) == set(expected_returns)
    for person_ref, origin_ref in expected_returns.items():
        assert planner.read(planner.owner_path(person_ref)).get("current_location") == origin_ref


def test_engaged_war_council_session_defers_soft_return_without_ejecting_attendees(campaign):
    planner = _planner(campaign)
    runtime = _register_cycle(planner)
    ...
    session = active_scene_session(planner)
    assert session is not None
    attach_open_question(planner, "interaction_attempt_campaign_council_test", at=council_at)
    touched = active_scene_session(planner)
    assert touched is not None
    assert touched["last_updated_at"] == council_at
    assert touched["open_question_refs"] == ["interaction_attempt_campaign_council_test"]

    runtime = planner.read("state/runtime.json")
    return_host = next(row for row in runtime["hosts"].values() if row.get("kind") == "campaign_command_council_return")
    settle_campaign_command_host(planner, return_host, return_host["next_due"])

    assert active_scene_session(planner) is not None
    cycle_ref = campaign_command_projection(planner, OPERATION_REF)["cycle_ref"]
    after_cycle = planner.read(planner.owner_path(cycle_ref))
    assert after_cycle["war_council"]["return_deferred_reason"] == "active_scene_session"
    assert "return_completed_at" not in after_cycle["war_council"]
    live_host = planner.read("state/runtime.json")["hosts"][return_host["host_id"]]
    assert int(live_host["recurrence_seconds"]) > 0
...
def test_routine_command_bookkeeping_does_not_force_downtime_boundaries():
    ...
    assert "campaign_command_dawn_briefing" not in _PLAYER_FACING_EVENT_KINDS
    assert "campaign_command_evening_sitrep" not in _PLAYER_FACING_EVENT_KINDS
...
def test_remote_superior_order_uses_physical_one_way_courier_before_delivery(campaign, monkeypatch):
    import sword_runtime.campaign_command_cycle as cycle_module
    ...
    # Put Wei and Mou Gou at different exact locations after the council. The
    # order may be issued now, but it is not received until the one-way courier
    # plus headquarters handling time has elapsed.
    player = copy.deepcopy(planner.read("state/player.json"))
    player["location"] = "loc_sanyou"
    player["current_location"] = "loc_sanyou"
    planner.put("state/player.json", player)
    mou_path = planner.owner_path("char_mou_gou")
    mou = copy.deepcopy(planner.read(mou_path))
    mou["current_location"] = "loc_qin_regional_01"
    planner.put(mou_path, mou)
    ...
    monkeypatch.setattr(
        ...
        lambda _read, origin, destination, *, round_trip=False: {
            ...
            "route_refs": ["route.test.superior_order"],
    ...
    operation = copy.deepcopy(planner.read(op_path))
    order = {
        "order_ref": "operational_order_test_remote_delivery",
        "issued_at": council_at,
        "issuer_ref": "state_qin",
        "superior_commander_ref": "char_mou_gou",
        "coordination_authority_ref": "inst_qin_military_bureau",
        "objective": "hold Sanyou until the eastern screen reports",
        "status": "issued_awaiting_commander_execution",
        "actionability_status": "actionable",
        "applies_to_formation_refs": list(operation["operational_orders"][-1].get("applies_to_formation_refs", [])),
    ...
    expected_due = str(CampaignTime.parse(council_at).add_seconds(3600 + 15 * 60))
    assert host["next_due"] == expected_due
    assert host["source_location_ref"] == "loc_qin_regional_01"
    assert host["target_location_ref"] == "loc_sanyou"
    assert host["communication_travel_seconds"] == 3600
    assert host["courier_route"]["round_trip"] is False
    cycle = planner.read(planner.owner_path(campaign_command_projection(planner, OPERATION_REF)["cycle_ref"]))
    assert order["order_ref"] not in cycle["delivered_superior_order_refs"]


def test_unroutable_superior_order_does_not_fall_back_to_fixed_delay(campaign, monkeypatch):
    import sword_runtime.campaign_command_cycle as cycle_module
    ...
    operation = copy.deepcopy(planner.read(op_path))
    order = {
        "order_ref": "operational_order_test_unroutable_delivery",
        "issued_at": council_at,
        "issuer_ref": "state_qin",
        "superior_commander_ref": "char_mou_gou",
        "objective": "test fail-closed routing",
        "status": "issued_awaiting_commander_execution",
        "actionability_status": "actionable",
    ...
    monkeypatch.setattr(
        cycle_module,
        "command_message_route",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(ValueError("no physical route")),
    )
    runtime = copy.deepcopy(planner.read("state/runtime.json"))
    runtime["world_time"] = council_at
    sync_campaign_command_cycle(planner, runtime)
    assert not any(
        row.get("kind") == "campaign_command_superior_order"
        and row.get("phase_instance_ref") == order["order_ref"]

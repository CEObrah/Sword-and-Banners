...
def _exact(name: str, *, sword: int, training: int, spear: int = 0, location: str = "loc_dojo") -> dict:
    return {
        "schema": "sab_character",
        "owner_id": name,
        "current_location": location,
        "health": "healthy",
        "skills": {"Sword": sword, "Spear": spear, "Training": training},
        "attributes": {"Coordination": 80},
...
def test_relevant_domain_mastery_beats_unrelated_high_skill() -> None:
    ...
    runtime = MemoryRuntime({
        "char_good": _exact("char_good", sword=180, training=120, spear=0),
        "char_wrong": _exact("char_wrong", sword=20, training=120, spear=240),
    })
    row = best_instructor_for_drill(
        runtime, registry=registry, training_rules=_rules(), drill_ref="drill.sword",
        trainee_skills={"Sword": 60, "Training": 20}, trainee_location="loc_dojo",
    )
...
def test_absent_instructor_falls_back_to_real_self_practice() -> None:
    registry = _registry()
    runtime = MemoryRuntime({
        "char_good": _exact("char_good", sword=180, training=120, location="loc_elsewhere"),
        "char_wrong": _exact("char_wrong", sword=20, training=120, location="loc_elsewhere"),
    })
    contexts = instructor_contexts_for_program(
        runtime, registry=registry, training_rules=_rules(), program_ref="program.sword",
        trainee_skills={"Sword": 60, "Training": 20}, student_count=1,
...
def test_mass_capacity_shortfall_reduces_verified_edu() -> None:
    ...
    no_leaders = distributed_instructor_capacity(
        ...
        formation=weak_formation, trainee_skills={"Training": 0, "Sword": 50},
    )
    trained_cadre = distributed_instructor_capacity(
        training_rules=rules, drill=drill, student_count=1000,
        formation=full_formation, trainee_skills={"Training": 100, "Sword": 50},
    )
...
def test_aggregate_command_posts_count_as_staffed_hierarchy() -> None:
    ...
    formation["command_structure"]["unit_command"] = {
        "commander_post": "materialize_when_individually_relevant",
        "deputy_post": "materialize_when_individually_relevant",
    ...
    assert distributed_instructor_capacity(
        ...
        trainee_skills={"Training": 80, "Sword": 50},
    ) == 1.0
...
def test_hierarchical_capacity_uses_weakest_required_command_echelon() -> None:
    ...
    kwargs = {"training_rules": rules, "drill": drill, "student_count": 2000, "trainee_skills": {"Training": 80, "Sword": 50}}
...
def test_mass_training_uses_higher_command_chain_without_unit_count_cap() -> None:
    ...
    runtime = MemoryRuntime(
        {
            "char_army": _exact("char_army", sword=190, training=180),
            "char_army_deputy": _exact("char_army_deputy", sword=160, training=150),
        },
    ...
    row = instructor_contexts_for_program(
        runtime, registry=registry, training_rules=_rules(), program_ref="program.sword",
        trainee_skills={"Sword": 60, "Training": 80}, student_count=2000,
...
def test_same_instructor_cannot_teach_overlapping_sessions_for_free() -> None:
    registry = _registry()
    runtime = MemoryRuntime({
        "char_good": _exact("char_good", sword=180, training=160),
        "char_wrong": _exact("char_wrong", sword=20, training=20),
    })
    rules = _rules()
    ...
    first = instructor_contexts_for_program(
        ...
        trainee_skills={"Sword": 60, "Training": 20}, student_count=1, location_ref="loc_dojo",
...
def test_instructor_can_still_train_personally_with_remaining_time() -> None:
    rules = _rules()
    registry = _registry()
    instructor = _exact("char_good", sword=120, training=120)
    start = "244-BCE-07-29T08:00:00+08:00"
    end = "244-BCE-07-29T18:00:00+08:00"
    ...
    entries = instructor["development_state"]["training_time_ledger"]["entries"]
...
def test_next_best_available_instructor_replaces_booked_top_teacher() -> None:
    registry = _registry()
    runtime = MemoryRuntime({
        "char_good": _exact("char_good", sword=190, training=170),
        "char_wrong": _exact("char_wrong", sword=150, training=140),
    })
    rules = _rules()
    start = "244-BCE-07-29T08:00:00+08:00"
    end = "244-BCE-07-29T18:00:00+08:00"

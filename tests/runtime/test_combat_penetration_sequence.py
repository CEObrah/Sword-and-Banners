...
def test_ranged_shield_interception_uses_serviceable_shield_quantity(campaign):
    ...
    shooter = [{
        ...
        "ranged_weapon_id": "weapon_bow", "ranged_strength": 100, "ranged_coordination": 100,
...
def test_fresh_replacement_shields_restore_quantity_and_weighted_condition(campaign):
    ...
    formation = {
        ...
        "equipment_staging_by_item": {"shield_standard": 500},
    }
    replaced = _replace_serviceable_shields(planner, formation, "shield_standard")
    assert replaced == 500
    assert formation["shield_units_by_role"]["line_infantry"] == 1000
    assert formation["shield_condition_by_role"]["line_infantry"] == 70.0
    assert "shield_standard" not in formation["equipment_staging_by_item"]

    formation["shield_units_by_role"]["line_infantry"] = 0
    formation["shield_condition_by_role"]["line_infantry"] = 0.0
    formation["equipment_staging_by_item"]["shield_standard"] = 1000
    replaced = _replace_serviceable_shields(planner, formation, "shield_standard")
...
def test_wei_doctrine_aim_is_preserved_separately_from_actual_contact_anatomy(campaign):
    import json
    from conftest import execute, execute_internal
    ...
    for contact in [row for row in result["causal_trace"] if row.get("kind") == "contact"]:
        ...
        # Doctrine owns the intended target. Contact geometry/defense may move
        # the actual impact elsewhere; anatomy must consume the actual contact,
        # while the trace retains the original aim separately.
        assert anatomy.get("structure") == contact.get("actual_contact_structure")
        assert anatomy.get("side") == contact.get("actual_contact_side")
        assert contact.get("aim_structure")
        if contact.get("aim_preserved"):
            assert contact.get("actual_contact_structure") == contact.get("aim_structure")
            assert contact.get("actual_contact_side") == contact.get("aim_side")
...
def test_fresh_protective_components_restore_armor_sets_and_weighted_condition(campaign):
    ...
    formation = {
        ...
        "equipment_staging_by_item": {"armor_heavy": 500, "helmet_standard": 500},
    }
    ...
    assert "armor_heavy" not in formation["equipment_staging_by_item"]
    assert "helmet_standard" not in formation["equipment_staging_by_item"]

    formation["armor_units_by_role"]["line_infantry"] = 0
    formation["armor_condition_by_role"]["line_infantry"] = 0.0
    formation["equipment_staging_by_item"].update({"armor_heavy": 1000, "helmet_standard": 700})
    replaced = _replace_serviceable_armor_sets(planner, formation)
    assert replaced == 700
    assert formation["armor_units_by_role"]["line_infantry"] == 700
    assert formation["armor_condition_by_role"]["line_infantry"] == 100.0
    assert formation["equipment_staging_by_item"]["armor_heavy"] == 300
    assert "helmet_standard" not in formation["equipment_staging_by_item"]

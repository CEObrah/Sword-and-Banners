...
def create_pair(campaign,tag,n,battlefield='loc_kankoku_pass'):
    ...
    # Siege engineering is a duty assigned from the formation's existing
    # manpower and capability; it is not a permanent troop caste.
...
def create_local_scale_pair(campaign,tag,n):
    ...
    for ref in (a,d):
        execute_internal(campaign,'resupply',{'formation_ref':ref,'war_arrows':max(1,n)})
...
def test_personal_duel(campaign):
    # Test-only exact opponents use the stable Qin staging helper in conftest;
    # the live campaign may place Wei somewhere with no conserved Qin command
    # cohort, which must not be treated as a materialization source.
    opponent='char_test_warfare_personal_duel_opponent'
...
def test_full_siege_lifecycle(campaign):
    ...
    execute_internal(campaign,'fortification_materialize',{'fortification_ref':'fort_kankoku_accept','location_ref':'loc_kankoku_pass','garrison_formation_refs':[q],'state':'qin','commander_ref':'char_warfare_siege_qin'})
    ...
    zdoc.setdefault('logistics',{})['construction_material_units']=325

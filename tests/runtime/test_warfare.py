...
def test_personal_duel(campaign):
    x=execute(campaign,'personal_combat',{'opponent_ref':'char_tang_zhu','objective':'controlled spar','duration_minutes':30}); assert x.receipt.result['scale']=='exact_personal'

def test_warfare_scale_ladder(campaign):
    for tag,n in [('skirmish',25),('hundreds',300),('thousands',5000),('major',50000)]:
        a,d,op=create_local_scale_pair(campaign,tag,n); x=execute_internal(campaign,'battle_resolve',{'attacker_formation_refs':[a],'defender_formation_refs':[d],'operation_ref':op,'objective':'field engagement'}); assert x.receipt.result['represented_personnel']==n*2; assert x.receipt.result['planning_reads']<=32; assert x.receipt.result['writes']<=24

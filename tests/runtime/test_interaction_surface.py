...
def test_interaction_action_is_surface_only_and_frozen_arrays_validate():
    ...
    assert attempt['surface_digest'] == command.semantic_digest
    assert attempt['world_response_status'] == 'not_established_by_attempt'




def test_semantic_digest_ignores_transport_request_identity():
    payload = {'target_ref': 'loc_kanyou', 'action': 'seek_contact', 'player_statement': 'Present the summons.'}
    first = _command(payload, request_id='transport-a')
    second = _command(payload, request_id='transport-b')
    assert first.digest != second.digest
    assert first.semantic_digest == second.semantic_digest
    first_attempt = parse_interaction_attempt_summary(translate_interaction_command(first).to_record()['payload']['summary'])
    second_attempt = parse_interaction_attempt_summary(translate_interaction_command(second).to_record()['payload']['summary'])
    assert first_attempt['surface_digest'] == second_attempt['surface_digest']
...
def test_rest_surface_blocks_new_raw_scene_consequence_and_commits_attempt_only(campaign):
    ...
    with TestClient(create_app(campaign, token)) as client:
        ...
        assert preview.json()['detail']['code'] == 'raw_scene_consequence_not_player_authored'
        assert execute.json()['detail']['code'] == 'raw_scene_consequence_not_player_authored'
        ...
        assert latest['event_id'].startswith('interaction_attempt_')
        assert 'request_id' not in latest
        assert latest['action'] == 'seek_contact'
        assert 'world_response_status' not in latest
        assert not ({'accepted', 'access_granted', 'appointment', 'world_outcome'} & set(latest))


def test_rest_scene_history_can_be_reused_as_fact_and_speech_basis(campaign):
    from sword_runtime.api.app import create_app
    ...
    with TestClient(create_app(campaign, token)) as client:
        ...
        def execute_scene(request_id, payload):
            current = client.get('/v1/play/context', headers=headers).json()['campaign']
            body = {
                ...
                'submitted_at': current['world_time'],
                'payload': payload,
                'mode': 'gameplay',
            }
            preview = client.post('/v1/commands/preview', headers=headers, json=body)
            assert preview.status_code == 200, preview.text
            receipt = client.post('/v1/commands/execute', headers=headers, json=body)
            assert receipt.status_code == 200, receipt.text
            assert receipt.json()['status'] in {'committed', 'duplicate'}
            return receipt.json()

        opened = execute_scene(
            'history-basis-open',
            {'action': 'open', 'kind': 'conversation', 'participant_refs': [player_ref], 'purpose': 'history basis regression'},
        )
        session_ref = opened['result']['session_ref']
        established = execute_scene(
            'history-basis-established',
            {
                'action': 'record_fact', 'session_ref': session_ref, 'actor_ref': player_ref,
                'fact_kind': 'object_state', 'description': 'A plain bronze bowl is already on the table within reach.',
                'participant_refs': [], 'basis_refs': [],
                'improvised_prop': {'form': 'small_rigid', 'material': 'metal', 'condition': 'intact'},
            },
        )
        established_ref = established['result']['fact_ref']
        typed = execute_scene(
            'history-basis-typed',
            {
                'action': 'record_fact', 'session_ref': session_ref, 'actor_ref': player_ref,
                'fact_kind': 'object_state', 'description': 'Tang Wei takes up the already-established bronze bowl.',
                'participant_refs': [], 'basis_refs': [established_ref],
                'improvised_prop': {'form': 'small_rigid', 'material': 'metal', 'condition': 'intact'},
            },
        )
        typed_ref = typed['result']['fact_ref']
        speech = execute_scene(
            'history-basis-speech',
            {
                'action': 'record_speech', 'session_ref': session_ref, 'speaker_ref': player_ref,
                'statement': 'This bowl is already in my hand.', 'speech_kind': 'clarification',
                'basis_refs': [typed_ref],
            },
        )
        assert speech['result']['speech_ref']

...
def test_war_ceremony_attendance_uses_exact_location_and_is_idempotent(campaign):
    ...
    # Restore is unnecessary in disposable fixture, but confirms there is no hidden travel owner.
    assert original_location != "loc_kanyou"
...
def test_scheduled_player_ceremony_is_a_real_chronology_boundary(campaign):
    ...
    result2 = execute_hosted_production(campaign, "advance_time", {"target_time": target}).receipt.result

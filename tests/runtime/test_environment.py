...
def test_civil_seasonality_and_market_transport_consume_environment(campaign: Path) -> None:
    ...
    basis = kanyou["production_runtime"]["last_land_basis"]["environment"]
    assert int(basis["agriculture_output_milli"]) in {850, 1020, 1080, 1120}
    assert int(basis["forage_availability_milli"]) > 0

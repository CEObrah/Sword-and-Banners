...
def _ensure_operation_fixture(planner, at):
    ...
    planner._register_owner(OPERATION_REF, operation_path)
...
def test_probationary_acceptance_survives_missing_secondary_operation_route(campaign):
    planner = _planner(campaign)
    offer_ref, normalized, at = _install_probationary_offer(planner)
    operation_index = copy.deepcopy(planner.read("state/operations/index.json"))
    operation_index.get("operations", {}).pop(str(normalized["operation_ref"]), None)
    planner.put("state/operations/index.json", operation_index)
    ...
    wake = settle_probationary_reply(
        planner,
        {
            ...
            "decision_event_ref": offer_ref + ".decision",
            "player_action": "proceed",
            "request_id": "test-qin-probationary-route-loss-accept",
        },
        at,
    )
    ...
    assert wake is not None
    appointment = next(
        row for row in planner.read("state/player.json")["career_state"]["appointments"]
        if row.get("source_event_ref") == offer_ref
    )
    assert appointment["status"] == "awaiting_assumption"


def test_probationary_assumption_and_service_do_not_depend_on_secondary_operation_route(campaign):
    planner = _planner(campaign)
    offer_ref, normalized, at = _install_probationary_offer(planner)
    settle_probationary_reply(
        planner,
        {
            ...
            "decision_event_ref": offer_ref + ".decision",
            "player_action": "proceed",
            "request_id": "test-qin-probationary-route-loss-accept-before-assume",
        },
        at,
    )
    operation_index = copy.deepcopy(planner.read("state/operations/index.json"))
    ...
    assert close_completed_service(planner, at) is None
    appointment = next(
        row for row in planner.read("state/player.json")["career_state"]["appointments"]
        if row.get("source_event_ref") == offer_ref
    )

...
def _make_wei_available_for_probationary_test(planner):
    # The clean campaign baseline already has Wei commanding Qin Border Detachment.
    # This unit fixture isolates the first-command progression rule by releasing
    # that current assignment only inside the disposable test repository.
    current_ref = "formation_qin_wei_unit_01"
    current_path = planner.owner_path(current_ref)
    current = copy.deepcopy(planner.read(current_path))
    if current.get("commander_ref") == "char_tang_wei":
        current["commander_ref"] = None
        current["command_authority"] = "state_qin"
        current["status"] = "commander_vacant"
        planner.put(current_path, current)
    planner._release_commander_index("char_tang_wei", current_ref)
    player = copy.deepcopy(planner.read("state/player.json"))

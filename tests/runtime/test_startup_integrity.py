...
def _current_player_operation(campaign: Path) -> tuple[str, str, dict]:
    group = _load(campaign, 'state/cmd/command-groups/cmdgrp.tang_wei.field_army.json')
    operation_ref = group['active_context_ref']
    owner_path = _load(campaign, 'state/index/owner-index.json')['owners'][operation_ref]
    return operation_ref, owner_path, _load(campaign, owner_path)


def test_startup_integrity_rejects_missing_operation_current_order_pointer(campaign: Path) -> None:
    operation_ref, path, operation = _current_player_operation(campaign)
    assert operation.get('operational_orders')
    operation.pop('last_operational_order_ref', None)
    _save(campaign, path, operation)
    with pytest.raises(StartupIntegrityError, match=f'operation current-order pointer is missing: {operation_ref}'):
        validate_startup_integrity(campaign)


def test_startup_integrity_rejects_undelivered_order_as_current(campaign: Path) -> None:
    operation_ref, path, operation = _current_player_operation(campaign)
    pending = next(
        row for row in reversed(operation['operational_orders'])
        if row.get('status') == 'issued_pending_delivery'
    )
    operation['last_operational_order_ref'] = pending['order_ref']
    _save(campaign, path, operation)
    with pytest.raises(StartupIntegrityError, match=f'undelivered campaign order is current: {operation_ref}'):
        validate_startup_integrity(campaign)


def test_startup_integrity_rejects_duplicate_operation_order_identity(campaign: Path) -> None:
    operation_ref, path, operation = _current_player_operation(campaign)
    current_ref = operation['last_operational_order_ref']
    current = next(row for row in operation['operational_orders'] if row.get('order_ref') == current_ref)
    operation['operational_orders'].append(dict(current))
    _save(campaign, path, operation)
    with pytest.raises(StartupIntegrityError, match=f'operation order identities are duplicated: {operation_ref}'):
        validate_startup_integrity(campaign)

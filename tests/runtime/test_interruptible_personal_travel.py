...
def test_personal_travel_uses_current_route_path_between_tang_garrison_and_family_hall(campaign, monkeypatch) -> None:
    ...
    monkeypatch.setattr(CausalLivingWorldSwordPlanner, "_advance_causal_runtime", fake_causal_advance)
...
def test_personal_travel_interruption_preserves_completed_route_nodes(campaign, monkeypatch) -> None:
    planner, meta, origin, runtime = _personal_travel_planner(campaign)
    reached = CampaignTime.parse(str(runtime["world_time"])).add_seconds(3 * 3600)
    wake = {
        "wake_ref": "wake.test.personal-travel-node-progress",
        "kind": "campaign_event",
        "at": str(reached),
        "reason": "test causal boundary after completed route edges",
    }
    ...
    command = CommandEnvelope(
        meta["campaign_id"],
        "test-personal-travel-node-progress",
        planner.PLAYER_ACTOR,
        "travel",
        int(meta["revision"]),
        str(meta["time"]),
    ...
    assert result["travel_completed"] is False
    assert result["world_time"] == str(reached)
    assert result["route_progress_node_index"] == 3
    assert result["current_location"] == "loc_tang_manor"
    assert result["elapsed_travel_seconds"] == 3 * 3600
    saved_player = planner.read("state/player.json")
    assert saved_player["location"] == "loc_tang_manor"
    assert saved_player["current_location"] == "loc_tang_manor"
    assert saved_player["location"] != origin
    health = saved_player.get("health")
    saved_fatigue = health.get("fatigue", saved_player.get("fatigue", 0)) if isinstance(health, dict) else saved_player.get("fatigue", 0)
    assert int(saved_fatigue) >= 1

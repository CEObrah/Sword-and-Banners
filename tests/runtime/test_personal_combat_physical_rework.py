...
from sword_runtime.combat_tactics import build_team_plan, choose_tactical_target, known_tactical_candidates
...
def test_explicit_ranged_method_does_not_imply_friendly_fire_lane_consent(campaign):
    from conftest import execute
    ally = _materialize(campaign, "char_test_friendly_lane_ally")
    target = _materialize(campaign, "char_test_friendly_lane_target")
    owners = json.loads((campaign / "state/index/owner-index.json").read_text())["owners"]
    changed = []
    for ref in (ally, target):
        path = campaign / owners[ref]
        person = json.loads(path.read_text())
        person.setdefault("combat_state", {})["immobilized"] = True
        path.write_text(json.dumps(person, ensure_ascii=False, indent=2) + "\n")
    ...
    _commit(campaign, *changed)
    result = execute(campaign, "personal_combat", {
        "opponent_ref": target, "ally_refs": [ally], "target_ref": target,
        "objective": "live combat", "duration_minutes": 1,
        "intent_sequence": ["shoot an arrow at him"],
        "participant_positions": {
            "char_tang_wei": {"x_m": 0, "y_m": 0, "facing_deg": 0},
            ally: {"x_m": 3, "y_m": 0, "facing_deg": 0},
            target: {"x_m": 6, "y_m": 0, "facing_deg": 180},
        },
    }).receipt.result
    releases = [r for r in result["causal_trace"] if r.get("kind") == "projectile_release" and r.get("actor_ref") == "char_tang_wei"]
    waits = [r for r in result["causal_trace"] if r.get("kind") == "action_wait" and r.get("actor_ref") == "char_tang_wei"]
    assert not releases
    assert any(r.get("reason") == "friendly_body_in_projectile_lane_requires_explicit_risk_acceptance" for r in waits)
    assert result["decision_boundary"]["kind"] == "unsafe_projectile_lane"


def test_explicit_friendly_fire_risk_flag_allows_physical_projectile_release(campaign):
    from conftest import execute
    ally = _materialize(campaign, "char_test_friendly_lane_risk_ally")
    target = _materialize(campaign, "char_test_friendly_lane_risk_target")
    owners = json.loads((campaign / "state/index/owner-index.json").read_text())["owners"]
    changed = []
    for ref in (ally, target):
        path = campaign / owners[ref]
    ...
    releases = [r for r in result["causal_trace"] if r.get("kind") == "projectile_release" and r.get("actor_ref") == "char_tang_wei"]
    assert releases
    assert releases[0].get("unsafe_lane_at_schedule", {}).get("unsafe_reason") == "friendly_body_in_projectile_lane"


def test_empty_current_knowledge_never_falls_back_to_exact_hidden_roster():
    people = {
        "actor": {"combat_state": {}},
        "hidden": {"combat_state": {}},
    }
    positions = {"actor": {"x_m": 0, "y_m": 0}, "hidden": {"x_m": 3, "y_m": 0}}
    plan = {"known_enemy_refs": [], "assignments": {"actor": {"role": "pressure", "target_ref": "hidden"}}}
    assert known_tactical_candidates(["hidden"], plan) == []
    assert choose_tactical_target(
        "actor", ["hidden"], plan=plan, people=people, positions=positions,
    ) is None


def test_hidden_explicit_target_does_not_grant_exact_runtime_coordinates(campaign):
    from conftest import execute

    hidden = _materialize(campaign, "char_test_hidden_exact_target")
    visible = _materialize(campaign, "char_test_visible_exact_target")
    result = execute(campaign, "personal_combat", {
        "opponent_refs": [hidden, visible],
        "target_ref": hidden,
        "objective": "controlled spar",
        "duration_minutes": 1,
        "intent_sequence": ["attack him"],
        "participant_positions": {
            "char_tang_wei": {"x_m": 0, "y_m": 0, "facing_deg": 0},
            hidden: {"x_m": 10, "y_m": 0, "facing_deg": 180},
            visible: {"x_m": 4, "y_m": 5, "facing_deg": 180},
        },
        "local_obstacles": [{
    ...
    assert result["decision_boundary"]["kind"] == "target_not_currently_observable"
    assert result["decision_boundary"]["target_ref"] == hidden
    assert result["combat_information"]["registered_hostiles"] == 2
    safe_info = result["player_visible_combat_information"]
    assert safe_info["currently_visible_hostiles"] == 1
    assert "registered_hostiles" not in safe_info


def test_coarse_player_attack_selects_visible_enemy_not_occluded_exact_body(campaign):
    from conftest import execute

    hidden = _materialize(campaign, "char_test_hidden_ai_target")
    visible = _materialize(campaign, "char_test_visible_ai_target")
    result = execute(campaign, "personal_combat", {
        "opponent_refs": [hidden, visible],
        "objective": "controlled spar",
        "duration_minutes": 1,
        "intent_sequence": ["attack"],
        "participant_positions": {
            "char_tang_wei": {"x_m": 0, "y_m": 0, "facing_deg": 0},
            hidden: {"x_m": 10, "y_m": 0, "facing_deg": 180},
            visible: {"x_m": 4, "y_m": 5, "facing_deg": 180},
    ...
    assert player_attacks[0]["target_ref"] == visible

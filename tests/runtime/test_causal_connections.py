...
def test_project_workers_are_reserved_from_craft_production_until_release(campaign):
    ...
    command = SimpleNamespace(command_type="institution_project", digest="labor1234567890", semantic_digest="labor1234567890", actor_id="internal:sword-autonomy", expected_revision=int(meta["revision"]), submitted_at=str(meta["time"]))
    ...
    assert reserved > 0
    allocation = planner.read(eco_path)["labor_allocation"]["projects"]["project_labor_reservation"]
    source_site = str(allocation["location_ref"])
    before_output = copy.deepcopy(planner.read(eco_path)["local_regions"]["regions"][source_site].get("finished_goods", {}))
    planner._settle_private_production("qin", 1, str(meta["time"]))
    after_eco = planner.read(eco_path)
    assert int(after_eco["labor_allocation"]["projects"]["project_labor_reservation"]["workers"]) == reserved
    assert after_eco["local_regions"]["regions"][source_site]["production_runtime"]["last_output"]
    assert after_eco["local_regions"]["regions"][source_site]["finished_goods"] != before_output or reserved > 0
...
def test_house_can_proclaim_proto_state_and_take_territory_with_exact_occupation(campaign):
    ...
    meta = planner.read("state/meta.json")
    proclaim = SimpleNamespace(command_type="house_action", digest="sovereignty123456", semantic_digest="sovereignty123456", request_id="sovereignty-proclaim", actor_id="char_tang_wei", expected_revision=int(meta["revision"]), submitted_at=str(meta["time"]))
    ...
    transfer = SimpleNamespace(command_type="territorial_consequence", digest="sovereignty654321", semantic_digest="sovereignty654321", request_id="sovereignty-transfer", actor_id="internal:sword-autonomy", expected_revision=int(planner.read("state/meta.json")["revision"]), submitted_at=str(planner.read("state/meta.json")["time"]))
...
def test_project_cancellation_releases_workers_and_refunds_unused_inputs(campaign):
    ...
    meta = planner.read("state/meta.json")
    start = SimpleNamespace(command_type="institution_project", digest="cancelproject1234", semantic_digest="cancelproject1234", request_id="cancel-project-start", actor_id="internal:sword-autonomy", expected_revision=int(meta["revision"]), submitted_at=str(meta["time"]))
    ...
    cancel = SimpleNamespace(command_type="project_cancel", digest="cancelproject5678", semantic_digest="cancelproject5678", request_id="cancel-project-do", actor_id="internal:sword-autonomy", expected_revision=int(planner.read("state/meta.json")["revision"]), submitted_at=str(planner.read("state/meta.json")["time"]))
...
def test_house_sovereignty_rejects_state_owned_force_with_only_player_command(campaign):
    import pytest
    planner = planner_for(campaign); planner._reset()
    formation_ref = "formation_high_guard_qin_a"
    formation_path = planner.owner_path(formation_ref)
    formation = copy.deepcopy(planner.read(formation_path))
    formation["location_ref"] = "loc_gyou"
    ...
    index = copy.deepcopy(planner.read("state/operations/index.json")); index.setdefault("operations", {})[operation_ref] = operation_path; planner.put("state/operations/index.json", index); planner._register_owner(operation_ref, operation_path)
    meta = planner.read("state/meta.json")
    command = SimpleNamespace(command_type="house_action", digest="rejectcommandsovereignty", semantic_digest="rejectcommandsovereignty", request_id="reject-command-sovereignty", actor_id="char_tang_wei", expected_revision=int(meta["revision"]), submitted_at=str(meta["time"]))
...
def test_house_polity_gets_monthly_causal_host_and_state_like_threat_response(campaign):
    ...
    formation_path = planner.owner_path("formation_red_lance_a"); formation = copy.deepcopy(planner.read(formation_path)); formation["commander_ref"] = None; formation["command_authority"] = "house_tang"; planner.put(formation_path, formation)
...
def test_recognized_polity_participates_in_shared_interstate_front_and_diplomacy(campaign):
    ...
    formation_path = planner.owner_path("formation_red_lance_a"); formation = copy.deepcopy(planner.read(formation_path)); formation["location_ref"] = "loc_gyou"; formation["commander_ref"] = None; formation["command_authority"] = "house_tang"; planner.put(formation_path, formation)
    ...
    assert "formation_red_lance_a" in dynamic["formation_refs"].values()
...
def test_explicit_territorial_grant_does_not_transfer_granting_state_force_to_house_polity(campaign):
    ...
    index = copy.deepcopy(planner.read("state/operations/index.json")); index.setdefault("operations", {})[operation_ref] = operation_path; planner.put("state/operations/index.json", index); planner._register_owner(operation_ref, operation_path)
    meta = planner.read("state/meta.json")
    command = SimpleNamespace(command_type="house_action", digest="grantsovereignty", semantic_digest="grantsovereignty", request_id="grant-sovereignty", actor_id="char_tang_wei", expected_revision=int(meta["revision"]), submitted_at=str(meta["time"]))
...
def test_adjacent_sovereigns_do_not_drift_into_war_from_elapsed_pressure_alone(campaign):
    ...
    qin["war_intents"] = [
        row for row in qin.get("war_intents", [])
        if not (isinstance(row, dict) and str(row.get("target_ref", "")) == "state_zhao")
    ]
    planner.put(qin_path, qin)
...
def test_autonomous_counterinsurgency_settles_real_contact_and_conserved_losses(campaign):
    ...
    old_loc = str(government.get("location_ref", "")); government["location_ref"] = "loc_gyou"; government["mobilized"] = True; government["status"] = "deployed"; planner.put(gp, government); planner._index_formation_location(government_ref, old_loc, "loc_gyou")
...
def test_operation_core_reducer_uses_exact_owner_when_secondary_route_is_missing(campaign):
    planner = planner_for(campaign); planner._reset()
    operation_ref = "operation_exact_owner_route_recovery"
    formation_ref = "formation_red_lance_a"
    formation = planner.read(planner.owner_path(formation_ref))
    path = f"state/operations/{operation_ref}.json"
    planner.put(path, {
        "schema": "sword-operation", "owner_id": operation_ref, "operation_ref": operation_ref,
        "objective": "exact owner routing regression", "status": "planned",
        "location_ref": formation.get("location_ref"), "formation_refs": [formation_ref],
        "administrative_authority": formation.get("administrative_owner"),
        "administrative_authorities": [formation.get("administrative_owner")],
        "created_at": str(planner._world_time()),
    })
    planner._register_owner(operation_ref, path)
    idx = copy.deepcopy(planner.read("state/operations/index.json"))
    idx.setdefault("operations", {}).pop(operation_ref, None)
    planner.put("state/operations/index.json", idx)

    meta = planner.read("state/meta.json")
    transition = SimpleNamespace(
        command_type="operation_transition", digest="operationroute0001", semantic_digest="operationroute0001",
        request_id="operation-route-transition", actor_id="internal:sword-autonomy",
        expected_revision=int(meta["revision"]), submitted_at=str(meta["time"]),
    )
    result = planner._dispatch(transition, {"operation_ref": operation_ref, "status": "mobilizing"})
    assert result["status"] == "mobilizing"
    assert planner.read(path)["status"] == "mobilizing"
    assert planner.read("state/operations/index.json")["operations"][operation_ref] == path

    duplicate = SimpleNamespace(
        command_type="operation_create", digest="operationroute0002", semantic_digest="operationroute0002",
        request_id="operation-route-duplicate", actor_id="internal:sword-autonomy",
        expected_revision=int(planner.read("state/meta.json")["revision"]), submitted_at=str(planner.read("state/meta.json")["time"]),
    )
    broken = copy.deepcopy(planner.read("state/operations/index.json"))
    broken.setdefault("operations", {}).pop(operation_ref, None)
    planner.put("state/operations/index.json", broken)
    with pytest.raises(ValueError, match="operation_ref already exists"):
        planner._dispatch(duplicate, {
            "operation_ref": operation_ref, "objective": "must not overwrite exact operation",
            "formation_refs": [formation_ref], "location_ref": formation.get("location_ref"),
        })

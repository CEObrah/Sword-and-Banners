...
def test_causal_event_store_archives_hot_overflow_and_rehydrates_exact_old_ref(campaign):
    ...
    assert int(hot["archive_segment_count"]) >= 1
    assert int(hot["archived_kind_counts"]["test_event"]) > 0

    # Archive compaction is a future-state write path. Validate the staged owner
    # against the same registered schema now so a long campaign cannot discover
    # schema/runtime drift only after the hot head first spills to disk.
    import jsonschema
    schema = planner.read("game/schemas/event-registry.schema.json")
    validator_class = jsonschema.validators.validator_for(schema)
    validator_class.check_schema(schema)
    validator_class(schema).validate(hot)

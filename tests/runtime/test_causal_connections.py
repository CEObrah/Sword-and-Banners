...
def test_conquered_site_tax_is_paid_to_current_controller_from_native_economy(campaign):
    ...
    at = "245-BCE-12-31T00:00:00+08:00"
...
def test_low_liquidity_state_cannot_recruit_against_uncollected_macro_revenue(campaign):
    ...
    assert after_state["civil_finance"]["revenue_collected_silver"] == 0
    assert after_state["civil_finance"]["native_recruits"] == 0
    assert int(planner.read(force_path)["headcount"]) == head_before
    assert int(planner.read("state/population/qin.json")["strata"]["active_military"]) == pop_before
...
def test_undergarrisoned_unfunded_occupation_can_revolt_and_becomes_state_threat(campaign):
    ...
    # opposing faction/force/formation rather than a scalar-only obstacle.
    planner._autonomy_state(_state_host(planner, "qin"), 1, at)
...
def test_route_disruption_reduces_market_delivery_and_raises_insecurity(campaign):
    ...
    planner._restock_capital_market("qin", "245-BCE-12-31T00:00:00+08:00")
...
def test_exact_information_wakes_reactive_faction_and_governs_action(campaign):
    before = planner_for(campaign).read("state/runtime.json")["hosts"]["host_faction_zhao_riboku_network"]["next_due"]
    ...
    planner = planner_for(campaign); planner._reset()
    faction = planner.read("state/factions/faction_zhao_riboku_network.json")
    ...
    after_due = planner.read("state/runtime.json")["hosts"]["host_faction_zhao_riboku_network"]["next_due"]
    assert after_due != before

    planner._autonomy_faction({"owner_ref": "faction_zhao_riboku_network"}, 3, str(planner.read("state/runtime.json")["world_time"]))
    faction = planner.read("state/factions/faction_zhao_riboku_network.json")
    commitment = faction["commitments"][-1]
...
def test_foreign_service_casualties_are_charged_to_origin_population(campaign):
    ...
    loss = 10
    # Reproduce the core battle reducer's legacy owner-state population debit.
...
def test_project_cancellation_releases_workers_and_refunds_unused_inputs(campaign):
    ...
    project = next(row for row in planner.read("state/institutions/inst_qin_fortification_bureau.json")["projects"] if row["project_ref"] == "project_cancel_test")
...
def test_weighted_local_baselines_preserve_state_totals_and_site_differences(campaign):
    ...
    assert saved["population_source_ref"] == "population_zhao"
...
def test_steppe_and_quanrong_can_satisfy_knowledge_requirement_through_registered_org_channels(campaign):
    planner = planner_for(campaign); planner._reset()
    at = str(planner.read("state/runtime.json")["world_time"])
    for ref in ("faction_northern_steppe_confederation", "faction_quanrong_polity"):
        before = copy.deepcopy(planner.read(planner.owner_path(ref)))
        before_count = len(before.get("commitments", []))
        planner._autonomy_faction({"owner_ref": ref}, 3, at)
        after = planner.read(planner.owner_path(ref))
        assert len(after.get("commitments", [])) == before_count + 1
        commitment = after["commitments"][-1]
        assert commitment["action"] == "confederation_muster"
        assert any(str(x).startswith("orginfo_") for x in commitment["knowledge_refs_used"])
        receipts = after.get("organizational_knowledge_receipts", [])
        assert any(str(row.get("information_ref", "")).startswith("orginfo_") for row in receipts)
...
def test_active_revolt_gets_dynamic_scheduler_and_autonomous_irregular_campaign(campaign):
    ...
    planner = planner_for(campaign); planner._reset(); at = "245-BCE-12-31T00:00:00+08:00"
    planner._occupation_initialize("loc_gyou", "state_qin", "state_zhao", at, "test_capture")
    ...
    before_commitments = len(planner.read(planner.owner_path(faction_ref)).get("commitments", []))
    ...
    assert len(faction.get("commitments", [])) == before_commitments + 1
    assert faction["commitments"][-1]["action"] == "irregular_campaign"
...
def test_recognized_polity_participates_in_shared_interstate_front_and_diplomacy(campaign):
    ...
    world.setdefault("theaters", {})[dynamic["theater_ref"]] = {"phase": "peace", "cycle": 0, "pressure": 99, "cooldown_quarters": 0, "history": []}
    ...
    assert polity["diplomacy"]["zhao"]["status"] == "war"
...
def test_recognized_polity_materializes_exact_institutions_with_polity_funding_and_routes(campaign):
    ...
    assert regional["administration_review"]["rule"].startswith("capacity constrains")
...
def test_adjacent_sovereigns_do_not_drift_into_war_from_elapsed_pressure_alone(campaign):
    ...
    theater.update({"phase": "peace", "pressure": 99, "cooldown_quarters": 0, "history": []})
...
def test_explicit_war_intent_authorizes_interstate_theater_without_pressure_countdown(campaign):
    ...
    planner.put(qin_path, qin)
    path = planner.owner_path("interstate_warring_states")
    world = copy.deepcopy(planner.read(path)); world["theaters"]["qin_zhao_gyou"].update({"phase": "peace", "pressure": 0, "cooldown_quarters": 0, "history": []}); planner.put(path, world)
    host = copy.deepcopy(planner.read("state/runtime.json")["hosts"]["host_interstate_wars"]); at = str(host["next_due"])
    planner._autonomy_interstate(host, 1, at)
...
def test_polity_action_occupation_policy_requires_current_territorial_control(campaign):
    polity_ref = _install_test_tang_polity(campaign)
    execute_production(campaign, "polity_action", {"polity_ref": polity_ref, "action": "set_occupation_policy", "location_ref": "loc_gyou", "policy_key": "tax_posture", "policy_value": "moderate compliance-first taxation"}, request_id="polity-occupation-policy")
    planner = planner_for(campaign); policy = planner.read("state/territory/control.json")["sites"]["loc_gyou"]["governance"]["occupation_policy"]["tax_posture"]
...
def test_defensive_alliance_creates_exact_third_party_war_obligation_when_ally_is_attacked(campaign):
    ...
    world = copy.deepcopy(planner.read(world_path)); world["theaters"]["qin_zhao_gyou"].update({"phase": "peace", "pressure": 0, "cooldown_quarters": 0, "history": []}); planner.put(world_path, world)
    host = copy.deepcopy(planner.read("state/runtime.json")["hosts"]["host_interstate_wars"])
    planner._autonomy_interstate(host, 1, at)
...
def test_incoming_treaty_to_player_polity_waits_for_explicit_player_decision(campaign):
    ...
    pending = planner.read(planner.owner_path(proposal_ref)); assert pending["status"] == "pending_response"; assert pending["response_basis"]["rule"] == "player sovereign decision required"

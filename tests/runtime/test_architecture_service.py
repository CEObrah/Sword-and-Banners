...
def test_world_density_and_cold_hosts():
    ...
    mercenary_owners=set()
    for path in (root/'state/merc').rglob('*.json'):
        record=json.load(open(path))
        if record.get('schema') in {'mercenary','mercenary-company','regional-mercenary-company.v1'} and isinstance(record.get('owner_id'),str):
            mercenary_owners.add(record['owner_id'])
    hosted_mercenaries={str(h.get('owner_ref')) for h in runtime['hosts'].values() if h.get('kind')=='mercenary'}
    assert hosted_mercenaries == mercenary_owners

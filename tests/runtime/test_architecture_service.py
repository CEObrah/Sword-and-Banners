...
def test_world_density_and_cold_hosts():
    ...
    # market rows stay conserved under state/merc and materialize a host only when they become
    # tactically causal; exact tactical companies must remain hosted.
    assert hosted_mercenaries == tactical_mercenary_owners
    assert hosted_mercenaries.isdisjoint(accounting_only_mercenary_owners)
...
def test_champions_doctrine_is_principal_survival():
    ...
    assert f['personnel'] == 500
    assert f['doctrine_behavior']['protected_principal_ref']=='char_tang_wei'
    assert f['doctrine_behavior']['survival_bias']
    assert f['doctrine_behavior']['deep_pursuit']=='prohibited_without_compatible_order'
    assert not (root/'state/formations/tang-champions-second.json').exists()

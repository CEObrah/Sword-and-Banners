...
def test_outside_bastion_applicant_relocates_before_selection(campaign) -> None:
    ...
    move = planner._bastion_relocate_outside_applicants(
        'stone_spear', source_state='wei', source_site_ref=source_site, applicant_count=24, at=at,
    )
    ...
    assert cohort['bastion_application']['corps'] == 'stone_spear'
    ...
    app = tang1['bastion_outside_applications']['stone_spear']
...
def test_house_action_accepts_applicants_but_does_not_create_bastion_bodies(campaign) -> None:
    ...
    force0 = int(planner.read('state/forces/bastion-red-thunder.json')['headcount'])
    cmd = CommandEnvelope(
        ...
        payload={
            ...
            'corps_key':'red_thunder', 'source_state':'han',
    ...
    assert int(planner.read('state/forces/bastion-red-thunder.json')['headcount']) == force0
...
def test_arrived_outside_applicants_can_be_considered_without_guaranteed_selection(campaign) -> None:
    ...
    at = str(planner._world_time())
    move = planner._bastion_relocate_outside_applicants(
        'white_blade', source_state='qin', source_site_ref='loc_kanyou', applicant_count=20, at=at,
    )
    planner._settle_population_mobility_arrival({'cohort_ref': move['migration_ref']}, str(move['arrives_at']))
    force_path='state/forces/bastion-white-blade.json'
    ...
    force=copy.deepcopy(planner.read(force_path))
    started=planner._bastion_start_pipeline('white_blade', force, at)
    assert started > 0

...
def test_command_staff_at_broad_depot_are_nearby_not_teleported_present(campaign):
    context=_ops(campaign).play_context()
    nearby=_cast(context,"nearby_people"); present=_cast(context,"present_people")
    for ref in ("char_lin_zhen","char_gao_yun","char_han_qiu","char_duan_jin","char_shen_rui"):
        assert ref in nearby
        assert ref not in present
        assert nearby[ref]["location"] == "loc_qin_eastern_depot"
    assert "controlled_formation_command" in nearby["char_gao_yun"]["scene_basis"]


def test_sword_manor_commanders_surface_at_exact_training_ground(campaign):
    ...
    assert "char_wei_jian" in present
    assert "char_ren_qiao" in present
    assert present["char_wei_jian"]["location"] == "loc_tang_manor_training_ground"
    assert "house_or_command_duty" in present["char_wei_jian"]["scene_basis"]
...
def test_scene_participants_become_bounded_permitted_people(campaign):
    ...
    assert {"char_lin_zhen","char_gao_yun","char_han_qiu"} <= permitted

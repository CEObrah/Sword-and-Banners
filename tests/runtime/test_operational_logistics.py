...
def test_operation_army_movement_may_bring_lawful_house_auxiliaries_without_transferring_ownership(campaign):
    ...
    # Current campaign truth has the complete recursive 9,500-man Tang Wei Army
    # assembled at the eastern depot. The Qin operational order legally compels
    # the 5,000 state-owned Qin bodies, while Wei may march the 4,500 House Tang
    # auxiliaries without transferring anyone's institutional ownership.
    assert {_owner(campaign, ref)['location_ref'] for ref in expected} == {'loc_qin_eastern_depot'}
    assert _read(campaign, 'state/player.json')['location'] == 'loc_qin_eastern_depot'
    ...
    assert latest_order['applies_to_formation_refs'] == assigned_refs
    for ref in private_refs:

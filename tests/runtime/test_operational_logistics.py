...
def test_operation_army_movement_may_bring_lawful_house_auxiliaries_without_transferring_ownership(campaign):
    ...
    player_before = _read(campaign, 'state/player.json')
    player_location_before = player_before.get('current_location') or player_before.get('location')
    ...
    # Muster time is causal, not a fixture constant. A current snapshot where
    # Tang Wei is already co-located with the column lawfully needs zero extra
    # muster time; a detached commander must still be physically mustered.
    if player_location_before == staging_location:
    ...
    assert set(current_order['applies_to_formation_refs']) == assigned_set
    for ref in private_refs:

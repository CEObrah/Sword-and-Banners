...
def test_partial_physical_mount_count_reduces_mounted_share_and_charge_expression():
    h = Harness()
    formation = {"cohesion": 95, "training_progress": 95, "mounts": {"horse_war_military": 500}}
...
def test_mount_factor_uses_actual_mounted_units_not_role_label():
    h = Harness()
    formation = {"mounts": {"horse_war_military": 500}}
    half = h._combat_mount_factor([_cavalry_row(mounted_units=500)], formation)
    full = h._combat_mount_factor([_cavalry_row(mounted_units=1000)], {"mounts": {"horse_war_military": 1000}})
    assert half < full

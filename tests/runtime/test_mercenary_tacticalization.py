...
def _seed_isolated_qin_field_contractor(
    planner: ProductionCampaignPlanner, *, at: str, company_ref: str = "merc.major.11"
) -> str:
    """Give one clean company the exact field-contract precondition this test needs.

    The maintained campaign is allowed to advance contracts and deployments. These
    tacticalization regressions must therefore create their own employer/status
    precondition instead of borrowing a company that happened to be under Qin
    contract in an older save. Each pytest campaign is disposable.
    """
    path = planner.owner_path(company_ref)
    company = json.loads(json.dumps(planner.read(path)))
    assert bool(company.get("accounting_only")) is True
    assert not company.get("tactical_formation_ref")
    field_kind = sorted(str(x) for x in planner._mercenary_tactical_rules()["field_contract_kinds"])[0]
    company["contracts"] = [{
        "contract_ref": f"contract.test.qin.field.{company_ref}",
        "employer_ref": "state_qin",
        "status": "active",
        "engagement_kind": field_kind,
        "active_at": at,
        "term_days": 3650,
        "deployment_location_ref": str(company.get("current_location_ref") or company.get("home_location_ref")),
    }]
...
def test_state_field_contracts_demand_load_normal_conserved_formations(campaign: Path) -> None:
    ...
    _seed_isolated_qin_field_contractor(planner, at=at)
...
def test_mercenary_catastrophic_losses_preserve_source_pool_identity(campaign: Path) -> None:
    planner = _planner(campaign)
    at = str(planner._world_time())
    company_ref = _seed_isolated_qin_field_contractor(planner, at=at)
...
def test_active_field_contract_is_a_real_independent_interstate_command_not_dead_unassigned_state(campaign: Path) -> None:
    ...
    planner = _planner(campaign)
    at = str(planner._world_time())
    mercenary_ref = _seed_isolated_qin_field_contractor(planner, at=at)

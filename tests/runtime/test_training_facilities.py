...
def test_distributed_house_tang_cohort_resolves_real_formation_locations() -> None:
    ...
    assert by_location["loc_qin_eastern_depot"] == 2991
    assert by_location["loc_tang_manor_garrison_yard"] == 14955
    assert sum(by_location.values()) == 17946
...
def test_complete_role_loadout_supplies_shield_until_separate_ledger_exists(campaign):
    ...
    formation = deepcopy(planner.read("state/formations/bastion-iron-rampart-01.json"))

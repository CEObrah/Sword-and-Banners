...
def test_field_army_can_establish_portable_drill_ground_but_not_fixed_specialist_works() -> None:
    ...
    commander = program_facility_access(
        RUNTIME, registry=REGISTRY, program_ref="program.commander_infantry", location_ref="loc_qin_regional_02"
    )
    assert commander["drill.command_orders"] == 1.0
    assert commander["drill.command_staff"] == 1.0
    assert commander["drill.command_battlefield"] == 1.0

    outer_wall = program_facility_access(
        RUNTIME, registry=REGISTRY, program_ref="program.house_infantry_outer_wall", location_ref="loc_qin_regional_02"
    )
    assert outer_wall["drill.engineering_works"] == 0.0
    assert outer_wall["drill.artillery_service"] == 0.0
    assert outer_wall["drill.siege_service"] == 0.0
...
def test_formation_facility_gate_multiplies_conserved_equipment_access() -> None:
    outer_wall = json.loads((ROOT / "state/formations/house-tang-outer-wall-iron-wall-01.json").read_text(encoding="utf-8"))
    outer_wall["location_ref"] = "loc_qin_regional_02"
    access = formation_drill_access(
        REGISTRY, "program.house_infantry_outer_wall", outer_wall, role="house_infantry", runtime=RUNTIME
    )
    assert access["drill.artillery_service"] == 0.0
    assert access["drill.engineering_works"] == 0.0

    outer_wall["location_ref"] = "loc_tang_manor_defense_camp"
    access = formation_drill_access(
        REGISTRY, "program.house_infantry_outer_wall", outer_wall, role="house_infantry", runtime=RUNTIME
    )
    assert access["drill.artillery_service"] == 1.0
    assert access["drill.engineering_works"] == 1.0
...
def test_complete_role_loadout_supplies_shield_until_separate_ledger_exists(campaign):
    ...
    formation = deepcopy(planner.read("state/formations/house-tang-outer-wall-iron-wall-01.json"))
    program_ref = "program.house_infantry_outer_wall"

    inherited = formation_drill_access(registry, program_ref, formation, role="house_infantry", runtime=planner)
    assert inherited["drill.spear_shield_line"] == 1.0

    formation["shield_units_by_role"] = {"house_infantry": 0}
    explicit_zero = formation_drill_access(registry, program_ref, formation, role="house_infantry", runtime=planner)

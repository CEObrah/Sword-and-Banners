...
def test_house_commercial_income_conserves_silver_and_sword_manor_has_no_market_service(campaign) -> None:
    ...
    # Sword Manor is a permanent House Tang military institution, not a dormant
    # private-service market program. The old contract owner/runtime path is gone.
    ...
    assert "sword_manor_private_service" not in house.get("administrative_programs", {})
...
def test_house_contingency_mercenary_offer_never_transfers_company_bodies(campaign) -> None:
    ...
    assert not ({"force_bastion_iron_rampart", "force_bastion_red_crane", "force_bastion_white_lantern", "force_bastion_deep_earth"} & set(refs))

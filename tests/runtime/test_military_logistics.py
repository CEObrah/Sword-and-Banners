...
def test_state_force_authorization_growth_creates_shortage_then_real_recruitment_fills_it(campaign):
    ...
    # This test is about the authorization -> conserved recruitment path, so
    # make its material-capacity premise explicit instead of inheriting whatever
    # treasury balance the live revision-45 campaign happens to carry. The runtime
    # must still enforce its configured reserve floor and per-recruit issue cost.
    economy_rules = planner.read('game/data/mechanics/economy.json')
    growth_rules = economy_rules['force_posture_hysteresis']['authorization_growth']
    reserve_months = float(growth_rules['treasury_reserve_months'])
    monthly_expense = int(state.get('normal_monthly_expense_silver', 0))
    issue_cost = int(economy_rules['military_finance']['recruitment_and_basic_issue_cost_silver_per_person'])
    funded_floor = int(monthly_expense * (reserve_months + 2.0)) + (20_000 * issue_cost)
    state['treasury_silver'] = max(int(state.get('treasury_silver', 0)), funded_floor)

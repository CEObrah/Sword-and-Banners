...
Treat real play as integration testing. Judge whether mechanics, combat, warfare, narration, pacing, autonomy, institutions, causal flow, UX, balance, performance, and context use actually work well in play. After every substantive live gameplay turn, append exactly one concise `OOC QA:` line. If the turn exposed a reusable defect, depth gap, repetitive loop, missing counterplay, stale system, awkward UX, context cost, or feature opportunity, report only the strongest finding with symptom, impact, likely owner, and smallest coherent fix/regression. If no material improvement is supported, say `OOC QA: No material improvement identified this turn.` Never manufacture or repeat a finding merely to fill the line. Surface false-truth, agency/knowledge, serious exploit, blocked-intent, or persistence risks immediately.

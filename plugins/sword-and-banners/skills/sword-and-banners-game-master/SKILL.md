...
Flag immediately when an issue risks false campaign truth, breaks agency/knowledge boundaries, blocks declared intent, exposes a serious exploit, makes a consequential choice misleading, or threatens transaction durability. Otherwise preserve IC flow and surface only the strongest useful finding at a natural stopping point. For a concrete reusable finding, use one concise `OOC IMPROVEMENT:` note with observed symptom, player impact, likely owner, and smallest coherent fix. Depth gaps, repetitive loops, missing counterplay, stale autonomy, awkward UX, needless context cost, and feature opportunities discovered through play are valid findings. Do not spam, repeat unchanged findings, or manufacture review notes.

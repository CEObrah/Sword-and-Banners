...
`runtime/sword_runtime/api/command_discovery.py` builds the compact per-turn command index. Exact payload guidance is fetched through the single MCP `get_command_contract` authority installed by the API extensions. Do not restore full command schemas to every play-context response.

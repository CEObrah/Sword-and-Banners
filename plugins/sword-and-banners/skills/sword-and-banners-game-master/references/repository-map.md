...
`runtime/sword_runtime/smart_training.py` — deterministic training-focus routing for exact people, person-lite officers, and cohorts; it spends only already-earned training time and prioritizes promotion deficits, loadout/role needs, command/logistics, and weak useful stats.

`game/data/mechanics/house-tang-force-policy.json` — static House Tang recruitment/training/force-employment policy. New House regulars enter only through conserved Sword Manor trainee intake; mercenary preference never changes manpower ownership.
...
- `game/data/world/routes.json#routes` owns strategic route identity/endpoints/base travel inputs. `#local_routes` owns compound/settlement/internal access. `#aliases` is compatibility routing only and never an independent road owner.
...
- `state/population/*.json#local_population.version=3` partitions exact national population only across legitimate demographic owners. Facilities, gates, depots, offices, training grounds, and halls do not become national demographic regions. `state/economy/private/*.json#local_regions.version=2` follows that partition without becoming population authority.
...
- `state/migrations/world-geography-overhaul-v1.json` records the current-campaign spatial migration provenance without advancing campaign time or replacing lawful semantic history.

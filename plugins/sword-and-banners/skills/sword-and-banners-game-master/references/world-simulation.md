...
Commissions are delayed institutional assignments. A request does not create an offer. The runtime commits the assignment objective/location before player tactics, settles the response through causal time, and may wake the player when the offer arrives. Any hidden risk profile is an issuer-side assessment only, never authority that actual opposition exists. Acceptance is separate from completion, and a player report can support later settlement only through relevant runtime-established evidence.

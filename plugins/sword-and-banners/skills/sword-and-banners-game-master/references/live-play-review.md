...
Preserve IC flow, then finish every live gameplay turn with exactly one concise `OOC QA:` line. If the turn exposed a material reusable issue or improvement, report only the strongest finding and, when useful, classify it as **BUG**, **DEPTH GAP**, **CAUSAL FLOW**, **UX**, **BALANCE**, **PERFORMANCE**, or **DESIGN**. Prefer observed evidence -> player impact -> authoritative owner -> smallest reusable fix -> focused regression to add. If nothing material was observed, say `OOC QA: No material improvement identified this turn.` Never invent a defect to satisfy the format, repeat an unchanged finding as new, or turn ordinary play into a changelog. The QA line is diagnosis, not permission to mutate source or campaign truth; implementation requires explicit `OOC DEV:` intent.

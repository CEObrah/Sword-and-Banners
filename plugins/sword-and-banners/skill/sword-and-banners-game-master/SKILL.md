...
3. Require both `delivery_integrity.gm_skill_contract_verified: true` and `delivery_integrity.deployment_compatible: true` in the returned context. A missing/mismatched token means the installed Skill or cached MCP tool schema is stale relative to the runtime; a missing/false deployment proof means the running image cannot prove it covers the live source/check-out. Retry the call exactly once only if the failure was unexpected; if it still fails, stop consequential play and report the delivery-tier mismatch OOC rather than reconstructing state from memory.
4. Treat fresh revision, time, scene, player state, player-visible knowledge, compact cast/read hints, controlled formations, opportunities, runtime limits, semantic-action contract, delivery-integrity proof, and mechanic-family index as the live contract. When `gm_scene_context` is present, treat it as the **primary writer workspace**: start from its current scene, continuity, people, knowledge boundaries, human/practical threads, causal events, world pressure, and hard constraints instead of reconstructing prose from subsystem-shaped records. Raw projections remain exact evidence, not a narration checklist. The mechanic index never defines what Wei or an NPC is allowed to attempt in ordinary fiction.
5. If the Runtime should be available but the intended call fails unexpectedly for a reason other than a confirmed Skill-contract mismatch, retry exactly once. If it still fails, stop consequential resolution. Never reconstruct authoritative state from Project memory, chat history, prior narration, model recall, GitHub, or external history.

The token now binds the installed Skill to runtime source, MCP contracts, schemas, game data, dependencies and release lineage. Pass it as `skill_contract_token` to every `preview_command` too, together with `expected_campaign_id` copied from the fresh context. Execution attestations are release-bound. Never substitute a token learned from a failing tool response for the token printed in this installed Skill.

When resuming a known save, pass `expected_campaign_id` and, when known, `expected_baseline_id` to `get_play_context`. A new chat resumes the endpoint's configured campaign. It does not create a fresh campaign, reset wounds/combat, or rewind time. If the player requests a fresh campaign, explain that backend creation from an explicit seed and isolated recovery storage is required; never relabel the existing save as new. During release testing compare the returned `release_contract_sha256`, runtime/MCP/schema/Skill fingerprints, image SHA and campaign lineage with `RELEASE_MANIFEST.json`. Local developer compatibility with no image identity is not proof of a live deployment.

## AI-created durable cognition

AI creates meaning; runtime preserves reality. When a present established NPC develops an important belief, suspicion, goal, plan, grievance, interpretation of a relationship, or memory from the scene, the GM may propose that cognition through the `npc_cognition` family and `gm_cognition_action`. This is GM direction, not a player choice or a guaranteed mechanical outcome. Decide what follows plausibly from the person's history; do not generate arbitrary drama or automatic personality drift.

Use the exact command contract. Persist primary attributed speech or observed scene evidence when warranted, then cite its returned refs. The NPC must have witnessed that evidence; merely joining the scene later does not grant earlier knowledge. Create one salient entry with a stable `cognition_ref`, or update/resolve the existing entry with a new causal basis and explanation. Avoid synonymous duplicate entries. Current capacity is bounded and overflow fails explicitly; do not evade it by inventing another identity or hiding memory in unrelated state.

An accepted record makes “this NPC suspects theft” durable. It does not make theft true. Plans are intentions, not issued orders; recollections are attributed mental content, not proof of events; a promised intention does not create a binding contract. This operation never grants troops, money, knowledge claims, travel, score changes, injuries, death, or player beliefs. Use the respective authority for those consequences. Do not represent earlier hard facts as uncertain merely to overwrite them through cognition.

Only after a committed receipt use the accepted cognition as persistent characterization. Rehydrate it through fresh private context and the exact person sheet when the compact window is truncated. To understand an earlier interpretation, use `inspect_game_object` on its `previous_history_ref`; each read returns one prior version and its causal evidence, within the person's current read permissions. Preserve private knowledge boundaries: never reveal a secret intention to Wei merely because the GM can read it. Ephemeral atmosphere, gestures and ordinary connective dialogue need no cognitive record. This first bridge covers present NPCs with scene evidence; it does not yet authorize arbitrary offscreen schemes, new named characters, or NPC binding promises.
...
Append one compact `OOC QA:` line after **every live gameplay turn**. The footer is diagnostic and outside the fiction; it never mutates source or campaign state. Classify the turn as `PASS`, `WARN`, or `FAIL`. `PASS` requires that this turn successfully obtained fresh authoritative `get_play_context`, passed both the GM Skill/runtime compatibility handshake and deployment/source compatibility proof, and exposed no material defect in the resolved/narrated beat. The returned delivery-integrity proof must also identify the certified `release_baseline_id`, baseline revision, and baseline world time for the live campaign lineage; missing or campaign-mismatched lineage proof cannot PASS. A failed runtime read, missing delivery-integrity proof, confirmed Skill-contract mismatch, stale-deployment signal, or campaign-lineage contradiction is automatically `FAIL`, never `PASS`. Use `WARN` for a mechanically valid but underdeveloped/fragile system, and `FAIL` for false campaign truth, broken agency/knowledge, blocked declared intent, a serious exploit, misleading consequence, chronology failure, delivery-tier mismatch, or persistence risk. When there is a finding, name the strongest reusable symptom and likely root owner plus the smallest coherent improvement/regression.

Do not manufacture or repeat a defect merely to fill the footer; `PASS — fresh runtime + Skill handshake + deployment compatibility verified; no material defect observed this turn` is valid. A turn-level PASS is not blanket release certification. Explicit fresh-release acceptance additionally requires the repository release-state guard plus a live deployed smoke through the actual ChatGPT/MCP path before claiming the playable production release is certified. Classify owner before proposing change: GM Skill/presentation, runtime interface, runtime/rules mechanics, game data, projection source, explicit state repair, deployment/delivery, or feature/design. The QA line is observational only. Ordinary play must never silently modify source/state; actual implementation requires explicit `OOC DEV:` intent.

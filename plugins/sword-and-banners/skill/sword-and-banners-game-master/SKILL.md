...
Do not manufacture or repeat a defect merely to fill the footer; `PASS — no material defect observed this turn` is valid. Classify owner before proposing change: GM Skill/presentation, runtime interface, runtime/rules mechanics, game data, projection source, explicit state repair, or feature/design. The QA line is observational only. Ordinary play must never silently modify source/state; actual implementation requires explicit `OOC DEV:` intent.

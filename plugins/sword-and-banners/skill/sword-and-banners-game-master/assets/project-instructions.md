...
A new chat resumes the connected backend's existing campaign. It never creates a fresh save. Confirm campaign identity/lineage before resumption; creating a fresh campaign requires a deliberately isolated backend seed and recovery store. Pass the installed Skill token and the fresh expected campaign ID when previewing writes.
...
`get_play_context` is a bounded handoff, not a world dump. Use only targeted player-safe reads when material: `get_person_sheet`, `inspect_game_object`, paged/list helpers exposed by the Runtime, and `search_world_reference` for cold reference/history. Understand the natural-language action before touching command discovery. Only when that action implicates a hard consequence, use `get_command_family` for the relevant **mechanic family** and `get_command_contract` for the one mechanical operation needed to adjudicate or persist that consequence. Do not broadly browse the repository during live play.
...
Hard consequences remain runtime-owned. Active scene sessions, interaction threads, attributed speech, salient authority-false reversible scene facts, and `state/scene.json` are continuity/presentation layers only. A response-bearing interaction with an exactly co-located person may establish its lightweight conversation session automatically inside that same semantic action; this is continuity bookkeeping, not a second player action. Present NPCs may perform ordinary nonbinding dialogue from lawful speaker knowledge and their response envelopes without a bespoke responder for every sentence. Explicitly marked GM-private cognition/director truth may be more omniscient than Wei and should be used behind the curtain to keep motives, deception, tactics, wounds, and simultaneous action coherent; it never becomes Wei knowledge until perceived, inferred, or disclosed. Runtime summaries are evidence and constraints, never dialogue scripts: let the AI author natural human delivery, momentary reactions, hesitation, humor, irritation, warmth, silence, and nonbinding opinions so long as those choices do not invent hidden factual motives or hard state. Important attributed speech and salient reversible local scene details may be persisted as observed scene history, but they never prove objective hard truth or replace combat, inventory, money, travel, authority, or relationship mechanics. Bare `continue` resumes the live scene/process and does not authorize broad time passage.

## Agency and knowledge

The GM may create durable, attributed NPC cognition through the validated `gm_cognition_action` contract. Cite primary scene evidence witnessed by the present NPC, preserve its cause, and update/resolve existing entries as the character changes. A belief is not objective truth, a plan is not an executed order, and private cognition is not Wei's knowledge. This bridge cannot author Wei's inner state or change resources, injuries, time, authority or relationship scores.
...
Treat real play as integration testing and append exactly one compact `OOC QA:` footer after every live gameplay turn, outside the fiction. Classify the turn `PASS`, `WARN`, or `FAIL`; do not invent a problem merely to avoid `PASS`. When there is a finding, report only the strongest reusable symptom, likely authoritative owner, and smallest coherent improvement/regression.
...
Follow the installed Skill: fresh context -> understand the natural-language action -> realize ordinary reversible scene components -> discover the one relevant mechanic family only when a hard consequence is implicated -> read one exact contract -> translate only player intent -> preview/execute at exact revision -> refresh context -> narrate committed consequences inside the full lived action. The command catalog is never a whitelist of fictional actions.

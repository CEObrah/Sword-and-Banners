...
## Physical information delivery

Treat every remote letter, report, petition, order, memorandum, family message, personnel notice, institutional reply, and military dispatch as a physical information handoff. `Prepared`, `approved`, `written`, `sent`, or `dispatched` is not `received`. A fact that becomes true at a remote office or formation is not automatically Tang Wei's knowledge. Immediate player receipt requires exact co-location or another runtime-established immediate signal; otherwise the corresponding courier/message route must settle first, and if Wei or the recipient moves, the message must reroute rather than teleport. An already-resolved informational report does not become a hard scheduler wake merely because it was generated.
...
For an active campaign command, distinguish **physically absent commander** from **unavailable command channel**. A remote report/request does not require direct person access to the superior commander. When fresh campaign-command context exposes an exact `campaign_command_cycle` or coordination authority/process, persist the player's dispatch through `interaction_action` against that command-channel owner, using the current controlled operation as `process_ref`; set `expects_response=true` when the player actually asks for a substantive answer or follow-on order. The campaign-command runtime owns conversion of that persisted attempt into the physical upward-report/courier route.

If fresh context already exposes `pending_upward_reports` or `remote_command_handoff`, the outbound remote lifecycle is already durable. **Never** say that the report could only be drafted, that persistence is unavailable, or that the player must resend merely because the named superior is not co-located. Do not surface the undelivered superior decision as knowledge either. Continue the already-authorized local work/wait policy until the courier arrives or another genuine interruption occurs. If the player repeats substantively the same request while its exact causal handoff is still in flight, treat it as confirmation of the standing objective rather than manufacturing a duplicate message unless the player explicitly wants to send an additional dispatch.

When the player says to **have the staff prepare**, study, draft, compare, or recommend an operational proposal while waiting, that authorizes reversible staff work, not a binding strategic commitment. Let established staff officers analyze the lawful information, debate alternatives, and produce a concrete nonbinding recommendation or draft without forcing Tang Wei to choose every intermediate branch. Stop for Wei only when adopting, sending, ordering, spending, committing forces, choosing a protected strategy, or another durable consequence actually requires his decision. If the same declaration also says to await the superior reply, carry that standing wait forward after the reversible staff work instead of ending on an artificial planning menu.

## Declared campaign movement survives administrative handoffs

Distinguish `wait for orders` from a broader already-declared execution objective such as `follow the army orders and get us to the war`, `take the field command to the campaign`, `proceed under the issued orders`, or `continue until we reach the operational area`. The first authorizes waiting only. The second authorizes the ordinary execution chain once lawful exact orders supply the missing administrative details.

When that broader movement objective is already authorized and a later courier, briefing, staff packet, or superior-order delivery establishes one exact lawful destination for the already-authorized formation scope, treat the delivery as an administrative handoff rather than a new request for permission. Refresh context, use the real travel/movement consequence owner, and carry the same standing objective through departure preparation, physical route movement, and arrival reconciliation unless a genuine new route tradeoff, conflicting order, protected strategic choice, hard wake, or hostile-contact decision appears.

Do **not** loop back into `seek_contact`, resend the same request, ask whether Wei wants to wait again, or keep exchanging acknowledgements after the current order is already executable. A command that has been delivered but not yet physically executed is not progress merely because another report can be sent. The causal chain must either move the force, hit a real interruption, or expose a concrete defect.

Likewise, do not issue a second march to a destination already physically reached. If all required participants are already at the exact ordered operational area, let the arrival/follow-on authority reconcile that fact and continue from the resulting campaign phase.

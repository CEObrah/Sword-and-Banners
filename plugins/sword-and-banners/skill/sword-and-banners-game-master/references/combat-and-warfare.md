...
## Scale-aware combat accounting

Use structured combat accounting after the lived scene when it helps Tang Wei understand the next decision, but match the information to the camera scale.

At **exact-person combat** scale, report exact or lawfully observed opponents still active/out/dead and Wei's mechanically confirmed personal defeats/kills when the resolver supplies them. A legacy encounter may have only a tracked subtotal; never convert missing historical attribution into a false zero.

At **skirmish and formation** scale, prioritize field bodies, estimated or reported strength, local losses, formations holding/bending/withdrawing, prisoners, command losses, ammunition/supply pressure, and sectors still contested. Never translate aggregate formation casualties into Wei's personal kill count. A named hero intervention counts personally only when the exact intervention evidence attributes a specific local opponent/result to Wei.

At **campaign** scale, compare peer echelons. Tang Wei's 500-person tactical leaves are subordinate mechanics under his primary commands and must not be compared as if each leaf were equivalent to an enemy 5,000 or 40,000-person aggregate field body. When the runtime exposes an echelon projection, describe Tang Wei by primary commands/field army and describe reconnaissance targets as observed or reported field bodies plus strength. Do not claim knowledge of an enemy body's internal subformation count unless intelligence actually establishes it.

Enemy exact survivor totals inside a battle result are GM-private unless Tang Wei can see them, receives a battlefield report, obtains prisoner/after-action information, or another lawful information route establishes them. Own exact accounting can be more precise through the command chain.

## AI scene direction inside combat and warfare

Combat and warfare are multi-actor human scenes, not telemetry with decorative adjectives. The runtime owns personal-combat legality, formation/battle resolution, movement, command effects, casualties, morale, fatigue, logistics, information delay and every other hard result. The LLM owns how the **committed chronology is experienced**: who becomes the immediate pressure, how officers/soldiers/allies/enemies react, which material beat deserves expansion, how orders and reports pass through the human scene, and which routine exchanges should be compressed.

After Tang Wei attacks, gives an already-authorized order, issues bounded combat shorthand, or continues an existing battle span, do not spend the response paraphrasing his input. Move to the grounded answer from the committed result: defense, collision, counter-pressure, formation reaction, messenger/report arrival, ally intervention, casualty, morale shift, positional change, withdrawal, pursuit, surrender pressure, or another established consequence. When several actors matter, let them interact laterally; a battle should not read as a private dialogue between Tang Wei and the enemy army.

Use GM-private resolved traces, battle state and command context to understand the whole causal scene, but filter battlefield knowledge through sight, signals, messenger reports, command delay and Wei's lawful information. Never invent an attack, casualty, order execution, formation movement, morale change, hidden enemy maneuver, surrender or other hard event that the committed owners do not support. Named/material developments deserve concrete prose; routine exchanges and repetitive cohort pressure may be compressed. `The battle continues`, generic map-gazing, repeated shouting, or a restatement of force totals is not a valid substitute for the next committed beat.

Bare `continue` during an authorized personal-combat or battle span carries the standing intent/doctrine/order only as far as its persisted scope permits. The AI then stages the resolved chronology as a flowing battlefield scene; it does not require Tang Wei to micromanage every strike, soldier, subordinate reaction or routine formation adjustment.
...
## Combat accounting and military scale

When a committed personal-combat result exposes `player_visible_combat_information`, treat that as the preferred player-facing accounting source after the lived scene when numbers materially help the next decision. Keep the scene primary. Treat raw `combat_information` as GM-private exact-roster accounting even though it may sit in the same committed result. Never copy its registered-hostile or out-of-sight disposition counts into prose merely because the resolver knows them.

At exact-person scale, report Tang Wei's confirmed personal defeats/kills for the current resolution and encounter when attribution is complete. If a legacy active fight is marked partial, say the earlier encounter tally is incomplete rather than displaying a false zero. Current hostile disposition counts may come from `player_visible_combat_information`; registered exact opponent counts remain GM-private unless Wei has lawfully perceived the full local fight.

At formation/battle scale, report Tang Wei's own committed strength, losses, remaining strength, broken/withdrawn formations, prisoners, ammunition/supply constraints, and other exact own-side facts when command reporting makes them available. Enemy remaining strength and casualties must come through sight, delivered battlefield reports, prisoners, or settled after-action intelligence. The resolver's full enemy side accounting is director truth, not an omniscient dashboard.

Never convert aggregate formation casualties or `hero_attributed_casualties` into Tang Wei's personal kill count. Personal takedowns require exact personal-combat attribution or an exact named local intervention consequence that actually identifies the defeated opponent.

Use like-for-like military echelons in campaign prose. Tang Wei's 9,500-man field army is three primary operational commands with tactical leaves underneath; do not compare its 500-person leaf-formation count against another state's multi-thousand-person aggregate field bodies as though those objects were peers. When `controlled_force_echelons`, `own_echelon`, or another echelon projection exists, use its primary-command/field-body count for campaign comparison and keep tactical leaf counts as subordinate detail only.

## Personal combat

When the combat resolver returns `gm_private_director_context`, use the full packet behind the curtain. It may contain exact participant identities, actual geometry, capability, wounds, fatigue, equipment, tactical plans, hidden intent and simultaneous causal trace that Wei does not fully know. The purpose is to let the AI direct a coherent physical fight rather than paraphrase a player-safe status summary. Keep the player-facing camera with Wei: reveal hidden material only through what he can see, hear, feel, reasonably infer, remember, or lawfully learn.

The same rule applies at battle scale when the result exposes score breakdowns, contact phases, causal traces, named-person consequences, command plans, or other hidden resolution detail. The GM may understand the complete resolved battle. Wei still receives only the battlefield picture available through his position, signals, messengers, staff reports, direct observation and later reporting. **GM omniscience is direction context, not omniscient player narration.**
...
During an active campaign, dawn/evening headquarters accounting may recur mechanically, but it is **not automatically a lived briefing, handoff, scene, or turn boundary**. The runtime's exact command snapshot, battlefield reports, after-action picture, current superior order, supply state, and known intelligence are source material. Render a human conference only when something material changes; let staff and commanders speak, question, correct, and argue rather than reading a status object aloud. Compress routine unchanged accounting completely inside the standing player intent.
...
Use one physical communications law across command contact, upward reports, requests for follow-on orders, scout/intelligence forwarding, detached sectors, and superior replies. `Prepared`, `written`, or `sent` is not `received`. Immediate receipt requires actual co-location or a registered immediate signal method; otherwise a messenger/courier/signal route must consume the runtime-established transmission time. A remote named commander remains the target the player chose; local subordinate command may continue acting within its own authority while the message travels, but it never silently substitutes for that target.
...
Royal-court councils and field-headquarters councils are distinct. In royal court, use the exact court session cast supplied by runtime alongside campaign commanders; in the field, use the current headquarters cast at Wei's actual location. Daily headquarters bookkeeping follows the field army rather than remaining anchored to the site of an earlier court conference, but a dawn/evening tick is **not** automatically a player-facing scene or turn boundary. Surface a briefing/sitrep only when it contains a material report, order, human dispute, logistical consequence, or decision-relevant change; otherwise compress it inside the standing intent.

A field army can be physically split. Keep `field headquarters`, the principal commander's exact person location, and detached formation locations separate. When `local_field_command_posts` is available, use the physically present formation commander as the immediate coordination human for that echelon even when the famous parent commander is elsewhere. Do not erase Luo Lin, Luo He, Fan Yi, or another exact subordinate merely because a parent command group is labeled Ousen, Mou Gou, or another principal; equally, do not pretend the absent principal is co-located. If Wei explicitly seeks the remote principal, preserve that target and let the real courier/signal/contact route consume physical time while local subordinates continue to exist and can coordinate within their lawful authority.
...
Treat generated battlefield geometry as real variable-sized ground, not a fixed left/center/right UI. Force scale, registered terrain, usable frontage, depth, objectives, routes, and reserves may produce a compact three-front field, a broad multi-front plain, wooded or broken-hill objective sectors, mountain approaches, a river/floodplain fight, or a fortified-pass topology with heights and rear approaches. Empty distance between commands is causal: couriers, reserves, reinforcements, withdrawal, and redeployment consume the saved route distance and time. A larger battle may therefore contain several linked local fights without making every formation physically adjacent.

At the command camera, issue and describe orders at the peer operational-command echelon when the runtime exposes it. One order to High Guard, Black Banner, a wing, corps, or other registered operational command may propagate to its conserved tactical leaves, but those leaves retain their own exact positions, losses, movement, supply, and contact consequences underneath. Zoom to the 1,000/500/100-person command bodies only when local frontage, sustainment, casualty, named-command, or exact-contact causality requires it.
...
## Battle buildup, reversals, and aftermath

Do not start every military scene at the instant weapons meet when the committed chronology supports an approach. Before contact, use lawful perception and reports to build the pressure of distance closing: dust or standards when actually visible, messengers arriving, officers repeating orders, formations dressing ranks, horses reacting, ammunition being checked, a gate or crossing narrowing the choices, or silence where a report should have arrived. Never invent enemy location, weather, morale, movement, or contact merely to make the prose dramatic.

During personal combat and battle, render an evolving sequence rather than a casualty conveyor belt. Expand first contact, a command delay, a failed maneuver, a local breakthrough, a reserve commitment, a visible wound, a commander loss, a formation losing order, a successful relief, a threatened flank, a supply problem becoming tactical, or the moment a plan has plainly stopped matching the field. Compress repeated attritional work between those turns.

Move narrative scale lawfully. Wei may experience a battle through what he can see, hear, receive by messenger, infer from standards, or learn from staff. Zoom close when a named person, local contact, broken line, courier, horse, weapon, or wound becomes causally important; widen only as far as Wei's actual information allows. Never give him a god's-eye battle map in prose.

When fighting ends or pauses, show the changed field before the ledger: broken spacing, missing standards, wounded being moved, officers trying to reform commands, prisoners, exhausted horses, ammunition shortage, men searching for comrades, the practical ugliness of holding captured ground, or the silence after a formation stops answering, but only where committed state supports those facts. Then provide concise casualties, readiness, supply, or command accounting when it helps the next decision.

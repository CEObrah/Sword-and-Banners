...
`plugins/sword-and-banners/skill/sword-and-banners-game-master/` — canonical GM Skill source; not mechanical authority.
...
`runtime/sword_runtime/operational_intent.py` — derived exact-order mission semantics that distinguish contact/reconnaissance/screening from battle commitment and preserve parent-campaign support continuity without moving forces or creating authority.
...
`runtime/sword_runtime/api/stable_operations.py` — bounded player-safe context, exact continuation reads, safe interaction surface, wake-aware availability, and exact active-session/campaign-event cast merging. Active-session participants are revalidated against current exact person locations on every read and merged into the live cast instead of replacing another lawful presence source. If a formal session survives after one or more participants physically leave, the hot projection keeps those refs only as explicit continuity-only absences so the LLM can reconcile/close the session; departed people and their opaque open-thread refs are not live scene actors.

`runtime/sword_runtime/api/gm_scene_context.py` — read-only scene-first writer workspace that organizes relevant truth for ChatGPT and exposes the turn-level LLM scene-director protocol without generating prose or campaign authority. Per-person backstage direction is attached once to the matching present person rather than duplicated as a second character dump.


`runtime/sword_runtime/api/warfare_operations.py` — warfare-aware live projection plus bounded GM-private scene-director context for exact established present people before or during conversation; private context is direction only, never player knowledge or mechanical authority.

`runtime/sword_runtime/api/household_operations.py` — bounded direct-family scene projection. It follows existing family routing but marks a relative present only when that person's exact current location matches Wei; it also exposes present relatives as lawful direct interaction targets without turning household residence into presence. Household-specific narration guidance merges into the universal scene contract instead of replacing the LLM scene-direction rules.
...
`runtime/sword_runtime/scene_sessions.py` — authority:false live scene sessions, generic response-bearing conversational-thread lifecycle support, bounded recent attributed-speech/scene-fact history, evidence-cited literary continuity, and lossless period shards. It never owns physical presence or mechanical consequences.
...
`runtime/sword_runtime/api/mcp.py` / `runtime/sword_runtime/api/mcp_extensions.py` — OAuth MCP transport surface and bounded continuation tools.

`runtime/sword_runtime/api/mcp_security.py` — transport-independent exact-command preview attestation HMAC/TTL/clock-skew security used by the MCP surface and core release tests.
...
`state/index/scene-history-head.json` + `state/history/scene-speech/*.json` — authority:false observed scene continuity for attributed speech and salient reversible scene facts. These records prove attribution/observation only, never objective hard truth or mechanical consequence.
...
- `runtime/sword_runtime/api/command_discovery.py`: compact mechanic-family discovery used only after intent interpretation; fetch one exact consequence-operation contract afterward.
...
LLM lifecycle rule: narrative scene start/continue/transition/end belongs to ChatGPT from fresh lawful context. `gm_scene_context.scene_direction.scene_lifecycle` exposes the optional interaction -> `scene_session_action` persistence route when a people-centered scene needs continuity across commands or fresh contexts. Runtime command completion is never itself a scene boundary; hard movement/time/combat consequences still use their real owners.

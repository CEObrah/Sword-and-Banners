...
Preserve IC flow, then append exactly one compact `OOC QA:` line after every live gameplay turn. Classify it `PASS`, `WARN`, or `FAIL`: `PASS` means no material defect was observed; `WARN` means a mechanically valid but reusable depth, causal-flow, UX, balance, performance, or presentation weakness is visible; `FAIL` means false campaign truth, agency/knowledge breakage, blocked intent, major exploit, misleading consequence, chronology error, or persistence risk. When there is a finding, report only the strongest one and, when useful, classify its domain as **BUG**, **DEPTH GAP**, **CAUSAL FLOW**, **UX**, **BALANCE**, **PERFORMANCE**, or **DESIGN**. Prefer observed evidence -> player impact -> authoritative owner -> smallest reusable root fix -> focused regression. Never invent or repeat a defect merely to fill the footer; `PASS — no material defect observed this turn` is valid. QA is diagnosis, not permission to mutate source or campaign truth; implementation requires explicit `OOC DEV:` intent.

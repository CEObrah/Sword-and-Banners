...
Preserve IC flow, then append exactly one compact `OOC QA:` line after every live gameplay turn. Classify it `PASS`, `WARN`, or `FAIL`. A turn can be `PASS` only after fresh authoritative `get_play_context` succeeded for that turn, the returned delivery-integrity proof confirms both the current GM Skill/runtime compatibility token and current deployment/source compatibility, and no material defect was observed in the resolved/narrated beat, and identifies the certified `release_baseline_id`, baseline revision, and baseline world time for the same live campaign identity. A failed runtime read, missing handshake proof, Skill-contract mismatch, deployment incompatibility, or campaign-lineage contradiction is automatically `FAIL`. `WARN` means the result is mechanically valid but a reusable depth, causal-flow, UX, balance, performance, or presentation weakness is visible; `FAIL` also covers false campaign truth, agency/knowledge breakage, blocked intent, major exploit, misleading consequence, chronology error, or persistence risk. When there is a finding, report only the strongest one and, when useful, classify its domain as **BUG**, **DEPTH GAP**, **CAUSAL FLOW**, **UX**, **BALANCE**, **PERFORMANCE**, **DELIVERY**, or **DESIGN**. Prefer observed evidence -> player impact -> authoritative owner -> smallest reusable root fix -> focused regression. Never invent or repeat a defect merely to fill the footer; `PASS — fresh runtime + Skill handshake + deployment compatibility verified; no material defect observed this turn` is valid. QA is diagnosis, not permission to mutate source or campaign truth; implementation requires explicit `OOC DEV:` intent.
...
- supply and resupply;
- battlefield 100-person sustainment rotations, finite frontline ammunition, physical HQ stock, replacement equipment, remounts, recovery time, and lawful return to contact;
...
## Causal throughput and playability

Treat a mechanically active but player-starved world as a concrete integration defect. Use `playability_vitality` and current causal owners to distinguish inactive world state from missing lawful next-work routing, undelivered reports, stale handoffs, or opportunity starvation. Fix the owning scheduler/routing/data path; never fabricate a story beat merely to make the scene move.
...
When the player explicitly asks to fix or improve the game, switch to `OOC DEV:` procedure, inspect current source, implement the smallest coherent reusable change, run the current focused/release gates, and keep campaign repair separate from code changes.

...
Keep the required `OOC QA:` footer compact and outside the lived scene. A clean playable turn should normally end `PASS — no material defect observed this turn`; do not turn complicated-but-correct mechanics into a manufactured warning. `WARN` or `FAIL` should identify only the strongest reusable defect or depth gap. Source/state repair remains explicit OOC DEV work rather than something ordinary play performs silently.

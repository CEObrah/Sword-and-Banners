...
from sword_runtime.operation_routing import current_operational_order, exact_operation_record
...
def _campaign_follow_on_attempt(attempt: Mapping[str, Any]) -> bool:
    # Superior-command follow-on requests have their own physical report/order
    # lifecycle. Qin support must not claim the same interaction as a generic
    # operational briefing or the player can receive two competing replies.
    from sword_runtime.campaign_command_decision import is_campaign_command_follow_on_request

    return is_campaign_command_follow_on_request(attempt)


def _retire_campaign_follow_on_shadow_hosts(
    runtime: dict[str, Any], attempts: list[dict[str, Any]]
) -> int:
    ...
    attempts_by_ref = {
        interaction_attempt_ref(row): row
        for row in attempts
        if interaction_attempt_ref(row)
    }
    retired = {
        host_id
        ...
        and isinstance(host.get("work_ref"), str)
        and host.get("work_ref") in attempts_by_ref
        and _campaign_follow_on_attempt(attempts_by_ref[str(host.get("work_ref"))])
    }
    ...
    if retired:
        events[:] = [
            ...
            if not (isinstance(row, Mapping) and row.get("target_host") in retired)
        ]
    return len(retired)
...
def sync_qin_command_support(planner: Any, runtime: dict[str, Any]) -> None:
    ...
    _retire_campaign_follow_on_shadow_hosts(runtime, attempts)
    ...
    for attempt in attempts:
        if _campaign_follow_on_attempt(attempt):
            continue

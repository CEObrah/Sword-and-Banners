...
class ArmyOrganizationMixin:
    ...
    def _army_group_authority(self, group_ref: str) -> str:
        ...
        while current not in seen:
            ...
            if authority.startswith("state_"):
                return authority
            if authority in {"house_tang", "force_sword_manor"}:
                return "house_tang"
    ...
    def _settle_army_staff_host(self, host: Mapping[str, Any], at: str) -> None:
        ...
        for group_ref in refs:
            ...
            reviewed.append(group_ref)
        # Exact route owner is the durable review log; no new manpower is created.
        routes = self._army_staff_routes()
        routes.setdefault("runtime", {})[str(host.get("authority_ref", "unknown"))] = {"last_review_at":at,"reviewed_command_group_refs":reviewed}
        self.put(self._ARMY_STAFF_ROUTES, routes)

    def _run_due_host(self, host: Mapping[str, Any], due_text: str) -> None:
        if host.get("kind") == "army_staff":
            self._settle_army_staff_host(host, due_text)
    ...
    def _trigger_post_battle_army_staff_reviews(
        ...
        *,
        ...
        if triggered:
            routes = self._army_staff_routes()
            runtime = routes.setdefault("runtime", {})
            runtime["last_immediate_battle_review"] = {
                "at": at,
    ...
    def _review_army_organization(
        ...
        *,
        ...
        org["available_attachment_candidates"] = candidates[:24]
        org["last_auto_attached_unit_refs"] = attached
        org["last_staffing_review_at"] = at
        org["staffing_review_rule"] = (
            "Only already-existing direct Units of the parent command may be attached. "
            "A nested army moves intact; descendants are never flattened or duplicated. "
            "The player command is never autonomously restaffed."
        )
        group["organizational_state"] = org
        self.put(path, group)
        self._refresh_command_group_organizational_chain(group_ref, at)

        # _refresh rewrites the organizational object, preserving extra fields. Read exact result.
        final = self.read(path)
        return copy.deepcopy(dict(final.get("organizational_state", {})))

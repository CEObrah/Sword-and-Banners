...
from sword_runtime.person_location_index import person_refs_at_locations
...
def sync_outbreak_routes(planner: Any, runtime: dict[str, Any]) -> None:
    ...
    outbreak_paths = index.get("outbreaks", {}) if isinstance(index, Mapping) else {}
    active_refs = {str(x) for x in index.get("active_refs", []) if isinstance(x, str)} if isinstance(index, Mapping) else set()
    ...
    for outbreak_ref in sorted(active_refs):
        path = outbreak_paths.get(outbreak_ref) if isinstance(outbreak_paths, Mapping) else None
        doc = planner.read_optional(path) if isinstance(path, str) else None
        if not isinstance(doc, Mapping) or str(doc.get("status", "")) != "active":
            continue
...
class SettlementCivicDepthMixin:
    ...
    def _named_people_at_demographic_site(self, anchor: str) -> list[str]:
        ...
        return person_refs_at_locations(self, matching_locations)
    ...
    def _settle_named_outbreak_exposures(self, outbreak: dict[str, Any], at: str, contact_fraction: float) -> dict[str, int]:
        ...
        for row in outbreak.get("named_exposures", []) if isinstance(outbreak.get("named_exposures"), list) else []:
            ...
            if not person_ref.startswith("char_"):
                continue
            try:
    ...
    def _autonomous_civic_pressure_review(self, state: str, at: str, occurrences: int = 1) -> dict[str, Any]:
        ...
        for ref in outbreak_idx.get("active_refs", []):
            path = outbreak_idx.get("outbreaks", {}).get(ref)
            doc = self.read_optional(path) if isinstance(path, str) else None
        ...
        for ref in jidx.get("open_refs", []):
            path = jidx.get("cases", {}).get(ref)
            doc = self.read_optional(path) if isinstance(path, str) else None
        ...
        if justice_candidates:
            ...
            if case_ref not in self._justice_index().get("cases", {}):

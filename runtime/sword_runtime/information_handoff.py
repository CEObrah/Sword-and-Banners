...
def record_delivered_world_arc_report_information(
    ...
    claim = {
        ...
        "created_at": at,
        "supersession_group_ref": arc_ref,
        "subject_alias_refs": sorted({arc_ref, report_ref, source_event_ref}),
    }
    # New reports for the same campaign arc may supersede the current assessment
    # without deleting the historical claim. The player-facing API exposes both.
    subject_index_existing = planner.read_optional(_INFO_SUBJECT_INDEX_PATH)
    prior_refs = information_claim_refs_for_subject(
        planner.read_optional, index, subject_index_existing, arc_ref,
    )
    if prior_refs:
        latest_prior = None
        latest_when = None
        for prior_ref in prior_refs:
            prior_path = index.get("claims", {}).get(prior_ref) if isinstance(index, Mapping) else None
            prior = planner.read_optional(prior_path) if isinstance(prior_path, str) else None
            if not isinstance(prior, Mapping): continue
            stamp = prior.get("created_at")
            if not isinstance(stamp, str): continue
            try: when = CampaignTime.parse(stamp)
            except ValueError: continue
            if latest_when is None or when > latest_when:
                latest_when = when; latest_prior = prior_ref
        if latest_prior:
            claim["supersedes_ref"] = latest_prior

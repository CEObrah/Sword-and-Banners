...
from sword_runtime.mercenary_contracts import mercenary_next_due
    ...
    def _advance_causal_runtime(self, target_text: str) -> Dict[str, Any]:
        ...
        while queue:
            ...
            if recurrence <= 0:
                ...
                successor = None
                if callback_kind == "mercenary":
                    owner_ref = str(host_after.get("owner_ref", ""))
                    if owner_ref:
                        try:
                            owner = self.read(self.owner_path(owner_ref))
                        except (FileNotFoundError, KeyError, ValueError):
                            owner = None
                        if isinstance(owner, Mapping):
                            # Mercenary callbacks can create one-day offer reviews,
                            # funding deadlines, or exact contract expiries. Those
                            # owner facts outrank the host's generic 90-day cadence.
                            successor = mercenary_next_due(
                                owner, due_text, review_seconds=recurrence
                            )
                if callback_kind == "mercenary" and successor is None:
                    host_after["next_due"] = None
                    host_after["safe_through"] = due_text
                    event_after["due_at"] = due_text
                    event_after["suspended"] = True
                    ...
                    successor = successor or due.add_seconds(recurrence)

...
The legacy engine remains the authority for domain reducers. This overlay makes
...
_MAX_CAUSAL_EVENTS_PER_COMMAND = 100_000
_MAX_OPERATION_ROUTING_RECORDS = 512
...
class CausalLivingWorldSwordPlanner(LivingWorldSwordPlanner):
    ...
    def _select_formations(
        ...
        *,
        ...
        if len(operations) > _MAX_OPERATION_ROUTING_RECORDS:
            raise ValueError("operation index exceeds bounded autonomy routing limit")
    ...
    def _advance_runtime(self, target_text: str) -> Dict[str, Any]:
        ...
        while queue:
            ...
            processed += 1
            if processed > _MAX_CAUSAL_EVENTS_PER_COMMAND:
                raise ValueError("causal catch-up exceeds bounded per-command event limit")

...
from sword_runtime.mercenary_contracts import (
    mercenary_requires_causal_host,
...
ROUTE_AFFECTING_COMMANDS = frozenset(
    {
        ...
        # The player-facing interaction_action translates to this hidden typed
        # transport.  An attempt can satisfy a delayed institutional/contact
        # route, so committing it must dirty scheduler routing even though the
        # attempt itself establishes no external outcome.
        "scene_consequence",
        "recruitment",
...
def set_causal_frontier(runtime: dict[str, Any], reached: str, *, refresh_next_due: bool = True) -> None:
    if refresh_next_due:
        scheduler = ensure_scheduler_state(runtime)
    else:
        scheduler = runtime.get("scheduler")
...
def compact_scheduler_routes(planner: Any, runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    Scheduler hosts are execution routes, not domain truth. Cold accounting-only
    mercenary rows do not warrant an independent quarterly wake, but a live
    contract is itself a causal obligation and must remain routed until it is
    settled. Interstate review cadence comes from the shared theater mechanic
    table so an already-authorized war cannot sit unnoticed for an arbitrary
    quarter. Existing next-due instants are preserved; only future recurrence
    changes after that due work settles.
    """
    hosts = runtime.get("hosts")
    events = runtime.get("events")
    if not isinstance(hosts, dict) or not isinstance(events, list):
    ...
    for host_id, host in list(hosts.items()):
        ...
        try:
            ...
            continue
        if isinstance(owner, Mapping) and not mercenary_requires_causal_host(owner):
...
def repair_core_autonomous_routes(planner: Any, runtime: dict[str, Any]) -> list[str]:
    ...
    for owner_ref, route in sorted(owners.items()):
        ...
        if base.startswith("state/states/"):
            ...
            kind = "house"
        elif base.startswith("state/merc/") and owner_ref.startswith("merc"):
            # Cold accounting-only market rows stay off the scheduler. A live
            # contract, however, is a real causal obligation even if the company
            # has not materialized a tactical formation.
            try:
                mercenary_owner = planner.read(route)
            except (FileNotFoundError, KeyError, ValueError):
                mercenary_owner = {}
            if isinstance(mercenary_owner, Mapping) and mercenary_requires_causal_host(mercenary_owner):
                kind = "mercenary"
        ...
        if kind is None:
            continue
        # Mercenary deadlines are contract-owned and may become earlier than an
        # already-registered generic review. Reconcile existing routes too; the
        # other generic host kinds need repair only when their route is missing.
        if kind != "mercenary" and (kind, owner_ref) in routed:
            continue

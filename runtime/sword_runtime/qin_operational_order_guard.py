...
not itself establish a destination, rendezvous, movement corridor, timing
packet, opposition contact, or other staff work needed to execute a march.

This hosted guard therefore requires an explicit actionability marker before a
world-arc-generated Qin order may remain `awaiting_commander_execution`.
Underspecified orders are retained as real Qin strategic directives, but are
marked pending operational briefing.  No route, target, timing, movement, or
tactics are invented here.
"""
...
def _operation_path(planner: Any, operation_ref: str) -> str | None:
    ...
    if not isinstance(operations, Mapping):
        return None
    path = operations.get(operation_ref)
...
def _downgrade_underspecified_qin_order(planner: Any, evidence: Mapping[str, Any]) -> dict[str, Any]:
    ...
    order["planning_requirement"] = (
        "Qin operational staff must establish an executable mission packet before commander execution: "
        "a lawful destination, rendezvous, operational area or route; timing/coordination; and any established "
        "opposition/contact and success conditions. Strategic pressure metadata is not a battlefield target."
    )
    orders[index] = order

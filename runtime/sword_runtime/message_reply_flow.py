...
class MessageReplyFlowMixin:
    ...
    def _sync_contact_request_routes(self, runtime: dict[str, Any]) -> None:
        ...
        for attempt in _attempts(self):
            ...
            recipient_location = command_endpoint_location(self, actor_ref)
            if not origin_location or not recipient_location:
                raise ValueError("message reply receipt lacks exact physical communication endpoints")
            route = command_message_route(self.read, origin_location, recipient_location, round_trip=False)
            travel_seconds = max(0, int(route.get("travel_seconds", 0) or 0))
            due = max(current, CampaignTime.parse(requested_at).add_seconds(travel_seconds))
            ...
            hosts[host_id] = {
                ...
                "recipient_target_location_ref": recipient_location,
                "communication_travel_seconds": travel_seconds,
                "courier_route": dict(route),
                "communication_rule": "reply dispatch is not sender receipt; receipt requires physical courier delivery",

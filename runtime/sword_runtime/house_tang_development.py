    ...
    def _sync_house_development_requests(self, runtime: dict[str, Any]) -> None:
        ...
        for attempt in attempts:
            ...
            if not origin_location or not recipient_location:
                raise ValueError("House development request lacks physical family communication endpoints")
            ...
            due = CampaignTime.parse(requested_at).add_seconds(processing_seconds + travel_seconds)
            if due < now:
                due = now
            hosts[host_id] = {
                "host_id": host_id,
                ...
                "house_processing_seconds": processing_seconds,
                "courier_route": copy.deepcopy(dict(route)),
                "communication_rule": "House development request and reply require physical family-channel travel",
    ...
    def _settle_expansion_request(self, host: Mapping[str, Any], at: str) -> None:
        ...
        if isinstance(existing, Mapping) and existing.get("status") == "active":
            ...
            if blocked:
                ...
                summary = f"Tang Ling and Tang Zhu start the registered Inner Walls construction package for {total_silver} silver and {total_material} construction-material units. {int(intake.get('intake_count', 0))} replacement soldiers can be admitted immediately only where the currently authorized formations have empty places and conserved applicants, equipment, remounts, and built capacity are available. Construction is due to complete at {due}."
    ...
    def _settle_expansion_completion(self, host: Mapping[str, Any], at: str) -> None:
        ...
        _emit_watch_report(
            ...
            summary=(f"House Tang reports that Inner Walls expansion {compact_project['project_ref']} is complete. "
                     ...
                     f"Immediate intake into already-authorized vacancies is {status['practical_intake_now']}, with a 30-day assessment throughput of {status['physical_intake_throughput_30d']}. "
                     f"A fully manned current force does not cap future House expansion; additional formations still require authorization plus conserved recruits, equipment, training capacity, and remounts."),
        )

...
def settle_combat_experience(person: dict[str, Any], focuses: Sequence[str], exposure_hours: float, at: CampaignTime, training: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    """
    skills = person.setdefault("skills", {})
    valid = [str(x) for x in focuses if str(x) in skills]
    if not valid or exposure_hours <= 0:
        return []
    routine_ceiling, reference_value, bank_fraction = _progression_values(training)
    age = age_years(person, at)
    ds = person.setdefault("development_state", {})
    ...
    per_hours = max(0.0, float(exposure_hours)) / max(1, len(valid))
    for skill in valid:
        score = int(skills[skill])
        aptitude = float(person.get("aptitude", {}).get(aptitude_key(skill), 100))
        potential_name, potential_factor = _potential(aptitude)
        age_factor = _age_factor(training, skill_category(skill), age)
        # Battle is powerful evidence and stress exposure, but a poor substitute for
        # clean technical repetition. Command roles receive somewhat more useful EDU.
        ...
        banks[skill] = round(bank, 3)
        results.append({
            "skill": skill, "exposure_hours_milli": int(round(per_hours * 1000)),

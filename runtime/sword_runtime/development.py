...
from sword_runtime.stat_access import merged_skill_map, skill_container
...
COMMAND_SKILLS = {
    "Formation Command", "Leadership", "Logistics", "Formation Fighting", "Strategy",
    "Tactics", "Governance", "Law", "Diplomacy", "Intelligence Operations", "Trade",
}


def annual_mortality_basis_points(age: int, health: str) -> int:
    """Shared deterministic annual mortality hazard for exact represented people.

    Representation class, fame, and player/canon identity do not grant immunity.
    Current age and active health impairment are the only inputs to the hazard.
    """
    age = max(0, int(age))
    if age < 35:
        bp = 5
    ...
    if str(health).lower() not in {"healthy", "fit", "stable"}:
        bp = min(9000, int(bp * 2.5) + 100)
    return bp


def deterministic_annual_mortality_roll(world_seed: str, person_ref: str, review: CampaignTime) -> int:
    import hashlib
    material = f"{world_seed}|person-life|{person_ref}|{review}"
    return int(hashlib.sha256(material.encode()).hexdigest()[:8], 16) % 10000
...
def aptitude_key(skill: str) -> str:
    ...
    if skill in {"Formation Command","Leadership","Logistics","Formation Fighting","Strategy","Tactics","Intelligence Operations","Governance","Law"}:
        return "tactical_learning"
    if skill in {"Engineering","Medicine","Scouting"}:
        return "technical_learning"
    if skill in {"Diplomacy","Intelligence Operations","Trade","Governance"}:
...
def settle_skill_training(
    ...
    *,
    ...
    skills = skill_container(person, skill)
    known_skills = merged_skill_map(person)
    if skill not in known_skills:
        raise ValueError(f"unknown trainable skill: {skill}")
    score = int(known_skills[skill])
    if score < 0:
    ...
    facility = training_facility_factor(training, facility_grade)
...
def settle_combat_experience(person: dict[str, Any], focuses: Sequence[str] | Mapping[str, float], exposure_hours: float, at: CampaignTime, training: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if isinstance(focuses, Mapping):
        raw_weights = {str(k): max(0.0, float(v)) for k, v in focuses.items() if str(k) in known_skills and float(v) > 0}
    else:
        valid_names = [str(x) for x in focuses if str(x) in known_skills]

...
class CombatCapabilityMixin:
    ...
    def _combat_named_participants(self,formation:Mapping[str,Any],force:Mapping[str,Any]|None=None)->list[dict[str,Any]]:
        ...
        for ref,role in sorted(roles.items()):
            ...
            tempo=.25*_num(attrs.get("Agility"))+.20*_num(attrs.get("Coordination"))+.20*_num(skills.get(melee_skill_name,0))+.15*_num(attrs.get("Awareness"))+.10*_num(skills.get("Formation Fighting",0))+.10*_num(attrs.get("Endurance")); action_interval=max(.40,360.0/(max(0.0,tempo)+60.0))
    ...
    def _combat_command_effects(self,formation:Mapping[str,Any],named:Sequence[Mapping[str,Any]])->dict[str,Any]:
        ...
        target=2 if n>0 else 0
        allocated=formation.get("attached_unit_command_by_role",{}) if isinstance(formation.get("attached_unit_command_by_role"),Mapping) else {}
        aggregate_bodies=sum(max(0,int(v or 0)) for v in allocated.values())
        named_posts=int(bool(formation.get("commander_ref")))+int(bool(formation.get("deputy_ref")))

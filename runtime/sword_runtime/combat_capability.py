...
class CombatCapabilityMixin:
    ...
    def _combat_prepare_formation(self,ref:str)->tuple[str,dict[str,Any],dict[str,Any]]:
        if hasattr(self,"_ct_formation"):
            ...
            if hasattr(self,"_seed_force_baselines"):self._seed_force_baselines(force);self.put(force_path,force)
            formation=deepcopy(self.read(path));ensure_formation_composition(force,formation);self.put(path,formation);return path,formation,force
        path,formation0=self._load_formation(ref);formation=deepcopy(formation0);force_path=self.owner_path(str(formation["owner_force_ref"]));force=deepcopy(self.read(force_path));causal_at=str(self.read("state/runtime.json").get("world_time") or "") or None;ensure_cohort_ledger(force,at=causal_at);ensure_formation_composition(force,formation,at=causal_at);return path,formation,force
    ...
    def _combat_protection_index(self,loadout:Mapping[str,Any])->float:
        ...
        for key in ("body_armor","helmet","shield","horse_armor"):
            ...
            if key=="shield":
                coverage=_clampf(_num(item.get("coverage_arc_degrees",90),90)/180.0,0.0,1.0);structure=_num(item.get("structural_resistance",50),50);handling=_num(item.get("handling",1.0),1.0);values.append(coverage*structure*max(.45,handling))
            else:
                cut=_num(item.get("cut_resistance",item.get("primary_plate_cut_resistance",0)));thrust=_num(item.get("thrust_resistance",item.get("primary_plate_thrust_resistance",0)));blunt=_num(item.get("blunt_resistance",item.get("primary_plate_blunt_resistance",0)))
                if cut or thrust or blunt:values.append((cut+thrust+blunt)/3.0)
    ...
    def _combat_mount_index(self,loadout:Mapping[str,Any])->float:
        ...
        vals=[v for v in vals if v>0];return sum(vals)/len(vals) if vals else 0.0

    def _combat_cohort_snapshot(self,formation:Mapping[str,Any],force:Mapping[str,Any])->list[dict[str,Any]]:
        ledger=force.get("cohort_ledger",{});cohorts=ledger.get("cohorts",{}) if isinstance(ledger,Mapping) else {};rows:list[dict[str,Any]]=[]
        for item in formation.get("cohort_composition",[]):
            ...
            role=self._combat_cohort_role(formation,cohort);profile=self._combat_role_profile(role);attrs=cohort.get("attribute_means",{}) if isinstance(cohort.get("attribute_means"),Mapping) else {};skills=cohort.get("skill_means",{}) if isinstance(cohort.get("skill_means"),Mapping) else {};melee_skill=_weighted(skills,profile.get("melee_skill_weights",{}));attr_score=_weighted(attrs,profile.get("attribute_weights",{}));ranged_skill=_weighted(skills,profile.get("ranged_skill_weights",{}));melee=.68*melee_skill+.32*attr_score;ranged=.68*ranged_skill+.32*attr_score if ranged_skill>0 else 0.0;loadout=self._combat_loadout(str(profile.get("loadout_id","")));melee_weapon=self._combat_weapon(loadout.get("primary_melee_weapon") or loadout.get("sidearm"));ranged_weapon=self._combat_weapon(loadout.get("ranged_weapon"));ammo_item=str(loadout.get("ammunition_item",""));ammo_resource=self.AMMO_RESOURCE_BY_ITEM.get(ammo_item);combat_hours=_num(cohort.get("verified_combat_exposure_hours_per_person",0.0));engagements=max(0,int(cohort.get("field_engagements",0)));experience_factor=1.0+min(.14,combat_hours/3000.0+engagements*.003)
            rows.append({"cohort_id":cid,"count":count,"role":role,"melee_score":melee,"ranged_score":ranged,"experience_factor":experience_factor,"melee_reach_m":_num(melee_weapon.get("reach_m",.75),.75),"melee_minimum_range_m":_num(melee_weapon.get("minimum_range_m",.10),.10),"melee_handling":_num(melee_weapon.get("handling",.8),.8),"melee_force":max(_num(melee_weapon.get("base_force_cut")),_num(melee_weapon.get("base_force_thrust")),_num(melee_weapon.get("base_force_blunt")),.35),"ranged_effective_range_m":_num(ranged_weapon.get("effective_range_m",0)),"ranged_max_direct_range_m":_num(ranged_weapon.get("maximum_direct_range_m",0)),"ranged_cycle_seconds":_num(ranged_weapon.get("base_shot_cycle_seconds",ranged_weapon.get("base_reload_cycle_seconds",0))),"ranged_power_index":_num(ranged_weapon.get("draw_power_index",ranged_weapon.get("launch_power_index",0))),"ammunition_item":ammo_item,"ammunition_resource":ammo_resource,"carried_ammunition":max(0,int(loadout.get("carried_ammunition",0) or 0)),"protection_index":self._combat_protection_index(loadout),"mount_index":self._combat_mount_index(loadout),"frontage_spacing_m":max(.45,_num(profile.get("frontage_spacing_m",.9),.9)),"depth_support_factor":_clampf(_num(profile.get("depth_support_factor",.3),.3),0,.7),"skills":deepcopy(dict(skills)),"attributes":deepcopy(dict(attrs))})
        return rows
    ...
    def _combat_named_participants(self,formation:Mapping[str,Any],force:Mapping[str,Any]|None=None)->list[dict[str,Any]]:
        ...
        for ref,role in sorted(roles.items()):
            ...
            attrs,skills=_stats(person);health=_health_factor(person);command=_weighted(skills,{"Formation Command":.28,"Tactics":.22,"Leadership":.18,"Strategy":.14,"Mass Combat":.18})*health;direct_attr=_weighted(attrs,{"Strength":.12,"Agility":.14,"Endurance":.10,"Toughness":.08,"Coordination":.20,"Awareness":.18,"Composure":.18});loadout=self._combat_person_loadout(person);melee_candidates:list[tuple[float,Mapping[str,Any],str]]=[]
            for key in ("primary_melee_weapon","sidearm"):
                weapon=self._combat_weapon(loadout.get(key))
                if not weapon:continue
                skill_name=self._combat_weapon_skill_name(weapon);skill=_num(skills.get(skill_name,0));defense=_num(skills.get("Defense",0));force_index=max(_num(weapon.get("base_force_cut")),_num(weapon.get("base_force_thrust")),_num(weapon.get("base_force_blunt")),.25);handling=_num(weapon.get("handling",.8),.8);mechanics=_clampf(.90+.06*handling+.06*min(2.0,force_index),.82,1.16);score=(.68*(.82*skill+.18*defense)+.32*direct_attr)*mechanics*health;melee_candidates.append((score,weapon,skill_name))
            if melee_candidates:melee_direct,melee_weapon,melee_skill_name=max(melee_candidates,key=lambda x:x[0])
            else:
                fallback_skill_name=max(("Spear","Sword","Glaive","Axe","Mace","Staff","Dagger","Defense"),key=lambda k:_num(skills.get(k,0)));melee_direct=(.68*_num(skills.get(fallback_skill_name,0))+.32*direct_attr)*health;melee_weapon={};melee_skill_name=fallback_skill_name
            ranged_weapon=self._combat_weapon(loadout.get("ranged_weapon"));ranged_skill_name=self._combat_weapon_skill_name(ranged_weapon) if ranged_weapon else "";ranged_skill=_num(skills.get(ranged_skill_name,0)) if ranged_skill_name else 0.0;ranged_direct=(.68*(.88*ranged_skill+.12*_num(skills.get("Defense",0)))+.32*direct_attr)*health if ranged_skill>0 else 0.0;ammo_item=str(loadout.get("ammunition_item",""));ammo_resource=self.AMMO_RESOURCE_BY_ITEM.get(ammo_item);role_scale={"staff":.15,"commander":.35,"deputy":.45,"specialist":.65,"notable":1.0,"embedded":1.0}.get(role,.75)
            def equivalent(score:float)->float:
                if score<=0:return 0.0
                return _clampf(1.0+max(0.0,(score-70.0)/32.0)**1.28,0.0,16.0)*role_scale
            assignment=assignments.get(ref) if isinstance(assignments,Mapping) else None;included=bool(isinstance(assignment,Mapping) and str(assignment.get("formation_ref",""))==fref);exposure={"commander":.55,"deputy":.70,"staff":.35,"specialist":.80,"notable":.90,"embedded":1.0}.get(role,.75)
            details.append({"person_ref":ref,"representation":str(person.get("schema")),"role":role,"command_score":command,"health_factor":health,"command_available":health>.25,"direct_combat_score":max(melee_direct,ranged_direct),"melee_direct_score":melee_direct,"ranged_direct_score":ranged_direct,"equivalent_frontline_bodies":equivalent(melee_direct),"ranged_equivalent_bodies":equivalent(ranged_direct),"included_in_personnel":included,"exposure_factor":exposure,"loadout_id":str(loadout.get("id","")),"melee_skill":melee_skill_name,"melee_weapon_id":str(melee_weapon.get("id","")),"melee_reach_m":_num(melee_weapon.get("reach_m",.75),.75),"melee_minimum_range_m":_num(melee_weapon.get("minimum_range_m",.10),.10),"melee_handling":_num(melee_weapon.get("handling",.8),.8),"ranged_skill":ranged_skill_name,"ranged_weapon_id":str(ranged_weapon.get("id","")) if ranged_weapon else "","ranged_effective_range_m":_num(ranged_weapon.get("effective_range_m",0)) if ranged_weapon else 0.0,"ranged_max_direct_range_m":_num(ranged_weapon.get("maximum_direct_range_m",0)) if ranged_weapon else 0.0,"ranged_cycle_seconds":_num(ranged_weapon.get("base_shot_cycle_seconds",ranged_weapon.get("base_reload_cycle_seconds",0))) if ranged_weapon else 0.0,"ranged_power_index":_num(ranged_weapon.get("draw_power_index",ranged_weapon.get("launch_power_index",0))) if ranged_weapon else 0.0,"ammunition_item":ammo_item,"ammunition_resource":ammo_resource,"carried_ammunition":max(0,int(loadout.get("carried_ammunition",0) or 0)),"protection_index":self._combat_protection_index(loadout),"mount_index":self._combat_mount_index(loadout)})
        return details
    ...
    def _combat_named_ammunition_rows(named:Sequence[Mapping[str,Any]])->list[dict[str,Any]]:
        rows=[]
        for person in named:
            if _num(person.get("ranged_effective_range_m"))<=0 or _num(person.get("ranged_direct_score"))<=0:continue
            rows.append({"count":1,"ranged_score":_num(person.get("ranged_direct_score")),"ranged_effective_range_m":_num(person.get("ranged_effective_range_m")),"ranged_max_direct_range_m":_num(person.get("ranged_max_direct_range_m")),"ranged_cycle_seconds":_num(person.get("ranged_cycle_seconds")),"ranged_power_index":_num(person.get("ranged_power_index")),"ammunition_item":person.get("ammunition_item"),"ammunition_resource":person.get("ammunition_resource"),"carried_ammunition":max(0,int(person.get("carried_ammunition",0)))})
        return rows
    ...
    def _combat_ranged_factor(self,rows:Sequence[Mapping[str,Any]],ammo_plan:Mapping[str,Any],opposing:Sequence[Mapping[str,Any]]|None=None)->float:
        ...
        opp_total=max(1,sum(max(0,int(r.get("count",0))) for r in (opposing or [])));opp_mounted=sum(max(0,int(r.get("count",0))) for r in (opposing or []) if _num(r.get("mount_index"))>0);closing_speed=_num(rules.get("mounted_closing_speed_mps",3.4),3.4) if opp_mounted/opp_total>=.40 else _num(rules.get("default_closing_speed_mps",1.6),1.6);volley_cap=max(0,int(rules.get("opening_volley_opportunity_cap",4)));volley_weight=_num(rules.get("opening_volley_bonus_per_opportunity",.035),.035);minimum_window=max(0.0,_num(rules.get("minimum_range_window_m",12.0),12.0));opening_bonus=0.0
        for row in ranged:
    ...
    def _combat_protection_factor(self,rows:Sequence[Mapping[str,Any]])->float:
        total=max(1,sum(int(r.get("count",0)) for r in rows));protection=sum(_num(r.get("protection_index"))*int(r.get("count",0)) for r in rows)/total if rows else 0;return _clampf(1.0+protection/800.0,1.0,1.18)
    ...
    def _combat_mount_factor(self,rows:Sequence[Mapping[str,Any]],formation:Mapping[str,Any])->float:
        ...
        actual=sum(max(0,int(v)) for v in formation.get("mounts",{}).values());complete=_clampf(actual/max(1,mounted_n),0,1);quality=sum(_num(r.get("mount_index"))*int(r.get("count",0)) for r in mounted)/mounted_n;return _clampf(.82+.18*complete+max(0,quality-70)/900.0,.75,1.16)

    def _combat_command_effects(self,formation:Mapping[str,Any],named:Sequence[Mapping[str,Any]])->dict[str,Any]:
        """Return bounded domain-specific command factors from real staffing.

        Internal hierarchy creates scale coverage, but quality is constrained by
        saved formation training/cohesion. Unit and support terms require named or
        actually allocated command/support bodies. A healthy deputy may act when
        the commander is unavailable; no replacement person is fabricated.
        """
        rules=self._combat_command_rules();cfg=rules.get("command_effect_scales",{}) if isinstance(rules,Mapping) else {};structure=formation.get("command_structure",{}) if isinstance(formation.get("command_structure"),Mapping) else {};hierarchy=structure.get("internal_hierarchy",[]) if isinstance(structure,Mapping) else []
        ...
        local_coverage=1.0 if any(100<=s<=200 for s in scales) else 0.0;maneuver_coverage=1.0 if any(500<=s<=1000 for s in scales) else 0.0;operational_coverage=1.0 if any(s>=2000 for s in scales) else 0.0
        local_cfg=cfg.get("local_100_200",{}) if isinstance(cfg,Mapping) else {};maneuver_cfg=cfg.get("maneuver_500_1000",{}) if isinstance(cfg,Mapping) else {};operational_cfg=cfg.get("operational_2000_plus",{}) if isinstance(cfg,Mapping) else {};unit_cfg=cfg.get("unit_command",{}) if isinstance(cfg,Mapping) else {};support_cfg=cfg.get("support",{}) if isinstance(cfg,Mapping) else {}
        ...
        if target<=0:unit_coverage=1.0 if acting_score>0 else 0.0
        ...
        support=structure.get("external_support",{}) if isinstance(structure,Mapping) else {};support_target=max(0,int(support.get("target_total",0))) if isinstance(support,Mapping) else 0;support_actual=max(0,int(support.get("allocated_total",0))) if isinstance(support,Mapping) else 0;support_ratio=1.0 if support_target<=0 else _clampf(support_actual/max(1,support_target),0,1)
        bonuses={"local":max(0.0,_num(local_cfg.get("maximum_quality_delta",.05)))*local_coverage*organizational_quality,"maneuver":max(0.0,_num(maneuver_cfg.get("maximum_quality_delta",.08)))*maneuver_coverage*organizational_quality,"operational":max(0.0,_num(operational_cfg.get("maximum_quality_delta",.07)))*operational_coverage*organizational_quality,"unit":max(0.0,_num(unit_cfg.get("maximum_quality_delta",.12)))*unit_coverage*unit_quality*(1.0+.18*deputy_throughput),"support":max(0.0,_num(support_cfg.get("maximum_quality_delta",.08)))*support_ratio}
        ...
        if product(1.0)>cap:
            ...
            scale=lo
        factors={key:1.0+bonus*scale for key,bonus in bonuses.items()};factors["combined_factor"]=product(scale);factors["continuity_mode"]=continuity;factors["acting_command_score"]=round(acting_score,3);factors["support_staffing_ratio"]=round(support_ratio,6);factors["unit_staffing_ratio"]=round(unit_coverage,6);return factors
    ...
    def _formation_combat_snapshot(self,formation:Mapping[str,Any],force:Mapping[str,Any],*,terrain_kind:str,ammo_plan:Mapping[str,Any]|None=None,battle_hours:float=3.0,opposing_rows:Sequence[Mapping[str,Any]]|None=None)->dict[str,Any]:
        rows=self._combat_cohort_snapshot(formation,force);n=max(1,int(formation.get("personnel",0)));cohort_n=max(1,sum(int(r.get("count",0)) for r in rows));melee_mean=sum(_num(r.get("melee_score"))*int(r.get("count",0)) for r in rows)/cohort_n if rows else 55.0;experience=sum(_num(r.get("experience_factor",1))*int(r.get("count",0)) for r in rows)/cohort_n if rows else 1.0;capability=_clampf(.35+melee_mean/100.0,.35,2.35)*experience;cohesion=_num(formation.get("cohesion",50),50);reach=self._combat_reach_factor(rows,opposing_rows or [],cohesion,terrain_kind);named=self._combat_named_participants(formation,force);ammo_rows=list(rows)+self._combat_named_ammunition_rows(named);plan=dict(ammo_plan or self._combat_ammunition_plan(ammo_rows,formation.get("logistics",{}),battle_hours));ranged=self._combat_ranged_factor(rows,plan,opposing_rows or []);frontage=self._combat_frontage_equivalent(rows,n,terrain_kind);named_equiv=0.0;suff=plan.get("sufficiency_by_resource",{}) if isinstance(plan,Mapping) else {};rules=self._combat_interaction_rules()
        for person in named:
            melee_eq=max(0.0,_num(person.get("equivalent_frontline_bodies")))
            if melee_eq>0:
                melee_eq*=self._combat_reach_factor([{"count":1,"melee_reach_m":person.get("melee_reach_m",.75),"melee_minimum_range_m":person.get("melee_minimum_range_m",.10)}],opposing_rows or [],cohesion,terrain_kind);protection=_clampf(1.0+_num(person.get("protection_index"))/1000.0,1.0,1.12);mount=_clampf(1.0+max(0.0,_num(person.get("mount_index"))-70.0)/1100.0,1.0,1.10);melee_eq*=protection*mount
            ranged_eq=max(0.0,_num(person.get("ranged_equivalent_bodies")));resource=str(person.get("ammunition_resource") or "");ammo=_clampf(_num(suff.get(resource,0) if isinstance(suff,Mapping) else 0),0,1) if resource else 0.0
            if ranged_eq>0 and ammo>0:
                range_factor=_clampf(_num(person.get("ranged_effective_range_m"))/_num(rules.get("ranged_effective_range_reference_m",120),120),.40,1.65);cadence=_clampf(_num(rules.get("ranged_cycle_reference_seconds",6),6)/max(1,_num(person.get("ranged_cycle_seconds",8),8)),.35,1.7);power=_clampf(_num(person.get("ranged_power_index",70),70)/70.0,.45,1.75);ranged_eq*=_clampf(.68+.12*range_factor+.10*cadence+.10*power,.65,1.35)*ammo
            else:ranged_eq=0.0
            effective=max(melee_eq,ranged_eq);person["effective_equivalent_bodies"]=effective;named_equiv+=effective
        command=self._combat_command_effects(formation,named)
        return {"rows":rows,"cohort_personnel":cohort_n,"melee_capability_mean":melee_mean,"capability_factor":capability,"melee_weapon_factor":self._combat_melee_weapon_factor(rows),"reach_factor":reach,"ranged_factor":ranged,"frontage_equivalent":frontage,"named_participants":named,"named_equivalent":named_equiv,"command_effects":command,"command_factor":command["combined_factor"],"protection_factor":self._combat_protection_factor(rows),"mount_factor":self._combat_mount_factor(rows,formation),"ammo_plan":plan}

...
class CombatCapabilityMixin:
    ...
    def _combat_prepare_formation(self,ref:str)->tuple[str,dict[str,Any],dict[str,Any]]:
        if hasattr(self,"_ct_formation"):
            ...
            formation=self.read(path);ensure_formation_composition(force,formation);self.put(path,formation);return path,formation,force
    ...
    def _combat_named_participants(self,formation:Mapping[str,Any],force:Mapping[str,Any]|None=None)->list[dict[str,Any]]:
        ...
        for ref,role in sorted(roles.items()):
            ...
            assignment=assignments.get(ref) if isinstance(assignments,Mapping) else None; included=bool(isinstance(assignment,Mapping) and str(assignment.get("formation_ref",""))==fref); exposure={"commander":.55,"deputy":.70,"staff":.35,"specialist":.80,"notable":.90,"embedded":1.0,"higher_commander":0.05,"higher_deputy":0.08}.get(role,.75)

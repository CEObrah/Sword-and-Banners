...
from sword_runtime.military_loadouts import institutional_role_loadout_id, explicit_personal_loadout_id, officer_loadout_id
from sword_runtime.cohort_personnel import ensure_cohort_ledger, ensure_formation_composition
from sword_runtime.chariot_platforms import chariot_rules, operational_chariot_capacity
from sword_runtime.fatigue import RULES_PATH as FATIGUE_RULES_PATH, person_fatigue_factors, settle_formation_idle_fatigue
from sword_runtime.military_supply import military_supply_sufficiency
from sword_runtime.terrain import terrain_context_for_location, terrain_effects_for_tags, terrain_has, terrain_tokens
...
def _health_factor(person: Mapping[str, Any]) -> float:
    ...
    if health in {"critical", "incapacitated"}: health_factor = 0.25
    elif health in {"injured", "wounded"}: health_factor = 0.68
    ...
    attrs,_skills=_stats(person)
    fatigue_factor=person_fatigue_factors(fatigue=_num(fatigue),endurance=_num(attrs.get("Endurance",0))).get("control_factor",1.0)
    return _clampf(health_factor*_num(fatigue_factor,1.0), 0.08, 1.0)


class CombatCapabilityMixin:
    ROLE_PATH = "game/data/mil/combat-role-profiles.json"
    AMMO_RESOURCE_BY_ITEM = {"ammo_arrow":"war_arrows","ammo_arrow":"war_arrows","ammo_bolt_war":"war_bolts"}
    ...
    def _combat_loadout(self, loadout_id: str) -> Mapping[str, Any]:
        ...
        value=load_loadout(self.read, loadout_id)
    ...
    def _combat_prepare_formation(self,ref:str)->tuple[str,dict[str,Any],dict[str,Any]]:
        ...
        if hasattr(self,"_ct_formation"):
            ...
            if str(formation.get("status", "")) == "contract_complete_withdrawing":
                raise ValueError("battle rejected: mercenary contract has ended and formation is withdrawing")
    ...
    def _combat_mount_index(self,loadout:Mapping[str,Any])->float:
        ...
        vals=[_num(mount.get(k,mount.get(k.lower(),0))) for k in ("Strength","Agility","Speed","Endurance","Composure")];training=_num(mount.get("training_score",mount.get("training",0)))
    ...
    def _combat_cohort_snapshot(self,formation:Mapping[str,Any],force:Mapping[str,Any])->list[dict[str,Any]]:
        ...
        mounted_required_by_id:dict[str,int]={}
        for item in formation.get("cohort_composition",[]):
            ...
            role=self._combat_cohort_role(formation,cohort); profile=self._combat_role_profile(role); loadout=self._combat_loadout(institutional_role_loadout_id(self.read, formation, role, profile=profile))
            mount_id=str(loadout.get("mount","") or "")
            if mount_id and self._combat_weapon(mount_id): mounted_required_by_id[mount_id]=mounted_required_by_id.get(mount_id,0)+count
        formation_mounts=formation.get("mounts",{}) if isinstance(formation.get("mounts"),Mapping) else {}
        ...
        for item in formation.get("cohort_composition",[]):
            ...
            loadout_id=institutional_role_loadout_id(self.read, formation, role, profile=profile)
            loadout=self._combat_loadout(loadout_id)
            ...
            mount_id=str(loadout.get("mount","") or "")
            if mount:
                mount_profile=mount_effective_speed_mps(mount,barding=barding,rider_mass_kg=75.0,rider_equipment_kg=burden["total_load_kg"],tack_mass_kg=_num(tack.get("mass_kg",0)),terrain_factor=1.0)
                mount_required=max(0,int(mounted_required_by_id.get(mount_id,0)))
                physical_for_mount=max(0,int(formation_mounts.get(mount_id,0) or 0))
                mount_fill=_clampf(physical_for_mount/max(1,mount_required),0.0,1.0) if mount_required>0 else 0.0
                mounted_units=float(count)*mount_fill
                mount_availability=_clampf(mounted_units/max(1.0,float(count)),0.0,1.0)
                mount_index=self._combat_mount_index(loadout)
                mount_required_units=count
                mount_speed_mps=_num(mount_profile.get("effective_speed_mps",0))
                mount_total_mass_kg=_num(mount_profile.get("total_mass_kg",0))
                charge_legal=bool(mount_profile.get("charge_legal")) and mounted_units>0
                chariot_state={"operational_platforms":0,"physical_platforms":0,"chariot_horses":0}
            elif role=="chariot":
                c_rules=chariot_rules(self.read); chariot_state=operational_chariot_capacity(formation,c_rules)
                role_operational=float(chariot_state.get("operational_crew",0))
                mounted_units=min(float(count),role_operational*float(count)/max(1.0,float(role_personnel)))
                mount_availability=_clampf(mounted_units/max(1.0,float(count)),0.0,1.0)
                team_horse=self._combat_weapon("horse")
                team_profile=mount_effective_speed_mps(team_horse,rider_mass_kg=0.0,rider_equipment_kg=0.0,tack_mass_kg=0.0,terrain_factor=1.0) if team_horse else {"mounted":False}
                speed_factor=_clampf(_num(c_rules.get("movement_speed_factor_vs_team_horse",.82),.82),.1,1.0)
                mount_speed_mps=_num(team_profile.get("effective_speed_mps",0))*speed_factor
                crew_per=max(1,int(chariot_state.get("crew_per_platform",3) or 3)); horses_per=max(1,int(chariot_state.get("horses_per_platform",3) or 3))
                platform_mass=max(0.0,_num(c_rules.get("platform_mass_kg",420),420)); horse_mass=max(0.0,_num(team_horse.get("mass_kg",0)))
                mount_total_mass_kg=(horse_mass*horses_per+platform_mass)/crew_per
                mount_index=self._combat_mount_index({"mount":"horse"})
                mount_required_units=int(math.ceil(float(count)*horses_per/crew_per))
                charge_legal=mounted_units>0 and mount_speed_mps>0
                mount_profile={"mounted":mounted_units>0}
            else:
                mount_profile={"mounted":False}; mounted_units=0.0; mount_availability=0.0; mount_index=0.0; mount_required_units=0; mount_speed_mps=0.0; mount_total_mass_kg=0.0; charge_legal=False; chariot_state={"operational_platforms":0,"physical_platforms":0,"chariot_horses":0}
            force_choices={
            ...
            rows.append({
                "cohort_id":cid,"count":count,"role":role,"loadout_id":loadout_id,
                ...
                "mount_index":mount_index,"mounted":bool(mount_profile.get("mounted")) and mounted_units>0,"mounted_units":round(mounted_units,6),"mount_required_units":mount_required_units,"mount_availability":round(mount_availability,6),"mount_speed_mps":mount_speed_mps,"mount_total_mass_kg":mount_total_mass_kg,"charge_legal":charge_legal,
                "chariot_platforms":int(chariot_state.get("physical_platforms",0) or 0),"operational_chariot_platforms":int(chariot_state.get("operational_platforms",0) or 0),"chariot_horses":int(chariot_state.get("chariot_horses",0) or 0),
    ...
    def _combat_weapon_skill_name(weapon:Mapping[str,Any])->str:
        family=str(weapon.get("family",weapon.get("combat_profile",""))).lower();aliases={"spear":"Polearms","lance":"Polearms","sword":"Sword","one_handed_sword":"Sword","two_handed_sword":"Sword","glaive":"Polearms","axe":"Heavy Weapons","mace":"Heavy Weapons","mace_hammer":"Heavy Weapons","staff":"Polearms","dagger":"Sword","bow":"Bow","crossbow":"Crossbow"};return aliases.get(family,family.title() if family else "")

    def _combat_person_loadout(self,person:Mapping[str,Any])->Mapping[str,Any]:
        # Personal equipment is independent of the troop mix being commanded.
        # Explicit personal issue wins. If no explicit issue exists, derive an
        # officer profile from the person's exact current formation. Composite
        # formations deliberately use an institutional command profile rather
        # than borrowing one arbitrary child arm's kit.
        ...
        if not loadout_id:
            ...
            formation_ref=assignment.get("formation_ref") or person.get("current_formation_id")
            if isinstance(formation_ref,str) and formation_ref:
                _,formation0=self.owner(formation_ref)
                if not isinstance(formation0,Mapping):
                    raise ValueError(f"combat loadout formation unresolved: {formation_ref}")
                formation=formation0
            loadout_id=officer_loadout_id(self.read,person,formation)
    ...
    def _combat_command_admission(self, formation: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(commander_ref,str) and commander_ref:
            if not isinstance(self._combat_person(commander_ref), Mapping):
                raise ValueError(f"battle rejected: exact commander unresolved: {commander_ref}")
            return {"mode":"exact_commander","commander_ref":commander_ref,"aggregate_unit_post":False,"higher_command":False}
        allocated=formation.get("attached_unit_command_by_role",{}) if isinstance(formation.get("attached_unit_command_by_role"),Mapping) else {}
        aggregate_post=sum(max(0,int(v or 0)) for v in allocated.values())>0
    ...
    def _combat_higher_command_participants(self, formation: Mapping[str, Any]) -> list[dict[str, Any]]:
        ...
        Command groups are organizational authority only. Their commander can
        improve operational coordination, but is never inserted into a subordinate
        formation's fighting strength or exposed as an extra combat body. Staff
        effects are handled by the command/staff cognition layer rather than by a
        generic second-commander combat slot.
        """
        ...
        while isinstance(current, str) and current.startswith("cmdgrp.") and depth < 8:
            if current in seen_groups:
                raise ValueError(f"battle rejected: command hierarchy cycle at {current}")
            seen_groups.add(current)
            group = self.read_optional(f"state/cmd/command-groups/{current}.json")
            if not isinstance(group, Mapping):
                raise ValueError(f"battle rejected: higher command unresolved: {current}")
            depth += 1
            ref = group.get("commander_ref")
            if isinstance(ref, str) and ref and ref not in seen_people:
                seen_people.add(ref)
                rows.append({"person_ref": ref, "role": "higher_commander", "command_group_ref": current, "command_depth": depth})
            parent = group.get("parent_command_group_ref")
            current = parent if isinstance(parent, str) else None
        if isinstance(current, str) and current.startswith("cmdgrp."):
            raise ValueError("battle rejected: command hierarchy exceeds bounded depth")
        return rows
    ...
    def _combat_named_participants(self,formation:Mapping[str,Any],force:Mapping[str,Any]|None=None)->list[dict[str,Any]]:
        ...
        ref=formation.get("commander_ref")
        if isinstance(ref,str) and ref:roles[ref]="commander"
        ...
        if isinstance(assignments,Mapping):
            for ref,assignment in assignments.items():
                ...
                assigned_role=str(assignment.get("combat_role") or assignment.get("role") or "embedded"); roles.setdefault(ref,assigned_role if assigned_role in {"commander","staff","specialist","notable","embedded"} else "embedded")
        details:list[dict[str,Any]]=[]
        for ref,role in sorted(roles.items()):
            person=self._combat_person(ref)
            if not isinstance(person,Mapping):
                raise ValueError(f"battle rejected: named combat participant unresolved: {ref}")
            ...
            if included and role not in {"commander","higher_commander"} and scale in {100,500,1000}:
            ...
            command=_weighted(skills,{"Formation Command":.28,"Tactics":.22,"Leadership":.18,"Strategy":.14,"Formation Fighting":.18})*health
            ...
            if hasattr(self,"_personal_equipment_profile"):
                exact=self._personal_equipment_profile(ref,person)
                if not isinstance(exact, Mapping):
                    raise ValueError(f"battle rejected: personal equipment profile unresolved: {ref}")
                loadout=deepcopy(dict(exact.get("loadout",{})))
                condition_by_item=exact.get("condition_by_item",{}) if isinstance(exact.get("condition_by_item"),Mapping) else {}
            ...
            for key in ("primary_melee_weapon","sidearm"):
                ...
                skill_name=self._combat_weapon_skill_name(weapon); skill=_num(skills.get(skill_name,0)); force_index=max(_num(weapon.get("base_force_cut")),_num(weapon.get("base_force_thrust")),_num(weapon.get("base_force_blunt")),.25); handling=_num(weapon.get("handling",.8),.8); cond=condition_factor(_num(condition_by_item.get(str(weapon_id),100),100)); mechanics=max(.55,.90+.06*handling+.06*math.sqrt(max(.01,force_index)))*cond; score=(.68*skill+.32*direct_attr)*mechanics*health; melee_candidates.append((score,weapon,skill_name,str(weapon_id)))
            if melee_candidates: melee_direct,melee_weapon,melee_skill_name,melee_weapon_id=max(melee_candidates,key=lambda x:x[0])
            else:
                fallback_skill_name=max(("Polearms","Sword","Heavy Weapons","Unarmed","Grappling"),key=lambda k:_num(skills.get(k,0))); melee_direct=(.68*_num(skills.get(fallback_skill_name,0))+.32*direct_attr)*health; melee_weapon={}; melee_skill_name=fallback_skill_name; melee_weapon_id=""
            ranged_weapon=self._combat_weapon(loadout.get("ranged_weapon")); ranged_skill_name=self._combat_weapon_skill_name(ranged_weapon) if ranged_weapon else ""; ranged_skill=_num(skills.get(ranged_skill_name,0)) if ranged_skill_name else 0.0; ranged_direct=(.68*ranged_skill+.32*direct_attr)*health if ranged_skill>0 else 0.0
            ranged_envelope=projectile_operating_envelope(ranged_weapon,weapon_skill=ranged_skill,strength=_num(attrs.get("Strength",0)),coordination=_num(attrs.get("Coordination",0)),awareness=_num(attrs.get("Awareness",0))) if ranged_weapon else {}
            ammo_item=str(loadout.get("ammunition_item","")); ammo_resource=self.AMMO_RESOURCE_BY_ITEM.get(ammo_item)
            ...
            melee_skill_value=_num(skills.get(melee_skill_name,0))
            weapon_defense=.45*melee_skill_value+.20*_num(skills.get("Athletics",0))+.15*_num(attrs.get("Agility",0))+.10*_num(attrs.get("Awareness",0))+.10*_num(attrs.get("Coordination",0))
            exposure={"commander":.55,"staff":.35,"specialist":.80,"notable":.90,"embedded":1.0,"internal_100_commander":.82,"internal_500_commander":.72,"internal_1000_commander":.62,"higher_commander":0.05}.get(role,.75)
            fatigue_value=person.get("fatigue")
            if fatigue_value is None and isinstance(person.get("health"),Mapping): fatigue_value=person.get("health",{}).get("fatigue",0)
            fatigue_perf=person_fatigue_factors(fatigue=_num(fatigue_value),endurance=_num(attrs.get("Endurance",0)))
            tempo=.25*_num(attrs.get("Agility"))+.20*_num(attrs.get("Coordination"))+.20*_num(skills.get(melee_skill_name,0))+.15*_num(attrs.get("Awareness"))+.10*_num(skills.get("Formation Fighting",0))+.10*_num(attrs.get("Endurance")); action_interval=max(.40,360.0/(max(0.0,tempo)+60.0))/max(.32,_num(fatigue_perf.get("tempo_factor",1.0),1.0))
            ...
            shield_defense=(.50*_num(skills.get("Shield",0))+.20*_num(attrs.get("Coordination",0))+.15*_num(attrs.get("Awareness",0))+.15*_num(attrs.get("Strength",0)))*condition_factor(shield_condition) if shield_id else 0.0
            defense_control=max(0.0,weapon_defense,shield_defense)
            ...
            row={"person_ref":ref,"representation":str(person.get("schema")),"role":role,"command_score":command,"health_factor":health,"command_available":command_available,"co_located":co_located,"person_location_ref":person_location or None,"formation_location_ref":formation_location or None,"direct_combat_score":max(melee_direct,ranged_direct),"melee_direct_score":melee_direct,"ranged_direct_score":ranged_direct,
                 ...
                 "ranged_skill":ranged_skill_name,"ranged_skill_value":ranged_skill,"ranged_weapon_id":str(ranged_weapon.get("id","")) if ranged_weapon else "","ranged_effective_range_m":_num(ranged_envelope.get("effective_range_m",0)),"ranged_max_direct_range_m":_num(ranged_envelope.get("maximum_direct_range_m",0)),"ranged_cycle_seconds":_num(ranged_envelope.get("cycle_seconds",0)),"ranged_power_index":_num(ranged_envelope.get("launch_power_index",0)),"ammunition_item":ammo_item,"ammunition_resource":ammo_resource,"carried_ammunition":carried_ammunition,"default_carried_ammunition":default_carried_ammunition,
                 "protection_index":self._combat_protection_index(loadout),"shield_id":shield_id,"body_armor_id":body_armor_id,"helmet_id":helmet_id,"shield_condition_pct":shield_condition,"armor_condition_pct":armor_condition,"defense_control_value":defense_control,"agility":_num(attrs.get("Agility",0)),"toughness":_num(attrs.get("Toughness",0)),"mount_protection_index":self._combat_mount_protection_index(loadout),"mount_index":self._combat_mount_index(loadout),"mounted":bool(mount_profile.get("mounted")),"mount_speed_mps":_num(mount_profile.get("effective_speed_mps",0)),"mount_total_mass_kg":_num(mount_profile.get("total_mass_kg",0)),"charge_legal":bool(mount_profile.get("charge_legal")),"riding":_num(skills.get("Riding",0)),"coordination":_num(attrs.get("Coordination",0)),"awareness":_num(attrs.get("Awareness",0)),"composure":_num(attrs.get("Composure",0)),"horse_training":_num(mount.get("training_score",0)),"strength":_num(attrs.get("Strength",0)),"endurance":_num(attrs.get("Endurance",0)),
                 "fatigue":max(0,int(_num(fatigue_value))),"fatigue_control_factor":round(_num(fatigue_perf.get("control_factor",1.0),1.0),6),"fatigue_tempo_factor":round(_num(fatigue_perf.get("tempo_factor",1.0),1.0),6),"fatigue_movement_factor":round(_num(fatigue_perf.get("movement_factor",1.0),1.0),6),"fatigue_exertion_capacity_factor":round(_num(fatigue_perf.get("exertion_capacity_factor",1.0),1.0),6),"fatigue_burden_multiplier":round(_num(burden.get("fatigue_multiplier",1.0),1.0),6),
                 "combat_doctrine_ref":str(person.get("combat_doctrine_ref","") or "")}
    ...
    def _combat_frontage_equivalent(self,rows:Sequence[Mapping[str,Any]],personnel:int,terrain_kind:str)->float:
        rules=self._combat_interaction_rules();terrain_effect=terrain_effects_for_tags(self, tuple(sorted(terrain_tokens(terrain_kind))));terrain=_num(terrain_effect.get("frontage_milli",750),750)/1000.0;ref_width=_num(rules.get("frontage_reference_m_per_1000_open",500),500);available_width=ref_width*terrain*math.sqrt(max(.05,personnel/1000.0))
    ...
    def _combat_reach_factor(self,own:Sequence[Mapping[str,Any]],opposing:Sequence[Mapping[str,Any]],cohesion:float,terrain_kind:str)->float:
        ...
        own_n=max(1,sum(int(r.get("count",0)) for r in own));opp_n=max(1,sum(int(r.get("count",0)) for r in opposing));own_reach=sum(_num(r.get("melee_reach_m",.8))*int(r.get("count",0)) for r in own)/own_n;opp_reach=sum(_num(r.get("melee_reach_m",.8))*int(r.get("count",0)) for r in opposing)/opp_n;own_min=sum(_num(r.get("melee_minimum_range_m",.1))*int(r.get("count",0)) for r in own)/own_n;opp_min=sum(_num(r.get("melee_minimum_range_m",.1))*int(r.get("count",0)) for r in opposing)/opp_n;rules=self._combat_interaction_rules();per_m=_num(rules.get("ordered_reach_advantage_per_meter",.16),.16);cap=_num(rules.get("ordered_reach_advantage_cap",.22),.22);retention=_num(rules.get("disorder_reach_retention",.35),.35);tight=terrain_has(terrain_kind,"pass","fortified","urban","woodland","mountain","marsh","wetland");order=_clampf(cohesion/80.0,0,1)*(.70 if tight else 1.0);reach_bonus=_clampf((own_reach-opp_reach)*per_m,-cap,cap)*(retention+(1-retention)*order);compression=_clampf((60-cohesion)/60.0+(.25 if tight else 0),0,1);min_cap=_num(rules.get("compressed_minimum_range_penalty_cap",.22),.22);min_penalty=min(min_cap,max(0,own_min-.25)*.22*compression);close_cap=_num(rules.get("compressed_short_weapon_bonus_cap",.14),.14);close_per_m=_num(rules.get("compressed_short_weapon_bonus_per_meter",.18),.18);close_bonus=min(close_cap,max(0.0,opp_min-own_min)*close_per_m*compression);return _clampf(1+reach_bonus-min_penalty+close_bonus,.72,1.28)
    ...
    def _combat_mount_factor(self,rows:Sequence[Mapping[str,Any]],formation:Mapping[str,Any])->float:
        ...
        subsistence=formation.get("subsistence",{}) if isinstance(formation.get("subsistence"),Mapping) else {}
        condition=_clampf(_num(subsistence.get("mount_condition_milli",1000),1000)/1000.0,.30,1.0)
        condition_factor=.72+.28*condition
        return _clampf((.82+.18*complete+max(0,quality-70)/900.0)*condition_factor,.58,1.16)
    ...
    def _combat_formation_method_profile(self,rows:Sequence[Mapping[str,Any]],formation:Mapping[str,Any],opposing_rows:Sequence[Mapping[str,Any]],terrain_kind:str)->dict[str,Any]:
        ...
        constrained=terrain_has(terrain_kind,"pass","fortified","urban","woodland","mountain","marsh","wetland")
    ...
    def _combat_hero_interventions(self,named:Sequence[Mapping[str,Any]],own_rows:Sequence[Mapping[str,Any]],opposing_rows:Sequence[Mapping[str,Any]],*,battle_hours:float,terrain_kind:str)->dict[str,Any]:
        ...
        def target_plan(person:Mapping[str,Any])->dict[str,str]:
            doctrine=load_personal_combat_doctrine(self.read, person)
            targeting=doctrine.get("targeting",{}) if isinstance(doctrine.get("targeting"),Mapping) else {}
            priorities=targeting.get("lethal_priority",[]) if isinstance(targeting,Mapping) else []
            if isinstance(priorities,list):
                for raw in priorities:
                    if isinstance(raw,Mapping) and raw.get("zone") and raw.get("structure"):
                        return {"zone":str(raw.get("zone")),"structure":str(raw.get("structure")),"purpose":str(raw.get("purpose","battlefield_incapacitation")),"basis":"registered_combat_doctrine"}
        ...
        def incoming_exposure_profile(person:Mapping[str,Any],*,active_seconds:float,local_front:int,intervention_mode:str)->dict[str,Any]:
            ...
            """
            hero_defense=(
                .46*max(0.0,_num(person.get("defense_control_value")))
            ...
            hero_target={
                ...
                "melee_score":hero_defense,"formation_fighting":max(0.0,_num(person.get("defense_control_value"))),
        ...
        for person in named:
            ...
            exertion_capacity=_clampf(_num(person.get("fatigue_exertion_capacity_factor",1.0),1.0),.20,1.0)
            active_seconds=min(120.0*exertion_capacity,available_seconds*exertion_capacity)
            ...
            has_personal_doctrine=bool(str(person.get("combat_doctrine_ref","") or ""))
            awareness_curve=1.0-math.exp(-max(0.0,_num(person.get("awareness",0)))/120.0)
            command_target_fraction=(.10+.18*awareness_curve) if has_personal_doctrine else (.035+.055*awareness_curve)
            ...
            command_attention=active_seconds if role=="commander" else 0.0
    ...
    def _combat_command_effects(self,formation:Mapping[str,Any],named:Sequence[Mapping[str,Any]])->dict[str,Any]:
        ...
        commander bodies. Routine staff/signal/logistics/medical work has
        ...
        readiness mechanics.
        """
        rules=self._combat_command_rules();cfg=rules.get("command_effect_scales",{}) if isinstance(rules,Mapping) else {}
        n=max(0,int(formation.get("personnel",0) or 0))
        ...
        hierarchy=hierarchy_rows(authorized_strength=authorized,current_personnel=n,formation_class=klass)
        commander=next((x for x in named if x.get("role")=="commander"),None);commander_ok=bool(commander and commander.get("command_available"));commander_score=_num(commander.get("command_score")) if commander_ok and commander else 0.0
        higher_commander=next((x for x in named if x.get("role")=="higher_commander" and x.get("command_available")),None);higher_commander_score=_num(higher_commander.get("command_score")) if higher_commander else 0.0
        if commander_score>0:acting_score=commander_score;continuity="commander"
        else:acting_score=0.0;continuity="aggregate_or_internal_only"
        ...
        # The Unit commander owns the Unit echelon itself. A 2,000+ Unit
        ...
        authorized_scale=max(0,int(formation.get("authorized_strength",formation.get("personnel",0)) or 0))
        ...
        allocated=formation.get("attached_unit_command_by_role",{}) if isinstance(formation.get("attached_unit_command_by_role"),Mapping) else {}
        aggregate_bodies=sum(max(0,int(v or 0)) for v in allocated.values())
        named_posts=int(bool(formation.get("commander_ref")))
        effective=min(target,named_posts+aggregate_bodies)
        aggregate_unit_post=aggregate_bodies>0
    ...
    def _autonomy_formation_power(self,ref:str,defender:bool=False,opposing_ref:str|None=None)->float:
        ...
        if n<=0:return 0.0
        logistics=formation.get("logistics",{});supply_state=military_supply_sufficiency(self,formation);terrain_kind=str(terrain_context_for_location(self,str(formation.get("location_ref")))["encoded"]);opposing=[]
        if opposing_ref:

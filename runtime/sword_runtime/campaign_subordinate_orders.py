...
from sword_runtime.operation_routing import append_operational_order, current_operational_order, exact_operation_record
...
def _formations_already_at_destination(planner: Any, formation_refs: list[str], destination_ref: str) -> bool:
    """Return true only when every assigned surviving formation is physically there now.

    Completed march orders are terminal history, but a still-unchanged staff
    assignment is not a reason to manufacture a new march to the place the
    formation already occupies.  Physical location is the authority for whether
    that movement work is satisfied.
    """
    if not formation_refs or not destination_ref:
        return False
    for formation_ref in formation_refs:
        ...
        if not isinstance(formation, Mapping):
            return False
        if str(formation.get("status", "")) in {"destroyed", "disbanded", "terminated"}:
            continue
...
def sync_campaign_subordinate_orders(planner: Any, *, at: str | None = None) -> list[str]:
    ...
    for assignment in assignments:
        ...
        for operation_ref in operation_refs:
            ...
            if _formations_already_at_destination(planner, applies, destination_ref):
                # The exact physical objective is already satisfied.  Preserve
                # the completed order as history and wait for a materially new
                # staff assignment instead of reissuing the same march forever.
                continue
            ...
            order = {
                ...
                "assignment_fingerprint": _digest(
                    "campaign_subordinate", cycle_ref, operation_ref, command_ref or "", destination_ref, ",".join(applies)
                ),

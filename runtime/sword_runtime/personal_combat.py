...
class PersonalCombatMixin:
    ...
    def _personal_equipment_profile(self, person_ref: str, person: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if melee_candidates:
            ...
            weapon = {"id": "unarmed", "family": "unarmed", "reach_m": 0.65, "minimum_range_m": 0.0, "handling": 1.0, "base_force_blunt": 0.35, "recovery_class": "quick"}
    ...
    def _personal_controls(self, person: Mapping[str, Any], eq: Mapping[str, Any], environment_effects: Mapping[str, Any]) -> dict[str, float]:
        ...
        defense = _num(skills.get("Defense"))
        ...
        impairment = fatigue * 0.35
        handling_adj = (handling - 0.8) * 25.0
        attack = 0.45 * weapon_skill + 0.20 * coord + 0.15 * agility + 0.10 * awareness + 0.10 * composure + handling_adj - impairment
        parry = 0.35 * weapon_skill + 0.25 * defense + 0.20 * coord + 0.10 * awareness + 0.10 * composure + handling_adj - impairment
        dodge = 0.35 * defense + 0.25 * agility + 0.15 * awareness + 0.10 * athletics + 0.10 * coord + 0.05 * composure - impairment
        block = 0.35 * shield + 0.25 * defense + 0.15 * coord + 0.10 * strength + 0.10 * awareness + 0.05 * composure - impairment
        attack *= max(0.45, articulation * recovery) * _num(anatomy.get("attack_factor"), 1.0) * _num(injury.get("attack_factor"), 1.0)
        parry *= max(0.45, articulation * recovery) * _num(anatomy.get("parry_factor"), 1.0) * _num(injury.get("parry_factor"), 1.0)
        dodge *= max(0.35, footing * _num(burden.get("movement_factor"), 1.0)) * _num(anatomy.get("movement_factor"), 1.0) * _num(injury.get("movement_factor"), 1.0)
        block *= max(0.45, articulation) * _num(anatomy.get("block_factor"), 1.0) * _num(injury.get("block_factor"), 1.0)
        return {
        ...
        *,
        ...
        if explicit:
            ...
            return {
                ...
                "structure": target.get("structure", zone) if zone != "mount" else "mount",
                "purpose": purpose,
                "selection_basis": "declared_intent",
            }

        behavior = actor.get("behavior", {}) if isinstance(actor, Mapping) and isinstance(actor.get("behavior"), Mapping) else {}
        doctrine = behavior.get("combat_targeting_doctrine", {}) if isinstance(behavior.get("combat_targeting_doctrine"), Mapping) else {}
        priorities = doctrine.get("lethal_priority" if lethal_intent else "disable_priority", []) if doctrine else []
        if isinstance(priorities, list) and priorities:
            ...
            for index, raw in enumerate(priorities):
                ...
                candidates.append((score, -index, {
                    ...
                    "selection_basis": "saved_combat_targeting_doctrine",
                }))
            if candidates:
    ...
    def _personal_defense_method(eq: Mapping[str, Any], controls: Mapping[str, float], distance: float) -> tuple[str, float]:
        ...
        reach = _num(weapon.get("reach_m"), 0.65)
    ...
    def _personal_apply_mount_wound(
        ...
        *,
        ...
        if str(mode) == "thrust":
            ...
            structures = ("foreleg", "hindleg", "chest", "neck", "eye")
        structure = structures[abs(int(seed)) % len(structures)]
        mobility_table = {
        ...
        state["health"] = "critical" if severity == "critical" else ("injured" if severity in {"moderate", "serious"} else state.get("health", "healthy"))
    ...
    def _personal_tempo(self, person: Mapping[str, Any], eq: Mapping[str, Any]) -> float:
        ...
        mass_combat = _num(skills.get("Mass Combat", skills.get("Mass_Combat", 0)))
        return max(
            0.0,
    ...
    def _personal_timing_profile(
        ...
        awareness = max(0.0, _num(attrs.get("Awareness")))

        # Root growth prevents absurd linear sprint speed while still allowing
        # exceptional >200 stats to keep producing a real advantage.
        ...
        movement_speed = max(foot_movement_speed, _num(mount_profile.get("effective_speed_mps"), 0.0) * 0.55) if mount_profile.get("mounted") else foot_movement_speed
        handling_factor = max(0.55, math.sqrt(handling))
        attack_startup = max(0.12, minimum_interval * 0.38 * recovery_class_factor / handling_factor)
        attack_recovery = max(0.16, minimum_interval * 0.62 * recovery_class_factor / load_recovery)
        defense_recovery = max(0.12, minimum_interval * 0.38 * recovery_class_factor / load_recovery)
        reaction = max(
            0.08,
            minimum_interval * 0.34 * (120.0 / (120.0 + awareness)),
        )
    ...
    def _personal_combat_slice(
        ...
        *,
        ...
        p_reach = max(0.45, _num(p_weapon.get("reach_m"), 0.65))
        o_reach = max(0.45, _num(o_weapon.get("reach_m"), 0.65))
        ...
        for ref in actors:
            ...
            if isinstance(row, Mapping):
                positions[ref] = {
                    "x_m": _num(row.get("x_m"), 0.0),
                    "y_m": _num(row.get("y_m"), 0.0),
                    "facing_deg": _num(row.get("facing_deg"), 0.0) % 360.0,
                }
        positions.setdefault(self.PLAYER_ACTOR, {"x_m": 0.0, "y_m": 0.0, "facing_deg": 0.0})
        ...
        start_positions = {
            ref: {"x_m": round(float(row["x_m"]), 6), "y_m": round(float(row["y_m"]), 6), "facing_deg": round(float(row["facing_deg"]), 6)}
        ...
        def transient_exertion_factor(ref: str) -> float:
            ...
            reserve = 10.0 + 0.90 * math.sqrt(endurance)
            spent = max(0.0, action_exertion[ref]) * burden_factor
            return _clamp(1.0 / (1.0 + spent / max(1.0, reserve)), 0.52, 1.0)
        ...
        def defensive_intent(text: str | None) -> bool:
            low = (text or "").lower()
            return any(token in low for token in ("parry", "block", "dodge", "evade", "guard", "intercept"))
        ...
        def obstacle_on_path(x0: float, y0: float, x1: float, y1: float, *, radius_m: float = 0.0) -> dict[str, Any] | None:
            ...
            # Skip the exact actor/target centers. Only the physical space between
            # them can block the contact path.
            for step in range(1,steps):
                t=step/steps
                obstacle=static_obstacle_at(x0+(x1-x0)*t,y0+(y1-y0)*t,radius_m=radius_m)
        ...
        def clamp_local_movement(ref: str, target_ref: str, x0: float, y0: float, x1: float, y1: float, at_s: float) -> tuple[float,float,dict[str,Any] | None]:
            ...
            for step in range(1,steps+1):
                ...
                if blocker is None:
                    for other in actors:
                        if other in {ref,target_ref} or not active(other):
                            continue
                        ox,oy=position_at(other,at_s); required=self_radius+actor_clearance_radius(other)+max(0.0,_num(local_obstacle_model.get("clearance_margin_m"),0.08))
        ...
        def choose_target(ref: str, at_s: float) -> str | None:
            ...
            if not alive:
                return None
            return min(alive, key=lambda other: (distance_between(ref, other, at_s), other))
        ...
        def schedule(ref: str) -> None:
            ...
            reach = max(0.45, _num(melee_weapon.get("reach_m"), _num(weapon.get("reach_m"), 0.65)))
            ...
            ranged_max = max(0.0, _num(ranged_weapon.get("maximum_direct_range_m"), _num(ranged_weapon.get("effective_range_m"), 0.0)))
            use_ranged = bool(ranged_weapon) and ammo_left > 0 and not move_only and (explicit_ranged or not melee_weapon or start_dist > reach + 0.75)
            ...
            elif start_dist > reach:
                desired = max(minimum + 0.18, min(reach * 0.88, 1.25))
                delta = -max(0.0, start_dist - desired)
                movement_label = "closes_distance"
                movement_action = abs(delta) > 1e-6
            elif start_dist < minimum:
                delta = min(max(0.0, minimum + 0.18 - start_dist), 0.75)
                movement_label = "recovers_weapon_space"
                movement_action = abs(delta) > 1e-6

            if movement_action:
                duration = max(0.08, abs(delta) / max(0.1, profile["movement_speed_mps"] * exertion_factor))
                resolve_at = start_at + duration
            ...
            contact_at = start_at + profile["attack_startup_seconds"] / exertion_factor
            recovery_complete = max(
                start_at + profile["minimum_action_interval_seconds"] / exertion_factor,
                contact_at + profile["attack_recovery_seconds"] / exertion_factor,
            )
            aim = self._personal_aim_plan(
                declared, lethal_intent=lethal_intent, seed=seed, sequence=action_serial,
                actor=people.get(ref), target_person=people.get(target_ref), target_eq=equipment.get(target_ref),
        ...
        while pending and boundary is None:
            ...
            a_reach = max(0.45, _num(actor_weapon.get("reach_m"), 0.65))
            a_min = max(0.0, _num(actor_weapon.get("minimum_range_m"), 0.0))
            d_reach = max(0.45, _num(target_weapon.get("reach_m"), 0.65))
            ...
            defense_method, defense_control = self._personal_defense_method(target_eq, target_ctrl, combat_distance)
            ...
            # Defenders also adapt. Repeating the same response forever is not a
            # free optimal policy when the attacker has just demonstrated how to
            # beat it. Explicit player guard wording still overrides this choice.
            recent_target = action_memory[target_ref][-4:]
            legal_defenses: list[tuple[str, float]] = [("dodge", _num(target_ctrl.get("dodge")))]
            if target_eq.get("loadout", {}).get("shield") and shield_geometry_reachable:
                legal_defenses.append(("block", _num(target_ctrl.get("block"))))
            if (not is_projectile) and parry_geometry_reachable and _num(target_weapon.get("minimum_range_m"), 0.0) <= combat_distance <= d_reach + 0.35:
                legal_defenses.append(("parry", _num(target_ctrl.get("parry"))))
            adjusted_defenses: list[tuple[str, float]] = []
            for method, base_control in legal_defenses:
            ...
            if defensive_plan:
                ...
                for token, method in (("parry", "parry"), ("block", "block"), ("dodge", "dodge"), ("evade", "dodge")):
                    if token in defensive_lower:
                        requested_method = method
                        break
                legal_methods = {"dodge"}
                if target_eq.get("loadout", {}).get("shield") and shield_geometry_reachable:
                    legal_methods.add("block")
                if (not is_projectile) and parry_geometry_reachable and _num(target_weapon.get("minimum_range_m"), 0.0) <= combat_distance <= d_reach + 0.35:
                    legal_methods.add("parry")
                if requested_method in legal_methods:
                    defense_method = requested_method
                    defense_control = _num(target_ctrl.get(requested_method), defense_control)
            ...
            if grapple_partner_ref:
                if str(grapple_partner_ref) == actor_ref:
                    ...
                    third_party_key={"parry":"third_party_parry_factor","block":"third_party_block_factor","dodge":"third_party_dodge_factor"}.get(defense_method,"third_party_dodge_factor")
            ...
            if since_last < 0.48:
                recovery_pressure = 1.0 - since_last / 0.48
                angular_pressure = conflict_angle / 180.0
                saturation_factor *= max(0.16, 1.0 - recovery_pressure * (0.30 + 0.58 * angular_pressure))
            ...
            defense_control *= timing_factor * saturation_factor * balance_factor * defender_exertion_factor
            defense_attempted = timing_factor > 0.10 or bool(defensive_plan)
            ...
            legal_range = (path_obstacle is None) and a_min <= combat_distance <= a_reach + 0.10
            attack_control = (actor_ctrl["attack"] + reach_advantage) * actor_exertion_factor + jitter(int(action.get("sequence", 0)) + len(trace), 0)
            if is_projectile:
                flight = action.get("projectile_flight", {}) if isinstance(action.get("projectile_flight"), Mapping) else {}
                attack_control = _num(flight.get("aim_control"), actor_ctrl["attack"]) * actor_exertion_factor + jitter(int(action.get("sequence", 0)) + len(trace), 0)
                legal_range = (path_obstacle is None) and combat_distance <= max(1.0, _num(actor_weapon.get("maximum_direct_range_m"), _num(actor_weapon.get("effective_range_m"), 0.0)))
                reach_advantage = 0.0
            if mounted_body_collision:
                ...
                ) * actor_exertion_factor + jitter(int(action.get("sequence", 0)) + len(trace), 0)
                reach_advantage = 0.0
                legal_range = (path_obstacle is None) and 0.25 <= combat_distance <= 1.65
            margin = attack_control - defense_control if legal_range else -40.0
            attack_event = {
            ...
            if defense_attempted:
                ...
                if timing_factor > 0.10:
                    action_exertion[target_ref] += {"dodge":0.72,"block":0.48,"parry":0.42}.get(defense_method,0.35) * (1.0 + (1.0 - saturation_factor) * 0.55)
                    ...
                    if defense_method == "parry":
                        ...
                        body_state[target_ref]["weapon_center_deg"] = incoming_bearing
                    elif defense_method == "block":
            ...
            if is_projectile:
                flight = action.get("projectile_flight", {}) if isinstance(action.get("projectile_flight"), Mapping) else {}
                impact = max(0.0, _num(flight.get("impact_index"), 0.0)) * max(0.0, grade_multiplier)
                penetration = max(0.0, _num(flight.get("penetration_index"), impact)) * max(0.0, grade_multiplier)
            ...
            hard_defensive_contact = defense_attempted and defense_method in {"parry", "block"}
            ...
            if defense_method == "parry" and defense_attempted:
                ...
                if target_weapon_id:
                    set_condition(target_ref, target_weapon_id, _num(target_parry_wear.get("remaining_condition_pct"), target_weapon_condition), reason="weapon_parry", event_id=action["id"] + "_defense")
                    if target_parry_wear.get("failed"):
                        target_eq["best_weapon"] = None
                        target_eq["weapon"] = {"id": "unarmed", "family": "unarmed", "reach_m": 0.65, "minimum_range_m": 0.0, "handling": 1.0, "base_force_blunt": 0.35, "recovery_class": "quick"}
            ...
            if weapon_id and not is_projectile:
                ...
                if weapon_wear_result.get("failed"):
                    ...
                    actor_eq["weapon"] = {"id": "unarmed", "family": "unarmed", "reach_m": 0.65, "minimum_range_m": 0.0, "handling": 1.0, "base_force_blunt": 0.35, "recovery_class": "quick"}
            ...
            if not body_contact:
                denial_result = (
                    "dodged" if defense_method == "dodge"
                ...
                if defense_method == "dodge":
                    ...
                    # Two perpendicular escape candidates plus some outward drift.
                    candidates = [(-dy / mag, dx / mag), (dy / mag, -dx / mag)]
                    step = _clamp(0.13 + 0.18 * timing_factor * saturation_factor, 0.10, 0.34)
                ...
                elif defense_method == "parry" and margin < -18:
                    # A dominant parry can carry the attacker's weapon/body line
                    # inward, but it does not teleport either fighter.
            ...
            if aimed_zone == "mount":
                actual_target = {"body_zone": "mount", "side": "mount", "structure": "mount", "deviation": "none", "aim_preserved": True}
            else:
            ...
            if (not is_projectile) and weapon_id and mode in {"cut","thrust"} and severity in {"moderate","serious","critical"}:
                ...
                if embed_ratio>=threshold and any(token in family for token in ("spear","sword","axe","glaive","polearm","blade")):
                    ...
                    actor_eq["best_weapon"]=None; actor_eq["weapon"]={"id":"unarmed","family":"unarmed","reach_m":0.65,"minimum_range_m":0.0,"handling":1.0,"base_force_blunt":0.35,"recovery_class":"quick"}; actor_eq.setdefault("loadout",{}).pop("primary_melee_weapon",None)
                ...
                if zone == "mount":
                    ...
                    if severity in {"serious", "critical"}:
                        ...
                        fall_event = {
                            ...
                            "kind": "mounted_state",
                            "actor_ref": target_ref,
                            "action": "mount_disabled_or_rider_forced_to_dismount",
                            "severity": severity,
                            "at_s": round(resolve_at, 3),
                        }
                        trace.append(fall_event); must_render.append(fall_event["id"])
                else:
        ...
        for ref in actors:
            ...
            action_cost = action_exertion[ref] * burden_factor * (32.0 / (45.0 + 3.0 * math.sqrt(endurance)))
            gain = max(0, int(math.ceil(baseline + action_cost - 1e-9)))
        ...
        player_equipment = {
            "best_weapon": p_eq.get("best_weapon"),
            "skill_name": p_eq.get("skill_name"),
            "equipped_item_ids": p_eq.get("equipped_item_ids", []),
            "total_load_kg": round(_num(p_burden.get("total_load_kg")), 2),
            "load_ratio": round(_num(p_burden.get("load_ratio")), 4),
            "movement_factor": round(_num(p_burden.get("movement_factor"), 1.0), 4),
            "fatigue_multiplier": round(_num(p_burden.get("fatigue_multiplier"), 1.0), 4),
        }
        opponent_equipment = {
        ...
        return {
            ...
            "timing_model": {
                ...
                "spatial_mode": "local_2d_shared_body_state",
                ...
                "rule": "all exact participants share one local timeline; actors act when their own startup/recovery state becomes ready, defenses consume persistent body/weapon/shield readiness, and near-simultaneous contacts share one pre-contact geometry snapshot instead of resetting the defender",
            },
            "sequencing_model": {

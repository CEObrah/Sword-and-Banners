...
def _stable_target_variation(*parts: Any) -> float:
    """Return deterministic actor/state variation in [-1, 1].

    Exact personal combat must replay deterministically, but deterministic replay
    is not the same thing as making every autonomous fighter consult the same
    ordered anatomy list.  Actor identity, target identity and coarse geometry
    therefore participate in a stable tie-break used only among tactically
    comparable aim candidates.
    """
    token = "|".join(str(part) for part in parts)
    raw = int.from_bytes(hashlib.sha256(token.encode("utf-8")).digest()[:8], "big")
    return (raw / float((1 << 64) - 1)) * 2.0 - 1.0


def _default_personal_combat_positions(
    *,
    player_ref: str,
    player_side: list[str] | tuple[str, ...] | set[str],
    hostile_side: list[str] | tuple[str, ...] | set[str],
    start_distance: float,
    mounted_refs: set[str] | None = None,
    preserve_start_distance: bool = False,
) -> dict[str, dict[str, float]]:
    """Stage unspecified multi-person combat as two opposed bodies, not a free encirclement.

    Explicit participant_positions remain authoritative.  This helper is only the
    neutral fallback when a fresh encounter supplied no local geometry at all.
    Numerical superiority is expressed as depth and extra frontage, while an
    encirclement must be earned by movement or explicitly supplied geometry.
    """
    player_refs = [str(ref) for ref in player_side]
    hostile_refs = [str(ref) for ref in hostile_side]
    if player_ref not in player_refs:
        player_refs.insert(0, player_ref)
    mounted = set(str(ref) for ref in (mounted_refs or set()))
    max_count = max(1, len(player_refs), len(hostile_refs))
    if max_count <= 1:
        front_capacity = 1
    else:
        front_capacity = int(math.ceil(math.sqrt(max_count) * 1.60))
        front_capacity = max(3, min(7, front_capacity))
        if front_capacity % 2 == 0:
            front_capacity = min(7, front_capacity + 1)
    max_radius = max([0.75 if ref in mounted else 0.28 for ref in player_refs + hostile_refs] or [0.28])
    lateral_spacing = max(0.72, max_radius * 2.0 + 0.14)
    rank_depth = max(0.72, max_radius * 2.0 + 0.18)
    # An explicit legacy distance is preserved when physically possible, but it
    # cannot place two occupied bodies inside one another. Multi-person fallback
    # geometry therefore raises only impossible center distances to the minimum
    # clearance required by the largest participant body.
    front_separation = (
        max(0.35, float(start_distance), max_radius * 2.0 + 0.10)
        if preserve_start_distance
        else max(0.60, float(start_distance), max_radius * 2.0 + 0.10)
    )

    def radius(ref: str) -> float:
        return 0.75 if ref in mounted else 0.28

    def rank_slots(count: int, *, hostile: bool) -> list[dict[str, float]]:
        slots: list[dict[str, float]] = []
        remaining = count
        rank = 0
        while remaining > 0:
            n = min(front_capacity, remaining)
            ys = [(idx - (n - 1) / 2.0) * lateral_spacing for idx in range(n)]
            x = (front_separation + rank * rank_depth) if hostile else (-rank * rank_depth)
            facing = 180.0 if hostile else 0.0
            for y in ys:
                slots.append({"x_m": round(x, 6), "y_m": round(y, 6), "elevation_m": 0.0, "facing_deg": facing})
            remaining -= n
            rank += 1
        return slots

    result: dict[str, dict[str, float]] = {}
    player_slots = rank_slots(len(player_refs), hostile=False)
    # Reserve the center-front slot for the player when it exists.
    player_slot_index = min(range(len(player_slots)), key=lambda i: (abs(player_slots[i]["x_m"]), abs(player_slots[i]["y_m"])))
    player_slot = player_slots.pop(player_slot_index)
    result[player_ref] = {**player_slot, "radius_m": radius(player_ref)}
    for ref, slot in zip(sorted(r for r in player_refs if r != player_ref), player_slots):
        result[ref] = {**slot, "radius_m": radius(ref)}

    hostile_slots = rank_slots(len(hostile_refs), hostile=True)
    # Center-out assignment avoids stable-ID ordering turning into a persistent
    # left/right tactical privilege. It changes identity placement, not geometry.
    hostile_slots.sort(key=lambda row: (row["x_m"], abs(row["y_m"]), row["y_m"]))
    for ref, slot in zip(sorted(hostile_refs), hostile_slots):
        result[ref] = {**slot, "radius_m": radius(ref)}
    return result


def _registered_handling_adjustment(handling: Any, model: Mapping[str, Any] | None = None) -> float:
    """Executable form of the registered melee handling adjustment."""
    registered = model if isinstance(model, Mapping) else {}
    neutral = _num(registered.get("neutral_handling"), 1.0)
    slope = _num(registered.get("slope"), 40.0)
    minimum = _num(registered.get("minimum_adjustment"), -16.0)
    maximum = _num(registered.get("maximum_adjustment"), 16.0)
    return _clamp(slope * (_num(handling, neutral) - neutral), minimum, maximum)


def _registered_reach_advantage_points(
    attacker_reach_m: Any,
    defender_reach_m: Any,
    combat_gap_m: Any,
    *,
    attacker_minimum_m: Any = 0.0,
    defender_minimum_m: Any = 0.0,
    bands: Mapping[str, Any] | None = None,
) -> float:
    """Resolve reach control from the registered bands and legal threat geometry.

    Reach is contextual rather than a permanent weapon ranking. A long weapon
    has decisive control while only it can threaten. If a shorter weapon gets
    inside the long weapon's registered minimum range, that one-sided advantage
    reverses. When both can threaten, the ordinary registered reach-delta bands
    apply.
    """
    table = bands if isinstance(bands, Mapping) else {}
    a_reach = max(0.0, _num(attacker_reach_m))
    d_reach = max(0.0, _num(defender_reach_m))
    gap = max(0.0, _num(combat_gap_m))
    a_min = max(0.0, _num(attacker_minimum_m))
    d_min = max(0.0, _num(defender_minimum_m))
    attacker_threat = a_min <= gap <= a_reach + 0.10
    defender_threat = d_min <= gap <= d_reach + 0.10
    decisive = abs(_num(table.get("decisive_one_sided_threat"), 16.0))
    if attacker_threat and not defender_threat:
        return decisive
    if defender_threat and not attacker_threat:
        return -decisive

    delta = a_reach - d_reach
    absolute = abs(delta)
    if absolute < 0.15:
        points = abs(_num(table.get("<0.15"), 0.0))
    elif absolute < 0.40:
        points = abs(_num(table.get("0.15_to_0.39"), 4.0))
    elif absolute < 0.75:
        points = abs(_num(table.get("0.40_to_0.74"), 8.0))
    else:
        points = abs(_num(table.get("0.75_plus"), 12.0))
    return points if delta > 0 else (-points if delta < 0 else 0.0)
...
class PersonalCombatMixin:
    ...
    def _personal_equipment_profile(self, person_ref: str, person: Mapping[str, Any]) -> dict[str, Any]:
        ...
        melee_selection = combat_mechanics.get("default_melee_weapon_selection", {}) if isinstance(combat_mechanics, Mapping) else {}

        def melee_candidate_utility(item: Mapping[str, Any]) -> float:
            force = max(_num(item.get("base_force_cut")), _num(item.get("base_force_thrust")), _num(item.get("base_force_blunt")))
            family = str(item.get("family", item.get("combat_profile", ""))).lower()
            skill_name = self._PERSONAL_FAMILY_SKILL.get(family, "Unarmed")
            proficiency = max(0.0, _num(skills.get(skill_name)))
            return (
                force * _num(melee_selection.get("force_weight"), 8.0)
                + _num(item.get("handling"), 0.8) * _num(melee_selection.get("handling_weight"), 5.0)
                + min(_num(melee_selection.get("reach_cap_m"), 4.0), _num(item.get("reach_m"), 0.75))
                * _num(melee_selection.get("reach_weight"), 1.5)
                + proficiency * _num(melee_selection.get("proficiency_weight"), 0.08)
            )
        ...
        for entry in equipped:
            ...
            if slot == "primary_melee_weapon":
                # Candidate selection considers both the exact item and the
                # actor who must use it. The registered weights keep this
                # policy auditable instead of burying another combat law here.
                melee_candidates.append((melee_candidate_utility(item), item_id, item))
    ...
    def _personal_controls(self, person: Mapping[str, Any], eq: Mapping[str, Any], environment_effects: Mapping[str, Any]) -> dict[str, float]:
        ...
        combat_mechanics = self.read("game/data/mechanics/combat.json")
        handling_model = combat_mechanics.get("handling_adjustment_model", {}) if isinstance(combat_mechanics, Mapping) else {}
        handling_adj = _registered_handling_adjustment(handling, handling_model)
    ...
    def _personal_aim_plan(
        ...
        *,
        ...
        attack_bearing_deg: float | None = None,
        target_facing_deg: float | None = None,
    ) -> dict[str, str]:
        ...
        actor_token = str((actor or {}).get("owner_id") or (actor or {}).get("id") or "anonymous_actor") if isinstance(actor, Mapping) else "anonymous_actor"
        target_token = str((target_person or {}).get("owner_id") or (target_person or {}).get("id") or "anonymous_target") if isinstance(target_person, Mapping) else "anonymous_target"
        attack_bearing = _num(attack_bearing_deg, 0.0) % 360.0
        target_facing = _num(target_facing_deg, 0.0) % 360.0
        relative_bearing = (attack_bearing - target_facing) % 360.0
        bearing_bucket = int(round(relative_bearing / 22.5))
        geometry_side = (
            "left" if 35.0 <= relative_bearing <= 145.0
            else "right" if 215.0 <= relative_bearing <= 325.0
            else None
        )
        if isinstance(priorities, list) and priorities:
            ...
            for index, raw in enumerate(priorities):
                ...
                if not candidate_zone or not candidate_structure:
                    continue
                actor_variation = _stable_target_variation(
                    actor_token, target_token, seed, sequence, index, candidate_structure, bearing_bucket
                )
                side_hint = geometry_side or ("right" if actor_variation >= 0.0 else "left")
                synthetic_intent = f"aim at {side_hint} {candidate_target}"
                ...
                # Registered doctrine is a preference hierarchy, not a shared
                # anatomy macro. Local attack angle chooses the exposed side and
                # actor/target-conditioned opening quality can reorder nearby
                # high-value choices. Armor, observed defense and recent failures
                # remain stronger reasons to abandon a bad line altogether.
                score = 100.0 - index * 5.0 - coverage_penalty + softspot_bonus + actor_variation * 6.0
        ...
        for index, (candidate_zone, candidate_purpose) in enumerate(pool):
            actor_variation = _stable_target_variation(
                actor_token, target_token, seed, sequence, index, candidate_zone, bearing_bucket
            )
            side_hint = geometry_side or ("right" if actor_variation >= 0.0 else "left")
            target = anatomical_target(candidate_zone, f"{side_hint} {intent or ''}", seed + sequence * 13 + index)
            structure = str(target.get("structure", candidate_zone))
            # Generic autonomous fighters do not all consult one ordered anatomy
            # list. Geometry selects the exposed side and a bounded individual
            # tie-break may swap only near-equivalent opening families; observed
            # defenses and recent failed attacks remain stronger replanning cues.
            score = 30.0 - index * 1.5 + actor_variation * 3.5
    ...
    def _personal_combat_slice(
        ...
        *,
        ...
        allow_friendly_fire_risk = bool(payload.get("allow_friendly_fire_risk", False))
        combo_links = [
        ...
        if not positions and len(actors) > 2:
            # A fresh multi-person encounter with no supplied geometry gets a
            # neutral opposed frontage. Preserve the established 1v1 initializer
            # below because duel distance/mounted-method semantics are already
            # covered independently; the fairness defect exists only when one
            # side has multiple bodies and the old fallback granted a free ring.
            # Numerical advantage produces depth and additional lanes, not a
            # free 360-degree encirclement around Tang Wei. Ambushes and
            # surrounding positions remain available through explicit geometry or
            # movement earned during the encounter.
            positions.update(_default_personal_combat_positions(
                player_ref=self.PLAYER_ACTOR,
                player_side=sorted(player_side),
                hostile_side=sorted(hostile_side),
                start_distance=start_distance,
                mounted_refs={ref for ref in actors if bool(timing.get(ref, {}).get("mounted"))},
                preserve_start_distance="distance_m" in payload,
            ))
        else:
            # Partial explicit/restored geometry remains authoritative. Missing
            # participants use the legacy local fill rather than rewriting the
            # caller's already-established scene around a new formation.
        ...
        def currently_targetable_enemies(ref: str, candidates: Any, at_s: float) -> list[str]:
            """Return exact enemies this actor can lawfully target right now.

            The resolver knows every represented participant, but the roster is not
            perception. Static occlusion blocks exact body targeting. A grappled
            opponent remains targetable through physical contact, while close-range
            awareness can supplement weak visual quality only on a clear local path.
            """
            poses = geometry_positions(at_s)
            partner = body_state.get(ref, {}).get("grappled_with")
            constraints = status_constraints(ref, at_s)
            visual_factor = _num(constraints.get("visual_detection_factor"), 1.0)
            awareness_factor = _num(constraints.get("awareness_factor"), 1.0)
            known: list[str] = []
            for enemy in candidates:
                if enemy == partner and enemy in people and active(enemy):
                    known.append(str(enemy))
                    continue
                if ref not in poses or enemy not in poses:
                    continue
                los = line_of_sight_query(ref, enemy, poses, local_obstacles)
                if not bool(los.get("clear")):
                    continue
                visibility = _num(los.get("visibility_factor"), 0.0) * visual_factor
                distance = distance_between(ref, enemy, at_s)
                if visibility >= 0.22 or (distance <= 2.0 and awareness_factor >= 0.20):
                    known.append(str(enemy))
            return known
        ...
        def current_team_plan(ref: str, at_s: float) -> dict[str, Any]:
            ...
            knowledge_by_actor: dict[str, list[str]] = {
                ally: currently_targetable_enemies(ally, enemies, at_s)
                for ally in side
            }
            recent_failures = {
                ally: sum(
        ...
        def tactical_choice(ref: str, targetable: list[str], at_s: float, *, plan: Mapping[str, Any] | None = None) -> str | None:
            if not targetable:
                return None
            team_plan = dict(plan) if isinstance(plan, Mapping) else current_team_plan(ref, at_s)
            poses = geometry_positions(at_s)
            motion_vectors: dict[str, dict[str, float]] = {}
            for other in targetable:
                vx = vy = 0.0
                move = pending.get(other)
                if isinstance(move, Mapping) and move.get("kind") == "movement":
                    start = _num(move.get("start_at_s"), at_s)
                    end = _num(move.get("resolve_at_s"), start)
                    duration = max(1e-6, end - start)
                    vx = (_num(move.get("to_x_m")) - _num(move.get("from_x_m"))) / duration
                    vy = (_num(move.get("to_y_m")) - _num(move.get("from_y_m"))) / duration
                elif at_s < _num(body_state.get(other, {}).get("movement_velocity_until_s"), 0.0):
                    prior = body_state.get(other, {}).get("movement_velocity_xy_mps", (0.0, 0.0))
                    try:
                        vx, vy = float(prior[0]), float(prior[1])
                    except (TypeError, ValueError, IndexError):
                        vx = vy = 0.0
                motion_vectors[other] = {"vx_mps": vx, "vy_mps": vy}
            plan_known = known_tactical_candidates(targetable, team_plan)
            if not plan_known:
                return None
            return choose_tactical_target(
                ref, plan_known, plan=team_plan, people=people, positions=poses,
                motion_vectors=motion_vectors,
            )

        def choose_target(ref: str, at_s: float) -> str | None:
            candidates = hostile_side if ref in player_side else player_side
            alive_all = [other for other in candidates if other != ref and active(other)]
            partner = body_state.get(ref, {}).get("grappled_with")
            if partner in alive_all:
                return str(partner)
            if partner and partner not in alive_all:
                clear_grapple(ref, str(partner))
            targetable = currently_targetable_enemies(ref, alive_all, at_s)
            if ref == self.PLAYER_ACTOR and requested_target:
                # Exact identity expresses player intent, not hidden coordinates.
                # Never convert a remembered/opaque target ID into exact current
                # body position merely because the resolver knows that position.
                return requested_target if requested_target in targetable else None
            if not targetable:
                return None
            plan = current_team_plan(ref, at_s)
            chosen = tactical_choice(ref, targetable, at_s, plan=plan)
            if not chosen:
                return None

            # A thrust-dominant fighter must treat occupied lanes as tactical
            # geometry, not learn about the blocker only after committing the
            # attack.  This is side-neutral and only redirects autonomous target
            # choice. Explicit Tang Wei targets remain protected above.
            eq = equipment.get(ref, {}) if isinstance(equipment.get(ref), Mapping) else {}
            weapon = eq.get("weapon", {}) if isinstance(eq.get("weapon"), Mapping) else {}
            melee_weapon = eq.get("melee_weapon", {}) if isinstance(eq.get("melee_weapon"), Mapping) else {}
            if not melee_weapon and str(weapon.get("schema", "")) == "melee_weapon":
                melee_weapon = weapon
            primary_mode = self._personal_attack_mode(melee_weapon or weapon)[0] if (melee_weapon or weapon) else ""
            if primary_mode == "thrust":
                poses = geometry_positions(at_s)
                if ref in poses and chosen in poses:
                    blocker = first_linear_melee_body_blocker(
                        actor_ref=ref, target_ref=chosen, attack_mode="thrust",
                        start=poses[ref], end=poses[chosen], positions=poses,
                    )
                    if blocker:
                        open_targets = [
                            other for other in targetable
                            if other in poses and first_linear_melee_body_blocker(
                                actor_ref=ref, target_ref=other, attack_mode="thrust",
                                start=poses[ref], end=poses[other], positions=poses,
                            ) is None
                        ]
                        blocker_ref = str(blocker.get("ref", ""))
                        # If the body physically screening the assigned target is
                        # itself a lawful enemy with a clear lane, it is the most
                        # immediate obstacle and a legitimate local target.
                        if blocker_ref in open_targets:
                            return blocker_ref
                        alternate = tactical_choice(ref, open_targets, at_s, plan=plan)
                        if alternate:
                            return alternate
            return chosen
        ...
        def schedule(ref: str) -> None:
            ...
            if target_ref is None:
                declared_now = current_intent() if ref == self.PLAYER_ACTOR else None
                candidates_now = hostile_side if ref in player_side else player_side
                requested_alive = bool(requested_target and requested_target in candidates_now and active(requested_target))
                visible_now = currently_targetable_enemies(ref, [x for x in candidates_now if active(x)], start_at)
                if ref == self.PLAYER_ACTOR and declared_now and (requested_alive or not visible_now):
                    action_serial += 1
                    action_id = f"action_{action_serial:04d}"
                    pending[ref] = {
                        "id": action_id, "sequence": action_serial, "kind": "guard_wait",
                        "actor_ref": ref, "target_ref": requested_target or None,
                        "declared_intent": declared_now,
                        "reason": "requested_target_not_currently_observable" if requested_alive else "no_currently_observable_hostile",
                        "decision_kind": "target_not_currently_observable",
                        "decision_reason": (
                            "the named opponent is not currently observable; exact runtime identity does not reveal a hidden body position"
                            if requested_alive else
                            "no hostile is currently observable well enough for an exact attack target"
                        ),
                        "consume_intent": True, "decision_required": True,
                        "start_at_s": start_at, "resolve_at_s": start_at + 0.05,
                        "recovery_complete_at_s": start_at + 0.05,
                        "decision_source": "player_target_withheld_by_current_perception",
                    }
                return
            ...
            # Weapon/method provenance is not trajectory-risk provenance.
            # "Shoot him" chooses a ranged method but does not silently authorize
            # firing through an ally. Autonomous/team AI always withholds the shot;
            # Tang Wei may accept that exact risk only through the closed structured
            # allow_friendly_fire_risk component. Once released, projectile physics
            # remains fully physical and may still hit an intervening ally.
            if unsafe_lane is not None and not (ref == self.PLAYER_ACTOR and allow_friendly_fire_risk):
                if ref == self.PLAYER_ACTOR and explicit_ranged:
                    pending[ref] = {
                        "id": action_id, "sequence": action_serial, "kind": "guard_wait",
                        "actor_ref": ref, "target_ref": target_ref,
                        "declared_intent": declared,
                        "reason": "friendly_body_in_projectile_lane_requires_explicit_risk_acceptance",
                        "unsafe_lane": dict(unsafe_lane),
                        "consume_intent": True, "decision_required": True,
                        "start_at_s": start_at, "resolve_at_s": start_at + 0.05,
                        "recovery_complete_at_s": start_at + 0.05,
                        "decision_source": "player_method_withheld_by_lane_safety",
                    }
                    return
                use_ranged = False
            ...
            if use_ranged and 0.75 <= start_dist <= max(0.75, ranged_max):
                aim = self._personal_aim_plan(
                    ...
                    attack_bearing_deg=bearing(target_ref, ref, start_at),
                    target_facing_deg=_num(body_state[target_ref].get("facing_deg"), positions[target_ref].get("facing_deg", 0.0)),
                )
                actor_skills_now, actor_attrs_now = _stats(people[ref])
        ...
        while pending and boundary is None:
            ...
            if action["kind"] == "guard_wait":
                ...
                if actor_ref == self.PLAYER_ACTOR and action.get("consume_intent"):
                    settle_combo_link("failed", event["id"])
                    pending_boundary = {
                        "kind": str(action.get("decision_kind") or "unsafe_projectile_lane"),
                        "player_decision_required": True,
                        "reason": str(action.get("decision_reason") or "the declared ranged method is blocked by a friendly-occupied lane; firing anyway requires explicit friendly-fire risk acceptance"),
                        "at_s": round(resolve_at, 3),
                    }
                    if isinstance(action.get("unsafe_lane"), Mapping):
                        pending_boundary["screen_ref"] = action["unsafe_lane"].get("ref")
                    if action.get("target_ref"):
                        pending_boundary["target_ref"] = action.get("target_ref")
                schedule(actor_ref)
                continue
            ...
            # Establish whether the attack lane physically reaches the intended
            # defender before asking that defender to spend reaction bandwidth.
            # A rear spear thrust stopped by a body/wall may be perceived, but it
            # cannot force the screened target to dodge/parry/block an attack that
            # never reaches their lane.
            if is_projectile:
                ...
                path_obstacle = None  # projectile lane was already resolved at release
            else:
                path_ax, path_ay = contact_group_positions.get(actor_ref, position_at(actor_ref, resolve_at))
                path_tx, path_ty = contact_group_positions.get(target_ref, position_at(target_ref, resolve_at))
            ...
            if path_obstacle is None and (detection.get("meaningful_reaction") or defensive_plan):
            ...
            if active_load <= 1e-9:
                recent_sources = {}
            current_source_was_new = defense_method != "none" and actor_ref not in recent_sources
            pressure_sources = set(recent_sources)
            if defense_method != "none":
                pressure_sources.add(actor_ref)
            source_count = len(pressure_sources)
            ...
            reach_advantage = _registered_reach_advantage_points(
                a_reach, d_reach, combat_gap,
                attacker_minimum_m=a_min,
                defender_minimum_m=max(0.0, _num(target_weapon.get("minimum_range_m"), 0.0)),
                bands=(combat_mechanics.get("reach_advantage_points", {}) if isinstance(combat_mechanics, Mapping) else {}),
            )
        ...
        # Scale-appropriate player combat accounting.  Defeat attribution is
        # conservative: a hostile counts for Wei in this resolution only when it
        # was active at the segment start, is no longer active at the end, and
        # Wei is the last material contact source recorded for that hostile in
        # this exact causal trace.  This avoids turning team damage or delayed
        # physiology into fictional personal kills.
        start_inactive = {"dead", "deceased", "destroyed", "killed", "incapacitated"}
        initial_active_hostiles = {
            ref for ref in hostile_side
            if str(initial_participant_health.get(ref, "active")).lower() not in start_inactive
        }
        active_hostile_refs = sorted(ref for ref in hostile_side if active(ref))
        newly_defeated_hostiles = sorted(ref for ref in initial_active_hostiles if ref not in active_hostile_refs)
        last_contact_actor: dict[str, str] = {}
        for event in trace:
            if not isinstance(event, Mapping) or str(event.get("kind") or "") != "contact":
                continue
            target_ref = event.get("target_ref")
            actor_ref = event.get("actor_ref")
            if isinstance(target_ref, str) and target_ref in hostile_side and isinstance(actor_ref, str):
                last_contact_actor[target_ref] = actor_ref
        player_resolution_defeat_refs = [ref for ref in newly_defeated_hostiles if last_contact_actor.get(ref) == self.PLAYER_ACTOR]
        dead_hostile_refs = sorted(
            ref for ref in hostile_side
            if str(people[ref].get("life_status", people[ref].get("status", "active"))).lower()
            in {"dead", "deceased", "destroyed", "killed"}
            or self._person_health(people[ref]).lower() in {"dead", "deceased"}
        )
        incapacitated_hostile_refs = sorted(
            ref for ref in hostile_side if ref not in active_hostile_refs and ref not in dead_hostile_refs
        )
        player_resolution_kill_refs = [ref for ref in player_resolution_defeat_refs if ref in dead_hostile_refs]
        prior_player_local = restored_local_state.get(self.PLAYER_ACTOR)
        prior_tally = prior_player_local.get("personal_tally") if isinstance(prior_player_local, Mapping) and isinstance(prior_player_local.get("personal_tally"), Mapping) else None
        if isinstance(prior_tally, Mapping):
            prior_defeats = max(0, int(prior_tally.get("confirmed_defeats", 0) or 0))
            prior_kills = max(0, int(prior_tally.get("confirmed_kills", 0) or 0))
            tally_status = str(prior_tally.get("status") or "complete_since_combat_start")
        elif isinstance(prior_player_local, Mapping):
            prior_defeats = 0
            prior_kills = 0
            tally_status = "partial_since_tracking_upgrade"
        else:
            prior_defeats = 0
            prior_kills = 0
            tally_status = "complete_since_combat_start"
        encounter_defeats = prior_defeats + len(player_resolution_defeat_refs)
        encounter_kills = prior_kills + len(player_resolution_kill_refs)
        combat_information = {
            "scale": "exact_personal_combat",
            "registered_hostiles": len(hostile_side),
            "hostiles_active": len(active_hostile_refs),
            "hostiles_incapacitated": len(incapacitated_hostile_refs),
            "hostiles_dead": len(dead_hostile_refs),
            "player_confirmed_defeats_this_resolution": len(player_resolution_defeat_refs),
            "player_confirmed_kills_this_resolution": len(player_resolution_kill_refs),
            "player_confirmed_defeats_encounter": encounter_defeats,
            "player_confirmed_kills_encounter": encounter_kills,
            "personal_tally_status": tally_status,
            "privacy": "gm_private_exact_roster_accounting",
            "disclosure_rule": "The exact registered local roster is GM-private resolution context unless Tang Wei has lawfully perceived the full fight. Personal defeat totals are safe only as confirmed attribution; legacy mid-fight encounter totals may be partial.",
        }
        visible_hostile_refs = currently_targetable_enemies(
            self.PLAYER_ACTOR, sorted(hostile_side), exact_elapsed_seconds,
        )
        visible_active = [ref for ref in visible_hostile_refs if ref in active_hostile_refs]
        visible_incapacitated = [ref for ref in visible_hostile_refs if ref in incapacitated_hostile_refs]
        visible_dead = [ref for ref in visible_hostile_refs if ref in dead_hostile_refs]
        player_visible_combat_information = {
            "scale": "exact_personal_combat",
            "currently_visible_hostiles": len(visible_hostile_refs),
            "currently_visible_hostiles_active": len(visible_active),
            "currently_visible_hostiles_incapacitated": len(visible_incapacitated),
            "currently_visible_hostiles_dead": len(visible_dead),
            "player_confirmed_defeats_this_resolution": len(player_resolution_defeat_refs),
            "player_confirmed_kills_this_resolution": len(player_resolution_kill_refs),
            "player_confirmed_defeats_encounter": encounter_defeats,
            "player_confirmed_kills_encounter": encounter_kills,
            "personal_tally_status": tally_status,
            "visibility_scope": "current_tang_wei_detection_plus_confirmed_personal_attribution",
            "disclosure_rule": "Use this projection for player-facing combat accounting. It never upgrades the exact registered roster or an out-of-sight hostile's current disposition into Tang Wei knowledge.",
        }
        ...
        for ref in actors:
            ...
            if ref == self.PLAYER_ACTOR:
                state["local_combat_state"]["personal_tally"] = {
                    "confirmed_defeats": encounter_defeats,
                    "confirmed_kills": encounter_kills,
                    "status": tally_status,
                }
        ...
        return {
            ...
            "causal_trace": trace,
            "end_state": end_state,
            "combat_information": combat_information,
            "player_visible_combat_information": player_visible_combat_information,

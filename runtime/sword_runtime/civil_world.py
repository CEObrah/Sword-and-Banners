...
from sword_runtime.operation_routing import append_operational_order, exact_operation_record, iter_exact_operation_records
from sword_runtime.mercenary_contracts import (
    LIVE_MERCENARY_CONTRACT_STATUSES,
    compact_mercenary_contracts,
    mercenary_has_live_contract,
    mercenary_is_company_owner,
...
from sword_runtime.house_lineage import settle_state_house_lineages
from sword_runtime.stat_access import merged_skill_map
from sword_runtime.state_levy import mobilization_strain_snapshot
from sword_runtime.strategic_war_planning import command_catalog
...
def sync_polity_routes(planner: Any, runtime: dict[str, Any]) -> None:
    ...
    # Polity index is authority:false routing state.  Independent sovereign
    # polities may have no House sovereignty_ref, so exact owner registration
    # is the bounded authoritative discovery set.  The legacy index remains a
    # compatibility hint only and cannot make a polity disappear from causal
    # chronology if its cache entry is lost.
    owner_index = planner.read_optional("state/index/owner-index.json")
    owners = owner_index.get("owners", {}) if isinstance(owner_index, Mapping) else {}
    if isinstance(owners, Mapping):
        for polity_ref, path in owners.items():
            if (
                isinstance(polity_ref, str)
                and isinstance(path, str)
                and path.split("#", 1)[0].startswith("state/politics/polities/")
            ):
                routed_polity_refs.add(polity_ref)
...
class CivilWorldMixin:
    ...
    def _record_private_realized_sale(
        ...
        *,
        ...
        """Accumulate cash-paid private activity for the next fiscal close.

        The caller already moved exact silver and any physical commodity/service
        consequence. Fiscal settlement needs only the current unclosed silver total;
        transaction durability and semantic history live in their dedicated owners.
        """
    ...
    def _settle_private_production(self, state: str, occurrences: int, at: str) -> None:
        ...
        staple_fraction = max(0.0, min(1.0, _fixed(crop_mix.get("staple", 0.971), 0.971)))
        pasture_value_per_km2_month = max(0.0, _fixed(pasture_rule.get("economic_output_silver_per_km2_month", 833.333333), 833.333333))
        ...
        # Active levy bodies are already absent from civilian strata.  The compact
        # mobilization accumulator additionally models post-service recovery lag,
        # so a mass army cannot demobilize at harvest and instantly restore full
        # farm/craft output.  This is derived on read and creates no economy owner.
        mobilization = mobilization_strain_snapshot(self, state=state, at=at)
        mobilization_labor_factor = max(0.25, min(1.0, int(mobilization.get("civil_labor_factor_milli", 1000) or 1000) / 1000.0))
        ...
        totals = {"agricultural": 0, "craft_total": 0, "craft_available": 0, "civilian_population": 0, "grain_due": 0, "grain_consumed": 0, "grain_produced": 0, "pasture_output_value_silver": 0, "construction_material_units": 0, "gross_output_value_silver": 0, "taxable_output_value_silver": 0, "realized_sales_silver": 0}
        ...
        for site_ref in sorted(regions):
            ...
            rural_labor_factor = (min(1.0, (agricultural * access_factor) / required_rural_workers) if required_rural_workers > 0 else 0.0) * mobilization_labor_factor
            material_labor_factor = (min(1.0, (craft * access_factor) / required_material_workers) if required_material_workers > 0 else 0.0) * mobilization_labor_factor
            ...
            pasture_output_value = int(math.floor(pasture_km2 * pasture_value_per_km2_month * rural_labor_factor * forage_factor * months))
            ...
            productive_craft = int(math.floor(min(craft, work_access) * mobilization_labor_factor))
            ...
            gross_output_value = produced_grain * _fixed(market_prices.get("grain_kg", 0.08), 0.08) + pasture_output_value + produced_material * material_value
            ...
            runtime["last_output"] = {"grain_kg": produced_grain, "pasture_output_value_silver": pasture_output_value, "construction_material_units": produced_material, "finished_goods": copy.deepcopy(goods_made), "mobilization_labor_factor_milli": int(round(mobilization_labor_factor * 1000))}
            ...
            regional_closes[site_ref] = {"grain_kg": produced_grain, "pasture_output_value_silver": pasture_output_value, "construction_material_units": produced_material, "finished_goods": goods_made, "grain_consumed_kg": grain_consumed, "grain_shortfall_kg_before_internal_transfer": shortfall, "gross_output_value_silver": max(0, int(round(gross_output_value))), "taxable_output_value_silver": taxable_realized_value, "cash_paid_private_sales_silver": pending_realized_sales, "private_owner_ref": private_owner_ref, "owner_income_due_silver": owner_due, "owner_income_paid_silver": owner_paid}
            ...
            totals["pasture_output_value_silver"] += pasture_output_value
    ...
    def _market_transport_conditions(self, state: str, market_location_ref: str) -> dict[str, Any]:
        ...
        for operation_ref, _path, operation in iter_exact_operation_records(self):
            if str(operation.get("status", "")) not in {"active", "mobilizing", "marching", "occupied"}:
                continue
    ...
    @staticmethod
    def _service_allocation_source_counts(record: Mapping[str, Any]) -> dict[str, int]:
        """Return conserved source-stratum provenance for one local service allocation.

        Older saves stored only one ``source_stratum`` even when later intake for
        the same force could come from a different civilian stratum.  Current
        allocations may carry ``source_strata`` so one exact force owner can keep
        mixed provenance without splitting itself back into retired force IDs.
        """
        raw = record.get("source_strata") if isinstance(record, Mapping) else None
        if isinstance(raw, Mapping):
            counts = {str(k): max(0, int(v)) for k, v in raw.items() if int(v) > 0}
            if sum(counts.values()) == max(0, int(record.get("personnel", 0))):
                return counts
        personnel = max(0, int(record.get("personnel", 0))) if isinstance(record, Mapping) else 0
        source = str(record.get("source_stratum", "agricultural")) if isinstance(record, Mapping) else "agricultural"
        if source == "mixed":
            raise ValueError("mixed local service allocation is missing exact source_strata provenance")
        return {source: personnel} if personnel else {}

    @staticmethod
    def _write_service_allocation_source_counts(record: dict[str, Any], counts: Mapping[str, Any]) -> None:
        clean = {str(k): max(0, int(v)) for k, v in counts.items() if int(v) > 0}
        if sum(clean.values()) != max(0, int(record.get("personnel", 0))):
            raise ValueError("local service allocation source provenance does not equal personnel")
        record["source_strata"] = dict(sorted(clean.items()))
        if len(clean) == 1:
            record["source_stratum"] = next(iter(clean))
        elif clean:
            record["source_stratum"] = "mixed"
        else:
            record.pop("source_stratum", None)

    @staticmethod
    def _partition_service_sources(count: int, available: Mapping[str, int]) -> dict[str, int]:
        """Deterministically partition a partial unspecific release/loss by provenance."""
        count = max(0, int(count))
        weights = {str(k): max(0, int(v)) for k, v in available.items() if int(v) > 0}
        total = sum(weights.values())
        if count <= 0 or total <= 0:
            return {}
        count = min(count, total)
        raw = {key: count * value / total for key, value in weights.items()}
        out = {key: int(math.floor(value)) for key, value in raw.items()}
        remainder = count - sum(out.values())
        for key in sorted(weights, key=lambda k: (-(raw[k] - out[k]), k))[:remainder]:
            out[key] += 1
    ...
    def _add_local_service_allocation(self, row: dict[str, Any], owner_ref: str, count: int, *, service_class: str, source_stratum: str) -> None:
        ...
        sources = self._service_allocation_source_counts(record)
        record["personnel"] = int(record.get("personnel", 0)) + int(count)
        sources[str(source_stratum)] = int(sources.get(str(source_stratum), 0)) + int(count)
        self._write_service_allocation_source_counts(record, sources)
        self._sync_local_population_row(row)
    ...
    def _release_local_service(self, population: dict[str, Any], native_state: str, location_ref: str, count: int, *, service_key: str, to_agricultural: bool = True, service_owner_ref: str | None = None) -> int:
        ...
        for key in keys:
            ...
            take=min(remaining,max(0,int(rec.get("personnel",0))))
            if take <= 0: continue
            sources=self._service_allocation_source_counts(rec)
            source_take=self._partition_service_sources(take,sources)
            if sum(source_take.values()) != take: raise ValueError("local service release could not preserve source provenance")
            rec["personnel"]=int(rec.get("personnel",0))-take; remaining-=take; released+=take
            for source,source_count in source_take.items():
                sources[source]=int(sources.get(source,0))-int(source_count)
                destination=source if to_agricultural else "dependents_children_elderly"
                row.setdefault("civilian_strata",{})[destination]=int(row.get("civilian_strata",{}).get(destination,0))+int(source_count)
            sources={k:v for k,v in sources.items() if int(v)>0}
            if int(rec.get("personnel",0))<=0:
                allocations.pop(key,None)
            else:
                self._write_service_allocation_source_counts(rec,sources)
            if remaining<=0: break
        self._sync_local_population_row(row); population.clear(); population.update(pop); return released

    def _record_local_service_deaths(self, population: dict[str, Any], native_state: str, location_ref: str, count: int, *, service_key: str, service_owner_ref: str | None = None, source_stratum: str | None = None) -> int:
        ...
        for key in keys:
            rec=allocations.get(key)
            if not isinstance(rec,dict) or str(rec.get("service_class",""))!=service_class: continue
            available=max(0,int(rec.get("personnel",0))); take=min(remaining,available)
            if take <= 0: continue
            sources=self._service_allocation_source_counts(rec)
            if source_stratum is not None:
                source_available=max(0,int(sources.get(str(source_stratum),0))); take=min(take,source_available)
                source_take={str(source_stratum):take} if take else {}
            else:
                source_take=self._partition_service_sources(take,sources)
            if take <= 0 or sum(source_take.values()) != take: continue
            rec["personnel"]=available-take; remaining-=take; applied+=take; row["deaths_cumulative"]=int(row.get("deaths_cumulative",0))+take
            for source,source_count in source_take.items(): sources[source]=int(sources.get(source,0))-int(source_count)
            sources={k:v for k,v in sources.items() if int(v)>0}
            if int(rec.get("personnel",0))<=0:
                allocations.pop(key,None)
            else:
    ...
    def _territorial_revenue_plan(self, state: str, occurrences: int) -> list[dict[str, Any]]:
        ...
        for native, location_ref, local_row in self._controlled_demographic_population_rows(state):
            ...
            baseline = site.get("local_baseline", {}) if isinstance(site, Mapping) else {}
            baseline_population = max(0, int(baseline.get("population_allocation", 0))) if isinstance(baseline, Mapping) else 0
            # Realized local output is the primary fiscal base, but a locality
            # cannot keep realizing its full historical administrative/tax
            # capacity after its conserved civilian population collapses. Treat
            # the cold local baseline only as a capacity ceiling: population
            # growth does not multiply already-realized output a second time,
            # while losses below that baseline proportionally reduce collection.
            population_capacity_factor = (
                min(1.0, current_population / baseline_population)
                if baseline_population > 0 else 1.0
            )
            due = max(0, int(round(
                taxable_output * tax_rate * collector_administration
                * compliance * realization * population_capacity_factor
            )))
            if due <= 0:
                continue
            ...
            plans.append({
                ...
                "baseline_population_capacity": baseline_population,
                "population_capacity_factor": round(population_capacity_factor, 6),
    ...
    def _ensure_occupation_rebel_force(
        ...
        *,
        ...
        if exact_operation_record(self, refs["operation_ref"]) is not None:
    ...
    def _support_occupation_rebel_force(self, *, refs: Mapping[str, Any], native_state: str, at: str) -> dict[str, int]:
        """Move bounded local cash support into an already materialized revolt.

        Local grain stays in the civilian economy.  Its scarcity/abundance affects
        the rebel formation through the same derived strategic-supply projection as
        every other field force instead of becoming carried ration inventory.
        """
        force_ref = str(refs.get("force_ref", "")); formation_ref = str(refs.get("formation_ref", "")); faction_ref = str(refs.get("faction_ref", ""))
        if not force_ref or not formation_ref or not faction_ref:
            return {"silver": 0}
        try:
            force_path = self.owner_path(force_ref); formation_path = self.owner_path(formation_ref); faction_path = self.owner_path(faction_ref)
        except (KeyError, ValueError):
            return {"silver": 0}
        force = copy.deepcopy(self.read(force_path)); formation = copy.deepcopy(self.read(formation_path)); faction = copy.deepcopy(self.read(faction_path))
        if int(force.get("headcount", 0)) <= 0 or int(formation.get("personnel", 0)) <= 0:
            return {"silver": 0}
        ep, eco = self._private_economy(native_state); location_ref = str(formation.get("location_ref", ""))
        try: site_ref, local_eco = self._local_economy_region(native_state, eco, location_ref)
        except ValueError: site_ref, local_eco = "", eco
        rules = self._civil_rules().get("occupation", {})
        ...
        if silver:
            ...
            self._sync_local_economy_aggregate(eco); self._write_private_economy(ep, eco); self.put(faction_path, faction)
    ...
    def _reconcile_local_state_service_casualties(
        ...
        """Update local service partitions for state-force deaths exactly once.

        New cohorts carry exact population/location provenance and debit that exact
        local allocation. Legacy baseline cohorts predate the local-origin ledger
        and therefore have no lawful site-level provenance. Their native population
        owner is still known from the state force, so those losses are distributed
        deterministically across that force's existing local service allocations
        instead of being silently skipped. This preserves bodies without inventing
        a false exact origin for legacy personnel.
        """
        ...
        for cohort_id, raw_loss in cohort_losses.items():
            ...
            raw_population_ref = origin.get("population_ref") if isinstance(origin, Mapping) else None
            raw_location_ref = origin.get("source_location_ref") if isinstance(origin, Mapping) else None
            population_ref = raw_population_ref if isinstance(raw_population_ref, str) else ""
            location_ref = raw_location_ref if isinstance(raw_location_ref, str) else ""
            ...
            if location_ref:
                exact[(native_state, location_ref, service_key)] = exact.get((native_state, location_ref, service_key), 0) + loss
            else:
                origin_kind = str(origin.get("kind", "")) if isinstance(origin, Mapping) else ""
                if origin_kind != "baseline_formation_allocation":
                    raise ValueError("unprovenanced state-force casualty is not a legacy baseline cohort")
                unresolved[(native_state, service_key)] = unresolved.get((native_state, service_key), 0) + loss

        out: dict[str, dict[str, int]] = {}
        populations: dict[str, dict[str, Any]] = {}

        def population_for(state: str) -> dict[str, Any]:
            if state not in populations:
                populations[state] = copy.deepcopy(self.read(f"state/population/{state}.json"))
            return populations[state]

        for (native_state, location_ref, service_key), count in sorted(exact.items()):
            pop = population_for(native_state)
        ...
        for (native_state, service_key), count in sorted(unresolved.items()):
            pop = population_for(native_state)
            _pp, pop = self._ensure_local_population_ledger(native_state, pop)
            populations[native_state] = pop
            sites = pop.get("local_population", {}).get("sites", {})
            if not isinstance(sites, Mapping):
                raise ValueError("legacy state-force casualty lacks local service ledger")
            service_class = self._local_service_class(service_key)
            remaining = count
            while remaining > 0:
                ...
                for location_ref, row in sorted(sites.items()):
                    allocations = row.get("service_allocations", {}) if isinstance(row, Mapping) else {}
                    allocation = allocations.get(force_ref) if isinstance(allocations, Mapping) else None
                    available = max(0, int(allocation.get("personnel", 0))) if isinstance(allocation, Mapping) and str(allocation.get("service_class", "")) == service_class else 0
                    if available > 0:
                        available_rows.append((str(location_ref), available))
                if sum(value for _ref, value in available_rows) < remaining:
                    raise ValueError("legacy state-force casualty exceeds conserved local service allocation")
                partition = self._weighted_integer_partition(remaining, [(ref, float(value)) for ref, value in available_rows])
                progressed = 0
                for location_ref, available in available_rows:
                    take = min(available, max(0, int(partition.get(location_ref, 0))))
                    if take <= 0:
                        continue
                    applied = self._record_local_service_deaths(pop, native_state, location_ref, take, service_key=service_key, service_owner_ref=force_ref)
                    if applied != take:
                        raise ValueError("legacy state-force casualty reconciliation failed")
                    out.setdefault(native_state, {})[location_ref] = out.setdefault(native_state, {}).get(location_ref, 0) + take
                    progressed += take
                if progressed <= 0:
                    raise ValueError("legacy state-force casualty reconciliation made no progress")
                remaining -= progressed

        for native_state, pop in sorted(populations.items()):
            self.put(f"state/population/{native_state}.json", pop)
        return out
    ...
    def _reconcile_private_service_casualties(self, force_ref: str, casualties: int, *, at: str, evidence_ref: str) -> dict[str, int]:
        ...
        self.put(pop_path,pop); return out
    ...
    def _autonomy_population(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        if bool(before0.get("subset_of_parent")):
            if owner_ref == "population_tang_manor":
                sync_tang_private_population(self, at=at, reason="legacy_subset_population_host_reconciliation", evidence_ref=str(host.get("host_id", "")))
            return
        ...
        settle_state_house_lineages(
            self, state=owner_key, at=at, years=max(1, int(occurrences)),
            parent_births=births, parent_deaths=deaths,
        )
        if owner_ref == "population_qin":
            sync_tang_private_population(self, at=at, reason="qin_annual_demography_close", evidence_ref=f"population_qin:{at}")
    ...
    def _autonomy_state(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        if hasattr(self, "_autonomous_civic_pressure_review"):
            self._autonomous_civic_pressure_review(state, at, occurrences)
    ...
    def _start_funded_institution_project(self, command: Any, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if raw_project_ref is None:
            project_ref = "project_" + str(command.semantic_digest)[:8]
        else:
            project_ref = str(raw_project_ref)
    ...
    def _proclaim_house_territorial_authority(self, command: Any, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        operation_path, operation = resolved_operation
    ...
    def _autonomy_polity_institution(self, path: str, inst: dict[str, Any], polity_ref: str, occurrences: int, at: str) -> None:
        ...
        if kind == "granary_depot_office" and seat:
            ...
            if native_state and treasury_ref:
                ...
                for resource, share in (("grain_kg", 1.0),):
        ...
        if kind == "horse_administration":
            ...
            for formation_ref in sorted(str(x) for x in allocated):
                ...
                if int(formation.get("readiness", 0)) < 55:
                    low_readiness.append(formation_ref)
                if int(formation.get("personnel", 0)) > 0:
                    if float(military_supply_sufficiency(self, formation)["overall_ratio"]) < 0.70:
    ...
    def _aggregate_idle_regional_mercenary(self, owner_ref: str, at: str) -> bool:
        ...
        if not isinstance(contracts, list):
            return False
        if str(company.get("status")) != "available" or mercenary_has_live_contract(company):
            return False
        ...
        host = next((row for row in runtime.get("hosts", {}).values() if isinstance(row, dict) and row.get("kind") == "mercenary" and row.get("owner_ref") == owner_ref), None) if isinstance(runtime.get("hosts"), Mapping) else None
    ...
    def _broker_one_mercenary_offer(self, at: str) -> dict[str, Any]:
        ...
        if isinstance(owner_index, Mapping):
            for ref, route in (owner_index.get("owners", {}) or {}).items():
                if not isinstance(ref, str) or not isinstance(route, str) or not route.split("#", 1)[0].startswith("state/merc/"):
                    continue
                try:
                    row = self.read(route)
                except (FileNotFoundError, KeyError, ValueError):
                    continue
                if isinstance(row, Mapping) and mercenary_is_company_owner(row):
                    exact_refs.add(str(ref))
        ...
        # Prefer cold regional market supply before already-materialized exact
        # companies. This keeps the mercenary ecology scale-aware: ordinary
        # regional companies materialize only when a real contract makes them
        # causally relevant, while famous/major exact companies remain available
        # as a fallback rather than crowding out the aggregate market.
        merc_refs = sorted(aggregate_refs) + sorted(exact_refs - aggregate_refs)
        ...
        for ref in merc_refs:
            ...
            if company.get("status") != "available" or mercenary_has_live_contract(company):
                continue
            ...
            company["contracts"] = compact_mercenary_contracts(contracts)
            self.put(path, company)
            runtime = copy.deepcopy(self.read("state/runtime.json"))
            if sync_mercenary_route(runtime, ref, company, at, recurrence_seconds=_MERCENARY_REVIEW_SECONDS, priority=75):
                self.put("state/runtime.json", runtime)
    ...
    def _retask_player_controlled_institutional_operation(
        ...
        *operation objective* and issue an exact order; it never moves Wei, chooses a
        ...
        for operation_ref, op_path, operation0 in iter_exact_operation_records(self):
            if str(operation0.get("status", "")) not in {"planned", "mobilizing", "active", "advancing", "engaged"}:
                continue
            ...
            try:
                ...
                pass
            semantic_material = f"{actor_ref}|{operation_ref}|{arc_ref}|{order_goal}|{'|'.join(sorted(state_owned_refs))}"
            semantic_key = hashlib.sha256(semantic_material.encode("utf-8")).hexdigest()[:24]
            # ``semantic_key`` deduplicates one still-live directive. Order identity
            # must instead identify an issuance occurrence, so a later lawful
            # reissue of the same mission after completion cannot collide with
            # durable history from the earlier issuance. ``action_ref`` is the
            # exact autonomous work identity for this occurrence.
            issuance_key = hashlib.sha256(f"{semantic_key}|{action_ref}".encode("utf-8")).hexdigest()[:18]
            order_ref = "operational_order_" + issuance_key
            orders = operation.setdefault("operational_orders", [])
            if not isinstance(orders, list):
                orders = []
                operation["operational_orders"] = orders
            unresolved_statuses = {
                "issued_awaiting_commander_execution",
                "staff_briefed_awaiting_commander_execution",
                "awaiting_operational_briefing",
                "awaiting_entry_authority",
                "strategic_directive_pending_operational_briefing",
            }
            # A completed muster/staging order remains the active semantic order
            # while the field command is waiting for lawful hostile-entry authority.
            # Without this guard, an equivalent autonomous Qin directive can reopen
            # the already-completed staging phase and issue a duplicate order to the
            # place where Tang Wei is already assembled.
            target_state_ref = next((
                str(ref) for ref in operation.get("objective_refs", [])
                if isinstance(ref, str) and ref.startswith("state_") and ref != actor_ref
            ), None)
            waiting_at_staging = (
                str(operation.get("campaign_phase", "")) == "awaiting_entry_authority"
                or str(operation.get("order_status", "")) == "awaiting_entry_authority"
            )
            if waiting_at_staging and target_state_ref:
                from sword_runtime.campaign_briefing import _hostile_entry_authorized
                if not _hostile_entry_authorized(self, actor_ref, target_state_ref):
                    unresolved_statuses.add("staged_awaiting_entry_authority")
            existing = next((
                row for row in reversed(orders)
                if isinstance(row, Mapping)
                and str(operation.get("objective", "")) == order_goal
                and str(row.get("status", "")) in unresolved_statuses
                and (
                    str(row.get("semantic_key", "")) == semantic_key
                    or (
                        str(row.get("issuer_ref", "")) == actor_ref
                        and str(row.get("arc_ref", "")) == arc_ref
                        and str(row.get("objective", "")) == order_goal
                        and sorted(str(x) for x in row.get("applies_to_formation_refs", []) if isinstance(x, str)) == sorted(state_owned_refs)
                    )
                )
            ), None)
            if existing is None:
                ...
                append_operational_order(operation, order)
                ...
                evidence_kind = "player_command_operational_order_issued"
                evidence_stage = "external_consequence"
            else:
                # Equivalent autonomous directives reaffirm one unresolved semantic order.
                # Persist only current facts that actually changed; counters/tails are
                # development-style diaries and do not affect execution.
                changed = False
                if target_ref is not None and str(existing.get("target_ref", "")) != str(target_ref):
                    existing["target_ref"] = target_ref
                    if str(existing.get("actionability_status", "")) == "pending_operational_briefing":
                        existing["strategic_pressure_target_ref"] = target_ref
                    changed = True
                if str(existing.get("semantic_key", "")) != semantic_key:
                ...
                coordination_authority_ref = operation.get("coordination_authority_ref")
                coordination_authority_ref = coordination_authority_ref if isinstance(coordination_authority_ref, str) and coordination_authority_ref else None
                if existing.get("superior_commander_ref") != superior_commander_ref:
                    existing["superior_commander_ref"] = superior_commander_ref
                    changed = True
                if existing.get("coordination_authority_ref") != coordination_authority_ref:
                    existing["coordination_authority_ref"] = coordination_authority_ref
                    changed = True
                operation["last_operational_order_ref"] = str(existing.get("order_ref", order_ref))
                operation["last_operational_order_at"] = str(existing.get("issued_at", at))
                if changed:
                    self.put(op_path, operation)
                order_ref = str(existing.get("order_ref", order_ref))
                evidence_kind = "player_command_operational_order_reaffirmed"
                evidence_stage = "commitment"
            return {
                "kind": evidence_kind,
                "operation_ref": operation_ref,
                "order_ref": order_ref,
    ...
    def _priority_operation_evidence(
        ...
        """Materialize bounded mobilization work from actor-owned forces.

        Sovereign states mobilize intact top-level command organizations rather
        than plucking one formation out of a nested army.  The operation still
        lists only formations owned by the actor's exact force authority, so an
        institutional state order never commandeers a House/private descendant.
        Permanent parentage is unchanged; this is campaign participation only.
        Non-state actors retain the ordinary one-formation priority path.
        """
        ...
        for _op_ref, _op_path, op in iter_exact_operation_records(self):
            if str(op.get("status", "")) not in {"active", "mobilizing", "advancing", "engaged", "occupied"}:
                continue
            active_formations.update(str(x) for x in op.get("formation_refs", []) if isinstance(x, str))

        allocated_refs: list[str] = []
        ...
        for force_ref in force_refs:
            ...
            allocated = force.get("allocated_to_formations", {}) if isinstance(force, Mapping) else {}
            for formation_ref in sorted(str(x) for x in allocated):
                if formation_ref in formation_rows:
                    continue
                try:
                ...
                if int(formation.get("personnel", 0)) <= 0:
                    continue
                formation_rows[formation_ref] = (formation_path, formation)
                allocated_refs.append(formation_ref)

        selection: dict[str, Any] | None = None
        if actor_ref.startswith("state_"):
            state = actor_ref.removeprefix("state_")
            commands = command_catalog(self, state, allocated_refs)
            viable: list[tuple[float, str, dict[str, Any]]] = []
            for row in commands:
                refs = [str(x) for x in row.get("formation_refs", []) if isinstance(x, str)]
                if not refs or any(ref in active_formations for ref in refs):
                    # One formation committed elsewhere makes the intact command
                    # unavailable. Never silently split a standing army.
                    continue
                if any(
                    str(formation_rows.get(ref, ("", {}))[1].get("command_authority", "")) == self.PLAYER_ACTOR
                    or str(formation_rows.get(ref, ("", {}))[1].get("commander_ref", "")) == self.PLAYER_ACTOR
                    for ref in refs
                ):
                    continue
                readiness = float(row.get("readiness_score", 0) or 0)
                personnel = max(0, int(row.get("personnel", 0) or 0))
                score = readiness + min(100.0, personnel / 500.0)
                key = str(row.get("command_group_ref") or row.get("independent_formation_ref") or "")
                mobility_order = 0 if str(row.get("mobility", "mobile")) == "mobile" else 1
                viable.append((mobility_order, score, key, dict(row)))
            if viable:
                selection = sorted(viable, key=lambda item: (item[0], -item[1], item[2]))[0][3]

        # Houses, polities, mercenary forces, and states with no represented army
        # organization retain the bounded formation fallback.
        if selection is None:
            candidates: list[tuple[int, str, str, dict[str, Any]]] = []
            for formation_ref, (formation_path, formation) in formation_rows.items():
            ...
            if not candidates:
                return None
            _score, primary_ref, _formation_path, _formation = sorted(candidates, key=lambda row: (-row[0], row[1]))[0]
            selection = {
                "command_group_ref": None,
                "independent_formation_ref": primary_ref,
                "formation_refs": [primary_ref],
                "location_ref": str(_formation.get("location_ref", "")),
            }

        reinforcement: dict[str, Any] | None = None
        selected_group_ref = selection.get("command_group_ref")
        if actor_ref.startswith("state_"):
            state_force_ref = next((str(ref) for ref in force_refs if str(ref).startswith("force_state_")), "")
            if state_force_ref and isinstance(selected_group_ref, str) and selected_group_ref:
                reinforcement = self._reinforce_state_field_army_for_mobilization(
                    selected_group_ref, state_ref=actor_ref, force_ref=state_force_ref, at=at,
                )
            elif state_force_ref:
                independent_ref = str(selection.get("independent_formation_ref", ""))
                if independent_ref:
                    reinforcement = self._reinforce_state_independent_formation_for_mobilization(
                        independent_ref, state_ref=actor_ref, force_ref=state_force_ref, at=at,
                    )
            if state_force_ref:
                # Reinforcement mutates exact formation owners. Refresh the bounded
                # snapshots before changing mobilization status so stale pre-growth
                # copies cannot overwrite the newly conserved strength.
                for formation_ref in [str(x) for x in selection.get("formation_refs", []) if isinstance(x, str)]:
                    try:
                        formation_rows[formation_ref] = self._load_formation(formation_ref)
                    except ValueError:
                        pass

        selected_refs = [str(x) for x in selection.get("formation_refs", []) if isinstance(x, str)]
        ...
        for formation_ref in selected_refs:
            formation_path, formation0 = formation_rows[formation_ref]
            formation = copy.deepcopy(formation0)
            old_status = str(formation.get("status", ""))
            old_statuses[formation_ref] = old_status
            ...
            self.put(formation_path, formation)
            new_statuses[formation_ref] = str(formation.get("status", old_status))
            loc = str(formation.get("location_ref", ""))
            if loc:
                locations[loc] = locations.get(loc, 0) + max(0, int(formation.get("personnel", 0)))

        operation_location = max(locations, key=lambda ref: (locations[ref], ref)) if locations else str(selection.get("location_ref", ""))
        ...
        if existing_operation is None:
            command_group_ref = selection.get("command_group_ref")
            operation = {
                "schema": "sword-operation",
                "owner_id": op_ref,
                ...
                "location_ref": operation_location,
                "formation_refs": selected_refs,
                "administrative_authorities": [actor_ref],
                "administrative_authority": actor_ref,
                "created_at": at,
                "authority_basis": {
                    "actor_ref": actor_ref,
                    "force_refs": sorted(set(force_refs)),
                    "command_group_ref": command_group_ref,
                    "rule": "actor-owned priority consumer mobilizes one intact lawful command; operation participation never changes permanent army parentage or manpower ownership",
                },
            ...
            if isinstance(command_group_ref, str) and command_group_ref:
                operation["command_group_ref"] = command_group_ref
            self.put(op_path, operation)
            ...
            routes = index.setdefault("operations", {})
            if routes.get(op_ref) != exact_path:
                routes[op_ref] = exact_path
                self.put("state/operations/index.json", index)
        return {
            "kind": "exact_operation_created",
            "operation_ref": op_ref,
            "command_group_ref": selection.get("command_group_ref"),
            "formation_ref": primary_ref,
            "formation_refs": selected_refs,
            "formation_status_before": old_statuses.get(primary_ref, ""),
            "formation_status_after": new_statuses.get(primary_ref, "mobilized"),
            "standing_army_reinforcement": reinforcement if isinstance(reinforcement, Mapping) and int(reinforcement.get("assigned", 0) or 0) > 0 else None,
    ...
    def _world_arc_actor_domain(self, actor_ref: str) -> str | None:
        """Resolve actor domain from authoritative owner/schema before ID convention.

        Full characters can retain a non-char identity after representation
        promotion.  Person Lite remains intentionally excluded here because the
        personal world-arc path requires a full exact character sheet.
        """
        try:
            self._exact_person(actor_ref, active=False)
        except (KeyError, ValueError, FileNotFoundError):
            pass
        else:
            return "person"
    ...
    def _world_arc_completed_priority(self, actor_ref: str, arc_ref: str, target_ref: str | None = None, goal: str | None = None) -> dict[str, Any] | None:
        domain = self._world_arc_actor_domain(actor_ref)
        if domain is None:
            return None
        try:
    ...
    def _world_arc_person_action(self, actor_ref: str, target_ref: str | None, goal: str, at: str, arc_ref: str) -> dict[str, Any]:
        ...
        saved_house_ref = person.get("house_ref")
        derived_house = "house_" + actor_ref.removeprefix("char_") + "_household" if actor_ref.startswith("char_") else ""
        house_ref = str(saved_house_ref) if isinstance(saved_house_ref, str) and saved_house_ref else derived_house
        try:
            hp = self.owner_path(house_ref) if house_ref else None
            house = self.read(hp) if isinstance(hp, str) else None
        except (KeyError, ValueError, FileNotFoundError):
            house = None
    ...
    def _world_arc_state_action(self, actor_ref: str, target_ref: str | None, goal: str, at: str, arc_ref: str) -> dict[str, Any]:
        ...
        existing = next((
            ...
            and str(row.get("arc_ref", "")) == arc_ref
            and str(row.get("goal", "")) == goal[:500]
            and str(row.get("status", "")) in {"queued", "commitment_settled", "material_settled"}
        ), None)
        if existing is not None:
            existing["last_reaffirmed_at"] = at
            if target_ref is not None:
                existing["latest_target_ref"] = target_ref
            status = str(existing.get("status", ""))
            active_material = False
            if status == "material_settled":
                evidence = existing.get("material_evidence") if isinstance(existing.get("material_evidence"), Mapping) else {}
                op_ref = str(evidence.get("operation_ref", ""))
                if op_ref:
                    resolved_operation = exact_operation_record(self, op_ref)
                    op = resolved_operation[1] if resolved_operation is not None else None
                    active_material = isinstance(op, Mapping) and str(op.get("status", "")) in {"planned", "mobilizing", "active", "advancing", "engaged"}
            self.put(path, doc)
            if status != "material_settled" or not active_material:
            ...
            return {
                ...
                "reason": "equivalent state priority remains active; target wording alone does not create a new order",
            }
    ...
    def _world_arc_polity_action(self, actor_ref: str, target_ref: str | None, goal: str, at: str, arc_ref: str) -> dict[str, Any]:
        ...
        self._schedule_world_arc_priority_work(actor_ref=actor_ref, action_ref=action_ref, domain="polity", at=at)
        return {
            "status": "work_queued",
            "actor_ref": actor_ref,
    ...
    def _autonomy_polity(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        if ranked:
            ...
            if formation_refs and severity >= int(mobilization["threat_threshold"]):
                ...
                already = None
                active_ops = 0
                for op_ref, _op_path, op in iter_exact_operation_records(self):
                    if str(op.get("administrative_authority", "")) != polity_ref:
                        continue
                    if threat_ref in [str(x) for x in op.get("objective_refs", [])]:
                        already = str(op_ref)
                        break
                    if str(op.get("status", "")) in {"planned", "mobilizing", "active", "engaged"}:
                        active_ops += 1
                if already is None:
                    if active_ops >= int(mobilization["operation_capacity"]):
                        ...
                        if existing_operation is None:
                            ...
                            exact_path, _existing = existing_operation
                            if index.setdefault("operations", {}).get(op_ref) != exact_path:
                                index["operations"][op_ref] = exact_path
                                self.put("state/operations/index.json", index)
    ...
    def _counterinsurgency_response_operation(self, controller_ref: str, location_ref: str) -> tuple[str, str, dict[str, Any]] | None:
        ...
        for op_ref, op_path, op in iter_exact_operation_records(self):
            if str(op.get("status", "")) not in {"planned", "mobilizing", "active", "engaged"}:
                continue
    ...
    def _autonomy_rebel_faction(self, ref: str, path: str, doc: dict[str, Any], occurrences: int, at: str) -> None:
        ...
        support = self._support_occupation_rebel_force(refs={"force_ref": force_ref, "formation_ref": formation_ref, "faction_ref": ref}, native_state=native_state, at=at) if native_state else {"silver": 0}
        ...
        if native_state in {"qin", "zhao", "chu", "wei", "han", "yan", "qi"}:
            ...
            supply_capacity = max(1, int(max(0, int(formation.get("personnel", 0))) * float(supply_state.get("score_milli", 0)) / 1000.0) + faction_silver // 5)
        ...
        for contract in contracts:
            ...
            if employer in {"house_tang", "institution_house_tang"}:
                ...
                if amount and int(treasury.get("silver", 0)) - amount >= protected:
                    ...
                    self.put(tp, treasury)
                continue
        if doc.get("status") == "available" and not mercenary_has_live_contract(doc):
            candidates = []
            for state in ("qin", "zhao", "chu", "wei", "han", "yan", "qi"):
                sd = self.read(f"state/states/{state}.json")
                threats = sd.get("known_threats", {}) if isinstance(sd, Mapping) else {}
    ...
    def _autonomy_polity_sustain_march(self, formation_ref: str, destination: str, at: str, theater_record: dict[str, Any], key: str, polity_ref: str) -> dict[str, Any]:
        """Return current strategic support for an autonomous polity march.

        Ordinary field-ration convoys were removed.  Route, territory, force size,
        mounts and civilian food stress already determine the formation's derived
        supply condition; movement mechanics apply that condition directly.
        """
        _path, formation = self._load_formation(formation_ref)
        origin = str(formation.get("location_ref", ""))
        if max(0, int(formation.get("personnel", 0))) <= 0 or origin == destination:
            return {"status": "not_needed", "location_ref": origin}
        try:
            _next_hop, _hours = self._formation_route_next(origin, destination, formation=formation, at=at)
        except ValueError:
            return {"status": "no_route", "location_ref": origin}
        supply = evaluate_military_supply(self, formation, at=at)
        return {
            "status": "derived_supply",
            "location_ref": origin,
            "condition": str(supply.get("condition", "adequate")),
            "score_milli": int(supply.get("score_milli", 0)),
            "movement_factor": float(supply.get("movement_factor", 1.0)),
            "nearest_support_ref": supply.get("nearest_support_ref"),
        }

    def _validate_formation_route_edge(
        self,
        ...
        """Enforce exact fortified-node and controlled-edge passage.

        Hostile forces may approach a fortified strategic node to contest it, but
        they may not enter that node and then walk out the far side while its
        physical controller remains hostile. Some passes/fortresses additionally
        control explicit adjacent route edges through ``control_site_ref``. Real
        bypasses remain possible only through other registered route edges.
        """
        ...
        if not sovereign_ref:
            return

        def controller(site_ref: str) -> str:
            row = sites.get(site_ref, {}) if isinstance(sites, Mapping) else {}
            return str(row.get("controller", "")) if isinstance(row, Mapping) else ""

        def is_hostile_controller(controller_ref: str) -> bool:
            if not controller_ref or controller_ref == sovereign_ref:
                return False
            try:
        ...
        def retreat_is_lawful(blocking_site: str) -> bool:
            last = formation.get("last_march_leg", {}) if isinstance(formation.get("last_march_leg"), Mapping) else {}
            if str(last.get("to", "")) == blocking_site and str(last.get("from", "")) == destination_ref:
                return True
            return controller(destination_ref) == sovereign_ref

        origin_gate = enclosing_fortification_site(self.read, origin_ref)
        if origin_gate == origin_ref:
            gate_controller = controller(origin_ref)
            if is_hostile_controller(gate_controller):
                if retreat_is_lawful(origin_ref):
                    return
                raise PermissionError(f"formation cannot transit through hostile fortified site {origin_ref}")

        control_site = str(route.get("control_site_ref", ""))
        if not control_site:
            return
        control_controller = controller(control_site)
        if not control_controller or control_controller == sovereign_ref:
            return
        hostile = is_hostile_controller(control_controller)
        if not hostile:
            self._validate_formation_transit(formation, control_site, at)
            return
        if destination_ref == control_site:
            return
        if origin_ref == control_site and retreat_is_lawful(control_site):
            return
        raise PermissionError(f"formation cannot pass hostile route-control site {control_site} via {route.get('ref')}")
    ...
    def _activate_diplomatic_treaty(self, proposal: dict[str, Any], at: str) -> str:
        ...
        if kind == "alliance":
            ...
            if isinstance(existing, dict):
                parties = existing.setdefault("parties", [])
                newly_joined = target not in parties
                if newly_joined: parties.append(target)
                ...
                if newly_joined:
                    ...
                    existing["accession_history"] = existing["accession_history"][-32:]
                self.put("state/politics/treaties.json", treaty_registry)
    ...
    def _generate_npc_diplomatic_initiative(self, sovereign_ref: str, at: str) -> dict[str, Any] | None:
        ...
        for proposal_ref in [str(x) for x in doc.get("outgoing_diplomatic_proposal_refs", []) if isinstance(x, str)]:
            try:
                proposal_path = self.owner_path(proposal_ref)
                proposal = self.read(proposal_path)
            except (KeyError, ValueError, FileNotFoundError):
                continue
            status = str(proposal.get("status", ""))
            if status not in {"in_transit", "pending_response"}:
                continue
            expires_at = proposal.get("expires_at")
            if isinstance(expires_at, str) and expires_at and CampaignTime.parse(expires_at) <= now:
                expired = copy.deepcopy(proposal)
                expired["status"] = "expired"
                expired["expired_at"] = at
                self.put(proposal_path, expired)
                continue
            return None
    ...
    def _command_layer_civil_world(self, command: Any, payload: Mapping[str, Any], next_dispatch: Any) -> dict[str, Any]:
            ...
            if isinstance(material_losses, Mapping):
                ...
                for formation_ref, raw in material_losses.items():
                    ...
                    local_losses = self._reconcile_local_state_service_casualties(
                        ...
                        evidence_ref=str(result.get("battle_event", command.semantic_digest)),
                    )

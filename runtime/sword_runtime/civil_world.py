...
class CivilWorldMixin:
    ...
    def _adjust_local_pool(regions: dict[str, Any], field: str, key: str | None, delta: int) -> None:
        ...
        if delta > 0:
            ...
            return
        need = -delta
        for ref, row in sorted(rows, key=lambda item: (-int(item[1].get("resident_population", 0)), item[0])):
            if need <= 0:
                break
            if key is None:
                have = max(0, int(row.get(field, 0)))
                take = min(have, need); row[field] = have - take
            else:
                pool = row.setdefault(field, {})
                have = max(0, int(pool.get(key, 0)))
                take = min(have, need); pool[key] = have - take
            need -= take
        if need:
    ...
    def _ensure_local_economy_ledger(self, state: str, eco: dict[str, Any]) -> dict[str, Any]:
        ...
        had_local = isinstance(pop.get("local_population"), Mapping) and int(pop.get("local_population", {}).get("version", 0)) >= 2
        ...
        if not isinstance(local, dict) or int(local.get("version", 0)) < 1 or not isinstance(local.get("regions"), Mapping):
            ...
            local = {
                "version": 1,
                "authority": True,
                "basis": "site-local physical production/cash partition nested inside the one native private-economy owner",
                "regions": regions,
            }
    ...
    def _local_site_weight(self, location_ref: str) -> float:
        row = self._location_record(location_ref)
        kind = str(row.get("kind", "site")) if isinstance(row, Mapping) else "site"
        weights = {
            "capital": 8.0,
            "major_city": 5.5,
            "city": 4.0,
        ...
        functions = {str(x) for x in row.get("functions", [])} if isinstance(row, Mapping) and isinstance(row.get("functions"), list) else set()
        if "taxation" in functions: weight += 1.5
        if "market" in functions: weight += 1.0
        if "recruitment" in functions: weight += 0.6
        if "supply" in functions: weight += 0.4
        if bool(row.get("fortified")) if isinstance(row, Mapping) else False: weight += 0.2
    ...
    def _ensure_local_site_baselines(self, native_state: str) -> dict[str, dict[str, Any]]:
        ...
        native_refs = sorted(str(ref) for ref in sites if self._native_site_state(str(ref)) == native_state)
        if not native_refs:
            return {}
        ...
        for ref in native_refs:
            ...
            if not isinstance(baseline, Mapping) or int(baseline.get("partition_version", 0)) != 1:
                complete = False
                break
    ...
    def _ensure_local_population_ledger(self, native_state: str, population: dict[str, Any] | None = None) -> tuple[str, dict[str, Any]]:
        ...
        Version 2 partitions every civilian source stratum by strategic site and
        ...
        if isinstance(local, Mapping) and int(local.get("version", 0)) >= 2 and isinstance(local.get("sites"), Mapping):
    ...
    def _local_population_row(self, native_state: str, location_ref: str, population: dict[str, Any] | None = None) -> tuple[str, dict[str, Any], dict[str, Any]]:
        ...
        if not isinstance(row, dict):
            raise ValueError(f"location {location_ref} is not allocated inside population_{native_state}")
        return path, pop, row
    ...
    def _consume_local_private_recruitment(self, population: dict[str, Any], native_state: str, requested_location: str, count: int, *, source_stratum: str, force_ref: str, controller_ref: str | None = None) -> list[dict[str, Any]]:
        ...
        for ref, row in sites.items():
            ...
            ranked.append((hours, str(ref)))
        remaining = max(0, int(count)); moved: list[dict[str, Any]] = []
        for _hours, ref in sorted(ranked):
    ...
    def _settle_house_tang_contracts(self, months: int, at: str, treasury_before: Mapping[str, Any]) -> None:
        ...
        for record in registry.get("records", []):
            ...
            available = max(0, int(depot.get("stocks", {}).get(key, 0)))
        ...
        named_ref = str(defense.get("named_marshal_id", ""))
        named_count = 0
        if named_ref:
            try:
                _mp, marshal = self._exact_person(named_ref, active=False)
                named_count = 0 if str(marshal.get("life_status", marshal.get("status", "active"))).lower() in {"dead", "deceased"} else 1
            except ValueError:
                named_count = 0
    ...
    def _broker_one_mercenary_offer(self, at: str) -> dict[str, Any]:
        ...
        for ref in merc_refs:
            ...
            if company.get("status") != "available" or any(str(c.get("status")) in {"offered", "accepted_unpaid", "active", "renewal_offered", "renewal_accepted"} for c in contracts):
                continue
            headcount = max(1, int(company.get("headcount", company.get("personnel", company.get("strength", 1)))))
    ...
    def _autonomy_polity(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        if not house_ref: raise ValueError("sovereign polity lost its House authority")
        occurrences = max(1, int(occurrences))
    ...
    def _autonomy_mercenary(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        for contract in contracts:
            ...
            if not employer.startswith("state_"):
                continue
            state = employer.replace("state_", "")
            sp = f"state/states/{state}.json"; sd = copy.deepcopy(self.read(sp)); amount = max(0, int(contract.get("amount_silver", 0)))
        ...
        if doc.get("status") == "available" and not any(str(c.get("status")) in {"offered", "accepted_unpaid", "active", "renewal_offered", "renewal_accepted"} for c in contracts):
            ...
            if candidates:
                ...
                headcount = max(1, int(doc.get("headcount", doc.get("personnel", doc.get("strength", 1)))))
    ...
    def _validate_formation_transit(self, formation: Mapping[str, Any], destination_ref: str, at: str) -> None:
        ...
        if not controller_ref or not sovereign_ref or controller_ref == sovereign_ref:
            return
        # An active war relation is sufficient authority for hostile entry.
        try:

...
from sword_runtime.infrastructure_projects import INFRASTRUCTURE_PATH, apply_infrastructure_work, infrastructure_work_spec
from sword_runtime.settlement_development import complete_settlement_foundation
...
class CivilWorldMixin:
    ...
    def _state_controlled_living_population(self, state: str) -> int:
        ...
        for location_ref, site in territory.get("sites", {}).items():
            ...
            if native is None:
                continue
            try:
                _pp, _pop, local_row = self._local_population_row(native, str(location_ref))
            except ValueError:
                continue
            total += max(0, int(self._local_taxable_civilian_population(local_row)))
        return total
    ...
    def _ensure_local_site_baselines(self, native_state: str) -> dict[str, dict[str, Any]]:
        ...
        for ref in native_refs:
            ...
            if not isinstance(baseline, Mapping) or int(baseline.get("partition_version", 0)) != 3:
                complete = False
                break
        ...
        taxable_capacity = max(0.0, _fixed(fiscal_rules.get("taxable_capacity_silver_per_local_civilian_month", 1.0), 1.0))
        administration_realization = self._state_administration_realization(native_state)
        ...
        for ref, weight in weighted:
            ...
            baseline = {
                ...
                "taxable_capacity_silver_per_civilian_month": round(taxable_capacity, 4),
                "administration_realization_at_rebaseline": round(administration_realization, 4),
                "relative_weight": round(float(weight), 4),
                "basis": "persistent weighted demographic reference; exact bodies remain conserved in the native state population owner and live tax capacity is recalculated from current local population",
            }
    ...
    def _ensure_local_population_ledger(self, native_state: str, population: dict[str, Any] | None = None) -> tuple[str, dict[str, Any]]:
        ...
        if native_state not in {"qin", "zhao", "chu", "wei", "han", "yan", "qi"}:
            raise ValueError("local population ledger requires a registered regional population owner")
    ...
    def _territorial_revenue_plan(self, state: str, occurrences: int) -> list[dict[str, Any]]:
        ...
        for location_ref, site in territory.get("sites", {}).items():
            ...
            initial_population = max(1, int(baseline.get("initial_taxable_civilian_population", local_row.get("initial_population", baseline.get("population_allocation", 1)))))
            current_population = max(0, int(self._local_taxable_civilian_population(local_row)))
            population_factor = max(0.0, current_population / initial_population)
            ...
            per_site = current_population * taxable_capacity * collector_administration
            due = max(0, int(round(per_site * compliance * realization * occurrences)))
            if due <= 0:
                continue
            plans.append({
                ...
                "local_taxable_civilian_population": current_population,
                "taxable_capacity_silver_per_civilian_month": round(taxable_capacity, 4),
    ...
    def _resolve_funded_project(self, inst: dict[str, Any], project: dict[str, Any], at: str) -> None:
        ...
        if kind == "infrastructure":
            ...
            record = apply_infrastructure_work(registry, work=work, project_ref=str(project.get("project_ref", "")), completed_at=at)
    ...
    def _house_tang_monthly_fiscal_plan(self) -> dict[str, Any]:
        economy = self.read("game/data/mechanics/economy.json")
        rules = economy.get("house_tang_fiscal_model", {}) if isinstance(economy, Mapping) else {}
        rules = rules if isinstance(rules, Mapping) else {}
        ordinary = economy.get("ordinary_market_reference", {}) if isinstance(economy, Mapping) else {}
        wage = max(0.0, _fixed(ordinary.get("professional_soldier_monthly_pay_silver_before_support", 7), 7.0))
        manor_population = self.read("state/population/tang-manor.json")
        civilians = max(0, int(manor_population.get("population_total", 0)))
        commercial_rate = max(0.0, _fixed(rules.get("commercial_receipts_silver_per_civilian_month", 2.8), 2.8))
        revenue = max(0, int(round(civilians * commercial_rate)))

        bastion_multipliers = rules.get("bastion_support_multiplier_by_force", {})
        bastion_multipliers = bastion_multipliers if isinstance(bastion_multipliers, Mapping) else {}
        bastion_rows: list[dict[str, Any]] = []
        bastion_due = 0
        for force_ref in (
            "force_bastion_iron_rampart",
            "force_bastion_red_crane",
            "force_bastion_white_lantern",
            "force_bastion_deep_earth",
        ):
            force = self.read(self.owner_path(force_ref))
            personnel = max(0, int(force.get("headcount", 0)))
            multiplier = max(0.0, _fixed(bastion_multipliers.get(force_ref, 1.3), 1.3))
            due = max(0, int(round(personnel * wage * multiplier)))
            bastion_due += due
            bastion_rows.append({"force_ref": force_ref, "personnel": personnel, "support_multiplier": round(multiplier, 4), "due_silver": due})

        house_force = self.read(self.owner_path("force_house_tang"))
        house_personnel = max(0, int(house_force.get("headcount", 0)))
        house_multiplier = max(0.0, _fixed(rules.get("house_regular_support_multiplier", 1.4), 1.4))
        house_regular_due = max(0, int(round(house_personnel * wage * house_multiplier)))

        sword = manor_population.get("sword_manor", {}) if isinstance(manor_population.get("sword_manor"), Mapping) else {}
        sword_members = max(0, int(sword.get("current_establishment", 0)))
        sword_rate = max(0.0, _fixed(rules.get("sword_manor_support_silver_per_member_month", 7.0), 7.0))
        sword_due = max(0, int(round(sword_members * sword_rate)))
        facility_due = max(0, int(round(_fixed(rules.get("facility_equipment_artillery_maintenance_silver_month", 300000), 300000))))
        estate_due = max(0, int(round(_fixed(rules.get("food_fodder_estate_operations_silver_month", 160000), 160000))))
        expense = bastion_due + house_regular_due + sword_due + facility_due + estate_due
        return {
            "civilian_population": civilians,
            "commercial_receipts_silver_per_civilian_month": round(commercial_rate, 4),
            "revenue_silver": revenue,
            "professional_soldier_monthly_pay_silver_before_support": round(wage, 4),
            "bastion_corps_payroll_expense_silver": bastion_due,
            "bastion_rows": bastion_rows,
            "house_tang_regular_expense_silver": house_regular_due,
            "house_tang_regular_personnel": house_personnel,
            "house_tang_regular_support_multiplier": round(house_multiplier, 4),
            "sword_manor_core_expense_silver": sword_due,
            "sword_manor_members": sword_members,
            "facility_equipment_artillery_maintenance_expense_silver": facility_due,
            "food_fodder_estate_operations_silver": estate_due,
            "expense_silver": expense,
            "net_silver_change": revenue - expense,
        }

    def _settle_house_tang_contracts(self, months: int, at: str, treasury_before: Mapping[str, Any]) -> None:
        """Settle House Tang cash flows through exact conserved transfers.
        ...
        to it. Permanent Bastion and House military payroll is derived from the
        exact current establishments and shared wage authority. Unpaid obligations
        become arrears rather than disappearing or minting money.
        """
        if months <= 0:
            return
        ...
        fiscal_plan = self._house_tang_monthly_fiscal_plan()
        flows = treasury.setdefault("stable_monthly_flows", {})
        flows["revenue_silver"] = int(fiscal_plan["revenue_silver"])
        flows["expense_silver"] = int(fiscal_plan["expense_silver"])
        flows["net_silver_change"] = int(fiscal_plan["net_silver_change"])
        cash_components = treasury.setdefault("monthly_flow_components", {}).setdefault("cash", {})
        cash_components.clear()
        for key in (
            "facility_equipment_artillery_maintenance_expense_silver",
            "food_fodder_estate_operations_silver",
            "house_tang_regular_expense_silver",
            "sword_manor_core_expense_silver",
            "bastion_corps_payroll_expense_silver",
        ):
            cash_components[key] = int(fiscal_plan[key])
        components = cash_components

        # Restore the pre-close cash balance before applying exact cash transfers.
        # Material stock changes already applied by the House owner remain intact and
        # are corrected below against actual delivery.
        ...
        for record in registry.get("records", []):
            ...
            if not isinstance(facts, Mapping) or not facts.get("source_stock") or facts.get("active") is False:
                continue
        ...
        bastion_due = max(0, int(components.get("bastion_corps_payroll_expense_silver", 0))) * months
        bastion_paid = min(bastion_due, int(treasury.get("silver", 0)))
        treasury["silver"] -= bastion_paid
        # Permanent Corps pay enters Tang Manor/Qin private household circulation.
        # The four force owners conserve people/equipment; they are not commercial payees.
        eco["cash_silver"] = int(eco.get("cash_silver", 0)) + bastion_paid

        total_expense_due = max(0, int(flows.get("expense_silver", 0))) * months
        other_due = max(0, total_expense_due - full_supply_due - bastion_due)
        ...
        finance.pop("defense_due_silver", None)
        finance.pop("defense_paid_silver", None)
        finance.pop("defense_arrears_silver", None)
        finance.update({
            "last_close": at,
            "months": months,
            ...
            "revenue_basis": {
                "civilian_population": fiscal_plan["civilian_population"],
                "commercial_receipts_silver_per_civilian_month": fiscal_plan["commercial_receipts_silver_per_civilian_month"],
                "rule": "dynamic Tang Manor commercial/estate receipts; not a fixed House revenue envelope",
            },
            "monthly_fiscal_plan": fiscal_plan,
            "supply_due_silver": supply_due,
            "supply_paid_silver": supply_paid,
            "supply_arrears_silver": max(0, supply_due - supply_paid),
            "bastion_payroll_due_silver": bastion_due,
            "bastion_payroll_paid_silver": bastion_paid,
            "bastion_payroll_arrears_silver": max(0, bastion_due - bastion_paid),
            "bastion_payroll_payee_ref": "tang_manor_service_households",
    ...
    def _world_arc_person_action(self, actor_ref: str, target_ref: str | None, goal: str, at: str, arc_ref: str) -> dict[str, Any]:
        ...
        if isinstance(existing, dict):
            # Reaffirming the same unresolved work collapses duplicate pending rows
            # into one live queue item instead of allowing pressure reviews to grow it.

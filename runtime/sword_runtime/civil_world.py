...
from sword_runtime.geography import demographic_anchor, enclosing_fortification_site, shortest_path as geography_shortest_path, route_exists
from sword_runtime.sim.calendar import CampaignTime


_CIVIL_RULES = "game/data/mechanics/civil-economy.json"
_FACTION_PROFILES = "game/data/politics/faction-profiles.json"
_MERCHANT_HOUSE_REGISTRY = "state/economy/merchant-houses.json"
...
def sync_polity_routes(planner: Any, runtime: dict[str, Any]) -> None:
    ...
    # Remove obsolete polity scheduler routes after dissolution; durable polity/history owners remain.
...
class CivilWorldMixin:
    ...
    def _adjust_local_pool(regions: dict[str, Any], field: str, key: str | None, delta: int) -> None:
        ...
        if delta > 0:
            ...
            return
        # A legacy aggregate-only deduction has no geographic source. Preserve
        # the existing spatial stock distribution proportionally rather than
        # draining the largest population region first (which would create an
        # arbitrary locality effect from a compatibility-field mutation).
        need = -delta
    ...
    def _ensure_local_economy_ledger(self, state: str, eco: dict[str, Any]) -> dict[str, Any]:
        ...
        Legacy aggregate mutations are reconciled into the regional partition on read.
        """
        pop_path = f"state/population/{state}.json"
        pop = copy.deepcopy(self.read(pop_path))
        had_local = isinstance(pop.get("local_population"), Mapping) and int(pop.get("local_population", {}).get("version", 0)) >= 3
        ...
        if not isinstance(local, dict) or int(local.get("version", 0)) < 2 or not isinstance(local.get("regions"), Mapping):
        ...
        # Reconcile aggregate compatibility fields into the exact regional partition.
    ...
    def _merchant_registry(self) -> dict[str, Any]:
        raw = self.read(_MERCHANT_HOUSE_REGISTRY)
        if not isinstance(raw, Mapping) or not isinstance(raw.get("houses"), Mapping):
            raise ValueError("merchant house registry is invalid")
        return copy.deepcopy(dict(raw))

    def _settle_merchant_credit(self, state: str, months: int, at: str) -> None:
        if months <= 0:
            return
        registry = self._merchant_registry()
        ep, eco = self._private_economy(state)
        cash = int(eco.get("cash_silver", 0))
        changed = False
        borrower = f"private_economy_{state}"
        for house_ref in sorted(registry.get("houses", {})):
            house = registry["houses"][house_ref]
            if not isinstance(house, dict):
                continue
            fraction = max(0.0, min(1.0, _fixed(house.get("credit_policy", {}).get("repayment_fraction_per_month", 0.10), 0.10)))
            for loan in house.get("loans", []):
                if not isinstance(loan, dict) or str(loan.get("borrower_ref")) != borrower or str(loan.get("status", "active")) != "active":
                    continue
                outstanding = max(0, int(loan.get("outstanding_silver", 0)))
                if outstanding <= 0:
                    loan["status"] = "repaid"
                    continue
                scheduled = max(1, int(math.ceil(int(loan.get("principal_silver", outstanding)) * fraction * months)))
                paid = min(outstanding, scheduled, cash)
                if paid <= 0:
                    loan["last_review_at"] = at
                    loan["arrears_silver"] = outstanding
                    changed = True
                    continue
                cash -= paid
                house["capital_silver"] = int(house.get("capital_silver", 0)) + paid
                loan["outstanding_silver"] = outstanding - paid
                loan["repaid_silver"] = int(loan.get("repaid_silver", 0)) + paid
                loan["last_payment_at"] = at
                loan["arrears_silver"] = max(0, outstanding - paid)
                if loan["outstanding_silver"] <= 0:
                    loan["status"] = "repaid"
                    loan["repaid_at"] = at
                changed = True
        if changed:
            eco["cash_silver"] = cash
            registry["aggregate_capital_silver"] = sum(max(0, int(h.get("capital_silver", 0))) for h in registry.get("houses", {}).values() if isinstance(h, Mapping))
            registry["last_credit_close"] = at
            self._write_private_economy(ep, eco)
            self.put(_MERCHANT_HOUSE_REGISTRY, registry)

    def _merchant_liquidity_loan(self, profile: Mapping[str, Any], at: str) -> dict[str, Any]:
        states = [str(x) for x in profile.get("states", []) if str(x) in {"qin", "zhao", "chu", "wei", "han", "yan", "qi"}]
        if not states:
            return {"status": "no_market_scope"}
        target = min(states, key=lambda key: int(self.read(f"state/economy/private/{key}.json").get("cash_silver", 0)))
        registry = self._merchant_registry()
        houses = registry.get("houses", {})
        eligible = [
            (int(row.get("capital_silver", 0)), ref, row)
            for ref, row in houses.items()
            if isinstance(row, dict) and str(row.get("status", "active")) == "active" and str(row.get("state")) == target and int(row.get("capital_silver", 0)) > 0
        ]
        if not eligible:
            eligible = [
                (int(row.get("capital_silver", 0)), ref, row)
                for ref, row in houses.items()
                if isinstance(row, dict) and str(row.get("status", "active")) == "active" and int(row.get("capital_silver", 0)) > 0
            ]
        if not eligible:
            return {"status": "no_solvent_merchant_house"}
        capital, house_ref, house = sorted(eligible, key=lambda row: (-row[0], row[1]))[0]
        policy = house.get("credit_policy", {}) if isinstance(house.get("credit_policy"), Mapping) else {}
        requested = max(1, int(profile.get("loan_size_silver", 10_000)))
        amount = min(capital, requested, max(1, int(policy.get("max_single_loan_silver", requested))))
        ep, eco = self._private_economy(target)
        house["capital_silver"] = capital - amount
        eco["cash_silver"] = int(eco.get("cash_silver", 0)) + amount
        loan_ref = "merchant_loan_" + hashlib.sha256(f"{house_ref}|{target}|{at}|{len(house.get('loans', []))}".encode()).hexdigest()[:16]
        loan = {
            "loan_ref": loan_ref,
            "lender_ref": house_ref,
            "borrower_ref": f"private_economy_{target}",
            "principal_silver": amount,
            "outstanding_silver": amount,
            "repaid_silver": 0,
            "issued_at": at,
            "status": "active",
            "interest_silver": 0,
            "basis": "merchant liquidity action using exact saved merchant capital",
        }
        house.setdefault("loans", []).append(loan)
        house["loans"] = house["loans"][-64:]
        registry["aggregate_capital_silver"] = sum(max(0, int(h.get("capital_silver", 0))) for h in houses.values() if isinstance(h, Mapping))
        registry["last_liquidity_action"] = {"at": at, "loan_ref": loan_ref, "lender_ref": house_ref, "borrower_ref": loan["borrower_ref"], "principal_silver": amount}
        self._write_private_economy(ep, eco)
        self.put(_MERCHANT_HOUSE_REGISTRY, registry)
        return {"status": "credit_extended", "loan_ref": loan_ref, "merchant_house_ref": house_ref, "state": target, "principal_silver": amount}
    ...
    def _settle_private_production(self, state: str, occurrences: int, at: str) -> None:
        ...
        rates = rules.get("monthly_output_per_worker", {})
        finished_rates = rules.get("monthly_finished_goods_per_craft_worker", {})
        food_rules = rules.get("labor", {})
        ration = max(0.0, _fixed(food_rules.get("civilian_grain_consumption_kg_per_person_per_month", 8.0), 8.0))
        totals = {"agricultural": 0, "craft_total": 0, "craft_available": 0, "civilian_population": 0, "grain_due": 0, "grain_consumed": 0, "grain_produced": 0, "fodder_produced": 0}
        ...
        for site_ref in sorted(regions):
            ...
            produced_grain = int(round(agricultural * _fixed(rates.get("agricultural_grain_kg", 0)))) * occurrences
            produced_fodder = int(round(agricultural * _fixed(rates.get("agricultural_fodder_kg", 0)))) * occurrences
            produced_material = int(round(craft * _fixed(rates.get("craft_construction_material_units", 0)))) * occurrences
            produced_horses = int(math.floor(agricultural * _fixed(rates.get("agricultural_horse_stock", 0)))) * occurrences
            ...
            stock["horse_stock"] = int(stock.get("horse_stock", 0)) + produced_horses
            goods_made: dict[str, int] = {}
            for key, rate in finished_rates.items():
                produced = int(math.floor(craft * _fixed(rate, 0.0))) * occurrences
                if produced > 0:
                    finished[str(key)] = int(finished.get(str(key), 0)) + produced
                    goods_made[str(key)] = produced
            grain_due = max(0, int(round(civilian_population * ration * occurrences)))
            ...
            runtime["last_labor_basis"] = {"agricultural": agricultural, "craft_total": craft_total, "construction_workers_reserved": reserved, "craft_available": craft}
            runtime["last_food_close"] = {"civilian_population": civilian_population, "grain_due_kg": grain_due, "grain_consumed_kg": grain_consumed, "grain_shortfall_kg": max(0, grain_due - grain_consumed), "produced_grain_kg": produced_grain}
            regional_closes[site_ref] = {"grain_kg": produced_grain, "fodder_kg": produced_fodder, "construction_material_units": produced_material, "horse_stock": produced_horses, "finished_goods": goods_made, "grain_consumed_kg": grain_consumed}
        ...
        self._settle_merchant_credit(state, occurrences, at)
        self._restock_capital_market(state, at)
    ...
    def _restock_capital_market(self, state: str, at: str) -> None:
        ...
        market["demand_factor"] = round(max(1.0, current_demand - recovery + demand_increase), 4)
    ...
    def _ensure_local_site_baselines(self, native_state: str) -> dict[str, dict[str, Any]]:
        ...
        for ref in native_refs:
            ...
            if not isinstance(baseline, Mapping) or int(baseline.get("partition_version", 0)) != 2:
                complete = False
                break
        ...
        monthly_revenue = max(0, int(state_doc.get("normal_monthly_revenue_silver", 0)))
        ...
        tax_partition = self._weighted_integer_partition(monthly_revenue, weighted)
        ...
        for ref, weight in weighted:
            ...
            baseline = {
                ...
                "state_revenue_source_ref": f"state_{native_state}",
                ...
                "basis": "persistent weighted allocation view; exact bodies remain conserved in the native state population owner",
            }
    ...
    def _sync_local_population_row(self, row: dict[str, Any]) -> None:
        ...
        # Compatibility/readability mirrors derived from the exact nested
        # allocations.  They are never writable population authorities.
    ...
    def _ensure_local_population_ledger(self, native_state: str, population: dict[str, Any] | None = None) -> tuple[str, dict[str, Any]]:
        ...
        Version 3 partitions every civilian source stratum only among legitimate demographic owners and
        tracks candidate reservations plus service allocations by exact force/owner.
        ...
        if native_state not in {"qin", "zhao", "chu", "wei", "han", "yan", "qi"}:
            raise ValueError("local population ledger requires a legacy regional population owner")
        ...
        if isinstance(local, Mapping) and int(local.get("version", 0)) >= 3 and isinstance(local.get("sites"), Mapping):
        ...
        for ref, _weight in weighted:
            ...
            if reserved:
                reservations["legacy_reserved_candidates"] = {"source_strata": {"unresolved": reserved}, "reserved_at": None}
            row = {
                "location_ref": ref,
    ...
    def _local_population_row(self, native_state: str, location_ref: str, population: dict[str, Any] | None = None) -> tuple[str, dict[str, Any], dict[str, Any]]:
        ...
        if not isinstance(row, dict):
            # Facilities, gates, depots and offices no longer own demographic
            # rows. Resolve them through explicit containment to the legitimate
            # settlement/territorial population owner instead of recreating the
            # old facility-as-region bug.
    ...
    def _territorial_revenue_plan(self, state: str, occurrences: int) -> list[dict[str, Any]]:
        """Route taxes by current control while preserving the local payer economy.

        A conquered site stops contributing to its former controller and can begin
        contributing to the occupier only through its saved tax-compliance state.
        The silver is collected from the site's homeland private economy, so conquest
        cannot mint a second copy of the same tax base.
        """
        if occurrences <= 0:
            return []
        territory = self.read("state/territory/control.json")
        ...
        for location_ref, site in territory.get("sites", {}).items():
            ...
            if native is None:
                continue
            baselines = self._ensure_local_site_baselines(native)
            baseline = baselines.get(str(location_ref), {})
            per_site = max(0.0, _fixed(baseline.get("monthly_tax_base_silver", 0), 0.0))
            _pp, pop, local_row = self._local_population_row(native, str(location_ref))
            initial_population = max(1, int(local_row.get("initial_population", baseline.get("population_allocation", 1))))
            current_population = self._local_origin_living(local_row)
            population_factor = max(0.0, min(1.25, current_population / initial_population))
            governance = site.get("governance") if isinstance(site.get("governance"), Mapping) else None
            ...
            due = max(0, int(round(per_site * population_factor * compliance * realization * occurrences)))
            if due <= 0:
                continue
            plans.append({
                ...
                "local_population_factor": round(population_factor, 4),
        ...
        *,
        ...
        force = {
            ...
            "cohort_ledger": {
                ...
                "individualization_policy": "anonymous revolt cohorts remain aggregate; exact leaders consume an existing cohort body",
                "cohorts": {},
            },
        }
    ...
    def _contain_occupation_rebel_force(self, *, refs: Mapping[str, Any], native_state: str, at: str) -> int:
        ...
        if isinstance(force.get("cohort_ledger"), Mapping):
            ...
            force["cohort_ledger"] = {
                "schema": "force-cohort-ledger.v1",
                "representation": "aggregate_provenance_cohorts",
                "individualization_policy": "demobilized revolt has no bodies remaining in military custody",
                "cohorts": {},
            }
            validate_cohort_ledger(force)
    ...
    def _occupation_policy_effects(self, governance: Mapping[str, Any]) -> dict[str, float]:
        ...
        tax = values.get("tax_posture", "")
        if any(x in tax for x in ("heavy", "extract", "maximum")):
            out.update({"tax_compliance_delta": 8.0, "resistance_delta": out["resistance_delta"] + 3.0, "elite_delta": out["elite_delta"] - 1.0, "loyalty_delta": out["loyalty_delta"] - 2.0})
        elif any(x in tax for x in ("light", "relief", "reduced")):
            out.update({"tax_compliance_delta": -8.0, "resistance_delta": out["resistance_delta"] - 2.0, "elite_delta": out["elite_delta"] + 1.0, "loyalty_delta": out["loyalty_delta"] + 2.0})
    ...
    def _governor_effects(self, polity: Mapping[str, Any], location_ref: str) -> dict[str, Any]:
        ...
        skills = person.get("skills", {}) if isinstance(person, Mapping) and isinstance(person.get("skills"), Mapping) else {}
        attrs = person.get("attributes", {}) if isinstance(person, Mapping) and isinstance(person.get("attributes"), Mapping) else {}
        values = [max(0, min(200, int(_fixed(skills.get(k, 0), 0)))) for k in ("Governance", "Law", "Diplomacy", "Leadership")]
        presence = max(0, min(200, int(_fixed(attrs.get("Presence", 0), 0))))
        ...
        normalized = max(0.0, min(1.0, score / 200.0 + familiarity))
        return {
    ...
    def _settle_occupation_administration(self, state: str, occurrences: int, at: str) -> None:
        ...
        admin_inst = self.read(f"state/institutions/inst_{state}_regional_administration.json")
    ...
    def _reconcile_private_service_casualties(self, force_ref: str, casualties: int, *, at: str, evidence_ref: str) -> dict[str, int]:
        ...
        for location_ref,row in sorted(sites.items()):
            ...
            if remaining<=0: break
        if remaining:
            # Legacy forces may predate v6 local service allocation. Partition the
            # missing provenance once, then consume it, rather than deleting a body
            # from an unrelated locality.
            territory=self.read("state/territory/control.json"); candidates=[]
            for location_ref,row in sorted(sites.items()):
                site=territory.get("sites",{}).get(location_ref,{}) if isinstance(territory,Mapping) else {}
                if str(site.get("controller",""))!=f"state_{state}": continue
                candidates.append(str(location_ref))
            if not candidates: raise ValueError("private-force casualties have no lawful local population provenance")
            ref=candidates[0]; row=sites[ref]; legacy=row.setdefault("service_allocations",{}).setdefault(force_ref,{"personnel":0,"service_class":"private_house_military","source_stratum":"legacy_private_service"}); legacy["personnel"]=int(legacy.get("personnel",0))+remaining; self._sync_local_population_row(row)
            applied=self._record_local_service_deaths(pop,state,ref,remaining,service_key="private_household_military",service_owner_ref=force_ref)
            if applied!=remaining: raise ValueError("legacy private casualty provenance repair failed")
            out[ref]=out.get(ref,0)+remaining; remaining=0
    ...
    def _reconcile_foreign_service_casualties(
        ...
        *,
        ...
        Core battle resolution historically assumes every body in force_state_X came
        from population_X.active_military. Occupation recruiting lawfully creates a
        mixed-provenance state force, so foreign-service deaths must instead reduce
        the origin population's foreign_military_service stratum. The owner's native
        population deduction is reversed only for those exact foreign cohort losses.
        """
        if not force_ref.startswith("force_state_") or not isinstance(cohort_losses, Mapping):
            return {}
        owner_state = force_ref.removeprefix("force_state_")
    ...
    def _autonomy_population(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        """Settle demography, age maturation, and the conserved local-origin partition."""
        state=self._state_key(str(host["owner_ref"])); pop_path=f"state/population/{state}.json"; before0=copy.deepcopy(self.read(pop_path)); _lp,before=self._ensure_local_population_ledger(state,before0); self.put(pop_path,before)
        before_total=max(0,int(before.get("population_total",0))); before_strata=copy.deepcopy(before.get("strata",{})); super()._autonomy_population(host,occurrences,at); after=copy.deepcopy(self.read(pop_path)); _lp,after=self._ensure_local_population_ledger(state,after); sites=after.get("local_population",{}).get("sites",{})
        delta_total=int(after.get("population_total",0))-before_total
        if isinstance(sites,dict) and sites:
            if delta_total>0:
                weighted=[(ref,max(1,int(row.get("civilian_population",0)))) for ref,row in sites.items() if isinstance(row,Mapping)]; additions=self._weighted_integer_partition(delta_total,weighted)
                for ref,count in additions.items(): row=sites[ref]; cs=row.setdefault("civilian_strata",{}); cs["dependents_children_elderly"]=int(cs.get("dependents_children_elderly",0))+count; self._sync_local_population_row(row)
            elif delta_total<0:
                remaining=-delta_total
                for ref,row in sorted(sites.items(),key=lambda kv:(-int(kv[1].get("agricultural_available",0)),str(kv[0]))):
                    if remaining<=0: break
                    cs=row.setdefault("civilian_strata",{}); available=max(0,int(cs.get("agricultural",0))); take=min(remaining,available); cs["agricultural"]=available-take; row["deaths_cumulative"]=int(row.get("deaths_cumulative",0))+take; remaining-=take; self._sync_local_population_row(row)
                if remaining: raise ValueError("demographic deaths exceed conserved local agricultural population")

        dem=after.setdefault("demography",{}); age=dem.get("age_cohorts") if isinstance(dem.get("age_cohorts"),Mapping) else None
        if not isinstance(age,Mapping) or sum(max(0,int(v)) for v in age.values())!=before_total:
            depend=max(0,int(before_strata.get("dependents_children_elderly",0))); child=min(depend,int(round(depend*0.72))); elder=max(0,depend-child); adult_total=max(0,before_total-depend); young_adult=int(round(adult_total*0.66)); mature_adult=max(0,adult_total-young_adult); age={"age_0_14":child,"age_15_39":young_adult,"age_40_59":mature_adult,"age_60_plus":elder}
        age={k:max(0,int(v)) for k,v in age.items()}; births=max(0,int(round(before_total*_fixed(dem.get("birth_rate_per_thousand",0),0)/1000.0*max(1,occurrences)))); deaths=max(0,int(round(before_total*_fixed(dem.get("death_rate_per_thousand",0),0)/1000.0*max(1,occurrences))))
        mature=min(age.get("age_0_14",0),max(0,int(round(age.get("age_0_14",0)/15.0*max(1,occurrences))))); to_mature=min(age.get("age_15_39",0),max(0,int(round(age.get("age_15_39",0)/25.0*max(1,occurrences))))); to_elder=min(age.get("age_40_59",0),max(0,int(round(age.get("age_40_59",0)/20.0*max(1,occurrences)))))
        # Deaths are age-weighted but remain aggregate; exact named-person mortality is separate.
        weights={"age_0_14":0.35,"age_15_39":0.25,"age_40_59":0.7,"age_60_plus":2.4}; caps={k:age.get(k,0) for k in weights}; death_alloc=self._weighted_integer_partition(min(deaths,sum(caps.values())),[(k,weights[k]*max(1,caps[k])) for k in weights]) if deaths else {k:0 for k in weights}
        age["age_0_14"]=max(0,age.get("age_0_14",0)+births-mature-int(death_alloc.get("age_0_14",0))); age["age_15_39"]=max(0,age.get("age_15_39",0)+mature-to_mature-int(death_alloc.get("age_15_39",0))); age["age_40_59"]=max(0,age.get("age_40_59",0)+to_mature-to_elder-int(death_alloc.get("age_40_59",0))); age["age_60_plus"]=max(0,age.get("age_60_plus",0)+to_elder-int(death_alloc.get("age_60_plus",0)))
        # Reconcile rounding to exact post-close total.
        post_total=max(0,int(after.get("population_total",0))); diff=post_total-sum(age.values()); age["age_0_14"]=max(0,age.get("age_0_14",0)+diff); dem["age_cohorts"]=age

        strata=after.setdefault("strata",{}); depend_key="dependents_children_elderly"; working_keys=[k for k in ("agricultural","craft_and_industry","merchant_and_transport","household_and_service","administration_and_education","camp_medical_support") if k in strata]
        mature_transfer=min(mature,max(0,int(strata.get(depend_key,0)))); strata[depend_key]=max(0,int(strata.get(depend_key,0))-mature_transfer); strata["agricultural"]=int(strata.get("agricultural",0))+mature_transfer
        aging_transfer=min(to_elder,sum(max(0,int(strata.get(k,0))) for k in working_keys)); remaining=aging_transfer
        for key in sorted(working_keys,key=lambda k:(0 if k=="agricultural" else 1,k)):
            if remaining<=0: break
            take=min(remaining,max(0,int(strata.get(key,0)))); strata[key]=int(strata.get(key,0))-take; remaining-=take
        strata[depend_key]=int(strata.get(depend_key,0))+aging_transfer
        if isinstance(sites,dict) and sites:
            # Mirror maturation/aging across locality using the same physical bodies.
            rem=mature_transfer
            for ref,row in sorted(sites.items(),key=lambda kv:(-int((kv[1].get("civilian_strata",{}) if isinstance(kv[1],Mapping) else {}).get(depend_key,0)),str(kv[0]))):
                if rem<=0: break
                cs=row.setdefault("civilian_strata",{}); avail=max(0,int(cs.get(depend_key,0))); take=min(rem,avail); cs[depend_key]=avail-take; cs["agricultural"]=int(cs.get("agricultural",0))+take; rem-=take; self._sync_local_population_row(row)
            rem=aging_transfer
            for ref,row in sorted(sites.items(),key=lambda kv:(-int((kv[1].get("civilian_strata",{}) if isinstance(kv[1],Mapping) else {}).get("agricultural",0)),str(kv[0]))):
                if rem<=0: break
                cs=row.setdefault("civilian_strata",{}); take=0
                for key in sorted(working_keys,key=lambda k:(0 if k=="agricultural" else 1,k)):
                    if rem<=0: break
                    x=min(rem,max(0,int(cs.get(key,0)))); cs[key]=int(cs.get(key,0))-x; rem-=x; take+=x
                cs[depend_key]=int(cs.get(depend_key,0))+take; self._sync_local_population_row(row)
            if rem: raise ValueError("population aging exceeded local working-age strata")
        after["population_total"]=sum(max(0,int(v)) for v in strata.values()); after.setdefault("local_population",{})["last_demography_close"]=at; after["local_population"]["last_maturation"]={"matured_to_working":mature_transfer,"aged_to_dependents":aging_transfer,"births_estimated":births,"deaths_estimated":deaths}
    ...
    def _autonomy_state(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        The legacy state reducer expects synthetic monthly revenue/expense fields.
        Production replaces that macro cash creation with exact transfers first, then
        invokes the existing military/state logic with those macro fields temporarily
        neutralized. This prevents recruiting against revenue that the private economy
        could not actually pay.
        """
        ...
        normal_revenue = max(0, int(original.get("normal_monthly_revenue_silver", 0)))
        normal_expense = max(0, int(original.get("normal_monthly_expense_silver", 0)))
        ...
        for row in revenue_plan:
            ...
            revenue_rows.append({**row, "collected_silver": paid, "arrears_silver": max(0, due - paid)})
        ...
        expense_due = normal_expense * occurrences
        ...
        after["normal_monthly_revenue_silver"] = normal_revenue
        after["normal_monthly_expense_silver"] = normal_expense
        ...
        office = self.read(f"state/institutions/inst_{state}_recruitment_office.json")
        ...
        finance.update({
            ...
            "territorial_realization_factor": round(due_total / max(1, normal_revenue * occurrences), 4),
            ...
            "cash_close_rule": "taxes and normal expenses settle as exact transfers before military autonomy; recruitment can spend only post-close treasury",
        })
    ...
    def _project_funding_source(self, inst: Mapping[str, Any]) -> tuple[str, dict[str, Any], str]:
        ...
        if str(inst.get("owner_id")) == "institution_sword_manor":
    ...
    def _resolve_funded_project(self, inst: dict[str, Any], project: dict[str, Any], at: str) -> None:
        if not isinstance(project.get("inputs_reserved"), Mapping):
            project["status"] = "blocked_unfunded_legacy_project"
            project["blocked_at"] = at
            project["resolution_basis"] = "project cannot create capacity/resources without reserved material inputs"
            return
        ...
        if kind in {"capacity", "construction", "expansion"}:
    ...
    def _cancel_funded_project(self, command: Any, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(inputs, Mapping):
            ...
            project["cancelled_at"] = str(self._world_time())
            project["cancellation_basis"] = "legacy unfunded project cancelled; no material refund existed"
            self.put(path, inst)
            world_time, metrics = self._advance_seconds(1800)
            self._write_meta(command, world_time)
            return self._result(institution_ref=ref, project_ref=project_ref, status="cancelled", refunds={}, world_time=world_time, **metrics)
    ...
    def _ensure_polity_institutions(self, polity_ref: str, polity: dict[str, Any], at: str) -> dict[str, str]:
        ...
        therefore joins the same institutional layer without being rewritten as one
        of the seven legacy state identities.
        """
        ...
        for kind, (capacity, policy) in specs.items():
            ...
            try:
                ...
                inst = {
                    ...
                    "last_review": at,
                    "operating_contract": {
                        "authority": "exact sovereign, treasury, territory, population, force and private-economy owners only",
                        "conservation_rule": "no people, money, mounts, food, equipment, materials, office or territory may be created by review prose alone",
                    },
                }
                self.put(path, inst)
    ...
    def _proclaim_house_territorial_authority(self, command: Any, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        house.setdefault("action_history", []).append({"kind": "proclaim_territorial_authority", "at": str(self._world_time()), "polity_ref": polity_ref, "location_ref": location_ref, "operation_ref": operation_ref})
        house["action_history"] = house["action_history"][-64:]
        ...
        if kind == "granary_depot_office" and seat:
            ...
            if native_state and treasury_ref:
                ...
                for resource, share in (("grain_kg", 1.0), ("fodder_kg", 0.35)):
                    available = max(0, int(regional.setdefault("commodity_stock", {}).get(resource, 0))); resource_target = max(0, int(round(target * share))); price = max(0.0, _fixed(prices.get(resource, 0.08 if resource == "grain_kg" else 0.10), 0.10)); affordable = int((funds - total_cost) // price) if price > 0 else available; qty = min(available, resource_target, max(0, affordable))
                    if qty <= 0: continue
                    cost = int(math.ceil(qty * price)); regional["commodity_stock"][resource] = available - qty; regional["cash_silver"] = int(regional.get("cash_silver", 0)) + cost; inst.setdefault("resources", {})[resource] = int(inst.get("resources", {}).get(resource, 0)) + qty; purchases[resource] = qty; total_cost += cost
                if purchases:
                    treasury[funds_key] = funds - total_cost
                    inst.setdefault("procurement_history", []).append({"at": at, **purchases, "silver": total_cost, "source_ref": f"private_economy_{native_state}", "regional_source_ref": site_ref or None, "destination_ref": str(inst.get("owner_id"))})
                    inst["procurement_history"] = inst["procurement_history"][-24:]
                    self._sync_local_economy_aggregate(eco)
                    self._write_private_economy(ep, eco)
                    self.put(treasury_path, treasury)
            ...
            inst["administration_review"] = {"at": at, "governed_sites": governed[-32:], "governed_site_count": len(governed), "rule": "capacity constrains the polity occupation close; this review does not fabricate compliance"}
            ...
            if treasury_ref and native_state and low_readiness:
                ...
                for formation_ref in sorted(low_readiness)[:max_actions]:
                    ...
                    formation.setdefault("institutional_training_history", []).append({"at": at, "institution_ref": str(inst.get("owner_id")), "silver": per_cost, "readiness_before": before, "readiness_after": formation["readiness"]}); formation["institutional_training_history"] = formation["institutional_training_history"][-16:]
                    funds -= per_cost; regional["cash_silver"] = int(regional.get("cash_silver", 0)) + per_cost; self.put(fp, formation); trained.append({"formation_ref": formation_ref, "silver": per_cost})
            ...
            inst["military_review"] = {"at": at, "formation_count": len(formation_refs), "commander_vacancies": vacancies, "low_readiness": low_readiness, "training_actions": trained, "rule": "bureau may fund bounded training for exact non-player-controlled formations; commander appointments remain separate authority"}
            ...
            inst["recruitment_review"] = {"at": at, "office_capacity": capacity, "controlled_site_access": access_rows[-32:], "rule": "office capacity limits conserved polity recruiting; no review creates people"}
            ...
            if cavalry_need and native_state and treasury_ref:
                ...
                for formation_ref, need in cavalry_need:
                    ...
                    if take <= 0: break
                    fp, formation = self._load_formation(formation_ref); formation = copy.deepcopy(formation); formation.setdefault("mounts", {})["horse_war_military"] = int(formation.get("mounts", {}).get("horse_war_military", 0)) + take; formation.setdefault("mount_procurement_history", []).append({"at": at, "institution_ref": str(inst.get("owner_id")), "mount_ref": "horse_war_military", "count": take, "silver": take * unit}); formation["mount_procurement_history"] = formation["mount_procurement_history"][-16:]; self.put(fp, formation); remaining -= take; purchased += take
                if purchased:
                    regional["commodity_stock"]["horse_stock"] = available - purchased; cost = purchased * unit; regional["cash_silver"] = int(regional.get("cash_silver", 0)) + cost; treasury[funds_key] = funds - cost; self._sync_local_economy_aggregate(eco); self._write_private_economy(ep, eco); self.put(treasury_path, treasury); mounted += purchased
    ...
    def _autonomy_institution(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        if kind == "horse_administration":
            ...
            econ = self.read("game/data/mechanics/economy.json")
            price = max(0.0, _fixed(econ.get("prices_silver", {}).get("grain_kg", 0.08), 0.08))
            sp = f"state/states/{state}.json"
            sd = copy.deepcopy(self.read(sp))
            ...
            if qty:
                ...
                inst.setdefault("procurement_history", []).append({"at": at, "grain_kg": qty, "silver": cost, "source_ref": f"private_economy_{state}", "regional_source_ref": regional_source_ref, "destination_ref": f"state_depot_{state}"})
                inst["procurement_history"] = inst["procurement_history"][-24:]
                # Regional stock is authoritative in v6; update the aggregate
                # compatibility mirror before writing so the legacy reconcile
            ...
            for formation_ref in low_readiness:
                ...
                formation.setdefault("institutional_training_history", []).append({"at": at, "institution_ref": str(inst.get("owner_id")), "silver": unit_cost, "readiness_gain": readiness_gain, "training_gain": training_gain})
                formation["institutional_training_history"] = formation["institutional_training_history"][-16:]
            ...
            inst["fortification_review"] = {"at": at, "priorities": [{"location_ref": loc, "priority_score": score, "already_fortified": fortified} for score, loc, fortified in priorities[:16]], "project_started_ref": started_project, "rule": "bureau may start one funded local-material/local-labor fortification project when exact treasury, materials and workers exist"}
        elif kind == "recruitment_office":
            ...
            inst["recruitment_review"] = {"at": at, "office_capacity": int(inst.get("capacity", 0)), "force_shortage": shortage, "civil_population_total": int(pop.get("population_total", 0)), "active_military": int(pop.get("strata", {}).get("active_military", 0)), "rule": "review does not create recruits; exact population transfer and force intake remain required"}
    ...
    def _settle_house_tang_contracts(self, months: int, at: str, treasury_before: Mapping[str, Any]) -> None:
        """Replace House Tang's legacy synthetic cash close with exact transfers.

        Material food/fodder close remains in the established House treasury owner,
        but contract deliveries are corrected to actual depot stock. Cash revenue is
        collected from the Qin private economy; operating procurement returns cash
        to it; contracted defense is paid into the exact member companies. Unpaid
        obligations become arrears rather than disappearing or minting money.
        """
        if months <= 0:
            return
        tp = "state/treasury/treasury-house-tang.json"
        treasury = copy.deepcopy(self.read(tp))
        registry_path = "state/contract/tang-supply-contracts.json"
        registry = copy.deepcopy(self.read(registry_path))
        depot_path = "state/depots/qin.json"
        depot = copy.deepcopy(self.read(depot_path))
        ep, eco = self._private_economy("qin")
        flows = treasury.get("stable_monthly_flows", {}) if isinstance(treasury.get("stable_monthly_flows"), Mapping) else {}
        components = treasury.get("monthly_flow_components", {}).get("cash", {}) if isinstance(treasury.get("monthly_flow_components"), Mapping) else {}

        # Undo only the legacy synthetic cash net. Any material close already applied
        # by the exact House owner remains intact and is corrected below for delivery.
        treasury["silver"] = max(0, int(treasury_before.get("silver", treasury.get("silver", 0))))
        revenue_due = max(0, int(flows.get("revenue_silver", 0))) * months
        revenue_collected = min(revenue_due, int(eco.get("cash_silver", 0)))
        eco["cash_silver"] = int(eco.get("cash_silver", 0)) - revenue_collected
        treasury["silver"] = int(treasury.get("silver", 0)) + revenue_collected

        full_expected = {"grain_kg": 0, "fodder_kg": 0}
        actual = {"grain_kg": 0, "fodder_kg": 0}
        supply_due = 0
        full_supply_due = 0
        delivery_rows: list[dict[str, Any]] = []
        for record in registry.get("records", []):
            facts = record.get("facts") if isinstance(record, Mapping) else None
            if not isinstance(facts, Mapping) or not facts.get("source_stock"):
                continue
            source_text = str(facts.get("source_stock"))
            key = "grain_kg" if source_text.endswith("grain_kg") else ("fodder_kg" if source_text.endswith("fodder_kg") else "")
            if not key:
                continue
            per_month = max(0, int(facts.get("quantity_per_delivery_kg", 0)))
            per_payment = max(0, int(facts.get("payment_per_delivery_silver", 0)))
            expected = per_month * months
            expected_payment = per_payment * months
            full_expected[key] += expected
            full_supply_due += expected_payment
            source_location = str(facts.get("source_location_ref") or depot.get("location_ref") or "")
            delivery_location = str(facts.get("delivery_location_ref") or "loc_tang_manor_outer_gate")
            contract_route_ref = str(facts.get("route_ref") or facts.get("route") or "")
            route_ok = bool(source_location and delivery_location and contract_route_ref and route_exists(self.read, contract_route_ref))
            route_plan = None
            if route_ok:
                try:
                    route_plan = geography_shortest_path(self.read, source_location, delivery_location, modes=("convoy",))
                    required_ref = str(facts.get("route_ref") or "")
                    if required_ref and required_ref not in set(route_plan.get("route_refs", [])):
                        route_ok = False
                except (ValueError, KeyError, FileNotFoundError):
                    route_ok = False
            available = max(0, int(depot.get("stocks", {}).get(key, 0))) if route_ok else 0
            deliver = min(expected, available)
            due = int(round(expected_payment * (deliver / expected))) if expected else 0
            depot["stocks"][key] = available - deliver
            actual[key] += deliver
            supply_due += due
            delivery_rows.append({
                "contract_ref": record.get("record_id"),
                "resource": key,
                "expected": expected,
                "delivered": deliver,
                "payment_due_silver": due,
                "source_ref": f"state_depot_qin.{key}",
                "seller": facts.get("seller"),
                "route_ref": facts.get("route_ref") or facts.get("route"),
                "route_refs": list(route_plan.get("route_refs", [])) if isinstance(route_plan, Mapping) else [],
                "route_open": bool(route_ok),
            })

        # Base material close assumed full deliveries. Remove only shortfall.
        treasury["food_kg"] += actual["grain_kg"] - full_expected["grain_kg"]
        treasury["fodder_kg"] += actual["fodder_kg"] - full_expected["fodder_kg"]

        supply_paid = min(supply_due, int(treasury.get("silver", 0)))
        treasury["silver"] -= supply_paid
        eco["cash_silver"] = int(eco.get("cash_silver", 0)) + supply_paid

        defense_due = max(0, int(components.get("contracted_outer_defense_expense_silver", 0))) * months
        defense_paid = min(defense_due, int(treasury.get("silver", 0)))
        treasury["silver"] -= defense_paid
        defense_path = "state/contract/tang-contracted-defense.json"
        defense = copy.deepcopy(self.read(defense_path))
        members: list[tuple[str, str, dict[str, Any], int]] = []
        for member_ref in defense.get("member_force_ids", []):
            try:
                member_path = self.owner_path(str(member_ref))
                company = copy.deepcopy(self.read(member_path))
            except (KeyError, ValueError, FileNotFoundError):
                continue
            headcount = max(0, int(company.get("headcount", 0)))
            company["contract_ref"] = "contract_tang_contracted_defense"
            company["contracting_household_ref"] = "house_tang"
            members.append((str(member_ref), member_path, company, headcount))
        total_heads = sum(row[3] for row in members)
        distributed = 0
        for index, (member_ref, member_path, company, headcount) in enumerate(sorted(members, key=lambda row: row[0])):
            if index == len(members) - 1:
                share = defense_paid - distributed
            else:
                share = int(math.floor(defense_paid * headcount / total_heads)) if total_heads > 0 else 0
                distributed += share
            company["treasury_silver"] = int(company.get("treasury_silver", 0)) + max(0, share)
            company.setdefault("contract_runtime", {})["last_payment"] = {
                "at": at,
                "contract_ref": "contract_tang_contracted_defense",
                "paid_silver": max(0, share),
                "payer_ref": "treasury_house_tang",
            }
            self.put(member_path, company)

        defense["anonymous_total"] = total_heads
        named_count = 0
        for field in ("named_marshal_id", "named_deputy_id"):
            named_ref = str(defense.get(field, ""))
            if not named_ref:
                continue
            try:
                _mp, named = self._exact_person(named_ref, active=False)
                if str(named.get("life_status", named.get("status", "active"))).lower() not in {"dead", "deceased"}:
                    named_count += 1
            except ValueError:
                continue
        defense["combined_total"] = total_heads + named_count
        defense.setdefault("runtime", {})["last_settled_at"] = at
        defense["runtime"]["status"] = "active" if total_heads > 0 else "inactive_no_member_force"
        defense["runtime"]["last_payment"] = {
            "months": months,
            "due_silver": defense_due,
            "paid_silver": defense_paid,
            "arrears_silver": max(0, defense_due - defense_paid),
            "payer_ref": "treasury_house_tang",
        }

        total_expense_due = max(0, int(flows.get("expense_silver", 0))) * months
        other_due = max(0, total_expense_due - full_supply_due - defense_due)
        other_paid = min(other_due, int(treasury.get("silver", 0)))
        treasury["silver"] -= other_paid
        eco["cash_silver"] = int(eco.get("cash_silver", 0)) + other_paid

        finance = treasury.setdefault("civil_finance", {})
        finance.update({
            "last_close": at,
            "months": months,
            "revenue_due_silver": revenue_due,
            "revenue_collected_silver": revenue_collected,
            "revenue_arrears_silver": max(0, revenue_due - revenue_collected),
            "revenue_source_ref": "private_economy_qin",
            "supply_due_silver": supply_due,
            "supply_paid_silver": supply_paid,
            "supply_arrears_silver": max(0, supply_due - supply_paid),
            "defense_due_silver": defense_due,
            "defense_paid_silver": defense_paid,
            "defense_arrears_silver": max(0, defense_due - defense_paid),
            "operating_due_silver": other_due,
            "operating_paid_silver": other_paid,
            "operating_arrears_silver": max(0, other_due - other_paid),
            "operating_payee_ref": "private_economy_qin",
            "cash_close_rule": "all realized revenue and expenses are exact transfers; unpaid amounts remain arrears",
        })
        registry.setdefault("runtime", {})["last_settled_at"] = at
        registry["runtime"]["last_delivery"] = {
            "months": months,
            "grain_kg": actual["grain_kg"],
            "fodder_kg": actual["fodder_kg"],
            "paid_silver": supply_paid,
            "rows": delivery_rows,
        }
    ...
    def _autonomy_house(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        if isinstance(force_ref, str):
            before_force_n = int(self.read(self.owner_path(force_ref)).get("headcount", 0))
        tang_treasury_before = copy.deepcopy(self.read("state/treasury/treasury-house-tang.json")) if house_ref == "house_tang" else None
        super()._autonomy_house(host, occurrences, at)
        if house_ref != "house_tang":
            ...
            lineage["representation"] = "aggregate household branches; no unnamed person is treated as exact"
            ...
            lineage.setdefault("reviews", []).append({
                "at": at,
                "occurrences": occurrences,
                "deltas": {key: int(after_cohort.get(key, 0)) - int(before_cohort.get(key, 0)) for key in ("children", "adults", "elders", "marriages")},
                "basis": "aggregate lineage close; exact people materialize only when individually relevant",
            })
            lineage["reviews"] = lineage["reviews"][-24:]
            self.put(house_path, reviewed)
        months = max(1, int(round(int(host.get("recurrence_seconds", 10368000)) * occurrences / 2592000)))
        if house_ref == "house_tang":
            recruited = 0
            ...
            if recruited:
                state = "qin"; pop_path = f"state/population/{state}.json"; pop = copy.deepcopy(self.read(pop_path)); force = self.read(self.owner_path(force_ref))
                source_loc = str(force.get("source_location_ref") or before_house.get("location_ref") or "loc_tang_manor_garrison_yard")
                rows = self._consume_local_private_recruitment(pop, state, source_loc, recruited, source_stratum="household_and_service", force_ref=str(force_ref), controller_ref="state_qin")
                self.put(pop_path, pop)
                house_after = copy.deepcopy(self.read(house_path)); house_after.setdefault("local_recruitment_history", []).append({"at": at, "personnel": recruited, "force_ref": force_ref, "local_sources": rows}); house_after["local_recruitment_history"] = house_after["local_recruitment_history"][-24:]; self.put(house_path, house_after)
            self._settle_house_tang_contracts(months, at, tang_treasury_before or {})
            return
        house = copy.deepcopy(self.read(house_path))
        state = self._state_key(house.get("state"))
        ep, eco = self._private_economy(state)
        house_location = str(house.get("location_ref") or "")
        try:
            estate_region_ref, estate_region = self._local_economy_region(state, eco, house_location)
        except ValueError:
            estate_region_ref, estate_region = "", eco
        synthetic = int(self._civil_rules().get("house", {}).get("quarterly_estate_income_silver", 1000)) * occurrences
        house["treasury_silver"] = max(0, int(house.get("treasury_silver", 0)) - synthetic)
        income = min(synthetic, int(estate_region.get("cash_silver", 0)))
        estate_region["cash_silver"] = int(estate_region.get("cash_silver", 0)) - income
        house["treasury_silver"] += income
        recruited = 0
        if isinstance(force_ref, str) and before_force_n is not None:
            recruited = max(0, int(self.read(self.owner_path(force_ref)).get("headcount", 0)) - before_force_n)
        if recruited and isinstance(force_ref, str):
            pop_path = f"state/population/{state}.json"; pop = copy.deepcopy(self.read(pop_path)); force_now = self.read(self.owner_path(force_ref)); source_loc = str(force_now.get("source_location_ref") or house.get("location_ref") or f"loc_{state}")
            rows = self._consume_local_private_recruitment(pop, state, source_loc, recruited, source_stratum="household_and_service", force_ref=force_ref, controller_ref=f"state_{state}")
            self.put(pop_path, pop); house.setdefault("local_recruitment_history", []).append({"at": at, "personnel": recruited, "force_ref": force_ref, "local_sources": rows}); house["local_recruitment_history"] = house["local_recruitment_history"][-24:]
        recruit_cost = recruited * int(self._civil_rules().get("house", {}).get("recruitment_basic_issue_cost_silver_per_person", 12))
        paid = min(recruit_cost, int(house.get("treasury_silver", 0)))
        house["treasury_silver"] -= paid
        estate_region["cash_silver"] = int(estate_region.get("cash_silver", 0)) + paid
        finance = house.setdefault("civil_finance", {})
        finance.update({"last_close": at, "estate_income_received_silver": income, "estate_income_due_silver": synthetic, "recruits": recruited, "recruitment_cost_due_silver": recruit_cost, "recruitment_cost_paid_silver": paid, "arrears_silver": max(0, recruit_cost - paid), "payer_ref": f"private_economy_{state}", "regional_economy_ref": estate_region_ref or None})
        self._sync_local_economy_aggregate(eco); self._write_private_economy(ep, eco); self.put(house_path, house)
    ...
    def _broker_one_mercenary_offer(self, at: str) -> dict[str, Any]:
        ...
        for ref in merc_refs:
            ...
            contracts.append({
                ...
                "term_days": 90,
                "offered_at": at,
                "broker_ref": "faction_mercenary_brokers",
    ...
    def _faction_organizational_observation(self, ref: str, doc: dict[str, Any], profile: Mapping[str, Any], at: str | None) -> list[str]:
        ...
        receipts = doc.setdefault("organizational_knowledge_receipts", [])
        if not isinstance(receipts, list):
            raise ValueError("faction organizational knowledge receipts are invalid")
        if not any(isinstance(row, Mapping) and row.get("information_ref") == receipt_ref for row in receipts):
            receipts.append({
                "information_ref": receipt_ref,
                "at": at,
                "kind": "organization_direct_observation",
                "channels": channel_kinds,
                "observed_refs": observed_refs[:16],
                "scope": "own patrol/contact network and directly observable frontier conditions only; no hidden enemy facts",
                "provenance": "registered organizational information channel plus saved faction resources and relationships",
            })
            del receipts[:-32]
        return [receipt_ref]
    ...
    def _priority_row(self, actor_ref: str, domain: str, action_ref: str) -> tuple[str, dict[str, Any], dict[str, Any] | None]:
        ...
        doc = copy.deepcopy(self.read(path))
        if domain == "person":
            queue = doc.setdefault("runtime", {}).setdefault("autonomous_action_attempts", [])
        ...
        row = next((item for item in queue if isinstance(item, dict) and str(item.get("action_ref", "")) == action_ref), None)
        return path, doc, row
    ...
    def _priority_operation_evidence(
        ...
        self.put(formation_path, formation)
        op_ref = "operation_arc_" + hashlib.sha256(action_ref.encode("utf-8")).hexdigest()[:18]
    ...
    def _settle_world_arc_priority_host(self, host: Mapping[str, Any], at: str) -> None:
        ...
        try:
            ...
            return
        if not isinstance(row, dict) or str(row.get("status", "")) not in {"queued", "queued_for_institutional_review", "attempted"}:
            return
        ...
        if domain == "person":
            # A person can always make a self-owned commitment, but this grants no
            # target consent, movement, office, injury, or relationship outcome.
            commitments = doc.setdefault("runtime", {}).setdefault("autonomous_material_actions", [])
            record = {"at": at, "action_ref": action_ref, "arc_ref": arc_ref, "kind": "personal_commitment", "goal": goal[:500], "target_ref": target_ref, "rule": "self-owned commitment only; external consequences require their own subsystem"}
            if not any(isinstance(x, Mapping) and str(x.get("action_ref", "")) == action_ref for x in commitments):
                commitments.append(record); del commitments[:-24]
    ...
    def _world_arc_completed_priority(self, actor_ref: str, arc_ref: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            return None
        queue = doc.get("runtime", {}).get("autonomous_action_attempts", []) if domain == "person" else doc.get("operational_priorities", []) if domain == "institution" else doc.get("world_arc_priorities", [])
        if not isinstance(queue, list):
            return None
        row = next((x for x in queue if isinstance(x, dict) and str(x.get("arc_ref", "")) == arc_ref and str(x.get("status", "")) in {"material_settled", "commitment_settled"} and x.get("arc_observed") is not True), None)
        if not isinstance(row, dict):
            return None
    ...
    def _world_arc_person_action(self, actor_ref: str, target_ref: str | None, goal: str, at: str, arc_ref: str) -> dict[str, Any]:
        ...
        text = goal.lower()
        skill_names = ("Diplomacy", "Intrigue", "Governance", "Leadership") if any(x in text for x in ("court", "polit", "royal", "advoc", "network", "service")) else ("Strategy", "Leadership", "Awareness", "Diplomacy")
        ...
        attempts = runtime.setdefault("autonomous_action_attempts", [])
        if not isinstance(attempts, list):
            raise ValueError("person autonomous action attempts are invalid")
        def same_pending_person_arc(row: object) -> bool:
            return (
                ...
                and str(row.get("status", "")) in {"queued", "attempted"}
            )
        existing = next((row for row in reversed(attempts) if same_pending_person_arc(row)), None)
        if isinstance(existing, dict):
            # v3 could already contain repeated dead attempt rows.  Reaffirming the
            # same unresolved work collapses those historical duplicates into the
            # one live queue item instead of allowing pressure reviews to grow it.
            attempts[:] = [row for row in attempts if row is existing or not same_pending_person_arc(row)]
            existing["last_reaffirmed_at"] = at
            existing["status"] = "queued"
            action_ref = str(existing.get("action_ref", ""))
            record = copy.deepcopy(existing)
        else:
            action_ref = "person_arc_" + hashlib.sha256(f"{actor_ref}|{arc_ref}|{goal}|{target_ref or ''}".encode("utf-8")).hexdigest()[:18]
            record = {
                "action_ref": action_ref,
                "at": at,
                ...
                "target_ref": target_ref,
                "goal": goal[:500],
                "capability_basis": [{"skill": name, "score": score} for score, name in capability[:3]],
                "status": "queued",
                "outcome_rule": "this exact-person action attempt creates no consent, appointment, movement, injury, relationship change, or external result by itself",
            }
            attempts.append(record)
            del attempts[:-24]
        runtime["last_autonomous_action_at"] = at
    ...
    def _world_arc_state_action(self, actor_ref: str, target_ref: str | None, goal: str, at: str, arc_ref: str) -> dict[str, Any]:
        ...
        action_ref = "state_arc_" + hashlib.sha256(f"{actor_ref}|{arc_ref}|{goal}|{target_ref or ''}".encode("utf-8")).hexdigest()[:18]
        ep, eco = self._private_economy(state)
    ...
    def _world_arc_polity_action(self, actor_ref: str, target_ref: str | None, goal: str, at: str, arc_ref: str) -> dict[str, Any]:
        ...
        action_ref = "polity_arc_" + hashlib.sha256(f"{actor_ref}|{arc_ref}|{goal}|{target_ref or ''}".encode("utf-8")).hexdigest()[:18]
        ep, eco = self._private_economy(native_state); treasury[funds_key] = funds - spend; eco["cash_silver"] = int(eco.get("cash_silver", 0)) + spend
        priority = {"action_ref": action_ref, "at": at, "arc_ref": arc_ref, "target_ref": target_ref, "goal": goal[:500], "status": "queued", "coordination_spent_silver": spend, "payer_ref": treasury_ref, "payee_ref": f"private_economy_{native_state}", "basis": "exact sovereign treasury funded preparation only; a separate domain consequence is required before arc momentum can increase"}
        ...
        return {
            ...
            "preparation_evidence": {
                ...
                "payee_ref": f"private_economy_{native_state}",
                "silver_transferred": spend,
            },
        }
    ...
    def _world_arc_house_action(self, actor_ref: str, target_ref: str | None, goal: str, at: str, arc_ref: str) -> dict[str, Any]:
        ...
        action_ref = "house_arc_" + hashlib.sha256(f"{actor_ref}|{arc_ref}|{goal}|{target_ref or ''}".encode("utf-8")).hexdigest()[:18]
        spend = 0
        payer_ref = actor_ref
        treasury_ref = str(house.get("treasury_ref", ""))
        treasury_path = None
        ...
        history = house.setdefault("action_history", [])
        if not isinstance(history, list):
            raise ValueError("House action history is invalid")
        history.append(record); del history[:-64]
        queue.append(copy.deepcopy(record))
    ...
    def _world_arc_institution_action(self, actor_ref: str, target_ref: str | None, goal: str, at: str, arc_ref: str) -> dict[str, Any]:
        ...
        existing = next((
            ...
            and (str(row.get("target_ref")) if row.get("target_ref") is not None else "") == (str(target_ref) if target_ref is not None else "")
            and str(row.get("goal", "")) == goal[:500]
            and str(row.get("status", "")).startswith("queued")
        ), None)
        if existing is not None:
            existing["last_reaffirmed_at"] = at
    ...
    def _world_arc_domain_action(self, actor_ref: str, target_ref: str | None, goal: str, at: str, arc_ref: str) -> dict[str, Any]:
        ...
        """
        completed = self._world_arc_completed_priority(actor_ref, arc_ref)
        ...
        before_count = len(before.get("commitments", [])) if isinstance(before.get("commitments"), list) else 0
        ...
        commitments = after.get("commitments", []) if isinstance(after.get("commitments"), list) else []
        if len(commitments) <= before_count:
        ...
        commitment = copy.deepcopy(commitments[-1])
    ...
    def _settle_minor_polity(self, polity_ref: str, occurrences: int, at: str) -> None:
        """Monthly bounded close for an exact non-House minor polity.

        Population, force, treasury and economy remain separate exact owners. This
        close only realizes the polity's saved local economy and standing-defense
        upkeep; it never mints soldiers from readiness or capability metadata.
        """
        polity_path = self.owner_path(polity_ref)
        polity = copy.deepcopy(self.read(polity_path))
        ...
        if not population_ref or not treasury_ref or not economy_ref:
        ...
        treasury_path = self.owner_path(treasury_ref); treasury = copy.deepcopy(self.read(treasury_path))
        economy_path = self.owner_path(economy_ref); economy = copy.deepcopy(self.read(economy_path))
        months = max(1, int(occurrences))
        flow = polity.get("minor_polity_monthly_flow", {}) if isinstance(polity.get("minor_polity_monthly_flow"), Mapping) else {}
        revenue = max(0, int(flow.get("revenue_silver", 0))) * months
        expense = max(0, int(flow.get("expense_silver", 0))) * months
        economy_cash = max(0, int(economy.get("cash_silver", 0)))
        realized = min(revenue, economy_cash)
        economy["cash_silver"] = economy_cash - realized
        treasury["silver"] = max(0, int(treasury.get("silver", 0)) + realized - min(expense, int(treasury.get("silver", 0)) + realized))
        civilian = max(0, int(population.get("population_total", 0)) - int(population.get("strata", {}).get("active_military", 0)))
        grain_due = civilian * max(0, int(flow.get("food_kg_per_civilian_per_month", 8))) * months
        grain = max(0, int(economy.get("commodity_stock", {}).get("grain_kg", 0)))
        consumed = min(grain, grain_due)
        economy.setdefault("commodity_stock", {})["grain_kg"] = grain - consumed
        treasury.setdefault("runtime", {})["last_monthly_close_at"] = at
        economy.setdefault("production_runtime", {})["last_close"] = at
        polity.setdefault("civil_finance", {})["last_close"] = at
        polity["civil_finance"]["last_realized_revenue_silver"] = realized
        polity["civil_finance"]["last_expense_silver"] = expense
        polity["civil_finance"]["last_civilian_grain_consumed_kg"] = consumed
        polity["civil_finance"]["rule"] = "minor-polity close transfers only saved exact economy stock/cash; no readiness score creates people or goods"
        for force_ref in force_refs:
            try:
                force_path = self.owner_path(force_ref)
                force = copy.deepcopy(self.read(force_path))
            ...
            # Minor-polity troops are real conserved forces and must develop on
            # the same elapsed-time rules as state and House forces. Baseline
            # seeding is performed by _fc_train before verified hours accrue.
            self._fc_train(force, "regular_army", float(months), f"minor_polity:{polity_ref}:{at}")
            validate_cohort_ledger(force)
            self.put(force_path, force)
        self.put(treasury_path, treasury); self.put(economy_path, economy); self.put(polity_path, polity)
    ...
    def _autonomy_polity(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        the seven legacy state files.  It settles territorial finance/recruitment on
        ...
        polity.setdefault("state_integration", {}).update({
            ...
            "population_authority_rule": "local populations remain conserved in their physical regional owners; sovereignty does not duplicate civilians",
        })
    ...
    def _autonomy_rebel_faction(self, ref: str, path: str, doc: dict[str, Any], occurrences: int, at: str) -> None:
        ...
        doc.setdefault("commitments", []).append({"at": at, "action": "irregular_campaign", "operation_ref": operation_ref, "route_ref": route_ref, "recruited": recruited, "support": support, "basis": "saved insurgent manpower, local support, exact route topology, and conserved supplies"}); doc["commitments"] = doc["commitments"][-24:]
    ...
    def _autonomy_faction(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        if can_act:
            ...
            if action == "merchant_liquidity":
                if ref == "faction_central_merchant_houses":
                    effect = self._merchant_liquidity_loan(profile, at)
                else:
                ...
                effect = {"available_instruction_slots": resources["available_instruction_slots"], "rule": "slots are opportunity capacity only; no skill gain without exact training settlement"}
                ...
                effect = {"muster_readiness": resources["muster_readiness"], "rule": "readiness does not create troops or equipment"}
            ...
            commitments.append({"at": at, "action": action, "goal": (doc.get("goals") or ["preserve organization"])[0], "spent": dict(spent), "effect": effect, "knowledge_refs_used": [str(x) for x in knowledge_basis[-3:]], "basis": "saved faction resources, registered profile, pressure, exact knowledge where required, and exact linked owners"})
            doc["pressure"] = max(0, pressure - 25)
        if len(commitments) > 24:
            del commitments[:-24]
        self.put(path, doc)
    ...
    def _interstate_war_decision(
        ...
        relation = attacker_doc.get("diplomacy", {}).get(defender, {}) if isinstance(attacker_doc.get("diplomacy"), Mapping) else {}
    ...
    def _generate_npc_war_intent(self, sovereign_ref: str, at: str) -> dict[str, Any] | None:
        ...
        if active:
            self.put(path, doc)
            return None
        config = self._interstate_theater_config(self.read("game/data/world/autonomous-theaters.json"), at=at)
        territory = self.read("state/territory/control.json")
        ...
        for cfg in config.get("theaters", []):
            ...
            relation = doc.get("diplomacy", {}).get(target_ref, doc.get("diplomacy", {}).get(other_side, {})) if isinstance(doc.get("diplomacy"), Mapping) else {}
    ...
    def _interstate_theater_config(self, base: Mapping[str, Any], *, at: str | None = None) -> dict[str, Any]:
        ...
        Legacy authored state theaters remain authoritative where their current
        ...
        sites = territory.get("sites", {}) if isinstance(territory, Mapping) else {}
        routes = self.read("game/data/world/routes.json").get("routes", [])
        legacy_states = {"qin", "zhao", "chu", "wei", "han", "yan", "qi"}
        ...
        def controller_side(location_ref: str) -> str:
            ...
            return static if static in legacy_states else ""

        def ranked_formations(side: str) -> list[str]:
            force_refs: list[str] = []
            if side in legacy_states:
        ...
        sovereign_refs = [f"state_{s}" for s in sorted(legacy_states)]
        ...
        for sovereign_ref in sovereign_refs:
            ...
            for intent in sovereign.get("war_intents", []) if isinstance(sovereign, Mapping) else []:
                ...
                for formation_ref in own_forms:
                    ...
                    for target_loc in candidate_sites:
                        try:
                            _next, _hours = self._formation_route_next(origin, target_loc, formation=formation, at=now_text)
                            travel = self._route_travel_hours(origin, target_loc)
                        except (ValueError, PermissionError): continue
                        row = (travel, target_loc, formation_ref)
                        if best is None or row < best: best = row
                if best is None: continue
                _travel, target_loc, preferred = best; own_forms = [preferred] + [ref for ref in own_forms if ref != preferred]
                slug = lambda value: value.replace("polity_", "polity-").replace("_", "-")
    ...
    def _update_sovereign_diplomacy(self, a_ref: str, b_ref: str, *, status: str | None = None, tension_delta: int = 0, at: str, treaty_ref: str | None = None) -> None:
        for source_ref, target_ref in ((a_ref, b_ref), (b_ref, a_ref)):
            ...
            if treaty_ref:
                ...
                del refs[:-16]
            # Preserve compatibility for legacy state theater logic that keys peers by short state id.
            if target_ref.startswith("state_"):
                diplomacy[target_ref.removeprefix("state_")] = copy.deepcopy(row)
            self.put(sp, sd)
    ...
    def _validate_formation_transit(self, formation: Mapping[str, Any], destination_ref: str, at: str) -> None:
        ...
        relation = diplomacy.get(controller_ref, diplomacy.get(controller_ref.removeprefix("state_"), {})) if isinstance(diplomacy, Mapping) else {}
    ...
    def _activate_diplomatic_treaty(self, proposal: dict[str, Any], at: str) -> str:
        ...
        if kind == "alliance":
            if direction != "mutual": raise ValueError("alliance must be mutual")
            active_terms.update({"alliance": True, "mutual_defense": True}); status = "allied"
        elif kind == "nonaggression":
            if direction != "mutual": raise ValueError("nonaggression must be mutual")
            ...
            if direction != "mutual": raise ValueError("coalition must be mutual")
    ...
    def _evaluate_diplomatic_proposal(self, proposal: Mapping[str, Any], target_ref: str, target_doc: Mapping[str, Any], at: str) -> tuple[bool, dict[str, Any]]:
        ...
        relation = diplomacy.get(proposer_ref, diplomacy.get(proposer_ref.removeprefix("state_"), {})) if isinstance(diplomacy, Mapping) else {}
        tension = max(0, min(100, int(_fixed(relation.get("tension", 0), 0)))) if isinstance(relation, Mapping) else 0
    ...
    def _generate_npc_diplomatic_initiative(self, sovereign_ref: str, at: str) -> dict[str, Any] | None:
        ...
        for target_ref in sorted(candidate_refs):
            ...
            relation = doc.get("diplomacy", {}).get(target_ref, doc.get("diplomacy", {}).get(target_ref.removeprefix("state_"), {})) if isinstance(doc.get("diplomacy"), Mapping) else {}
            tension = max(0, min(100, int(_fixed(relation.get("tension", 0), 0)))) if isinstance(relation, Mapping) else 0
            status = str(relation.get("status", "")) if isinstance(relation, Mapping) else ""
            ...
            if tension <= 25 and own_threat >= 45 and status not in {"allied", "client_relation"}:
                choices.append((70 + own_threat * 0.2, target_ref, "alliance", "mutual", {"duration_days": 720}))
            if 25 <= tension <= 65 and status not in {"nonaggression", "allied"}:
                choices.append((55 + (65 - tension) * 0.2, target_ref, "nonaggression", "mutual", {"duration_days": 365}))
            if ratio >= 1.8 and target_threat >= 45 and tension <= 60 and status != "client_relation":
                choices.append((62 + min(25, (ratio - 1.8) * 10) + target_threat * 0.1, target_ref, "client_state", "target_to_proposer", {"duration_days": 1080}))
            if ratio >= 1.5 and tension <= 50 and target_threat >= 35:
                choices.append((58 + target_threat * 0.12, target_ref, "guarantee", "proposer_to_target", {"duration_days": 720}))
            if tension <= 30 and own_threat >= 55:
                choices.append((57 + own_threat * 0.1, target_ref, "military_access", "target_to_proposer", {"duration_days": 365}))
            ...
            if shared and tension <= 40:
                choices.append((72 + min(18, min(own_threat, target_threat) * 0.12), target_ref, "coalition", "mutual", {"duration_days": 720, "coalition_target_ref": shared[0]}))
        ...
        for proposal_ref in [str(x) for x in routes if isinstance(x, str)]:
            ...
            if sovereign_ref.startswith("polity_") and str(sovereign_doc.get("sovereign_house_ref", "")) == "house_tang":
                proposal["response_basis"] = {
                    "rule": "player sovereign decision required",
    ...
    def _dispatch_polity_action(self, command: Any, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if action == "set_strategic_goal":
            ...
            appointment = {"office": office_key, "polity_ref": polity_ref, "appointed_at": now, "grantor_ref": command.actor_id}; polity.setdefault("officeholders", {})[office_key] = {"person_ref": person_ref, **appointment}; person.setdefault("career_state", {}).setdefault("appointments", []).append({"person_ref": person_ref, **appointment}); person["career_state"]["appointments"] = person["career_state"]["appointments"][-32:]; self.put(person_path, person); result.update({"person_ref": person_ref, "office_key": office_key})
    ...
    def _dispatch_market(self, command: Any, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if t == "market_purchase":
            ...
            wallet["silver"] -= total; market["stock"][key] -= qty; regional["cash_silver"] = int(regional.get("cash_silver", 0)) + total; inv["items"][item_id] = int(inv["items"].get(item_id, 0)) + exact_qty
        ...
        if command.command_type == "battle_resolve" and isinstance(result, Mapping):
            ...
            if isinstance(material_losses, Mapping):
                ...
                for formation_ref, raw in material_losses.items():
                    ...
                    local_losses = self._reconcile_local_state_service_casualties(
                        force_ref, cohort_losses,
                        at=str(result.get("world_time", self._world_time())),

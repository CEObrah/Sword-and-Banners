...
class EnvironmentMechanicsMixin:
    ...
    def _formation_combat_snapshot(self, formation: Mapping[str, Any], force: Mapping[str, Any], **kwargs: Any) -> dict[str, Any]:
        ...
        if float(snapshot.get("named_equivalent", 0.0)) > 0:
            snapshot["named_equivalent"] = float(snapshot["named_equivalent"]) * max(0.90, int(effects["formation_mobility_milli"]) / 1000.0)

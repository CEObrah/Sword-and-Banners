...
from sword_runtime.terrain import terrain_context_for_location
...
def daylight_window(at: CampaignTime) -> tuple[CampaignTime, CampaignTime]:
    """Return the deterministic sunrise and sunset bounding ``at``'s civil day."""
    season = SEASON[at.month]
    sunrise_hour, sunset_hour = LIGHT[season]
    sunrise = CampaignTime(at.sort_year, at.month, at.day, sunrise_hour, 0, 0, at.offset)
    sunset = CampaignTime(at.sort_year, at.month, at.day, sunset_hour, 0, 0, at.offset)
    return sunrise, sunset


def is_daylight(at: CampaignTime) -> bool:
    sunrise, sunset = daylight_window(at)
    return sunrise <= at < sunset


def next_sunrise_after(at: CampaignTime) -> CampaignTime:
    sunrise, _sunset = daylight_window(at)
    if at < sunrise:
        return sunrise
    tomorrow = CampaignTime(at.sort_year, at.month, at.day, 0, 0, 0, at.offset).add_days(1)
    return daylight_window(tomorrow)[0]


def next_sunset_after(at: CampaignTime) -> CampaignTime:
    _sunrise, sunset = daylight_window(at)
    if at < sunset:
        return sunset
    tomorrow = CampaignTime(at.sort_year, at.month, at.day, 0, 0, 0, at.offset).add_days(1)
    return daylight_window(tomorrow)[1]


def next_daylight_transition(at: CampaignTime) -> tuple[CampaignTime, str]:
    """Return the next exact organized-battle daylight boundary after ``at``."""
    sunrise = next_sunrise_after(at)
    sunset = next_sunset_after(at)
    if sunrise < sunset:
        return sunrise, "dawn"
    return sunset, "dusk"
...
def environment_snapshot(reader: Any, *, world_time: str, location_ref: str) -> dict[str, Any]:
    ...
    # Keep the weather-only layer separate. Strategic route geometry already
    # includes canonical static terrain; callers that adjust a route must add
    # weather only or terrain would be charged twice.
    weather_effects = deepcopy(effects)
    terrain = terrain_context_for_location(reader, location_ref)
    te = terrain["mechanical_effects"]
    for key in ("travel_time_milli", "visibility_milli", "ranged_effectiveness_milli", "formation_mobility_milli", "mounted_mobility_milli"):
        effects[key] = max(250, min(2000, int(effects[key]) * int(te[key]) // 1000))
    effects["chariot_mobility_milli"] = int(te["chariot_mobility_milli"])
    effects["terrain_frontage_milli"] = int(te["frontage_milli"])
    effects["terrain_concealment_milli"] = int(te["concealment_milli"])
    effects["terrain_scouting_milli"] = int(te["scouting_milli"])
    effects["terrain_pursuit_milli"] = int(te["pursuit_milli"])
    effects["terrain_fatigue_milli"] = int(te["fatigue_milli"])
    effects["terrain_defense_milli"] = int(te["defense_milli"])
    visibility = "good" if effects["visibility_milli"] >= 900 else "reduced" if effects["visibility_milli"] >= 725 else "poor" if effects["visibility_milli"] >= 575 else "severe"
    ...
    return {
        ...
        "terrain": {"tags": list(terrain["tags"]), "encoded": terrain["encoded"]},
        "weather_mechanical_effects": weather_effects,
        "mechanical_effects": effects,
        "scope": "Derived from campaign time + world seed + static climate + canonical layered terrain. Mutable hazards and institutional schedules remain owned elsewhere.",
    }
...
def route_travel_factor_milli(reader: Any, *, world_time: str, origin_ref: str, destination_ref: str, base_hours: int) -> int:
    ...
    return max(850, min(1400, (int(start["weather_mechanical_effects"]["travel_time_milli"]) + int(end["weather_mechanical_effects"]["travel_time_milli"]) + 1) // 2))
...
class EnvironmentMechanicsMixin:
    ...
    def _formation_combat_snapshot(self, formation: Mapping[str, Any], force: Mapping[str, Any], **kwargs: Any) -> dict[str, Any]:
        ...
        if not isinstance(location, str) or not location:
            return snapshot
        env = self._environment_snapshot(location); effects = env["mechanical_effects"]; weather_effects = env["weather_mechanical_effects"]
        # Static terrain frontage is already applied by the base combat
        # capability layer. Apply only weather mobility here to avoid charging
        # hills/woods/marsh twice. Other combat channels below still use the
        # combined terrain+weather effect because the base snapshot does not.
        snapshot["frontage_equivalent"] = max(1.0, float(snapshot.get("frontage_equivalent", 1.0)) * int(weather_effects["formation_mobility_milli"]) / 1000.0)
...
__all__ = ["CLIMATE_PATH", "EnvironmentMechanicsMixin", "daylight_window", "environment_snapshot", "is_daylight", "next_daylight_transition", "next_sunrise_after", "next_sunset_after", "route_travel_factor_milli"]

...
def build_campaign_dossier(planner: Any, operation_ref: str) -> dict[str, Any]:
    ...
    same_arc = _same_arc_operations(planner, arc_ref, locations) if arc_ref else [own]
    ...
    for row in same_arc:
        try:
            ...
            continue
        if explicit_commander is None and isinstance(op.get("campaign_commander_ref"), str) and op.get("campaign_commander_ref"):
            explicit_commander = str(op["campaign_commander_ref"])
...
def render_campaign_briefing(planner: Any, dossier: Mapping[str, Any], mission_packet: Mapping[str, Any] | None = None) -> str:
    ...
    entry_text = (
        f" Immediate lawful staging is {area_text}; hostile entry is not yet authorized, so {strategic_target_text} remains the strategic target rather than the current march destination."
        if area and not bool(area.get("hostile_entry_authorized"))
        else f" Hostile entry authority is established; {strategic_target_text} is an executable theater destination."
    )
    ...
    ownership_text = (
        f" Of Tang Wei's present field army, {assigned_strength:,} are institutionally assigned troops and {auxiliary_strength:,} are lawful accompanying House/retinue auxiliaries whose ownership does not transfer."
    ...
    if isinstance(mission_packet, Mapping):
        ...
        packet_text = (
            f" Executable staff packet: phase {mission_packet.get('mission_phase')}; concentrate/report at {rendezvous}, then operate toward {dest} when the commander executes the order. "
            f"Success condition: {mission_packet.get('success_condition')}. {mission_packet.get('next_phase_trigger')}"
        )
    return (
        f"Qin Military Bureau operational briefing. Campaign objective: {dossier.get('objective') or 'not established'}. "
        f"Tang Wei's present field command is {int(own.get('strength', 0)):,} troops in {int(own.get('formation_count', 0))} formation(s), currently at {', '.join(own_locs) or 'an unestablished location'}.{ownership_text} "
        f"Other Qin forces formally tied to this campaign: {'; '.join(friendly_parts) if friendly_parts else 'none established in current operation owners'}. "
        f"Combined Qin strength represented by those current operation owners is {int(dossier.get('friendly_total_strength', 0) or 0):,}. "
        f"Current operational area selected by staff planning is {area_text}.{entry_text} Enemy picture: {enemy_strength_text}; reported opposing commanders: {enemy_commanders}; {enemy.get('contact_status', 'contact status uncertain')}. "
        f"These enemy figures are command-intelligence estimates, not omniscient exact truth.{packet_text}"
    )[:4000]
...
def ensure_actionable_mission_packet(planner: Any, operation_ref: str, dossier: Mapping[str, Any], *, at: str) -> dict[str, Any] | None:
    ...
    if str(order.get("actionability_status", "")) == "actionable" and isinstance(existing_packet, Mapping):
        current_authorized = bool(area.get("hostile_entry_authorized")) if isinstance(area, Mapping) else False
        packet_authorized = bool(existing_packet.get("hostile_entry_authorized"))
        current_destination = area.get("destination_ref") if isinstance(area, Mapping) else None
        current_friendlies = [
            str(x.get("operation_ref"))
            for x in dossier.get("other_friendly_participants", [])
            if isinstance(x, Mapping) and x.get("operation_ref")
        ]
        enemy_now = dossier.get("enemy_intelligence") if isinstance(dossier.get("enemy_intelligence"), Mapping) else {}
        ...
        packet_is_current = (
            ...
            and current_destination == existing_packet.get("destination_ref")
            and current_friendlies == [str(x) for x in existing_packet.get("friendly_participant_operation_refs", []) if isinstance(x, str)]
            and int(enemy_now.get("estimated_strength_low", 0) or 0) == int(enemy_then.get("strength_low", 0) or 0)
            and int(enemy_now.get("estimated_strength_high", 0) or 0) == int(enemy_then.get("strength_high", 0) or 0)
            and int(enemy_now.get("confidence_milli", 0) or 0) == int(enemy_then.get("confidence_milli", 0) or 0)
            and enemy_now.get("contact_status") == enemy_then.get("contact_status")
        )
    ...
    if not isinstance(destination_ref, str) or not destination_ref or not rendezvous_ref:
        return None
    locations = _location_rows(planner)
    enemy = dossier.get("enemy_intelligence") if isinstance(dossier.get("enemy_intelligence"), Mapping) else {}
    ...
    packet = {
        ...
        "friendly_participant_operation_refs": [str(x.get("operation_ref")) for x in dossier.get("other_friendly_participants", []) if isinstance(x, Mapping) and x.get("operation_ref")],
        "enemy_estimate": {
        ...
        "agency_rule": "This packet establishes Qin's immediate lawful destination and coordination requirement but does not move Tang Wei, commandeer privately owned auxiliaries, commit him to battle, or choose his tactics.",
    }

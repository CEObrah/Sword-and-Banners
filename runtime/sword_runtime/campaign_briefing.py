...
from sword_runtime.operation_routing import current_operational_order, current_operational_order_slot, exact_operation_record, iter_exact_operation_records
...
def _routing_document(planner: Any, path: str) -> Any:
    """Reuse planner-local non-authoritative routing/static documents when supported."""
    fn = getattr(planner, "routing_document", None)
    return fn(path) if callable(fn) else planner.read(path)
...
def _latest_order(operation: Mapping[str, Any]) -> dict[str, Any]:
    row = current_operational_order(operation)
...
def _location_rows(planner: Any) -> dict[str, Mapping[str, Any]]:
    doc = _routing_document(planner, _LOCATIONS)
    return {
        str(row.get("ref")): row
...
def _command_group_for_formation(planner: Any, formation_ref: str) -> tuple[str | None, Mapping[str, Any] | None]:
    try:
        index = _routing_document(planner, _COMMAND_GROUP_INDEX)
...
def _same_arc_operations(planner: Any, arc_ref: str, locations: Mapping[str, Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for operation_ref, _path, raw in iter_exact_operation_records(planner):
        if str(raw.get("status", "")) not in _ACTIVE_OPERATION_STATUSES:
            continue
...
def _operational_area(
    ...
    locations: Mapping[str, Mapping[str, Any]],
    *,
    operation_location_ref: str | None = None,
) -> dict[str, Any] | None:
    ...
    targets = [
        ...
        and bool(row.get("strategic_node"))
        and not bool(row.get("flavor_only"))
        and str(row.get("kind", "")) != "region"
    ]
    if not targets:
        targets = [
            ref for ref, row in locations.items()
    ...
    if not targets:
        return None
    strategic_target = ""
    explicit = locations.get(str(operation_location_ref or ""))
    if (
        isinstance(explicit, Mapping)
        and str(explicit.get("state", "")) == target_key
        and bool(explicit.get("strategic_node"))
        and not bool(explicit.get("flavor_only"))
        and str(explicit.get("kind", "")) != "region"
    ):
        try:
            shortest_path(planner.read, origin, str(operation_location_ref), modes=("formation", "foot", "horse"))
            strategic_target = str(operation_location_ref)
        except ValueError:
            strategic_target = ""
...
def _confirmed_hostile_contacts(
    ...
    *,
    ...
    if not target_state_ref or not location_ref:
        return []
    try:
        owner_index = _routing_document(planner, "state/index/owner-index.json")
...
def build_campaign_dossier(planner: Any, operation_ref: str) -> dict[str, Any]:
    ...
    area = _operational_area(
        planner, origin, friendly_state, target_state, locations,
        operation_location_ref=str(operation.get("location_ref") or "") or None,
    )
    opposing_refs = {str(ref) for ref in operation.get("opposing_formation_refs", []) if isinstance(ref, str) and ref}
    ...
    return {
        ...
        "enemy_intelligence": {
            ...
            "reported_field_body_count": len(enemy_forms),
            "reported_field_body_semantics": "Distinct observed/reported aggregate field bodies; their internal subformation count is not established by this reconnaissance.",
            ...
            "confirmed_contact": {
                ...
                "field_body_count": len(confirmed_contacts),
...
def safe_campaign_context(dossier: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for row in dossier.get("other_friendly_participants", []) if isinstance(dossier.get("other_friendly_participants"), list) else []:
        ...
        friendlies.append({
            ...
            "echelon": copy.deepcopy(row.get("echelon")),
    ...
    return {
        ...
        "own_echelon": copy.deepcopy(own.get("echelon")) if isinstance(own, Mapping) else None,
        ...
        "enemy_intelligence": {
            ...
            "reported_formation_count": enemy.get("reported_formation_count"), "reported_field_body_count": enemy.get("reported_field_body_count"),
            "reported_field_body_semantics": enemy.get("reported_field_body_semantics"), "reported_commanders": copy.deepcopy(enemy.get("reported_commanders", [])),
...
def render_campaign_briefing(planner: Any, dossier: Mapping[str, Any], mission_packet: Mapping[str, Any] | None = None) -> str:
    ...
    for row in dossier.get("other_friendly_participants", []) if isinstance(dossier.get("other_friendly_participants"), list) else []:
        ...
        echelon = row.get("echelon") if isinstance(row.get("echelon"), Mapping) else {}
        primary_count = int(echelon.get("primary_command_count", 0) or 0)
        representation = str(echelon.get("representation") or "")
        unit_text = (
            f"organized under {primary_count} primary command(s)"
            if representation == "primary_commands" and primary_count
            else f"represented by {primary_count} aggregate field body/bodies"
            if primary_count
            else "with command echelon not established in this briefing"
        )
        friendly_parts.append(f"{int(row.get('strength', 0)):,} troops {unit_text}, under {commander_text}, at {loc_text}, task: {str(row.get('objective') or 'campaign support')}")
    enemy = dossier.get("enemy_intelligence") if isinstance(dossier.get("enemy_intelligence"), Mapping) else {}
    ...
    return (
        f"Qin Military Bureau briefing. Tang Wei's field command numbers {int(own.get('strength', 0)):,} troops organized under {int((own.get('echelon') or {}).get('primary_command_count', 0) or 0)} primary command(s), currently at {', '.join(own_locs) or 'an unestablished location'}.{ownership_text} "
        ...
        f"The staff designate {area_text} as the present operational area.{entry_text} Intelligence estimate: {enemy_strength_text}; {int(enemy.get('reported_field_body_count', enemy.get('reported_formation_count', 0)) or 0)} distinct opposing field body/bodies reported; opposing commanders: {enemy_commanders}; {enemy.get('contact_status', 'contact status uncertain')}. "
        f"These are staff estimates. A reported enemy field body is not assumed to have the same internal echelon structure as one of Tang Wei's tactical leaf formations.{order_text}"
    )[:4000]
...
def latest_campaign_briefing_ref(store: Any, operation_ref: str, arc_ref: str | None) -> str | None:
    try:
        index = store.read_json(_INFO_INDEX)
        subject_index = store.read_json(_INFO_SUBJECT_INDEX)
    except (FileNotFoundError, KeyError):
        try:
            index = store.read_json(_INFO_INDEX)
        except (FileNotFoundError, KeyError):
            return None
        subject_index = {"schema": "sword-information-subject-index", "authority": False, "subjects": {}}
    ...
    for subject in (operation_ref, arc_ref):
        if not isinstance(subject, str) or not subject:
            continue
        for ref in information_claim_refs_for_subject(
            lambda path: store.read_json(path), index, subject_index, subject,
        ):

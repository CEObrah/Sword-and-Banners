...
def _leadership_score(
    ...
    deputy: Mapping[str, Any] | None,
    weights: Mapping[str, Any],
) -> float:
    def score(person: Mapping[str, Any] | None) -> float:
        return sum(_person_skill(person, str(key)) * _number(weight) for key, weight in weights.items())
    # Commander owns the duty. Deputy quality matters materially for continuity and internal delegation.
    return 0.75 * score(commander) + 0.25 * score(deputy)
...
def unit_duty_suitability(
    ...
    leadership = _leadership_score(commander, deputy, skill_weights)
...
def assign_phase_duties(
    *,
    ...
    for ref, formation in units:
        ...
        deputy_ref = str(formation.get("deputy_ref", ""))
        for duty_id in duty_ids:
            score_map[(ref, duty_id)] = unit_duty_suitability(
                formation, people_by_ref.get(commander_ref), people_by_ref.get(deputy_ref),
                duties[duty_id], role_tags,
            )

...
class StrategicCrossingStateMixin:
    ...
    def _mutate_crossing(self, payload: Mapping[str, Any], at: str) -> dict[str, Any]:
        ...
        row.setdefault("change_history", []).append({"at": at, "action": action, "amount": payload.get("amount"), "quantity": payload.get("quantity"), "water_stage": payload.get("water_stage")})
        row["change_history"] = row["change_history"][-24:]

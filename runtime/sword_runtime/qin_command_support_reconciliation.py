...
link predates the appointment ``operation_ref`` field. Older one-shot Qin briefing
routes may also survive in the scheduler registry after exhaustion with no next
due time, or may never have been registered when legacy order history was stored
out of issuance order. A legacy briefing may also have written its deterministic
...
This module normalizes legacy routing facts and catches recovered briefing routes
up to the current causal frontier. It deliberately does **not** infer current-order
identity from issuance chronology. ``last_operational_order_ref`` is explicit
lifecycle authority and may only move at the real briefing/delivery boundary.
Historical repair of a bad pointer belongs in an explicit migration, not a
recurring gameplay reconciliation hook.
"""
...
def reconcile_legacy_qin_command_support_state(planner: Any) -> list[str]:
    """Normalize legacy Qin appointment routing without inferring current-order identity."""

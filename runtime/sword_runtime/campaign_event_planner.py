...
class CampaignEventPlayerGroupActionPlanner(PlayerGroupActionPlanner):
    ...
    @classmethod
    def _clear_one_shot_campaign_event_route(
        cls, runtime: dict[str, Any], wake: Mapping[str, Any] | None
    ) -> None:
        """Remove an inert one-shot scheduler route after its player handoff is served."""
        if not cls._is_campaign_event_wake(wake):
            return
        host_id = wake.get("target_host") if isinstance(wake, Mapping) else None
        event_id = wake.get("event_id") if isinstance(wake, Mapping) else None
        hosts = runtime.get("hosts")
        host = hosts.get(host_id) if isinstance(hosts, dict) and isinstance(host_id, str) else None
        if not isinstance(host, Mapping) or host.get("next_due") is not None:
            return
        # These hosts have already committed their durable result elsewhere. The
        # scheduler route is transport, not history, and should not accumulate.
        if host.get("kind") not in {"world_arc_report", "commission"}:
            return
    ...
    def _clear_completed_campaign_event_ack(cls, runtime: dict[str, Any]) -> None:
        ...
        if cls._is_campaign_event_wake(acknowledged):
            cls._clear_one_shot_campaign_event_route(runtime, acknowledged)
    ...
    def _resume_pending_wake(self, runtime: dict[str, Any]) -> dict[str, Any] | None:
        ...
        self._clear_one_shot_campaign_event_route(runtime, wake)
        self._clear_completed_campaign_event_ack(runtime)
        return None
    ...
    def _dispatch(self, command: Any, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if self._is_campaign_event_wake(pending) and command.command_type != "advance_time":
            ...
            self._clear_one_shot_campaign_event_route(updated, pending)

...
class CampaignEventPlayerGroupActionPlanner(PlayerGroupActionPlanner):
    ...
    def _clear_completed_campaign_event_ack(cls, runtime: dict[str, Any]) -> None:
        ...
        if cls._is_campaign_event_wake(acknowledged):
            host_id = acknowledged.get("target_host") if isinstance(acknowledged, Mapping) else None
            event_id = acknowledged.get("event_id") if isinstance(acknowledged, Mapping) else None
            host = runtime.get("hosts", {}).get(host_id) if isinstance(runtime.get("hosts"), dict) and isinstance(host_id, str) else None
            if isinstance(host, Mapping) and host.get("kind") == "world_arc_report" and host.get("next_due") is None:
                runtime["hosts"].pop(host_id, None)

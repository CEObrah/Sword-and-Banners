...
class MilitaryCareerLoyaltyPoliticsMixin(MilitaryCareerLoyaltyIntegrityMixin):
    ...
    def _career_concentration_event(
        ...
        *,
        ...
        _event_owner_write(self, event_ref, payload, at, source_owner_ref=f"inst_{state_name}_military_bureau")
    ...
    def _transfer_index(self) -> dict[str, Any]:
        ...
        if raw is None:
            return {
                "schema": "sword-military-personnel-transfer-index.v1",
                "authority": False,
                "orders": {},
                "active_by_state": {},
        ...
        if not isinstance(raw, Mapping) or raw.get("schema") != "sword-military-personnel-transfer-index.v1":
    ...
    def _set_force_allocation(force: MutableMapping[str, Any], formation_ref: str, delta: int, role: str) -> None:
        ...
        if isinstance(row, MutableMapping):
            row["personnel"] = max(0, int(row.get("personnel", 0)) + int(delta))
            if not row.get("role"):
                row["role"] = role
        elif row is not None:
            allocations[formation_ref] = max(0, int(row) + int(delta))
        else:
    ...
    def _detach_person_from_formation(
        ...
        *,
        ...
        if formation.get("deputy_ref") == person_ref:
    ...
    def _create_transfer_order(
        ...
        *,
        ...
        order = {
            "schema": "sword-military-personnel-transfer.v1",
            "owner_id": order_ref,
            "order_ref": order_ref,
            ...
            "travel_hours": hours,
            "status": "in_transit",
            "authority_rule": "state/House approval authorizes movement only; conserved force and formation owners remain authoritative throughout",
        }
    ...
    def _settle_transfer_order(self, host: Mapping[str, Any], at: str) -> None:
        ...
        if request_kind == "campaign_attachment":
            ...
            if isinstance(operations, Mapping):
                for candidate_ref, candidate_path in sorted(operations.items()):
                    if not isinstance(candidate_path, str):
                        continue
                    operation = self.read(candidate_path)
        ...
        if order.get("desired_commander_ref") == _PLAYER_REF:
            ...
            _event_owner_write(self, event_ref, {
                "event_ref": event_ref,
                "kind": "military_personnel_arrival",
                "status": "triggered",
                "due_at": at,
                "triggered_at": at,
                "actor_ref": f"inst_{str(order['state_ref']).removeprefix('state_')}_military_bureau",
                "target_ref": _PLAYER_REF,
                "process_kind": "military_career_petition",
                "process_stage": "officer_arrived",
                "source_event_ref": order.get("petition_ref"),
                "summary": summary,
                "delivery": _player_delivery(self, "military personnel arrival report"),
            }, at, source_owner_ref=f"inst_{str(order['state_ref']).removeprefix('state_')}_military_bureau")
            self._pending_wake_created = {
                "wake_ref": f"wake.military_personnel.{_digest([event_ref, at])}",
                "kind": "campaign_event",
                "at": at,
    ...
    def _settle_temporary_attachments(self, state_ref: str, at: str) -> None:
        ...
        for source_order_ref in rows:
            ...
            if isinstance(operation_ref, str):
                ...
                operation = self.read_optional(op_path) if isinstance(op_path, str) else None
        ...
        *,
        ...
        person = {
            ...
            "military_career_state": {
                "schema": "sword-military-career-state.v1",
        ...
        if state_ref:
            ...
            if alignment_ref.startswith("char_"):
    ...
    def _immediate_officer_support(self, formation_ref: str, proposed_ref: str) -> tuple[int, dict[str, dict[str, Any]]]:
        ...
        for person_ref in self._formation_named_officer_refs(formation):
            ...
            role_weight = 4 if formation.get("commander_ref") == person_ref else (3 if formation.get("deputy_ref") == person_ref else 1)
    ...
    def _fragment_formation(
        ...
        *,
        ...
        self._set_force_allocation(force, formation_ref, -anonymous_follow, next(iter(original.get("composition", {})), "line_infantry"))
        self._set_force_allocation(force, child_ref, anonymous_follow, next(iter(child_comp), next(iter(original.get("composition", {})), "line_infantry")))
        ...
        original_deputy = original.get("deputy_ref")
        if isinstance(original_deputy, str) and decisions.get(original_deputy, {}).get("decision") == "follow":
            parent["deputy_ref"] = None
            child["deputy_ref"] = original_deputy
    ...
    def _resolve_military_allegiance_action(self, command: CommandEnvelope, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        crisis_ref = f"military_allegiance_crisis_{_digest([command.request_id, action, proposed_ref, formation_refs, at])}"
    ...
    def _run_due_host(self, host: Mapping[str, Any], due_text: str) -> None:
        if host.get("kind") == "military_personnel_transfer":
            self._pending_wake_created = None
            self._settle_transfer_order(host, due_text)
            return

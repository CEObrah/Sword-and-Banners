...
def normalize_formation_establishment(formation: dict[str, Any]) -> dict[str, Any]:
    """Persist deterministic Unit/detachment establishment without changing manpower.

    Current personnel is the surviving fighting strength. ``authorized_strength``
    is the durable establishment. Casualties may reduce the former but never
    rewrite the latter. Existing per-scale representation/loadout metadata is
    preserved while the standard subordinate command counts are regenerated from
    ...
    if saved_est and sum(max(0, int(v)) for v in saved_est.values()) == authorized:
        establishment = {str(k): max(0, int(v)) for k, v in saved_est.items() if max(0, int(v)) > 0}
    else:
        basis = current_comp or saved_est
        establishment = establishment_composition(basis, authorized) if basis else {}
    formation["establishment_composition"] = establishment

    structure = formation.setdefault("command_structure", {})
    if not isinstance(structure, dict):
        structure = {}
        formation["command_structure"] = structure
    existing = _existing_hierarchy_rows(structure.get("internal_hierarchy"))
    metadata: dict[int, dict[str, Any]] = {}
    for row in existing:
        scale = max(0, int(row.get("scale", 0) or 0))
        if scale <= 0:
            continue
        metadata.setdefault(scale, {})
        for key in ("representation", "loadout_ref", "deputy_policy", "inside_fighting_establishment"):
            if key in row:
                metadata[scale][key] = row[key]
    rows = hierarchy_rows(
        authorized_strength=authorized, current_personnel=current, formation_class=klass,
        representation_by_scale={scale: str(meta.get("representation", "aggregate")) for scale, meta in metadata.items()},
    )
    for row in rows:
        meta = metadata.get(int(row.get("scale", 0)), {})
        for key, value in meta.items():
            if key != "representation":
                row[key] = value
    structure["internal_hierarchy"] = rows
    structure["establishment_topology"] = hierarchy_topology(authorized_strength=authorized, formation_class=klass)
    structure["establishment_rule"] = (
        "Unit commander/deputy occupy the Unit echelon outside fighting strength; internal 1,000/500/100 billets are generated "
        "only from authorized establishment and never from casualty-depleted current manpower."
    )
    return {

...
_SWORD_PROGRESSION = "game/data/mil/sword-manor-progression.json"
...
def _house_digest_event(planner: Any, at: str) -> str | None:
    sword=planner.read(_SWORD_FORCE); progression=planner.read(_SWORD_PROGRESSION); infrastructure=planner.read(_INFRASTRUCTURE_PATH)
    ...
    counts={role:conserved_establishment_role_count(sword,role) for role in roles}; caps={role:max(0,int(authorized.get(role,0))) for role in roles}
    site=(infrastructure.get("sites",{}) or {}).get("loc_sword_manor",{}) if isinstance(infrastructure,Mapping) else {}
    military=site.get("military_support",{}) if isinstance(site,Mapping) and isinstance(site.get("military_support"),Mapping) else {}
    intake_cfg=site.get("intake_support",{}) if isinstance(site,Mapping) and isinstance(site.get("intake_support"),Mapping) else {}
    pr=progression.get("runtime",{}) if isinstance(progression,Mapping) and isinstance(progression.get("runtime"),Mapping) else {}
    signature={"counts":counts,"caps":caps,"reviews":int(pr.get("completed_monthly_reviews",0)),"housing":int(military.get("permanent_bed_capacity_people",0)),"intake":int(intake_cfg.get("induction_equipment_issue_capacity_per_30d",0))}
    event_ref="event_story_house_digest_"+_story_digest(signature)
    if isinstance(get_causal_event(planner,event_ref),Mapping): return None
    ...
    bottleneck="Current institutional vacancies: "+(", ".join(vacancies) if vacancies else "none")+"."
    housing_open=max(0,signature["housing"]-counts["trainee"])
    summary=(f"Tang Ling sends Tang Wei the current House development ledger. Sword Manor has completed {signature['reviews']} monthly development closes. "
             f"Current conserved establishments: {role_text}. Trainee housing is {signature['housing']} and normal monthly intake capacity is {signature['intake']}. {bottleneck} "
             f"Housing currently permits {housing_open} additional trainees before construction or other capacity changes. "
             "This status report records current capacity only; recruitment and promotion require the normal conserved mechanics.")

...
def _house_digest_event(planner: Any, at: str) -> str | None:
    sword=planner.read(_SWORD_FORCE); manor=planner.read(_MANOR_POPULATION); progression=planner.read(_SWORD_PROGRESSION)
    ...
    signature={"counts":counts,"caps":caps,"reviews":int(pr.get("completed_monthly_reviews",0)),"housing":int(sword_cfg.get("trainee_housing_capacity",0)),"intake":int(sword_cfg.get("monthly_intake_capacity",0))}

...
def _expire_pending_career_offers(planner: Any, at: str) -> list[str]:
    ...
    for raw_ref in pending:
        ...
        _event_owner_write(planner, event_ref, {
            "event_ref": event_ref,
...
def _career_offer(planner: Any, at: str) -> str | None:
    ...
    offers[offer_ref] = {**copy.deepcopy(dict(vacancy)), "offered_at": at, "expires_at": expires_at}
    planner.put(_PLAYER_PATH, player)
...
def _house_digest_event(planner: Any, at: str) -> str | None:
    ...
    summary = (f"Tang Ling sends Tang Wei the current House military ledger. The House force has completed {closes} monthly training cycle(s). "
...
def settle_player_story_review(planner: Any, host: Mapping[str, Any], at: str) -> dict[str, Any] | None:
    ...
    created.extend(lapsed_offers)
    builders = [_house_digest_event, _family_invitation_event]
    # Do not close an ignored offer and immediately replace it with another sealed
    # career offer in the same beat. The institution may reconsider next review.

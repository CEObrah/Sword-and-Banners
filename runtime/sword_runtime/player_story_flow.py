...
def _house_digest_event(planner: Any, at: str) -> str | None:
    ...
    counts={role:role_count(sword,role) for role in roles}; caps={role:max(0,int(authorized.get(role,0))) for role in roles}
    ...
    summary=(f"Tang Ling sends Tang Wei the current House development ledger. Sword Manor has completed {signature['reviews']} monthly development closes. "
             f"Current establishments: {role_text}. Trainee housing is {signature['housing']} and normal monthly intake capacity is {signature['intake']}. {bottleneck} "
             "This status report does not create recruits, promotions, or equipment by narration.")

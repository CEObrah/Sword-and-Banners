...
def _legacy_route_path(event_ref: str) -> str:
    """Compatibility path for v2 archives that used two-hex route shards."""
    return _route_path(_route_prefix(event_ref, 2))
...
def get_causal_event_from_reader(reader: Any, event_ref: str) -> Mapping[str, Any] | None:
    ...
    if not isinstance(segment_path, str) or not segment_path:
        legacy = _read_optional(reader, _legacy_route_path(event_ref))
        legacy_routes = legacy.get("routes", {}) if isinstance(legacy, Mapping) else {}
        segment_path = legacy_routes.get(event_ref) if isinstance(legacy_routes, Mapping) else None
    if not isinstance(segment_path, str) or not segment_path:
        return None
...
def causal_event_kind_count_from_reader(reader: Any, kinds: set[str] | frozenset[str]) -> int:
    ...
    if isinstance(archived, Mapping):
        ...
        # Legacy v2 owners did not maintain kind counts. Fall back to an exact
        # bounded archive walk only for those older saves.
        total = sum(1 for _ref, _event in iter_causal_events_newest(reader, kinds=kinds))
    return total

...
class PopulationMobilityMixin:
    ...
    def _settle_population_mobility_arrival(self, host: Mapping[str, Any], at: str) -> None:
        ...
        if isinstance(explicit_ref, str) and explicit_ref:
            ...
            # Arrival settles the conserved obligation completely. The exact source
            # and destination population owners now contain the result, so hot
            # mobility state keeps only the current processing frontier.

...
def _sync_compact_player_state(player: dict[str, Any], manifest: Mapping[str, Any]) -> None:
    ...
    # Preparing a horse and fitting tack/barding does not teleport the animal
    # into an interior room. Literal mounting remains distinct from stable prep.
    compact["mounted"] = any(
...
class EquipmentStateProjectionMixin:
    """Post-process player equipment writes without creating a second authority.

    The engine remains the custody/inventory authority. This mixin only derives
    the compact player projection from the exact manifest and gives mount gear a
    physically meaningful prepared/fitted state instead of "on person".
    """
...
__all__ = ["EquipmentAwareCampaignOperations", "EquipmentStateProjectionMixin"]

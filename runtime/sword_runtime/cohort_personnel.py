    ...
    for allocation_key in ("allocated_to_formations", "external_personnel_allocations"):
        ...
        for value in allocations.values():
            ...
            if allocation_key == "external_personnel_allocations":
                # External formation slots are already conserved Sword Manor bodies,
    ...
    if minimum_attribute_values:
        for name, threshold in zip(ATTRIBUTE_ORDER, minimum_attribute_values):
            if name not in attrs:
                return 0
            fractions.append(_tail_fraction(float(attrs[name]), cohort_spread_value(cohort, "attribute", name, 8.0), float(threshold)))
    skills = cohort.get("skill_means", {})
    if minimum_skill_values:
        for name, threshold in zip(SKILL_ORDER, minimum_skill_values):
            if name not in skills:
                return 0
            fractions.append(_tail_fraction(float(skills[name]), cohort_spread_value(cohort, "skill", name, 10.0), float(threshold)))
    if not fractions:
        return count

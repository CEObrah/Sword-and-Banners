...
def ensure_cohort_ledger(force: MutableMapping[str, Any], *, at: str | None = None) -> dict[str, Any]:
    ...
    Release-baseline manpower is deliberately marked ``release_baseline`` with unknown
    ...
    if isinstance(by_loc, Mapping) and by_loc:
        for location_ref in sorted(by_loc):
            ...
            for role in sorted(pool):
                ...
                if not count:
                    continue
                cid = _cohort_id(force, "baseline_reserve", location_ref, role)
        ...
        for role in sorted(force.get("available_by_role", {})):
            ...
            if count:
                cid = _cohort_id(force, "baseline_reserve", source_loc, role)
    ...
    for formation_ref, allocation in sorted(force.get("allocated_to_formations", {}).items()):
        ...
        cid = _cohort_id(force, "baseline_allocation", formation_ref, role)
...
def _baseline_cohort(cid: str, role: str, reserve: int, location_ref: str | None, at: str | None) -> dict[str, Any]:
    return {
        ...
        "origin": {
            ...
            "recruited_at": None,
            "baseline_recorded_at": at,
            "provenance_note": "release-baseline personnel preserve capability without fabricating earlier recruitment ancestry",
        },
        ...
        "training_history": [],
        "tags": ["release_baseline", "quality_not_reconstructed"],
    }
...
def validate_cohort_ledger(force: Mapping[str, Any]) -> None:
    ...
    if isinstance(assignments, Mapping):
        for assignment in assignments.values():
            ...
            if formation_ref:
                allocations[formation_ref] = allocations.get(formation_ref, 0) + max(1, int(assignment.get("personnel", 1)))
...
def ensure_formation_composition(
    ...
    *,
    ...
    """Attach a release-baseline allocation to its deterministic conserved cohort."""
...
def partition_formation_slices(
    ...
    for item in raw:
        ...
        exact = count * target / max(1, anonymous_total)
...
def trim_formation_to_personnel(
    ...
    *,
    ...
    formation["cohort_composition"]=sorted(survivors,key=lambda x:x["cohort_id"]); return loss_map
...
def record_formation_combat_experience(
    ...
    *,
    ...
    for item in formation.get("cohort_composition", []):
        ...
        skills = focus.get("skills", []) if isinstance(focus, Mapping) else []
...
def seed_cohort_capability(
    ...
    *,
    ...
    if attribute_means:
        ...
        cohort["attribute_max"] = {str(k): min(200.0, float(v) + 2.5 * float(sds.get(str(k), 8.0))) for k, v in attribute_means.items()}
    if skill_means:
        sds = cohort.get("skill_sd", {})
        cohort["skill_min"] = {str(k): max(0.0, float(v) - 2.5 * float(sds.get(str(k), 10.0))) for k, v in skill_means.items()}
        cohort["skill_max"] = {str(k): min(200.0, float(v) + 2.5 * float(sds.get(str(k), 10.0))) for k, v in skill_means.items()}
    if aptitude_means:
        ...
        cohort["aptitude_max"] = {str(k): min(200.0, float(v) + 30.0) for k, v in aptitude_means.items()}

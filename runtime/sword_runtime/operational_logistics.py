...
def formation_movement_profile(read: Callable[[str], Mapping[str, Any]], formation: Mapping[str, Any], route: Mapping[str, Any]) -> dict[str, Any]:
    ...
    # Depth consumed per body when marching in a file. Mounted/chariot bodies
    # impose more longitudinal space than ordinary foot troops. Support work is
    # temporary duty from the same conserved manpower, not a troop role.
    depth_m = 0.0
    for role, raw in comp.items():
        n=max(0,int(raw)); name=str(role)
        per=2.6 if "cavalry" in name else (2.2 if "chariot" in name else 1.15)
        depth_m += n * per / files
    if depth_m <= 0: depth_m = personnel * 1.15 / files
    # Baggage is an operational requirement projection, not a spawned wagon owner.
    wagon_equiv=max(0,int(math.ceil(personnel/240.0)))
    ...
    mount_condition=max(300,min(1000,int(subsistence.get("mount_condition_milli",1000) or 1000)))
    mounted_bodies=sum(max(0,int(count)) for role,count in comp.items() if any(token in str(role).lower() for token in ("cavalry","mounted","chariot")))
    mounted_share=min(1.0,mounted_bodies/max(1,personnel))
    mount_drag=1.0+(1.0-mount_condition/1000.0)*0.30*mounted_share
    supply_state=evaluate_military_supply(read,formation)
    supply_factor=max(0.40,min(1.0,float(supply_state.get("movement_factor",1.0) or 1.0)))
    supply_drag=1.0/supply_factor
    base_adjusted=float(base)*mount_drag*supply_drag
    clearance*=mount_drag*supply_drag
    tail=max(base_adjusted, base_adjusted+clearance)
    ...
    return {
        **rp,
        ...
        "mount_condition_milli": mount_condition,
        "mount_condition_travel_factor": round(mount_drag,4),
        "supply_condition": str(supply_state.get("condition","adequate")),
        "supply_score_milli": int(supply_state.get("score_milli",1000)),
        "supply_movement_factor": round(supply_factor,4),
...
def recursive_army_movement_plan(read: Callable[[str], Mapping[str, Any]], read_group: Callable[[str], Mapping[str, Any]], load_formation: Callable[[str], Mapping[str, Any]], group_ref: str, route: Mapping[str, Any], formation_refs: list[str] | None = None) -> dict[str, Any]:
    ordered=army_ordered_formation_refs(read_group,group_ref)
    if formation_refs is None:
        refs=ordered
    else:
        requested={str(ref) for ref in formation_refs}
        missing=sorted(requested-set(ordered))
        if missing:
            raise ValueError(f"scoped army movement names formations outside the command hierarchy: {missing}")
        refs=[ref for ref in ordered if ref in requested]
    ...
    for ref in refs:
        ...
        units.append({"formation_ref":ref,"personnel":n,"required_wagon_equivalents":int(prof.get("required_wagon_equivalents",0)),"departure_offset_hours":round(start_offset,3),"head_arrival_hours":round(head,3),"tail_arrival_hours":round(tail,3),"battle_ready_hours":round(ready,3),"column_length_km":prof["column_length_km"],"supply_condition":str(prof.get("supply_condition","adequate")),"supply_score_milli":int(prof.get("supply_score_milli",1000)),"supply_movement_factor":float(prof.get("supply_movement_factor",1.0))})
        cursor += clearance

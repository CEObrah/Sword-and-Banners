...
class FortifiedSiteRuntimeMixin:
    ...
    def _formations_at(self, site_ref: str) -> list[str]:
        """Return exact persistent formations physically inside the site or descendants."""
        ...
        if isinstance(locations, Mapping):
            for loc, refs in locations.items():
                # Tang Manor outer-fort logistics exclude the independently enclosed
                # Sword Manor sub-fort. Sword Manor materializes its own exact depot.
                if site_ref == "loc_tang_manor" and self._site_contains_location("loc_sword_manor", str(loc)):
                    continue
                if self._site_contains_location(site_ref, str(loc)) and isinstance(refs, list):
    ...
    def _ensure_hot_fortified_site_resources(self, site_ref: str, *, at: str, authority_ref: str | None = None) -> dict[str, Any]:
        ...
        depot["water_reserve"] = {
            ...
            "source": str(water0.get("source") or "local wells/cisterns/stored water from exact static blueprint"),
        }
        ...
        if site_ref != "loc_tang_manor":
            ...
            if authority == "house_tang":
                depot["resupply_sources"] = {
                    ...
                    "rule": "House Tang military stock is replenished only from exact House Tang/Tang Manor conserved production and inventory; Qin state depots are not an implicit source",
                }
        ...
        depot.setdefault("campaign_resupply_rule",
            "post-baseline stock moves only through exact fortification logistics convoys; review/materialization itself transfers no consumables")
    ...
    def _siege_defender_reserve_draw(self, fort: dict[str, Any], *, days: int, defenders: int, at: str) -> dict[str, Any]:
        ...
        if grain_shortfall or water_shortfall:
            ...
            for fr in fort.get("garrison_formation_refs", []):
                ...
                f["fatigue"] = min(100, int(f.get("fatigue", 0)) + max(1, int(round(15 * severity))))
                self.put(fp, f)
        return entry

...
class FortifiedSiteRuntimeMixin:
                ...
                if isinstance(refs, list):
                    for ref in refs:
                        ...
                        try:
                            ...
                            continue
                        # The authority:false location index discovers bounded
                        # candidates only.  Exact formation state decides where
                        # the body actually is; a stale bucket must never create
                        # a phantom fortress garrison or logistics consumer.
                        location_ref = str(formation.get("location_ref", "")) if isinstance(formation, Mapping) else ""
                        if not location_ref or location_ref != str(loc):
                            continue
    ...
    def _site_depot_identity(site_ref: str) -> tuple[str, str]:
        ...
        # Citadel.  The enclosing Tang Manor and Inner Walls may materialize
    ...
    def _site_artillery_identity(site_ref: str) -> tuple[str, str]:
        ...
        # The Inner Walls fixed battery was materialized earlier under the
        # historical Sword Manor owner. Reuse that exact physical authority
        # instead of minting a second owner for the same 60 bed crossbows and
        # 18 trebuchets during later hot-site reviews.
        if site_ref == "loc_tang_inner_walls":
            return "artillery_fort_sword_manor", "state/art/fort-sword-manor.json"
        slug = site_slug(site_ref)
    ...
    def _siege_emplacement_row_score(row: Mapping[str, Any]) -> float:
        ...
        Bed-crossbow launch energy belongs to the latched mechanism. Existing
        garrison soldiers can crew it; engineering familiarity, formation drill,
        coordination, awareness and composure improve pointing/ranging/reload only.
        """
        skills = row.get("skills", {}) if isinstance(row.get("skills"), Mapping) else {}
        attrs = row.get("attributes", {}) if isinstance(row.get("attributes"), Mapping) else {}
        engineering = float(skills.get("Engineering", 0) or 0)
        formation = float(skills.get("Formation Fighting", skills.get("Formation_Fighting", row.get("formation_fighting", 0))) or 0)
        coordination = float(attrs.get("Coordination", row.get("ranged_coordination", 0)) or 0)
        awareness = float(attrs.get("Awareness", row.get("ranged_awareness", 0)) or 0)
        composure = float(attrs.get("Composure", 0) or 0)
        intelligence = float(attrs.get("Intelligence", 0) or 0)
        training = float(row.get("formation_training", 20) or 20)
        cohesion = float(row.get("formation_cohesion", 50) or 50)
        return max(0.0,
            0.24 * engineering + 0.18 * formation + 0.15 * coordination +
            0.14 * awareness + 0.10 * composure + 0.07 * intelligence +
            0.06 * training + 0.06 * cohesion
        )

    def _siege_emplacement_crew_rows(self, refs: list[str]) -> list[dict[str, float | int]]:
        """Return all lawful existing garrison bodies ranked for fixed-emplacement service."""
        ...
        if not hasattr(self, "_combat_prepare_formation") or not hasattr(self, "_combat_cohort_snapshot"):
            summary = self._garrison_summary(refs)
            n = max(0, int(summary.get("personnel", 0) or 0))
            if n:
                rows_out.append({"count": n, "score": 75.0})
            return rows_out
        for ref in refs:
            try:
            ...
            for row in rows:
                ...
                if count <= 0:
                    continue
                rows_out.append({"count": count, "score": self._siege_emplacement_row_score(row)})
        return rows_out

    @staticmethod
    def _siege_allocate_crew(rows: list[dict[str, float | int]], requested_crew: int) -> tuple[int, float]:
        need = max(0, int(requested_crew))
        if need <= 0:
            return 0, 0.0
        used = 0
        weighted = 0.0
        for row in sorted(rows, key=lambda x: float(x.get("score", 0) or 0), reverse=True):
            take = min(need - used, max(0, int(row.get("count", 0) or 0)))
            if take <= 0:
                continue
            used += take
            weighted += take * float(row.get("score", 0) or 0)
            if used >= need:
                break
        return used, (weighted / used if used else 0.0)

    def _siege_bed_crossbow_crew_control(self, refs: list[str], *, requested_crew: int | None = None) -> float:
        """Return fixed-emplacement service control without using Crossbow skill."""
        rows = self._siege_emplacement_crew_rows(refs)
        if requested_crew is None:
            requested_crew = sum(max(0, int(r.get("count", 0) or 0)) for r in rows)
        _used, score = self._siege_allocate_crew(rows, requested_crew)
        return score

    def _siege_crossbow_crew_control(self, refs: list[str]) -> float:
        """Compatibility alias: fixed fortress crossbows use emplacement service, not Crossbow proficiency."""
        return self._siege_bed_crossbow_crew_control(refs)
    ...
    def _fortified_site_installed_bed_crossbows(self, site_ref: str, blueprint: Mapping[str, Any]) -> int:
        """Resolve physical bed-crossbow fixtures for stock targets without troop-role inference."""
        _art_ref, art_path = self._site_artillery_identity(str(site_ref))
        ...
        if isinstance(art, Mapping):
            ...
            if isinstance(installed, Mapping) and "bed_crossbows" in installed:
                return max(0, int(installed.get("bed_crossbows", 0) or 0))
        fixtures = blueprint.get("installed_equipment", {}) if isinstance(blueprint.get("installed_equipment"), Mapping) else {}
        if "bed_crossbows" in fixtures:
            return max(0, int(fixtures.get("bed_crossbows", 0) or 0))
        parent = self.read_optional("state/art/tang-manor-artillery.json")
        if isinstance(parent, Mapping):
            deriv = parent.get("derivation", {}) if isinstance(parent.get("derivation"), Mapping) else {}
            if site_ref == "loc_tang_inner_walls":
                return max(0, int(deriv.get("inner_walls_bed_crossbows", 0) or 0))
            if site_ref == "loc_tang_inner_citadel":
                return max(0, int(deriv.get("inner_citadel_bed_crossbows", 0) or 0))
            if site_ref == "loc_tang_manor":
                installed = parent.get("installed_systems", {}) if isinstance(parent.get("installed_systems"), Mapping) else {}
                return max(0, int(installed.get("bed_crossbows", 0) or 0))
        return 0
    ...
    def _ensure_hot_fortified_site_resources(self, site_ref: str, *, at: str, authority_ref: str | None = None) -> dict[str, Any]:
        ...
        bed_crossbows = self._fortified_site_installed_bed_crossbows(site_ref, blueprint)
        ...
        if site_ref == "loc_tang_manor":
            ...
            if str(installed.get("mode", "")) == "reference_existing_exact_fixture_authority" and site_ref == "loc_tang_inner_walls":
                ...
                art["rule"] = "physical Inner Walls subset of the Tang Manor artillery authority; not duplicate systems"
            else:
    ...
    def _siege_defender_reserve_draw(self, fort: dict[str, Any], *, days: int, defenders: int, at: str) -> dict[str, Any]:
        """Consume exact fortified-site grain and water during a siege interval.

        Field armies do not own ration inventory. A sealed fortress/city is the
        important exception because stored food and water directly determine siege
        endurance. Those reserves live only in the fortified-site depot.
        """
        ...
        days = max(0, int(days)); defenders = max(0, int(defenders))
        grain_need = defenders * days * 2
        grain_before = max(0, int(stocks.get("grain_kg", 0)))
        grain_used = min(grain_need, grain_before); stocks["grain_kg"] = grain_before - grain_used
        grain_shortfall = max(0, grain_need - grain_used)
        water = depot.setdefault("water_reserve", {})
        water_need = defenders * days
        water_before = max(0, int(water.get("current_person_days", 0)))
        water_used = min(water_need, water_before); water["current_person_days"] = water_before - water_used
        water_shortfall = max(0, water_need - water_used)
        entry = {
            "at": at, "reason": "sealed fortified-site siege endurance", "days": days,
            "garrison_personnel": defenders,
            ...
            "ration_authority": str(depot.get("owner_id", "fortified_site_depot")),
        }
        depot.setdefault("consumption_history", []).append(entry); depot["consumption_history"] = depot["consumption_history"][-64:]
        self.put(depot_path, depot)
        fort["food_kg"] = max(0, int(stocks.get("grain_kg", 0))); fort["food_projection_only"] = True; fort["fortified_site_depot_ref"] = str(depot.get("owner_id", ""))
        if grain_shortfall or water_shortfall:
            ...
            for fr in fort.get("garrison_formation_refs", []):
                try: fp, f0 = self._load_formation(str(fr))
                except ValueError: continue
                f = copy.deepcopy(f0); current = CampaignTime.parse(at); settle_formation_idle_fatigue(f, current=current, rules=self.read(FATIGUE_RULES_PATH))
                supply_condition = "exhausted" if severity >= 0.75 else ("critical" if severity >= 0.35 else "strained")
                morale_resolution = resolve_formation_morale(
                    self,
                    base_morale=int(f.get("morale", 50)),
                    registered_fear_pressure=5.0 * severity,
                    cohesion=int(f.get("cohesion", 50)),
                    supply_condition=supply_condition,
                )
                f["morale"] = int(morale_resolution["effective_morale"])
                stamp_formation_activity_fatigue(f, completed_at=current, fatigue_gain=max(1, int(round(15 * severity))), activity_kind="siege_deprivation")
                self.put(fp, f)
        return entry
    ...
    def _siege_prepare_fortress_artillery(self, fort: Mapping[str, Any], *, defender_refs: list[str], battle_hours: int, at: str, attacker_refs: list[str] | None = None, engagement_range_m: float | None = None) -> dict[str, Any]:
        ...
        bed_optimal_crew = max(bed_min_crew, int(bed_cfg.get("optimal_crew", 6) or 6))
        bed_serviceable = int(math.floor(bed_inst * condition))
        # Fixed emplacements draw crew from the ordinary garrison, not from a
        # fictitious handheld-crossbow troop species. Best suited bodies are
        # allocated first, but any fit garrison body may fill the minimum crew.
        bed = min(bed_serviceable, personnel // bed_min_crew)
        crew_rows = self._siege_emplacement_crew_rows([str(x) for x in defender_refs])
        bed_requested_crew = bed * bed_optimal_crew
        bed_fit_crew, bed_crew_control = self._siege_allocate_crew(crew_rows, bed_requested_crew)
        if bed_fit_crew < bed * bed_min_crew:
            bed = bed_fit_crew // bed_min_crew
            bed_requested_crew = bed * bed_optimal_crew
            bed_fit_crew, bed_crew_control = self._siege_allocate_crew(crew_rows, bed_requested_crew)
        personnel_left = max(0, personnel - bed_fit_crew)
        # Trebuchets still prefer real engineering/sapper competence; bed-crossbow
        # service does not consume a separate crossbow-profession headcount.
        engineers_left_for_other = min(engineers, personnel_left)
        treb = min(int(math.floor(treb_inst * condition)), engineers_left_for_other // 60)
        engineer_left = max(0, engineers_left_for_other - treb * 60)
        drop = min(int(math.floor(drop_inst * condition)), engineer_left // 4)
        fire = min(int(math.floor(fire_inst * condition)), max(0, personnel_left - treb * 60 - drop * 4) // 3)
    ...
    def _settle_private_production(self, state: str, occurrences: int, at: str) -> None:
        ...
        if state == "qin":
            for site_ref in ("loc_tang_manor", "loc_tang_inner_walls"):
    ...
    def _fort_exact_convoy(self, convoy_ref: str) -> tuple[str, Mapping[str, Any]] | None:
        try:
            path = self.owner_path(str(convoy_ref))
        except (FileNotFoundError, KeyError, ValueError):
            return None
        if not isinstance(path, str) or not path.startswith("state/logistics/fortification-convoys/") or path.endswith("/index.json"):
            return None
        convoy = self.read_optional(path)
        if not isinstance(convoy, Mapping):
            return None
        if str(convoy.get("schema", "")) != "sword-logistics-convoy" or str(convoy.get("owner_id", "")) != str(convoy_ref):
            return None
        return path, convoy

    def _fort_active_convoy_records(self) -> list[tuple[str, str, Mapping[str, Any]]]:
        """Project active convoy routing from exact registered owners.

        The convoy index is a cache.  Exact convoy owners registered in the
        authoritative owner index remain discoverable even if both the route
        mapping and ``active_refs`` entry are lost.
        """
        ...
        for ref, path in sorted(owners.items()) if isinstance(owners, Mapping) else []:
            if not isinstance(ref, str) or not isinstance(path, str):
                continue
            if not path.startswith("state/logistics/fortification-convoys/") or path.endswith("/index.json"):
                continue
            exact = self._fort_exact_convoy(ref)
            if exact is None:
                continue
            exact_path, convoy = exact
            routes[ref] = exact_path
            if str(convoy.get("status", "")) in {"in_transit", "arrived_holding"}:
                rows.append((ref, exact_path, convoy))
                active_refs.append(ref)
        idx = self._fort_logistics_convoy_index()
        changed = idx.get("convoys") != routes or sorted(str(x) for x in idx.get("active_refs", []) if isinstance(x, str)) != active_refs
        if changed:
            idx["convoys"] = routes
            idx["active_refs"] = active_refs
            self.put("state/logistics/fortification-convoys/index.json", idx)
        return rows
    ...
    def _fort_dispatch_field_siege_convoy(
        self,
        *,
        formation_ref: str,
        destination_site_ref: str,
        source_depot_ref: str,
        construction_material_units: int,
        at: str,
    ) -> dict[str, Any]:
        """Dispatch exact siege-engineering material to a field formation.

        The stock leaves the source depot immediately and exists only in this
        detached aggregate convoy until it physically reaches the besieged site.
        This is intentionally separate from the removed persistent army-train
        model: the convoy exists because the cargo is independently in transit.
        """
        formation_ref = str(formation_ref)
        destination_site_ref = str(destination_site_ref)
        quantity = max(0, int(construction_material_units))
        if quantity <= 0:
            raise ValueError("field siege resupply requires positive construction material")
        _fp, formation = self._load_formation(formation_ref)
        source_path = self.owner_path(str(source_depot_ref))
        source = copy.deepcopy(self.read(source_path))
        if str(source.get("schema", "")) != "sword-depot":
            raise ValueError("field siege resupply source must be an exact depot")
        source_loc = str(source.get("location_ref") or source.get("site_ref") or "")
        if not source_loc:
            raise ValueError("field siege resupply source depot has no physical location")
        route = shortest_path(self.read, source_loc, destination_site_ref, modes=("convoy",))
        stocks = source.setdefault("stocks", {})
        available = max(0, int(stocks.get("construction_material_units", 0)))
        if quantity > available:
            raise ValueError("source depot lacks construction_material_units for field siege resupply")
        stocks["construction_material_units"] = available - quantity
        token = hashlib.sha256((str(source_depot_ref)+"|"+formation_ref+"|"+destination_site_ref+"|"+at+"|"+str(quantity)).encode()).hexdigest()[:18]
        ref = f"field_siege_supply_{token}"
        path = f"state/logistics/fortification-convoys/{token}.json"
        hours = max(1, int(math.ceil(float(route.get("duration_hours", 1)))))
        doc = {
            "schema":"sword-logistics-convoy", "owner_id":ref, "kind":"field_siege_resupply",
            "source_depot_ref":str(source_depot_ref), "destination_site_ref":destination_site_ref,
            "destination_depot_ref":"", "destination_formation_ref":formation_ref,
            "cargo":{"construction_material_units":quantity}, "status":"in_transit",
            "departed_at":at, "arrives_at":str(CampaignTime.parse(at).add_hours(hours)),
            ...
            "rule":"detached siege-engineering cargo is conserved in transit and enters formation custody only after physical arrival at the siege site",
        }
        source.setdefault("transfer_history", []).append({"at":at,"reason":f"dispatch field siege material to {formation_ref} at {destination_site_ref}","destination_ref":ref,"moved":{"construction_material_units":quantity}})
        source["transfer_history"] = source["transfer_history"][-64:]
        self.put(source_path, source); self.put(path, doc); self._register_owner(ref, path)
        idx = self._fort_logistics_convoy_index(); idx.setdefault("convoys", {})[ref]=path; idx.setdefault("active_refs", []).append(ref); idx["active_refs"]=sorted(set(idx["active_refs"])); self.put("state/logistics/fortification-convoys/index.json", idx)
        return {"convoy_ref":ref,"status":"in_transit","arrives_at":doc["arrives_at"],"cargo":copy.deepcopy(doc["cargo"]),"route_refs":doc["route_refs"]}

    def _fort_active_field_siege_convoy(self, formation_ref: str, site_ref: str) -> bool:
        for _ref, _path, convoy in self._fort_active_convoy_records():
            if str(convoy.get("kind", "")) == "field_siege_resupply" and str(convoy.get("destination_formation_ref", "")) == str(formation_ref) and str(convoy.get("destination_site_ref", "")) == str(site_ref):
                return True
        return False
    ...
    def _fort_settle_resupply_convoy(self, convoy_ref: str, *, at: str) -> dict[str, Any]:
        exact = self._fort_exact_convoy(str(convoy_ref))
        if exact is None:
            raise ValueError("unknown fortification resupply convoy")
        path, convoy0 = exact
        convoy = copy.deepcopy(dict(convoy0))
        idx = self._fort_logistics_convoy_index()
        idx.setdefault("convoys", {})[str(convoy_ref)] = path
        ...
        if kind == "field_siege_resupply":
            formation_ref = str(convoy.get("destination_formation_ref", ""))
            site_ref = str(convoy.get("destination_site_ref", ""))
            fp, formation0 = self._load_formation(formation_ref)
            formation = copy.deepcopy(formation0)
            formation_loc = str(formation.get("location_ref", ""))
            contact_site = enclosing_fortification_site(self.read, formation_loc) or formation_loc
            if contact_site != site_ref:
                convoy["status"] = "arrived_holding"
                convoy["arrival"] = {"at":at,"delivered":{},"remaining":copy.deepcopy(convoy.get("cargo", {})),"reason":"destination formation is not physically present at the siege site"}
                self.put(path, convoy)
                return {"convoy_ref":convoy_ref,"status":"arrived_holding","delivered":{},"remaining":copy.deepcopy(convoy.get("cargo", {}))}
            delivered = {str(k):max(0,int(v)) for k,v in (convoy.get("cargo", {}) or {}).items() if max(0,int(v)) > 0}
            logistics = formation.setdefault("logistics", {})
            for key, qty in delivered.items():
                logistics[key] = int(logistics.get(key, 0)) + qty
            formation["last_resupplied_at"] = at
            convoy["cargo"] = {}; convoy["status"] = "delivered"; convoy["arrival"] = {"at":at,"delivered":delivered,"remaining":{}}
            self.put(fp, formation); self.put(path, convoy)
            idx["active_refs"]=[str(x) for x in idx.get("active_refs", []) if str(x)!=str(convoy_ref)]; self.put("state/logistics/fortification-convoys/index.json",idx)
            return {"convoy_ref":convoy_ref,"status":"delivered","delivered":delivered,"remaining":{}}
    ...
    def _fort_active_convoy_for_site(self, site_ref: str) -> bool:
        for _ref, _path, convoy in self._fort_active_convoy_records():
            if str(convoy.get("destination_site_ref", "")) == str(site_ref):
                return True
        return False
    ...
    def _fort_settle_due_campaign_convoys(self, at: str) -> list[str]:
        ...
        settled: list[str] = []
        for ref, _path, convoy in self._fort_active_convoy_records():
    ...
    def _fort_dematerialize_if_cold(self, site_ref: str, *, at: str) -> dict[str, Any]:
        ...
        for _ref, _path, cv in self._fort_active_convoy_records():
            if str(cv.get("destination_site_ref",""))==site_ref:
    ...
    def _authorize_command(self, command: Any, payload: Mapping[str, Any]) -> None:
        ...
        if action=="dispatch_resupply":
            ...
            if action=="settle_convoy":
                exact=self._fort_exact_convoy(str(payload.get("convoy_ref",""))); cv=exact[1] if exact is not None else None; site=str(cv.get("destination_site_ref","")) if isinstance(cv,Mapping) else ""
    ...
    def _command_layer_fortified_site(self, command: Any, payload: Mapping[str, Any], next_dispatch: Any) -> dict[str, Any]:
        if command.command_type == "fortification_logistics":

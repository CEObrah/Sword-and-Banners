...
class IndependentOrganizationMixin:
    ...
    def _organization_exact(self, organization_ref: str) -> tuple[str, dict[str, Any]]:
        candidates: list[str] = []
        try:
            owner_path = self.owner_path(organization_ref)
        except (KeyError, ValueError, FileNotFoundError):
            owner_path = None
        if isinstance(owner_path, str) and owner_path:
            candidates.append(owner_path)
        idx = self._organization_index()
        routed = idx.get("organizations", {}).get(organization_ref) if isinstance(idx.get("organizations"), Mapping) else None
        if isinstance(routed, str) and routed and routed not in candidates:
            candidates.append(routed)
        canonical = self._organization_path(organization_ref)
        ...
        for path in candidates:
            ...
            if str(doc.get("schema", "")) != "sword-independent-organization":
                continue
            if str(doc.get("owner_id", "")) != organization_ref:
                continue
            return path, copy.deepcopy(dict(doc))
        raise ValueError("unknown independent organization")

    def _active_organization_records(self) -> list[tuple[str, str, dict[str, Any]]]:
        """Recover the organization routing cache from exact registered owners.

        ``state/organizations/index.json`` is explicitly non-authoritative.  A
        missing route or ``active_refs`` entry must therefore never make an
        existing active organization disappear from scheduler reconciliation.
        """
        idx = self._organization_index()
        candidates = {
            str(ref) for ref in idx.get("organizations", {})
            if isinstance(ref, str) and ref
        } if isinstance(idx.get("organizations"), Mapping) else set()
        try:
        ...
        if isinstance(owners, Mapping):
            for ref, path in owners.items():
                if (
                    ...
                    and path.startswith("state/organizations/")
                    and path != self._ORG_INDEX
                ):
                    candidates.add(ref)
        ...
        for ref in sorted(candidates):
            ...
            if str(org.get("status", "")) == "active":
                ...
                rows.append((ref, path, org))
        if idx.get("organizations") != routes or idx.get("active_refs") != active_refs:
            idx["organizations"] = routes
    ...
    def _autonomy_institution(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        active = {ref for ref, _path, _org in self._active_organization_records()}
        if organization_ref not in active:
    ...
    def _command_layer_independent_organizations(self, command: Any, payload: Mapping[str, Any], next_dispatch: Any) -> dict[str, Any]:
        if command.command_type == "organization_action":

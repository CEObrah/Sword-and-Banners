...
from sword_runtime.causal_event_store import EVENT_OWNER_REF, get_causal_event, write_causal_event_owner


CAMPAIGN_CAUSAL_WORK_PATH = "state/index/campaign-causal-work.json"
_MAX_SUMMARY_CHARS = 4000
_PLAYER_DELIVERY_KINDS = frozenset({"message", "audience_response", "institutional_response", "petition_response"})
_EVENT_EFFECT_METADATA_KEYS = (
    "actor_ref", "target_ref", "basis_goal", "process_kind", "process_stage",
    "route_ref", "route_domain", "delivery",
)
...
def _work_document(planner: Any) -> dict[str, Any]:
    ...
    if not isinstance(targets, list):
        raise ValueError("campaign causal work routing is invalid")
    return document
...
def _target_fields(target: Mapping[str, Any]) -> tuple[str, str, CampaignTime, int, str, bool]:
    ...
    if kind in _PLAYER_DELIVERY_KINDS:
        if effect.get("target_ref") != "char_tang_wei":
            raise ValueError("player-delivery campaign causal work requires exact Tang Wei target_ref")
        delivery = effect.get("delivery")
        if isinstance(delivery, Mapping) and delivery.get("target_ref") != "char_tang_wei":
            raise ValueError("player-delivery campaign causal work delivery target must match Tang Wei")
...
def sync_campaign_work_routes(planner: Any, runtime: dict[str, Any]) -> None:
    ...
    for work_ref, target in sorted(targets.items()):
        ...
        existing_record = get_causal_event(planner, work_ref) if owner_ref == EVENT_OWNER_REF else (recorded.get(work_ref) if isinstance(recorded, Mapping) else None)
...
def settle_campaign_work_target(planner: Any, host: Mapping[str, Any], at: str) -> dict[str, Any] | None:
    ...
    existing = get_causal_event(planner, work_ref) if owner_ref == EVENT_OWNER_REF else causal_events.get(work_ref)
    ...
    effect = target.get("effect") if isinstance(target.get("effect"), Mapping) else {}
    for key in _EVENT_EFFECT_METADATA_KEYS:
        if key in effect:
            event[key] = copy.deepcopy(effect.get(key))
    causal_events[work_ref] = event

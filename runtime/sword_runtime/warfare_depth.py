...
class WarfareDepthMixin:
    ...
    def _mercenary_tactical_active_use_refs(self, formation_ref: str) -> list[str]:
        ...
        op_index = self.read_optional("state/operations/index.json")
        ...
        if isinstance(operations, Mapping):
            for operation_ref, operation_path in sorted(operations.items()):
                if not isinstance(operation_path, str):
                    continue
                operation = self.read_optional(operation_path)
                if not isinstance(operation, Mapping) or str(operation.get("status", "")) in terminal_operation_statuses:
                    continue
    ...
    def _release_withdrawing_mercenary_from_operations(self, formation_ref: str, *, at: str) -> None:
        ...
        operations = index0.get("operations", {}) if isinstance(index0, Mapping) else {}
        ...
        for operation_ref, operation_path in sorted(operations.items()):
            if not isinstance(operation_path, str):
                continue
            operation0 = self.read_optional(operation_path)
            if not isinstance(operation0, Mapping):
                continue
    ...
    def _command_layer_warfare_depth(self, command: Any, payload: Mapping[str, Any], next_dispatch: Any) -> dict[str, Any]:
        ...
        for ref in sorted(refs):
            try:
                ...
                try:
                    formation = self._load_formation(ref)[1]

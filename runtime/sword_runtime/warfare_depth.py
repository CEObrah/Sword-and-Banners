...
def build_formation_command_structure(formation: Mapping[str, Any], rules: Mapping[str, Any]) -> dict[str, Any]:
    ...
    # Internal command echelons are durable appointments, not a live formula that
    # shrinks whenever casualties cross a round-number threshold.  A surviving
    # 1,000-commander can still command a 998-, 700-, or 300-person remnant until
    # an explicit reorganization, death, reassignment, relief, or promotion changes
    # that appointment.  Prefer the formation's already-established hierarchy when
    # present; derive a fresh establishment only for a genuinely new/reorganized
    # formation that has no saved command establishment yet.
    saved_structure = formation.get("command_structure") if isinstance(formation.get("command_structure"), Mapping) else {}
    saved_internal = saved_structure.get("internal_hierarchy", []) if isinstance(saved_structure, Mapping) else []
    internal_output: Any = None
    internal_rows: list[Mapping[str, Any]] = []
    if isinstance(saved_internal, Mapping):
        summary = saved_internal.get("summary", [])
        if isinstance(summary, list):
            internal_rows = [row for row in summary if isinstance(row, Mapping)]
        internal_output = copy.deepcopy(saved_internal)
    elif isinstance(saved_internal, list) and saved_internal:
        internal_rows = [row for row in saved_internal if isinstance(row, Mapping)]
    if not internal_rows:
        profile_internal = profile.get("internal_hierarchy", []) if isinstance(profile, Mapping) else []
        if isinstance(profile_internal, list):
            internal_rows = [row for row in profile_internal if isinstance(row, Mapping)]
    if not internal_rows:
        ...
        for row in internal_rows:
            ...
            # full_elements/partial_tail_personnel describe current span, while
            # count remains the durable surviving command establishment. Do not
            # delete or demote billets merely because current strength fell below
            # their nominal echelon size.
            full = min(count, n // scale) if n else 0
            tail = n % scale if count > full and n else 0
            hierarchy.append({
                "scale": scale,
                "count": count,
                "full_elements": full,
    ...
    if isinstance(internal_output, Mapping):
        internal_output = copy.deepcopy(internal_output)
        internal_output["summary"] = copy.deepcopy(hierarchy)
    else:
        internal_output = copy.deepcopy(hierarchy)
...
def build_mercenary_command_structure(company: Mapping[str, Any], rules: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "officer_cadre": {
            ...
            "rule": "mercenary command roles are aggregate duty assignments carved from the conserved company headcount; this projection creates zero people",
        },
        "internal_commander_assignments": internal_commanders,
        "internal_commanders_inside_fighting_establishment": internal_commanders,
        "support": {
...
class WarfareDepthMixin:
    ...
    def _settle_mercenary_training(self, mercenary_ref: str, occurrences: int, at: str) -> None:
        ...
        for pool in company.get("troop_pools", []) if isinstance(company.get("troop_pools"), list) else []:
            ...
            tr0 = capability.get("training_runtime", {}) if isinstance(capability.get("training_runtime"), Mapping) else {}
            smart_cursor = max(0, int(tr0.get("smart_focus_cursor", 0)))
            def _smart(names: list[str], values: Mapping[str, float], slots: int) -> list[str]:
                unique = list(dict.fromkeys(str(x) for x in names if str(x)))
                ranked = sorted(unique, key=lambda name: (float(values.get(name, 0.0)), name))
                if len(ranked) <= slots:
                    return ranked
                fixed = ranked[:max(1, slots - 1)]
                tail = ranked[max(1, slots - 1):]
                fixed.append(tail[smart_cursor % len(tail)])
                return fixed[:slots]
            skills = _smart(skills, skill_means, 6)
            attrs = _smart(attrs, attr_means, 3)
            ...
            advance_cohort_training(
                ...
                facility_grade=str(override.get("facility_grade", cfg.get("facility_grade", "adequate"))),
                equipment_grade=str(override.get("equipment_grade", cfg.get("equipment_grade", "adequate"))),
                recovery_grade=str(override.get("recovery_grade", cfg.get("recovery_grade", "adequate"))),
                practice_mode="drill",
            ...
            capability["training_runtime"] = {
                ...
                "smart_focus_cursor": smart_cursor + 1,

...
def _target_billets(personnel: int) -> dict[str, int]:
    n = max(0, int(personnel))
    result: dict[str, int] = {}
    for scale in RANK_SCALES:
        result[RANK_KEY[scale]] = 0 if n <= 0 else (n // scale + (1 if n % scale else 0))
    return result
...
def ensure_officer_cadre(formation: MutableMapping[str, Any]) -> MutableMapping[str, Any]:
    ...
    if isinstance(raw, MutableMapping):
        ...
        if not any(inventory.values()):
            inventory = _target_billets(int(formation.get("personnel", 0)))
        cadre = {
...
def reorganize_officer_cadre(formation: MutableMapping[str, Any], *, at: str | None = None, reason: str = "strength_reorganization") -> dict[str, Any]:
    ...
    targets = _target_billets(int(formation.get("personnel", 0)))

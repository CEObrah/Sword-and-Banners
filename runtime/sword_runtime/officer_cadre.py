...
def _target_billets(formation: Mapping[str, Any]) -> dict[str, int]:
    ...
    The Unit commander owns the Unit echelon itself and therefore does
    ...
    saved_mix = formation.get("establishment_composition") if isinstance(formation.get("establishment_composition"), Mapping) else None
    composition = saved_mix or represented_establishment_composition(formation)
...
def record_officer_cadre_combat_experience(
    formation: MutableMapping[str, Any],
    *,
    battle_hours: float,
    ...
    """Season the formation and its aggregate officers through real combat.

    Named commanders/officers receive exact skill development elsewhere. This
    function covers the unmaterialized officer cadre and the formation as an
    organization. It uses the existing promotion-development accumulator rather
    than creating a second XP currency.
    """
    hours = max(0.0, float(battle_hours))
    contact = max(0.0, min(1.0, float(contact_fraction)))
    exposure = hours * (0.25 + 0.75 * contact)
    if exposure <= 0.0 or max(0, int(formation.get("personnel", 0) or 0)) <= 0:
        return {"combat_exposure_hours": 0.0, "promotion_credit_hours": 0, "training_progress_gain": 0}

    cadre = ensure_officer_cadre(formation)
    cadre["verified_combat_exposure_hours"] = round(
        float(cadre.get("verified_combat_exposure_hours", 0.0) or 0.0) + exposure, 3
    )
    cadre["field_engagements"] = max(0, int(cadre.get("field_engagements", 0) or 0)) + 1

    # Battlefield command is high-value development, but a few hours of action
    # must not mint senior officers. Credit is bounded and feeds the same slow
    # promotion law as verified training.
    promotion_credit = max(1, min(24, int(round(exposure * 1.5))))
    promoted = develop_officer_cadre(formation, training_hours=promotion_credit, at=at)

    # The Unit itself learns to operate together under pressure. This is a small
    # organizational gain and remains bounded by the existing training-progress
    # scale; casualties/fatigue/cohesion losses are settled independently.
    progress_gain = max(1, min(3, int(round(exposure / 2.0))))
    before_progress = max(0, int(formation.get("training_progress", 0) or 0))
    formation["training_progress"] = max(0, min(100, before_progress + progress_gain))
    actual_gain = int(formation["training_progress"]) - before_progress
    return {
        "combat_exposure_hours": round(exposure, 3),
        "promotion_credit_hours": promotion_credit,
        "training_progress_gain": actual_gain,
        "promoted": promoted,
    }
...
def officer_cadre_summary(formation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "verified_combat_exposure_hours": round(float(cadre.get("verified_combat_exposure_hours", 0.0) or 0.0), 3),
        "field_engagements": max(0, int(cadre.get("field_engagements", 0) or 0)),
    }

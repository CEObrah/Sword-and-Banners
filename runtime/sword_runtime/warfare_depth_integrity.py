...
* House Tang / Inner Walls command representation is inherited force-wide,
...
_SCOPED_OWNER_FORCES = frozenset({"force_house_tang"})
...
def _formation_in_scoped_standard(formation: Mapping[str, Any]) -> str:
    ...
    if owner_force in _SCOPED_OWNER_FORCES:
        return "house_tang_institution_wide"
...
def _normalize_scoped_explicit_profile(
    ...
    unit-command representation marker when the scoped policy declares the top
    commander a full character, avoiding an aggregate fallback from an omitted
    combined-representation marker.
    """
    ...
    if commander_rep == "full_character":
...
def _synthesized_scoped_formation_profile(formation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "hierarchy_rule": (
            "House Tang formations inherit one full-character persistent unit commander "
...
class WarfareDepthIntegrityMixin:
    ...
    def _command_layer_warfare_depth_integrity(self, command: Any, payload: Mapping[str, Any], next_dispatch: Any) -> dict[str, Any]:

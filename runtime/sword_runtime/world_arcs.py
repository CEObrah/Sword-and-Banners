...
def _schedule_report_route(
    ...
    *,
    ...
    physical_route = None
    player_target = None
    if visibility == "direct" and isinstance(source_location_ref, str) and source_location_ref:
        player_target = player_command_location(planner)
        if player_target:
            try:
                physical_route = command_message_route(
                    planner.read, source_location_ref, player_target, round_trip=False,
                )
            except (FileNotFoundError, KeyError, TypeError, ValueError):
                physical_route = None
    if isinstance(physical_route, Mapping):
        travel_seconds = max(0, int(physical_route.get("travel_seconds", 0) or 0))
        # Keep scheduler causality strictly forward even for co-located dispatch.
        due = CampaignTime.parse(at).add_seconds(max(1, travel_seconds))
    ...
    hosts[host_id] = {
        ...
        **({
            ...
            "response_target_location_ref": player_target,
            "communication_travel_seconds": max(0, int(physical_route.get("travel_seconds", 0) or 0)),
            "courier_route": copy.deepcopy(dict(physical_route)),
            "communication_rule": "direct world-arc dispatch is not receipt; delivery follows physical geography and reroutes if Tang Wei moves",
            "retire_after_settlement": True,
        } if isinstance(physical_route, Mapping) else {}),
    }
    found = next(
...
def _world_arc_report_origin_location(
    planner: Any, *, actor_ref: str, actor_owner: Mapping[str, Any],
    material_evidence: Mapping[str, Any] | None = None,
    controlled_evidence: Mapping[str, Any] | None = None,
) -> str | None:
    """Resolve the physical origin of a deliberately addressed arc report.

    Exact operation/formation evidence outranks the actor's administrative home.
    Ambient rumor/merchant propagation does not call this helper as a courier
    authority.  Failure to resolve a physical source leaves the legacy ambient
    retry model in place rather than inventing a route.
    """
    for evidence in (controlled_evidence, material_evidence):
        ...
        if isinstance(operation_ref, str) and operation_ref:
            for ref, _path, operation in iter_exact_operation_records(planner):
                if ref != operation_ref or not isinstance(operation, Mapping):
                    continue
                location = operation.get("location_ref")
                if isinstance(location, str) and location:
                    return location
                break
    ...
    for key in ("current_location", "location_ref", "location"):
        location = actor_owner.get(key)
        if isinstance(location, str) and location:
            return location
    return command_endpoint_location(planner, actor_ref)
...
def _player_controlled_arc_operation(
    ...
    *,
    ...
    try:
        ...
        return None
    for operation_ref, _path, operation in iter_exact_operation_records(planner):
...
def settle_world_arc_review(planner: Any, host: Mapping[str, Any], at: str) -> None:
    ...
    if drivers:
        ...
        if (result in _PLAYER_REPORTABLE_RESULTS or report_controlled_stall) and (visibility in {"discoverable", "direct"} or report_controlled_stall):
            ...
            if report_route:
                source_location_ref = _world_arc_report_origin_location(
                    planner, actor_ref=actor_ref, actor_owner=actor_owner,
                    material_evidence=material_evidence, controlled_evidence=controlled_operation_evidence,
                ) if report_visibility == "direct" else None
                _schedule_report_route(
                    planner, arc_ref=arc_ref, source_event_ref=event_ref, at=at, route=report_route,
                    origin_state=origin_state, pressure_stage=pressure_stage, visibility=report_visibility,
                    source_location_ref=source_location_ref,
                )
...
def settle_world_arc_report(planner: Any, host: Mapping[str, Any], at: str) -> dict[str, Any] | None:
    ...
    if runtime_host.get("physical_delivery") is True:
        if not ensure_player_message_delivery(planner, runtime_host, at):
            return None
        # Delivery helpers update the scheduler-owned host. Refresh before the
        # remainder of settlement mutates retry/retirement fields.
        runtime = copy.deepcopy(planner.read(RUNTIME_PATH))
        runtime_host = runtime.get("hosts", {}).get(host.get("host_id"))
        if not isinstance(runtime_host, dict):
            raise ValueError("world arc report lost its physical delivery host")
    ...
    # A route explicitly classified as direct is already a lawful delivery
    # channel to Tang Wei (for example a military dispatch, family courier, or
    # other deliberately addressed report).  Its scheduled delay represents
    # the causal handoff; do not then discard it merely because Wei crossed a
    # state border before the delivery attempt.  Discoverable ambient channels
    # still require geographic/channel fit and therefore cannot teleport news.
    state_fit = (
        visibility == "direct"
        or not origin_state

...
from sword_runtime.campaign_communications import ensure_player_message_delivery
...
def render_probationary_offer(details: Mapping[str, Any]) -> str:
    ...
    return (
        ...
        "This is an offer, not an automatic appointment: no command authority, troop custody, march order, or allegiance changes unless Tang Wei accepts and reports."
    )[:4000]


def probationary_offer_details(
...
def _split_detachment(planner: Any, appointment: Mapping[str, Any], at: str) -> tuple[str, dict[str, Any]]:
    ...
    parent_shields, child_shields = planner._partition_material(planner._shield_units(original), n, total)
    parent_armor, child_armor = planner._partition_material(planner._armor_units(original), n, total)
    planner._set_equipment_units(parent, parent_eq)
    planner._set_equipment_units(child, child_eq)
    planner._set_shield_units(parent, parent_shields)
    planner._set_shield_units(child, child_shields)
    planner._set_armor_units(parent, parent_armor)
    planner._set_armor_units(child, child_armor)
    child_path = f"state/formations/{child_ref.removeprefix('formation_').replace('_', '-')}.json"
    ...
    force["allocated_to_formations"][parent_ref] = {"personnel": parent["personnel"], "composition": copy.deepcopy(parent["composition"])}
    force["allocated_to_formations"][child_ref] = {"personnel": n, "composition": copy.deepcopy(child["composition"])}
...
def assume_probationary_command(planner: Any, at: str) -> str | None:
    ...
    for appointment in appointments:
        ...
        parent = planner.read(planner.owner_path(parent_ref))
        resolved_operation = exact_operation_record(planner, op_ref)
...
def close_completed_service(planner: Any, at: str) -> str | None:
    ...
    for appointment in appointments:
        if not isinstance(appointment, dict) or appointment.get("offer_kind") != _OFFER_KIND or appointment.get("status") != "active":
            continue
        resolved_operation = exact_operation_record(planner, str(appointment.get("operation_ref", "")))
        operation = resolved_operation[1] if resolved_operation is not None else None
...
class QinCommandProgressionMixin:
    """Forward-only command-scale progression from exact current service state."""

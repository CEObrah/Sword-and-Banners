...
_FORMATION_REFS = (
    ...
    "formation_tang_wei_house_guard_first",
)
...
def _issue_material_reserve(
    ...
    *,
    ...
    history = formation.setdefault("field_supply_issue_history", [])
    if isinstance(history, list):
        history.append({
            "at": at,
            "kind": "house_field_preparation_issue",
            "source_ref": "depot_house_tang",
            "reserve_days": reserve_days,
            "arrow_loads_total": arrow_loads_total,
            "issued": copy.deepcopy(issued),
            "targets": copy.deepcopy(targets),
        })
        formation["field_supply_issue_history"] = history[-24:]
...
def _issue_spare_equipment(
    ...
    *,
    ...
    personnel = max(0, int(formation.get("personnel", 0)))
    desired_each = int(math.ceil(personnel * spare_basis_points / 10000.0))
    loadout = _loadout(planner, formation)
    item_keys: list[str] = []
    for field in eligible_fields:
        item = loadout.get(field)
        if isinstance(item, str) and item and item not in item_keys:
            item_keys.append(item)

    issued: dict[str, int] = {}
    shortfalls: dict[str, int] = {}
    for item_key in item_keys:
        available = house_armory_reserve_available(planner, item_key)
        amount = min(desired_each, available)
        if amount:
            issue_house_armory_to_formation(
                planner,
                formation_ref=formation_ref,
        ...
        issued[item_key] = amount
        missing = max(0, desired_each - amount)
        if missing:
            shortfalls[item_key] = missing
    return {
        "formation_ref": formation_ref,
        "desired_each": desired_each,
        "issued": issued,
        "shortfalls": shortfalls,
    }
...
def _format_issue_summary(report: Mapping[str, Any]) -> str:
    ...
    if isinstance(by_formation, Mapping):
        labels = {
            ...
            "formation_tang_wei_house_guard_first": "House Guard",
        }
        for formation_ref in _FORMATION_REFS:
            ...
            parts.append(
                ...
                f"{kit_count} exact spare equipment items are transferred from House armory reserve into formation staging custody."
            )
    shortfalls = report.get("shortfalls", [])
    if isinstance(shortfalls, list) and shortfalls:
        parts.append("Unfilled exact reserve shortfalls remain recorded for normal House production or later lawful procurement: " + "; ".join(str(value) for value in shortfalls[:12]) + ".")
    else:
...
def issue_house_field_preparation_package(
    ...
    *,
    ...
    prep["manufacturing_truth"] = (
        "House Tang has a registered resource-bounded monthly armory/remount production owner. Current departure issue uses only exact stock already on hand; any recorded shortfall remains for later normal production or lawful procurement."
    )

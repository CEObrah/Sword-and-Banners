...
_FORCE_PATHS = {
    "iron_wall": "state/forces/bastion-iron-wall.json",
    "red_thunder": "state/forces/bastion-red-thunder.json",
    "white_blade": "state/forces/bastion-white-blade.json",
    "stone_spear": "state/forces/bastion-stone-spear.json",
}
...
class BastionPersonnelLifecycleMixin:
    ...
    def _bastion_relocate_outside_applicants(
        ...
        *,
        ...
        infrastructure_sites = self.read(_INFRA_PATH).get("sites", {})
        demographic_support = infrastructure_sites.get(_SITE, {}) if isinstance(infrastructure_sites, Mapping) else {}
        residence_policy = demographic_support.get("residence_policy", {}) if isinstance(demographic_support, Mapping) else {}
        residence_site_ref = str(residence_policy.get("resident_site_ref", _SITE)) if isinstance(residence_policy, Mapping) else _SITE
        support = infrastructure_sites.get(residence_site_ref, {}) if isinstance(infrastructure_sites, Mapping) else {}
        ...
        if headroom <= 0:
            raise ValueError("Tang Manor residence site has no physical resident-support headroom for outside applicants")
    ...
    def _bastion_start_pipeline(self, corps_key: str, force: MutableMapping[str, Any], at: str) -> int:
        ...
        if outside_considered:
            ...
            for arrival in app_row.get("arrival_history", []) if isinstance(app_row.get("arrival_history"), list) else []:
                ...
                need_refs -= take
            app_row["arrival_history"] = [
                arrival for arrival in app_row.get("arrival_history", [])
                if isinstance(arrival, Mapping) and int(arrival.get("unconsidered_applicants", arrival.get("count", 0)) or 0) > 0
            ]
    ...
    def _bastion_training_focus(self, role: str, policy: Mapping[str, Any]) -> tuple[list[str], list[str]]:
        ...
        if not isinstance(focus, Mapping):
            return ["Formation Fighting", "Shield", "Athletics"], ["Endurance", "Coordination", "Composure"]

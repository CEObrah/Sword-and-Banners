...
class BastionPersonnelLifecycleMixin:
    ...
    def _bastion_admit_from_row(self, corps_key: str, force: MutableMapping[str, Any], row: MutableMapping[str, Any], count: int, at: str) -> int:
        ...
        deliberate_hours = months * float(regimen.get("deliberate_hours_per_30d", 168.0))
        ...
        for role, target in sorted(role_counts.items()):
            need = int(target)
            skills, attrs = self._bastion_training_focus(role, policy)
            for candidate in slices:
                ...
                if isinstance(cohort, MutableMapping):
                    advance_cohort_training(
                        ...
                        role_exposure_hours=exposure_hours,
                        ...
                        practice_mode="drill",
                        evidence_ref=f"{row.get('pipeline_ref')}:qualification:{role}",
                    )

...
class LivingWorldSwordPlanner(RepositoryCommandPlanner):
    ...
    def _prune_formation_memory(cls, memory: Dict[str, Any]) -> None:
        ...
        for formation_ref, row in formations.items():
            ...
            activity = (
                ...
                + int(row.get("deployments", 0))
                + int(row.get("training_reviews", 0))
            )
    ...
    def _role_doctrine_defaults(role: str) -> tuple[int, int, str]:
        ...
        if any(token in key for token in ("cavalry", "mounted", "chariot")):
            return 70, 35, "moderate"
        if any(token in key for token in ("siege", "engineer", "artillery")):
            return 35, 45, "low"
        if any(token in key for token in ("missile", "archer", "crossbow", "signal", "logistics")):
            return 55, 40, "low"
        return 50, 30, "moderate"
    ...
    def _derive_doctrine_behavior(self, formation: Mapping[str, Any]) -> Dict[str, Any]:
        ...
        weighted_reserve = 0
        weighted_withdrawal = 0
        low_tolerance = 0
        for role, count in roles.items():
            reserve, withdrawal, tolerance = self._role_doctrine_defaults(role)
            weighted_reserve += reserve * count
            weighted_withdrawal += withdrawal * count
            if tolerance == "low":
                low_tolerance += count
        defaults = {
            "casualty_tolerance": "low" if low_tolerance * 2 > total else "moderate",
            "reserve_commitment": int(round(weighted_reserve / total)) if roles else 50,
            "withdrawal_threshold": int(round(weighted_withdrawal / total)) if roles else 30,
        }
    ...
    def _update_formation_review_memory(
        ...
        *,
        ...
        row["role_mix"] = self._formation_roles(formation)
        row["dominant_role"] = self._formation_role(formation)
        ...
        row["last_review_at"] = str(memory.get("as_of") or "")
        ...
        *,
        ...
        return {
            ...
            "supply_plan": {
                ...
                "formation_logistics_at_review": snapshots,
                "resupply_rule": "physical transfer or convoy before consumption; no synthetic replenishment",
            },
            "opposition_ref": threat_ref,
            "intelligence_basis": {
                "state_known_threat_ref": threat_ref,
                "exact_information_refs": sorted(set(intelligence_refs)),
                "knowledge_rule": "operation may act only on saved state knowledge and later delivered exact information",
            },
            "review_by": review_by,
            "victory_criteria": [

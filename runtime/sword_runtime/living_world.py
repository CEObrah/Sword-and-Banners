    ...
    def _autonomy_state(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        for index, (severity, threat_ref, threat_value) in enumerate(material[:capacity]):
            ...
            if existing_operation is not None:
                path, operation0 = existing_operation
                op = _deepcopy(operation0)
        ...
        previous_active = [
            (ref, path, op)
            for ref, path, op in iter_exact_operation_records(self)
            if ref.startswith(f"operation_auto_{state}_")
            and op.get("autonomous") is True
            and str(op.get("status", "")) in {"planned", "mobilizing", "active", "engaged", "occupied"}
        ]
        for op_ref, path, operation0 in previous_active:

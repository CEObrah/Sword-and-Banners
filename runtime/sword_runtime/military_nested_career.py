...
When the authorized officer already commands a formation with a saved deputy and
that formation is a direct Unit of a higher army, the promotion may elevate the
existing commander/deputy pair into a new zero-body nested army instead of hunting
for an unrelated vacant formation. The original formation becomes Unit 1 and is
left two bodies understrength because its new top commander/deputy are promoted
from already-counted embedded cadre.
"""
...
class MilitaryNestedCareerMixin:
    def _career_nested_army_promotion(self, petition_ref: str, at: str) -> bool:
        ...
        if not isinstance(deputy_ref, str) or not isinstance(parent_ref, str) or not parent_ref.startswith("cmdgrp."):
            return False
        ...
        command = SimpleNamespace(actor_id=officer_ref, digest=digest, command_type="career_nested_army_promotion")
        ...
        petition["promotion_result"] = {
            ...
            "new_formation_deputy_ref": result.get("new_formation_deputy_ref"),
            "rule": "authorized commander/deputy rise together; original formation becomes Unit 1 and no replacement bodies are spawned",
        }

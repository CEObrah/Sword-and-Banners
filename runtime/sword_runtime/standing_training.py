...
class StandingTrainingSettlementMixin:
    ...
    def _formation_weekly_training_rate(self, formation: Mapping[str, Any]) -> float:
        ...
        regimen_name = (
            ...
            if str(force.get("owner_id")) in {"force_house_tang", "force_sword_manor"}
            else "regular_army"
        )
    ...
    def _consume_player_standing_credit(self, current: CampaignTime, request_id: str) -> dict[str, Any]:
        ...
        if not player_location:
            scene = self.read_optional("state/scene.json") if hasattr(self, "read_optional") else None
            if isinstance(scene, Mapping):
                player_location = str(scene.get("location_ref", scene.get("location", "")) or "")
        ...
        instructor_contexts = instructor_contexts_for_program(
            ...
            trainee_skills=(player.get("skills", {}) if isinstance(player.get("skills"), Mapping) else {}),
        ...
        started_at = training_window_start
        history = player.setdefault("training_history", [])
        if not isinstance(history, list):
            raise ValueError("player training history is invalid")
        history.append({
            "started_at": started_at, "completed_at": str(current),
            "mode": "standing_downtime", "hours": consumed, "requested_hours": whole, "request_id": request_id,
            "program_ref": program_ref, "development": development, "recovery": recovery,
        })
    ...
    def _formation_capability_snapshot(self, formation_ref: str) -> dict[str, Any]:
        ...
        # or deputy. This keeps a 500 establishment represented as 495 aggregate

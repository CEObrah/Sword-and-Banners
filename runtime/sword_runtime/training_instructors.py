...
def _copy_person_for_training_time(person: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(raw_dev, Mapping):
        ...
        if isinstance(raw_ledger, Mapping):
            ...
            for list_key in ("entries", "active_entries", "active_windows"):
...
def _skills(person: Mapping[str, Any]) -> Mapping[str, Any]:
    if str(person.get("schema", "")) == "person-lite":
        ...
        return stats.get("skills", {}) if isinstance(stats.get("skills"), Mapping) else {}
    return person.get("skills", {}) if isinstance(person.get("skills"), Mapping) else {}
...
def _candidate_refs(registry: Mapping[str, Any], instructor_role: str, formation: Mapping[str, Any] | None) -> list[str]:
    ...
    if isinstance(formation, Mapping):
        ...
        cadre = structure.get("officer_cadre") if isinstance(structure, Mapping) and isinstance(structure.get("officer_cadre"), Mapping) else {}
...
def _higher_command_instructor_refs(runtime: Any, formation: Mapping[str, Any] | None) -> list[str]:
    """Return the actual higher commander/deputy for a formation when routed.

    This is the army-level training link: army commander/deputy set the standard for
    their directly controlled Units, then each Unit command propagates it through its
    internal 1000/500/100 hierarchy. Missing/unrouted higher command simply contributes
    no candidate; it never fabricates an instructor.
    """
...
def _loadout_items(runtime: Any, person: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if loadout_id:
        try:
            doc = runtime.read(f"game/data/loadout-records/{loadout_id}.json")
            row = doc.get("loadout") if isinstance(doc, Mapping) and isinstance(doc.get("loadout"), Mapping) else doc
...
def _ranked_instructor_candidates(
    ...
    *,
    ...
    for ref in candidate_refs:
        ...
        training_score = max(0.0, float(skills.get("Training", 0.0) or 0.0))
        domain_score = _mean_skill(skills, domains)
        quality = instructor_quality_factor(
            training_rules,
            instructor_training=training_score,
    ...
    return sorted(
        candidates,
        key=lambda row: (-float(row["quality_factor"]), -float(row["training_score"]), str(row["instructor_ref"])),
    )
...
def best_instructor_for_drill(
    ...
    *,
    ...
    # Their quality is bounded by the formation/cohort's own saved Training/domain means;
    # this is not a new manpower class or a fabricated specialist slot.
    internal_training = max(0.0, float(trainee_skills.get("Training", 80.0) or 0.0))
    internal_domain = trainee_domain
    quality = instructor_quality_factor(
        training_rules,
        instructor_training=internal_training,
    ...
    return {
        ...
        "training_score": round(internal_training, 3),
...
def distributed_instructor_capacity(
    *,
    ...
    Formation size never creates a synthetic instructor penalty. Unit commander/deputy
    ...
    structure = formation.get("command_structure") if isinstance(formation.get("command_structure"), Mapping) else {}
    cadre = structure.get("officer_cadre") if isinstance(structure, Mapping) and isinstance(structure.get("officer_cadre"), Mapping) else {}
    active = cadre.get("active_billets") if isinstance(cadre, Mapping) and isinstance(cadre.get("active_billets"), Mapping) else {}
    ...
    unit_command = structure.get("unit_command") if isinstance(structure, Mapping) and isinstance(structure.get("unit_command"), Mapping) else {}
    ...
    def _top_post_staffed(ref_key: str, post_key: str) -> bool | None:
        ...
        # commander/deputy whose individual sheet has simply not been materialized.
    ...
    top_states = (_top_post_staffed("commander_ref", "commander_post"), _top_post_staffed("deputy_ref", "deputy_post"))
    known = [state for state in top_states if state is not None]
    if known:
        present = sum(1 for state in known if state)
        expected = len(known)
        if expected >= 2 and present == 1:
            coverage *= 0.85
        elif present == 0:
            coverage *= 0.5
...
def instructor_contexts_for_program(
    ...
    *,
    ...
    cannot overlap for free. Military mass training is different: commander/deputy
    ...
    for allocation in allocations:
        ...
        if reserve_duty and evidence_ref and window_start and window_end and allocation_hours > 0.0:
            ...
            if mass_chain:
                ...
                # The commander/deputy can set one standard for every subordinate Unit

...
def record_attributed_speech(
    ...
    *,
    ...
    # Evidence access must survive a later session membership change. Only
    # exact participants physically present when this line was spoken witnessed
    # it. Legacy speech without this field establishes only the speaker's access.
    index = _read_optional(writer, 'state/index/owner-index.json') or {}
    owners = index.get('owners', {})
    witnesses = []
    for ref in sorted(active_participants):
        path = owners.get(ref)
        person = _read_optional(writer, path) if isinstance(path, str) else None
        if isinstance(person, Mapping):
            location = person.get('current_location') or person.get('location_ref') or person.get('location')
            if location == active.get('location_ref'):
                witnesses.append(ref)
    row['witness_refs'] = witnesses

...
def _append_history_row(writer: Any, row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    recent_duplicate = any(_history_row_ref(x) == record_ref for x in recent if isinstance(x, Mapping) and (x.get("speech_ref") or x.get("fact_ref")))
    ...
    duplicate = any(_history_row_ref(x) == record_ref for x in records if isinstance(x, Mapping) and (x.get("speech_ref") or x.get("fact_ref")))
    if duplicate:
        return record
...
__all__ = [
    "ACTIVE_SESSION_PATH", "CLOSE_REASONS", "HISTORY_HEAD_PATH", "INTERACTION_CLOSED_HISTORY_LIMIT", "SESSION_KINDS", "SPEECH_KINDS", "SCENE_FACT_KINDS",
    ...
    "history_period_ref", "inspect_scene_history", "recent_scene_history", "scene_history_record", "record_attributed_speech", "record_scene_fact",

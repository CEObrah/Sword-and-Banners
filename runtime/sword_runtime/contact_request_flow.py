...
from sword_runtime.campaign_communications import (
    ...
    ensure_recipient_message_delivery,
...
def _digest(prefix: str, contact_ref: str) -> str:
    return hashlib.sha256(f"{prefix}|{contact_ref}".encode("utf-8")).hexdigest()[:20]


def _request_ids(contact_ref: str) -> tuple[str, str]:
    d = _digest("institutional-contact", contact_ref)
    return f"host_contact_request_{d}", f"event_contact_request_{d}"


def _response_ref(contact_ref: str) -> str:
    return f"event_contact_audience_{_digest('institutional-contact-response', contact_ref)}"


def _disposition_ids(contact_ref: str) -> tuple[str, str]:
    d = _digest("institutional-disposition", contact_ref)
    return f"host_audience_disposition_{d}", f"event_audience_disposition_{d}"


def _disposition_response_ref(contact_ref: str) -> str:
    return f"event_petition_response_{_digest('institutional-disposition-response', contact_ref)}"


def _followup_ids(contact_ref: str) -> tuple[str, str]:
    d = _digest("institutional-followup", contact_ref)
    return f"host_institutional_followup_{d}", f"event_institutional_followup_{d}"


def _followup_response_ref(contact_ref: str) -> str:
    return f"event_institutional_followup_response_{_digest('institutional-followup-response', contact_ref)}"
...
def _route_for_attempt(planner: Any, attempt: Mapping[str, Any]) -> Mapping[str, Any] | None:
    ...
    for route in _routes(planner):
        if route.get("location_ref") == location_ref and route.get("arc_ref") == arc_ref:
            ...
            # seek_contact names a physical receiving venue. Merely naming a
            # remote place cannot create presence there; remote correspondence
            # must use a message/request path with courier travel.
            if origin_location != location_ref:
                return None
            return route
    return None
...
def _attempt_origin_location(planner: Any, attempt: Mapping[str, Any]) -> str:
    origin = attempt.get("origin_location_ref")
    if not isinstance(origin, str) or not origin:
        origin = player_command_location(planner)
    if not isinstance(origin, str) or not origin:
        raise ValueError("institutional request lost Tang Wei's physical origin")
    return origin


def _followup_source_location(planner: Any, spec: Mapping[str, Any], process: Mapping[str, Any]) -> str:
    # source_location_actor_ref is routing-only authority. It may locate a
    # moving staff/HQ channel without falsely attributing the response itself
    # to that commander.
    candidates = (
        spec.get("source_location_actor_ref"),
        spec.get("source_actor_ref"),
        spec.get("actor_ref"),
        process.get("actor_ref"),
        spec.get("source_owner_ref"),
    )
    for endpoint_ref in candidates:
        location = command_endpoint_location(planner, endpoint_ref)
        if location:
            return location
    raise ValueError("institutional follow-up lacks a physical response endpoint")
...
def _score_disposition(planner: Any, process_ref: str, spec: Mapping[str, Any]) -> tuple[str, int, int]:
    ...
    skills = merged_skill_map(player) if isinstance(player, Mapping) else {}
...
def _settle_contact_request(planner: Any, host: Mapping[str, Any], at: str) -> str:
    if not ensure_player_message_delivery(planner, host, at):
        return ""
    required = ("contact_ref", "institution_ref", "route_ref", "source_process_ref", "audience_summary", "delivery_route")
    ...
    event_ref = _response_ref(str(host["contact_ref"]))
    return _write_player_event(planner, event_ref, {
        ...
        "basis_goal": f"Receiving access for player contact request {host['contact_ref']}"[:500],
...
def _settle_audience_disposition(planner: Any, host: Mapping[str, Any], at: str) -> str:
    ...
    event_ref = _disposition_response_ref(str(host.get("contact_ref", "")))
    return _write_player_event(planner, event_ref, {
        "event_ref": event_ref,
...
def _settle_institutional_followup(planner: Any, host: Mapping[str, Any], at: str) -> str:
    required = ("contact_ref", "source_process_ref", "source_owner_ref", "response_summary", "delivery_route", "response_stage")
    if any(not isinstance(host.get(k), str) or not host.get(k) for k in required):
        raise ValueError("institutional follow-up host is invalid")
    if host.get("route_domain") == "message_reply_receipt":
        recipient_ref = str(host.get("actor_ref") or host.get("source_owner_ref") or "")
        if not recipient_ref:
            raise ValueError("message reply receipt lost its exact recipient")
        if not ensure_recipient_message_delivery(planner, host, at, recipient_ref=recipient_ref):
            return ""
        event_ref = _followup_response_ref(str(host["contact_ref"]))
        return _write_player_event(planner, event_ref, {
            "event_ref": event_ref,
            ...
            "target_ref": recipient_ref,
            "basis_goal": f"Physical receipt of Tang Wei's reply to {host['source_process_ref']}"[:500],
            "process_kind": "message_reply_receipt",
            "process_stage": "reply_received",
            "source_event_ref": host["source_process_ref"],
            "summary": str(host["response_summary"])[:4000],
            "visibility_class": "hidden",
            "delivery": {
                "target_ref": recipient_ref,
                "location_ref": str(host.get("recipient_target_location_ref", "")),
                "route": str(host["delivery_route"])[:1000],
                "communication_travel_seconds": host.get("communication_travel_seconds"),
            },
            "provenance": {
                "kind": "causal_runtime_settlement",
    ...
    row: dict[str, Any] = {
        ...
        "triggered_at": at,
        "target_ref": "char_tang_wei",
        "basis_goal": f"Institutional answer to Tang Wei follow-up request {host['contact_ref']}"[:500],
...
class ContactRequestFlowMixin:
    ...
    def _sync_contact_request_routes(self, runtime: dict[str, Any]) -> None:
        ...
        for attempt in attempts:
            contact_ref, requested_at = interaction_attempt_ref(attempt), attempt.get("at")
            if not isinstance(requested_at, str):
                continue
            response_target = _attempt_origin_location(self, attempt)
            route = _route_for_attempt(self, attempt)
            if route is not None and get_causal_event_from_reader(self, _response_ref(contact_ref)) is None:
                institution_location = str(route.get("location_ref") or "")
                processing_seconds = int(route["delay_seconds"])
                due = max(current, CampaignTime.parse(requested_at).add_seconds(processing_seconds))
                h, e = _request_ids(contact_ref)
                self._schedule_one_shot(runtime, host_id=h, event_id=e, kind="contact_request", priority=46, due=due, row={
                    "owner_ref": route["institution_ref"], "contact_ref": contact_ref, "source_event_id": attempt.get("event_id"),
                    ...
                    "request_origin_location_ref": response_target,
                    ...
                    "communication_rule": "contact requires physical presence; institutional processing is not later player receipt, so a response must physically reroute if Tang Wei leaves",
                    "resolved_through": str(current if current < due else due.add_seconds(-1)),
                })

            match = _match_disposition(self, attempt)
            if match is not None and get_causal_event_from_reader(self, _disposition_response_ref(contact_ref)) is None:
                ...
                origin_location = _attempt_origin_location(self, attempt)
                institution_location = command_endpoint_location(self, droute.get("institution_ref")) or str(droute.get("location_ref") or "")
                if not institution_location:
                    raise ValueError("audience disposition lacks a physical institutional endpoint")
                courier = command_message_route(self.read, origin_location, institution_location, round_trip=True)
                travel_seconds = max(0, int(courier.get("travel_seconds", 0) or 0))
                due = max(current, CampaignTime.parse(requested_at).add_seconds(travel_seconds + delay))
                h, e = _disposition_ids(contact_ref)
                ...
                if isinstance(existing, dict):
                    ...
                    self._schedule_one_shot(runtime, host_id=h, event_id=e, kind="audience_disposition", priority=47, due=due, row={
                        "owner_ref": droute["institution_ref"], "contact_ref": contact_ref, "source_event_id": attempt.get("event_id"),
                        ...
                        "institution_processing_seconds": delay,
                        "courier_route": copy.deepcopy(dict(courier)),
                        "communication_rule": "request and response use physical geography; institutional processing never substitutes for courier travel",
                        "resolved_through": str(current if current < due else due.add_seconds(-1)),
                    })

            followup = _match_followup(self, attempt)
            if followup is None or get_causal_event_from_reader(self, _followup_response_ref(contact_ref)) is not None:
                continue
            ...
            origin_location = _attempt_origin_location(self, attempt)
            source_location = _followup_source_location(self, spec, process)
            courier = command_message_route(self.read, origin_location, source_location, round_trip=True)
            travel_seconds = max(0, int(courier.get("travel_seconds", 0) or 0))
            due_raw = CampaignTime.parse(requested_at).add_seconds(travel_seconds + delay)
            due = max(current, due_raw)
            h, e = _followup_ids(contact_ref)
            self._schedule_one_shot(runtime, host_id=h, event_id=e, kind="institutional_followup", priority=48, due=due, row={
                ...
                "institution_processing_seconds": delay,
                "courier_route": copy.deepcopy(dict(courier)),
                "communication_rule": "dispatch is not receipt; request and institutional answer travel through physical geography",

...
def materialize_reconciled_campaign_follow_on_orders(
    ...
    routes = index.get("operations") if isinstance(index, Mapping) else None
    if not isinstance(at, str) or not isinstance(routes, Mapping):
        return []

    created: list[str] = []
    for operation_ref in dict.fromkeys(str(ref) for ref in operation_refs if isinstance(ref, str) and ref):
        path = routes.get(operation_ref)

...
from sword_runtime.smart_training import (
    role_core_skills,
...
def project_person_lite_stats(
    ...
    *,
    ...
    attr_sd = cohort.get("attribute_sd", {}) if isinstance(cohort.get("attribute_sd"), Mapping) else {}
    skills = cohort.get("skill_means", {}) if isinstance(cohort.get("skill_means"), Mapping) else {}
    skill_sd = cohort.get("skill_sd", {}) if isinstance(cohort.get("skill_sd"), Mapping) else {}
    projected = {
        "attributes": {
            key: _sample_metric(cohort, person_ref=person_ref, kind="attribute", key=key, mean=float(attrs.get(key, 50.0)), sd=float(attr_sd.get(key, 8.0)))
            for key in ATTRIBUTE_ORDER
        } if attrs else {},
        "skills": {
            key: _sample_metric(cohort, person_ref=person_ref, kind="skill", key=key, mean=float(skills.get(key, 0.0)), sd=float(skill_sd.get(key, 4.0)))
    ...
    command_skills = {
        ...
        "Mass Combat", "Training",
    }
    command_attrs = {"Awareness", "Composure", "Coordination", "Intelligence", "Presence"}
    mounted = str(loadout_id or "") == "loadout_house_champion"
    loadout_skills = {"Spear", "Shield", "Sword", "Bow", "Defense", "Formation Fighting"}
    if mounted:
    ...
    for key in command_skills:
        ...
        sd = max(0.0, float(skill_sd.get(key, 0.0)))
    ...
    for key in command_attrs:
        ...
        sd = max(0.0, float(attr_sd.get(key, 0.0)))
    ...
    # of becoming accidental specialties. Lance is handled as Spear by the combat
    # weapon-family mapping.
    primary_keys = ["Spear", "Sword", "Shield", "Bow"] + (["Riding"] if mounted else [])
    ...
    if not mounted and "Riding" in out_skills:
        ...
        sd = max(0.0, float(skill_sd.get("Riding", 0.0)))
        out_skills["Riding"] = min(
            int(out_skills["Riding"]),
            int(round(mean + sd)),
            secondary_role_ceiling,
        )
    for key in {"Axe", "Crossbow", "Dagger", "Glaive", "Mace", "Staff"}:
        if key not in out_skills:
            continue
        mean = float(skills.get(key, 0.0))
        sd = max(0.0, float(skill_sd.get(key, 0.0)))
...
class CohortTxSupportMixin:
    ...
    @staticmethod
    def _ct_command_refs(formation: Mapping[str, Any]) -> tuple[dict[str, str], dict[str, str]]:
        ...
        saved_unit = saved_structure.get("unit_command", {}) if isinstance(saved_structure.get("unit_command"), Mapping) else {}
        add(saved_unit.get("commander_ref"), "persistent_unit_commander")
        add(saved_unit.get("deputy_ref"), "persistent_unit_deputy")
        saved_internal = saved_structure.get("internal_person_refs", []) if isinstance(saved_structure.get("internal_person_refs"), list) else []
        if not saved_internal and isinstance(formation.get("embedded_person_refs"), list):
            saved_internal = formation.get("embedded_person_refs", [])
        for ref in saved_internal:
            ...
            add(ref, role)
        cells = formation.get("unit_command_cells", [])
        if isinstance(cells, list):
            for cell in cells:
                ...
                for ref in cell.get("internal_1000_commander_refs", []) if isinstance(cell.get("internal_1000_commander_refs"), list) else []:
                    add(ref, "internal_1000_commander")
                for ref in cell.get("internal_500_commander_refs", []) if isinstance(cell.get("internal_500_commander_refs"), list) else []:
                    add(ref, "internal_500_commander")
        return lite, exact
        ...
        *,
        ...
        for person_ref, role_label in sorted(exact_refs.items()):
            ...
            if str(health).lower() not in {"healthy", "fit", "stable"}:
                ...
                continue
            contract = {"focus": role_core_skills(role_label)}
            ds = person.setdefault("development_state", {})
            cursor = max(0, int(ds.get("formation_smart_training_cursor", 0)))
            selected: list[str] = []
            probe = cursor
            for _ in range(min(6, len(contract["focus"]))):
                focus = select_exact_focus(person, contract, probe)
                probe += 1
                if focus and focus not in selected:
                    selected.append(focus)
            if not selected:
                results.append({"person_ref": person_ref, "trained": False, "reason": "no_trainable_focus"})
                continue
            base, remainder = divmod(whole, len(selected))
            development: list[dict[str, Any]] = []
            for index, focus in enumerate(selected):
                focus_hours = base + (1 if index < remainder else 0)
                if focus_hours <= 0:
                    continue
                development.append(
                    settle_training_session(person, focus, focus_hours, current, training_rules, session_rules)
                )
            ds["formation_smart_training_cursor"] = cursor + whole
            history = person.setdefault("training_history", [])
            if isinstance(history, list):
                history.append({
                    "completed_at": str(current),
                    "mode": "delegated_formation_standing_training",
                ...
                person["training_history"] = history[-64:]
            self.put(path, person)
            results.append({"person_ref": person_ref, "trained": True, "focuses": selected, "development": development})
        return results
    ...
    def _ct_train_formation(self, ref: str, hours: float, evidence: str) -> None:
        ...
        regimen_name = "house_tang_max_sustainable" if str(force.get("owner_id")) in {"force_house_tang", "force_sword_manor"} else "regular_army"
        ...
        for item in formation.get("cohort_composition", []):
            ...
            if cohort.get("attribute_means") or cohort.get("skill_means"):
                cursor = max(0, int(cohort.get("smart_training_cursor", 0)))
                skill_focuses, attribute_focuses = select_cohort_focuses(
                    cohort,
                    role=role,
                    role_profile=role_profile if isinstance(role_profile, Mapping) else {},
                    promotion_facts=None,
                    cursor=cursor,
                    skill_slots=6,
                    attribute_slots=3,
                )
                ...
                cohort["smart_training_cursor"] = cursor + 1
                history = cohort.setdefault("smart_training_history", [])
                if isinstance(history, list):
                    history.append({
                        "evidence_ref": evidence,
                        "skill_focuses": skill_focuses,
                        "attribute_focuses": attribute_focuses,
                        "deliberate_hours": round(float(hours), 3),
                    })
                    cohort["smart_training_history"] = history[-24:]
            else:
        ...
        if officer_results or exact_results:
            formation["delegated_training_last_officer_results"] = {
                "evidence_ref": evidence,
                "person_lite": officer_results,
                "exact_command_staff": exact_results,
            }
        self.put(force_path, force)
        self.put(path, formation)

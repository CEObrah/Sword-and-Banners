...
class CohortTxSupportMixin:
    def _ct_force(self, path: str) -> dict[str, Any]:
        ...
        ensure_cohort_ledger(force, at=str(self._world_time()))
        if hasattr(self, "_seed_house_tang_baselines"):
    ...
    def _ct_formation(self, ref: str) -> tuple[str, dict[str, Any], str]:
        ...
        ensure_formation_composition(force, formation, at=str(self._world_time()))
    ...
    def _ct_materialize_from_cohort(self, force: dict[str, Any], role: str, location: str, person_ref: str, person: dict[str, Any]) -> None:
        ...
        if cohort.get("attribute_means") and not person.get("attributes"):
            person["attributes"] = {str(k): max(0, int(round(float(v) + NormalDist().inv_cdf(min(.999999, max(.000001, stable_fraction(person_ref, "attribute", k)))) * float(cohort.get("attribute_sd", {}).get(k, 8.0))))) for k, v in cohort.get("attribute_means", {}).items()}
        if cohort.get("skill_means") and not person.get("skills"):
            person["skills"] = {str(k): max(0, int(round(float(v) + NormalDist().inv_cdf(min(.999999, max(.000001, stable_fraction(person_ref, "skill", k)))) * float(cohort.get("skill_sd", {}).get(k, 10.0))))) for k, v in cohort.get("skill_means", {}).items()}

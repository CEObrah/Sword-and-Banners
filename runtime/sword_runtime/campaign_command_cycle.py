...
from sword_runtime.campaign_report_projection import pending_upward_report_count, select_pending_upward_reports, select_recent_delivered_upward_reports
...
from sword_runtime.campaign_communications import (
    command_message_route, command_person_location, queue_upward_report, settle_upward_report_delivery,
)
...
from sword_runtime.scene_sessions import active_scene_session, close_active_scene, start_scene_session
from sword_runtime.operation_routing import current_operational_order, current_operational_order_slot, exact_operation_record
...
class _ReadAdapter:
    ...
    def _load_formation(self, formation_ref: str) -> tuple[str, dict[str, Any]]:
        """Provide the minimal formation-loader contract used by command grouping.

        Read-only play-context projections may arrive through a repository store
        rather than the production planner.  Formation ownership is still exact
        in the owner index, so adapt that authority instead of requiring a
        mutating planner merely to identify the local field commander.
        """
        path = self.owner_path(formation_ref)
        raw = self.read(path)
        if not isinstance(raw, Mapping):
            raise ValueError(f"invalid formation owner: {formation_ref}")
...
def _latest_order(operation: Mapping[str, Any]) -> Mapping[str, Any] | None:
    return current_operational_order(operation)
...
def _person(planner: Any, ref: str | None) -> tuple[str, dict[str, Any]] | None:
    ...
    if not isinstance(row, Mapping):
        return None
    # Scene/person routing follows the authoritative owner + schema rather than
    # an ID spelling convention. Formal appointment rules can still restrict
    # specific offices independently of this identity helper.
    is_full_character = path == "state/player.json" or str(path).startswith("state/char/")
    is_materialized_person = (
        str(path).startswith("state/person/")
        and str(row.get("schema", "")) in {"sab_character", "person-lite", "sword-materialized-person"}
    )
    if not (is_full_character or is_materialized_person):
        return None
...
def _operation_commander_refs(planner: Any, operation: Mapping[str, Any], *, include_nested: bool) -> list[str]:
    ...
    if isinstance(root_ref, str) and root_ref:
        ...
        if not isinstance(group, Mapping):
            raise ValueError(f"campaign command group unresolved: {root_ref}")
        commander_ref = group.get("commander_ref")
        if isinstance(commander_ref, str) and commander_ref:
            if _person(planner, commander_ref) is None:
                raise ValueError(f"campaign commander unresolved: {commander_ref}")
            refs.append(commander_ref)
    if include_nested:
        formation_rows = operation.get("formation_refs", [])
        if not isinstance(formation_rows, list):
            raise ValueError("campaign operation formation_refs is malformed")
        for formation_ref in formation_rows:
            if not isinstance(formation_ref, str) or not formation_ref:
                raise ValueError("campaign operation formation ref is malformed")
            ...
            if isinstance(group, Mapping) and isinstance(group.get("commander_ref"), str):
                ...
                if _person(planner, commander_ref) is None:
                    raise ValueError(f"campaign commander unresolved: {commander_ref}")
                refs.append(commander_ref)
                # Strategic participant identity stays at the parent command
                # echelon.  Exact detached formation commanders are projected
                # separately by _local_field_command_posts so they are visible
                # where physically present without being inflated into peer
                # campaign principals or automatically summoned to councils.
                continue
            formation = planner.read(planner.owner_path(formation_ref))
            if not isinstance(formation, Mapping):
                raise ValueError(f"campaign formation unresolved: {formation_ref}")
            commander = formation.get("commander_ref")
            if isinstance(commander, str) and commander:
                if _person(planner, commander) is None:
                    raise ValueError(f"campaign commander unresolved: {commander}")
...
def _campaign_participants(planner: Any, operation: Mapping[str, Any]) -> tuple[list[str], list[str]]:
    ...
    if isinstance(own_root, str) and own_root:
        ...
        if not isinstance(group, Mapping):
            raise ValueError(f"campaign command group unresolved: {own_root}")
        commander_ref = group.get("commander_ref")
        if isinstance(commander_ref, str) and commander_ref:
            if _person(planner, commander_ref) is None:
                raise ValueError(f"campaign commander unresolved: {commander_ref}")
            commanders.append(commander_ref)
    ...
    for ref in operation_refs:
        ...
        if not isinstance(path, str):
            raise ValueError(f"campaign participant operation unresolved: {ref}")
        row = planner.read(path)
        if not isinstance(row, Mapping):
            raise ValueError(f"campaign participant operation invalid: {ref}")
        if str(row.get("status", "")) not in _ACTIVE_OPERATION_STATUSES:
            continue
...
def _person_location_ref(planner: Any, person_ref: object) -> str | None:
    row = _person(planner, person_ref) if isinstance(person_ref, str) else None
    if row is None:
        return None
    person = row[1]
...
def _local_field_command_posts(planner: Any, operation: Mapping[str, Any], *, venue_ref: str) -> list[dict[str, Any]]:
    """Project exact subordinate command bodies physically present at the field HQ site.

    A campaign participant may be split across several locations.  The famous
    principal's absence therefore does not erase the real formation commander
    who is physically standing with a detached body at Tang Wei's location.
    """
    ...
    for participant_ref in participant_refs:
        resolved = exact_operation_record(planner, participant_ref)
        if resolved is None:
            continue
        _path, participant = resolved
        if not isinstance(participant, Mapping) or str(participant.get("status", "")) not in _ACTIVE_OPERATION_STATUSES:
            continue
        principal_refs = _operation_commander_refs(planner, participant, include_nested=False)
        formation_refs = participant.get("formation_refs", []) if isinstance(participant.get("formation_refs"), list) else []
        for formation_ref in formation_refs:
            ...
            try:
                ...
                continue
            if not isinstance(formation, Mapping) or str(formation.get("location_ref") or "") != venue_ref:
                continue
            group = _command_group_for_formation(planner, formation_ref)
            group_ref = (group.get("id") or group.get("group_ref")) if isinstance(group, Mapping) else None
            direct = formation.get("commander_ref")
            direct_ref = direct if isinstance(direct, str) and direct else None
            group_commander = group.get("commander_ref") if isinstance(group, Mapping) else None
            group_ref_commander = group_commander if isinstance(group_commander, str) and group_commander else None
            direct_location = _person_location_ref(planner, direct_ref) if direct_ref else None
            group_commander_location = _person_location_ref(planner, group_ref_commander) if group_ref_commander else None
            # The formation's own commander is the local echelon.  Fall back to
            # the parent-group commander only when no exact formation commander
            # exists; never overwrite a co-located subordinate with a remote
            # famous principal.
            commander_ref = direct_ref or group_ref_commander
            commander_location = direct_location if direct_ref else group_commander_location
            commander_present = bool(commander_ref and commander_location == venue_ref)
            rows.append({
                "participant_operation_ref": participant_ref,
                ...
                "command_group_ref": group_ref if isinstance(group_ref, str) and group_ref else None,
                "commander_ref": commander_ref,
                "commander_location_ref": commander_location,
                "commander_physically_present": commander_present,
                "parent_command_commander_ref": group_ref_commander,
                "parent_command_commander_location_ref": group_commander_location,
                "operation_principal_commander_refs": principal_refs,
                "echelon_semantics": "physical formation command body; this local commander does not imply the participant operation's principal commander is co-located",
            })
    return sorted(rows, key=lambda row: (str(row.get("participant_operation_ref") or ""), str(row.get("formation_ref") or "")))
...
def _full_person(planner: Any, ref: object) -> tuple[str, dict[str, Any]] | None:
    if not isinstance(ref, str) or not ref:
        return None
    row = _person(planner, ref)
    if row is None:
        return None
    schema = str(row[1].get("schema", ""))
    return row if schema in {"sab_character", "sword-materialized-person"} else None


def _supreme_commander(planner: Any, operation: Mapping[str, Any]) -> str | None:
    for key in ("campaign_commander_ref", "supreme_commander_ref"):
        value = operation.get(key)
        if _full_person(planner, value) is not None:
            return value
    return None
...
def _appoint_supreme_commander_if_needed(
    ...
    *,
    ...
    if _full_person(planner, existing) is not None:
        return None
    participant_refs = {
        str(ref) for ref in cycle.get("participant_commander_refs", [])
        if _full_person(planner, ref) is not None
    }
    ...
    current_slot = current_operational_order_slot(operation)
    if current_slot is not None:
        orders, current_index = current_slot
        row = orders[current_index]
...
def _ensure_cycle(planner: Any, operation_ref: str, *, at: str) -> dict[str, Any] | None:
    ...
    supreme = _supreme_commander(planner, operation)
...
def _register_host(
    ...
    instance_ref: str | None = None, host_context: Mapping[str, Any] | None = None,
) -> None:
    ...
    if host_id not in hosts:
        ...
        if isinstance(host_context, Mapping):
            hosts[host_id].update(copy.deepcopy(dict(host_context)))
    if not any(isinstance(row, Mapping) and row.get("event_id") == event_id for row in events):
        events.append({
            "event_id": event_id,
            "kind": f"campaign_command_{phase}",
    ...
    for operation_ref in _player_operation_refs(planner):
        ...
        if isinstance(current_order, Mapping):
            ...
            if isinstance(order_ref, str) and order_ref and order_ref not in delivered_orders:
                # A prepared/issued order is not a received order. Deliver it
                # through the same geography-backed one-way command-message law
                # used by reports and explicit commander contact.
                delay_minutes = max(0, int(mechanics.get("superior_order_delivery_delay_minutes", 15) or 0))
                issued_at = current_order.get("issued_at")
                issued_time = CampaignTime.parse(issued_at) if isinstance(issued_at, str) and issued_at else current
                commander_ref = current_order.get("superior_commander_ref") or cycle.get("supreme_commander_ref")
                source_location = command_person_location(planner, commander_ref)
                player = planner.read_optional("state/player.json")
                target_location = None
                if isinstance(player, Mapping):
                    raw_target = player.get("location") or player.get("current_location") or player.get("location_ref")
                    target_location = str(raw_target) if isinstance(raw_target, str) and raw_target else None
                # Missing exact endpoints fail closed: never manufacture a fixed
                # short delivery time when physical routing is unresolved.
                if not source_location or not target_location:
                    continue
                try:
                    courier_route = command_message_route(
                        planner.read, source_location, target_location, round_trip=False,
                    )
                except (FileNotFoundError, KeyError, TypeError, ValueError):
                    continue
                travel_seconds = max(0, int(courier_route.get("travel_seconds", 0) or 0))
                due_text = str(issued_time.add_seconds(travel_seconds + delay_minutes * 60))
                _register_host(
                    runtime, cycle_ref=cycle_ref, operation_ref=operation_ref,
                    ...
                    host_context={
                        ...
                        "communication_rule": (
                            "issued_at is not receipt; superior orders become player-available only after one-way physical courier delivery plus staff handling"
                        ),
                    },
                )
...
def _ensure_supreme_directive(
    ...
    if _full_person(planner, commander_ref) is None:
        return None
    op_path, operation = _load_operation(planner, operation_ref)
...
def _player_hq_people(planner: Any, *, location_ref: str) -> list[str]:
    ...
    if isinstance(root, Mapping):
        ...
        if isinstance(commander, str) and _person(planner, commander) is not None:
            refs.append(commander)
        roles = root.get("role_assignments") if isinstance(root.get("role_assignments"), Mapping) else {}
        refs.extend(str(ref) for ref in roles if isinstance(ref, str) and _person(planner, ref) is not None)
...
def _settle_council(planner: Any, host: Mapping[str, Any], at: str) -> dict[str, Any] | None:
    ...
    soft_end_at = str(CampaignTime.parse(at).add_seconds(duration_hours * 3600))
    if _PLAYER_REF in set(present):
        start_scene_session(
            planner,
            session_ref=f"scene_session_{_digest('council_session', str(cycle['cycle_ref']) + '|' + at)}",
            kind="war_council",
            location_ref=venue_ref,
            participant_refs=present,
            started_at=at,
            process_ref=str(cycle["cycle_ref"]),
            purpose="Qin campaign command conference",
            agenda=[
                "command hierarchy", "current intelligence", "army assignments",
                "routes and supply", "reserve responsibilities", "entry authority",
                "standing order for the next campaign phase",
            ],
            soft_end_at=soft_end_at,
        )
        council = copy.deepcopy(dict(cycle.get("war_council", {})))
        council["scene_session_ref"] = f"scene_session_{_digest('council_session', str(cycle['cycle_ref']) + '|' + at)}"
        council["soft_end_at"] = soft_end_at
        cycle["war_council"] = council
        planner.put(path, cycle)
    runtime = copy.deepcopy(planner.read(_RUNTIME_PATH))
    _register_host(
        runtime, cycle_ref=str(cycle["cycle_ref"]), operation_ref=operation_ref,
        phase="council_return", due_at=soft_end_at,
...
def _settle_council_return(planner: Any, host: Mapping[str, Any], at: str) -> None:
    ...
    council = cycle.get("war_council") if isinstance(cycle.get("war_council"), Mapping) else {}
    venue_ref = str(cycle.get("venue_ref", ""))
    cycle_ref = str(cycle.get("cycle_ref", ""))
    active = active_scene_session(planner)
    matches_council = bool(
        isinstance(active, Mapping)
        and active.get("kind") == "war_council"
        and str(active.get("process_ref", "")) == cycle_ref
    )

    # The configured duration is a soft scheduling boundary, not an eject
    # button.  While Wei remains in a still-active council, poll the return
    # obligation conservatively rather than making the attendees disappear
    # underneath unresolved conversation.  No maximum scene duration is
    # imposed here.
    scene_engaged = bool(
        matches_council
        and (
            bool(active.get("open_thread_refs", active.get("open_question_refs", [])))
            or str(active.get("last_updated_at", "")) != str(active.get("started_at", ""))
        )
    )
    if matches_council and _person_location(planner, _PLAYER_REF) == venue_ref and scene_engaged:
        runtime = copy.deepcopy(planner.read(_RUNTIME_PATH))
        ...
        if isinstance(live_host, dict):
            live_host["recurrence_seconds"] = max(60, int(_mechanics(planner).get("council_scene_recheck_seconds", 900) or 900))
            planner.put(_RUNTIME_PATH, runtime)
        council2 = copy.deepcopy(dict(council))
        council2["return_deferred_at"] = at
        council2["return_deferred_reason"] = "active_scene_session"
    ...
    if matches_council:
        close_active_scene(
            planner, at=at,
            reason="completed" if _person_location(planner, _PLAYER_REF) == venue_ref else "player_left",
        )

    # A previously deferred host must become one-shot again once the session is
    # closed, otherwise the scheduler would keep polling after the return.
    runtime = copy.deepcopy(planner.read(_RUNTIME_PATH))
    hosts = runtime.get("hosts") if isinstance(runtime, Mapping) else None
    host_id = str(host.get("host_id", ""))
    live_host = hosts.get(host_id) if isinstance(hosts, dict) else None
    if isinstance(live_host, dict) and int(live_host.get("recurrence_seconds", 0) or 0) != 0:
        live_host["recurrence_seconds"] = 0
        planner.put(_RUNTIME_PATH, runtime)
    ...
    council2.pop("return_deferred_reason", None)
    cycle["war_council"] = council2
    cycle["updated_at"] = at
    planner.put(path, cycle)
...
def _record_upward_report(
    planner: Any, cycle: dict[str, Any], *, at: str, phase: str, snapshot: Mapping[str, Any], event_ref: str,
    source_location_ref: str, extra: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    payload = {
        "event_ref": event_ref,
        ...
        "rule": "Routine field-command reporting forwards only already-saved command facts; preparation is distinct from physical receipt and creates no tactical or superior outcome.",
    }
    if isinstance(extra, Mapping):
        payload.update(copy.deepcopy(dict(extra)))
    return queue_upward_report(
        planner, cycle, at=at, phase=phase, source_location_ref=source_location_ref, payload=payload,
    )
...
def _settle_after_action_review(planner: Any, host: Mapping[str, Any], at: str) -> dict[str, Any] | None:
    ...
    _record_upward_report(
        planner, cycle, at=at, phase="after_action", snapshot=own, event_ref=event_ref,
        source_location_ref=location_ref, extra={
            "battlefield_ref": after_action.get("battlefield_ref"),
...
def _settle_daily(planner: Any, host: Mapping[str, Any], at: str, *, phase: str) -> dict[str, Any] | None:
    ...
    if phase == "dawn":
        summary = (
            f"Dawn field-command briefing. Tang Wei's command accounts for {int(own.get('personnel', 0) or 0):,} troops organized under {int((own.get('echelon') or {}).get('primary_command_count', 0) or 0)} primary commands; current distribution: {location_text}. "
    ...
    _record_upward_report(
        planner, cycle, at=at, phase=phase, snapshot=own, event_ref=event_ref,
        source_location_ref=hq_location_ref,
    )
    cycle["updated_at"] = at
    planner.put(path, cycle)
...
def settle_campaign_command_host(planner: Any, host: Mapping[str, Any], at: str) -> dict[str, Any] | None:
    ...
    if kind == "campaign_command_report_delivery":
        return settle_upward_report_delivery(planner, host, at)
    raise ValueError(f"unsupported campaign command host kind: {kind}")


def _player_safe_upward_report_window(value: Any) -> tuple[list[dict[str, Any]], int]:
    """Keep current outbound courier work visible without turning context into report history.

    Tang Wei knows reports his headquarters has actually dispatched or failed to
    route, including response-bearing requests that remain unresolved.  Every
    later compact layer consumes the same selector so chronology-only tail slices
    cannot erase those requests or relabel delivered history as pending.
    """
    pending = select_pending_upward_reports(value, maximum=8)
    delivered = select_recent_delivered_upward_reports(value, maximum=2)
    selected_refs = {str(row.get("report_ref") or "") for row in pending}
    rows = list(pending)
    rows.extend(row for row in delivered if str(row.get("report_ref") or "") not in selected_refs)
    return rows, pending_upward_report_count(value)
...
def campaign_command_projection(planner: Any, operation_ref: str) -> dict[str, Any] | None:
    ...
    if existing is not None:
        cycle = existing[1]
        current_order = _current_superior_order(operation, cycle)
        venue_ref = str(cycle.get("venue_ref") or operation.get("location_ref") or "")
        superior_ref = cycle.get("superior_command_ref") or cycle.get("supreme_commander_ref")
        superior_location = _person_location_ref(planner, superior_ref)
        local_posts = _local_field_command_posts(planner, operation, venue_ref=venue_ref) if venue_ref else []
        projected_reports, pending_report_count = _player_safe_upward_report_window(cycle.get("upward_reports"))
        return {
            "cycle_ref": cycle.get("cycle_ref"),
            "status": cycle.get("status"),
            "venue_ref": cycle.get("venue_ref"),
            "headquarters": {
                "field_headquarters_location_ref": venue_ref or None,
                "superior_command_ref": superior_ref,
                "superior_command_location_ref": superior_location,
                "superior_physically_co_located": bool(venue_ref and superior_location == venue_ref),
                "rule": "field-army HQ location, named principal location, and detached subordinate command locations remain distinct authorities",
            },
            "local_field_command_posts": local_posts,
            "forum_kind": cycle.get("forum_kind"),
            "court_state_ref": cycle.get("court_state_ref"),
            "coordination_authority_ref": cycle.get("coordination_authority_ref"),
            ...
            "upward_reports": projected_reports,
            "pending_upward_report_count": pending_report_count,
            "current_superior_order": current_order,
            "current_superior_directive": _current_superior_directive(operation),
            "operational_intent_contract": operational_intent_contract(operation, current_order),
        }
    coordination = _coordination_authority(operation)
    supreme = _supreme_commander(planner, operation)
    participant_refs, commanders = _campaign_participants(planner, operation)
    current_order = _current_superior_order(operation, {
        "superior_command_ref": supreme or coordination,
        "coordination_authority_ref": coordination,
        "supreme_commander_ref": supreme,
    })
    venue_ref = str(operation.get("location_ref") or "")
    superior_ref = supreme or coordination
    superior_location = _person_location_ref(planner, superior_ref)
    local_posts = _local_field_command_posts(planner, operation, venue_ref=venue_ref) if venue_ref else []
    return {
        "cycle_ref": _cycle_ref(operation_ref),
...
def retain_campaign_command_decisions(rows: Any, *, recent_delivered: int = 32) -> list[dict[str, Any]]:
    """Keep every unresolved decision plus a bounded tail of delivered history.

    Delivery hosts rehydrate decisions by exact ``order_ref``. A plain list tail
    can therefore delete still-live work when a long or disrupted route overlaps
    many newer decisions. Bound only terminal history; unresolved causal work is
    never a cache entry.
    """
    valid = [row for row in rows if isinstance(row, dict)] if isinstance(rows, list) else []
    delivered = [i for i, row in enumerate(valid) if str(row.get("delivery_status", "")) == "delivered"]
    keep = set(delivered[-max(0, int(recent_delivered)):])
    keep.update(i for i, row in enumerate(valid) if str(row.get("delivery_status", "")) != "delivered")
    return [row for i, row in enumerate(valid) if i in keep]


__all__ = [
    "campaign_command_projection",
    "retain_campaign_command_decisions",

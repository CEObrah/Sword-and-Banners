...
class PrisonerSystemMixin:
    @staticmethod
    def _custody_record_history(group: MutableMapping[str, Any], row: Mapping[str, Any]) -> None:
        """Keep only bounded current custody evidence, never an append-only diary."""
    ...
    def _custody_group(self, ref: str) -> tuple[str, dict[str, Any]]:
        canonical_path = _group_path(ref)
        index = self._custody_index()
        routed_path = index.get("groups", {}).get(ref) if isinstance(index.get("groups"), Mapping) else None
        candidates = [routed_path, canonical_path] if isinstance(routed_path, str) else [canonical_path]
        seen: set[str] = set()
        for path in candidates:
            ...
            if not isinstance(row, Mapping):
                continue
            if str(row.get("schema", "")) != "sword-prisoner-group" or str(row.get("owner_id", "")) != ref:
                continue
            return path, copy.deepcopy(dict(row))
        raise ValueError("unknown prisoner group")

    def _custody_active_groups(self) -> list[tuple[str, str, dict[str, Any]]]:
        """Recover active custody routing from exact registered prisoner owners."""
        index = self._custody_index()
        candidates = {
            str(ref) for ref in index.get("groups", {})
            if isinstance(ref, str) and ref
        } if isinstance(index.get("groups"), Mapping) else set()
        candidates.update(
            str(ref) for ref in index.get("active_refs", [])
            if isinstance(ref, str) and ref
        )
        try:
        ...
        if isinstance(owners, Mapping):
            for ref, path in owners.items():
                if (
                    isinstance(ref, str) and ref.startswith("prisoners_")
                    and isinstance(path, str) and path.startswith("state/custody/groups/")
                ):
                    candidates.add(ref)
        ...
        for ref in sorted(candidates):
            try:
                path, group = self._custody_group(ref)
            except (FileNotFoundError, KeyError, ValueError):
                continue
            routes[ref] = path
            is_active = (
                str(group.get("status", "")) in {"held", "in_transit"}
                and (int(group.get("personnel", 0)) > 0 or bool(group.get("named_prisoner_refs")))
            )
            if is_active:
                active_refs.append(ref)
                rows.append((ref, path, group))
        if index.get("groups") != routes or index.get("active_refs") != active_refs:
            index["groups"] = routes
            index["active_refs"] = active_refs
    ...
    def _custody_ensure_review_host(self) -> None:
        ...
        refs=[ref for ref, _path, _group in self._custody_active_groups()]
    ...
    def _custody_release_aggregate(self, group: MutableMapping[str,Any], *, status: str, at: str) -> int:
        ...
        released=0
        if n>0:
            force_ref=str(group.get("source_force_ref","")); fp=self.owner_path(force_ref); force=copy.deepcopy(self.read(fp)); ledger=ensure_cohort_ledger(force); loc=str(group.get("location_ref",""))
            ...
            group["personnel"]=max(0,n-released)
        # A named-only custody group legitimately has zero aggregate personnel.
        # Disposition still changes the exact legal/lifecycle state of the group.
        group["status"]=status;group["released_at"]=at;group["legal_status"]=status
        return released

    def _custody_sync_named_locations(self, group: Mapping[str, Any], location_ref: str) -> None:
        """Keep each named captive's exact person owner co-located with custody.

        The prisoner group is the custody authority, but location-sensitive
        systems read exact people and the person-location routing index.  Moving
        only the group creates one body at two places, so synchronize the exact
        person aliases and custody pointer whenever the group moves.
        """
        ...
        for pref in group.get("named_prisoner_refs", []) if isinstance(group.get("named_prisoner_refs"), list) else []:
            person_ref=str(pref)
            pp,p0=self._exact_person(person_ref,active=False);person=copy.deepcopy(p0)
            self._set_person_location(person, location_ref)
            custody=copy.deepcopy(person.get("custody_state")) if isinstance(person.get("custody_state"),Mapping) else {}
            custody["status"]="prisoner";custody["prisoner_group_ref"]=group_ref;custody["location_ref"]=location_ref
            person["custody_state"]=custody
            self.put(pp,person)

    def _custody_release_named(self, group: MutableMapping[str, Any], *, status: str, at: str) -> list[str]:
        """Release all named people from one exact custody group at its location."""
        refs=[str(x) for x in group.get("named_prisoner_refs", []) if isinstance(x,str)] if isinstance(group.get("named_prisoner_refs"),list) else []
        location_ref=str(group.get("location_ref", ""))
        group_ref=str(group.get("owner_id", ""))
        for person_ref in refs:
            pp,p0=self._exact_person(person_ref,active=False);person=copy.deepcopy(p0)
            if location_ref:
                self._set_person_location(person, location_ref)
            person["custody_state"]={
                "status":status,
                "released_at":at,
                "location_ref":location_ref or None,
                "former_prisoner_group_ref":group_ref,
            }
            self.put(pp,person)
        group["named_prisoner_refs"]=[]
        group["guard_requirement"]=(max(0,int(group.get("personnel",0)))+4)//5
        return refs
    ...
    def _custody_kill_aggregate(self, group: MutableMapping[str,Any], count: int, *, at: str, reason: str) -> int:
        ...
        group["by_role"]={k:int(v) for k,v in by_role.items() if int(v)>0};group["personnel"]=max(0,int(group.get("personnel",0))-killed);group["last_death"]={"at":at,"count":killed,"reason":reason};self.put(fp,force);self._custody_debit_population_deaths(force_ref,killed);return killed

    @staticmethod
    def _custody_group_people(group: Mapping[str, Any]) -> int:
        return max(0, int(group.get("personnel", 0))) + len(group.get("named_prisoner_refs", []) if isinstance(group.get("named_prisoner_refs"), list) else [])

    def _custody_holding_site_ref(self, location_ref: str) -> str:
        location_ref = str(location_ref)
        try:
            return str(enclosing_fortification_site(self.read, location_ref) or location_ref)
        except (KeyError, FileNotFoundError, ValueError):
            return location_ref

    def _custody_existing_occupancy(
        self,
        *,
        custodian_formation_ref: str,
        holding_site_ref: str,
        exclude_group_ref: str | None = None,
    ) -> dict[str, int]:
        """Count already-held prisoners sharing guards and fixed holding space.

        Active custody groups are intentionally few and explicitly indexed. This
        bounded read prevents one formation or fortress from independently
        promising its full capacity to every group.
        """
        by_custodian = 0
        by_site = 0
        for ref, _path, group in self._custody_active_groups():
            if exclude_group_ref and ref == str(exclude_group_ref):
                continue
            if str(group.get("status", "")) not in {"held", "in_transit"}:
                continue
            people = self._custody_group_people(group)
            if people <= 0:
                continue
            if str(group.get("custodian_formation_ref", "")) == str(custodian_formation_ref):
                by_custodian += people
            group_site = str(((group.get("holding_capacity") or {}) if isinstance(group.get("holding_capacity"), Mapping) else {}).get("holding_site_ref", ""))
            if not group_site:
                group_site = self._custody_holding_site_ref(str(group.get("location_ref", "")))
            if group_site == str(holding_site_ref):
                by_site += people
        return {"custodian_people": by_custodian, "site_people": by_site}

    def _custody_holding_capacity(
        self,
        custodian_formation_ref: str,
        location_ref: str,
        *,
        ...
        """Derive available secure/emergency capacity from guards + real site geometry.

        Capacity is shared. Existing groups consume the same custodian's guard
        envelope and, at a fixed fortification, the same physical detention area.
        """
        _fp, custodian = self._load_formation(str(custodian_formation_ref))
        personnel = max(0, int(custodian.get("personnel", 0)))
        rules = self.read("game/data/mechanics/custody.json")
        guarding = rules.get("guarding", {}) if isinstance(rules, Mapping) else {}
        per_guard = max(1, int(guarding.get("prisoners_per_required_guard", 5)))
        guard_fraction = max(1, min(1000, int(guarding.get("maximum_routine_guard_duty_fraction_milli", 400))))
        max_guard_duty = max(0, int(math.floor(personnel * guard_fraction / 1000.0)))
        guard_limited = max_guard_duty * per_guard
        emergency_ratio = max(1, int(guarding.get("maximum_emergency_prisoners_per_custodian_person", 2)))
        emergency_guard_limited = personnel * emergency_ratio

        holding_site_ref = self._custody_holding_site_ref(str(location_ref))
        profile = None
        try:
            ...
            for row in profiles.get("profiles", []) if isinstance(profiles, Mapping) else []:
                if isinstance(row, Mapping) and str(row.get("site_ref", "")) == holding_site_ref:
                    profile = row
                    break
        except (KeyError, FileNotFoundError, ValueError):
            profile = None
        ...
        fixed_capacity = None
        fixed_emergency_capacity = None
        basis = "field_guard_limited_open_enclosure"
        if isinstance(profile, Mapping):
            cfg = rules.get("fixed_site_holding", {}) if isinstance(rules, Mapping) else {}
            baseline = profile.get("physical_baseline", {}) if isinstance(profile.get("physical_baseline"), Mapping) else {}
            perimeter_m = max(0.0, float(baseline.get("constructed_wall_centerline_perimeter_km", 0.0) or 0.0) * 1000.0)
            depth = max(0.5, float(cfg.get("perimeter_holding_depth_m", 2.0) or 2.0))
            area_each = max(1.0, float(cfg.get("secure_floor_area_m2_per_prisoner", 4.0) or 4.0))
            perimeter_capacity = int(math.floor(perimeter_m * depth / area_each)) if perimeter_m > 0 else 0
            logistics = profile.get("logistics_blueprint", {}) if isinstance(profile.get("logistics_blueprint"), Mapping) else {}
            nominal = max(0, int(logistics.get("nominal_garrison_capacity", 0) or 0))
            nominal_fraction = max(0, int(cfg.get("nominal_garrison_detention_fraction_milli", 250) or 250))
            nominal_capacity = int(math.floor(nominal * nominal_fraction / 1000.0))
            candidates = [x for x in (perimeter_capacity, nominal_capacity) if x > 0]
            fixed_capacity = max(candidates) if candidates else 0
            mult = max(1000, int(cfg.get("emergency_capacity_multiplier_milli", 1500) or 1500))
            fixed_emergency_capacity = int(math.floor(fixed_capacity * mult / 1000.0))
            basis = "registered_fortification_geometry_and_guard_manpower"

        occupancy = self._custody_existing_occupancy(
            custodian_formation_ref=str(custodian_formation_ref),
            holding_site_ref=holding_site_ref,
            exclude_group_ref=exclude_group_ref,
        )
        occupied_by_custodian = max(0, int(occupancy["custodian_people"]))
        occupied_at_site = max(0, int(occupancy["site_people"]))
        available_guard_secure = max(0, guard_limited - occupied_by_custodian)
        available_guard_emergency = max(0, emergency_guard_limited - occupied_by_custodian)
        secure = available_guard_secure
        emergency = available_guard_emergency
        if fixed_capacity is not None:
            secure = min(secure, max(0, fixed_capacity - occupied_at_site))
            emergency = min(emergency, max(0, int(fixed_emergency_capacity or 0) - occupied_at_site))
        emergency = max(secure, emergency)
        return {
            "location_ref": str(location_ref), "holding_site_ref": holding_site_ref,
            "custodian_formation_ref": str(custodian_formation_ref), "custodian_personnel": personnel,
            "max_routine_guard_duty_personnel": max_guard_duty,
            "guard_limited_secure_capacity": guard_limited,
            "guard_limited_emergency_capacity": emergency_guard_limited,
            "physical_fixed_capacity": fixed_capacity, "physical_fixed_emergency_capacity": fixed_emergency_capacity,
            "occupied_by_same_custodian_people": occupied_by_custodian, "occupied_at_holding_site_people": occupied_at_site,
            "secure_capacity_people": secure, "emergency_capacity_people": emergency, "basis": basis,
        }

    def _custody_apply_capacity(self, group: MutableMapping[str, Any]) -> dict[str, Any]:
        total = self._custody_group_people(group)
        capacity = self._custody_holding_capacity(
            str(group.get("custodian_formation_ref", "")),
            str(group.get("location_ref", "")),
            exclude_group_ref=str(group.get("owner_id", "")) or None,
        )
        secure = max(0, int(capacity.get("secure_capacity_people", 0)))
        emergency = max(0, int(capacity.get("emergency_capacity_people", 0)))
        if total > emergency:
            mode = "beyond_emergency_capacity"
        elif total > secure:
            mode = "emergency_open_field"
        elif capacity.get("physical_fixed_capacity") is not None:
            mode = "secure_fixed_site"
        else:
            mode = "guarded_field_enclosure"
        capacity["current_prisoners"] = total
        capacity["holding_mode"] = mode
        capacity["secure_overflow_people"] = max(0, total-secure)
        capacity["emergency_overflow_people"] = max(0, total-emergency)
        group["holding_capacity"] = capacity
        group["holding_mode"] = mode
        field = self.read("game/data/mechanics/custody.json").get("field_holding", {})
        if mode == "emergency_open_field":
            group["enclosure_condition_milli"] = min(int(group.get("enclosure_condition_milli", 650)), int(field.get("emergency_enclosure_condition_milli", 380)))
        elif mode in {"guarded_field_enclosure", "secure_fixed_site"}:
            group["enclosure_condition_milli"] = max(int(group.get("enclosure_condition_milli", 650)), int(field.get("secure_enclosure_condition_milli", 600)))
        return capacity
    ...
    def _custody_new_group(self, *, source_formation_ref: str, custodian_formation_ref: str, count: int, at: str, terms: str = "unconditional_surrender") -> dict[str,Any]:
        ...
        pre_capacity=self._custody_holding_capacity(custodian_formation_ref,str(source.get("location_ref","")))
        if int(count)>int(pre_capacity.get("emergency_capacity_people",0)):
            raise ValueError(f"custody capacity exceeded at {source.get('location_ref')}: requested {int(count)}, emergency capacity {pre_capacity.get('emergency_capacity_people',0)}")
        ...
        capacity=self._custody_apply_capacity(group)
        if n>int(capacity.get("emergency_capacity_people",0)):
            raise ValueError("custody capacity changed during surrender settlement")
        self._custody_record_history(group,{"at":at,"kind":"holding_capacity_assessed","holding_mode":group.get("holding_mode"),"secure_capacity_people":capacity.get("secure_capacity_people"),"emergency_capacity_people":capacity.get("emergency_capacity_people")})
    ...
    def _custody_attach_named_capture(self, person_ref: str, custodian_formation_ref: str, battle_ref: str, at: str, *, source_formation_ref: str | None = None) -> str:
        ...
        if isinstance(source_formation_ref,str) and source_formation_ref:
            _sp,source=self._load_formation(source_formation_ref)
            source_force_ref=str(source.get("owner_force_ref") or "") or None
        # Named prisoners from different forces must not share one ransom/source
        # authority merely because the same winner captured them in one battle.
        source_key=source_force_ref or source_formation_ref or "no_source_force"
        ref="prisoners_named_"+hashlib.sha256(f"{battle_ref}|{custodian_formation_ref}|{source_key}".encode()).hexdigest()[:16]; path=_group_path(ref); existing=self.read_optional(path)
        group=copy.deepcopy(dict(existing)) if isinstance(existing,Mapping) else {"schema":"sword-prisoner-group","owner_id":ref,"source_formation_ref":source_formation_ref,"source_force_ref":source_force_ref,"custodian_formation_ref":custodian_formation_ref,"captor_authority_ref":authority,"location_ref":loc,"personnel":0,"by_role":{},"cohort_slices":[],"named_prisoner_refs":[],"guards_allocated":0,"guard_requirement":0,"restraint_condition_milli":650,"enclosure_condition_milli":650,"health_milli":900,"food_kg":0,"water_person_days":0,"legal_status":"prisoner_of_war","status":"held","captured_at":at,"history":[]}
        refs=group.setdefault("named_prisoner_refs",[])
        if person_ref not in refs:refs.append(person_ref);refs.sort();group["guard_requirement"]=(int(group.get("personnel",0))+len(refs)+4)//5;self._custody_record_history(group,{"at":at,"kind":"named_capture","person_ref":person_ref,"battle_ref":battle_ref})
        capacity=self._custody_apply_capacity(group)
        if int(capacity.get("current_prisoners",0))>int(capacity.get("emergency_capacity_people",0)):
            raise ValueError("named capture exceeds physical emergency custody capacity")
        pp,person0=self._exact_person(person_ref,active=False);person=copy.deepcopy(person0);self._set_person_location(person,loc);person["custody_state"]={"status":"prisoner","prisoner_group_ref":ref,"captured_at":at,"captured_by":authority,"location_ref":loc,"battle_ref":battle_ref,"source_formation_ref":source_formation_ref,"source_force_ref":source_force_ref};self.put(pp,person);self.put(path,group);self._custody_register(group,path);return ref
    ...
    def _battle(self, command: Any, payload: Mapping[str,Any], *, context: Mapping[str,Any] | None = None) -> dict[str,Any]:
        ...
        if custodian:
            for person_ref,row in outcomes.items():
                if isinstance(row,Mapping) and row.get("outcome")=="captured":groups.append(self._custody_attach_named_capture(str(person_ref),custodian,str(result.get("battle_event","battle")),result_time,source_formation_ref=str(row.get("formation_ref") or "") or None))
        offers=[]
    ...
    def _custody_daily_review(self, group_ref: str, at: str) -> None:
        ...
        capacity=self._custody_apply_capacity(group); secure=max(0,int(capacity.get("secure_capacity_people",0))); emergency=max(0,int(capacity.get("emergency_capacity_people",0))); capacity_pressure=0
        if total>secure: capacity_pressure=min(400,int(round((total-secure)/max(1,secure or emergency or total)*300)))
        if total>emergency: capacity_pressure=500
        ...
        escape_pressure=max(0,required-guard)*120+max(0,500-int(group.get("restraint_condition_milli",650)))+max(0,500-int(group.get("enclosure_condition_milli",650)))+capacity_pressure
        group["escape_risk_milli"]=min(1000,escape_pressure);group["last_care_review"]={"at":at,"food_consumed_kg":food,"water_person_days_consumed":water,"guards":guard,"required_guards":required,"health_milli":health,"deaths":deaths,"holding_mode":group.get("holding_mode"),"secure_capacity_people":secure,"emergency_capacity_people":emergency}
    ...
    def _command_layer_prisoner_system(self, command: Any, payload: Mapping[str,Any], next_dispatch: Any) -> dict[str,Any]:
        if command.command_type=="siege_action" and str(payload.get("action","")) in {"offer_surrender","accept_surrender_terms"}:
        ...
        if command.command_type!="custody_action":
            ...
            if command.command_type=="command_group_action" and str(payload.get("action",""))=="move_army":
                # Exact active custody groups are few. Any moved army may contain
                # their custodian formation, so enter the bounded sync below.
                moved_refs={"__check_all_active_custody__"}
            if moved_refs:
                for ref, path, g in self._custody_active_groups():
                    try:cust_ref=str(g.get("custodian_formation_ref",""));_fp,cust=self._load_formation(cust_ref)
                    ...
                    if loc and loc!=str(g.get("location_ref","")):
                        old_loc=str(g.get("location_ref",""));g["location_ref"]=loc
                        capacity=self._custody_apply_capacity(g)
                        if self._custody_group_people(g)>int(capacity.get("emergency_capacity_people",0)):
                            raise ValueError(f"custodian movement would exceed physical custody capacity at {loc}")
                        self._custody_sync_named_locations(g,loc)
                        self._custody_record_history(g,{"at":str(self._world_time()),"kind":"custodian_movement","custodian_formation_ref":cust_ref,"from_location_ref":old_loc,"location_ref":loc,"holding_capacity":copy.deepcopy(capacity)});self.put(path,g)
            return result
        ...
        if action=="allocate_guards":
            ...
            total=max(0,int(group.get("personnel",0)))+len(group.get("named_prisoner_refs",[]) if isinstance(group.get("named_prisoner_refs"),list) else [])
            new_capacity=self._custody_holding_capacity(new_ref,str(group.get("location_ref","")),exclude_group_ref=str(group.get("owner_id","")) or None)
            if total>int(new_capacity.get("emergency_capacity_people",0)):
                raise ValueError("new custodian cannot physically hold this prisoner group at the current location")
            group["custodian_formation_ref"]=new_ref;self._custody_apply_capacity(group);self._custody_record_history(group,{"at":now,"kind":"custodian_transfer","from_formation_ref":old_ref,"to_formation_ref":new_ref,"holding_capacity":copy.deepcopy(group.get("holding_capacity"))})
        elif action in {"release","parole"}:
            self._custody_release_aggregate(group,status="paroled" if action=="parole" else "released",at=now)
            self._custody_release_named(group,status="paroled" if action=="parole" else "released",at=now)
            ...
            if person_ref:
                ...
                pp,p0=self._exact_person(person_ref,active=False);person=copy.deepcopy(p0);self._settle_person_death(person_ref,pp,person,now,"lawful custody execution",settle_force_body=True);refs.remove(person_ref);group["named_prisoner_refs"]=refs;group["guard_requirement"]=(int(group.get("personnel",0))+len(refs)+4)//5;self._custody_record_history(group,{"at":now,"kind":"named_execution","person_ref":person_ref,"authority_ref":command.actor_id,"legal_basis_ref":payload.get("legal_basis_ref")})
            else:
                killed=self._custody_kill_aggregate(group,int(payload.get("personnel",group.get("personnel",0))),at=now,reason="execution");self._custody_record_history(group,{"at":now,"kind":"execution","personnel":killed,"authority_ref":command.actor_id,"legal_basis_ref":payload.get("legal_basis_ref")})
            ...
            amount=max(1,int(payload.get("amount_silver",0)));group["ransom_offer"]={"status":"offered","amount_silver":amount,"offered_at":now,"payee_authority_ref":group.get("captor_authority_ref"),"payer_authority_ref":self._custody_force_authority(str(group.get("source_force_ref","")))};self._custody_record_history(group,{"at":now,"kind":"ransom_offered","amount_silver":amount})
            ...
            self._custody_release_named(group,status="ransomed",at=now)
            offer=dict(offer);offer["status"]="paid_and_released";offer["accepted_at"]=now;group["ransom_offer"]=offer;self._custody_clear_guard_duty(group);group["guards_allocated"]=0
            ...
            group["exchange_offer"]={"status":"offered","other_prisoner_group_ref":other_ref,"offered_at":now,"offered_by_ref":command.actor_id};self._custody_record_history(group,{"at":now,"kind":"exchange_offered","other_prisoner_group_ref":other_ref})
        elif action=="accept_exchange":
            ...
            if str(group.get("location_ref"))!=str(other.get("location_ref")):raise ValueError("prisoner exchange requires exact exchange-point co-location")
            self._custody_release_aggregate(group,status="exchanged",at=now);self._custody_release_named(group,status="exchanged",at=now);self._custody_release_aggregate(other,status="exchanged",at=now);self._custody_release_named(other,status="exchanged",at=now);self._custody_clear_guard_duty(group);self._custody_clear_guard_duty(other);group["guards_allocated"]=0;other["guards_allocated"]=0;offer=dict(offer);offer["status"]="accepted";offer["accepted_at"]=now;group["exchange_offer"]=offer;self._custody_record_history(other,{"at":now,"kind":"exchange_completed","other_prisoner_group_ref":group["owner_id"]});self.put(op,other);self._custody_set_active(other)
            ...
            group["recruitment_offer"]={"status":"offered","destination_force_ref":dest_ref,"offered_at":now,"review_at":str(CampaignTime.parse(now).add_seconds(7*86400)),"rule":"captor offers service; prisoners decide autonomously after a cooling period"};self._custody_record_history(group,{"at":now,"kind":"recruitment_offered","destination_force_ref":dest_ref})
        elif action=="finalize_recruitment":
            dest_ref=str(payload.get("destination_force_ref",""));moved=self._custody_finalize_recruitment(group,dest_ref,at=now);self._custody_record_history(group,{"at":now,"kind":"voluntary_recruitment_finalized","destination_force_ref":dest_ref,"personnel":moved})

...
class PrisonerSystemMixin:
    ...
    def _custody_existing_occupancy(
        ...
        *,
        ...
        for ref in [str(x) for x in index.get("active_refs", []) if isinstance(x, str)]:
            if exclude_group_ref and ref == str(exclude_group_ref):
                continue
            try:
                _path, group = self._custody_group(ref)
    ...
    def _command_layer_prisoner_system(self, command: Any, payload: Mapping[str,Any], next_dispatch: Any) -> dict[str,Any]:
        ...
        if action=="allocate_guards":
            ...
            for pref in group.get("named_prisoner_refs",[]) if isinstance(group.get("named_prisoner_refs"),list) else []:
                pp,p0=self._exact_person(str(pref),active=False);p=copy.deepcopy(p0);p["custody_state"]={"status":"paroled" if action=="parole" else "released","released_at":now,"location_ref":group.get("location_ref"),"former_prisoner_group_ref":group["owner_id"]};self.put(pp,p)
            ...
            for pref in list(group.get("named_prisoner_refs",[])):
                pp,p0=self._exact_person(str(pref),active=False);p=copy.deepcopy(p0);p["custody_state"]={"status":"ransomed","released_at":now,"former_prisoner_group_ref":group["owner_id"],"location_ref":group.get("location_ref")};self.put(pp,p)

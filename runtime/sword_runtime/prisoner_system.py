...
class PrisonerSystemMixin:
    ...
    def _custody_debit_population_deaths(self, force_ref: str, count: int) -> None:
        ...
        if force_ref.startswith("force_state_"):
            state=force_ref.replace("force_state_",""); path=f"state/population/{state}.json"; pop=copy.deepcopy(self.read(path)); pop["strata"]["active_military"]=max(0,int(pop["strata"].get("active_military",0))-count); pop["population_total"]=max(0,int(pop.get("population_total",0))-count);self.put(path,pop)

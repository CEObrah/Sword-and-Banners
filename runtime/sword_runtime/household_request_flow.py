...
def _sword_manor_status(planner: Any) -> dict[str, Any]:
    ...
    tang_site = sites.get("loc_tang_manor", {}) if isinstance(sites, Mapping) else {}
    physical = tang_site.get("sword_manor", {}) if isinstance(tang_site, Mapping) else {}
    if not isinstance(physical, Mapping):
        raise ValueError("Sword Manor physical-capacity authority is missing")

    trainee_beds = max(0, int(physical.get("trainee_dormitory_beds", 0)))
    total_beds = max(0, int(physical.get("total_residential_beds", 0)))
    support_capacity = min(
        max(0, int(physical.get("instruction_capacity_people", 0))),
        max(0, int(physical.get("dining_capacity_people_per_day", 0))),
        max(0, int(physical.get("water_capacity_people_per_day", 0))),
        max(0, int(physical.get("medical_support_capacity_people", 0))),
        max(0, int(physical.get("training_space_capacity_people", 0))),
        total_beds,
    )
    trainee_capacity = max(0, min(trainee_beds, support_capacity - higher_rank_count))
    assessment_per_day = max(0, int(physical.get("intake_assessment_candidates_per_day", 0)))
    physical_intake_throughput_30d = assessment_per_day * 30
    equipment_issue_capacity = max(0, int(physical.get("induction_equipment_issue_capacity_per_30d", 0)))
    ...
    return {
        ...
        "capacity_ref": "state/infrastructure/settlements.json#/sites/loc_tang_manor/sword_manor",

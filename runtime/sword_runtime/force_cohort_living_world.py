...
class ForceCohortLivingWorldMixin(MilitaryCareerTransferSafetyMixin):
 ...
 def _fc_train_person_lite_batch(self, force: Mapping[str, Any], *, deliberate_hours: float, role_exposure_hours: float, training_rules: Mapping[str, Any], facility_grade: str, equipment_grade: str, recovery_grade: str, evidence_prefix: str, ref_prefix: str | None = None) -> int:
  ...
  for base,rows in sorted(grouped.items()):
   ...
   for person_ref,tokens in rows:
    ...
    train_person_lite(person,deliberate_hours=deliberate_hours,role_exposure_hours=role_exposure_hours,training_rules=training_rules,facility_grade=facility_grade,equipment_grade=equipment_grade,recovery_grade=recovery_grade,evidence_ref=f'{evidence_prefix}:{person_ref}')
 ...
 def _fc_train_person_lites(self, force: Mapping[str, Any], regimen: str, months: float, ref: str, *, ref_prefix: str | None = None) -> int:
  ...
  return self._fc_train_person_lite_batch(force,deliberate_hours=dh,role_exposure_hours=eh,training_rules=rules,facility_grade=str(r.get('facility_grade','adequate')),equipment_grade=str(r.get('equipment_grade','adequate')),recovery_grade=str(r.get('recovery_grade','adequate')),evidence_prefix=f'{ref}:person_lite',ref_prefix=ref_prefix)
 ...
 def _fc_train_person_lites_extra(self, force: Mapping[str, Any], *, target_regimen: str, baseline_regimen: str, months: float, ref: str, ref_prefix: str | None = None) -> int:
  ...
  return self._fc_train_person_lite_batch(force,deliberate_hours=dh,role_exposure_hours=eh,training_rules=rules,facility_grade=str(target.get('facility_grade','adequate')),equipment_grade=str(target.get('equipment_grade','adequate')),recovery_grade=str(target.get('recovery_grade','adequate')),evidence_prefix=f'{ref}:person_lite_extra',ref_prefix=ref_prefix)
 ...
 def _autonomy_state(self,host:Mapping[str,Any],occurrences:int,at:str)->None:
  ...
  if state=='qin':
   ...
   for formation_ref in ('formation_qin_wei_unit_01','formation_qin_wei_unit_02','formation_qin_wei_unit_03','formation_qin_wei_unit_04'):
    self._fc_train_formation_extra(actual,formation_ref,target_regimen='house_tang_max_sustainable',baseline_regimen='regular_army',months=months,ref=f'wei_designated_qin:{formation_ref}:{at}')

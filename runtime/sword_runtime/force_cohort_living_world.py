...
from sword_runtime.training_programs import formation_training_ref_for_role, REGISTRY_PATH as TRAINING_PROGRAM_REGISTRY_PATH, formation_drill_access, resolve_program_ref, settle_cohort_program, settle_person_lite_program
...
from sword_runtime.officer_cadre import develop_officer_cadre, reorganize_officer_cadre
...
from sword_runtime.development import age_years, annual_mortality_basis_points, deterministic_annual_mortality_roll
...
class ForceCohortLivingWorldMixin(MilitaryCareerTransferSafetyMixin):
 ...
 def _fc_operational_training_factor(self, formation: Mapping[str, Any] | None, rules: Mapping[str, Any]) -> float:
  """Return deliberate-training availability from the formation's real posture."""
  if not isinstance(formation,Mapping): return 1.0
  budget=rules.get('time_budget',{}) if isinstance(rules.get('time_budget'),Mapping) else {}
  factors=budget.get('formation_operational_deliberate_training_factor',{}) if isinstance(budget,Mapping) else {}
  if not isinstance(factors,Mapping): return 1.0
  status=str(formation.get('status','formed')).lower()
  raw=factors.get(status,factors.get('default',1.0))
 ...
 def _fc_train(self,force:dict[str,Any],regimen:str,months:float,ref:str)->None:
  ...
  profiles=self._fc_profiles(); r=self._fc_regimen(regimen); rp=profiles.get('role_training_profiles',{}); rules=self.read(T); ledger=ensure_cohort_ledger(force)
  # A cohort may begin as one conserved recruitment pool feeding several Units.
  # The first real development interval must split those allocated bodies into
  # formation-owned capability slices. Otherwise different Unit curricula,
  # facilities, combat histories or field postures would be averaged back into
  # one shared cohort and training one Unit could improve its sibling. Isolation
  # is lazy and stable: once a slice is unique to a formation it is reused.
  if hasattr(self,'_ct_isolate_training'):
   formation_refs=sorted({
    str(formation_ref)
    for cohort in ledger.get('cohorts',{}).values() if isinstance(cohort,Mapping)
    for formation_ref,count in (cohort.get('allocated_by_formation',{}) if isinstance(cohort.get('allocated_by_formation'),Mapping) else {}).items()
    if max(0,int(count or 0))>0
   })
   force_ref=str(force.get('owner_id') or force.get('force_ref') or '')
   for formation_ref in formation_refs:
    try: fp,formation0=self._load_formation(formation_ref)
    except (KeyError,ValueError,FileNotFoundError): continue
    if str(formation0.get('owner_force_ref',''))!=force_ref: continue
    formation=deepcopy(formation0)
    self._ct_isolate_training(force,formation,f'{ref}:autonomous_training:{formation_ref}')
    self.put(fp,formation)
   ledger=ensure_cohort_ledger(force); validate_cohort_ledger(force)
  whole=int(months); rem=max(0.,months-whole)
  age_policy=profiles.get('rules',{}).get('cohort_age_lifecycle',{}) if isinstance(profiles.get('rules'),Mapping) else {}
  unknown_entry_age_mean=float(age_policy.get('unknown_entry_age_mean',24.0)) if isinstance(age_policy,Mapping) else 24.0
  ...
  for bi,block in enumerate([1.]*whole+([rem] if rem>1e-9 else [])):
   ...
   formation_training_hours: dict[str,float] = {}
   for c in ledger['cohorts'].values():
    ...
    advance_service_months(c,block,unknown_entry_age_mean=unknown_entry_age_mean)
    if c.get('attribute_means') or c.get('skill_means'):
     ...
     for si,training_slice in enumerate(training_slices):
      ...
      posture_factor=self._fc_operational_training_factor(formation,rules)
      slice_dh*=posture_factor; slice_eh*=posture_factor
      environment=training_environment(self,location_ref=cohort_location,simultaneous_trainees=max(1,slice_count)) if cohort_location else {'facility_grade':'none','capacity_factor':0.0}
      slice_dh*=max(0.0,min(1.0,float(environment.get('capacity_factor',0.0))))
      if isinstance(formation,Mapping):
       formation_ref=str(formation.get('formation_ref','') or training_slice.get('source_ref',''))
       if formation_ref:
        # Formal command staff share the same deliberate schedule as the Unit.
        # Record the full per-person available clock once per Formation rather
        # than summing cohort slices and accidentally multiplying time by size.
        full_hours=dh*posture_factor
        formation_training_hours[formation_ref]=max(formation_training_hours.get(formation_ref,0.0),full_hours)
      ...
      training_ref=formation_training_ref_for_role(formation, role) if isinstance(formation,Mapping) else ''
   ...
   # Verified Unit training feeds the already-existing aggregate officer-cadre
   # promotion mechanism. Promotion reclassifies conserved bodies only when a
   # real billet vacancy exists; it never adds personnel or changes top command.
   # The formation hot-state sanitizer intentionally removes local history tails,
   # so durable proof is written once per force/block to semantic history instead.
   promotion_rows=[]
   promotion_totals={}
   for formation_ref,hours in sorted(formation_training_hours.items()):
    whole_hours=max(0,int(round(hours)))
    if whole_hours<=0: continue
    try: fp,formation0=self._load_formation(formation_ref)
    except (KeyError,ValueError,FileNotFoundError): continue
    formation=deepcopy(formation0)
    promoted=develop_officer_cadre(formation,training_hours=whole_hours,at=str(block_end))
    promoted_clean={k:int(v) for k,v in promoted.items() if int(v)>0}
    if promoted_clean:
     promotion_rows.append({'formation_ref':formation_ref,'verified_training_hours':whole_hours,'promoted':promoted_clean})
     for rank,count in promoted_clean.items(): promotion_totals[rank]=promotion_totals.get(rank,0)+count
    reorganize_officer_cadre(formation,at=str(block_end),reason='autonomous_verified_training_promotion_review')
    self.put(fp,formation)
   if promotion_rows:
    force_ref=str(force.get('owner_id') or force.get('force_ref') or ref)
    token=hashlib.sha256(f'{force_ref}|{block_end}|aggregate-officer-promotions'.encode()).hexdigest()[:16]
    history=deepcopy(self.read('state/history/events/index.json'))
    history.setdefault('events',[]).append({
     'event_id':f'aggregate_officer_promotions_{token}',
     'kind':'aggregate_officer_promotions',
     'at':str(block_end),
     'force_ref':force_ref,
     'promoted':promotion_totals,
     'formation_promotions':promotion_rows,
     'basis':'verified formation training plus existing billet vacancies; conserved personnel reclassified without headcount creation',
    })
  ...
  # parent commander and explicit staff chain propagate the training standard through the command
 ...
 def _fc_person_lite_review_time(self, person_ref: str, sort_year: int, offset: str) -> CampaignTime:
  # Spread annual reviews deterministically through the year.  The review day is
  # derived, not persisted, so there is no per-person scheduler/history state.
  day_offset=int(hashlib.sha256(f"person-lite-life-day|{person_ref}".encode()).hexdigest()[:8],16)%365
  return CampaignTime(int(sort_year),1,1,0,0,0,offset).add_days(day_offset)

 def _fc_review_person_lite_life(self, force: MutableMapping[str, Any], start: CampaignTime, end: CampaignTime, *, ref_prefix: str | None = None) -> int:
  ...
  deaths=0
  for person_ref in sorted(str(x) for x in list(materialized) if not ref_prefix or str(x).startswith(ref_prefix)):
   try: person_path,person=self._command_person(person_ref,active=False)
   except (KeyError,ValueError,FileNotFoundError): continue
   if str(person.get('schema',''))!='person-lite': continue
   if str(person.get('life_status',person.get('status','active'))).lower() in {'dead','deceased','destroyed'}: continue
   for sort_year in range(start.sort_year,end.sort_year+1):
    review=self._fc_person_lite_review_time(person_ref,sort_year,start.offset)
    if not (start<review<=end): continue
    age=age_years(person,review); health=self._person_health(person); bp=annual_mortality_basis_points(age,health)
    if deterministic_annual_mortality_roll(world_seed,person_ref,review)>=bp: continue
    # The force object already contains this cycle's aggregate development. Stage
    # it before generic exact-body death settlement, then refresh the caller's
    # mutable image so later training cannot overwrite the conserved removal.
    force_path=self.owner_path(str(force.get('owner_id',''))); self.put(force_path,force)
    self._settle_person_death(person_ref,person_path,person,str(review),'deterministic life-course mortality',settle_force_body=True)
    refreshed=deepcopy(self.read(force_path)); force.clear(); force.update(refreshed)
    deaths+=1; break
  return deaths
 ...
 def _fc_train_person_lite_batch(self, force: Mapping[str, Any], *, deliberate_hours: float, role_exposure_hours: float, training_rules: Mapping[str, Any], facility_grade: str, equipment_grade: str, recovery_grade: str, evidence_prefix: str, ref_prefix: str | None = None, window_start: str | None = None, window_end: str | None = None, command_only: bool = False) -> int:
  ...
  for base,rows in sorted(grouped.items()):
   ...
   for person_ref,tokens in rows:
    ...
    env_hours=deliberate_hours*max(0.0,min(1.0,float(environment.get('capacity_factor',0.0))))
    settle_person_lite_program(person,registry=registry,program_ref=program_ref,deliberate_hours=env_hours,role_exposure_hours=role_exposure_hours,training_rules=training_rules,facility_grade=str(environment.get('facility_grade','none')),equipment_grade=equipment_grade,recovery_grade=recovery_grade,evidence_ref=evidence,promotion_facts=promotion_facts,instructor_context_by_drill=instructor_contexts,drill_access=drill_access,time_window_start=window_start,time_window_end=window_end)
 ...
 def _fc_train_person_lites(self, force: Mapping[str, Any], regimen: str, months: float, ref: str, *, ref_prefix: str | None = None) -> int:
  ...
  if isinstance(force,MutableMapping): self._fc_review_person_lite_life(force,start,end,ref_prefix=ref_prefix)
  return self._fc_train_person_lite_batch(force,deliberate_hours=dh,role_exposure_hours=eh,training_rules=rules,facility_grade=str(r.get('facility_grade','home_garrison')),equipment_grade=str(r.get('equipment_grade','adequate')),recovery_grade=str(r.get('recovery_grade','adequate')),evidence_prefix=f'{ref}:person_lite',ref_prefix=ref_prefix,window_start=str(start),window_end=str(end))
 ...
 def _fc_train_person_lites_extra(self, force: Mapping[str, Any], *, target_regimen: str, baseline_regimen: str, months: float, ref: str, ref_prefix: str | None = None, command_only: bool = False) -> int:
  ...
  return self._fc_train_person_lite_batch(force,deliberate_hours=dh,role_exposure_hours=eh,training_rules=rules,facility_grade=str(target.get('facility_grade','home_garrison')),equipment_grade=str(target.get('equipment_grade','adequate')),recovery_grade=str(target.get('recovery_grade','adequate')),evidence_prefix=f'{ref}:person_lite_extra',ref_prefix=ref_prefix,window_start=str(start),window_end=str(end),command_only=command_only)
 ...
 def _fc_train_formation_extra(self, force: dict[str, Any], formation_ref: str, *, target_regimen: str, baseline_regimen: str, months: float, ref: str) -> int:
  ...
  for item in formation.get('cohort_composition',[]) if isinstance(formation,Mapping) else []:
   ...
   if dh>0 or eh>0:
    registry=self.read(TRAINING_PROGRAM_REGISTRY_PATH); program_ref=resolve_program_ref(registry,role=role,training_ref=formation_training_ref_for_role(formation, role))
    ...
    environment=training_environment(self,location_ref=str(formation.get('location_ref','') or ''),simultaneous_trainees=max(1,int(item.get('count',0))))
    env_dh=dh*max(0.0,min(1.0,float(environment.get('capacity_factor',0.0))))
    settle_cohort_program(c,registry=registry,program_ref=program_ref,deliberate_hours=env_dh,role_exposure_hours=eh,training_rules=rules,facility_grade=str(environment.get('facility_grade','none')),equipment_grade=str(target.get('equipment_grade','adequate')),recovery_grade=str(target.get('recovery_grade','adequate')),evidence_ref=evidence,promotion_facts=self._fc_promotion_facts(force,role),instructor_context_by_drill=instructor_contexts,drill_access=drill_access)
   changed+=1
  if changed:
 ...
 def _autonomy_state(self,host:Mapping[str,Any],occurrences:int,at:str)->None:
  ...
  # Active levies are temporary state service, but they are still real cohorts.
  # They develop only through their registered levy regimen and the physical
  # training environment at their actual locations; no state identity bonus.
  state_doc=self.read(f'state/states/{state}.json')
  for levy_force_ref in state_doc.get('active_levy_refs',[]) if isinstance(state_doc,Mapping) else []:
   if not isinstance(levy_force_ref,str) or not levy_force_ref: continue
   try: levy_path=self.owner_path(levy_force_ref); levy=self._fc_prepare(levy_path,at)
   except (KeyError,ValueError,FileNotFoundError): continue
   if str(levy.get('service_class',''))!='state_levy': continue
   self._fc_train(levy,'levy_basic',months,f'state_levy:{state}:{levy_force_ref}:{at}')
   validate_cohort_ledger(levy); self.put(levy_path,levy)

...
class ForceCohortLivingWorldMixin(MilitaryCareerTransferSafetyMixin):
 ...
 def _fc_promotion_facts(self, force: Mapping[str, Any], role: str) -> Mapping[str, Any]:
  ...
  if owner=='force_sword_manor':
   ...
   path='game/data/mil/sword-manor-progression.json'
  elif owner=='force_house_tang':
   if role=='house_guard': record_id='house_guard_to_house_guardian_cavalry'; path='game/data/mil/sword-manor-progression.json'
   elif role=='guardian_cavalry': record_id='guardian_cavalry_to_tang_champion'; path='game/data/mil/house-tang-champion-progression.json'
 ...
 def _fc_procure_ammunition(self, force: Mapping[str, Any], *, depot_path: str, treasury_path: str, treasury_field: str, occurrences: int, at: str, owner_ref: str) -> dict[str, int]:
  ...
  if spent:
   ...
   history=depot.setdefault('procurement_history',[])
   if not isinstance(history,list): raise ValueError('depot procurement history is invalid')
   history.append({'at':at,'owner_ref':owner_ref,'war_arrows':bought['war_arrows'],'war_bolts':bought['war_bolts'],'silver_spent':spent})
   depot['procurement_history']=history[-24:]

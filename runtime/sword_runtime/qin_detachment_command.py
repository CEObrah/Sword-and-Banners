...
keeps its own top commander and internal command hierarchy. Tang Wei's assumption therefore transfers the
...
def _assign_detachment_operation_to_wei(
    planner: Any,
    operation_path: str,
    ...
    *,
    ...
    """Transfer only an exact registered detachment operation to Wei's field authority."""

    operation_refs = [str(ref) for ref in operation.get("formation_refs", []) if isinstance(ref, str) and ref]
    if set(operation_refs) != set(refs):
        return
    updated = copy.deepcopy(operation)
    updated["kind"] = "assigned_qin_field_detachment_operation"
    updated["autonomous"] = False
    updated["administrative_authority"] = "char_tang_wei"
    updated["administrative_authorities"] = ["char_tang_wei"]
    updated["assignment_authority_ref"] = "char_tang_wei"
    updated["institutional_owner_ref"] = "state_qin"
    updated["source_force_ref"] = "force_state_qin"
    updated["command_group_ref"] = "cmdgrp.tang_wei.field_army"
    updated["authority_basis"] = {
        ...
        "appointment_ref": office,
        "source_event_ref": offer_ref,
        "rule": (
            "Qin retains institutional ownership and manpower provenance; Tang Wei holds current "
            "operational assignment authority over the exact registered detachment until lawfully relieved."
        ),
    }
    planner.put(operation_path, updated)
...
def assume_registered_qin_detachment_command(planner: Any, at: str) -> str | None:
    ...
    for raw in appointments:
        ...
        operation_path = resolved_operation[0] if resolved_operation is not None else None
        operation = copy.deepcopy(resolved_operation[1]) if resolved_operation is not None else None
        ...
        if isinstance(operation_path, str) and isinstance(operation, Mapping):
            _assign_detachment_operation_to_wei(
                planner, operation_path, operation, refs=refs, office=office, offer_ref=offer_ref
            )
        ...
        summary = (
            ...
            "Each unit keeps its own exact top commander and internal command hierarchy; administrative ownership remains Qin's. The appointment itself chooses no march route, battle plan, sovereign allegiance, or permanent strategy."
        )[:4000]
        return _event_owner_write(planner, event_ref, {
            "event_ref": event_ref,
            "kind": "institutional_response",

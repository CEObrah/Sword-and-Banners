...
class FamilyAutonomyMixin:
    ...
    def _family_active_union(self, idx: Mapping[str, Any], person_ref: str) -> tuple[str, str, dict[str, Any]] | None:
        for ref, path, union in exact_family_records_for_person(self.read_optional, idx, "unions", person_ref):
            if str(union.get("status", "")) in {"betrothed", "married"}:
                return ref, path, copy.deepcopy(dict(union))
        return None

    def _family_close_kin(self, idx: Mapping[str, Any], a: str, b: str) -> bool:
        if a == b:
            return True
        # The kinship index is routing-only and its person_links entries are
        # structured category maps, not flat relationship IDs.  Consequential
        # eligibility therefore comes from exact kinship/parentage owners.
        for kinship_ref in (idx.get("kinships", {}) or {}):
            ...
            if {a, b}.issubset(participants) and str(row.get("status", "active")) == "active":
                return True
        parents: dict[str, set[str]] = {}
        for parentage_ref in (idx.get("parentage", {}) or {}):
            ...
            parents[child] = {
                str(x.get("parent_id"))
                for x in row.get("parent_links", [])
                if isinstance(x, Mapping) and x.get("parent_id")
            }
    ...
    def _family_create_autonomous_proposal(self, proposer_ref: str, target_ref: str, relationship_ref: str, at: str) -> str:
        ...
        for pref in (idx.get("proposals", {}) or {}):
            resolved = exact_family_record(self.read_optional, idx, "proposals", str(pref))
            row = resolved[1] if resolved is not None else None
    ...
    def _family_accept_autonomous_proposal(self, proposal_ref: str, target_ref: str, at: str) -> str | None:
        idx=self._family_index_doc(); resolved=exact_family_record(self.read_optional,idx,"proposals",proposal_ref)
        if resolved is None:
            return None
        path, proposal = resolved
    ...
    def _autonomy_person(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        idx=self._family_index_doc()
        proposals=exact_family_records_for_person(self.read_optional,idx,"proposals",person_ref)
        for proposal_ref,_proposal_path,row in proposals:

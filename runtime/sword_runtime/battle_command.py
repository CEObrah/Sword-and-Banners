...
def _command_group_for_person(planner: Any, person_ref: str | None) -> Mapping[str, Any] | None:
    """Return the routed primary command group for one exact commander.
    ...
    group_ref = primary.get(person_ref) if isinstance(primary, Mapping) else None
    template = index.get("path_template") if isinstance(index, Mapping) else None
    if not isinstance(group_ref, str) or not group_ref or not isinstance(template, str):
        return None
    path = template.replace("{command_group_id}", group_ref).replace("{ref}", group_ref)
...
def formation_command_person(planner: Any, formation_ref: str, formation: Mapping[str, Any]) -> str | None:
    ...
    if isinstance(authority, str) and authority.startswith("char_") and _person(planner, authority) is not None:
        return authority
    commander = formation.get("commander_ref")
    if isinstance(commander, str) and commander.startswith("char_") and _person(planner, commander) is not None:
        return commander
    group = _command_group_for_formation(planner, formation_ref)
...
def _sector_roles(battlefield: Mapping[str, Any]) -> tuple[list[str], str | None, str | None]:
    ...
    for ref, row in sectors.items():
        ...
        if "reserve" in name:
            reserve = ref
            continue
        if "command" in name or "headquarters" in name:
            command = ref
            continue
        priority = 1
        if "center" in name:
            ...
            priority = 0
        frontline.append((priority, ref))
    return [ref for _priority, ref in sorted(frontline)], reserve, command
...
def _rebuild_sector_membership(battlefield: dict[str, Any]) -> None:
    ...
    for formation_ref, assignment in assignments.items():
        ...
        if isinstance(sector_ref, str) and sector_ref in sectors and assignment.get("status") != "redeploying":
...
def _sector_side_strength(planner: Any, battlefield: Mapping[str, Any], sector_ref: str, side_ref: str | None, *, at: str) -> dict[str, Any]:
    ...
    for formation_ref, assignment in sorted(assignments.items()):
        if not isinstance(formation_ref, str) or not isinstance(assignment, Mapping):
            continue
        if assignment.get("sector_ref") != sector_ref or assignment.get("side_ref") != side_ref or assignment.get("status") == "redeploying":
            continue
        try:
...
def _observed_enemy_sector_strength(planner: Any, battlefield: Mapping[str, Any], sector_ref: str, side_ref: str | None, *, pressure_milli: int) -> dict[str, Any]:
    ...
    for formation_ref, assignment in assignments.items():
        if not isinstance(formation_ref, str) or not isinstance(assignment, Mapping):
            continue
        if str(assignment.get("side_ref", "")) != side_ref or str(assignment.get("sector_ref", "")) != sector_ref or assignment.get("status") == "redeploying":
            continue
        try:
            _path, formation = planner._load_formation(formation_ref)
        ...
        observed = max(100, int(round(personnel / 100.0)) * 100)
        total += observed
        rows += 1
...
def review_battle_command_plan(planner: Any, operation: Mapping[str, Any], battlefield: dict[str, Any], *, at: str) -> list[dict[str, Any]]:
    ...
    for mission_ref, mission_raw in sorted(missions.items()):
        if not isinstance(mission_ref, str) or not isinstance(mission_raw, dict):
            continue
        mission = mission_raw
        sector_ref = mission.get("sector_ref")
        if not isinstance(sector_ref, str) or not isinstance(sectors.get(sector_ref), Mapping):
            continue
        side_ref = None
        formation_refs = [str(ref) for ref in mission.get("formation_refs", []) if isinstance(ref, str)]
        active_mission_refs: list[str] = []
        for ref in formation_refs:
            row = assignments.get(ref)
            if not isinstance(row, Mapping) or row.get("sector_ref") != sector_ref or row.get("status") == "redeploying":
                continue
            if isinstance(row.get("side_ref"), str):
                side_ref = str(row["side_ref"]); active_mission_refs.append(ref)
        if not side_ref or not active_mission_refs:
            continue
...
def initialize_battle_command_plan(planner: Any, operation: Mapping[str, Any], battlefield: dict[str, Any], *, at: str) -> dict[str, Any]:
    ...
    for formation_ref, assignment in sorted(assignments.items()):
        ...
        if not (isinstance(commander, str) and commander.startswith("char_") and _person(planner, commander) is not None):
            commander = leaf_commander
        block_key = commander or f"authority:{authority_key}"
    ...
    for side in side_refs:
        ...
        # Largest commands get the most central/frontline responsibility. A fourth
        # or later independent command can become reserve; this never changes who
        # owns the formations.
        blocks.sort(key=lambda row: (-int(row.get("personnel", 0)), str(row.get("recipient_ref") or row.get("authority_ref"))))
        side_missions: list[dict[str, Any]] = []
        for index, block in enumerate(blocks):
            use_reserve = reserve_ref is not None and len(blocks) >= 4 and index == len(blocks) - 1
            sector_ref = reserve_ref if use_reserve else frontline[index % len(frontline)]
            order = "reserve" if use_reserve else str(initial_orders.get(side, "hold"))
            sector = sectors[sector_ref]
    ...
    _rebuild_sector_membership(battlefield)

...
from sword_runtime.campaign_communications import (
    command_message_route, command_person_location, player_command_location,
)
...
def sync_explicit_house_field_preparation(planner: Any, runtime: dict[str, Any]) -> None:
    ...
    for attempt in reversed(attempts):
        ...
        recipient_location = command_person_location(planner, attempt.get("target_ref"))
        if not origin_location or not recipient_location:
            raise ValueError("House field-preparation request lacks physical family communication endpoints")
        route = command_message_route(planner.read, origin_location, recipient_location, round_trip=True)
        ...
        hosts[host_id] = {
            ...
            "communication_rule": "family request and reply require physical delivery when not co-located",

...
class MerchantConvoyMixin:
    ...
    def _sync_convoy_escorts(self, convoy: dict[str, Any], at: str) -> str:
        ...
        for formation_ref in refs:
            ...
            if origin != current:
                departure = CampaignTime.parse(last_sync)
                settle_formation_idle_fatigue(formation, current=departure, rules=self.read(FATIGUE_RULES_PATH))
                formation["location_ref"] = current
                formation["status"] = "convoy_escort"
                stamp_formation_activity_fatigue(
                    formation, completed_at=CampaignTime.parse(at), fatigue_gain=max(0, int(math.ceil(elapsed_hours / 12.0))), activity_kind="convoy_escort"
                )

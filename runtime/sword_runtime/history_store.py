...
def write_history_index(planner: Any, history: Mapping[str, Any]) -> None:
    """Persist append-only semantic history without same-transaction lost updates.

    Multiple causal domains may append history during one broad chronology
    transaction.  A domain is allowed to hold an older read image while another
    domain appends first, so a later whole-document write must merge only genuinely
    new event IDs into the current staged authority instead of replacing it.
    Semantic history is append-only; existing event IDs are immutable.
    """
    staged = getattr(planner, "_writes", {})
    staged_doc = staged.get(HISTORY_INDEX_PATH) if isinstance(staged, dict) else None
    if isinstance(history, dict) and isinstance(staged_doc, dict) and staged_doc is history:
        ...
        doc = history
    elif isinstance(staged_doc, Mapping):
        # Preserve every event already staged by another domain.  Only append event
        # IDs from the caller's older image that are not present anywhere in the
        # current head/archive authority.  This prevents a stale local history copy
        # from deleting a ceremony, death, battle, or other event written earlier in
        # the same transaction.
        doc = copy.deepcopy(dict(staged_doc))
        current_ids = {
            str(row.get("event_id"))
            for row in iter_history_events(planner)
            if isinstance(row.get("event_id"), str) and row.get("event_id")
        }
        incoming = history.get("events", []) if isinstance(history, Mapping) else []
        ...
        for event in incoming:
            if not isinstance(event, Mapping):
                raise ValueError("semantic history event is invalid")
            event_id = event.get("event_id")
            if not isinstance(event_id, str) or not event_id:
                raise ValueError("semantic history event lacks event_id")
            if event_id in current_ids:
                continue
            target.append(copy.deepcopy(dict(event)))

...
from sword_runtime.campaign_command_cycle import _latest_order, _load_operation, _read_cycle, _reader, retain_campaign_command_decisions
...
from sword_runtime.operation_routing import append_operational_order
...
def is_campaign_command_follow_on_request(row: Mapping[str, Any]) -> bool:
...
def campaign_command_follow_on_route(
    ...
    *,
    ...
    if not is_campaign_command_follow_on_request(attempt):
        return None
...
def _mission_order(
    ...
    mission_packet.update({
        ...
        "phase_status": "issued_pending_delivery",
...
def sync_campaign_command_decisions(planner: Any) -> list[str]:
    ...
    # Issuance at superior headquarters is a durable historical fact, not receipt.
    # Keep Tang Wei's exact current order/phase unchanged until the physical
    # campaign_command_superior_order delivery host settles and activates this ref.
    ...
    cycle["campaign_command_decisions"] = retain_campaign_command_decisions(decisions)
    cycle["campaign_command_decision_refs"] = list(dict.fromkeys(decision_refs + [decision_ref]))[-64:]
    # current_superior_order is receipt authority. The new decision remains only
    # in campaign_command_decisions until its courier delivery is recorded.
    cycle["updated_at"] = at
    planner.put(cycle_path, cycle)
    return [order_ref]
...
__all__ = [
    ...
    "is_campaign_command_follow_on_request",

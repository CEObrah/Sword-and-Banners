...
def _fortification_for_site(planner: Any, site_ref: str) -> tuple[str, str, dict[str, Any]] | None:
    idx = planner.read("state/fortifications/index.json")
    for ref, path in (idx.get("fortifications", {}) if isinstance(idx, Mapping) else {}).items():
        if not isinstance(path, str):
            continue
        row = planner.read_optional(path)
        if isinstance(row, Mapping) and str(row.get("site_ref") or row.get("location_ref") or "") == str(site_ref):
...
def _derived_relief_prospect(planner: Any, siege: Mapping[str, Any], site_ref: str, *, at: str) -> dict[str, Any]:
    ...
    nearest = min((int(row["route_hours"]) for row in plausible), default=None)
    ...
    return {
        ...
        "plausible_relief_formation_refs": [str(row["formation_ref"]) for row in plausible],
        "plausible_relief_personnel": sum(int(row["personnel"]) for row in plausible),

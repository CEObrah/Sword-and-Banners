...
_ARMORY_COUNTERS: dict[str, tuple[str, str, str]] = {
    ...
    "weapon_spear_long": ("weapons", "Long Spear armory reserve", "Long Spear issued"),
    "weapon_lance_cavalry": ("weapons", "Cavalry Lance armory reserve", "Cavalry Lance issued"),
    "weapon_sword_one_hand_long": ("weapons", "One-Handed Long Sword armory reserve", "One-Handed Long Sword issued"),
...
def _loadout_items(planner: Any, formation: Mapping[str, Any]) -> tuple[str, ...]:
    loadout_id = str(formation.get("equipment_loadout_id", ""))
    if not loadout_id:
        return ()
    index = planner.read(_LOADOUT_INDEX_PATH)
    rel = index.get("records", {}).get(loadout_id) if isinstance(index, Mapping) else None
    ...
    if not isinstance(loadout, Mapping):
        return ()
    keys = ("body_armor","helmet","ranged_weapon","primary_melee_weapon","shield","sidearm","mount","tack","horse_armor")
...
def issue_house_armory_to_formation(planner: Any, *, formation_ref: str, item_key: str, quantity: int, actor_ref: str, at: str) -> dict[str, Any]:
    ...
    if quantity<=0: raise ValueError("formation armory issue quantity must be positive")
    counter=_ARMORY_COUNTERS.get(str(item_key))
    if counter is None: raise ValueError("House Tang has no exact armory reserve counter for this item")
    formation_path, formation0=planner._load_formation(str(formation_ref)); formation=copy.deepcopy(formation0)
    ...
    house=planner.read(_HOUSE_PATH); programs=house.get("administrative_programs",{}) if isinstance(house,Mapping) else {}
    ...
    registry=copy.deepcopy(planner.read(_INVENTORY_PATH)); record_id,reserve_key,issued_key=counter; facts=_record(registry,record_id)
    reserve=max(0,int(facts.get(reserve_key,0)))
    if reserve<quantity: raise ValueError(f"House Tang armory reserve lacks {item_key}: requested {quantity}, available {reserve}")
    facts[reserve_key]=reserve-quantity; facts[issued_key]=max(0,int(facts.get(issued_key,0)))+quantity
    staging=formation.setdefault("equipment_staging_by_item",{})
    if not isinstance(staging,MutableMapping): raise ValueError("formation equipment staging ledger is invalid")
    staging[str(item_key)]=max(0,int(staging.get(str(item_key),0)))+quantity
    converted=_convert_complete_sets(planner,formation); _recompute_completeness(formation)
    formation.setdefault("equipment_issue_history",[]).append({"at":at,"kind":"house_armory_issue","item_key":str(item_key),"quantity":quantity,"source_ref":"equipment_inventories","complete_loadout_units_converted":converted})
    formation["equipment_issue_history"]=formation["equipment_issue_history"][-32:]
    planner.put(_INVENTORY_PATH,registry); planner.put(formation_path,formation)
    return {"formation_ref":str(formation_ref),"item_key":str(item_key),"quantity":quantity,"complete_loadout_units_converted":converted,"equipment_units_by_role":copy.deepcopy(dict(formation.get("equipment_units_by_role",{}))),"equipment_completeness":formation.get("equipment_completeness"),"equipment_staging_by_item":copy.deepcopy(dict(formation.get("equipment_staging_by_item",{})))}

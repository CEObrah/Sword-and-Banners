"""Final commit-integrity layer for military career and loyalty.
...
from sword_runtime.military_career_release_integrity import MilitaryCareerReleaseIntegrityMixin


class MilitaryCareerCommitIntegrityMixin(MilitaryCareerReleaseIntegrityMixin):
    """Fail closed before consequential personnel writes and preserve config shape."""
    ...
    def _interstate_theater_config(self, base: Mapping[str, Any], *, at: str | None = None) -> dict[str, Any]:
        # Bypass only the release overlay's projection wrapper so all unrelated
        # civil-world config keys survive unchanged, then apply the same lawful
        # formation filter to the theater lists.
        config = copy.deepcopy(
            super(MilitaryCareerReleaseIntegrityMixin, self)._interstate_theater_config(base, at=at)
        )
...
__all__ = ["MilitaryCareerCommitIntegrityMixin"]

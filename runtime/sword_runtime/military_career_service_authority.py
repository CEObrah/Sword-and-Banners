...
* derived strategic-supply pressure affects loyalty only while a formation is
  actually in a field/mobilized posture, while explicit starvation/history
  evidence still applies everywhere; and
...
class MilitaryCareerServiceAuthorityMixin(MilitaryCareerFormationDynamicsMixin):
    ...
    # ------------------------------------------------------------------
    # Field strategic supply is not the same thing as garrison subsistence.
    # ------------------------------------------------------------------
    ...
    def _update_formation_loyalty(self, formation_ref: str, at: str) -> None:
        ...
        if not operational and float(supply_state["overall_ratio"]) < 1.0:
            # Base settlement uses low strategic support as a field-operating signal.
            # Undo only that fixed increment in a non-field posture. Explicit
            # missed-pay/starvation/history events remain bounded evidence.
            ...
            loyalty["nonfield_supply_interpretation"] = "derived field-supply pressure ignored outside field posture; explicit subsistence evidence remains authoritative"
    ...
    def _service_authority_release_score(self, petition: Mapping[str, Any]) -> int:
        ...
        if service_ref.startswith("house_"):
            ...
            try:
                self._command_person(service_ref, active=False)
            except (KeyError, ValueError, FileNotFoundError):
                is_person_authority = False
            else:
                is_person_authority = True
        if not service_ref.startswith("house_") and is_person_authority:
    ...
    def _settle_petitions(self, state_ref: str, at: str) -> None:
        self._recover_pending_petition_routes(state_ref)

"""Release-boundary integrity for the military career and loyalty subsystem.
...
from sword_runtime.military_career_final_integrity import MilitaryCareerFinalIntegrityMixin
...
class MilitaryCareerReleaseIntegrityMixin(MilitaryCareerFinalIntegrityMixin):
    """Final authority/conservation bridges for production release."""
    ...
    def _review_service_authority(self, state_ref: str, at: str) -> None:
        ...
        for petition_ref in pending:
            ...
            if target_ref and target_admin == service_ref and str(petition.get("request_kind")) != "independent_command":
                ...
                continue
            score = self._service_authority_release_score(petition)
    ...
    def _create_transfer_order(
        ...
        *,
        ...
        order = {
            ...
            "travel_hours": hours,
            "status": "in_transit",
            "authority_rule": "service/state approval authorizes movement only; conserved force and formation owners remain authoritative throughout",
        }
    ...
    def _interstate_theater_config(self, base: Mapping[str, Any], *, at: str | None = None) -> dict[str, Any]:
        config = copy.deepcopy(super()._interstate_theater_config(base, at=at))
...
__all__ = ["MilitaryCareerReleaseIntegrityMixin"]

...
from sword_runtime.campaign_communications import (
    command_endpoint_location, command_message_route, ensure_player_message_delivery,
...
def _text_blob(person: Mapping[str, Any]) -> str:
    ...
    for key in ("role", "role_archetype", "authority", "affiliation"):
        ...
        if isinstance(value, str):
            pieces.append(value)
    for owner in (person.get("goal_state"), person.get("career_state")):
...
def _military_score(person: Mapping[str, Any]) -> int:
    ...
    values = []
    for name in ("Formation Command", "Leadership", "Tactics", "Strategy", "Formation Fighting", "Logistics", "Riding"):
...
def _combat_service_score(person: Mapping[str, Any]) -> int:
    """Capability signal for entry as a rank-and-file combatant, not officer merit."""
    skills = person.get("skills") if isinstance(person.get("skills"), Mapping) else {}
    values: list[float] = []
    for name in ("Athletics", "Sword", "Polearms", "Bow", "Crossbow", "Unarmed", "Grappling", "Scouting", "Survival"):
        value = skills.get(name)
        if isinstance(value, (int, float)) and not isinstance(value, bool):
            values.append(float(value))
    values.sort(reverse=True)
    practical = int(round(sum(values[:5]) / max(1, len(values[:5])))) if values else 0
    return max(_military_score(person), practical)


def _staff_service_score(person: Mapping[str, Any]) -> int:
    """Capability signal for a non-command military staff/strategist entry track."""
    skills = person.get("skills") if isinstance(person.get("skills"), Mapping) else {}
    professional = person.get("professional_skills") if isinstance(person.get("professional_skills"), Mapping) else {}
    attributes = person.get("attributes") if isinstance(person.get("attributes"), Mapping) else {}
    values: list[float] = []
    for owner, names in (
        (skills, ("Strategy", "Tactics", "Logistics", "Formation Command", "Scouting")),
        (professional, ("Intelligence Operations", "Diplomacy", "Law")),
        (attributes, ("Intelligence", "Awareness", "Composure")),
    ):
        for name in names:
            ...
            if isinstance(value, (int, float)) and not isinstance(value, bool):
                values.append(float(value))
    values.sort(reverse=True)
    return int(round(sum(values[:6]) / max(1, len(values[:6])))) if values else 0


def _service_entry_track(person: Mapping[str, Any]) -> str | None:
    """Classify a genuine civilian/independent service aspirant by positive evidence.

    Absence of a formation is never enough. Senior commanders may live in command
    groups, while sovereigns and court officials need not occupy formations. A
    track therefore requires civilian/no-office state plus saved aspiration or a
    current training path that actually points toward military work.
    """
    if not _is_alive_adult(person) or isinstance(person.get("current_formation_id"), str):
        return None
    provenance = person.get("population_provenance") if isinstance(person.get("population_provenance"), Mapping) else {}
    if str(provenance.get("service_stratum", "")) in {"active_military", "private_household_military", "rebel_military"}:
        return None
    if isinstance(provenance.get("force_ref"), str) and provenance.get("force_ref"):
        return None
    career = person.get("career_state") if isinstance(person.get("career_state"), Mapping) else {}
    office = str(career.get("office_or_command", "")).lower()
    path = str(career.get("current_professional_path", "")).lower()
    if office and "no military office" not in office and "no state military office" not in office:
        return None
    rank = person.get("military_rank") if isinstance(person.get("military_rank"), Mapping) else {}
    grade = str(rank.get("grade", "not_formally_recorded")).lower()
    if grade not in {"", "none", "not_formally_recorded", "civilian"}:
        return None
    ...
    regimen = str(activity.get("resolved_training_regimen_ref", "")).lower()
    combat_paths = {
        "martial_aspirant", "military_aspirant", "military_candidate", "army_candidate",
        "royal_service_candidate", "independent_martial_operative",
    }
    combat_terms = (
        "enter the world of generals", "enter military", "enter the military",
        "enter service", "military service", "become a great general",
        "rise to great general", "martial aspirant", "aspiring martial",
        "join the army", "join army", "become a soldier", "command identity",
    )
    if path in combat_paths or any(term in blob for term in combat_terms):
        return "combatant"

    staff_paths = {"strategic_apprentice", "military_staff_aspirant", "strategist_candidate", "staff_candidate"}
    if path in staff_paths or regimen in {"strategic_apprentice", "military_staff_apprentice"}:
        return "staff"
    return None


def _civilian_service_entry_eligible(person: Mapping[str, Any]) -> bool:
    return _service_entry_track(person) is not None
...
class MilitaryCareerLoyaltyMixin:
    ...
    def _career_dossier(self, commander_ref: str | None) -> Mapping[str, Any] | None:
        """Resolve one commander dossier without granting authority to its routing index."""
        if not isinstance(commander_ref, str) or not commander_ref:
            return None
        network = self._career_network()
        routed = network.get("commanders", {}).get(commander_ref) if isinstance(network.get("commanders"), Mapping) else None
        canonical = f"state/military/career-network/commanders/{_slug(commander_ref)}.json"
        ...
        for path in candidates:
            if not isinstance(path, str) or not path or path in seen:
                continue
            seen.add(path)
            dossier = self.read_optional(path)
            if not isinstance(dossier, Mapping):
                continue
            if str(dossier.get("schema", "")) != "sword-commander-career-dossier":
                continue
            if str(dossier.get("commander_ref", "")) != commander_ref:
                continue
            return dossier
        return None
    ...
    def _recover_pending_petition_routes(self, state_ref: str) -> dict[str, Any]:
        """Repair pending petition routing from bounded exact officer authority.

        The petition index is explicitly a cache.  An exact officer's active
        petition references plus the exact petition owner are sufficient to
        restore a dropped pending route without scanning repository files.
        """
        index = self._petition_index()
        state_rows = index.setdefault("pending_by_state", {})
        pending = state_rows.setdefault(state_ref, [])
        known = {str(ref) for ref in pending if isinstance(ref, str)}
        network = self._career_network()
        people = network.get("people", {}) if isinstance(network.get("people"), Mapping) else {}
        changed = False
        for person_ref in sorted(str(ref) for ref in people if isinstance(ref, str)):
            try:
                _person_path, person = self._exact_person(person_ref, active=False)
            except (FileNotFoundError, KeyError, ValueError):
                continue
            for petition_ref in self._active_petition_refs(person):
                if petition_ref in known:
                    continue
                petition = self.read_optional(self._petition_path(petition_ref))
                if not isinstance(petition, Mapping):
                    continue
                if str(petition.get("schema", "")) != "sword-military-career-petition":
                    continue
                if str(petition.get("petition_ref", "")) != petition_ref:
                    continue
                if str(petition.get("officer_ref", "")) != person_ref:
                    continue
                if str(petition.get("state_ref", "")) != state_ref:
                    continue
                pending.append(petition_ref)
                known.add(petition_ref)
                changed = True
        if changed:
    ...
    def _update_formation_loyalty(self, formation_ref: str, at: str) -> None:
        ...
        if float(supply_state["overall_ratio"]) < 1.0:
    ...
    def _commander_dossier(self, commander_ref: str, person: Mapping[str, Any], at: str) -> dict[str, Any]:
        ...
        food_signal = 500
        if command_scale > 0:
            food_signal = _clamp(int(military_supply_sufficiency(self, formation)["overall_milli"]))
    ...
    def _schedule_player_petition_delivery(self, petition: Mapping[str, Any], at: str) -> dict[str, Any]:
        """Route an approved personnel petition to Wei through real geography.

        Institutional approval is bureau-side truth.  It is not player knowledge
        until a courier reaches Wei's current position.  The one-shot host keeps
        the original target so the shared delivery law can chase him if he moves.
        """
        petition_ref = str(petition.get("petition_ref") or "")
        state_ref = str(petition.get("state_ref") or "")
        if not petition_ref or not state_ref:
            raise ValueError("military career petition delivery lacks exact refs")
        bureau_ref = f"inst_{state_ref.removeprefix('state_')}_military_bureau"
        origin = command_endpoint_location(self, bureau_ref)
        target = player_command_location(self)
        if not origin or not target:
            raise ValueError("military career petition delivery lacks physical endpoints")
        route = command_message_route(self.read, origin, target, round_trip=False)
        travel_seconds = max(0, int(route.get("travel_seconds", 0) or 0))
        due = CampaignTime.parse(at).add_seconds(travel_seconds)
        token = _digest(f"petition-delivery:{petition_ref}")
        host_id = f"host_military_career_petition_delivery_{token}"
        event_id = f"event_military_career_petition_delivery_{token}"
        runtime = copy.deepcopy(self.read(_RUNTIME_PATH))
        hosts = runtime.get("hosts")
        events = runtime.get("events")
        if not isinstance(hosts, dict) or not isinstance(events, list):
        ...
        return {
            ...
            "event_id": event_id,
            "due_at": str(due),
            "origin_location_ref": origin,
            "target_location_ref": target,
            "travel_seconds": travel_seconds,
        }

    def _settle_player_petition_delivery(self, host: Mapping[str, Any], at: str) -> str | None:
        petition_ref = str(host.get("petition_ref") or host.get("owner_ref") or "")
        if not petition_ref:
            raise ValueError("military career petition delivery lost petition_ref")
        path = self._petition_path(petition_ref)
        raw = self.read_optional(path)
        if not isinstance(raw, Mapping):
            return None
        petition = copy.deepcopy(dict(raw))
        if petition.get("status") not in {"approved_pending_commander_delivery", "awaiting_commander_response"}:
            return None
        if petition.get("status") == "awaiting_commander_response":
            return str(petition.get("delivered_event_ref") or "") or None
        if not ensure_player_message_delivery(self, host, at):
            return None
        event_ref = self._deliver_player_petition(petition, at)
        petition["status"] = "awaiting_commander_response"
        petition["delivered_event_ref"] = event_ref
        petition["delivered_at"] = at
        petition["delivery_rule"] = "bureau approval is not player receipt; the petition becomes answerable only after physical courier delivery"
        self.put(path, petition)
        return event_ref
    ...
    def _player_petition_response(self, petition: Mapping[str, Any]) -> str | None:
        ...
        event_ref = f"event_{petition['petition_ref']}_delivered"
        attempts, _total = recent_interaction_attempts(self, _PLAYER_REF, limit=64)
        for parsed in reversed(attempts):
            ...
            if petition.get("status") == "awaiting_commander_response" and petition.get("desired_commander_ref") == _PLAYER_REF:
                ...
                if score >= int(rules["approve_at_milli"]):
                    if petition.get("desired_commander_ref") == _PLAYER_REF:
                        petition["status"] = "approved_pending_commander_delivery"
                        petition["institutional_decision"] = "approved_subject_to_prospective_commander_response"
                        delivery = self._schedule_player_petition_delivery(petition, at)
                        petition["delivery_due_at"] = delivery["due_at"]
                        petition["delivery_origin_location_ref"] = delivery["origin_location_ref"]
                        petition["delivery_target_location_ref"] = delivery["target_location_ref"]
                        petition["delivery_host_id"] = delivery["host_id"]
                        keep.append(petition_ref)
                    else:
                ...
                self.put(path, petition)
            elif petition.get("status") == "approved_pending_commander_delivery":
                keep.append(petition_ref)
    ...
    def _service_entry_target_state(self, person: Mapping[str, Any]) -> str | None:
        """Resolve the lawful state that can receive a civilian/independent applicant.

        Existing state affiliation wins. Independents require an exact physical
        location and use the current territorial controller of that location or
        its enclosing demographic/territorial parent. Static polity identity is
        only a fallback when no current control row exists.
        """
        state_ref = _state_ref(person)
        if isinstance(state_ref, str) and state_ref.startswith("state_") and not state_ref.startswith("state_independent"):
            return state_ref
        location_ref = self._person_location(person) if hasattr(self, "_person_location") else None
        if not isinstance(location_ref, str) or not location_ref:
            return None
        territory = self.read_optional("state/territory/control.json")
        sites = territory.get("sites", {}) if isinstance(territory, Mapping) and isinstance(territory.get("sites"), Mapping) else {}
        cursor = location_ref
        seen: set[str] = set()
        fallback: str | None = None
        while isinstance(cursor, str) and cursor and cursor not in seen:
            seen.add(cursor)
            site = sites.get(cursor)
            if isinstance(site, Mapping):
                controller = site.get("controller")
                if isinstance(controller, str) and controller.startswith("state_"):
                    return controller
                parent = site.get("demographic_parent_ref") or site.get("parent_location_ref")
                if isinstance(parent, str) and parent and parent not in seen:
                    cursor = parent
                    continue
            try:
                row = self._location_record(cursor)
            except (KeyError, ValueError, FileNotFoundError):
                break
            polity = row.get("polity_ref") if isinstance(row, Mapping) else None
            if fallback is None and isinstance(polity, str) and polity.startswith("state_"):
                fallback = polity
            parent = row.get("parent_ref") if isinstance(row, Mapping) else None
    ...
    def _settle_person_career(self, person_ref: str, at: str) -> None:
        ...
        if person_ref != _PLAYER_REF and not self._active_petition_refs(person):
            ...
            entry_track = _service_entry_track(person)
            if entry_track:
                entry_rules = self._military_rules().get("service_entry", {})
                target_state_ref = self._service_entry_target_state(person)
                signal = _combat_service_score(person) if entry_track == "combatant" else _staff_service_score(person)
                minimum_key = "minimum_military_signal" if entry_track == "combatant" else "minimum_staff_signal"
                minimum = int(entry_rules.get(minimum_key, self._military_rules()["career_review"]["minimum_military_signal"]))
                interest = career.setdefault("service_entry_interest", {})
                interest.update({
                    "track": entry_track,
                    "signal": signal,
                    ...
                    "reviewed_at": at,
                    "status": "eligible" if target_state_ref and signal >= minimum else ("awaiting_exact_location_or_state_access" if not target_state_ref else "developing"),
                })
                if target_state_ref and signal >= minimum:
                    attraction = _clamp(
                        int(entry_rules.get("petition_base_milli", 500))
                        + signal * int(entry_rules.get("military_signal_weight_milli_per_point", 2))
                        + int(prefs.get("ambition", 500)) // int(entry_rules.get("ambition_divisor", 5))
                    )
                    petition_ref = self._create_petition(
                        person,
                        state_ref=target_state_ref,
                        desired_commander_ref=None,
                        request_kind="service_entry",
                        attraction_milli=attraction,
                        evidence_refs=[],
                        at=at,
                    )
                    if petition_ref:
                        ...
                        petition["service_entry_signal"] = signal
                        self.put(petition_path, petition)
                        interest["status"] = "petition_submitted"
                        interest["petition_ref"] = petition_ref
            else:

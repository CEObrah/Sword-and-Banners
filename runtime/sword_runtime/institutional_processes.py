...
def _event_owner(planner: Any) -> tuple[str, dict[str, Any]]:
    owners = planner.read("state/index/owner-index-gold.json").get("owners", {})
    path = owners.get(EVENT_OWNER_REF)
    if not isinstance(path, str):
        raise ValueError("institutional event owner route is invalid")
    owner = copy.deepcopy(planner.read(path))
    if owner.get("schema") != "event-registry" or owner.get("owner_id") != EVENT_OWNER_REF:
        raise ValueError("institutional event owner is invalid")
    if not isinstance(owner.setdefault("causal_events", {}), dict):
        raise ValueError("institutional event owner causal_events are invalid")
    return path, owner
...
def _trigger_at(planner: Any, source: Mapping[str, Any], route: Mapping[str, Any]) -> str | None:
    ...
    history = planner.read(HISTORY_PATH).get("events", [])
    if not isinstance(history, list):
        raise ValueError("institutional interaction history is invalid")
...
def sync_institutional_process_routes(planner: Any, runtime: dict[str, Any]) -> None:
    ...
    for route in routes:
        source = causal.get(route["source_event_ref"])
        if not isinstance(source, Mapping) or source.get("status") != "triggered":
            continue
        if route["response_event_ref"] in causal:
            continue

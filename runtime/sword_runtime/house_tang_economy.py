"""House Tang force-employment and commercial settlement.

Sword Manor is a permanent House Tang military force. This layer never treats
it as a public school or automatic private-service labor pool.
"""
...
class HouseTangEconomyMixin:
    def _settle_house_tang_commercial_infrastructure(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        runtime.update({
            "schema": "house-tang-commercial-runtime",
            "last_close": at,
            "months": months,
    ...
    def _house_tang_operation_location(self) -> str | None:
        ...
        for _ref, path in sorted(operations.items()):
            ...
            for formation_ref in op.get("formation_refs", []) if isinstance(op.get("formation_refs"), list) else []:
                ...
                if source in {"force_house_tang", "force_tang_wei", "force_sword_manor"}:
    ...
    def _house_tang_contingency_mercenary_offers(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        standing: set[str] = set()  # Four Bastion Corps are permanent House Tang forces, not mercenary hosts.
        committed = 0
        ...
        for neg_headcount, ref, path, company in sorted(candidates):
            ...
            contracts.append({
                ...
                "basis": "House Tang contingency external-auxiliary policy, exact threat/operation need, short-notice company availability, fair-pay floor and protected treasury reserve",
            })
        ...
        if offers:
            house = copy.deepcopy(self.read(_HOUSE))
            runtime_row = house.setdefault("administrative_programs", {}).setdefault("contingency_external_auxiliaries", {})
    ...
    def _run_due_host(self, host: Mapping[str, Any], due_text: str) -> None:
        ...
        kind = host.get("kind")
        if kind == "house" and host.get("owner_ref") == "house_tang":

...
from sword_runtime.terrain import terrain_context_for_label
...
def demographic_anchor(read: Callable[[str], Mapping[str, Any]], location_ref: str, eligible_refs: Iterable[str]) -> str | None:
    ...
    while current in rows and current not in seen:
        ...
        if current in eligible:
            return current
        row = rows[current]
        for key in ("demographic_parent_ref", "settlement_ref", "parent_ref", "region_ref", "access_node_ref"):
            nxt = row.get(key)
...
def _edge_hours(read: Callable[[str], Mapping[str, Any]], route: Mapping[str, Any], mode: str) -> int:
    ...
    terrain = terrain_context_for_label(read, str(route.get("terrain", "plain")))
    terrain_factor = float(int(terrain["mechanical_effects"]["travel_time_milli"])) / 1000.0
    road_quality = str(route.get("road_quality", "maintained"))
    road_map = rules.get("road_quality_factors", {}) or {}
    if road_quality not in road_map:
        raise ValueError(f"unregistered road quality: {road_quality}")
    road_factor = float(road_map[road_quality])
    ...
    if base <= 1 and crossing_delay <= 0:
        return 1
...
def _weighted_route_graph(
    ...
    The graph depends on immutable route/location/rule data plus three mutable
    routing owners: territory route state, strategic crossings, and dynamic
    geography. Repository planners invalidate ``_geography_route_epoch`` whenever
    one of those owners is written, allowing all other path queries in the same
    causal generation to reuse the exact weighted graph. Bare read callables have
    no planner owner and therefore remain uncached for test overlays.
    """
    owner = getattr(read, "__self__", None)
    epoch = int(getattr(owner, "_geography_route_epoch", 0)) if owner is not None else -1
    cache = getattr(owner, "_geography_route_graph_cache", None) if owner is not None else None
    if owner is not None and not isinstance(cache, dict):
        cache = {}
        setattr(owner, "_geography_route_graph_cache", cache)
    cache_key = (epoch, tuple(requested))
...
def nearest_reachable_destination(
    ...
    *,
    ...
    owner = getattr(read, "__self__", None)
    epoch = int(getattr(owner, "_geography_route_epoch", 0)) if owner is not None else -1
    path_cache = getattr(owner, "_geography_path_cache", None) if owner is not None else None
    if owner is not None and not isinstance(path_cache, dict):
        path_cache = {}
        setattr(owner, "_geography_path_cache", path_cache)
    cache_key = (epoch, "nearest", origin, tuple(sorted(targets)), requested)
    if edge_allowed is None and isinstance(path_cache, dict) and cache_key in path_cache:
        return copy.deepcopy(path_cache[cache_key])
    ...
    if origin in targets:
        result = {"destination": origin, "duration_hours": 0, "modes": list(requested)}
        if edge_allowed is None and isinstance(path_cache, dict):
            path_cache[cache_key] = copy.deepcopy(result)
        return result
    ...
    while queue:
        ...
        if cost != best.get(node):
            continue
        if node in targets:
            result = {"destination": node, "duration_hours": int(cost), "modes": list(requested)}
            if edge_allowed is None and isinstance(path_cache, dict):
                path_cache[cache_key] = copy.deepcopy(result)
            return result
...
def shortest_path(
    ...
    *,
    ...
    # Reuse exact path results only while the planner's mutable route epoch is
    # unchanged. Custom edge predicates remain uncached because caller-local
    # admission logic may change independently of the physical route owners.
    owner = getattr(read, "__self__", None)
    epoch = int(getattr(owner, "_geography_route_epoch", 0)) if owner is not None else -1
    path_cache = getattr(owner, "_geography_path_cache", None) if owner is not None else None
    if owner is not None and not isinstance(path_cache, dict):
        path_cache = {}
        setattr(owner, "_geography_path_cache", path_cache)
    cache_key = (epoch, "shortest", origin, destination, requested)
    if edge_allowed is None and isinstance(path_cache, dict) and cache_key in path_cache:
        return copy.deepcopy(path_cache[cache_key])

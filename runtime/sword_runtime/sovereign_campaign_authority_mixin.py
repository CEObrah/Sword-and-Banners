...
class SovereignCampaignAuthorityMixin:
    ...
    def put(self, path: str, value: Any) -> None:
        """Never serialize read-time campaign-authority projections.

        ``read()`` deliberately exposes an exact sovereign campaign order through
        the familiar ``war_intents`` compatibility shape.  Callers frequently use
        ordinary read-modify-write flows on state documents, so the persistence
        boundary must strip those derived rows again or a projection can silently
        become a second sovereign fact in the save.
        """
        state_ref = self._state_ref_from_path(path)
        if state_ref is not None and isinstance(value, Mapping):
            cleaned = copy.deepcopy(dict(value))
            intents = cleaned.get("war_intents")
            if isinstance(intents, list):
                cleaned["war_intents"] = [
                    ...
                    if not (
                        ...
                        and str(row.get("kind", "")) == "exact_state_campaign_order_entry_authority"
                    )
                ]
            value = cleaned
    ...
    def _reconcile_campaign_entry_authority(self) -> list[str]:
        ...
        for operation_ref, _path, operation in iter_exact_operation_records(self):
            ...
            current_order = current_operational_order(operation)
            if isinstance(current_order, Mapping) and str(current_order.get("status", "")) == "staged_awaiting_entry_authority":
                waiting = True
            if not waiting:
                continue
            owner_ref = str(

...
def open_reward_review(
    ...
    *,
    ...
    if _state_key(subject.get("state")) != state:
        raise ValueError("reward subject belongs to a different state")
...
def review_state_reward_candidates(planner, *, state: str, at: str) -> dict[str, Any]:
    ...
    for row in iter_history_events(planner):
        ...
        if merit < floor:
            continue
        candidates.append((merit, person_ref, str(row.get("event_id", ""))))
    if not candidates:
        return {"created": False, "reason": "no_reward_candidate"}
    candidates.sort(key=lambda x: (-x[0], x[1], x[2]))
    _merit, person_ref, evidence_ref = candidates[0]

...
def resolve_program_ref(
    ...
    *,
    ...
    if isinstance(person, Mapping):
        ...
        role_text = " ".join(
            str(person.get(key, ""))
            for key in ("role", "rank", "role_archetype", "authority", "current_goal")
        ).lower()
    ...
    command_role = bool(role_text and any(token in role_text for token in ("commander", "general", "marshal", "deputy", "officer")))
    if branch_program:
    ...
    # Saved role/rank labels can prove command status even when older records lack
    # a structured billet. This is a deterministic legacy compatibility route.
    if command_role:
...
def instructor_quality_factor(
    ...
    *,
    instructor_training: float,
    instructor_domain_skill: float,
    trainee_skill: float,
) -> float:
    """Registered deterministic instructor-quality formula.

    Teaching ability and actual domain mastery both matter. The formula is data-owned
    ...
    """
    base = 0.78
    training_term = min(0.32, max(0.0, float(instructor_training)) / 300.0)
    domain_term = min(0.25, max(0.0, float(instructor_domain_skill) - float(trainee_skill)) / 320.0)
    return max(0.75, min(1.35, base + training_term + domain_term))
...
def combat_skill_weights(registry: Mapping[str, Any], program_ref: str) -> dict[str, float]:
    ...
    for row in program.get("rotation", []):
        if not isinstance(row, Mapping):
            continue
        drill = drill_record(registry, str(row.get("drill_ref", "")))
        skills = [str(x) for x in drill.get("skills", []) if str(x) and str(x) != "Training"]
        if not skills:
            continue
...
_COMBAT_COMMAND_ROLES = {
    "commander", "deputy", "higher_commander", "higher_deputy",
...
def combat_skill_weights_for_participant(
    ...
    remains eligible for the program's physical/weapon/Mass Combat domains.
    """
...
def settle_exact_program(
    ...
    *,
    ...
    available = person.get("skills") if isinstance(person.get("skills"), Mapping) else {}
...
def formation_drill_access(
    ...
    *,
    ...
    # Mounts are shared physical assets. Determine how many current formation bodies
    # are on roles whose registered program actually contains mounted work, then
    # distribute the exact serviceable mount pool as a bounded aggregate share.
    mounted_required = 0
    training_ref = str(formation.get("training_ref", ""))
    for other_role, count in composition.items():
        n = max(0, int(count or 0))
        if n <= 0:
            continue
        other_program = resolve_program_ref(registry, role=str(other_role), training_ref=training_ref)
        needs_mount = False
    ...
    mount_share = max(0.0, min(1.0, physical_mounts / float(max(1, mounted_required)))) if mounted_required > 0 else 0.0
...
def settle_cohort_program(
    ...
    *,
    ...
    d_alloc = {row["drill_ref"]: row for row in module_allocations(registry, program_ref, deliberate, integer_hours=False, skill_values=(cohort.get("skill_means") if isinstance(cohort.get("skill_means"), Mapping) else {}), attribute_values=(cohort.get("attribute_means") if isinstance(cohort.get("attribute_means"), Mapping) else {}), promotion_facts=promotion_facts, cursor=int(cohort.get("smart_training_cursor", 0) or 0))}
    e_alloc = {row["drill_ref"]: row for row in module_allocations(registry, program_ref, exposure, integer_hours=False, skill_values=(cohort.get("skill_means") if isinstance(cohort.get("skill_means"), Mapping) else {}), attribute_values=(cohort.get("attribute_means") if isinstance(cohort.get("attribute_means"), Mapping) else {}), promotion_facts=promotion_facts, cursor=int(cohort.get("smart_training_cursor", 0) or 0))}
    ...
    history = cohort.setdefault("deterministic_training_history", [])
    if isinstance(history, list):
        history.append({"evidence_ref": evidence_ref, "program_ref": program_ref, "deliberate_hours": round(deliberate, 3), "role_exposure_hours": round(exposure, 3), "modules": trace})
        cohort["deterministic_training_history"] = history[-24:]
...
def settle_person_lite_program(
    ...
    *,
    ...
    history_row = {
        "evidence_ref": evidence_ref, "program_ref": program_ref,
        "requested_deliberate_hours": round(requested_deliberate, 3),
        "deliberate_hours": round(actual_deliberate, 3),
        "role_exposure_hours": round(float(role_exposure_hours), 3),
        "time_reservation": deepcopy(time_reservation) if time_reservation is not None else None,
        "model": "deterministic_registered_program", "modules": deepcopy(result.get("modules", [])),
    }
    history = dev.setdefault("training_history", [])
    if isinstance(history, list):
        history.append(history_row)
        dev["training_history"] = history[-24:]
    # Keep the person-level summary for existing readers, but development_state is
    # the cumulative training authority for person-lite officers.
    top_history = person.setdefault("training_history", [])
    if isinstance(top_history, list):
        top_history.append(history_row)
        person["training_history"] = top_history[-24:]

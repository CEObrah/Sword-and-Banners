...
from sword_runtime.development import PHYSICAL_SKILLS, age_years, settle_skill_training
...
def settle_training_session(
    ...
    *,
    ...
    attribute_targets: Sequence[str] | None = None,
    ...
    body_dependent_skills = set(PHYSICAL_SKILLS) | {"Engineering", "Medicine", "Scouting", "Stealth", "Survival"}
    bodily_function_factor = anatomy_activity_factor(person, skill) if skill in body_dependent_skills else 1.0
    effective_instruction_factor = max(0.0, float(instruction_factor)) * bodily_function_factor
    result = settle_skill_training(
        person, skill, hours, at, training,
        facility_grade=facility_grade, equipment_grade=equipment_grade,
        recovery_grade=recovery_grade, practice_mode=practice_mode,
        intensity=intensity, feedback_grade=feedback_grade,
        instruction_factor=effective_instruction_factor, instructor_capacity_factor=instructor_capacity_factor,
        instructor_ref=instructor_ref,
    )
    result["bodily_function_factor"] = round(bodily_function_factor, 6)
    ...
    if attribute_targets is not None:
        raw_targets: Sequence[str] = attribute_targets
    else:
    ...
    instruction = max(0.0, min(1.35, float(effective_instruction_factor)))
    ...
    for attribute in targets:
        ...
        raw_edu = per_target_hours * mode_factor * intensity_factor * instruction * instructor_capacity * facility * equipment * recovery * feedback * health * age_factor * aptitude_factor

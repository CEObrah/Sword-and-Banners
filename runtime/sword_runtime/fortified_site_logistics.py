...
def build_logistics_blueprint(profile: Mapping[str, Any], rules: Mapping[str, Any]) -> dict[str, Any]:
    ...
    tang_nested = str(profile.get("site_ref", "")) in {"loc_tang_manor", "loc_tang_inner_walls", "loc_tang_inner_citadel"}
    if tang_nested:
...
def current_garrison_requirements(
    *,
    ...
    desired = {
        ...
        "construction_material_units": max(50, int(round(max(0, personnel) * 0.1225))),

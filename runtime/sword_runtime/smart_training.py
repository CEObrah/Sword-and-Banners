"""Legacy compatibility helpers for pre-registry training records.

Production gain-bearing training must not call this module to choose future skill
or attribute development from current stats. The canonical authority is
``training_programs.py`` plus ``deterministic-training-programs.json``: saved role,
billet and training refs select a finite registered rotation with fixed gain targets.
These helpers remain only for migration/diagnostic compatibility with older data.
"""

...
def _pending_qin_briefings_without_support_path(store: Any, hosts: Mapping[str, Any]) -> int:
    ...
    for operation_ref, order_ref in _iter_player_qin_pending_briefings(store):
        has_route = any(
            ...
            and host.get("next_due") is not None
            and host.get("order_ref") == order_ref
            for host in hosts.values()
        )
        if not has_route:
            blocked += 1
...
def _redundant_qin_support_follow_on_hosts(store: Any, hosts: Mapping[str, Any]) -> int:
    ledger = _optional_json(store, "state/index/interaction-attempts.json")
    ...
    return sum(
        ...
        and isinstance(by_ref.get(str(host.get("work_ref"))), Mapping)
        and is_campaign_command_follow_on_request(by_ref[str(host.get("work_ref"))])
    )
...
def summarize_playability_vitality(store: Any) -> dict[str, Any]:
    ...
    redundant_qin_follow_on_support = _redundant_qin_support_follow_on_hosts(store, hosts)
    ...
    if redundant_qin_follow_on_support:
        diagnostics.append("campaign_follow_on_request_has_redundant_qin_support_route")
        suggestions.append("retire_qin_support_shadow_route_and_preserve_superior_command_delivery")
    ...
    return {
        ...
        "redundant_qin_support_campaign_follow_on_hosts": redundant_qin_follow_on_support,

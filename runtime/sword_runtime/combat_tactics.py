...
def build_team_plan(
    ...
    *,
    ...
    if knowledge_by_actor is not None:
        ...
        # An explicit knowledge map is authoritative even when it contains no
        # enemies. Empty current detection must never fall back to the exact
        # hidden combat roster merely because the resolver knows those bodies.
        active_enemy = [r for r in active_enemy if r in shared_known]
        if not active_enemy:
            return {
                "plan_id": "none", "leader_ref": leader,
                "primary_threat_ref": None, "desired_state": "hold",
                "assignments": {}, "known_enemy_refs": [],
            }
...
def known_tactical_candidates(candidates: Sequence[str], plan: Mapping[str, Any] | None) -> list[str]:
    """Bound autonomous target candidates by the current team knowledge plan.

    Absence of a knowledge field preserves legacy benchmark callers. An explicit
    empty knowledge list means exactly that: no target is currently attributable.
    """
    raw = (plan or {}).get("known_enemy_refs") if isinstance(plan, Mapping) else None
    if not isinstance(raw, list):
        return [str(ref) for ref in candidates]
    known = {str(ref) for ref in raw}
    return [str(ref) for ref in candidates if str(ref) in known]
...
def choose_tactical_target(
    ...
    *,
    ...
    """
    raw_known = (plan or {}).get("known_enemy_refs")
    known_is_authoritative = isinstance(raw_known, list)
    known = {str(x) for x in raw_known} if known_is_authoritative else set()
    alive = [
        r for r in candidates
        if r in people
        and not bool((people[r].get("combat_state") or {}).get("incapacitated"))
        and (r in known if known_is_authoritative else True)
    ]
    if not alive:
        return None

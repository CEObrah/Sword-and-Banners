...
def choose_tactical_target(
    ...
    *,
    ...
    def score(target: str) -> tuple[float, str]:
        d = distance_2d(positions[actor_ref], positions[target]) if actor_ref in positions and target in positions else 100.0
        injury = _health_pressure(people[target])
        if role == "exploit":
            value = injury * 1.6 - d * 2.0
        else:
            value = -d * 2.0 + injury * 0.4
        return value, target

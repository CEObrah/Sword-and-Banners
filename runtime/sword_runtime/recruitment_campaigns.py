...
def _registry(planner: Any) -> dict[str, Any]:
    ...
    if not isinstance(value,Mapping): return {"schema":"recruitment-candidate-pool-registry.v1","owner_id":"recruitment_candidate_pools","campaigns":{}}
...
def train_campaign(planner: Any, payload: Mapping[str, Any], *, evidence_ref: str) -> dict[str, Any]:
    ...
    profiles=planner.read(PROFILE_PATH); rules=planner.read("game/data/mechanics/training.json"); regimens=profiles.get("training_regimens",{}); regimen=regimens.get("house_tang_max_sustainable",{}) if isinstance(regimens,Mapping) else {}; role_profiles=profiles.get("role_training_profiles",{}); focus=role_profiles.get(str(campaign.get("role","household_retainer")),{}) if isinstance(role_profiles,Mapping) else {}
    changed=0
    for s in campaign.get("slices",[]):
        ...
        advance_cohort_training(profile,deliberate_hours=float(hours),role_exposure_hours=0.0,skill_focuses=focus.get("skills",[]) if isinstance(focus,Mapping) else [],attribute_focuses=focus.get("attributes",[]) if isinstance(focus,Mapping) else [],training_rules=rules,facility_grade=str(regimen.get("facility_grade","adequate")),equipment_grade=str(regimen.get("equipment_grade","adequate")),recovery_grade=str(regimen.get("recovery_grade","adequate")),evidence_ref=evidence_ref)
        changed+=1

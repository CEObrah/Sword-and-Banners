...
def start_campaign(planner: Any, payload: Mapping[str, Any], *, evidence_ref: str) -> dict[str, Any]:
    ...
    for source,count in source_counts.items():
        ...
        for bg,n in bg_counts.items():
            ...
            slices.append({"slice_id":_slice_id(campaign_ref,source,bg),"source_stratum":source,"background_profile":bg,"count":n,"profile":deepcopy(dict(base))})
        strata[source]=int(strata.get(source,0))-count
    strata[reserved_key]=int(strata.get(reserved_key,0))+applicants
    campaigns[campaign_ref]={"campaign_ref":campaign_ref,"state":state,"status":"screening","destination_force_ref":force_ref,"role":role,"location_ref":location,"started_at":str(planner._world_time()),"initial_applicants":applicants,"remaining_candidates":applicants,"reserved_stratum":reserved_key,"slices":slices,"provenance_ref":evidence_ref,"local_reservations":local_reservations}
    _credit_recruitment_payment(planner,state,contact_cost,kind="candidate_contact",evidence_ref=evidence_ref,campaign_ref=campaign_ref,location_ref=location)
...
def stage_campaign(planner: Any, payload: Mapping[str, Any], *, evidence_ref: str) -> dict[str, Any]:
    ...
    for s in slices:
        ...
        if after<=0: continue
        fraction=after/max(1,before); apply_selection_profile(s["profile"],selection,retain_fraction=fraction); s["count"]=after; new_slices.append(s)
    if hasattr(planner,"_release_local_candidate_rejections"):
        planner._release_local_candidate_rejections(pop,campaign_ref,rejected_by_source)
    campaign["slices"]=new_slices; campaign["remaining_candidates"]=target
    _credit_recruitment_payment(planner,str(campaign["state"]),screening_cost,kind="candidate_screening",evidence_ref=evidence_ref,campaign_ref=campaign_ref,location_ref=str(campaign.get("location_ref", "")))
    planner.put(pop_path,pop); planner.put(treasury_path,treasury); planner.put(REGISTRY_PATH,reg)
...
def train_campaign(planner: Any, payload: Mapping[str, Any], *, evidence_ref: str) -> dict[str, Any]:
    ...
    changed=0
    training_location = str(campaign.get("location_ref", ""))
    environment = training_environment(planner, location_ref=training_location, simultaneous_trainees=max(1, candidates)) if training_location else {"facility_grade": "none", "capacity_factor": 0.0}
    ...
    for slice_index, s in enumerate(campaign.get("slices",[])):
        ...
        profile=s["profile"]
        profile.setdefault("skill_edu_banks",{}); profile.setdefault("attribute_edu_banks",{}); profile.setdefault("verified_training_hours_per_person",0.0); profile.setdefault("verified_role_exposure_hours_per_person",0.0)
        ...
        instructor_contexts=instructor_contexts_for_program(
            ...
            trainee_skills=cohort_merged_skill_means(profile),
            student_count=max(1,int(s.get("count",0) or 0)),location_ref=training_location,
        ...
        drill_access=program_facility_access(planner,registry=registry,program_ref=program_ref,location_ref=training_location)
        settle_cohort_program(profile,registry=registry,program_ref=program_ref,deliberate_hours=effective_hours,role_exposure_hours=0.0,training_rules=rules,facility_grade=str(environment.get("facility_grade","none")),equipment_grade=str(regimen.get("equipment_grade","adequate")),recovery_grade=str(regimen.get("recovery_grade","adequate")),evidence_ref=slice_evidence,instructor_context_by_drill=instructor_contexts,drill_access=drill_access)
        changed+=1
    campaign["status"]="training_candidate"; campaign["verified_training_hours_per_person"]=round(float(campaign.get("verified_training_hours_per_person",0.0))+hours,3); planner.put(treasury_path,treasury); planner.put(REGISTRY_PATH,reg)
...
def cancel_campaign(planner: Any, payload: Mapping[str, Any], *, evidence_ref: str) -> dict[str, Any]:
    ...
    if hasattr(planner,"_release_local_candidate_rejections"):
        planner._release_local_candidate_rejections(pop,campaign_ref,returned)
    if int(strata.get(reserved,0))<total: raise ValueError("reserved candidate population is inconsistent")
    strata[reserved]-=total; reg.get("campaigns", {}).pop(campaign_ref, None)

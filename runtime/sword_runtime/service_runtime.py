...
class CommandRoutedProductionPlanner(ProductionCampaignPlanner):
    ...
    def _command_activity_contract(person: Mapping[str, Any]) -> dict[str, Any] | None:
        ...
        identity supplies standing-role status; House Tang/Sword Manor/Tang-retinue
        ...
        tang_owned = any(token in affiliation for token in ("house tang", "house_tang", "sword manor", "tang wei"))
        tang_owned = tang_owned or any(token in formation_ref for token in ("house_tang", "sword_manor", "tang_champion", "house_guard"))
        tang_owned = tang_owned or program_ref.startswith(("program.sword_", "program.commander_champion", "program.commander_guard", "program.commander_cavalry", "program.bastion_", "program.tang_field_"))
        ...
        contract.setdefault(
            "growth_rule",
            "verified elapsed hours settle only through the finite deterministic training-program registry selected from saved role/billet; current stat ranking and prose-created focuses are non-authoritative",
        )
        return contract
    ...
    def _ensure_activity_routes(self) -> None:
        ...
        for person_ref, path in sorted(record_index.items()):
            ...
            if (
                ...
                and int(development.get("settled_training_hours", 0) or 0) == 0
                and not isinstance(development.get("inherited_training_baseline"), Mapping)
                and not isinstance(development.get("progression_tracking_baseline"), Mapping)
            ):
                development["progression_tracking_baseline"] = {
                    "tracking_started_at": str(activity.get("routed_at", now_text) or now_text),
            ...
            activity["verification_rule"] = (
                "exact command staff use verified elapsed deterministic role training; current saved contract/regimen is re-resolved on every bounded route reconciliation"
            )

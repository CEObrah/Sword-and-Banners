...
class CommandRoutedProductionPlanner(ProductionCampaignPlanner):
    ...
    def _named_unit_command_count(formation: Mapping[str, Any], target: int) -> int:
        """Count the one exact external top commander without embedded ranks."""
        target = max(0, int(target))
        if target <= 0:
            return 0
        ref = formation.get("commander_ref")
        return min(target, 1 if isinstance(ref, str) and ref else 0)

    @staticmethod
    def _is_full_exact_command_person(path: object, person: object) -> bool:
        """Classify full command people from owner/schema authority.

        Storage layout and identity spelling are not representation authority.
        Full exact command people normally live under ``state/char`` today, but
        representation upgrades may retain a routed ``state/person`` owner and
        exact sparse materializations use ``sword-materialized-person``.
        Person Lite remains excluded from this exact-person activity path.
        """
        if not isinstance(path, str) or not isinstance(person, Mapping):
            return False
        if not (path == "state/player.json" or path.startswith("state/char/") or path.startswith("state/person/")):
            return False
        return str(person.get("schema", "")) in {"sab_character", "sword-materialized-person"}

    @staticmethod
    def _command_activity_contract(person: Mapping[str, Any]) -> dict[str, Any] | None:
        """Return a complete standing-activity contract for exact command staff.

        Repository migrations can materialize a full exact commander with only a
        deterministic ``training_program_ref``. That partial record is not allowed
        to fall through to the historical 48-hour generic cache. The saved command
        identity supplies standing-role status; House Tang/Inner Walls/Tang-retinue
        command personnel use the canonical maximum-sustainable regimen unless an
        explicit regimen already says otherwise.
        """
        explicit = person.get("activity_contract")
        if isinstance(explicit, Mapping):
            contract = copy.deepcopy(dict(explicit))
        else:
            derived = CommandRoutedProductionPlanner._derived_activity_contract(person)
            if not isinstance(derived, Mapping):
                return None
            contract = copy.deepcopy(dict(derived))
        contract.setdefault("autonomous_enabled", True)
        contract.setdefault("mode", "standing_role_training")
        affiliation = str(person.get("affiliation", "")).lower()
        formation_ref = str(person.get("current_formation_id", "")).lower()
        program_ref = str(contract.get("training_program_ref", "")).lower()
        tang_owned = any(token in affiliation for token in ("house tang", "house_tang", "inner walls", "tang wei"))
        tang_owned = tang_owned or any(token in formation_ref for token in ("house_tang", "red_lance", "high_guard"))
        tang_owned = tang_owned or program_ref.startswith(("program.house_infantry", "program.house_cavalry", "program.commander_cavalry", "program.tang_field_"))
        if not str(contract.get("training_regimen_ref", "")).strip():
            contract["training_regimen_ref"] = "house_tang_max_sustainable" if tang_owned else "regular_army"
        return contract

    def _ensure_activity_routes(self) -> None:
        """Reconcile full exact command staff into life and smart-training hosts.

        The command-personnel index is a bounded routing surface. Every active full
        character in it must have both an annual person host and, when its current
        saved contract permits training, one named-person activity route. Existing
        routes are reconciled too, so representation upgrades and regimen changes do
        not preserve stale cached rates such as the old 56-hour person-lite value.
        """
        ...
        added: list[str] = []
        reconciled: list[str] = []
        life_host_refs: list[str] = []

        for person_ref, path in sorted(record_index.items()):
            if not isinstance(person_ref, str) or person_ref == self.PLAYER_ACTOR or not isinstance(path, str):
                continue
            person = copy.deepcopy(self.read(path))
            if not self._is_full_exact_command_person(path, person):
                continue
            life = str(person.get("life_status", person.get("status", "active"))).lower()
            if life in {"dead", "deceased"}:
                continue
            life_host_refs.append(person_ref)
            contract = self._command_activity_contract(person)
            ...
            if person.get("activity_contract") != contract:
                person["activity_contract"] = copy.deepcopy(contract)
            ...
            development = person.setdefault("development_state", {})
            if not isinstance(development, dict):
                raise ValueError("exact command person development_state is invalid")
            ...
            if shards and len(shards[-1][1].get("routed_person_refs", [])) < _ACTIVITY_SHARD_SIZE:
                ...
                target_host = self._ensure_activity_shard(
                    hosts, events, index=len(shards), now=now, now_text=now_text,
                )
        ...
        if isinstance(routing, dict):
            ...
            routing["exact_command_reconciled"] = len(reconciled)
        metrics = runtime.setdefault("metrics", {})
        if isinstance(metrics, dict) and added:
        ...
        # Life hosts must be created after the activity-runtime write above because
        # _ensure_person_life_host performs its own keyed state/runtime mutation.
        before_hosts = set(hosts)
        for person_ref in sorted(set(life_host_refs)):
            self._ensure_person_life_host(person_ref, now)
        after_runtime = self.read("state/runtime.json")
        after_hosts = after_runtime.get("hosts", {}) if isinstance(after_runtime, Mapping) else {}
        created_life_hosts = bool(set(after_hosts) - before_hosts) if isinstance(after_hosts, Mapping) else False
        if created_life_hosts:
            # Classify the newly created bounded person hosts immediately. This is
            # idempotent and does not add duplicate activity-route refs.
            super()._ensure_activity_routes()
...
class ProductionSwordRuntime(SwordRuntime):
    ...
    def _exact_person_record(self, person_ref: str) -> dict[str, Any]:
        ...
        if not isinstance(person, dict) or str(person.get("schema", "")) not in {"sab_character", "sword-materialized-person"}:
    ...
    def _validate_player_authored_agency(self, command) -> None:
        ...
        if command.command_type == "house_action" and str(payload.get("action", "assign_duty")) == "assign_duty":
            ...
            pforce = self.store.read_json("state/pforce/wei.json")
            pforce_members = {str(x) for x in pforce.get("members", []) if isinstance(x, str)} if isinstance(pforce, Mapping) else set()
            personally_controlled = subject_ref in pforce_members
            if (
    ...
    def execute(self, command, *args, **kwargs):
        # Retries remain valid after a successful action changes agency/wake
        # preconditions. Public adapters authorize the actor before reaching here.
        identity = kwargs.get("receipt_command") or command

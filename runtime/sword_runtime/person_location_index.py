...
The index owns no person state. It is maintained whenever an exact character
owner is written so systems such as outbreaks can review only named people who
are materially present at a site instead of globally scanning the cast.
"""
...
def person_refs_at_locations(planner: Any, location_refs: set[str]) -> list[str]:
    ...
    for loc in sorted(str(x) for x in location_refs if isinstance(x, str)):
        ...
        out.update(str(x) for x in rows if isinstance(x, str) and str(x).startswith("char_"))
    return sorted(out)

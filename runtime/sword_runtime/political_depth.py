...
class PoliticalDepthMixin:
    ...
    def _court_judgment_relationship(self, assessment: Mapping[str, Any], decision: str) -> dict[str, Any]:
        ...
        return {
            ...
            "rule": "evidentiary support and sovereign judgment are separate; a divergent judgment remains a real political/legal act and does not rewrite the evidence",
        }
    ...
    def _court_detain_person(self, case: dict[str,Any], person_ref: str, custodian_formation_ref: str, *, at: str) -> dict[str,Any]:
        ...
        group_ref=self._custody_attach_named_capture(person_ref,custodian_formation_ref,str(case.get("case_ref","court_case")),at);gp,group=self._custody_group(group_ref);group["legal_status"]="court_detention";group.setdefault("history",[]).append({"at":at,"kind":"court_detention","case_ref":case.get("case_ref")});self.put(gp,group)
    ...
    def _court_release_person(self, person_ref: str, prisoner_group_ref: str, *, at: str, case_ref: str) -> dict[str,Any]:
        ...
        refs.remove(person_ref);group["named_prisoner_refs"]=refs;group["guard_requirement"]=(int(group.get("personnel",0))+len(refs)+4)//5;group.setdefault("history",[]).append({"at":at,"kind":"court_release","case_ref":case_ref,"person_ref":person_ref});self.put(gp,group);self._custody_set_active(group)
    ...
    def _dispatch_polity_action(self, command: Any, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if action == "enforce_court_case":
            ...
            if remedy=="office_removal":effect=self._court_remove_office(polity_ref,polity,person_ref,str(payload.get("office_key")) if payload.get("office_key") else None,at=now,case_ref=case_ref)
                ...
                pp,p0=self._exact_person(person_ref,active=False);person=copy.deepcopy(p0);self._settle_person_death(person_ref,pp,person,now,"court sentence execution");refs.remove(person_ref);group["named_prisoner_refs"]=refs;group["guard_requirement"]=(int(group.get("personnel",0))+len(refs)+4)//5;group.setdefault("history",[]).append({"at":now,"kind":"court_execution","person_ref":person_ref,"case_ref":case_ref});self.put(gp,group);self._custody_set_active(group);effect={"kind":"execution_completed","person_ref":person_ref,"prisoner_group_ref":group_ref}

...
from sword_runtime.unit_establishment import normalize_formation_establishment, validate_establishment
...
class RepositoryCommandPlanner(OperationalBattlefieldMixin, CohortTxSupportMixin, PersonalCombatMixin, CombatCapabilityMixin, StandingForceCapabilityMixin):
    ...
    def _autonomy_person(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        for i in range(max(0,int(occurrences))):
            ...
            # every elapsed year.
            if person_ref!=self.PLAYER_ACTOR and contract and not routed_activity_training and "not automatic progress" not in str(contract.get("growth_rule","")).lower() and self._person_health(person) in {"healthy","fit","stable"}:
                ...
                contexts=instructor_contexts_for_program(self,registry=registry,training_rules=training,program_ref=program_ref,trainee_skills=(person.get("skills",{}) if isinstance(person.get("skills"),Mapping) else {}),student_count=1,location_ref=str(self._person_location(person) or ""),trainee_ref=person_ref,scheduled_hours=48.0,window_start=str(window_start),window_end=str(review),evidence_ref=evidence,reserve_duty=True,hierarchical_delivery=bool(person.get("command_assignment")))
                ...
                person.setdefault("autonomous_development_history",[]).append({"at":str(review),"program_ref":program_ref,"hours":48,"gain_authority":"registered_deterministic_program","development":development}); person["autonomous_development_history"]=person["autonomous_development_history"][-12:]
    ...
    def _dispatch(self, command: CommandEnvelope, payload: Mapping[str, Any]) -> Dict[str, Any]:
        ...
        if t=="travel":
            ...
            route=self._find_route(origin,dest,mode=mode); duration=int(route.get("duration_hours",route.get("hours",24))); current=self._world_time(); settle_person_idle_fatigue(player,current=current,rules=self.read(FATIGUE_RULES_PATH),state="ordinary"); target_time=current.add_seconds(duration*3600); target=str(target_time); m=self._advance_runtime(target); player["location"]=dest
            ...
            self.put("state/player.json",player); self._write_meta(command,target); return self._result(origin=origin,destination=dest,route_ref=route.get("ref", route.get("route_ref")),route_refs=route.get("route_refs",[]),route_path=route.get("path",[]),duration_hours=duration,world_time=target,**m)
        if t=="individual_training":
            ...
            contexts=instructor_contexts_for_program(self,registry=registry,training_rules=training,program_ref=program_ref,trainee_skills=(player.get("skills",{}) if isinstance(player.get("skills"),Mapping) else {}),student_count=1,location_ref=str(self._person_location(player) or ""),trainee_ref=self.PLAYER_ACTOR,scheduled_hours=float(hours),window_start=str(current),window_end=target,evidence_ref=evidence,reserve_duty=True,focus_drill_ref=drill_ref)
        ...
        if t=="cohort_training":
            ...
            for cid,cohort in ledger.get("cohorts",{}).items():
                ...
                gains={k:round(float(v)-before.get(k,float(v)),3) for k,v in cohort.get("skill_means",{}).items() if float(v)-before.get(k,float(v))>1e-9}
        ...
        if t in {"formation_reconstitute","formation_train","formation_mobilize","formation_demobilize","formation_doctrine_set","formation_training_set","formation_assign","force_assignment","command_assign","command_transfer","formation_move","resupply"}:
            ...
            if t=="formation_reconstitute":
                ...
                hours=int(payload.get("hours",1)); current=self._world_time(); settle_formation_idle_fatigue(f,current=current,rules=self.read(FATIGUE_RULES_PATH)); world_time=str(current.add_seconds(hours*3600)); time_metrics=self._advance_runtime(world_time); f["training_progress"]=_clamp(int(f.get("training_progress",0))+max(1,hours//4)); f["cohesion"]=_clamp(int(f.get("cohesion",50))+max(1,hours//4)); f["readiness"]=_clamp(int(f.get("readiness",50))+max(0,hours//6)); stamp_formation_activity_fatigue(f,completed_at=CampaignTime.parse(world_time),fatigue_gain=max(1,hours//5),activity_kind="training"); f["verified_training_hours"]=int(f.get("verified_training_hours",0))+hours; f["last_training_at"]=world_time

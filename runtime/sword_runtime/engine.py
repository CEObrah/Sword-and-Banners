...
from sword_runtime.family_routing import exact_family_record, exact_family_records_for_person
from sword_runtime.fortification_routing import exact_fortification_record
from sword_runtime.reputation_routing import (
    reputation_profile_path,
    reputation_subject_path,
    resolve_reputation_profile,
    resolve_reputation_subject,
)
...
class RepositoryCommandPlanner(OperationalBattlefieldMixin, CohortTxSupportMixin, PersonalCombatMixin, CombatCapabilityMixin, StandingForceCapabilityMixin):
    ...
    def __init__(self, root: object) -> None:
        ...
        # These are planner-local routing caches only. Exact owner documents stay
        # authoritative; the cache merely avoids repeatedly counting identical
        # reads of stable indexes during one long autonomous transaction.
        self._owner_index_owners_cache: Optional[Mapping[str, Any]] = None
        self._routing_document_cache: Dict[str, Any] = {}
    ...
    def _reset(self) -> None:
        ...
        self._writes = {}
        self._deletes = set()
        self._owner_index_owners_cache = None
        self._routing_document_cache = {}
        self._geography_route_epoch = 0
    ...
    def read(self, path: str) -> Any:
        ...
        # The planner is a transactional overlay. Once a whole owner has been
        # staged for deletion, neither a previously cached fragment nor the
        # baseline repository may resurrect it inside the same transaction.
        if base_path in self._deletes:
    ...
    def put(self, path: str, value: Any) -> None:
        base_path, tokens = split_json_fragment(path)
        self._routing_document_cache.pop(base_path, None)
        if base_path == "state/index/owner-index.json":
            self._owner_index_owners_cache = None
        if base_path in {
        ...
        if not tokens:
            ...
            person_schema = str(stored_value.get("schema", "")) if isinstance(stored_value, Mapping) else ""
            is_person_owner_path = (
                base_path == "state/player.json"
                or base_path.startswith("state/char/")
                or base_path.startswith("state/person/")
            )
            if is_person_owner_path and person_schema in {"sab_character", "sword-materialized-person", "person-lite"}:
                person_ref = str(stored_value.get("owner_id") or stored_value.get("id") or "")
                if person_ref:
                    sync_person_location(self, person_ref=person_ref, person=stored_value)
                    # Political alignment candidates require a full exact person.
                    # Person Lite remains individually physical but compressed.
                    if person_schema in {"sab_character", "sword-materialized-person"}:
        ...
        # Fragment-backed person-lite/materialized people are still exact
        # conserved bodies. Keep the same bounded physical-location projection
        # current whenever their individual fragment changes; otherwise a named
        # officer could become mechanically invisible to site-local hazards just
        # because compact storage is used.
        if isinstance(value, Mapping) and str(value.get("schema", "")) in {"person-lite", "sword-materialized-person", "sab_character"}:
            person_ref = str(value.get("id") or value.get("owner_id") or "")
            if person_ref:
                sync_person_location(self, person_ref=person_ref, person=value)
    ...
    def delete(self, path: str) -> None:
        ...
        if base_path in {
            ...
            if isinstance(path_cache, dict):
                path_cache.clear()
        if not tokens:
            if base_path == "state/player.json" or base_path.startswith("state/char/") or base_path.startswith("state/person/"):
                prior = self.read_optional(base_path)
                if isinstance(prior, Mapping):
                    person_schema = str(prior.get("schema", ""))
                    person_ref = str(prior.get("owner_id") or prior.get("id") or "")
                    if person_ref and person_schema in {"sab_character", "sword-materialized-person", "person-lite"}:
            ...
            self._cache.pop(base_path, None)
            return
        prior_fragment = self.read_optional(path)
        if isinstance(prior_fragment, Mapping) and str(prior_fragment.get("schema", "")) in {"person-lite", "sword-materialized-person", "sab_character"}:
            person_ref = str(prior_fragment.get("id") or prior_fragment.get("owner_id") or "")
            if person_ref:
                remove_person_location(self, person_ref)
        base_value = copy.deepcopy(self._writes[base_path]) if base_path in self._writes else copy.deepcopy(self.read(base_path))
    ...
    def _owner_index_owners(self) -> Mapping[str, Any]:
        cached = self._owner_index_owners_cache
        if isinstance(cached, Mapping):
            return cached
        idx = self.read("state/index/owner-index.json")
        owners = idx.get("owners", {}) if isinstance(idx, Mapping) else {}
        if not isinstance(owners, Mapping):
            raise ValueError("authoritative owner index is invalid")
        self._owner_index_owners_cache = owners
        return owners

    def routing_document(self, path: str) -> Any:
        base_path, tokens = split_json_fragment(path)
        if tokens:
            raise ValueError("routing_document requires a whole-document path")
        if base_path in self._routing_document_cache:
            return self._routing_document_cache[base_path]
        value = self.read(base_path)
        self._routing_document_cache[base_path] = value
        return value

    def routing_document_optional(self, path: str) -> Any:
        base_path, tokens = split_json_fragment(path)
        if tokens:
            raise ValueError("routing_document_optional requires a whole-document path")
        if base_path in self._routing_document_cache:
            return self._routing_document_cache[base_path]
        value = self.read_optional(base_path)
        self._routing_document_cache[base_path] = value
        return value

    def owner_path(self, owner_ref: str) -> str:
        path = self._owner_index_owners().get(owner_ref)
    ...
    def _validate_invariants(self, overlay: StagedOverlay, paths: Iterable[str]) -> None:
        ...
        if isinstance(rt, dict):
            ...
            # Committed chronology has one exact frontier.  A transaction may
            # stage these documents in either order, but the final overlay may
            # never persist split campaign clocks.  Catching this here prevents
            # silent scheduler drift from becoming a later gameplay handoff bug.
            world_time = rt.get("world_time")
            if isinstance(world_time, str) and meta.get("time") != world_time:
                raise ValueError("campaign meta time diverged from runtime world time")
            scheduler = rt.get("scheduler", {})
            if isinstance(scheduler, Mapping):
                settled = scheduler.get("causal_settled_through")
                if isinstance(world_time, str) and isinstance(settled, str) and settled != world_time:
                    raise ValueError("scheduler causal frontier diverged from runtime world time")

        # Exact-person location aliases are legacy-compatible, not independent
        # authorities.  If both spellings survive on one record they must agree.
        # Check the player every commit because a split player location can poison
        # travel, command, scene, and delivery routing even when the current
        # command did not explicitly touch state/player.json.
        player = overlay.read_json("state/player.json")
        if isinstance(player, Mapping):
            ...
            if (
                ...
                raise ValueError("player location aliases diverged")

        # Formation strength and exact commander service metadata are one current
        # billet fact.  Validate only formations/people touched by this transaction
        # so normal commits stay bounded while any writer that changes strength or
        # billet state must leave the commander projection synchronized.
        owners_doc = overlay.read_json("state/index/owner-index.json")
        owners = owners_doc.get("owners", {}) if isinstance(owners_doc, Mapping) else {}
        formation_refs: set[str] = set()
        custody_group_paths: set[str] = set()

        def full_person_record(path: str, person: Mapping[str, Any]) -> bool:
        ...
        for raw_path in paths:
            path = str(raw_path)
            if path.startswith("state/custody/groups/") and overlay.read_optional_bytes(path):
                custody_group_paths.add(path)
                continue
            if path.startswith("state/formations/") and overlay.read_optional_bytes(path):
                formation = overlay.read_json(path)
                ref = formation.get("formation_ref") if isinstance(formation, Mapping) else None
                if isinstance(ref, str) and ref:
                    formation_refs.add(ref)
            elif (
                (path.startswith("state/char/") or path.startswith("state/person/"))
                and "#/" not in path
                and overlay.read_optional_bytes(path)
            ):
                person = overlay.read_json(path)
                if not isinstance(person, Mapping) or not full_person_record(path, person):
                    continue
                ...
                if isinstance(loc, str) and loc and isinstance(cur, str) and cur and loc != cur:
                    raise ValueError("exact-person location aliases diverged: %s" % path)
                assignment = person.get("command_assignment")
                ref = assignment.get("formation_ref") if isinstance(assignment, Mapping) else None
                if isinstance(ref, str) and ref.startswith("formation_"):
                    formation_refs.add(ref)
                ...
                if isinstance(custody, Mapping) and str(custody.get("status", "")) == "prisoner":
                    ...
                    group_path = owners.get(group_ref) if isinstance(group_ref, str) and isinstance(owners, Mapping) else None
                    if not isinstance(group_path, str) or not group_path.startswith("state/custody/groups/"):
                        raise ValueError("named prisoner has no authoritative custody group: %s" % path)
                    custody_group_paths.add(group_path)

        # Named custody is one physical fact represented by two exact owners:
        # the custody group and the already-existing person.  Validate only
        # groups touched directly or referenced by a touched prisoner so normal
        # transactions remain bounded.
        for group_path in sorted(custody_group_paths):
            if not overlay.read_optional_bytes(group_path):
                raise ValueError("named custody group owner is missing: %s" % group_path)
            group = overlay.read_json(group_path)
            if not isinstance(group, Mapping) or str(group.get("schema", "")) != "sword-prisoner-group":
                raise ValueError("named custody group owner is invalid: %s" % group_path)
            group_ref = str(group.get("owner_id", ""))
            group_location = str(group.get("location_ref", ""))
            named_refs = group.get("named_prisoner_refs", [])
            if not isinstance(named_refs, list):
                raise ValueError("named prisoner refs are invalid: %s" % group_ref)
            active_custody = str(group.get("status", "")) in {"held", "in_transit"}
            if not active_custody and named_refs:
                raise ValueError("inactive custody group still owns named prisoners: %s" % group_ref)
            for person_ref in named_refs:
                if not isinstance(person_ref, str) or not person_ref:
                    raise ValueError("named prisoner ref is invalid: %s" % group_ref)
                person_path = owners.get(person_ref) if isinstance(owners, Mapping) else None
                if not isinstance(person_path, str) or not overlay.read_optional_bytes(person_path.split("#/", 1)[0]):
                    raise ValueError("named prisoner has no authoritative person owner: %s:%s" % (group_ref, person_ref))
                person = overlay.read_json(person_path)
                if not isinstance(person, Mapping) or str(person.get("schema", "")) not in {"sab_character", "sword-materialized-person"}:
                    raise ValueError("named prisoner owner is not an exact person: %s:%s" % (group_ref, person_ref))
                custody = person.get("custody_state") if isinstance(person.get("custody_state"), Mapping) else {}
                if str(custody.get("status", "")) != "prisoner" or str(custody.get("prisoner_group_ref", "")) != group_ref:
                    raise ValueError("named prisoner custody pointer diverged: %s:%s" % (group_ref, person_ref))
                person_location = self._person_location(person)
                if group_location and person_location != group_location:
                    raise ValueError("named prisoner location diverged from custody group: %s:%s" % (group_ref, person_ref))
                if group_location and str(custody.get("location_ref", "")) != group_location:
                    raise ValueError("named prisoner custody location diverged: %s:%s" % (group_ref, person_ref))

        for formation_ref in sorted(formation_refs):
            formation_path = owners.get(formation_ref) if isinstance(owners, Mapping) else None
            if not isinstance(formation_path, str) or "#/" in formation_path or not overlay.read_optional_bytes(formation_path):
                continue
            formation = overlay.read_json(formation_path)
            if not isinstance(formation, Mapping):
                continue
            embedded_refs = formation.get("embedded_person_refs", [])
            if not isinstance(embedded_refs, list):
                raise ValueError("formation embedded person refs are invalid: %s" % formation_ref)
            for embedded_ref in embedded_refs:
                if not isinstance(embedded_ref, str) or not embedded_ref:
                    raise ValueError("formation embedded person ref is invalid: %s" % formation_ref)
                embedded_path = owners.get(embedded_ref) if isinstance(owners, Mapping) else None
                if not isinstance(embedded_path, str) or not embedded_path:
                    raise ValueError(
                        "formation embedded person has no authoritative owner: %s:%s"
                        % (formation_ref, embedded_ref)
                    )
                embedded_base, _embedded_tokens = split_json_fragment(embedded_path)
                if not overlay.read_optional_bytes(embedded_base):
                    raise ValueError(
                        "formation embedded person owner is missing: %s:%s"
                        % (formation_ref, embedded_ref)
                    )
                try:
                    embedded_person = overlay.read_json(embedded_path)
                except FileNotFoundError as exc:
                    raise ValueError(
                        "formation embedded person owner fragment is missing: %s:%s"
                        % (formation_ref, embedded_ref)
                    ) from exc
                if not isinstance(embedded_person, Mapping) or str(embedded_person.get("schema", "")) not in {
                    ...
                    raise ValueError(
                        "formation embedded owner is not an individual person: %s:%s"
                        % (formation_ref, embedded_ref)
                    )
            commander_ref = formation.get("commander_ref")
            if not isinstance(commander_ref, str) or not commander_ref:
                continue
            person_path = owners.get(commander_ref) if isinstance(owners, Mapping) else None
            if not isinstance(person_path, str) or not person_path:
                raise ValueError("formation commander has no authoritative person owner: %s" % formation_ref)
            person_base, _person_tokens = split_json_fragment(person_path)
            if not overlay.read_optional_bytes(person_base):
                raise ValueError("formation commander owner is missing: %s" % formation_ref)
            try:
                person = overlay.read_json(person_path)
            except FileNotFoundError as exc:
                raise ValueError("formation commander owner fragment is missing: %s" % formation_ref) from exc
            if not isinstance(person, Mapping) or str(person.get("schema", "")) not in {
                "sab_character", "sword-materialized-person", "person-lite",
            }:
                raise ValueError("formation commander owner is not an individual person: %s" % formation_ref)
            self._validate_commander_representation_for_formation(commander_ref, person, formation)
            assignment = person.get("command_assignment")
            if not isinstance(assignment, Mapping) or str(assignment.get("formation_ref", "")) != formation_ref:
                raise ValueError("formation commander assignment diverged: %s" % formation_ref)
            expected_span = max(0, int(formation.get("personnel", 0) or 0))
            if "current_command_span" in assignment and int(assignment.get("current_command_span", -1) or 0) != expected_span:
                raise ValueError("formation commander assignment span diverged: %s" % formation_ref)
            career = person.get("career_state")
            if isinstance(career, Mapping) and "current_command_span" in career:
                if int(career.get("current_command_span", -1) or 0) != expected_span:
                    raise ValueError("formation commander career span diverged: %s" % formation_ref)
            ...
            if isinstance(level, str) and level.endswith("_commander"):
                ...
                if prefix.isdigit() and int(prefix) != expected_span:
                    raise ValueError("formation commander level diverged: %s" % formation_ref)
    ...
    def _formation_path(self, ref: str) -> str:
        p = self._owner_index_owners().get(ref)
        if isinstance(p, str):
            return p
    ...
    def _load_formation(self, ref: str) -> tuple[str, Any]:
        ...
        row = self.read(p)
        if not isinstance(row, Mapping) or str(row.get("schema") or "") != "sword-formation" or str(row.get("formation_ref") or "") != str(ref):
            raise ValueError("formation owner identity mismatch: %s" % ref)
        return p, row
    ...
    def _formations_at(self, location_ref: str) -> list[str]:
        """Return exact formations physically at one location.

        The location index is an authority:false routing accelerator.  It may
        nominate candidates, but it can never establish physical presence by
        itself.  Re-read every candidate's exact formation owner before a
        caller uses the result for combat, occupation, logistics, training or
        other consequence-bearing mechanics.
        """
        wanted = str(location_ref)
        raw = self._formation_location_index().get("locations", {}).get(wanted, [])
        out: list[str] = []
        for value in raw if isinstance(raw, list) else []:
            if not isinstance(value, str) or not value:
                continue
            try:
                _path, formation = self._load_formation(value)
            except (KeyError, ValueError, FileNotFoundError):
                continue
            if isinstance(formation, Mapping) and str(formation.get("location_ref", "")) == wanted:
                out.append(value)
    ...
    def _require_commandable_person(self, actor_ref: str, person_ref: str, formation_ref: str | None = None) -> None:
        """Prove that a proposed individual commander is subject to the actor's command.
        ...
        person/person-lite officer whose saved service record binds them to Tang Wei's
        personal force. Establishment scale is validated separately, so this authority
        check must not reject a lawful sub-500 person-lite detachment commander merely
        because their representation is compact.
        ...
        if isinstance(formation_ref, str) and formation_ref:
            ...
            if str(formation.get("commander_ref") or "") == person_ref:
                return
        _, person = self._command_person(person_ref)
        affiliation = str(person.get("affiliation", "")).strip().lower()
        force = str(person.get("owner", "") or person.get("force_ref", "") or person.get("current_formation_id", "")).strip().lower()
        ...
        saved_retainer = (
            ...
            or force in {"personal_force_tang_wei", "force_tang_wei_personal"}
    ...
    def _authoritative_commander_formation_refs(self, commander_ref: str) -> list[str]:
        """Return bounded exact formations that actually name ``commander_ref``.

        ``commander-formation-index.json`` is authority:false routing state.  It
        may discover candidates, but it cannot create or suppress a command
        billet by itself.  The commander's exact sheet is the other bounded
        discovery source, and every candidate is revalidated against the exact
        formation owner before it is returned.
        """
        commander_ref = str(commander_ref)
        idx = self._commander_index()
        assignments = idx.get("assignments", {}) if isinstance(idx, Mapping) else {}
        candidates = {
            str(ref)
            for ref in (assignments.get(commander_ref, []) if isinstance(assignments, Mapping) else [])
            if isinstance(ref, str) and ref
        }
        try:
            _person_path, person = self._command_person(commander_ref, active=False)
        except (KeyError, ValueError, FileNotFoundError):
            person = None
        if isinstance(person, Mapping):
            command_assignment = person.get("command_assignment")
            if isinstance(command_assignment, Mapping):
                ref = command_assignment.get("formation_ref")
                if isinstance(ref, str) and ref:
                    candidates.add(ref)
            current = person.get("current_formation_id")
        ...
        for formation_ref in sorted(candidates):
            try:
                ...
                continue
            if str(formation.get("commander_ref") or "") == commander_ref:
                exact.append(formation_ref)
        return exact
    ...
    def _assign_commander_index(self, commander_ref: str, formation_ref: str, *, replace: bool = False) -> None:
        ...
        current = self._authoritative_commander_formation_refs(commander_ref)
    ...
    def _release_commander_index(self, commander_ref: Optional[str], formation_ref: str) -> None:
        if not commander_ref:
            return
        idx = self._commander_index(); assignments = idx.setdefault("assignments", {})
        current = [ref for ref in self._authoritative_commander_formation_refs(str(commander_ref)) if ref != formation_ref]
        if current:
    ...
    def _bind_formation_commander_sheet(
        ...
        *, person_path: str | None = None, person: Dict[str, Any] | None = None,
        ...
        existing_assignment = person.get("command_assignment")
        existing_ref = existing_assignment.get("formation_ref") if isinstance(existing_assignment, Mapping) else None
        if isinstance(existing_ref, str) and existing_ref and existing_ref != str(formation_ref):
            try:
                _existing_path, existing_formation = self._load_formation(existing_ref)
            except (KeyError, ValueError, FileNotFoundError):
                existing_formation = None
            if isinstance(existing_formation, Mapping) and str(existing_formation.get("commander_ref") or "") == str(commander_ref):
                raise ValueError(
                    f"exact commander {commander_ref} is already assigned to {existing_ref}"
                )
    ...
    def _authorize_command(self, command: CommandEnvelope, payload: Mapping[str, Any]) -> None:
        ...
        if t in state_capabilities:
            ...
            if t == "formation_create" and payload.get("force_ref"):
                ...
                if payload.get("commander_ref"):
                    # A new formation has no existing formation_ref to use as an
                    # authority bridge.  Creating it must not become a backdoor
                    # for forcibly appointing any co-located named person.
                    self._require_commandable_person(actor, str(payload["commander_ref"]))
            else:
        ...
        if t in formation_commands:
            ...
            if action == "record_terminal_evidence":
            ...
            elif action in {"redeploy", "set_order"}:
                ...
                if operational_command_ref:
                    ...
                    resolved = exact_operation_record(self, op_ref)
                    if resolved is None:
                        raise ValueError("unknown battlefield operation")
                    _op_path, op = resolved
                    battlefield = (op.get("battlefields") or {}).get(battlefield_ref)
                    if not isinstance(battlefield, Mapping):
                        raise ValueError("unknown operational battlefield")
                    _side_ref, refs = self._battlefield_operational_command_assignment_refs(battlefield, operational_command_ref)
                    for ref in refs:
                        self._require_formation_operational_authority(actor, ref)
                else:
                    self._require_formation_operational_authority(actor, str(payload.get("formation_ref", "")))
            else:
                op_ref = str(payload.get("operation_ref", ""))
    ...
    def _set_person_location(person: Dict[str, Any], value: str) -> None:
        # ``sab_character`` records historically used both ``location`` and
        # ``current_location``.  When both aliases survive on one exact person,
        # moving only one leaves different subsystems observing different places.
        # Keep every already-present location alias synchronized, while retaining
        # the normal single-field shape for records that use only one spelling.
        wrote = False
        if "location" in person:
            person["location"] = value
            wrote = True
        if "current_location" in person:
    ...
    def _formation_task_leader(self, formation: Mapping[str, Any]) -> Dict[str, Any] | None:
        ...
        if not isinstance(group_ref, str) or not group_ref:
            try:
                ...
                group_ref = authoritative_group_ref_for_formation(
                    idx if isinstance(idx, Mapping) else {},
                    lambda ref: self.read(f"state/cmd/command-groups/{ref}.json"),
                    formation_ref,
                    formation,
                ) if formation_ref else None
            except Exception:
                group_ref = None
    ...
    def _autonomy_apply_battle_losses(self, formation_ref: str, loss: int, at: str, *, losing_side: bool, opponent_state: str, seed_material: str) -> Dict[str, Any]:
        ...
        # Casualties change the live billet span of a surviving exact formation
        # commander.  Keep that service projection synchronized through the same
        # officer authority used by player-facing battle settlement; durable rank
        # remains independent from current commanded headcount.
    ...
    def _validate_person_location_for_formation(self, person_ref: str, formation: Mapping[str, Any]) -> tuple[str, Dict[str, Any]]:
        ...
        self._validate_commander_representation_for_formation(person_ref, person, formation)
    ...
    def _validate_commander_representation_for_formation(
        self,
        person_ref: str,
        person: Mapping[str, Any],
        formation: Mapping[str, Any],
    ) -> None:
        """Enforce the saved officer-representation threshold for top command.

        A persistent Unit-scale (500+) top command is a full-character billet.
        Sub-500 detachments may use an individually conserved person-lite officer.
        Use authorized establishment rather than casualty-depleted current manpower so
        a 487/500 Unit does not silently demote its command representation after losses.
        """
        current = max(0, int(formation.get("personnel", 0) or 0))
        klass = formation_class_for(
            formation,
            personnel=current,
            explicit=formation.get("formation_class"),
        )
        authorized = authorized_strength_for(
            formation,
            personnel=current,
            explicit_authorized=(
                int(formation.get("authorized_strength"))
                if formation.get("authorized_strength") is not None
                else None
            ),
            formation_class=klass,
        )
        policy = self.read("game/data/mechanics/officer-representation.json")
        full = policy.get("automatic_full_character", {}) if isinstance(policy, Mapping) else {}
        minimum = max(1, int(full.get("minimum_persistent_commanded_personnel", 500) or 500))
        if authorized >= minimum and str(person.get("schema", "")) == "person-lite":
            raise ValueError(
                f"{person_ref} cannot command persistent {authorized}-person establishment as person-lite; "
                f"{minimum}+ command requires a full exact character"
            )
    ...
    def _settle_person_death(self, person_ref: str, person_path: str, person: Dict[str, Any], at: str, reason: str, *, settle_force_body: bool = False) -> None:
        ...
        touched_group_refs:set[str]=set(affected_groups)
        ...
        def promote_vacated_descendant(group_ref: str, *, excluded: set[str]) -> str | None:
            ...
            touched_group_refs.add(group_ref)
            try:
                ...
                for candidate in list(group.get("successor_refs",[])):
                    ...
                    for prior_group_ref in list(command_routes.get(candidate,[])):
                        ...
                        if prior_group.get("commander_ref")==candidate:
                            ...
                            touched_group_refs.add(prior_group_ref)
        ...
        # Refresh only the people whose routing could have changed. Exact command
        # groups remain authority; this projection update must never scan every
        # command group merely because one commander died.
        command_projection=group_index.setdefault("command_person_groups",{})
        primary_projection=group_index.setdefault("primary_person_group",{})
        staff_projection=group_index.setdefault("staff_person_groups",{})
        routing_people={person_ref,*promoted_candidates}
        for routed_person_ref in sorted(routing_people):
            candidate_group_refs=set(touched_group_refs)
            for mapping in (command_routes, staff_routes):
                refs=mapping.get(routed_person_ref,[]) if isinstance(mapping,Mapping) else []
                candidate_group_refs.update(str(ref) for ref in refs if isinstance(ref,str))
            previous_primary=primary_projection.get(routed_person_ref) if isinstance(primary_projection,Mapping) else None
            if isinstance(previous_primary,str) and previous_primary:
                candidate_group_refs.add(previous_primary)

            commanded=[]; staffed=[]; memberships=[]; group_docs={}
            for candidate_group_ref in sorted(candidate_group_refs):
                try:
                    candidate_group=self.read(f"state/cmd/command-groups/{candidate_group_ref}.json")
                except (FileNotFoundError,KeyError,ValueError):
                    continue
                group_docs[candidate_group_ref]=candidate_group
                if str(candidate_group.get("commander_ref") or "")==routed_person_ref:
                    commanded.append(candidate_group_ref)
                roles=candidate_group.get("role_assignments")
                if isinstance(roles,Mapping) and routed_person_ref in roles:
                    staffed.append(candidate_group_ref)
                direct=candidate_group.get("direct_person_refs")
                if isinstance(direct,list) and routed_person_ref in direct:
                    memberships.append(candidate_group_ref)

            if routed_person_ref==person_ref:
                commanded=[]; staffed=[]; memberships=[]
            if commanded:
                command_projection[routed_person_ref]=sorted(set(commanded))
            else:
                command_projection.pop(routed_person_ref,None)
            if staffed:
                staff_projection[routed_person_ref]=sorted(set(staffed))
            else:
                staff_projection.pop(routed_person_ref,None)

            primary_candidates=commanded or memberships
            if primary_candidates:
                ranked=[]
                for candidate_group_ref in sorted(set(primary_candidates)):
                    candidate_group=group_docs.get(candidate_group_ref)
                    if not isinstance(candidate_group,Mapping):
                        continue
                    org=candidate_group.get("organizational_state") if isinstance(candidate_group.get("organizational_state"),Mapping) else {}
                    strength=int(org.get("current_recursive_strength",org.get("authorized_strength",0)) or 0)
                    parent=candidate_group.get("parent_command_group_ref")
                    is_child=1 if isinstance(parent,str) and parent else 0
                    ranked.append((-strength,is_child,candidate_group_ref))
                if ranked:
                    primary_projection[routed_person_ref]=sorted(ranked)[0][2]
                else:
                    primary_projection.pop(routed_person_ref,None)
            else:
                primary_projection.pop(routed_person_ref,None)
        ...
        for uid in list(person_family.get("unions",[])):
            resolved=exact_family_record(self.read_optional,fidx,"unions",str(uid))
            if resolved is None: continue
            up,union0=resolved; union=_deepcopy(union0)
        ...
        for sid in list(person_family.get("successions",[])):
            resolved=exact_family_record(self.read_optional,fidx,"successions",str(sid))
            if resolved is None: continue
            sp,succession0=resolved; succession=_deepcopy(succession0)
    ...
    def _reputation_subject_type(self, subject_ref: str) -> str:
        """Classify reputation owners without encoding identity in the ref spelling.

        ``organization`` was never a registered reputation-subject type, and
        person-lite/materialized people do not have to start with ``char_``.
        Resolve the authoritative owner first, then fall back to conservative
        ref-domain categories so every newly created subject is schema-valid.
        """
        try:
            _path, subject = self.owner(subject_ref)
        except (FileNotFoundError, KeyError, ValueError):
            subject = None
        if isinstance(subject, Mapping):
            schema = str(subject.get("schema", ""))
            if schema in {"sab_character", "sword-materialized-person", "person-lite"}:
                return "person"
            if "formation" in schema or schema in {"military-unit", "army", "command-group"}:
                return "unit"
            if "force" in schema or schema in {"force-state"}:
                return "force"
            if schema in {"sword-institution", "institution"}:
                return "institution"
            if schema in {"sword-house", "house", "faction", "polity", "state"}:
                return "faction"
            if "settlement" in schema:
                return "settlement"
        if subject_ref.startswith(("state_", "polity_", "house_", "faction_")):
            return "faction"
        if subject_ref.startswith(("formation_", "unit_", "cmdgrp_")):
            return "unit"
        if subject_ref.startswith("force_"):
            return "force"
        if subject_ref.startswith(("inst_", "institution_")):
            return "institution"
        return "other"
    ...
    def _record_reputation_signal(self, subject_ref: str, audience_ref: str, delta: int, dimension: str, source_event_ref: str, at: str, basis: str) -> str:
        ...
        idxp="state/reputation/index.json"; idx=_deepcopy(self.read(idxp))
        resolved_subject=resolve_reputation_subject(self.read_optional,idx,subject_ref)
        if resolved_subject is None:
            subject_path=reputation_subject_path(subject_ref)
            occupied=self.read_optional(subject_path)
            if occupied is not None:
                raise ValueError("reputation subject canonical path is occupied by another owner")
            subject={"schema":"reputation-subject","subject_id":subject_ref,"subject_type":self._reputation_subject_type(subject_ref),"as_of":at,"authority":True,"audience_profiles":{},"institutional_status_sources":[],"notes":[]}
        else:
            subject_path,subject0=resolved_subject; subject=_deepcopy(subject0)
        idx.setdefault("subjects",{})[subject_ref]=subject_path
        resolved_profile=resolve_reputation_profile(self.read_optional,subject,subject_ref,audience_ref)
        existing_profile=resolved_profile is not None
        if resolved_profile is None:
            profile_path=reputation_profile_path(subject_ref,audience_ref)
            occupied=self.read_optional(profile_path)
            if occupied is not None:
                raise ValueError("reputation profile canonical path is occupied by another owner")
            profile={"schema":"reputation-audience-profile","subject_id":subject_ref,"audience_id":audience_ref,"as_of":at,"authority":True,"standing":{"overall":0},"dimensions":{},"evidence_count":0,"last_event_refs":[],"memory_class":"normal"}
        else:
            profile_path,profile0=resolved_profile; profile=_deepcopy(profile0)
        eid="reputation."+hashlib.sha256((subject_ref+"|"+audience_ref+"|"+source_event_ref+"|"+dimension).encode()).hexdigest()[:16]; ep=f"state/reputation/events/{eid}.json"; new_event=self.read_optional(ep) is None
        if new_event:
        ...
        profile["as_of"]=at; self.put(profile_path,profile); subject.setdefault("audience_profiles",{})[audience_ref]=profile_path; subject["as_of"]=at; self.put(subject_path,subject); idx["subject_count"]=len(idx.get("subjects",{})); idx["audience_profile_count"]=int(idx.get("audience_profile_count",0))+(0 if existing_profile else 1); self.put(idxp,idx); return eid
    ...
    def _settle_due_pregnancy(self, mother_ref: str, mother_path: str, mother: Dict[str, Any], at: str) -> Optional[str]:
        ...
        idxp="state/family/index.json"; idx=_deepcopy(self.read(idxp)); union_ref=str(preg.get("union_ref","")); resolved_union=exact_family_record(self.read_optional,idx,"unions",union_ref)
        if resolved_union is None: raise ValueError("saved pregnancy lost its exact union authority")
        union_path,union0=resolved_union; union=_deepcopy(union0)
    ...
    def _settle_person_family_life_stage(self, person_ref: str, person: Dict[str, Any], review: CampaignTime) -> None:
        ...
        if age < 18:
            household_ids = [ref for ref, _path, _row in exact_family_records_for_person(
                self.read_optional, current_idx(), "households", person_ref,
            )]
            if not household_ids:
                parent_refs: list[str] = []
                idx_view = current_idx()
                for par_id, _par_path, par in exact_family_records_for_person(
                    self.read_optional, idx_view, "parentage", person_ref,
                ):
                    if str(par.get("child_id", "")) != person_ref:
                        continue
                    parent_refs.extend(str(x.get("parent_id")) for x in par.get("parent_links", []) if isinstance(x, Mapping) and x.get("parent_id"))
                parent_households: list[set[str]] = []
                for pref in sorted(set(parent_refs)):
                    vals = {ref for ref, _path, _row in exact_family_records_for_person(
                        self.read_optional, idx_view, "households", pref,
                    )}
                    if vals:
                ...
                for hid in sorted(shared):
                    resolved = exact_family_record(self.read_optional, idx_view, "households", hid)
                    if resolved is None:
                        continue
                    hpath, household_ro = resolved
                    members = {str(x) for x in household_ro.get("member_refs", []) if isinstance(x, str)}
                    if parent_refs and not set(parent_refs).issubset(members):
                        continue
                ...
                for hid in household_ids:
                    resolved = exact_family_record(self.read_optional, current_idx(), "households", hid)
                    if resolved is None:
                        continue
                    hpath, household_ro = resolved
                    deps = household_ro.get("dependent_refs", []) if isinstance(household_ro.get("dependent_refs"), list) else []
                    members = household_ro.get("member_refs", []) if isinstance(household_ro.get("member_refs"), list) else []
                    # person_index is routing only; it cannot add a person to an
                    # unrelated household. Existing exact membership is required.
                    if person_ref not in deps and person_ref not in members:
                        continue
            ...
            for hid, hpath, household_ro in exact_family_records_for_person(
                self.read_optional, current_idx(), "households", person_ref,
            ):
                deps = household_ro.get("dependent_refs", []) if isinstance(household_ro.get("dependent_refs"), list) else []
                members = household_ro.get("member_refs", []) if isinstance(household_ro.get("member_refs"), list) else []
    ...
    def _validate_command_semantics(self, command: CommandEnvelope, payload: Mapping[str, Any]) -> None:
        ...
        if t in {"recruitment", "population_transfer", "formation_create"}:
            ...
            if t in {"recruitment","formation_create"}:
                ...
                if t == "formation_create":
                    ...
                    if payload.get("commander_ref") is not None:
                        commander_ref = str(payload["commander_ref"])
                        _cp, commander = self._command_person(commander_ref)
                        self._validate_commander_representation_for_formation(
                            commander_ref,
                            commander,
                            {
                                "personnel": current,
                                "authorized_strength": authorized,
                                "formation_class": requested_class,
                            },
                        )
        ...
        if t in {"command_assign","command_transfer","formation_assign","force_assignment"}:
            if payload.get("commander_ref") is not None:
                commander_ref = str(payload["commander_ref"])
                _cp, commander = self._command_person(commander_ref)
                _fp, formation = self._load_formation(str(payload["formation_ref"]))
                self._validate_commander_representation_for_formation(commander_ref, commander, formation)
        ...
        if t == "battlefield_control":
            ...
            if action=="open":
                require_text(payload,"name",max_length=200)
                if "layout_ref" in payload:
                    require_text(payload,"layout_ref",allowed={"battlefield.layout.dynamic","battlefield.layout.line_three","battlefield.layout.deep_five"})
                ...
                formation_ref = require_text(payload,"formation_ref",max_length=160) if "formation_ref" in payload else ""
                operational_command_ref = require_text(payload,"operational_command_ref",max_length=220) if "operational_command_ref" in payload else ""
                if bool(formation_ref) == bool(operational_command_ref): raise ValueError("battlefield redeploy requires exactly one formation_ref or operational_command_ref")
                require_text(payload,"target_sector_ref",max_length=220)
                if "pace" in payload: require_text(payload,"pace",allowed={"forced","standard","cautious"})
                if "order" in payload: require_text(payload,"order",allowed={"hold","attack","breakthrough","delay","reserve","withdraw"})
            elif action=="set_order":
                formation_ref = require_text(payload,"formation_ref",max_length=160) if "formation_ref" in payload else ""
                operational_command_ref = require_text(payload,"operational_command_ref",max_length=220) if "operational_command_ref" in payload else ""
                if bool(formation_ref) == bool(operational_command_ref): raise ValueError("battlefield order requires exactly one formation_ref or operational_command_ref")
                require_text(payload,"order",allowed={"hold","attack","breakthrough","delay","reserve","withdraw"})
        ...
        if t == "personal_combat":
            ...
            if "allow_friendly_fire_risk" in payload and not isinstance(payload.get("allow_friendly_fire_risk"),bool):
                raise ValueError("personal combat allow_friendly_fire_risk must be boolean")
            if "improvised_prop_fact_ref" in payload:
                prop_ref=require_text(payload,"improvised_prop_fact_ref",max_length=192)
                self._personal_improvised_prop_profile(prop_ref)
    ...
    def _autonomy_state(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        for bp in blueprints:
            ...
            if existing:
                ...
                if take:
                    ...
                    # This legacy state-review reconstitution path predates the
                    # generalized replacement mixin, but it still changes the
                    # same exact formation strength.  Keep a surviving exact
                    # commander's live billet span synchronized through the
                    # canonical officer projection authority.
                    sync_materialized_officer_billets(self, formation)
        ...
        if max_threat>=35:
            ...
            existing_operation = exact_operation_record(self, op_ref)
            if existing_operation is None:
                ...
                exact_path, _existing = existing_operation
                if op_idx.setdefault("operations",{}).get(op_ref) != exact_path:
                    op_idx["operations"][op_ref] = exact_path
                    self.put("state/operations/index.json", op_idx)
        self.put(sp,state_doc)
    ...
    def _autonomy_interstate(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        for _operation_ref, _operation_path, _operation in iter_exact_operation_records(self):
            if str(_operation.get("status", "")) in terminal_operation_statuses:
                continue
    ...
    def _battle(
        ...
        *,
        ...
        if context is None or internal_operational_contact:
            ...
            if resolved_operation is None: raise ValueError("unknown battle operation")
            _op_path,operation=resolved_operation
        ...
        operational_battlefield: Mapping[str, Any] | None = None
        contact_commitment_milli={ref:1000 for ref in all_refs}
        if operational_contact:
            ...
            assignments=operational_battlefield.get("assignments") if isinstance(operational_battlefield.get("assignments"),Mapping) else {}
            for ref in all_refs:
                assignment=assignments.get(ref)
                if not isinstance(assignment,Mapping):
                    raise ValueError("operational battle contact lacks an exact battlefield assignment")
                share=self._battlefield_assignment_sector_commitment_milli(assignment,str(operational_sector_ref))
                if share<=0:
                    raise ValueError("operational battle contact includes a formation with no local frontage commitment")
                contact_commitment_milli[ref]=share

        def committed_personnel(ref:str)->int:
            full=max(0,int(formations[ref][1].get("personnel",0) or 0))
            share=max(0,min(1000,int(contact_commitment_milli.get(ref,1000))))
            if full<=0 or share<=0:
                return 0
            return max(1,int(round(full*share/1000.0)))

        represented=sum(committed_personnel(r) for r in all_refs)
        base_battle_hours=max(1,min(12,2+int(math.log10(max(10,represented)))))
        contact_plan: Dict[str, Any]={}
        if operational_contact:
            if not isinstance(operational_battlefield,Mapping):
                raise ValueError("operational battle contact lacks battlefield authority")
            if internal_operational_contact:
        ...
        for phase_index,spec in enumerate(phase_specs):
            ...
            for ref in all_refs:
                ...
                commitment=max(0,min(1000,int(contact_commitment_milli.get(ref,1000))))
                if commitment<1000:
                    # A coarse formation owner may cover several operational
                    # sectors, but this exact contact resolves only its conserved
                    # local share. Scale ammunition demand/use before anything is
                    # debited so one sector cannot fire the whole army's quivers.
                    for field in ("desired_by_resource","consumed_by_resource"):
                        raw=phase_ammo[ref].get(field,{})
                        if isinstance(raw,Mapping):
                            phase_ammo[ref][field]={
                                str(resource):max(0,int(round(max(0,int(amount))*commitment/1000.0)))
                                for resource,amount in raw.items()
                            }
                    phase_ammo[ref]["ranged_personnel"]=max(0,int(round(int(phase_ammo[ref].get("ranged_personnel",0) or 0)*commitment/1000.0)))
                    desired_local=sum(max(0,int(v)) for v in phase_ammo[ref].get("desired_by_resource",{}).values())
                    consumed_local=sum(max(0,int(v)) for v in phase_ammo[ref].get("consumed_by_resource",{}).values())
                    phase_ammo[ref]["overall_sufficiency"]=(consumed_local/desired_local if desired_local>0 else 1.0)
            ...
            for ref in all_refs:
                ...
                local_commitment=max(0.0,min(1.0,float(contact_commitment_milli.get(ref,1000))/1000.0))
                phase_contact_base=max(0.0,.10*phase_hours*contact_wear_factor*local_commitment)
                ...
                mount_phase_loss=max(0,min(current_mounts,int(round(int(mount_phase.get("units_lost",0) or 0)*local_commitment))))
                ...
                cohesion_loss=max(0,int(round((.55+1.8*pressure)*cohesion_wear_factor*phase_fraction*3.0*contact_duration_factor*local_commitment)))
                ...
                for sector,integrity in prior_sector_state.items():
                    ...
                    loss_pct=(3.0+6.0*contact_ratio+min(8.0,hero_frontage*.04))*phase_fraction*contact_wear_factor*sector_bias*contact_duration_factor*local_commitment
        ...
        attacker_personnel_before=max(1,sum(committed_personnel(ref) for ref in attackers))
        defender_personnel_before=max(1,sum(committed_personnel(ref) for ref in defenders))
        ...
        for refs,rate in ((attackers,a_rate),(defenders,d_rate)):
            for ref in refs:
                path,formation=formations[ref]; before=int(formation["personnel"]); engaged_before=committed_personnel(ref); before_comp={str(role):max(0,int(count)) for role,count in formation.get("composition",{}).items()}; adjusted_rate=rate*casualty_modifiers.get(ref,1); loss=min(engaged_before,max(0,int(round(engaged_before*adjusted_rate)))); survivor_comp,dead_comp=self._partition_counts(before_comp,loss,before)
                if loss > 0:
                ...
                hero_officer_pressure_alloc=source_officer_pressure*engaged_before/max(1,source_total)
                hero_cohesion_shock_alloc=source_cohesion_shock*engaged_before/max(1,source_total)
                hero_artillery_pressure_alloc=source_artillery_pressure*engaged_before/max(1,source_total)
        ...
        def _battle_side_accounting(refs):
            remaining = sum(max(0, int(formations[ref][1].get("personnel", 0) or 0)) for ref in refs)
            losses = sum(max(0, int(killed.get(ref, 0) or 0)) for ref in refs)
            return {
                "field_body_count": len(refs),
                "personnel_committed": remaining + losses,
                "killed_this_contact": losses,
                "personnel_remaining": remaining,
            }
        combat_information = {
            "scale": "formation_battle",
            "attacker": _battle_side_accounting(attackers),
            "defender": _battle_side_accounting(defenders),
            "winner_scope": winner_scope,
            "disclosure_rule": "This full side accounting is GM-private resolver truth. Live player narration may give Tang Wei exact own figures and only enemy figures established by sight, delivered battlefield reports, prisoners, or settled after-action reporting. Do not convert aggregate casualties into personal kills.",
            "personal_takedown_rule": "Tang Wei receives a personal confirmed-defeat count only from exact personal combat or an exact named hero-intervention consequence that attributes a local opponent to him. Formation casualties are never personal takedowns.",
        }
        result={"battle_event":event_id,"battlefield_ref":battlefield,"operational_battlefield_ref":operational_battlefield_ref,"sector_ref":operational_sector_ref,"contact_proof":contact_proof,"terrain_kind":terrain_primary,"terrain_tags":terrain_tags,"terrain_profile":terrain_kind,"represented_personnel":represented,"casualties":killed,"material_losses":material_losses,"winner":"attacker" if attacker_won else "defender","winner_scope":winner_scope,"operational_contact":operational_contact,"contact_plan":_deepcopy(contact_plan),"contact_status":_deepcopy(contact_status),"combat_information":combat_information,"score_breakdown":score_details,"named_person_outcomes":named_person_outcomes,"causal_trace":causal_trace,"narration_contract":narration_contract,"ammunition_plans":{ref:{"desired_by_resource":ammo_plans[ref].get("desired_by_resource",{}),"consumed_by_resource":ammo_plans[ref].get("consumed_by_resource",{}),"overall_sufficiency_basis_points":int(round(float(ammo_plans[ref].get("overall_sufficiency",1))*10000))} for ref in all_refs},"battlefield_sustainment":battlefield_sustainment_summary,"duration_hours":battle_hours,"contact_phases":battle_phase_trace,"hero_object_consequences":hero_object_consequences,"immediate_army_staff_review":immediate_army_staff_review,"world_time":str(battle_completed)}; result.update(time_metrics); return result

    def _dispatch(self, command: CommandEnvelope, payload: Mapping[str, Any]) -> Dict[str, Any]:
        t=command.command_type
        if t not in COMMAND_TYPES:
            raise ValueError("unsupported Sword semantic command: %s" % t)
        if t == "gm_cognition_action":
            from sword_runtime.gm_cognition import apply_proposal
            result = apply_proposal(self, command)
        ...
        if t=="scene_consequence":
            ...
            from sword_runtime.api.interaction_surface import apply_scene_action_record, record_interaction_attempt
            ...
            scene_record = apply_scene_action_record(self, summary, at=command.submitted_at)
            if scene_record is not None:
                self._write_meta(command)
                return self._result(**scene_record)
        ...
        if t=="travel":
            ...
            current_scene_time = str(self.read("state/runtime.json").get("world_time", command.submitted_at))
            close_active_scene(self, at=current_scene_time, reason="player_left")
            ...
            duration=max(1,int(math.ceil(base_duration/max(0.05,body_factor)))) if base_duration>0 else 0; current=self._world_time(); settle_person_idle_fatigue(player,current=current,rules=self.read(FATIGUE_RULES_PATH),state="ordinary"); target_time=current.add_seconds(duration*3600); target=str(target_time); m=self._advance_runtime(target); player["location"]=dest; player["current_location"]=dest
        ...
        if t=="personal_combat":
            from sword_runtime.scene_sessions import close_active_scene
            improvised_prop_profile = None
            if payload.get("improvised_prop_fact_ref"):
                improvised_prop_profile = self._personal_improvised_prop_profile(str(payload.get("improvised_prop_fact_ref")))
            current_scene_time = str(self.read("state/runtime.json").get("world_time", command.submitted_at))
            close_active_scene(self, at=current_scene_time, reason="hard_interruption")
            player=_deepcopy(self.read("state/player.json"))
            ...
            resolution=self._personal_combat_slice(
                ...
                player_improvised_prop=improvised_prop_profile,
            )
        ...
        if t in {"operation_create","operation_transition"}:
            ...
            if t=="operation_create":
                ...
                if exact_operation_record(self,ref) is not None: raise ValueError("operation_ref already exists")
            ...
            ref=str(payload["operation_ref"]); resolved_operation=exact_operation_record(self,ref)
            if resolved_operation is None: raise ValueError("unknown operation")
            path,operation0=resolved_operation
            if idx.setdefault("operations",{}).get(ref) != path:
                idx["operations"][ref]=path
                self.put(idxp,idx)
            doc=_deepcopy(operation0); status=str(payload["status"]); old=str(doc.get("status","planned")); legal={"planned":{"mobilizing","cancelled"},"mobilizing":{"active","cancelled"},"active":{"engaged","occupied","completed","cancelled"},"engaged":{"active","occupied","completed","cancelled"},"occupied":{"completed","cancelled"},"completed":set(),"cancelled":set()}
        ...
        if t=="reputation_event":
            subject_ref=str(payload["subject_ref"]); audience_ref=str(payload["audience_ref"]); delta=int(payload["delta"]); event_type=str(payload.get("event_type","material_conduct")); now=str(self._world_time()); idxp="state/reputation/index.json"; idx=_deepcopy(self.read(idxp)); resolved_subject=resolve_reputation_subject(self.read_optional,idx,subject_ref)
            if resolved_subject is None: raise ValueError("reputation subject is not registered")
            subject_path,subject0=resolved_subject; subject=_deepcopy(subject0); idx.setdefault("subjects",{})[subject_ref]=subject_path; resolved_profile=resolve_reputation_profile(self.read_optional,subject,subject_ref,audience_ref); existing_profile=resolved_profile is not None
            if resolved_profile is None:
                profile_path=reputation_profile_path(subject_ref,audience_ref); occupied=self.read_optional(profile_path)
                if occupied is not None: raise ValueError("reputation profile canonical path is occupied by another owner")
                profile={"schema":"reputation-audience-profile","subject_id":subject_ref,"audience_id":audience_ref,"as_of":now,"authority":True,"standing":{"overall":0},"dimensions":{},"evidence_count":0,"last_event_refs":[],"memory_class":"normal"}
            else:
                profile_path,profile0=resolved_profile; profile=_deepcopy(profile0)
            profile.setdefault("standing",{})["overall"]=_clamp(int(profile.get("standing",{}).get("overall",0))+delta,-100,100); dimension=str(payload.get("dimension","general")); profile.setdefault("dimensions",{})[dimension]=_clamp(int(profile.get("dimensions",{}).get(dimension,0))+delta,-100,100); eid="reputation."+hashlib.sha256((now+":"+subject_ref+":"+audience_ref+":"+str(command.expected_revision)).encode()).hexdigest()[:16]; event_path=f"state/reputation/events/{eid}.json"; event={"schema":"reputation-event","event_id":eid,"subject_id":subject_ref,"event_type":event_type,"occurred_at":now,"source_event_ref":payload.get("source_event_ref"),"authority":True,"signals":{dimension:delta},"standing_signals":{"overall":delta},"visibility":{"audience_ref":audience_ref,"basis":str(payload.get("basis","verified material evidence"))},"witnesses":[str(x) for x in payload.get("witnesses",[])],"report_routes":[],"deliveries":{},"status":"settled"}; self.put(event_path,event); profile["as_of"]=now; profile["evidence_count"]=int(profile.get("evidence_count",0))+1; profile.setdefault("last_event_refs",[]).append(eid); profile["last_event_refs"]=profile["last_event_refs"][-16:]; self.put(profile_path,profile); subject.setdefault("audience_profiles",{})[audience_ref]=profile_path; subject["as_of"]=now; self.put(subject_path,subject); idx["event_count"]=int(idx.get("event_count",0))+1; idx["subject_count"]=len(idx.get("subjects",{})); idx["audience_profile_count"]=int(idx.get("audience_profile_count",0))+(0 if existing_profile else 1); self.put(idxp,idx); world_time,metrics=self._advance_seconds(300); self._write_meta(command,world_time); return self._result(event_ref=eid,subject_ref=subject_ref,audience_ref=audience_ref,standing=profile["standing"]["overall"],world_time=world_time,**metrics)
        if t=="career_event":
        ...
        if t=="fortification_materialize":
            ...
            if exact_fortification_record(self, ref) is not None: raise ValueError("fortification_ref already exists")
        ...
        if t=="territorial_consequence":
            ...
            if payload.get("siege_ref"):
                ...
                evidence_ref=str(payload["operation_ref"]); resolved_operation=exact_operation_record(self,evidence_ref)
                if resolved_operation is None: raise ValueError("unknown occupation operation")
                _op_path,op=resolved_operation
        ...
        if t=="family_event":
            ...
            def active_union(a: str, b: Optional[str]=None) -> tuple[Optional[str],Optional[str],Optional[Dict[str,Any]]]:
                for uid in idx.get("unions",{}):
                    resolved=exact_family_record(self.read_optional,idx,"unions",str(uid))
                    if resolved is None: continue
                    path,u=resolved; participants={str(x) for x in u.get("participants",[]) if isinstance(x,str)}
            ...
            def close_kin(a: str, b: str) -> bool:
                if a==b: return True
                for kinship_ref in idx.get("kinships",{}):
                    resolved=exact_family_record(self.read_optional,idx,"kinships",str(kinship_ref))
                    if resolved is None: continue
                    _kp,kin=resolved; participants={str(x) for x in kin.get("participants",[]) if isinstance(x,str)}
                    if {a,b}.issubset(participants) and str(kin.get("status","active"))=="active": return True
                parents={}
                for parentage_ref in idx.get("parentage",{}):
                    resolved=exact_family_record(self.read_optional,idx,"parentage",str(parentage_ref))
                    if resolved is None: continue
                    _pp,par=resolved; child=str(par.get("child_id","")); parents[child]={str(x.get("parent_id")) for x in par.get("parent_links",[]) if isinstance(x,Mapping) and x.get("parent_id")}
            ...
            if kind=="proposal":
                ...
                pid=str(payload.get("proposal_ref","")); resolved_proposal=exact_family_record(self.read_optional,idx,"proposals",pid)
                if resolved_proposal is None: raise ValueError("engagement requires an exact saved proposal")
                path,proposal0=resolved_proposal; proposal=_deepcopy(proposal0);
                ...
                for uid in list(idx.get("unions",{})):
                    resolved_union=exact_family_record(self.read_optional,idx,"unions",str(uid))
                    if resolved_union is None: continue
                    up,union0=resolved_union; union=_deepcopy(union0);
                ...
                sid=str(payload.get("succession_ref","succession.house_tang")); resolved_succession=exact_family_record(self.read_optional,idx,"successions",sid)
                if resolved_succession is None: raise ValueError("unknown succession record")
                sp,succession0=resolved_succession; succession=_deepcopy(succession0); holder=str(succession.get("current_holder_id","")); holder_dead=False
                if holder:
...
from sword_runtime.campaign_access import isolated_runtime
...
class SwordRuntime:
    ...
    @isolated_runtime
    def execute(self, command: CommandEnvelope, crash_injector=None, *, receipt_command=None) -> TransactionExecution:
        identity = command if receipt_command is None else receipt_command
        if receipt_command is not None and any(
            getattr(identity, key) != getattr(command, key)
            for key in ("campaign_id", "request_id", "actor_id", "expected_revision", "submitted_at", "mode")
        ):
            raise ValueError("translated command changed request identity")
        # Durable duplicate lookup comes before planning because a retry is intentionally stale.
        existing=self.coordinator.lookup_receipt(identity)
        if existing is not None:
        ...
        execution=self.coordinator.execute(identity,plan.transaction_id,plan.created_at,plan.writes,plan.result,plan.validator,crash_injector=crash_injector)

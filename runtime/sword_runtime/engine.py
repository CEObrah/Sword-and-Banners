...
from sword_runtime.anatomy import apply_structural_injury_state, resolve_structural_injury
...
from sword_runtime.siege_physics import (
    apply_ram_damage, assault_access, build_hours, engineering_blueprints,
    initial_physical_state, ram_access, sync_integrity_projection,
    work_materials, work_record,
)
...
COMMAND_TYPES = frozenset({
    ...
    "reputation_event","career_event","mercenary_contract","organization_action","project_resolve","project_cancel","strategic_crossing_action","army_train_action","settlement_civic_action","fortification_logistics","custody_action","scheme_action","merchant_convoy_action"
})
...
class RepositoryCommandPlanner(OperationalBattlefieldMixin, CohortTxSupportMixin, PersonalCombatMixin, CombatCapabilityMixin, StandingForceCapabilityMixin):
    ...
    def read(self, path: str) -> Any:
        ...
        if base_path in self._writes:
            ...
            try:
                return select_json_fragment(self._writes[base_path], tokens)
    ...
    def _validate_command_semantics(self, command: CommandEnvelope, payload: Mapping[str, Any]) -> None:
        ...
        if t == "army_train_action":
            ...
            require_text(payload, "action", allowed={"damage_carts","destroy_carts","repair_carts","delay_baggage","clear_delay","set_relief_corridor","break_camp"})
            if str(payload.get("action")) in {"damage_carts","destroy_carts","repair_carts"}:
        ...
        if t == "house_action":
            ...
            action=require_text(payload,"action",allowed={"assign_duty","set_policy","proclaim_territorial_authority","accept_bastion_applicants"},default="assign_duty")
            if action=="assign_duty":
                ...
                require_text(payload,"corps_key",allowed={"iron_rampart","red_crane","white_lantern","deep_earth"})
    ...
    def _battle(
        ...
        *,
        ...
        for ref in all_refs:
            ...
            caps=commander.get("capabilities",commander.get("skills",{})) if isinstance(commander,Mapping) else {}; command_score=sum(_fixed(caps.get(k,0)) for k in ("Formation Command","Tactics","Leadership","Strategy","Mass Combat")) if isinstance(caps,dict) else 0.0; commander_scores[ref]=command_score
    ...
    def _dispatch(self, command: CommandEnvelope, payload: Mapping[str, Any]) -> Dict[str, Any]:
        ...
        if t in {"health_injury","health_recovery"}:
            ...
            if isinstance(injury,dict) and injury.get("active"):
                ...
                if int(injury["recovered_hours"])>=int(injury.get("minimum_recovery_hours",0)):
                    injury["active"]=False; injury["resolved_at"]=target
                    bleeding=injury.get("bleeding") if isinstance(injury.get("bleeding"),dict) else None
                    if bleeding is not None:
                        bleeding["controlled"]=True; bleeding["rate_units_per_minute"]=0.0; bleeding["internal_rate_units_per_minute"]=0.0
                    injury["bleeding_units_per_minute"]=0.0; injury["internal_bleeding_units_per_minute"]=0.0
                    if injury.get("permanent_anatomy"):
                        injury["healed_with_permanent_sequelae"] = True
                        injury["recovery_rule"] = "soft-tissue recovery does not regenerate absent/destroyed anatomy"
                    sync_injury_record(player,injury)
                    if active_injury_rows(player): self._set_person_health(player,"injured")
                    ...
                    else: self._set_person_health(player,"healthy")
                else:
                    sync_injury_record(player,injury)
                    self._set_person_health(player,"injured")
        ...
        if t=="person_materialize":
            ...
            if representation=="person_lite":
                put_person_lite(self, person=person, scope_ref=(formation_ref or force_ref), scope_rule="Individually relevant existing force members share compact routed roster shards; aggregate personnel remain sheetless.")
            else:
        ...
        if t=="formation_create":
            ...
            muster_hours=max(1,min(48,int(math.ceil(n/500.0)))); current=self._world_time(); target=str(current.add_seconds(muster_hours*3600)); metrics=self._advance_runtime(target); self._write_meta(command,target); return self._result(formation_ref=ref,personnel=n,composition=composition,world_time=target,muster_hours=muster_hours,**metrics)
        if t in {"formation_reconstitute","formation_train","formation_mobilize","formation_demobilize","formation_doctrine_set","formation_training_set","formation_assign","force_assignment","command_assign","command_transfer","formation_move","resupply"}:
            ...
            if t=="formation_reconstitute":
                ...
                develop_officer_cadre(f, training_hours=hours, at=world_time); reorganize_officer_cadre(f, at=world_time, reason="post_training_officer_review"); sync_materialized_officer_billets(self, f)
        ...
        if t=="personal_combat":
            ...
            for participant_ref,person in participant_people.items():
                ...
                if isinstance(ammo_state,Mapping) and isinstance(manifest_path,str) and manifest_path:
                    manifest0=self.read_optional(manifest_path)
                    if isinstance(manifest0,Mapping):
        ...
        if t=="career_event":
            person_ref=str(payload["person_ref"]); pp,person0=self._exact_person(person_ref); person=_deepcopy(person0); kind=str(payload["kind"]); registry_path="state/career/merit-and-career-history.json"; registry=_deepcopy(self.read(registry_path)); career=person.setdefault("career_state",{}); career.setdefault("merit_total",0); career.setdefault("qualifications",[]); career.setdefault("appointments",[]); rank=person.setdefault("military_rank",{"grade":"unranked","durable":True,"source":"runtime_career"}); now=str(self._world_time()); record={"record_id":"career."+hashlib.sha256((now+":"+person_ref+":"+kind+":"+str(command.expected_revision)).encode()).hexdigest()[:14],"person_ref":person_ref,"kind":kind,"at":now,"authority":True}
            if kind=="merit":
                ...
                rank.update({"grade":grade,"durable":True,"source":"runtime_career","changed_at":now,"change_kind":kind,"grantor_ref":payload.get("grantor_ref"),"evidence_ref":payload.get("evidence_ref"),"rule":"Only explicit promotion/demotion changes grade; billet, casualty strength, relief, reserve status and current command span do not."}); person["rank"]=grade; record.update({"prior_grade":current,"grade":grade,"grantor_ref":payload.get("grantor_ref"),"evidence_ref":payload.get("evidence_ref")})
            ...
            registry.setdefault("records",[]).append(record); registry.setdefault("runtime",{})["last_settled_at"]=now; self.put(registry_path,registry); self.put(pp,person); world_time,metrics=self._advance_seconds(3600); self._write_meta(command,world_time); return self._result(person_ref=person_ref,kind=kind,military_rank=person.get("military_rank"),career_state=career,record_id=record["record_id"],world_time=world_time,**metrics)
        if t=="mercenary_contract":
        ...
        if t in {"siege_start","siege_action"}:
            ...
            if action=="blockade":
                ...
                engineer_count=sum(max(0,int(n)) for role,n in sf.get("composition",{}).items() if "engineer" in str(role).lower() or "engineering" in str(role).lower()); engineer_min=max(0,int(blueprint.get("engineer_min",0)))
                if engineer_count<engineer_min: raise ValueError(f"siege blueprint requires at least {engineer_min} engineering specialists in the source formation")
                ...
                hours=build_hours(blueprint,quantity,int(sf.get("personnel",0))); crew=min(int(sf.get("personnel",0)),max(1,int(blueprint.get("crew_optimal",blueprint.get("crew_min",1))))); food_need=max(1,int(math.ceil(crew*0.8*hours/24.0)))
                ...
                logistics["food_kg"]=int(logistics.get("food_kg",0))-food_need; sf["fatigue"]=_clamp(int(sf.get("fatigue",0))+max(1,min(25,hours//8))); self.put(sp,sf)
                now=str(self._world_time()); work_ref="siege_work_"+hashlib.sha256((ref+"|"+now+"|"+blueprint_ref+"|"+target+"|"+str(len(siege.get("engineering_works",[])))).encode()).hexdigest()[:18]; work=work_record(blueprint_ref,blueprint,work_ref=work_ref,target=target,quantity=quantity,at=now,source_formation_ref=source_ref,materials=materials); work["engineer_personnel_used"]=min(engineer_count,max(engineer_min,engineer_count)); work["crew_used"]=crew; work["construction_hours"]=hours; work["condition_pct"]=100.0
                ...
                candidates=[w for w in siege.get("engineering_works",[]) if isinstance(w,dict) and w.get("status")=="serviceable" and w.get("kind")=="battering_ram" and w.get("target")=="gate"]
                ...
                siege["last_assault_event"]=result["battle_event"]; siege.setdefault("assault_sectors",[]).append({"battle_ref":result["battle_event"],"at":str(result["world_time"]),"attacker_formation_refs":sector_attackers,"defender_formation_refs":sector_defenders,"access":access,"structural_damage_from_troop_battle":0}); siege["assault_sectors"]=siege["assault_sectors"][-32:]; world_time=str(result["world_time"]); metrics={k:int(result.get(k,0)) for k in ("hosts_woken","events_processed") if k in result}
                defenders_left=sum(int(self._load_formation(fr)[1].get("personnel",0)) for fr in fort.get("garrison_formation_refs",[]))
                if result.get("winner")=="attacker" and defenders_left<=0: siege["status"]="captured"; siege["outcome"]="attacker_control"; siege["captured_at"]=world_time
                access_event={"phase":"fortification_access","event":"attacking formations reach the defended contact through a physically admitted access path","target":target,"method":access.get("method"),"access_class":access.get("access_class"),"breach_width_m":access.get("breach_width_m"),"crossing_length_m":access.get("crossing_length_m")}
                ...
                if not isinstance(state,dict) or state.get("model")!="exact_perimeter": raise ValueError("physical repair requires exact registered fortification geometry")
                engineers=sum(max(0,int(n)) for role,n in sf.get("composition",{}).items() if "engineer" in str(role).lower() or "engineering" in str(role).lower())
                if engineers<=0: raise ValueError("physical structural repair requires engineering personnel in the selected garrison formation")
                if target=="gate":
                ...
                restored=min(damaged,max(0.5,hours*engineers/1000.0)); material_units=max(1,int(math.ceil(restored*100.0))); logistics=sf.setdefault("logistics",{})
                ...
                logistics["construction_material_units"]=carried-from_carried; sf["fatigue"]=_clamp(int(sf.get("fatigue",0))+max(1,hours//8)); structure["structural_condition_percent"]=min(100.0,condition+restored)
                ...
                fort["physical_state"]=state; sync_integrity_projection(fort); self.put(sp,sf); world_time,metrics=self._advance_seconds(hours*3600); action_details={"target":target,"integrity_restored_pct":round(restored,3),"breach_width_reduced_m":round(old_breach-new_breach,3),"construction_material_units_consumed":material_units,"construction_material_from_carried":from_carried,"construction_material_from_site_depot":from_depot,"engineer_personnel":engineers}

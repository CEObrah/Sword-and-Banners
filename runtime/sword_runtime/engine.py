    ...
    def _dispatch(self, command: CommandEnvelope, payload: Mapping[str, Any]) -> Dict[str, Any]:
        ...
        if t=="personal_combat":
            ...
            def combat_score(person_ref:str,person:Mapping[str,Any])->tuple[float,Dict[str,Any]]:
                ...
                eq=equipment_profile(person_ref,person); weapon=_fixed(skills.get(eq["skill_name"],0)); defense=_fixed(skills.get("Defense",0)); support=sum(_fixed(attrs.get(k,0)) for k in ("Agility","Awareness","Coordination","Composure","Endurance"))/5.0

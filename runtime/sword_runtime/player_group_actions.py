...
from sword_runtime.training_programs import REGISTRY_PATH as TRAINING_PROGRAM_REGISTRY_PATH, resolve_program_ref, settle_exact_program
    ...
    def _dispatch_group_train(
        ...
        for path, formation in loaded:
            ...
            if hasattr(self, "_ct_train_formation"):
                ...
                self._ct_train_formation(formation_ref, float(hours), f"formation_training:{command.request_id}:{formation_ref}")
        ...
        if participants:
            ...
            for person_path, person in participants:
                ...
                if not regimen_name:
                    ...
                    regimen_name = "house_tang_max_sustainable" if owner_force in {"force_house_tang", "force_sword_manor", "force_tang_wei_personal"} else "regular_army"
                regimen = regimens.get(regimen_name, {}) if isinstance(regimens, Mapping) else {}
                if not isinstance(regimen, Mapping):
                    regimen = {}
                participant_evidence = f"formation_participant_training:{command.request_id}:{person_ref}"

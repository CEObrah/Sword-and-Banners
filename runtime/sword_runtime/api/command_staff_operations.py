...
Formation owners remain the sole authority for top-commander assignment and
...
class CommandStaffAwareCampaignOperations(HouseholdAwareCampaignOperations):
    ...
    def _command_staff_for_formation(self, formation_ref: object) -> dict[str, str]:
        ...
        return {"commander_ref": value} if isinstance(value, str) and value else {}
    ...
    def _command_refs(structure: Mapping[str, Any], formation: Mapping[str, Any] | None = None) -> dict[str, str]:
        ...
        if isinstance(formation, Mapping):
            ...
            internal = formation.get("embedded_person_refs", []) if isinstance(formation.get("embedded_person_refs"), list) else []
    ...
    def _command_person_representation(self, person_ref: str) -> str | None:
        """Classify one command person from durable owner/schema authority.

        Identity spelling is not representation authority. A promoted full
        character may retain an officer.* ref, and legacy fixtures may use
        unusual IDs. The command-personnel index is only an accelerator, so
        fall back to the canonical owner route before using a narrow legacy
        char_ compatibility rule.
        """
        ...
        if not isinstance(record, Mapping):
            ...
            record = self._read_optional_mapping(path) if isinstance(path, str) else None
    ...
    def _source_cohort_for_person_lite(
        ...
        if not source_role and role_label == "persistent_unit_commander":
    ...
    def _person_lite_service_sheet(
        ...
        if isinstance(record, Mapping):
            ...
            projected["professional_skills"] = dict(record.get("professional_skills", {})) if isinstance(record.get("professional_skills"), Mapping) else {}
        ...
        if isinstance(cohort, Mapping):
            projected_loadout = officer_loadout_id(self.read, {}, formation, command_role=role_label)
            stats = project_person_lite_stats(
                cohort, person_ref, command_rank=role_label,
                loadout_id=projected_loadout,
            )
            projected["personal_loadout_ref"] = projected_loadout
            projected["attributes"] = stats["attributes"]
            projected["skills"] = stats["skills"]
            projected["professional_skills"] = stats.get("professional_skills", {})
        ...
        if isinstance(cohorts, Mapping):
            for cohort_id, cohort in sorted(cohorts.items()):
                ...
                projected.append({
                    ...
                    "professional_skill_means": cohort.get("professional_skill_means", {}),
    ...
    def play_context(self):
        ...
        for row in formations:
            ...
            } if isinstance(row.get("commander_ref"), str) and row.get("commander_ref") else {}
    ...
    def _exact_controlled_command_role(self, person_id: str) -> tuple[str, str] | None:
        ...
        if not isinstance(record, Mapping):
            ...
            if isinstance(path, str):
                candidate = self._read_optional_mapping(path)
                if (
                    isinstance(candidate, Mapping)
                    and (
                        path == "state/player.json"
                        or path.startswith("state/char/")
                        or (
                            path.startswith("state/person/")
                            and str(candidate.get("schema", ""))
                            in {"sab_character", "sword-materialized-person", "person-lite"}
                        )
                    )
                ):
                    record = candidate
    ...
    def person_sheet(self, person_id: str) -> dict[str, Any]:
        ...
        if not isinstance(record, Mapping):
            ...
            path = owners.get(person_id) if isinstance(owners, Mapping) else None
            record = self._read_optional_mapping(path) if isinstance(path, str) else None
        representation_schema = str(record.get("schema", "")) if isinstance(record, Mapping) else ""
        if representation_schema == "person-lite":
    ...
    def inspect_game_object(self, object_ref: str) -> dict[str, Any]:
        ...
        if object_ref.startswith("formation_") and isinstance(obj, Mapping):
            ...
            person_lite = [
                ...
                if self._command_person_representation(ref) == "person_lite"
            ]
            full_command = [
                {"person_ref": ref, "role": role, "representation": "full_character"}
                for ref, role in sorted(refs.items())
                if self._command_person_representation(ref) == "full_character"
            ]
            enriched["person_lite_officers"] = person_lite
            enriched["person_lite_officer_count"] = len(person_lite)
            enriched["full_command_officers"] = full_command
            enriched["full_command_officer_count"] = len(full_command)

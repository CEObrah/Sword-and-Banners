...
def house_readiness_snapshot(operations: Any)->dict[str,Any]:
    ...
    treasury=store.read_json(_TREASURY_PATH); depot=store.read_json(_DEPOT_PATH); mount_pool=store.read_json(_MOUNT_POOL_PATH); inventory=store.read_json(_INVENTORY_PATH); outfitting=store.read_json(_OUTFITTING_RULES_PATH)
    ...
    return {
      ...
      "strategic_stores":{"depot_ref":depot.get("depot_ref",depot.get("owner_id")),"stocks":_mapping(depot.get("stocks")),"storage_capacity":_mapping(depot.get("storage_capacity")),"garrison_support_targets":_mapping(depot.get("garrison_support_targets")),"current_shortfalls":_mapping(depot.get("current_shortfalls")),"mounts":_mapping((mount_pool.get("regional_reserve", {}) or {}).get("loc_tang_manor_garrison_yard", {}))},
      "armory_and_remount_reserves":{"aggregate_outfitting_sets":_outfitting_facts(inventory),"outfitting_rule":outfitting.get("principle"),"ammunition_owner_ref":"depot_house_tang","mount_owner_ref":"mount_pool_force_house_tang"},
      "estate_support":{k:estate_support.get(k) for k in ("food_monthly_output_kg","supported_fighting_personnel","supported_resident_military","resident_house_mounts","house_mounts","strategic_autarky") if estate_support.get(k) is not None},
      ...
      "source_owners":[_TREASURY_PATH,_DEPOT_PATH,_MOUNT_POOL_PATH,_INVENTORY_PATH,_HOUSE_PATH,_OUTFITTING_RULES_PATH]
    }

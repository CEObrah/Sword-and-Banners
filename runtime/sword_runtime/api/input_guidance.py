...
COMMAND_INPUT_GUIDANCE: Mapping[str, Mapping[str, Any]] = {
    ...
    "merchant_convoy_action": {
        "action": {"allowed_values": ["assign_escort", "remove_escort", "interdict"], "rule": "player surface exposes only direct military interactions; physical delay and reroute remain autonomous/internal"},
        "convoy_ref": {"rule": "use an exact currently player-visible materialized convoy ref"},
        "formation_ref": {"rule": "use an exact controlled formation; escort assignment requires mobilization and co-location, interdiction requires the convoy route-node time window"},
        "disposition": {"allowed_values": ["seize", "destroy"], "default": "seize", "applies_to": ["interdict"]},
    },
    "economy_transfer": {
    ...
    "personal_combat": {
        ...
        "opponent_ref": {"rule": "legacy single-opponent form; use an exact permitted person ref"},
        ...
        "local_obstacles": {"type": "array", "maximum_items": 24, "rule": "optional player-visible/runtime-supplied local geometry only: circles use x_m/y_m/radius_m and segments use x1_m/y1_m/x2_m/y2_m/clearance_m; this constrains movement and contact paths but never creates world objects"},
    ...
    "house_action": {
        "action": {"allowed_values": ["assign_duty", "set_policy", "proclaim_territorial_authority", "accept_bastion_applicants"], "default": "assign_duty"},
        ...
        "corps_key": {"allowed_values": ["iron_rampart", "red_crane", "white_lantern", "deep_earth"], "applies_to": ["accept_bastion_applicants"]},
    ...
    "command_group_action": {
        "action": {"allowed_values": ["create", "add_person", "remove_person", "attach_formation", "detach_formation", "attach_command_group", "detach_command_group", "move_unit", "move_army", "promote_formation_to_army", "set_deputy", "set_successors", "set_order", "set_communication"]},
        ...
        "rule": "command groups are recursive command structure only and never create manpower; attaching a field army to a larger army preserves every subordinate unit and person owner",
    },
    "command_group_train": {
        "hours": {"type": "integer", "minimum": 1, "maximum": 12},
    ...
    "army_train_action": {
        "army_train_ref": {"rule": "use an exact player-visible train ref attached to a controlled army or independently moving controlled formation"},
        "action": {"allowed_values": ["damage_carts", "destroy_carts", "repair_carts", "delay_baggage", "clear_delay", "set_relief_corridor", "break_camp"]},
        "quantity": {"type": "integer", "minimum": 1, "maximum": 100000, "applies_to": ["damage_carts", "destroy_carts", "repair_carts"]},
        "hours": {"type": "integer", "minimum": 1, "maximum": 8760, "applies_to": ["delay_baggage"]},
        "route_refs": {"type": "array", "minimum_items": 1, "maximum_items": 128, "rule": "exact connected strategic route refs from the current camp", "applies_to": ["set_relief_corridor"]},
        "rule": "train actions mutate already-owned carts/camp state; they never create supply, wagons, drivers, guards, or personnel",
    },
    ...
    "scheme_action": {
        "action": {"allowed_values": ["start", "work", "fund", "cleanup", "cancel"]},
        "objective": {"allowed_values": ["bribery", "blackmail", "forgery", "defection", "sabotage", "gate_opening", "assassination", "kidnapping", "theft", "misinformation", "discrediting", "prisoner_escape", "revolt"], "applies_to": ["start"]},
        "agent_refs": {"type": "array", "minimum_items": 1, "maximum_items": 8, "rule": "exact existing agents under lawful interaction authority", "applies_to": ["start"]},
        "budget_silver": {"type": "integer", "minimum": 0, "applies_to": ["start"]},
        "amount_silver": {"type": "integer", "minimum": 1, "applies_to": ["fund"]},
        "rule": "scheme progress/exposure is runtime-owned; completion establishes only the causal opportunity/effect supported by access, agents, tools, money and evidence, never an automatic kill/kidnap/world-truth rewrite",
    },

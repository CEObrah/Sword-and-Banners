...
COMMAND_INPUT_GUIDANCE: Mapping[str, Mapping[str, Any]] = {
    ...
    "personal_combat": {
        ...
        "allow_friendly_fire_risk": {"type": "boolean", "default": False, "rule": "set true only when the player explicitly accepts releasing a projectile through a currently unsafe friendly-occupied lane; choosing a bow/arrow/shot or target alone is not consent to friendly-fire risk"},
    ...
    "battlefield_control": {
        ...
        "layout_ref": {"allowed_values": ["battlefield.layout.dynamic", "battlefield.layout.line_three", "battlefield.layout.deep_five"], "default": "battlefield.layout.dynamic", "applies_to": ["open"], "rule": "dynamic derives variable operational geometry from exact participating force scale and registered terrain; fixed layouts remain compatibility/testing tools"},
        ...
        "operational_command_ref": {"rule": "peer battlefield command such as a wing or primary command; for redeploy/set_order this atomically carries one player order to every conserved tactical formation currently belonging to that command, while each leaf keeps its own physical movement/casualty state", "applies_to": ["redeploy", "set_order"]},
        ...
        "rule": "operational battlefield state owns variable physical sector geometry, peer operational-command grouping, command pressure, redeployment clocks, contacts and messenger delay only; tactical 1,000/500/100-person bodies remain subordinate mechanics and battle_resolve remains casualty/outcome authority"
    },
    "state_action": {
    ...
    "medical_treatment": {
        ...
        "medical_supply_ref": {"rule": "optional exact co-located support/depot owner carrying medicine_lots; when omitted the runtime may use the exact live fortified-site medical depot at the treatment location"},
        "rule": "requires exact co-location, procedure-specific Medicine qualification, and conserved medical stock for stabilize/treat/surgery; facility-backed care requires an exact co-located facility owner/location; treatment consequences are runtime-owned and surgery preview is hidden until execute",
    },
    "commitment_action": {

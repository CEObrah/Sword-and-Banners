...
def _gm_private_goal_cognition(person: Mapping[str, Any], reader=None, person_ref=None, *, full=False) -> dict[str, Any]:
    ...
    """
    out: dict[str, Any] = {}
    if reader is not None and isinstance(person_ref, str):
        from sword_runtime.gm_cognition import private_cognition
        out.update(private_cognition(reader, person_ref, full=full))
...
class WarfareCampaignOperations(EquipmentAwareCampaignOperations):
    ...
    def _gm_private_behavior_profile(self, person_id: str) -> dict[str, Any]:
        ...
        # Some authored profiles are deliberately evidence-limited rather than
        # personality-rich. They still contain useful GM-only role/goal anchors
        # and explicit anti-invention constraints. Preserve those instead of
        # silently turning the profile into an empty packet.
        anchors = profile.get("known_anchors") if isinstance(profile.get("known_anchors"), Mapping) else {}
        constraints = profile.get("dialogue_constraints") if isinstance(profile.get("dialogue_constraints"), list) else []
        basis = profile.get("decision_basis") if isinstance(profile.get("decision_basis"), list) else []
        if anchors:
            out["known_anchors"] = dict(anchors)
        if constraints:
            out["dialogue_constraints"] = [str(value)[:300] for value in constraints if isinstance(value, str)][:8]
        if basis:
            out["decision_basis"] = [str(value)[:240] for value in basis if isinstance(value, str)][:8]
        if anchors or constraints or basis:
            out["mode"] = str(profile.get("mode") or "evidence_limited_role_driven")
            return out
        return {}
    ...
    def _with_scene_response_envelope(self, result: dict[str, Any], person_id: str, context: Mapping[str, Any]) -> dict[str, Any]:
        ...
        private_cognition = _gm_private_goal_cognition(person, self.store, person_id, full=True)
        ...
        if isinstance(edges, list):
            for edge in edges:
                ...
                relation_rows.append({
                    key: edge.get(key) for key in ("edge_ref", "source_ref", "target_ref", "kind", "value", "evidence_refs", "last_basis_ref", "last_changed_at", "history", "current_tension", "dimensions")
    ...
    def _with_gm_private_scene_director_context(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        for ref in prioritized_refs:
            ...
            cognition = _gm_private_goal_cognition(person, self.store, ref)
        ...
        director["present_people_context"] = {
            ...
            "performance_cues_rule": (
                "Use exact role, rank, lawful relationship/history evidence, current cognition/condition, open human threads, practical command or household responsibility, audience and any registered behavior profile as qualitative directing evidence. "
                "A missing behavior profile is not a reason to make an established person mute or generic. Infer only reversible moment-to-moment performance; never invent a hidden durable personality fact or quote numeric private state as characterization."
            ),
            "relationship_edges": edges_out,
            "mechanical_consequence_authority": False,
            "director_rule": (
                "Use this backstage truth to actively direct the people already in the scene, including before a formal conversation session exists. Present NPCs do not wait for Tang Wei to activate them: on continue or after Wei acts, the LLM may select whoever has the strongest grounded reason to speak, react, interrupt, joke, disagree, lie, omit, coordinate, leave, or address another NPC. "
                "Repetition, generic nods/pauses, or rephrased summaries are not scene progress; if a meaningful reversible beat exists, perform it, and if none exists, compress or transition. "

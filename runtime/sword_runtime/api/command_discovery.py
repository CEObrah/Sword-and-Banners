...
_DOMAIN_RULES: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("npc_cognition", ("gm_cognition_action",)),
...
def _compact_upward_reports(value: Any) -> tuple[list[dict[str, Any]], int]:
    if not isinstance(value, list):
        return [], 0
    # This is a second bounded projection layer.  It must preserve the same
    # unresolved priority invariant as the campaign owner rather than taking a
    # chronological tail that can hide an older response-bearing request.
    sources = select_pending_upward_reports(value, maximum=4)
    rows: list[dict[str, Any]] = []
    for source in sources:
        row = _pick(
            source,
            (
                "report_ref", "event_ref", "phase", "prepared_at", "reported_at", "from_ref", "to_ref",
                "order_ref", "directive_ref", "personnel", "information_refs",
                "follow_on_request_refs", "delivery_status", "delivered_at", "delivery_due_at",
                "source_location_ref", "target_location_ref", "communication_travel_seconds",
            ),
        )
        rows.append(row)
...
def _compact_campaign_command(value: Mapping[str, Any], operational_order: Any) -> tuple[dict[str, Any], bool]:
    ...
    if isinstance(upward_reports, list) and upward_reports:
        compact_reports, report_count = _compact_upward_reports(upward_reports)
        out["upward_report_count"] = report_count
        projected_pending = pending_upward_report_count(upward_reports)
        exact_pending_count = value.get("pending_upward_report_count")
        if isinstance(exact_pending_count, int) and not isinstance(exact_pending_count, bool) and exact_pending_count >= projected_pending:
            projected_pending = exact_pending_count
        if compact_reports:
            out["pending_upward_reports"] = compact_reports
            out["pending_upward_report_count"] = projected_pending
        elif projected_pending:
            # The exact owner says unresolved work exists outside this bounded
            # window.  Preserve the count but never manufacture a fake row.
            out["pending_upward_report_count"] = projected_pending
        else:
            delivered_tail = select_recent_delivered_upward_reports(upward_reports, maximum=2)
            if delivered_tail:
                out["recent_delivered_upward_reports"] = [
                    _pick(row, (
                        "report_ref", "event_ref", "phase", "prepared_at", "reported_at",
                        "delivery_status", "delivered_at", "from_ref", "to_ref",
                    )) for row in delivered_tail
                ]
        out["upward_report_detail_rule"] = "demand_load_exact_operation_for_report_history"

    if out.get("pending_upward_reports"):
        pending_rows = out["pending_upward_reports"]
        response_requested = any(
            isinstance(row, Mapping) and bool(row.get("follow_on_request_refs"))
        ...
        route_unresolved = any(
            isinstance(row, Mapping) and str(row.get("delivery_status", "")) == "route_unresolved"
            for row in pending_rows
        )
        if in_flight:
            handoff_rule = (
                "A persisted outbound campaign courier handoff is already active. Do not claim remote command persistence is unavailable "
                "and do not resend the same request merely to create progress. An outbound report proves dispatch, not superior receipt or reply. "
                "Carry the standing objective through lawful local preparation or chronology until delivery or another real interruption."
            )
        else:
            handoff_rule = (
                "A persisted outbound campaign report/request exists, but no physical courier route is currently confirmed. Do not claim remote "
                "command persistence is unavailable and do not describe the report as in flight or wait as though delivery were scheduled. "
                "Treat this as unresolved physical routing, preserve the standing request, and continue only lawful local work until routing is established."
            )
        out["remote_command_handoff"] = {
            "outbound_report_in_flight": in_flight,
...
def _compact_controlled_force_echelons(value: Any) -> list[dict[str, Any]]:
    if not isinstance(value, list):
        return []
    rows: list[dict[str, Any]] = []
    for source in value:
        if not isinstance(source, Mapping):
            continue
        row = _pick(source, (
            "field_army_ref", "display_name", "commander_ref", "commander_location_ref",
            "location_ref", "headquarters_location_ref", "formation_location_refs",
            "commander_physically_with_command", "total_strength", "primary_command_count",
            "tactical_leaf_formation_count", "comparison_rule",
        ))
        primary = source.get("primary_commands")
        if isinstance(primary, list):
            row["primary_commands"] = [
                _pick(item, (
                    "command_ref", "display_name", "commander_ref", "commander_location_ref",
                    "headquarters_location_ref", "formation_location_refs", "commander_physically_with_command",
                    "strength", "tactical_leaf_count", "status", "echelon_role",
                ))
                for item in primary if isinstance(item, Mapping)
            ][:8]
        rows.append(row)
    return rows
...
def compact_play_context(context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(context.get("controlled_force_echelons"), list):
        result["controlled_force_echelons"] = _compact_controlled_force_echelons(context["controlled_force_echelons"])

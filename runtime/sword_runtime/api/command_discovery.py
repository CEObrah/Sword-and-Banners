...
def _compact_interaction_handles(value: Any) -> list[dict[str, Any]]:
    ...
    for source in value:
        ...
        if isinstance(refs, list) and len(refs) > 8:
            row["present_person_refs"] = refs[:8]
            row["present_person_ref_count"] = len(refs)
            row["present_person_refs_truncated"] = True
        summary = source.get("summary")
        if isinstance(summary, str) and len(summary) > 200:
            row["summary"] = summary[:197] + "..."
...
def compact_play_context(context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(attempts, list) and len(attempts) > 5:
        result["recent_interaction_attempts"] = attempts[:5]
        result["recent_interaction_attempts_compact_source_count"] = len(attempts)

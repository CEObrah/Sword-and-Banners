...
from sword_runtime.release_baseline import load_release_contract
from sword_runtime.release_identity import release_fingerprints, runtime_dependencies, SOURCE_ROOT
from sword_runtime.campaign_access import isolated_public_operation
...
def _attested_play_context(
    operations: CampaignOperations,
    skill_contract_token: Optional[str],
    expected_campaign_id: Optional[str] = None,
    expected_baseline_id: Optional[str] = None,
) -> dict[str, Any]:
    return isolated_public_operation(
        operations.runtime,
        lambda: _build_attested_play_context(
            operations, skill_contract_token, expected_campaign_id, expected_baseline_id,
        ),
    )()


def _build_attested_play_context(
    operations: CampaignOperations,
    skill_contract_token: Optional[str],
    expected_campaign_id: Optional[str],
    expected_baseline_id: Optional[str],
) -> dict[str, Any]:
    """Return compact context only for the currently packaged GM Skill.

    The token is deliberately non-secret. Its only purpose is to make a stale
    installed Skill or stale cached MCP schema fail closed instead of silently
    driving a newer runtime with obsolete operating rules.
    """
    if not verify_gm_skill_contract_token(skill_contract_token):
        raise OperationError(409, "gm_skill_contract_mismatch")
    fingerprints = release_fingerprints(SOURCE_ROOT)
    if fingerprints['release_contract_sha256'] != skill_contract_token:
        raise OperationError(503, "release_contract_unsynchronized")
    dependencies = runtime_dependencies(SOURCE_ROOT)
    if dependencies['mismatches']:
        raise OperationError(503, "runtime_dependency_mismatch")
    try:
    ...
    result = compact_play_context(operations.play_context())
    try:
        release_contract = load_release_contract(operations.runtime.root)
        baseline_id = release_contract.get("release_baseline_id")
        release_campaign_id = release_contract.get("campaign_id")
        baseline_revision = release_contract.get("revision")
        baseline_world_time = release_contract.get("world_time")
    except Exception as exc:
        raise OperationError(503, "release_lineage_unavailable") from exc
    campaign = result.get("campaign") if isinstance(result.get("campaign"), Mapping) else {}
    if expected_campaign_id is not None and expected_campaign_id != campaign.get('campaign_id'):
        raise OperationError(409, 'campaign_identity_mismatch')
    if expected_baseline_id is not None and expected_baseline_id != baseline_id:
        raise OperationError(409, 'campaign_baseline_mismatch')
    if (
        not isinstance(baseline_id, str)
        or not baseline_id.strip()
        or not isinstance(release_campaign_id, str)
        or release_campaign_id != campaign.get("campaign_id")
        or not isinstance(baseline_revision, int)
        or not isinstance(baseline_world_time, str)
        or not baseline_world_time
    ):
        raise OperationError(503, "release_lineage_mismatch")
    integrity = verified_delivery_integrity()
    integrity["release_tier_rule"] = (
        "This proves the packaged GM Skill/runtime fingerprint and this process's local "
        "deployment/source compatibility. External Git freshness, Railway routing/domain, "
        "connector schema publication, and the intended campaign acceptance baseline remain "
        "separate release evidence."
    )
    integrity.update({
        "deployment_compatible": deployment.get('image_source_commit') is not None,
        "deployment_source_sync": str(deployment.get("source_sync_status") or "unknown"),
        "release_baseline_id": baseline_id,
        "release_baseline_revision": baseline_revision,
        "release_baseline_world_time": baseline_world_time,
    })
    integrity.update(fingerprints)
    integrity['runtime_dependencies'] = dependencies
    integrity.update({
        'image_source_commit': deployment.get('image_source_commit'),
        'campaign_checkout_commit': deployment.get('checkout_commit'),
        'source_tracking_commit': deployment.get('tracking_commit'),
        'deployment_id': deployment.get('deployment_id'),
        'runtime_identity_basis': 'immutable_image_source_files; local_environment_is_not_live_deployment_proof',
        'campaign_selection': 'one_configured_campaign; a_new_chat_resumes_it; no_fresh_campaign_created',
    })
    result["delivery_integrity"] = integrity
    return result
    ...
    *,
    ...
    server = MCPServer(
        ...
        instructions=(
            ...
            "knowledge. After understanding the natural-language action, preview one currently supported mechanical consequence operation when a hard write is actually required, then pass the exact returned "
            ...
            "Interpret the player's natural-language action first. Ordinary reversible scene behavior and conversation are not limited by the command catalog. When the attempted action implicates a hard consequence, select the relevant mechanic family, call get_command_family for that family, then get_command_contract for the mechanical operation before preview. Unsupported hard-state mutation fails closed; absence of a bespoke command never makes ordinary fiction impossible. Treat gm_scene_context as the primary writer workspace when present. The LLM owns reversible scene direction: established present NPCs may initiate, react, interrupt, talk to each other, and continue practical local behavior without player activation or a bespoke response command. After Tang Wei acts, normally stage the world's grounded response rather than paraphrasing the player's action. Repetition without a new human, practical, causal, or decision-relevant beat is not valid scene progression; compress or transition instead of padding."
        ),
        version="0.4.0+" + GM_SKILL_CONTRACT_TOKEN[:12],
    ...
    @server.tool(
        ...
        description="Required first live-play call. Pass the exact non-secret GM Skill compatibility token printed in the installed Skill. A missing/mismatched token fails closed so a stale Skill or cached MCP schema cannot silently drive this runtime. Re-checks local deployment/source compatibility on every authoritative read, then returns bounded player-visible state, revision, scene, permitted IDs, a scene-first GM writer workspace with active-scene direction, semantic-action guidance, delivery-integrity proof, and compact mechanic families. Mechanic families are not an action menu.",
        annotations=read_annotations,
        meta=read_security_meta,
        structured_output=True,
    )
    def get_play_context(
        skill_contract_token: Optional[str] = None,
        expected_campaign_id: Optional[str] = None,
        expected_baseline_id: Optional[str] = None,
    ) -> ReadToolOutput:
        return _tool_call(lambda: _attested_play_context(
            operations, skill_contract_token, expected_campaign_id, expected_baseline_id,
        ))
    ...
    def search_world_reference(
        ...
        try:
            result = isolated_public_operation(
                operations.runtime,
                lambda: _search_world_reference(
                    operations.store, query, category=category, offset=offset, limit=limit,
                ),
            )()
        except OperationError as exc:
            return _failure(exc)
    ...
    @server.tool(
        ...
        description=(
            "Read-only preview for one currently supported mechanical consequence operation at the exact revision. "
            "Pass the installed Skill's skill_contract_token and expected_campaign_id from fresh context. "
    ...
    def preview_command(
        ...
        try:
            ...
            if expected_campaign_id is None:
                raise OperationError(409, "campaign_identity_required")
            context = _attested_play_context(operations, skill_contract_token, expected_campaign_id)
            campaign = context["campaign"]
            if expected_revision != campaign["revision"]:
                raise OperationError(409, "stale_revision")
            contract = operations.get_command_contract(command_type)
            if not contract.get('availability', {}).get('available', False):
                raise OperationError(409, "command_currently_unavailable")

...
class CampaignOperations:
    ...
    def _information_supersession_groups(self) -> dict[str, str]:
        try:
            subject_index = self.store.read_json("state/information/subject-index.json")
        except (FileNotFoundError, ValueError):
            return {}
        groups: dict[str, list[str]] = {}
        subjects = subject_index.get("subjects", {}) if isinstance(subject_index, Mapping) else {}
        for subject_ref, refs in subjects.items() if isinstance(subjects, Mapping) else []:
            if not isinstance(subject_ref, str) or not isinstance(refs, list):
                continue
            if subject_ref.startswith(("event_", "information.")) or subject_ref.endswith(".report"):
                continue
            priority = 0 if subject_ref.startswith(("arc_", "operation_", "formation_", "state_", "house_", "char_")) else 1
            for ref in refs:
                if isinstance(ref, str):
                    groups.setdefault(ref, []).append((priority, subject_ref))
        return {ref: sorted(rows, key=lambda row: (row[0], row[1]))[0][1] for ref, rows in groups.items() if rows}

...
class CampaignOperations:
    ...
    def __init__(self, runtime: ProductionSwordRuntime) -> None:
        ...
        # Bind the final subclass implementation, not merely this base's reads.
        # Otherwise a subclass can append owners after a base snapshot unlocks.
        from sword_runtime.campaign_access import isolated_public_operation
        for name in (
            "play_context", "person_sheet", "inspect_game_object", "ooc_audit",
            "get_command_family", "get_command_contract", "list_controlled_formations",
            "list_known_information", "list_interaction_handles", "preview_command",
            "lookup_command_receipt", "execute_command",
        ):
            method = getattr(self, name, None)
            if callable(method):
                setattr(self, name, isolated_public_operation(runtime, method))

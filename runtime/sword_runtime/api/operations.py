...
class CampaignOperations:
    ...
    @staticmethod
    def _merchant_convoy_progress_node(convoy: Mapping[str, Any], world_time: str) -> str:
        ...
        if isinstance(raw, list) and len(raw) == max(0, len(path) - 1):
            edges = [max(1, int(x)) for x in raw]
        else:
            if len(path) <= 1:
                return path[0]
            departed = CampaignTime.parse(str(convoy.get("leg_departed_at") or convoy.get("departed_at")))
            total = max(1, int(departed.seconds_until(CampaignTime.parse(str(convoy.get("arrives_at")))) / 3600))
            base, rem = divmod(total, len(path) - 1)
            edges = [max(1, base + (1 if i < rem else 0)) for i in range(len(path) - 1)]
        departed = CampaignTime.parse(str(convoy.get("leg_departed_at") or convoy.get("departed_at")))
        elapsed = max(0, int(departed.seconds_until(CampaignTime.parse(world_time)) / 3600))
        total = 0
        node = path[0]
        for i, hours in enumerate(edges):
            if elapsed < total + hours:
                break
            total += hours
            node = path[i + 1]
        return node

    def _observable_merchant_convoys(
        self, *, player_location_ref: str, controlled_refs: set[str], world_time: str
    ) -> list[dict[str, Any]]:
        try:
            index = self.store.read_json("state/economy/merchant-convoys.json")
        except (FileNotFoundError, ValueError):
            return []
        routes = index.get("convoys", {}) if isinstance(index, Mapping) else {}
        active = index.get("active_refs", []) if isinstance(index, Mapping) else []
        if not isinstance(routes, Mapping) or not isinstance(active, list):
            return []
        rows: list[dict[str, Any]] = []
        # Active exact strategic convoys are deliberately sparse. This is a
        # bounded routing read, not a merchant/world scan.
        for ref in [str(x) for x in active if isinstance(x, str)][:128]:
            path = routes.get(ref)
            if not isinstance(path, str):
                continue
            convoy = self.store.read_json(path)
            if str(convoy.get("status", "")) != "in_transit":
                continue
            escort_refs = {str(x) for x in convoy.get("escort_formation_refs", []) if isinstance(x, str)}
            current = self._merchant_convoy_progress_node(convoy, world_time)
            controlled_escort = bool(escort_refs.intersection(controlled_refs))
            if current != player_location_ref and not controlled_escort:
                continue
            row = {
                "convoy_ref": ref,
                "merchant_house_ref": convoy.get("merchant_house_ref"),
                "status": convoy.get("status"),
                "current_location_ref": current,
                "destination_market_ref": convoy.get("destination_market_ref"),
                "wagon_equivalents": convoy.get("wagon_equivalents"),
                "controlled_escort_refs": sorted(escort_refs.intersection(controlled_refs)),
                "visibility_basis": "controlled_escort" if controlled_escort else "player_co_location",
            }
            if controlled_escort:
                row["cargo"] = dict(convoy.get("cargo", {})) if isinstance(convoy.get("cargo"), Mapping) else {}
            rows.append(row)
        return rows
    ...
    def play_context(self) -> dict[str, Any]:
        ...
        merchant_convoys = self._observable_merchant_convoys(
            player_location_ref=str(player.get("location") or ""),
            controlled_refs=controlled_refs,
            world_time=str(meta["time"]),
        )
        ...
        permitted_objects.update(str(row["convoy_ref"]) for row in merchant_convoys if row.get("convoy_ref"))
        ...
        return {
            ...
            "observable_merchant_convoys": merchant_convoys,
    ...
    def inspect_game_object(self, object_ref: str) -> dict[str, Any]:
        ...
        if obj.get("schema") == "sword-merchant-convoy":
            convoy_row = next((row for row in context.get("observable_merchant_convoys", []) if row.get("convoy_ref") == object_ref), None)
            if not isinstance(convoy_row, Mapping):
                raise OperationError(404, "object_not_player_visible")
            return {
                "object_ref": object_ref,
                "visibility": "controlled_escort_convoy" if convoy_row.get("visibility_basis") == "controlled_escort" else "co_located_material_convoy",
                "object": dict(convoy_row),
            }

...
def _project_equipment_state(
    ...
    mounted = any("mounted by tang wei" in state for state in _states(equipment, "horse"))
...
class EquipmentAwareCampaignOperations(StandingTrainingCampaignOperations):
    ...
    def play_context(self):
        ...
        all_equipment = self._owned_equipment_view()
        equipment = all_equipment[:64]
        player = context.setdefault("player", {})
        player["owned_equipment"] = equipment
        player["owned_equipment_count"] = len(all_equipment)
        player["owned_equipment_truncated"] = len(all_equipment) > len(equipment)

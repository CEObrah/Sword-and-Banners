...
from sword_runtime.scene_sessions import (
    active_scene_session, inspect_scene_history, recent_scene_history, scene_history_record, scene_session_projection,
)
...
def _project_active_session_presence(
    ...
    *,
    ...
    mutating the underlying session owner.  A one-on-one scene whose only other
    participant left naturally disappears from fresh context.
    """
    ...
    for ref in original:
        ...
        try:
            ...
            continue
        location = (
            person.get("current_location") or person.get("location_ref") or person.get("location")
            if isinstance(person, Mapping) else None
        )
...
class StableCampaignOperations(CampaignOperations):
    ...
    def play_context(self):
        ...
        command_types["scene_session_action"] = {
            "accepted_payload_keys": ["action", "actor_ref", "agenda", "basis_refs", "close_reason", "fact_kind", "kind", "participant_refs", "process_ref", "purpose", "resolves_thread_ref", "resolves_question_ref", "session_ref", "speaker_ref", "speech_kind", "statement", "description", "improvised_prop"],
            "input_guidance": {
    ...
    def get_command_contract(self, command_type: str) -> dict[str, Any]:
        if command_type == "interaction_action":
            ...
            record = {
                "accepted_payload_keys": ["action", "actor_ref", "agenda", "basis_refs", "close_reason", "fact_kind", "kind", "participant_refs", "process_ref", "purpose", "resolves_thread_ref", "resolves_question_ref", "session_ref", "speaker_ref", "speech_kind", "statement", "description"],
                "input_guidance": {
                    "action": {"allowed_values": sorted(SCENE_SESSION_ACTIONS)},

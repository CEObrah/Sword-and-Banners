...
class StableCampaignOperations(CampaignOperations):
    ...
    def get_command_contract(self, command_type: str) -> dict[str, Any]:
        if command_type == "interaction_action":
            ...
            record = {
                ...
                "input_guidance": {
                    ...
                    "focus_rule": "caller-supplied focuses are forbidden; saved plans remain authoritative",
                    "time_rule": "settlement advances no campaign time",
                },
                "contested_preview_policy": "deterministic_server_owned_credit_only",
            }
        elif command_type in COMMAND_TYPES and command_type not in {"repair", "scene_consequence"}:
            record = {
        ...
        available = True
        scope = "normal"
        if isinstance(wake, Mapping) and wake.get("kind") != "campaign_event":

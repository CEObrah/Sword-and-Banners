...
from sword_runtime.operation_routing import current_operational_order, iter_exact_operation_records
...
class StableCampaignOperations(CampaignOperations):
    ...
    def play_context(self):
        ...
        command_types["interaction_action"] = {
            ...
            "input_guidance": {
                ...
                "campaign_command_remote_rule": (
                    "For a remote campaign report/request to the current superior command, target the exact current campaign_command_cycle "
                    "or its returned coordination authority/process rather than the physically absent commander. Use action=request/report, "
                    "the current controlled operation as process_ref, the player's exact dispatch wording in player_statement, and expects_response=true "
                    "when a substantive answer/order is requested. The campaign-command runtime converts that persisted attempt into the physical "
                    "upward-report/courier lifecycle; do not claim the interface cannot send it merely because the named commander is remote."
                ),
    ...
    def get_command_contract(self, command_type: str) -> dict[str, Any]:
        if command_type == "gm_cognition_action":
            from sword_runtime.gm_cognition import PAYLOAD_KEYS, command_guidance
            record = {
                "accepted_payload_keys": sorted(PAYLOAD_KEYS),
                "input_guidance": command_guidance(),
                "contested_preview_policy": "validated_attributed_cognition_only",
            }
        elif command_type == "interaction_action":
            record: dict[str, Any] = {
                "accepted_payload_keys": ["action", "expects_response", "formation_refs", "player_statement", "posture", "process_ref", "scopes", "target_ref", "topic"],
                "input_guidance": {
                    ...
                    "expects_response": {"type": "boolean", "rule": "set true when the declared remote request/report actually asks for a substantive reply or follow-on order"},
                    "campaign_command_remote_rule": (
                        "For a remote campaign report/request to the current superior command, target the exact current campaign_command_cycle "
                        "or returned coordination authority/process, use the current controlled operation as process_ref, and set expects_response=true "
                        "when an answer/order is requested. This is the durable player-authored dispatch intent that the campaign-command scheduler "
                        "routes physically; direct face-to-face access to the absent commander is not required."
                    ),
                    "formation_refs": {"rule": "optional unique exact controlled formation refs"},
                    "player_statement": {"type": "string", "maximum_length": 2000, "rule": "player-authored speech only"},
                    "posture": {"type": "string", "maximum_length": 500, "rule": "player-authored posture only"},
            ...
            from sword_runtime.scene_sessions import SPEECH_KINDS, SCENE_FACT_KINDS, CONTINUITY_NOTE_KINDS, SESSION_KINDS
            from sword_runtime.api.interaction_surface import SCENE_ACTION_PAYLOAD_KEYS
            record = {
                "accepted_payload_keys": sorted(SCENE_ACTION_PAYLOAD_KEYS),
                "input_guidance": {
                    "action": {"allowed_values": sorted(SCENE_SESSION_ACTIONS)},
                    "speech_kind": {"allowed_values": sorted(SPEECH_KINDS)},
                    "fact_kind": {"allowed_values": sorted(SCENE_FACT_KINDS)},
                    "continuity_kind": {"allowed_values": sorted(CONTINUITY_NOTE_KINDS)},
                    "kind": {"allowed_values": sorted(SESSION_KINDS)},
    ...
    def inspect_game_object(self, object_ref: str) -> dict[str, Any]:
        from sword_runtime.gm_cognition import cognition_history
        prior_cognition = cognition_history(self.store, object_ref)
        if prior_cognition is not None:
            # Reuse the exact person's current visibility boundary. A private
            # historical interpretation never becomes player knowledge.
            self.person_sheet(prior_cognition['subject_ref'])
            return {
                'object_ref': object_ref,
                'privacy': 'gm_private_cognition_not_player_knowledge',
                'object': prior_cognition,
            }
    ...
    def lookup_command_receipt(self, command: CommandEnvelope) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            # Releases before public-envelope receipts stored these two pure
            # translations. Recover those exact bytes without re-running scene
            # presence or thread-open checks changed by the successful action.
            if command.command_type == "scene_session_action":
                legacy = translate_scene_action_command(command)
            elif command.command_type == "interaction_action":
                legacy = translate_interaction_command(command)
            else:
                raise
            receipt = super().lookup_command_receipt(legacy)
    ...
    def execute_command(self, command):
        self._authorize_public_command(command)
        existing = self.lookup_command_receipt(command)
        ...
        translated = self._translate_surface_command(command)
        try:
            receipt = _receipt_record(self.runtime.execute(translated, receipt_command=command))
            if command.command_type in {"interaction_action", "scene_session_action"}:
                receipt["surface_command_type"] = command.command_type
            return receipt
        except HighSalienceWakeRequired as exc:

...
from sword_runtime.scene_sessions import (
    INTERACTION_CLOSED_HISTORY_LIMIT, SESSION_KINDS, SPEECH_KINDS, active_scene_session, attach_open_question,
    close_active_scene, record_attributed_speech, start_scene_session, touch_active_scene,
)
...
INTERACTION_PAYLOAD_KEYS = frozenset({
    ...
    "formation_refs", "posture", "topic", "scopes",
})
...
SCENE_SESSION_ACTIONS = frozenset({"open", "close", "record_speech"})
SCENE_ACTION_PAYLOAD_KEYS = frozenset({
    ...
    "basis_refs", "resolves_question_ref",
})
...
def _normalized_attempt_row(raw: Mapping[str, Any]) -> dict[str, Any]:
    ...
    is_question = str(row.get("action")) == "ask" and isinstance(row.get("player_statement"), str) and bool(row.get("player_statement"))
    belongs_to_scene = isinstance(row.get("scene_session_ref"), str) and bool(row.get("scene_session_ref"))
    row.setdefault("thread_status", "open" if is_question and belongs_to_scene else "not_applicable")
...
def record_interaction_attempt(store, summary: object, *, at: str) -> str | None:
    ...
    is_question = str(attempt.get("action")) == "ask" and isinstance(attempt.get("player_statement"), str) and bool(attempt.get("player_statement"))
    rows.append({
        "event_id": ref, "at": at, **persisted_attempt,
        "scene_session_ref": session_ref,
        "thread_status": "open" if is_question and session_ref is not None else "not_applicable",
        "resolved_at": None,
        "response_ref": None,
    })
    ...
    if is_question and session_ref is not None:
        attach_open_question(store, ref, at=at)
    return ref
...
def mark_interaction_question_resolved(store: Any, question_ref: str, *, at: str, response_ref: str) -> bool:
    ...
    for raw in raw_rows:
        ...
        if str(row.get("event_id")) == question_ref and row.get("thread_status") == "open":
...
def apply_scene_action_record(store: Any, summary: object, *, at: str) -> dict[str, Any] | None:
    ...
    question_ref = speech.get("resolves_question_ref")
    if isinstance(question_ref, str):
        mark_interaction_question_resolved(store, question_ref, at=at, response_ref=str(speech.get("speech_ref")))
...
def fresh_runtime_projection(
    ...
    active_questions = [
        ...
        and item.get("thread_status", "open") == "open"
...
__all__ = [
    ...
    "parse_interaction_attempt_summary", "parse_scene_action_summary", "recent_interaction_attempts",

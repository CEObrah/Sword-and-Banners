...
from sword_runtime.scene_sessions import (
    INTERACTION_CLOSED_HISTORY_LIMIT, CONTINUITY_NOTE_KINDS, SCENE_FACT_KINDS, SESSION_KINDS, SPEECH_KINDS, active_scene_session, attach_open_thread,
    close_active_scene, record_attributed_speech, record_continuity_note, record_scene_fact, start_scene_session, touch_active_scene,
)
...
SCENE_SESSION_ACTIONS = frozenset({"open", "close", "record_speech", "record_fact", "record_continuity"})
SCENE_ACTION_PAYLOAD_KEYS = frozenset({
    ...
    "fact_kind", "description", "actor_ref", "improvised_prop", "continuity_kind", "subject_refs",
})
...
SCENE_LOCAL_NARRATION_CONTRACT = {
    ...
    "llm_owns_moment_to_moment_scene_direction": True,
    "llm_owns_narrative_scene_lifecycle": True,
    "formal_scene_session_is_optional_continuity_tool": True,
    "runtime_command_completion_is_not_scene_completion": True,
    "present_people_are_active_agents": True,
    "npc_initiative_requires_player_activation": False,
    "player_action_should_receive_world_response_before_recap": True,
    "repetition_without_new_human_practical_or_causal_information_is_not_progress": True,
    "continue_resumes_next_grounded_scene_beat": True,
    "multi_beat_scene_flow_allowed_until_real_handoff": True,
    "do_not_end_every_beat_with_a_player_prompt": True,
    "compression_preferred_over_filler": True,
    "spent_scene_should_transition_or_compress_not_loop": True,
    "contested_physical_actions_remain_runtime_owned": True,
    "recent_runtime_events_are_evidence_not_mandatory_recap": True,
    "subjective_scene_latitude": (
        "The GM may author momentary nonbinding reactions, tone, hesitation, humor, irritation, warmth, silence, ordinary opinions, "
        "cross-talk, interruptions, and practical local behavior when consistent with established role, relationship, prior behavior, current cognition, and pressure. "
        "These are scene performance, not hidden factual motives or durable state."
    ),
    "anti_inertia_rule": (
        "When exact present people or an obvious reversible process can naturally carry the scene, the LLM should normally stage that next beat instead of waiting for Tang Wei to activate everyone one at a time. "
        "If no meaningful beat exists, compress or transition; never invent a hard consequence merely to create motion."
    ),
}
...
def validate_scene_action_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if action == "record_continuity":
        continuity_kind = payload.get("continuity_kind")
        if not isinstance(continuity_kind, str) or continuity_kind not in CONTINUITY_NOTE_KINDS:
            raise ValueError("scene_session_action continuity_kind is unsupported")
        description = _optional_text(payload.get("description"), "description", 1500)
        if description is None:
            raise ValueError("scene_session_action description is required")
        subject_values = payload.get("subject_refs", [])
        if not isinstance(subject_values, Sequence) or isinstance(subject_values, (str, bytes, bytearray)) or len(subject_values) > 16:
            raise ValueError("scene_session_action subject_refs is invalid")
        subject_refs = [_require_safe_ref(ref, "subject_refs") for ref in subject_values]
        if len(set(subject_refs)) != len(subject_refs):
            raise ValueError("scene_session_action subject_refs must be unique")
        basis_values = payload.get("basis_refs", [])
        if not isinstance(basis_values, Sequence) or isinstance(basis_values, (str, bytes, bytearray)) or not basis_values or len(basis_values) > 16:
            raise ValueError("scene_session_action continuity basis_refs is invalid")
        basis_refs = [_require_safe_ref(ref, "basis_refs") for ref in basis_values]
        if len(set(basis_refs)) != len(basis_refs):
            raise ValueError("scene_session_action basis_refs must be unique")
        return {
            "action": action, "session_ref": session_ref, "continuity_kind": continuity_kind,
            "description": description, "subject_refs": subject_refs, "basis_refs": basis_refs,
        }
...
def record_interaction_attempt(store, summary: object, *, at: str) -> str | None:
    ...
    # Preserve the physical origin of the attempt for any later remote routing.
    # The attempt digest remains player-authored intent; origin_location_ref is
    # runtime observation used only by delivery/response mechanics.  Reading the
    # player's location later would let a message silently teleport when Wei moves.
    player_at_attempt = _read_optional_json(store, "state/player.json")
    if isinstance(player_at_attempt, Mapping):
        origin_location_ref = (
            player_at_attempt.get("location")
            or player_at_attempt.get("current_location")
            or player_at_attempt.get("location_ref")
        )
        if isinstance(origin_location_ref, str) and origin_location_ref:
            persisted_attempt["origin_location_ref"] = origin_location_ref
...
def apply_scene_action_record(store: Any, summary: object, *, at: str) -> dict[str, Any] | None:
    ...
    if action == "open":
        ...
        actor_ref = record.get("actor_id")
        if isinstance(actor_ref, str) and actor_ref and actor_ref not in participant_refs:
            participant_refs.insert(0, actor_ref)
        session = start_scene_session(
            store,
    ...
    if action == "record_continuity":
        note = record_continuity_note(
            ...
            at=at,
            summary=str(record.get("description")),
            continuity_kind=str(record.get("continuity_kind")),
            session_ref=str(record.get("session_ref")),
            subject_refs=[str(x) for x in record.get("subject_refs", []) if isinstance(x, str)],
            basis_refs=[str(x) for x in record.get("basis_refs", []) if isinstance(x, str)],
        )
        return {
            "record_kind": "derived_narrative_continuity",
            "continuity_ref": note.get("continuity_ref"),
            "session_ref": note.get("session_ref"),
            "mechanical_consequence_authority": False,
        }

...
_INNER_WALLS_COMMAND_REF = "cmdgrp.house_tang.inner_walls"
...
class HouseholdAwareCampaignOperations(StableCampaignOperations):
    ...
    def _retinue_projection(self) -> tuple[list[dict[str, Any]], set[str], set[str]]:
        ...
        for doc in docs:
            ...
            exact_people = []
            for value in [doc.get("commander_ref"), *doc.get("direct_person_refs", [])]:
    ...
    def _generic_scene_people(self, context: Mapping[str, Any], player_location: object) -> list[dict[str, Any]]:
        ...
        Tang/Inner Walls command routing rather than a global person scan.
        """
        ...
        def add(ref: object, basis: str) -> None:
            ...
            if not is_person and path.startswith("state/person/"):
                ...
                is_person = isinstance(record, Mapping) and str(record.get("schema", "")) in {"sab_character", "person-lite", "sword-materialized-person"}
            if is_person:
        ...
        # Top commanders of formations currently controlled by Wei are lawful
        ...
        for row in context.get("controlled_formations", []) if isinstance(context.get("controlled_formations"), list) else []:
            ...
            if isinstance(formation, Mapping):
                ...
                for person_ref in formation.get("embedded_person_refs", []) if isinstance(formation.get("embedded_person_refs"), list) else []:
                    add(person_ref, "controlled_formation_command_staff")

        # House Tang/Inner Walls high command is a bounded explicit hierarchy.
        ...
        allowed_authorities = {"house_tang", "char_tang_wei", "pforce.tang_wei", "force_tang_wei_personal"}
    ...
    def _direct_family_scene_people(self, player_id: str, player_location: object) -> list[dict[str, Any]]:
        ...
        if isinstance(parentage_index, Mapping):
            parentage_refs = {str(ref) for ref in links.get("parentage", []) if isinstance(ref, str)}
            # person_index is routing-only.  Recover direct family from the
            # bounded exact parentage registry so a dropped reverse pointer
            # cannot erase a co-located parent or child from player context.
            parentage_refs.update(str(ref) for ref in parentage_index if isinstance(ref, str) and ref)
            for parentage_ref in sorted(parentage_refs):
                resolved = exact_family_record(self._read_optional_mapping, family, "parentage", parentage_ref)
                record = resolved[1] if resolved is not None else None
                ...
                if child_id == player_id and isinstance(parent_links, list):
                    for link in parent_links:
                        ...
                        if isinstance(parent_id, str) and person_owner_path(self.store, parent_id) is not None:
                            relation_role = link.get("relation_role")
                            relations.setdefault(parent_id, str(relation_role) if isinstance(relation_role, str) and relation_role else "parent")
        ...
        if isinstance(kinship_index, Mapping):
            kinship_refs = {str(ref) for ref in links.get("kinships", []) if isinstance(ref, str)}
            kinship_refs.update(str(ref) for ref in kinship_index if isinstance(ref, str) and ref)
            for kinship_ref in sorted(kinship_refs):
                resolved = exact_family_record(self._read_optional_mapping, family, "kinships", kinship_ref)
                record = resolved[1] if resolved is not None else None
    ...
    def play_context(self):
        ...
        if family_people:
            ...
            # Inner Walls is an exact House Tang command owner. While Wei
            ...
            if isinstance(player_location, str) and player_location.startswith("loc_tang_manor_"):
                owner_index = self._read_optional_mapping("state/index/owner-index.json")
                owners = owner_index.get("owners", {}) if isinstance(owner_index, Mapping) else {}
                if isinstance(owners, Mapping) and isinstance(owners.get(_INNER_WALLS_COMMAND_REF), str):
                    permitted_objects = set(context.get("permitted_object_refs", []))
                    permitted_objects.add(_INNER_WALLS_COMMAND_REF)
                    context["permitted_object_refs"] = sorted(permitted_objects)
                    scene["household_military_forces"] = [{
                        "object_ref": _INNER_WALLS_COMMAND_REF,
                        "name": "Inner Walls",
        ...
        if isinstance(scene, dict) and isinstance(present_people, list) and any(
            ...
            existing_contract = scene.get("scene_local_narration_contract")
            narration_contract = dict(existing_contract) if isinstance(existing_contract, Mapping) else {
            ...
            narration_contract.update({
                ...
                "household_scene_direction_rule": (
                    "Family and household participants who are exactly present remain active people. The LLM may let them speak, react, continue ordinary household activity, "
                    "or address each other from established context without waiting for Tang Wei to activate them; this never creates a hard relationship, resource, access, or commitment outcome."
                ),
            })
            scene["scene_local_narration_contract"] = narration_contract

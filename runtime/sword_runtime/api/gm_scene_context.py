...
from sword_runtime.campaign_report_projection import select_pending_upward_reports
    ...
    if isinstance(controlled_ops, list):
        ...
        for op in controlled_ops:
            if not isinstance(op, Mapping):
                continue
            command = op.get("campaign_command")
            if not isinstance(command, Mapping):
                continue
            pending_sources = command.get("pending_upward_reports")
            if not isinstance(pending_sources, list):
                pending_sources = command.get("upward_reports", [])
            selected_pending = select_pending_upward_reports(pending_sources, maximum=4)
            pending_reports = [
                _pick(row, (
                    "report_ref", "event_ref", "phase", "prepared_at", "from_ref", "to_ref",
                    "order_ref", "directive_ref", "information_refs", "follow_on_request_refs",
                    "delivery_status", "delivery_due_at", "source_location_ref",
                    "target_location_ref", "communication_travel_seconds",
                ))
                for row in selected_pending
            ]
            if not pending_reports:
                continue
            handoff = command.get("remote_command_handoff")
            response_requested = any(bool(row.get("follow_on_request_refs")) for row in pending_reports)
            in_flight = any(str(row.get("delivery_status", "")) == "in_transit" for row in pending_reports)
            route_unresolved = any(str(row.get("delivery_status", "")) == "route_unresolved" for row in pending_reports)
            rule = handoff.get("rule") if isinstance(handoff, Mapping) else None
            if not isinstance(rule, str) or not rule:
                if in_flight:
                    rule = (
                        ...
                        "and do not resend the same request merely to create progress. Dispatch is not receipt or reply; continue the standing "
                        "objective through lawful local preparation or chronology until delivery or another real interruption."
                    )
                else:
                    rule = (
                        ...
                        "Treat this as unresolved physical routing and preserve the standing request until routing is established."
                    )
            remote_command_handoffs.append({
                "operation_ref": op.get("operation_ref"),
                "cycle_ref": command.get("cycle_ref"),
                "superior_command_ref": command.get("superior_command_ref"),
                "outbound_report_in_flight": in_flight,
                "route_unresolved": route_unresolved,
                "response_requested": response_requested,
                "pending_upward_reports": pending_reports,
                "rule": rule,
            })
            if len(remote_command_handoffs) >= 4:
                break
    return {
        ...
        "hard_constraints": {
            ...
            "remote_command_handoffs": remote_command_handoffs,
        },
        "deep_reads": {

...
def install_extended_tools(server, operations, oauth) -> None:
    ...
    @server.tool(
        ...
        title="Get one mechanic family",
        description="Demand-load exact mechanical operation names inside one mechanic family only after natural-language intent has been understood and a hard consequence requires adjudication. This is not a fictional-action whitelist.",
    ...
    @server.tool(
        ...
        description="Read the exact payload contract and guidance for one mechanical consequence operation returned by the selected mechanic family. Do not load it for ordinary reversible scene behavior.",
        annotations=read_annotations,
        meta=read_security_meta,
        structured_output=True,
    )
    ...
    def get_house_readiness() -> ReadToolOutput:
        ...
        return _tool_call(isolated_public_operation(operations.runtime, lambda: house_readiness_snapshot(operations)))

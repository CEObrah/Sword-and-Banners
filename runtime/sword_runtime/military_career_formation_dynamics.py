...
class MilitaryCareerFormationDynamicsMixin(MilitaryCareerCommandSurfaceMixin):
    ...
    def _formation_patron_ref(self, formation: Mapping[str, Any]) -> str | None:
        ...
        if isinstance(admin, str):
            try:
                _path, admin_person = self._command_person(admin, active=False)
            except (KeyError, ValueError, FileNotFoundError):
                admin_person = None
        if isinstance(admin, str) and isinstance(admin_person, Mapping):
            family = admin_person.get("family")
        ...
        if isinstance(force_ref, str):
            ...
            if isinstance(force, Mapping):
                ...
                if isinstance(force_admin, str) and force_admin.startswith("house_"):
                    return force_admin
                if isinstance(force_admin, str):
                    try:
                        self._command_person(force_admin, active=False)
                    except (KeyError, ValueError, FileNotFoundError):
                        pass
                    else:
                        return force_admin
    ...
    def _patron_leader_ref(self, patron_ref: str | None) -> str | None:
        if not patron_ref:
            return None
        try:
            self._command_person(patron_ref, active=False)
        except (KeyError, ValueError, FileNotFoundError):
            pass
        else:
            return patron_ref
...
__all__ = ["MilitaryCareerFormationDynamicsMixin"]

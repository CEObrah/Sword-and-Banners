...
class ActivityCampaignEventPlanner(CampaignEventPlayerGroupActionPlanner):
    ...
    def _derived_activity_contract(cls, person: Mapping[str, Any]) -> dict[str, Any] | None:
        ...
        if schema == "person-lite":
            role_text = " ".join(str(person.get(k, "")) for k in ("role", "rank")).strip()
        else:
    ...
    def _ensure_activity_routes(self) -> None:
        ...
        person_lite_life_refs: list[str] = []
        ...
        if isinstance(record_index, Mapping):
            for person_ref in sorted(str(ref) for ref in record_index):
                ...
                if life not in {"dead", "deceased", "destroyed"}:
                    person_lite_life_refs.append(person_ref)
                contract = self._effective_activity_contract(person)
                focuses = self._activity_focuses(person, contract) if isinstance(contract, Mapping) else []
        ...
        # Non-force person-lite staff are persistent people. Their activity host
        # owns development, while a separate annual life host owns mortality.
        # Force-owned person-lite bodies stay on the cohort life path so one body
        # never receives two mortality clocks.
        for person_ref in sorted(set(person_lite_life_refs)):
    ...
    def _autonomy_person(self, host: Mapping[str, Any], occurrences: int, at: str) -> None:
        ...
        guarded_contract["annual_training_fallback_enabled"] = False

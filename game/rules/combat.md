...
Attack Control, Dodge Control, Parry Control, Block Control, handling adjustment, combined pressure, contact grades, and contact multipliers are owned by `game/data/mechanics/combat.json`.
...
Only attackers with a real lane, timing window, weapon line, and target access contribute. The combined-pressure formula is registered in `game/data/mechanics/combat.json`. Crowding may prevent later attackers from contributing at all.

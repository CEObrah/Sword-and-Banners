...
`game/data/mechanics/core.json` owns resolution order and deterministic RNG. `game/data/mechanics/body.json` owns body geometry. `game/data/mechanics/combat.json` owns personal, ranged, mounted, fatigue, recovery, handling, contact, wear, and heroic numerical formulas. `game/data/mechanics/injury.json` owns protection, wound, bleeding, shock, and structural-injury calculations.
...
Permanent attributes and skills are not copied into temporary combat values. Current readiness, fatigue, relevant injury impairment, body load, equipment condition, stance, and legal position modify execution through `game/data/mechanics/combat.json`.
...
Equipment carriage balance, heat, terrain, injury, and load use the factors in `game/data/mechanics/combat.json` only when physically relevant.

## Geometry, reach, closing, and hand occupancy

Current height and mass come from body state. Weapon reach comes from the item profile plus the registered body adjustment. Reach affects whether a threat can contact and the control advantage before contact; it never multiplies damage. Reach control is contextual: a longer weapon gains its strongest advantage while it alone can threaten, but that advantage cannot persist after an opponent legally closes inside the longer weapon's registered minimum range. If only the shorter weapon can legally threaten at that gap, the one-sided reach advantage reverses.
...
Attack Control, Dodge Control, Parry Control, Block Control, handling adjustment, combined pressure, contact grades, and contact multipliers are owned by `game/data/mechanics/combat.json`. Runtime combat code must consume those registered laws rather than maintain private copies of the same formulas. When several melee weapons are already lawfully readied and no explicit player choice selects one, default weapon selection considers both the actor's relevant proficiency and the weapon's physical suitability; item reach or force alone cannot override a large skill disparity.
...
Only attackers with a real lane, timing window, weapon line, and target access contribute. The combined-pressure formula is registered in `game/data/mechanics/combat.json`. Crowding may prevent later attackers from contributing at all. A blocked linear attack is resolved as blocked **before** the screened defender spends dodge/parry/block bandwidth; an attack that cannot physically reach that defender cannot create phantom defensive saturation. The same body-lane rule applies to both sides.

A group does not gain pressure from bodies that cannot physically enter the contact. When a fresh exact-person encounter supplies no local participant geometry, neutral fallback staging begins as opposed fronts with numerical surplus expressed through frontage/depth. Encirclement, flank access, and rear contact must come from supplied scene geometry or be earned through movement rather than being granted to one side by the fallback initializer.

Temporary tactical role assignment never turns a profession into a permanent combat posture. A fit present fighter who also has Medicine, healing, logistics, signal, or another support capability remains an active fighter unless an explicit order or an actual casualty/treatment/extraction task lawfully commits that person elsewhere.

Autonomous anatomical targeting is deterministic for replay but not a global body-part macro. Registered targeting priorities are preferences. Actor identity, target state, attack geometry, armor/exposure, observed defenses, and recent failed/denied lines may change which comparable opening is selected. A shared doctrine alone must not make every fighter independently choose the same exact knee, wrist, or other structure.
...
Wounds use `game/data/mechanics/injury.json`. Contact severity, substructure, bleeding, pain, blood loss, shock, functional impairment, consciousness, treatment, infection, and recovery remain separate state.
...
Range bands, draw completion, retained force, ranged control, projectile impact, cycle factors, movement factors, ammunition access, and recovery are owned by `game/data/mechanics/combat.json` and the item catalog.
...
Mounted control, horse load ratio, effective speed, charge alignment, panic control, horse fatigue, and collision index are owned by `game/data/mechanics/combat.json`.
...
## Continuous personal-combat time

Personal combat has no alternating-turn entitlement and no universal per-character turn duration. Each actor carries an independent action-ready clock. Movement completion, attack startup/contact, defensive reaction, weapon recovery, repositioning, and other physical actions consume their own registered or stat-derived durations. The next causal event is whichever action becomes physically resolvable first; one actor may therefore complete multiple legal actions while another remains committed or recovering.

A declared defense is a contingent reaction, not a free turn. It succeeds at full control only when the defender can perceive and physically become defense-ready before contact. Aborting another committed action to defend consumes readiness and recovery time. Initiative affects local initial timing only and never creates a permanent order.

Exact traces preserve sub-second timing in canonical milliseconds and may expose narration-friendly fractional-second projections. Campaign world time remains whole-second authority and rounds an exact combat slice upward rather than truncating it.

## Heroic personal action in mass battle

Exceptional individuals remain bound by the same body, timing, fatigue, contact, injury, weapon, and lane rules. Heroic tempo and minimum action interval are registered in `game/data/mechanics/combat.json`. Character and cohort combat stats may exceed 200; 200 is a reference value, not a combat ceiling. Values above 200 continue to affect control, tempo, cohort capability, and other formulas unless a separate physical constraint such as geometry, frontage, recovery floor, equipment, fatigue, or terrain limits expression.
...
## Cross-scale fairness acceptance

Combat fairness is certified at the representation scale that actually owns the fight. Exact-person regressions cover local geometry, body blocking, action-ready timing, defense load, medical/professional role neutrality, and anatomy target choice. They do not stand in for army-scale verification.

Formation and battlefield regressions separately cover thousands to hundreds of thousands of represented soldiers. At that scale, only lawful frontage and bounded depth support may express melee pressure; a coarse large formation must be distributed across operational sectors rather than duplicated into every local contact; support expertise remains a temporary duty rather than a permanent non-fighting troop caste; and long-weapon reach advantage must decay as cohesion and contact geometry compress. Aggregate battle resolution must remain bounded and must never materialize anonymous soldiers into synchronized exact-person target scripts.

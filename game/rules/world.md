...
This file owns universal arc, pressure, event, review, propagation, successor, and hot-projection rules. Current arc facts live in their durable current-save owners. `state/index/owner-index.json` and the active-scene dependency set are generated projections and never override typed owners.
...
`state/scene.json` identifies the current dependency boundary, while `state/index/owner-index.json` routes to every non-null active layer required by the current campaign, including immediate, legal or financial, personal force, career, equipment, regional, and global layers when present.
...
Every active arc, pressure, operation, message, movement, institution, House, faction, population process, or state process that can change while cold is attached to an explicit causal host or exact scheduled owner. `state/runtime.json` owns the bounded due-event queue, host `resolved_through`, host `safe_through`, and the next known successor boundary. `state/index/owner-index.json` resolves mutable owners directly.
...
Current people are represented only when the world requires them: exact characters, individual-lite people, or aggregate population/unit/institution members. Anonymous institutional staffing remains aggregate until an individual becomes causally relevant; functional continuity does not create a hidden pseudo-person or private biography.
...
Static identity-catalog names have no scheduler entries. Aggregate institutional staffing settles only through a real institution/population process that owns the relevant mechanic. Exact personal clocks exist only when exact personal causality requires them.

...
`game/data/mechanics/settlement.json` is the structured numerical authority for settlement balances, production, labor progress, disease exposure, recruitment availability, and levy burden.
...
Food balance is calculated by `game/data/mechanics/settlement.json`.

Recruitment availability is calculated by `game/data/mechanics/settlement.json`.

This is an upper bound, not willingness. `game/rules/careers.md` resolves actual enlistment.
...
Cycle output is calculated by `game/data/mechanics/settlement.json`.
...
Physical facility geometry, construction-material demand, labor classes, capacity, and construction time are defined by `game/data/mechanics/infrastructure-blueprints.json` plus `game/data/mechanics/construction-physics.json`. Construction also consumes finite land through `state/development/land.json`. There is no second settlement-specific building-cost table.
...
Daily progress labor is calculated by `game/data/mechanics/settlement.json`.
...
Estate net cash is calculated by `game/data/mechanics/settlement.json`.
...
Production runtime materializes outbreak state only when disease becomes materially relevant. One outbreak record owns aggregate incubating, symptomatic, recovered, dead, exposed-susceptible, quarantine, review, and propagation state for ordinary civilians while the regional population document remains the sole body authority. Deaths debit both the exact local demographic partition and its parent population strata. Propagation requires an actual strategic route and a represented destination population; route adjacency never creates people or a settlement. Named-person illness remains an exact person-health consequence rather than being silently inferred from an aggregate outbreak.

Local justice uses the same selective rule. A material local allegation may create one exact local case record with jurisdiction, allegation kind, subject if actually known, evidence refs if they exist, severity, disposition, and escalation link. Registration does not establish guilt or invent an accused, witness, evidence, sentence, or sovereign judgment. Formal sovereign adjudication continues through the court-case system in `game/rules/politics/authority-law.md`.
...
Levy total burden is calculated by `game/data/mechanics/settlement.json`.
